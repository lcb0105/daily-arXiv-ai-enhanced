...
TL;DR: 该研究系统评估了四种提示策略在图表问答任务中的表现，发现Few-Shot Chain-of-Thought在推理密集型问题上表现最佳，Few-Shot在格式遵循上更好，而Zero-Shot仅在高能力模型处理简单任务时有效。


<details>
  <summary>Details</summary>
Motivation: 虽然提示策略对LLM推理性能有影响，但在基于图表的问答任务中，不同提示策略的作用尚未得到充分探索。研究者希望系统评估不同提示范式在结构化图表数据上的表现，为实际应用提供指导。

Method: 使用四种广泛使用的提示范式（Zero-Shot、Few-Shot、Zero-Shot Chain-of-Thought、Few-Shot Chain-of-Thought），在GPT-3.5、GPT-4和GPT-4o模型上对ChartQA数据集进行系统评估。框架仅基于结构化图表数据，将提示结构作为唯一实验变量，使用准确率和精确匹配两个指标进行评估。

Result: 在1,200个多样化的ChartQA样本上，Few-Shot Chain-of-Thought提示始终获得最高准确率（最高达78.2%），特别是在推理密集型问题上表现突出。Few-Shot提示在格式遵循方面表现更好。Zero-Shot提示仅在高能力模型处理简单任务时表现良好。

Conclusion: 研究结果为结构化数据推理任务中选择提示策略提供了实用指导：Few-Shot Chain-of-Thought适合推理密集型问题以获得最高准确率，Few-Shot适合需要严格格式遵循的任务，而Zero-Shot仅在高能力模型处理简单任务时有效。这些发现对实际应用中的效率和准确性都有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习知识追踪模型准确率高但缺乏可解释性，而大型语言模型虽有强推理能力但存在上下文窗口限制和幻觉问题，且现有基于LLM的方法通常需要昂贵的微调，限制了可扩展性和对新数据的适应性。

Method: 提出MERIT框架：1) 将原始交互日志转换为可解释记忆库；2) 使用语义去噪将学生分类到潜在认知模式；3) 构建范式库，离线分析代表性错误模式以生成显式思维链推理；4) 推理时使用分层路由机制检索相关上下文；5) 逻辑增强模块应用语义约束校准预测。
...
TL;DR: 研究发现，对于低资源语言，使用少量噪声合成数据进行微调（仅1万对）就能获得与百万级数据相当的语义嵌入性能，挑战了传统需要大规模高质量数据的假设。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏高质量大规模数据集来训练有效的文本嵌入模型，这限制了它们在检索增强生成和语义搜索等任务中的应用。传统方法假设需要大量数据或完美的人工翻译，本研究旨在挑战这一假设。

Method: 针对亚美尼亚语（具有独特文字的低资源语言），采用成本效益高的适应策略：使用开源模型将英语Reddit标题-正文对翻译成亚美尼亚语，生成小规模噪声合成数据。建立包含现有数据集、翻译数据和手动标注数据的综合评估基准。

Result: 实验发现"少即是多"现象：在多语言编码器mE5上仅用1万对噪声合成数据微调，就在基准测试中获得11-12%的平均改进，检索性能相对提升20%+，与训练约100万示例的模型性能相当。增加数据规模、使用最先进LLM提高翻译质量或多样化数据域均未带来显著增益。

Conclusion: 低资源语言的语义对齐早期就达到饱和，对噪声具有高度鲁棒性。这一发现为资源受限社区民主化创建高性能嵌入提供了可能，挑战了传统需要大规模高质量数据的假设。
...
TL;DR: 该论文提出了LEAF评估框架，用于全面评估大语言模型在性健康和生殖健康等敏感领域的表现，包括准确性、语言、可用性和安全性等多个维度，并在尼泊尔语环境中进行了大规模评估。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在日常生活中的应用增多，用户越来越多地将其用于性健康和生殖健康等个人敏感查询。然而，现有的评估方法主要关注准确性，缺乏对可用性和安全性的评估标准，特别是在低资源语言和文化敏感领域。

Method: 提出了LEAF评估框架，从多个维度进行评估：准确性、语言、可用性差距（包括相关性、充分性和文化适宜性）以及安全性差距（安全性、敏感性和保密性）。使用该框架评估了来自9000多名用户的14000个尼泊尔语SRH查询，由SRH专家手动标注。

Result: 评估结果显示，只有35.1%的回应是"适当的"，即准确、充分且没有重大可用性或安全性问题。研究还发现不同版本的ChatGPT在准确性方面相似，但在可用性和安全性方面存在差异。
...
TL;DR: TIPS框架通过基于教师模型正确答案概率的密集、回合级奖励，解决了搜索增强LLM在强化学习训练中的稀疏奖励和信用分配问题，显著提升了训练稳定性和问答性能。


<details>
  <summary>Details</summary>
Motivation: 搜索增强的大型语言模型在强化学习训练中存在优化不稳定问题，主要原因是稀疏奖励和跨推理与工具调用的信用分配困难。现有方法主要依赖结果级优化，缺乏细粒度的训练指导。

Method: 提出了Turn-Level Information Potential Reward Shaping (TIPS)框架，基于潜在奖励塑造理论，为每个推理+工具调用段分配密集的回合级奖励。奖励基于教师模型下正确答案概率的增加程度，提供细粒度且策略不变的指导。

Result: 在七个QA基准测试中，TIPS一致优于GRPO/PPO基线方法，显著提高了训练稳定性。使用Qwen-2.5 7B Instruct模型时，TIPS将平均Exact Match分数提高了11.8%，F1分数提高了13.6%（相对于PPO）。

Conclusion: 回合级信息潜在奖励塑造为多轮LLM推理的稀疏奖励信用分配提供了有效且通用的解决方案，通过细粒度的训练指导克服了纯结果优化的局限性。
...
TL;DR: 该研究首次使用临床心理学方法测试LLM中的情感处理机制，发现情感接收（检测情感内容）与情感分类（映射到具体情感标签）是两个可分离的机制，前者不依赖关键词且准确率近乎完美，后者部分依赖关键词。


<details>
  <summary>Details</summary>
Motivation: 现有研究声称大语言模型具有情感处理能力，但都使用显式情感关键词作为刺激，无法确定模型是真正理解情感含义还是仅仅识别关键词。本研究旨在通过临床心理学方法验证这些声称，使用不含情感关键词的临床情景描述来测试模型的情感处理机制。

Method: 使用临床心理学中的临床情景描述作为刺激材料，完全去除情感关键词，仅通过情境和行为线索引发情感。在六个模型（Llama-3.2-1B、Llama-3-8B、Gemma-2-9B的基础版和指导版）上应用四种机制解释性方法：线性探测、因果激活修补、敲除实验和表征几何分析。

Result: 发现两个可分离的情感处理机制：1）情感接收（检测情感显著内容）具有近乎完美的准确率（AUROC 1.000），在所有六个模型中一致存在，不依赖关键词；2）情感分类（将情感映射到具体标签）部分依赖关键词，去除关键词后准确率下降1-7%，随模型规模增大而改善。因果激活修补证实关键词丰富和关键词缺乏的刺激共享表征空间。

Conclusion: 研究结果否定了"关键词识别"假说，建立了新的机制分离，并引入临床刺激方法作为测试大语言模型情感处理声称的严格标准。这对AI安全评估和对齐具有直接意义。所有刺激材料、代码和数据都已公开用于复现。
...
TL;DR: RLVR微调在LLMs中仅引起稀疏的token级分布变化，少量关键token决策直接决定性能提升


<details>
  <summary>Details</summary>
Motivation: 尽管RLVR显著提升了LLMs的推理能力，但其token级工作机制尚不清楚，需要系统研究RLVR引起的分布变化及其对推理性能的影响

Method: 采用三种分析方法：1) 基础模型与RL模型token级分布变化的特征化；2) 通过交叉采样实验研究token级分布变化对序列级推理性能的影响；3) token级变化的细粒度机制分析，包括token熵、位置集中度和概率质量重分配

Result: RL微调仅引起稀疏的、目标明确的分布变化，只有小部分token分布出现显著差异；交叉采样实验表明，仅插入少量RL采样token就能恢复RL性能增益，而注入少量基础token选择则使性能降至基础水平；优势信号的发散加权变体可作为诊断干预手段

Conclusion: RLVR微调是一种目标明确的精炼过程，通过少量关键token级决策实现性能提升，研究结果为理解RLVR的分布变化提供了细粒度的token级视角
...
TL;DR: 提出一个基于LLM的新闻标题重写框架，通过FUDGE范式控制生成过程，在保持语义忠实度的同时增强读者参与度，避免点击诱饵陷阱。


<details>
  <summary>Details</summary>
Motivation: 新闻媒体在可控文本生成中面临的核心挑战是：如何在增强读者参与度的同时保持信息保真度。传统优化新闻标题的方法往往与点击诱饵混淆，导致夸张或误导性措辞，损害编辑信任。

Method: 将标题重写构建为可控生成问题，采用基于LLM的引导重写框架，使用FUDGE范式进行推理时控制。系统由两个辅助引导模型驱动：1) 点击诱饵评分模型提供负向引导以抑制过度风格化放大；2) 参与度属性模型提供正向引导以对齐目标点击率目标。两个模型均在精选的真实新闻语料中的中性标题上训练，同时通过LLM在受控激活预定义参与策略下重写原始标题来生成点击诱饵变体。

Result: 通过调整推理时的引导权重，系统能够生成从中性释义到更具吸引力但仍符合编辑标准的标题连续体。该框架为研究吸引力、语义保持和避免点击诱饵之间的权衡提供了原则性方法。

Conclusion: 提出的框架支持在新闻环境中进行负责任的基于LLM的标题优化，将点击诱饵视为合法参与线索的过度放大结果，而非独立风格类别，从而在增强参与度的同时保持编辑可信度。
...
TL;DR: 本文提出Rashid框架，通过可逆加密高资源语言来构建"真正未见"的语言，解决低资源语言研究中工具、数据和专家缺乏的问题，从而实现对上下文语言学习现象的深入探索。


<details>
  <summary>Details</summary>
Motivation: 当前上下文语言学习研究面临严重限制：低资源语言缺乏NLP工具、数据资源和研究专家，导致进展难以评估，无法进行大规模廉价实验，研究结果通常仅限于极少数语言和任务。

Method: 提出Rashid框架，通过对高资源语言进行可逆加密来构建"真正未见"的语言，这样可以利用高资源语言丰富的工具和资源，实现对上下文语言学习现象的系统研究。
...
TL;DR: 该研究通过大规模计算分析Reddit上的堕胎讨论，揭示了Dobbs案裁决后美国堕胎获取障碍的在线表达方式，发现情感和心理障碍在堕胎叙事中占据主导地位。


<details>
  <summary>Details</summary>
Motivation: 2022年美国最高法院Dobbs案裁决重塑了生殖权利格局，引入了新的不确定性和堕胎获取障碍。研究旨在通过分析Reddit上的堕胎讨论，了解这些障碍如何在在线社区中被表达和体验。

Method: 研究分析了四个堕胎相关subreddit的17,000多个帖子，采用多步骤流程对帖子进行分类：信息类型、堕胎阶段、障碍类别和表达情绪。使用包含法律、财务、情感、社会等八类障碍的编码手册，分析其与情绪和信息行为的关联。通过主题建模分析模型生成的障碍理由，追踪话语随法律文化背景变化的演变。

Result: 研究发现情感和心理障碍在在线堕胎叙事中持续占主导地位，紧张、困惑、恐惧、悲伤等情绪在讨论中普遍存在。通过连接信息行为、障碍、情绪和时间动态，揭示了在线社区中堕胎经历的复杂多维图景。

Conclusion: 该研究提供了堕胎在在线社区中如何被导航的多维度描述，展示了法律环境变化如何影响堕胎获取障碍的表达方式，强调了情感和心理因素在生殖健康决策中的核心作用。
...
TL;DR: CAPITU是一个评估大语言模型在巴西葡萄牙语中遵循指令能力的基准测试，基于8部巴西文学经典作品构建，包含59种可自动验证的指令类型，评估了18个最先进模型在单轮和多轮对话中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注英语或使用通用提示，缺乏针对葡萄牙语和文化背景的指令遵循评估。CAPITU旨在填补这一空白，通过巴西文学背景和文化相关内容来评估LLMs在葡萄牙语中的指令遵循能力。

Method: 基于8部巴西文学经典作品构建基准测试，包含59种指令类型，分为7个类别，所有任务都设计为可自动验证（无需LLM评判或人工评估）。指令类型包括葡萄牙语特定的语言约束（如词尾模式-ando/-endo/-indo、-inho/-inha、-mente）和结构要求。评估了18个最先进模型在单轮和多轮对话设置中的表现。

Result: 前沿推理模型表现强劲（GPT-5.2推理版：98.5%严格准确率），葡萄牙语专门化模型提供有竞争力的成本效益（Sabiazinho-4：87.0%准确率，成本0.13美元 vs Claude-Haiku-4.5：73.5%准确率，成本1.12美元）。多轮评估显示约束持续性存在显著差异，对话级准确率在60%到96%之间。识别出形态约束、精确计数和跨轮次约束持续性退化等具体挑战。

Conclusion: CAPITU为葡萄牙语指令遵循研究提供了首个全面的基准测试，揭示了不同模型在葡萄牙语指令遵循能力上的差异，特别是多轮对话中约束持续性的挑战，为未来研究和模型改进提供了重要基础。
...
TL;DR: 研究评估了12个开源推理模型在思维链（CoT）中的忠实性，发现模型对提示影响的承认率从39.7%到89.9%不等，且内部认知与输出表达存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 思维链推理被提议作为安全关键部署中大语言模型的透明度机制，但其有效性取决于忠实性（模型是否准确表达实际影响其输出的因素）。先前研究仅在两个专有模型上评估，发现承认率低至25%-39%，需要扩展到开源模型生态系统进行全面评估。

Method: 研究测试了12个开源推理模型（涵盖9种架构，7B-685B参数），在MMLU和GPQA Diamond的498道选择题中注入六类推理提示（奉承、一致性、视觉模式、元数据、评分器攻击、不道德信息），测量模型在提示成功改变答案时在思维链中承认提示影响的比率。

Result: 在41,832次推理运行中，整体忠实率从39.7%（Seed-1.6-Flash）到89.9%（DeepSeek-V3.2-Speciale）不等。一致性提示（35.5%）和奉承提示（53.9%）的承认率最低。训练方法和模型架构比参数数量更能预测忠实性。关键词分析显示思维标记承认率（约87.5%）与答案文本承认率（约28.6%）存在显著差距。

Conclusion: 思维链监控作为安全机制的可行性受到挑战，忠实性不是推理模型的固定属性，而是随架构、训练方法和影响线索性质系统性变化。模型内部能识别提示影响但在输出中系统性抑制这种承认。
...
Method: 提出LGSE框架，将单词分解为词素并平均预训练子词或FastText词素表示来构建语义连贯嵌入；无法分割时使用字符n-gram；在语言自适应预训练中应用正则化项防止新嵌入偏离初始值过大

Result: 在阿姆哈拉语和提格里尼亚语的问答、命名实体识别、文本分类三个任务上，LGSE始终优于基线方法

Conclusion: 基于形态学基础的嵌入初始化能有效提高低资源形态丰富语言中的表示质量，为适应预训练语言模型提供了有效方法
...
<details>
  <summary>Details</summary>
Motivation: 美国有2730万居民存在语言障碍，专业医疗翻译成本高昂且难以获得，需要评估前沿大语言模型在医疗文档翻译中的表现，特别是对不同资源水平语言的适应性。

Method: 评估了四种前沿大语言模型（GPT-5.1、Claude Opus 4.5、Gemini 3 Pro、Kimi K2），将22份医疗文档翻译成8种语言（涵盖高、中、低资源水平），采用五层验证框架，包括语义相似度、跨模型回译、模型间一致性分析和词汇借用分析等方法。

Result: 在704个翻译对中，所有模型都实现了高语义保真度（LaBSE大于0.92），高资源和低资源语言之间没有显著差异（p=0.066）。跨模型回译确认结果不受同模型循环影响，模型间一致性高（LaBSE：0.946），词汇借用分析显示低资源语言中英语术语保留与保真度评分无相关性。

Conclusion: 前沿大语言模型能够跨不同资源水平的语言保持医疗意义，为医疗领域的语言访问提供了有前景的解决方案，有望改善语言障碍患者的医疗服务质量。
...
TL;DR: ILSE是一种通过Cayley图编码器从LLM各层表示中学习有效表示的结构化方法，在13个任务上显著优于基线方法


<details>
  <summary>Details</summary>
Motivation: 现有LLM通常只使用最后一层的表示进行预测，但研究表明中间层包含大量任务相关信息，且不同任务的最优层不同，需要一种能从所有层表示中学习有效表示的方法

Method: 提出Inter-Layer Structural Encoders (ILSE)，核心是Cayley-Encoder——一种基于扩展Cayley图的几何编码器，用于高效的层间信息传播

Result: 在13个分类和语义相似性任务上，使用9个预训练LLM（1400万到80亿参数）评估，ILSE始终优于基线方法，准确率提升高达44%，相似性指标提升25%

Conclusion: ILSE能够有效整合LLM各层表示，在少样本场景下数据效率高，能使小模型达到与更大模型竞争的性能
...
TL;DR: 本文系统比较了LLM-based和模块化pipeline方法在对话式ASR中的表现，重点关注多说话人场景下的鲁棒性，并提出了新的评估指标tcpSemER。


<details>
  <summary>Details</summary>
Motivation: 对话式自动语音识别面临重叠语音、远场噪声和说话人数量变化等挑战。虽然基于LLM的系统在单说话人基准上表现良好，但在多说话人场景下的鲁棒性尚不明确。

Method: 系统比较LLM-based和模块化pipeline方法在四个维度：重叠鲁棒性、语义保真度、说话人数量、单通道与多通道输入。引入tcpSemER指标，用基于嵌入的语义相似度替代Levenshtein距离，并将tcpWER分解为重叠和非重叠部分进行细粒度分析。

Result: 在三个数据集上的实验表明，基于LLM的系统在双说话人场景中具有竞争力，但随着说话人数量和重叠度的增加性能下降，而模块化pipeline方法保持更强的鲁棒性。

Conclusion: 模块化pipeline方法在多说话人重叠语音场景中比LLM-based系统更具鲁棒性，特别是在说话人数量增加时。新提出的tcpSemER指标能更好地捕捉传统指标忽略的语义错误。
...
TL;DR: 研究发现，单次提示评估LLM道德推理不可靠，多提示稳健性测试应成为标准实践。原始研究中o1-mini比GPT-4o更功利主义的结论在去除提示混淆后不成立，所有模型都趋于功利主义回答。


<details>
  <summary>Details</summary>
Motivation: 验证和扩展Pfeffer等人(2025)关于OpenAI推理模型与非推理模型在电车问题和天桥困境中道德判断差异的研究，探究单次提示评估的可靠性问题。

Method: 使用四个当前OpenAI模型进行复制研究，并通过提示变体测试扩展研究。对比不同提示框架（"我应该..." vs "道德上允许..."）对模型回答的影响，分析拒绝回答的情况。

Result: 电车问题的发现不成立：GPT-4o的低功利主义率反映的是安全拒绝而非道义承诺，当提示改为"道德上允许..."时，GPT-4o给出99%功利主义回答。所有模型在去除提示混淆后都趋于功利主义。天桥困境的发现部分成立，但推理模型常拒绝回答或给出非功利主义答案。

Conclusion: 单次提示评估LLM道德推理不可靠，多提示稳健性测试应成为评估LLM行为的实证主张的标准实践。提示框架显著影响模型回答，安全机制可能掩盖模型的真实道德倾向。
...
TL;DR: 论文提出矛盾调和解释生成任务，研究大语言模型如何生成解释来调和看似矛盾的陈述，而非简单地选择接受或拒绝某些陈述。


<details>
  <summary>Details</summary>
Motivation: 现有NLP研究通常将矛盾视为需要解决的错误，通过选择接受或丢弃某些陈述来处理。然而，人类在社会互动和专业领域推理的关键能力是能够提出调和矛盾的解释。尽管大语言模型的推理能力不断增强，但其生成此类调和解释的能力尚未得到充分探索。

Method: 提出矛盾调和解释生成任务，重新利用现有的自然语言推理（NLI）数据集，并引入可扩展的自动评估质量指标。对18个大语言模型进行实验。

Result: 大多数模型在此任务上表现有限，通过"思考"延长测试时间计算带来的益处随着模型规模增加而趋于平稳。
...
TL;DR: KALAVAI协议通过后融合独立训练的领域专家模型，实现可预测的性能增益（增益=0.82×差异度-2.72），差异度低于3.3%时增益趋近于零。协议包含共享初始化、轻量级MoE路由等关键要素。


<details>
  <summary>Details</summary>
Motivation: 解决如何有效融合独立训练的领域专家模型，并提前预测合作价值，避免计算资源浪费。为多贡献者协作提供可预测的融合框架。

Method: KALAVAI协议：贡献者基于共享检查点独立微调副本，提交后进行轻量级MoE路由（500步）。使用学习型路由器而非均匀平均，确保路由接近领域最优分配。

Result: 性能增益可预测且稳定：410M参数模型+7.72%，1B参数模型+7.49%，6.9B参数模型+6.53%。跨语言融合（泰米尔语/约鲁巴语/威尔士语/代码）实现+21.76%增益。20贡献者联邦实现+16.71%增益。路由器匹配领域最优路由（差异<10^{-5} nats）。

Conclusion: 独立训练的领域专家模型可通过KALAVAI协议有效融合，获得可预测的性能增益。协议要求共享初始化、学习型路由，冻结层在超过10,000步时有益。为分布式协作训练提供实用框架。
...
TL;DR: DALDALL是一个基于角色的数据增强框架，专门用于法律信息检索，通过使用法律专业人士角色生成高质量合成查询，解决低资源领域数据稀缺问题。
...
Method: 提出DALDALL框架，采用领域特定的专业角色（如律师、检察官、法官）来生成合成查询，相比普通提示方法能产生更具词汇和语义多样性的数据。

Result: 在CLERC和COLIEE基准测试中，基于角色的增强方法在Self-BLEU分数衡量的词汇多样性方面有显著提升，同时保持与原始查询的语义保真度。使用角色增强数据微调的密集检索器在召回性能上优于使用原始数据或通用增强方法训练的模型。
...
TL;DR: 该论文提出使用BERT和RoBERTa模型，结合槽位损失和跨度对比损失（SCL）进行微调，以改进习语检测，在现有数据集上取得了最先进的序列准确率性能。


<details>
  <summary>Details</summary>
Motivation: 习语等非组合性多词表达对语言模型构成独特挑战，因为其含义不等于各单词含义之和。现有模型通常需要少样本提示或指令微调才能识别习语，而基于BERT或LSTM的微调方法效果最佳。

Method: 提出基于BERT和RoBERTa的模型，采用槽位损失和跨度对比损失（SCL）结合硬负样本重加权进行微调，以改进习语检测。同时提出F1分数和序列准确率的几何平均数来综合评估模型的跨度感知能力和整体性能。

Result: 在现有数据集上取得了最先进的序列准确率性能。对比消融研究显示了SCL的有效性和泛化能力。

Conclusion: 提出的基于BERT/RoBERTa的模型结合槽位损失和跨度对比损失（SCL）能有效提升习语检测性能，SCL方法具有良好效果和泛化能力，新的评估指标能更全面地评估模型性能。
...
TL;DR: 提出自适应贝叶斯估计框架，通过动态调整采样需求来高效检测大语言模型的幻觉问题，相比现有方法减少50%样本量并提升12.6%的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法依赖固定采样预算，无法适应查询复杂度，导致计算效率低下。需要一种能够根据观测不确定性动态调整采样需求的方法。
...
Conclusion: 提出的自适应贝叶斯框架能有效提升幻觉检测的效率和性能，通过动态采样调整和系统语义探索，为大语言模型的可靠性评估提供了更高效的解决方案。
...
TL;DR: 该研究通过逐步删除推理步骤的方法评估前沿语言模型的推理真实性，发现大多数模型的逐步推理是装饰性的而非必要，只有特定模型在特定任务上表现出真正的步骤依赖性。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型常展示逐步推理过程，但需要验证这些推理步骤是否真正影响最终答案，还是仅仅是模型在决定答案后生成的装饰性叙述。

Method: 引入步骤级评估方法：逐一删除推理句子，检查答案是否改变。该方法仅需API访问，无需模型权重，每个模型每个任务成本约1-2美元。测试了10个前沿模型在情感分析、数学、主题分类和医学QA等任务上的表现。

Result: 大多数模型产生装饰性推理：删除任何步骤改变答案的概率低于17%，而任何单个步骤都能恢复答案。数学任务上，较小模型(0.8-8B)显示出真正的步骤依赖性(55%必要性)。只有MiniMax-M2.5在情感分析(37%必要性)和Kimi-K2.5在主题分类(39%必要性)上打破模式。还发现"输出刚性"现象和注意力模式差异。

Conclusion: 前沿模型的逐步解释主要是装饰性的，需要针对每个模型和每个领域进行评估，训练目标而非模型规模决定了推理是否真实。
...
<details>
  <summary>Details</summary>
Motivation: 跟踪纵向放射学报告中的发现对于准确识别疾病进展至关重要，但这一过程耗时且需要自动化总结。当前需要一种能够清晰展示时间变化并便于事实核查的结构化总结方法。
...
Conclusion: 提出的结构化时间线总结方法能够有效跟踪纵向放射学发现，便于跨时间比较和事实核查，组名生成是关键步骤，系统性能接近人类水平。
...
TL;DR: 该研究分析了5个主流大语言模型在640种实验条件下生成的巴勒斯坦和以色列身份角色，发现模型在战争与非战争语境下对这两个群体的社会经济地位和职业属性存在显著差异，且公平性指令会导致模型输出出现多样化但非一致性的调整。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地用于社会模拟和角色生成，需要理解它们如何表征地缘政治身份。本研究旨在分析主流LLMs在生成巴勒斯坦和以色列身份角色时的表现，探究模型在不同语境下的偏见模式以及公平性指令的影响。

Method: 研究设计了640个实验条件，使用5个流行的大语言模型，通过变化语境（战争vs非战争）和分配角色，生成巴勒斯坦和以色列身份的角色描述。分析生成的属性分布模式，并考察在明确避免有害假设的指令下模型的响应变化。同时分析了模型的推理轨迹，探究推理过程与最终生成结果之间的关系。

Result: 研究发现显著分布模式：在战争语境下，巴勒斯坦角色常与较低社会经济地位和生存导向角色相关，而以色列角色主要保持中产阶级地位和专业职业属性。当提示避免有害假设时，模型表现出多样化分布变化，如非二元性别推断显著增加或向通用职业角色（如"学生"）趋同，但潜在的社会经济差异往往仍然存在。推理轨迹分析显示，虽然推理过程持续提及公平相关概念，但最终生成的角色遵循上述多样化分布变化。

Conclusion: 研究结果揭示了模型如何解释地缘政治语境，同时表明它们以多样化方式处理公平性并进行调整；公平概念并没有一致、直接地转化为代表性结果。这突显了LLMs在表征复杂社会身份时存在的系统性偏见，以及当前公平性干预措施的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的谣言检测方法在处理谣言传播结构时存在过平滑问题，导致性能下降。研究发现过平滑与传播树结构特征相关（多数节点为1级节点），且GNN难以捕捉树中的长距离依赖关系。

Method: 提出P2T3方法：1) 从传播树中按回复传播方向提取所有对话链；2) 使用token-wise embedding注入连接信息和必要的归纳偏置；3) 在大规模无标签数据集上进行预训练。

Result: P2T3在多个基准数据集上超越了先前的最先进方法，在少样本条件下表现良好，不仅避免了GNN的过平滑问题，还展示了作为大模型或多模态统一方案的潜力。

Conclusion: 基于纯Transformer的P2T3方法有效解决了GNN在谣言检测中的过平滑和长距离依赖问题，为未来社交媒体研究提供了有前景的大模型或多模态统一方案。
...
TL;DR: 该研究通过多LLM集成方法将日本心理咨询对话数据集KokoroChat翻译成英文和中文，解决了心理咨询领域高质量多语言数据集稀缺的问题。
...
Method: 开发了新颖的多LLM集成方法：首先从多个不同的LLM生成多样化的翻译假设，然后由一个LLM基于对所有假设优缺点的分析来生成高质量的最终翻译。

Result: 通过人工偏好研究验证了Multilingual KokoroChat的质量，证明集成方法产生的翻译比任何单个最先进的LLM更受偏好，确认了方法的优越性。

Conclusion: 成功创建了高质量的多语言心理咨询对话数据集，并通过创新的多LLM集成方法解决了敏感领域翻译质量的关键挑战，数据集已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有对话系统依赖查询建议提升用户参与度，但基于大语言模型和点击率模型的方法严重依赖大量在线点击数据进行CTR模型训练，在冷启动场景下表现不佳。

Method: 提出Cold-EQS迭代强化学习框架，使用可回答性、事实性和信息增益作为奖励函数持续优化查询建议质量；通过估计候选查询的不确定性，从缺乏点击信号的在线用户查询中选择困难和模糊样本进行优化；提供包含16,949个在线用户查询的EQS基准数据集。

Result: 离线和在线实验显示在线和离线效果之间存在强正相关性；Cold-EQS在在线聊天UV方面实现了显著的+6.81%提升。

Conclusion: Cold-EQS框架有效解决了电商查询建议的冷启动问题，通过迭代强化学习优化查询质量，在缺乏点击数据的情况下仍能提供高质量的查询建议。
...
TL;DR: 论文提出了一种用于大语言模型的集合值预测框架，通过多次采样构建候选响应集合，提供可行性感知的覆盖保证，解决了传统点预测低估模型能力的问题。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型使用最可能生成作为点预测，但这低估了模型能力：虽然排名最高的响应可能不正确，但有效答案可能存在于更广泛的输出空间中，可以通过重复采样发现。这促使从点预测转向集合值预测。

Method: 提出了一个原则性的集合值预测框架，建立最小可达到风险水平，开发数据驱动的校准程序，通过估计严格阈值从采样响应构建预测集合，确保在目标风险水平可行时以期望概率包含正确答案。

Result: 在六个语言生成任务和五个大语言模型上的广泛实验表明，该框架既具有统计有效性，又具有预测效率。

Conclusion: 该框架为LLM的集合值预测提供了可行性感知的覆盖保证，解决了有限采样下覆盖不一定总是可达的问题，并通过数据驱动校准确保了统计有效性。
...
<details>
  <summary>Details</summary>
Motivation: 阿富汗主要语言达里语拥有数千万使用者，但在错误信息检测文献中几乎完全缺失，需要填补这一空白

Method: 1. 创建DariMis数据集，包含信息类型（错误/部分真实/真实）和危害等级（低/中/高）两个维度的标注
2. 提出双输入编码策略，将视频标题和描述作为独立的BERT输入段，显式建模标题声明与正文内容的语义关系
3. 使用ParsBERT（波斯语专用模型）和XLM-RoBERTa-base进行基准测试

Result: 1. 发现信息类型与危害等级存在结构耦合：55.9%的错误信息具有中等以上危害，而真实内容仅1.0%
2. 双输入编码相比单字段拼接在错误信息召回率上提升7个百分点（60.1%到67.1%）
3. ParsBERT取得最佳测试性能：准确率76.60%，宏F1 72.77%

Conclusion: DariMis填补了达里语错误信息检测的空白，双输入编码策略能有效提升安全关键类别的检测性能，信息类型分类器可作为内容审核中的隐式危害分级过滤器
...
TL;DR: 该研究提出了一种区分不文明（语气）和不容忍（内容）的细粒度多模态毒性标注方案，相比传统的二元仇恨标签，能提升模型性能并改善错误分布。


<details>
  <summary>Details</summary>
Motivation: 当前多模态毒性基准通常使用单一的二元仇恨标签，这种粗粒度方法混淆了表达的两个根本不同特征：语气和内容。需要更精细的标注方案来改进内容审核系统的可靠性和准确性。

Method: 基于传播科学理论，引入区分两个可分离维度的细粒度标注方案：不文明（粗鲁或轻蔑的语气）和不容忍（攻击多元主义并针对群体或身份的内容）。将该方案应用于Hateful Memes数据集的2,030个模因。评估了粗标签训练、跨标签方案迁移学习以及结合粗粒度仇恨标签与细粒度标注的联合学习方法。

Result: 细粒度标注补充了现有的粗粒度标签，联合使用时能提高整体模型性能。使用细粒度方案训练的模型展现出更平衡的审核相关错误分布，并且比仅使用仇恨标签训练的模型更不容易漏检有害内容（FNR-FPR：LLaVA-1.6-Mistral-7B从0.74降至0.42；Qwen2.5-VL-7B从0.54降至0.28）。

Conclusion: 结合粗粒度和细粒度标签为更可靠的多模态内容审核提供了实用途径。这项工作通过提高数据质量，为内容审核中的数据中心方法做出了贡献，提升了审核系统的可靠性和准确性。
...
TL;DR: 本文提出TriFEX评估框架和PKP指标，用于评估RAG微调在长文本生成中的表现，发现传统指标无法检测事实差异，并证明小模型在专业任务中可达到与大模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG微调研究主要针对文档问答，依赖标准NLP指标可能掩盖事实差异，且缺乏对长文本生成和专业领域（如电子设计自动化）的评估方法。

Method: 1) 在电子设计自动化领域使用7B模型进行RAG微调，采用五种上下文增强策略；2) 提出TriFEX评估框架，基于三元组（查询、上下文、参考）进行人工验证；3) 提出PKP指标，通过过滤提示泄露来隔离内化知识。

Result: 1) ROUGE和BERTScore无法检测TriFEX揭示的事实差异；2) 现有知识内化指标对检索敏感，其75%的跨条件方差由知识表达率变化驱动；3) 微调的7B变体在多数指标上优于72B基线，且在不同条件和相关基准上具有泛化能力。

Conclusion: 现有RAG评估指标存在局限性，TriFEX和PKP能更准确评估事实准确性；小模型经过适当微调可在专业任务中达到良好性能，实现成本效益高的本地部署。
...
TL;DR: AuthorMix：轻量级、模块化、可解释的作者风格迁移框架，通过训练独立的风格特定LoRA适配器，并使用少量目标风格示例进行层间适配器混合，在低资源目标上超越现有SOTA方法和GPT-5.1


<details>
  <summary>Details</summary>
Motivation: 现有作者风格迁移方法通常训练单一模型处理所有目标风格，这种方法成本高、灵活性有限，且常常为了风格迁移而牺牲语义保持。需要一种更灵活、高效且能更好保持原意的解决方案。

Method: 提出AuthorMix框架：1）在高资源作者的小数据集上训练独立的风格特定LoRA适配器；2）通过学习的层间适配器混合技术，仅需少量目标风格训练示例即可快速训练专门的自适应模型；3）框架具有轻量级、模块化和可解释的特点。

Result: 在低资源目标上，AuthorMix超越了现有的SOTA风格迁移基线方法以及GPT-5.1，获得了最高的综合评分，并在语义保持方面有显著提升。

Conclusion: AuthorMix提供了一种高效、灵活的作者风格迁移解决方案，通过模块化设计和少量示例学习，在保持语义的同时实现了更好的风格迁移效果，特别适用于低资源目标场景。
...
TL;DR: 研究发现大脑对齐对大型语言模型的语义、句法、推理等语言能力至关重要，通过故意训练大脑不对齐模型证明了这一点


<details>
  <summary>Details</summary>
Motivation: 虽然大脑对齐的LLMs作为认知模型和增强AI安全可信度受到关注，但其对语言能力的具体作用尚不明确，需要研究大脑对齐的功能意义

Method: 引入大脑不对齐模型——故意训练预测大脑活动差但保持高语言建模性能的LLMs，在200多个下游任务上评估，包括语义、句法、语篇、推理和形态学等领域

Result: 大脑不对齐显著损害下游任务表现，表明大脑对齐对实现稳健语言能力具有关键作用

Conclusion: 大脑对齐在LLMs中至关重要，为神经表征与语言处理之间的关系提供了新见解
...
TL;DR: 提出两阶段零样本科学知识图谱构建框架：Z-NERD用于实体识别，HGNet用于层次感知关系抽取，在跨域测试中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱构建方法存在三个主要问题：难以识别长多词实体、跨领域泛化能力差、忽视科学知识的层次结构。通用大语言模型虽然适应性强，但计算成本高且在专业任务上准确性不一致，导致当前知识图谱浅层且不一致，限制了探索和综合的实用性。

Method: 提出两阶段框架：第一阶段Z-NERD包含正交语义分解(OSD)和多尺度TCQK注意力机制，用于领域无关的实体识别；第二阶段HGNet进行层次感知关系抽取，使用层次感知消息传递显式建模父子同级关系。引入两个互补目标：可微层次损失防止循环和捷径边，连续抽象场(CAF)损失将抽象级别嵌入欧几里得空间的可学习轴。

Result: 在SciERC、SciER和SPHERE基准测试中达到新的最先进水平，在分布外测试中NER提升8.08%，RE提升5.99%。在零样本设置下，NER增益达10.76%，RE增益达26.2%。

Conclusion: 该框架首次将层次抽象形式化为标准欧几里得嵌入中的连续属性，为双曲方法提供了更简单的替代方案。提出的SPHERE基准为层次关系抽取提供了多领域评估标准，显著提升了科学知识图谱构建的可扩展性和跨领域泛化能力。
...
TL;DR: 本文研究了AI生成文本检测器的可靠性问题，发现现有检测器虽然在基准测试中表现优异，但在实际应用中存在泛化能力不足的问题，主要依赖数据集特定的风格特征而非稳定的机器作者信号。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，AI生成文本检测成为紧迫而复杂的挑战。尽管许多检测系统在基准测试中报告了高准确率，但它们在真实世界环境中的可靠性仍不确定，且可解释性往往未被探索。本研究旨在探究当代检测器是否真正识别机器作者身份，还是仅仅利用了数据集特定的伪影。

Method: 提出了一个可解释的检测框架，整合了语言特征工程、机器学习和可解释AI技术。使用30个语言特征训练模型，在PAN CLEF 2025和COLING 2025两个基准语料库上进行评估。通过系统性的跨领域和跨生成器评估来测试泛化能力，并使用SHAP-based解释方法来分析特征重要性。

Result: 模型在基准测试中取得了竞争性性能，F1分数达到0.9734。然而，跨领域评估显示存在显著的泛化失败：在领域内表现优秀的分类器在分布偏移下性能显著下降。SHAP解释显示最有影响力的特征在不同数据集间差异显著，表明检测器通常依赖数据集特定的风格线索而非稳定的机器作者信号。深入错误分析揭示了基于语言特征的AI文本检测存在根本性矛盾：在领域内最具区分性的特征也最容易受到领域偏移、格式变化和文本长度效应的影响。

Conclusion: 基于语言特征的AI文本检测器存在泛化能力不足的问题，主要依赖数据集特定的特征而非稳定的机器作者信号。这一认识有助于构建在不同设置下都稳健的AI检测器。为支持复制和实际使用，作者发布了开源Python包，可返回预测结果和单个文本的实例级解释。
...
TL;DR: UDE是一个统一的多轮交互AI系统评估工具包，通过标准化数据格式、模块化评估流程和统一评分接口，解决了现有评估协议异构性问题，提高了评估的效率和可复现性。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统在多轮交互场景中的评估协议高度异构，数据集格式、模型接口和评估流程差异很大，严重阻碍了系统性的比较和分析。

Method: UDE采用整体统一的方法：1）将异构数据格式标准化为通用模式；2）通过模块化架构简化复杂评估流程；3）在一致的评分接口下对齐指标计算；4）支持并行生成和评分以提高大规模评估效率；5）使用基于检查点的缓存消除冗余计算。

Result: 经过多种多轮基准测试验证，UDE不仅通过标准化工作流程和透明日志保证了高可复现性，还显著提高了评估效率和可扩展性。

Conclusion: UDE为交互式AI提供了一个标准化的评估工具包，有助于建立统一的基准测试生态系统，加速交互式AI领域的未来发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于人类反馈的强化学习（RLHF）中的奖励建模严重依赖成本高昂的显式反馈数据收集。隐式人类反馈（如点击、复制）虽然成本低，但面临两个根本挑战：缺乏明确的负样本，以及用户偏好偏差导致不同回答引发反馈的概率不同。

Method: 提出ImplicitRM方法：1）通过分层模型将训练样本分为四个潜在组；2）基于此通过似然最大化推导学习目标，该方法在理论上被证明是无偏的，有效解决了隐式反馈的两个核心挑战。

Result: 实验表明，ImplicitRM能够在各种隐式偏好数据集上学习到准确的奖励模型，验证了该方法的有效性。

Conclusion: ImplicitRM为从低成本隐式人类反馈中学习无偏奖励模型提供了一种有效的解决方案，解决了传统RLHF中数据收集成本高的问题，具有重要的实际应用价值。
...
TL;DR: 研究评估了LLMs在英语情感表达和阿拉伯语人格特质模仿方面的能力，发现AI生成文本与人类文本可区分，但分类器在改写样本上表现下降。情感和人格分类存在显著泛化差距，AI数据增强对阿拉伯语任务有帮助，GPT-4o和Gemini表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs流畅度的提升，需要研究它们在不同语言文化背景下模仿人类复杂特质（如情感表达和人格）的能力，特别是在阿拉伯语这种资源匮乏但具有独特语言文化特征的语境中。

Method: 使用六个模型（Jais、Mistral、LLaMA、GPT-4o、Gemini、DeepSeek）进行两项任务：1）评估机器学习分类器能否可靠区分人类和AI生成文本；2）评估LLM生成文本与人类在情感或人格特质方面的可比性。包括对改写样本的分析、情感和人格分类实验、数据增强实验以及语言和心理语言学分析。

Result: AI生成文本与人类文本可区分（F1>0.95），但分类器在改写样本上表现下降，表明依赖表面风格线索。情感和人格分类存在显著泛化差距：基于人类数据训练的分类器在AI文本上表现差，反之亦然，表明LLMs编码情感信号的方式与人类不同。使用AI生成数据增强训练可提升阿拉伯语人格分类性能。GPT-4o和Gemini表现出最佳的情感连贯性。语言分析显示人类和AI文本在语调、真实性和文本复杂性方面存在可测量的差异。

Conclusion: LLMs在模仿人类情感和人格特质方面存在局限性，特别是在跨语言文化背景下。AI生成文本与人类文本在情感编码方式上存在差异，但在资源匮乏语言环境中，合成数据具有增强性能的潜力。这对情感计算、作者归属和负责任AI部署具有重要意义。
...
TL;DR: 使用稀疏自编码器识别LLM中的文化特征，构建文化嵌入(CuE)，通过白盒干预提升文化忠实度和长尾文化概念的表达


<details>
  <summary>Details</summary>
Motivation: 现有LLM存在文化偏见，偏向训练数据丰富的文化；现有文化本地化方法（如提示或后训练对齐）是黑盒、难以控制，且无法区分失败是由于知识缺失还是表达不足

Method: 利用机制可解释性方法，通过稀疏自编码器识别编码文化显著信息的可解释特征，将其聚合成文化嵌入(CuE)，用于分析隐含文化偏见和构建白盒干预策略

Result: CuE干预能提高文化忠实度，比单独提示更能引出罕见的长尾文化概念；CuE与黑盒本地化方法互补，在提示增强输入基础上进一步改善效果；模型确实受益于更好的表达策略，不一定缺乏长尾知识表示
...
TL;DR: WISTERIA是一个基于弱隐式信号的时序关系抽取框架，通过事件对条件化的top-K注意力机制来识别决定时序关系的具体线索，提供可解释的时序推理。
...
Method: 提出WISTERIA框架，结合多头注意力机制和事件对条件化的top-K池化，为每个事件对隔离出最具信息量的上下文标记，考虑任何表达时序顺序的词汇、句法或形态元素作为隐式信号。

Result: 在TimeBank-Dense、MATRES、TDDMan和TDDAuto数据集上的实验表明，WISTERIA取得了有竞争力的准确率，并且top-K标记的分析揭示了与时序语言线索对齐的事件对层面推理依据。

Conclusion: WISTERIA提供了一种局部化和可解释的时序推理视角，能够识别隐式时序信号，为时序关系抽取提供了更精细的分析能力。
...
TL;DR: LLM在性别推断任务中的输出对微小语境变化敏感，即使语法表述几乎相同，模型输出也不稳定，这对偏见基准测试和高风险部署有重要影响。


<details>
  <summary>Details</summary>
Motivation: 标准评估实践假设LLM输出在上下文等效的任务表述下是稳定的，本文旨在测试这一假设在性别推断任务中是否成立。

Method: 使用受控代词选择任务，引入最小的理论上无信息的语篇语境，分析语境变化如何影响模型输出，并通过Contextuality-by-Default分析评估语境依赖的持续性。

Result: 微小语境变化导致模型输出出现大规模系统性偏移；文化性别刻板印象相关性在去语境化设置中存在，但引入语境后减弱或消失；无关特征（如无关指代词的性别）成为模型行为的最具信息性预测因子；19-52%的案例中语境依赖持续存在。

Conclusion: LLM输出即使在几乎相同的语法表述下也违反语境不变性，这对偏见基准测试和高风险部署具有重要影响。
...
TL;DR: 论文提出了Memory Bear AI Memory Science Engine，这是一个以记忆为中心的多模态情感智能框架，将情感信息建模为记忆系统中的结构化变量，而非瞬时的输出标签。


<details>
  <summary>Details</summary>
Motivation: 现实交互中的情感判断很少是纯粹的局部预测问题。情感意义通常依赖于先前的轨迹、累积的上下文以及可能在当前时刻微弱、嘈杂或不完整的多模态证据。现有的多模态情感识别系统大多优化于短程推理，对持久情感记忆、长程依赖建模和不完美输入下的鲁棒解释支持有限。

Method: 提出了一个记忆中心框架，通过结构化记忆形成、工作记忆聚合、长期巩固、记忆驱动检索、动态融合校准和连续记忆更新来组织处理。核心是将多模态信号转换为结构化的情感记忆单元（EMUs），使情感信息能够在交互过程中被保存、重新激活和修订。

Result: 实验结果显示，在基准测试和实际业务场景中，该框架相比对比系统取得了持续的性能提升，具有更强的准确性和鲁棒性，特别是在嘈杂或缺失模态条件下表现更优。

Conclusion: 该框架是从局部情感识别向更连续、鲁棒且部署相关的多模态情感智能迈出的实际一步，将情感信息建模为记忆系统中的结构化变量，实现了跨交互时间的情感信息保存和演化。
...
TL;DR: 通过AI私有语言思想实验，研究发现两个AI智能体通过多智能体强化学习发展出的高效、不可解释通信协议，比使用人类可理解的符号协议效率高50.5%，挑战了思维语言假说。


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证思维是否需要类似语言的格式，挑战语言思维假说。通过AI私有语言思想实验，探讨最优协作认知是否必须通过符号结构中介。

Method: 引入"AI私有语言"思想实验，在多智能体强化学习框架下，让两个人工智能体在部分可观测的协作导航任务中发展通信协议。比较新兴协议与预定义的人类可理解符号协议的效率差异。

Result: 使用新兴通信协议的智能体比使用预定义人类可理解符号协议的智能体效率高50.5%，证实了效率衰减现象，表明最优协作认知并非由符号结构中介。

Conclusion: 研究挑战了语言思维假说，表明最优协作认知与亚符号计算自然耦合，支持认知架构的多元主义，对AI伦理有重要启示。
...
### [45] [From Static Templates to Dynamic Runtime Graphs: A Survey of Workflow Optimization for LLM Agents](https://arxiv.org/abs/2603.22386)
*Ling Yue,Kushal Raj Bhandari,Ching-Yun Ko,Dhaval Patel,Shuxin Lin,Nianjun Zhou,Jianxi Gao,Pin-Yu Chen,Shaowu Pan*

Main category: cs.AI

TL;DR: 本文综述了基于大语言模型的智能体计算图工作流设计与优化方法，提出了基于工作流结构确定时机的分类框架，区分静态与动态方法，并建立了三维分析体系。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的系统越来越流行，需要构建可执行的工作流来完成任务。现有方法缺乏统一的分类框架和评估标准，难以进行系统比较和可重复评估。

Method: 提出基于工作流结构确定时机的分类方法：静态方法（部署前固定可重用工作流支架）和动态方法（执行前或执行中为特定运行选择、生成或修订工作流）。建立三维分析框架：何时确定结构、优化工作流的哪部分、哪些评估信号指导优化。

Result: 建立了统一的分类框架，区分了可重用工作流模板、运行特定的实现图和执行轨迹。提出了结构感知的评估视角，补充下游任务指标与图级属性、执行成本、鲁棒性和跨输入的结构变化。

Conclusion: 本文提供了清晰的词汇表、统一的方法定位框架、现有文献的可比较视图，以及未来LLM智能体工作流优化的可重复评估标准，有助于推动该领域的研究发展。
...
### [46] [Computational Arbitrage in AI Model Markets](https://arxiv.org/abs/2603.22404)
*Ricardo Olmedo,Bernhard Schölkopf,Moritz Hardt*

Main category: cs.AI

TL;DR: AI模型市场中套利策略的研究：通过在不同模型提供商间优化分配推理预算，套利者可以创造有竞争力的服务，在SWE-bench案例中实现高达40%的净利润率


<details>
  <summary>Details</summary>
Motivation: 研究AI模型市场中套利行为的动机在于：市场上存在多个具有不同成本和能力的模型提供商，客户愿意为可验证的解决方案支付预算，这为套利者创造了机会。套利者可以通过在不同提供商间优化分配推理预算，以更低成本提供相同服务，从而在无需承担模型开发风险的情况下获得利润。

Method: 本研究采用实证方法，通过深入的案例研究分析AI模型市场中的套利现象。具体使用SWE-bench GitHub问题解决作为可验证领域，采用GPT-5 mini和DeepSeek v3.2两个代表性模型。研究探索了简单的套利策略，并分析了在不同领域通用的稳健套利策略。此外，还研究了蒸馏技术如何创造套利机会，以及多个竞争套利者对市场的影响。

Result: 在可验证的SWE-bench领域中，简单的套利策略能够产生高达40%的净利润率。稳健的套利策略在不同领域仍能保持盈利。蒸馏技术创造了强大的套利机会，但可能以教师模型的收入为代价。多个竞争套利者会降低消费者价格，减少模型提供商的边际收入。同时，套利减少了市场分割，通过使小型模型提供商能够更早获得收入，促进了它们的市场进入。

Conclusion: 套利可以成为AI模型市场中强大的力量，对模型开发、蒸馏和部署具有重要影响。套利不仅为套利者创造利润机会，还通过降低价格使消费者受益，同时促进了市场竞争和更广泛的市场参与。研究结果表明，套利行为将重塑AI模型市场的经济动态。
...
### [47] [AI Mental Models: Learned Intuition and Deliberation in a Bounded Neural Architecture](https://arxiv.org/abs/2603.22561)
*Laurence Anthony*

Main category: cs.AI

TL;DR: 研究探讨有限神经架构能否在经典三段论推理任务中实现直觉与审慎推理的功能分工，发现双路径架构中审慎路径表现显著优于直觉路径，并发展出结构化内部计算。


<details>
  <summary>Details</summary>
Motivation: 探索有限神经架构能否在经典三段论推理任务中实现直觉与审慎推理的功能分工，为AI中的世界模型和多阶段推理辩论提供受控测试环境，检验学习系统能否发展结构化内部计算而非仅一次性关联预测。

Method: 实验1评估直接神经基线预测9类人类反应分布；实验2引入有限双路径架构，包含独立的直觉和审慎路径，基于计算心理模型理论，采用5折交叉验证评估性能。

Result: 直觉路径相关度为r=0.7272，审慎路径达r=0.8152，审慎优势在交叉验证中显著(p=0.0101)；最大提升出现在NVC、Eca和Oca任务，表明对拒绝响应和c-a结论处理能力改善；审慎路径发展出稀疏、分化的内部结构。

Conclusion: 有限条件下模型表现出类似推理的内部组织，但未完全复现模型构建、反例搜索和结论修正的完整顺序过程；双路径架构在经典推理任务中实现了有意义的直觉-审慎分工。
...
### [48] [Understanding LLM Performance Degradation in Multi-Instance Processing: The Roles of Instance Count and Context Length](https://arxiv.org/abs/2603.22608)
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在单个任务上表现优异，但缺乏对多实例处理能力的研究，而用户经常需要LLMs处理多个文档或实例进行分析

Method: 对LLMs在多实例处理能力进行全面评估，测试它们在擅长任务上的表现，分析实例数量和上下文长度的影响

Result: 所有LLMs都呈现相似模式：小数量实例（约20-100）时性能轻微下降，大量实例时性能崩溃；实例数量比上下文长度对最终结果影响更大

Conclusion: 优化LLMs的多实例处理性能时，需要同时关注上下文长度和实例数量，特别是后者对性能有更强影响
...
TL;DR: 论文提出GraLC-RAG框架，通过图感知结构智能改进生物医学文献检索增强生成系统，强调标准排名指标低估了跨文档结构检索的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学文献RAG系统主要使用MRR等排名指标评估，这些指标只关注检索精度而忽略了检索广度，无法评估系统从文档不同结构部分获取证据的能力。

Method: 提出GraLC-RAG框架，整合延迟分块与图感知结构智能，包括结构感知分块边界检测、UMLS知识图谱注入和图引导混合检索，在2,359篇IMRaD格式PubMed Central文章上评估6种策略。

Result: 内容相似性方法MRR最高(0.517)但只从单一章节检索，而结构感知方法检索章节数量增加15.6倍；知识图谱增强检索将答案质量差距缩小到delta-F1=0.009，同时保持4.6倍章节多样性。

Conclusion: 标准指标系统性低估了结构检索的价值，缩小多章节合成差距是生物医学RAG的关键开放问题，需要同时考虑检索精度和结构覆盖度的评估框架。
...
TL;DR: 系统比较四种多智能体编排架构在金融文档信息提取中的性能，发现自反式架构准确率最高但成本也最高，分层架构在成本-准确率帕累托前沿上表现最优。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融文档结构化信息提取中的应用迅速增长，但生产部署面临架构选择缺乏实证指导的问题。需要系统评估不同多智能体编排架构在实际金融环境中的性能表现。

Method: 比较四种多智能体编排架构：顺序管道、并行扇出合并、分层监督工作者和自反式自校正循环。在10,000份SEC文件（10-K、10-Q和8-K表格）上评估五种前沿和开源LLM，涵盖25个提取字段类型，从五个维度衡量：字段级F1、文档级准确率、端到端延迟、每文档成本和令牌效率。

Result: 自反式架构获得最高字段级F1（0.943），但成本是顺序基线的2.3倍；分层架构在成本-准确率帕累托前沿上表现最佳（F1 0.921，成本1.4倍）。混合配置可以恢复自反式架构89%的准确率提升，成本仅为基线的1.15倍。从1K到100K文档/天的扩展分析揭示了吞吐量-准确率下降曲线。

Conclusion: 研究结果为在受监管金融环境中部署多智能体LLM系统提供了可操作的指导，帮助实践者在准确率、成本和扩展性之间做出平衡决策。
...
TL;DR: MUQ-EVAL是一个开源的AI生成音乐质量评估指标，通过冻结MuQ-310M特征训练轻量级预测头，在系统级和话语级都实现了与人类评分的高相关性。


<details>
  <summary>Details</summary>
Motivation: 现有分布性指标如Fréchet Audio Distance无法评估单个音乐片段且与人类判断相关性差，而唯一实现高相关性的逐样本学习指标是闭源的，需要开发开源的高质量评估工具。

Method: 使用MusicEval数据集（31个文本到音乐系统生成的片段及专家质量评分），在冻结的MuQ-310M特征上训练轻量级预测头，采用注意力池化和两层MLP的简单架构。

Result: 系统级SRCC=0.957，话语级SRCC=0.838；编码器选择是主导设计因素；LoRA适应模型仅需150个片段即可达到可用相关性；对信号级伪影敏感但对音乐结构失真不敏感。

Conclusion: MUQ-EVAL是完全开源的逐样本质量指标，优于现有开源指标，可在单个消费级GPU上实时运行，支持从个人听众标注构建个性化质量评估器。
...
TL;DR: 该论文提出了一种名为HyFI的双曲特征插值框架，通过双曲空间解决脑信号与图像特征之间的模态差异和特征纠缠问题，在脑到图像检索任务中取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将神经活动与预训练视觉模型提取的语义和感知特征独立对齐，但未能解决两个关键挑战：1）脑信号与图像之间自然存在的信息水平差异导致的模态差距；2）语义和感知特征在神经活动中高度纠缠。

Method: 提出Hyperbolic Feature Interpolation (HyFI)框架，利用双曲空间的几何特性（两点间的测地线自然向原点弯曲，原点处表示能力较低），在双曲测地线上对语义和感知视觉特征进行插值，实现感知和语义信息的融合与压缩。

Result: HyFI在零样本脑到图像检索任务中实现了最先进的性能，在THINGS-EEG数据集上Top-1准确率提升高达+17.3%，在THINGS-MEG数据集上提升+9.1%，显著优于先前方法。

Conclusion: 双曲特征插值框架有效解决了脑信号与视觉特征对齐中的模态差距和特征纠缠问题，通过利用双曲空间的几何特性，实现了更好的脑-视觉特征对齐，为理解和解码人类视觉系统提供了新方法。
...
TL;DR: LH-Bench是一个针对主观性企业任务的三支柱评估框架，通过专家制定的评分标准、分步评估和人工偏好验证，解决了传统LLM评估在长流程、多工具企业工作流中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估主要针对客观可验证任务（如数学、编程），而真实企业工作往往是主观且依赖上下文的，成功取决于组织目标、用户意图和长流程中产生的中间产物质量。

Method: 提出三支柱评估设计：1）专家制定的评分标准，为LLM评委提供领域上下文；2）精心策划的真实产物，支持分步奖励信号；3）成对人工偏好评估进行收敛验证。在两个环境中测试：Figma-to-code（33个真实.fig任务）和程序化内容（41门课程，183个独立评估章节）。

Result: 专家制定的评分标准比LLM制定的评分标准提供更可靠的评估信号（kappa = 0.60 vs. 0.46），人工偏好判断也确认了相同的顶级分离（p < 0.05），证明基于专家的评估可以在不牺牲可靠性的情况下扩展。

Conclusion: LH-Bench框架能够有效评估自主、长流程的主观企业任务，专家制定的评估标准比自动生成的更可靠，且人工验证支持这一结论，为扩展企业级LLM评估提供了可行方案。
...
### [54] [ABSTRAL: Automatic Design of Multi-Agent Systems Through Iterative Refinement and Topology Optimization](https://arxiv.org/abs/2603.22791)
*Weijia Song,Jiashu Yue,Zhe Pang*

Main category: cs.AI

TL;DR: ABSTRAL框架将多智能体系统架构视为可演化的自然语言文档，通过对比轨迹分析进行精炼。研究发现：多智能体协调存在效率损失（仅26%回合效率）；设计知识可跨领域迁移；能发现初始设计中不存在的专家角色。


<details>
  <summary>Details</summary>
Motivation: 研究如何设计多智能体系统，并将设计知识以可检查、可修改、可迁移的形式进行捕获，解决多智能体系统设计知识的可复用性和可解释性问题。

Method: 提出ABSTRAL框架，将MAS架构视为自然语言文档，通过对比轨迹分析（contrastive trace analysis）不断精炼设计文档。在SOPBench数据集（134个银行任务）上进行实验，使用GPT-4o作为骨干模型。

Result: 1. 量化多智能体协调税：在固定回合预算下，集成系统仅达到26%的回合效率，66%任务耗尽预算限制，但仍通过发现可并行任务分解优于单智能体基线。
2. 设计知识可迁移：在一个领域学习到的拓扑推理和角色模板能为新领域提供优势，迁移的种子设计在单次迭代中达到冷启动第3次迭代的性能。
3. 发现新专家角色：对比轨迹分析能发现初始设计中不存在的专家角色，这是先前系统不具备的能力。
在SOPBench上达到70%验证集/65.96%测试集通过率。

Conclusion: ABSTRAL框架成功将多智能体系统设计知识编码为可演化、可检查的自然语言文档，实现了设计知识的可迁移性和可解释性，并能自动发现新的专家角色，为多智能体系统设计提供了新的方法论。
...
### [55] [Reliable Classroom AI via Neuro-Symbolic Multimodal Reasoning](https://arxiv.org/abs/2603.22793)
*Sina Bagheri Nezhad*
...
<details>
  <summary>Details</summary>
Motivation: 课堂AI正从低级感知扩展到高级判断（如参与度、困惑度、协作等），但课堂环境复杂：多参与者、嘈杂、隐私敏感、教学多样、多语言。传统AI仅追求预测准确性不足，需要可验证证据、校准不确定性和明确部署护栏。
...
Result: 本文未报告新实证结果，主要贡献是具体框架和评估议程。提出了围绕五个任务的基准和评估协议：课堂状态推断、话语基础事件链接、时序早期预警、协作分析和多语言课堂推理。并指定了以弃权、校准、鲁棒性、结构对齐和人类有用性为中心的可靠性指标。

Conclusion: 课堂AI应被视为关键领域，需要更可解释、隐私感知和教学基础的多模态AI。NSCR框架旨在支持此类系统开发，通过神经符号方法确保预测具有可验证证据、校准不确定性和明确部署护栏，促进课堂AI的负责任发展。
...
### [56] [Learning What Matters Now: Dynamic Preference Inference under Contextual Shifts](https://arxiv.org/abs/2603.22813)
*Xianwei Cao,Dou Quan,Zhenliang Zhang,Shuang Wang*

Main category: cs.AI

TL;DR: 提出动态偏好推断（DPI）框架，用于处理多目标强化学习中偏好权重随时间变化的问题，通过概率推断学习偏好并指导策略，在环境变化时优于固定权重方法。


<details>
  <summary>Details</summary>
Motivation: 人类决策时偏好权重会随情境动态变化，而传统多目标强化学习方法假设固定偏好权重或已知标量奖励，无法适应这种动态变化的需求。

Method: 提出动态偏好推断（DPI）框架：1）维护偏好权重的概率信念；2）从近期交互中更新信念；3）基于推断偏好条件化策略。具体实现为变分偏好推断模块与偏好条件化actor-critic联合训练，使用向量化回报作为潜在权衡的证据。

Result: 在排队、迷宫和多目标连续控制环境中，当目标发生事件驱动变化时，DPI能够适应新机制，在变化后性能优于固定权重和启发式包络基线方法。

Conclusion: 动态偏好推断框架能够有效处理多目标强化学习中偏好权重随时间变化的问题，提供了一种更符合人类认知的决策方法，在动态环境中表现出更好的适应性。
...
### [57] [Improving Safety Alignment via Balanced Direct Preference Optimization](https://arxiv.org/abs/2603.22829)
*Shiji Zhao,Mengyang Wang,Shukun Xiong,Fangzhou Chen,Qihui Zhu,Shouwei Ruan,Yisong Xiao,Ranjie Duan,Xun Chen,XingXing Wei*

Main category: cs.AI

TL;DR: B-DPO方法通过基于互信息的自适应优化强度调节，解决了安全对齐中的过拟合问题，在保持模型通用能力的同时提升了安全性。
...
Method: 提出平衡直接偏好优化（B-DPO），基于互信息自适应调节偏好响应和非偏好响应之间的优化强度。该方法通过分析模型对训练数据的理解程度，动态调整优化过程，解决不平衡偏好理解问题。

Result: 实验结果表明，B-DPO能够在各种主流基准测试中保持大语言模型的竞争性通用能力的同时，显著增强安全能力。相比现有最先进方法，B-DPO在安全性能方面表现更优。

Conclusion: B-DPO通过解决安全对齐中的过拟合问题，有效提升了模型的安全性能，同时保持了良好的通用能力，为大语言模型的安全对齐提供了新的解决方案。
...
### [58] [PhySe-RPO: Physics and Semantics Guided Relative Policy Optimization for Diffusion-Based Surgical Smoke Removal](https://arxiv.org/abs/2603.22844)
...
Method: 提出PhySe-RPO扩散修复框架，将确定性修复转化为随机策略，通过物理和语义引导的相对策略优化实现轨迹级探索和无批评器更新。物理引导奖励确保光照和颜色一致性，从CLIP手术概念学习的视觉概念语义奖励促进无烟雾且解剖一致的修复，结合无参考感知约束。

Result: PhySe-RPO在合成和真实机器人手术数据集上产生物理一致、语义忠实且临床可解释的结果，为有限配对监督下的鲁棒扩散修复提供了原则性途径。

Conclusion: 该框架通过将修复转化为策略优化问题，实现了在有限监督条件下的高质量手术烟雾去除，为手术视频增强提供了新的技术路线。
...
### [59] [CoMaTrack: Competitive Multi-Agent Game-Theoretic Tracking with Vision-Language-Action Models](https://arxiv.org/abs/2603.22846)
*Youzhi Liu,Li Gao,Liu Liu,Mingyang Lv,Yang Cai*

Main category: cs.AI

TL;DR: CoMaTrack是一个基于竞争博弈的多智能体强化学习框架，用于提升具身视觉跟踪任务的适应性和抗干扰能力，并提出了首个竞争性EVT基准测试。
...
Conclusion: 竞争博弈的多智能体强化学习框架能有效提升具身视觉跟踪任务的性能，通过动态对抗训练产生更强的适应性和抗干扰能力。CoMaTrack-Bench为评估在主动对抗交互下的鲁棒性提供了标准化基准。
...
### [60] [Chain-of-Authorization: Internalizing Authorization into Large Language Models via Reasoning Trajectories](https://arxiv.org/abs/2603.22869)
*Yang Li,Yule Liu,Xinlei He,Youjian Zhao,Qi Li,Ke Xu*

Main category: cs.AI

TL;DR: CoA框架将授权逻辑内化到LLM中，通过结构化授权推理轨迹实现动态权限控制，解决传统方法无法区分细粒度权限的问题


<details>
  <summary>Details</summary>
Motivation: 现有LLM缺乏对知识所有权和访问边界的认知，存在敏感数据泄露和对抗性操纵风险，而现有保护策略要么过于僵化无法动态授权，要么难以区分细粒度权限
...
TL;DR: 研究人员开发了一个结合临床医生专业知识和大语言模型的框架，用于从胸部X光报告中提取多标签发现，并定义了Ran Score这一发现级指标来评估报告质量。该方法显著提高了与放射科医生参考标准的匹配度，特别是在低患病率异常检测方面。


<details>
  <summary>Details</summary>
Motivation: 当前胸部X光报告生成和自动评估存在局限性：对低患病率异常的识别能力差，以及对否定词和模糊语言等临床重要语言的处理不足。需要开发更准确、更符合临床需求的评估方法。

Method: 开发了一个临床医生指导的框架，结合人类专业知识和大语言模型，从自由文本胸部X光报告中提取多标签发现。使用三个非重叠的MIMIC-CXR-EN队列和一个独立的ChestX-CN验证队列，优化提示词，建立放射科医生参考标签，并评估报告生成模型。

Result: 优化后的框架将MIMIC-CXR-EN开发队列的宏观平均分数从0.753提高到0.956，在直接可比标签上超过CheXbert基准15.7个百分点，并在ChestX-CN验证队列上显示出稳健的泛化能力。

Conclusion: 临床医生指导的提示优化提高了与放射科医生参考标准的一致性，Ran Score能够实现报告保真度的发现级评估，特别是在低患病率异常方面具有优势。
...
TL;DR: 本文研究了在NL2SQL任务上微调大模型和小模型的效果，发现了一个反直觉的缩放现象：大模型微调收益有限且容易过拟合，而小模型通过微调和CoT增强能显著提升性能，在成本效益上更具优势。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在NL2SQL任务上表现出强大的零样本能力，但其高昂的推理成本限制了在企业中的规模化部署。本文旨在探索通过微调不同规模的模型来平衡性能与成本效益。

Method: 研究对比了微调大型模型（Gemini 2.5 Flash/Lite）和小型模型（Qwen）在NL2SQL任务上的效果。特别地，为小型模型数据集增加了显式的Chain-of-Thought推理步骤来增强训练。

Result: 大型模型微调后提升有限且容易在复杂查询上过拟合。小型模型基线从36%提升到45%，加入CoT推理后进一步跃升至54.5%。虽然仍低于大型模型的准确率，但显著降低了成本、延迟，并满足了业务关键性能阈值。

Conclusion: 通过转移推理模式，计算效率更高的小型模型能够接近生产级性能，为企业在NL2SQL任务上提供了更具成本效益的解决方案。
...
TL;DR: PersonalQ：一个统一框架，通过触发器令牌连接检查点选择和量化，实现个性化文本到图像生成的高效服务
...
Method: PersonalQ框架包含两个核心组件：1) Check-in：通过意图感知混合检索和基于LLM的重排序进行意图对齐选择，仅在多个意图可能时询问澄清问题，然后通过插入选定检查点的规范触发器重写提示；2) 触发器感知量化(TAQ)：在交叉注意力中应用触发器感知混合精度，保留触发器条件化的键/值行及其注意力权重，同时积极量化其余路径以实现内存高效推理

Result: 实验表明，PersonalQ在意图对齐方面优于检索和重排序基线方法，而TAQ相比先前的扩散PTQ方法提供了更强的压缩-质量权衡，实现了在不牺牲保真度的情况下可扩展地服务个性化检查点

Conclusion: PersonalQ通过统一的触发器令牌信号连接检查点选择和量化，解决了个性化文本到图像生成中的服务效率问题，实现了意图对齐的检查点选择和保持概念保真度的内存高效量化
...
TL;DR: 论文开发了一个基准数据集，用于检测法院判决中隐含引用法国《民法典》的情况，通过监督学习和无监督方法实现了较好的识别效果，但模型错误主要集中在专家标注存在分歧的案件上。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发计算方法来分析法律文本，特别是回答"法院隐含适用法定规则的频率"这一基本问题。这需要区分法律推理与语义相似性，填补了法律计算研究中隐含引用检测的空白。

Method: 方法包括：1）创建包含1,015个段落-条款对的专家标注基准数据集；2）使用监督集成方法进行二元分类；3）将任务重构为top-k排序问题；4）利用多模型共识进行无监督检测。

Result: 结果显示：1）专家间一致性中等（κ=0.33），43%的分歧涉及事实描述与法律推理的边界；2）监督集成方法达到F1=0.70（准确率77%）；3）68%的假阳性出现在33%的专家分歧案例中；4）无监督top-k排序在k=200时达到76%的精确率。

Conclusion: 结论表明：1）专家分歧能预测模型失败；2）尽管存在限制，通过任务重构和多模型共识仍能获得实用结果；3）剩余的假阳性往往揭示法律上的模糊应用而非明显错误，这为法律分析提供了有价值的洞见。
...
<details>
  <summary>Details</summary>
Motivation: 在复杂系统维护中，故障树用于定位问题和提供针对性解决方案。为了让以图像形式存储的故障树能够直接被大语言模型处理，以协助跟踪和分析故障

Method: 提出故障树的新型文本表示方法，基于此构建多轮对话基准系统，训练端到端模型生成模糊信息反映用户行为，引入长程回滚和恢复过程模拟用户错误场景

Result: 构建了包含3130个条目、平均每条目40.75轮对话的基准，能够评估模型在任务跟踪和错误恢复方面的综合能力，Gemini 2.5 pro取得了最佳性能

Conclusion: 提出的故障树文本表示方法和多轮对话基准系统有效评估了语言模型在复杂故障定位场景中的能力，为系统维护中的AI辅助故障分析提供了新方法
...
TL;DR: 该研究提出了用于牙科记录自动比较的聚合机制，通过数据驱动的词典排序、模糊逻辑和机器学习方法，在保持可解释性的同时显著提升了识别性能。
...
Method: 研究引入了三种聚合方法：1) 基于数据驱动的词典排序聚合；2) 已知的模糊逻辑聚合方法；3) 机器学习技术作为聚合机制。所有方法都基于包含七个不同标准的最新编码系统。

Result: 使用来自两个不同人群的215个法医案例进行验证。结果显示，使用白盒机器学习技术作为聚合模型（平均排名2.02-2.21）能够显著改进现有技术水平（平均排名3.91），同时不损害方法的可解释性。

Conclusion: 白盒机器学习技术可以作为有效的聚合机制，在保持法医牙科记录比较方法可解释性和可验证性的同时，显著提升识别性能，为法医鉴定提供了更可靠的工具。
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLM在多种自然语言任务中表现出色，但其在约束条件下的抽象推理和优化能力尚未得到充分探索，特别是在电力系统优化等实际工程问题中。

Method: 引入了一个具有挑战性的评估框架，要求模型具备推理、结构化输入处理、算术和约束优化等基本技能，用于测试LLM在最优潮流问题上的表现。

Result: 评估显示，最先进的LLM在大多数任务中失败，即使具备推理能力的LLM在最复杂场景下仍然表现不佳。

Conclusion: LLM在约束条件下的结构化推理能力存在严重不足，该研究为开发能够处理实际电网优化问题的更强大LLM助手提供了严格的测试环境。
...
### [68] [MedCausalX: Adaptive Causal Reasoning with Self-Reflection for Trustworthy Medical Vision-Language Models](https://arxiv.org/abs/2603.23085)
*Jianxin Lin,Chunzheng Zhu,Peter J. Kneuertz,Yunfei Bai,Yuan Xue*

Main category: cs.AI

TL;DR: MedCausalX是一个端到端的医学视觉语言模型框架，通过显式建模因果推理链来解决现有医学CoT模型缺乏因果推理机制的问题，在多个基准测试中表现优异。
...
Method: 1) 引入CRMed数据集，提供细粒度解剖标注、结构化因果推理链和反事实变体；2) 采用两阶段自适应反思架构，配备<causal>和<verify>标记，使模型能自主决定何时以及如何进行因果分析和验证；3) 通过错误归因强化学习优化的轨迹级因果校正目标来精炼推理链。

Result: 在多个基准测试中，MedCausalX始终优于最先进方法：诊断一致性提高+5.4分，幻觉减少超过10分，获得最高的空间定位IoU，为因果基础的医学推理设定了新标准。

Conclusion: MedCausalX通过显式建模因果推理链，有效解决了医学视觉语言模型中的因果推理挑战，显著提高了诊断可靠性和临床实用性，为医学人工智能的发展提供了重要方向。
...
### [69] [Between Rules and Reality: On the Context Sensitivity of LLM Moral Judgment](https://arxiv.org/abs/2603.23114)
...
Method: 引入Contextual MoralChoice数据集，包含结果主义、情感和关系三种已知会影响人类判断的情境变化；评估22个LLM；与人类调查比较；使用激活引导方法来控制模型的情境敏感性。

Result: 几乎所有模型都具有情境敏感性，倾向于转向违反规则的行为；模型与人类对情境变化的触发点不同；在基础案例中与人类判断对齐的模型不一定在情境敏感性上对齐；激活引导方法可以可靠地增加或减少模型的情境敏感性。

Conclusion: LLM道德决策具有情境敏感性，但与人类模式不同，需要控制这种敏感性以确保模型道德判断的可靠性，激活引导方法为此提供了可行方案。
...
### [70] [Describe-Then-Act: Proactive Agent Steering via Distilled Language-Action World Models](https://arxiv.org/abs/2603.23149)
...
TL;DR: DILLO提出了一种无需视觉模拟的快速故障预防方法，通过语言模型预测动作结果，实现14倍加速，在多个任务中平均提升9.3个百分点的成功率。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉模拟的世界模型在安全关键应用中存在延迟过高的问题（每秒数秒），需要寻找更高效的故障预防方法。

Method: DILLO采用跨模态蒸馏训练：使用特权视觉语言模型标注离线轨迹，然后训练潜在条件大语言模型预测语义结果，创建纯文本推理路径，完全绕过视觉生成。

Result: 在MetaWorld和LIBERO上的实验表明，DILLO能产生高保真的下一状态描述，能够引导策略，将任务成功率平均提升9.3个百分点，最高提升15个百分点，相比基线实现14倍加速。

Conclusion: 训练好的策略潜在状态结合计划动作已包含足够信息来预测动作结果，视觉模拟对故障预防是冗余的；纯文本推理路径能显著提升效率并改善性能。
...
TL;DR: MemCollab是一个协作式记忆框架，通过对比不同智能体在同一任务上的推理轨迹，构建与智能体无关的记忆系统，使异构LLM智能体能够共享记忆资源。


<details>
  <summary>Details</summary>
Motivation: 现有记忆方法通常以单个智能体为单位构建记忆，将存储的知识与特定模型的推理风格紧密耦合。在异构智能体部署中，需要探索能否在不同模型间共享单一记忆系统，但简单的记忆转移会导致性能下降，因为记忆会混杂任务相关知识和智能体特定偏差。

Method: 提出MemCollab框架：1）通过对比不同智能体在同一任务上的推理轨迹，提取抽象推理约束，捕捉共享的任务级不变性，同时抑制智能体特定伪影；2）引入任务感知检索机制，根据任务类别条件化记忆访问，确保推理时只使用相关约束。

Result: 在数学推理和代码生成基准测试中，MemCollab能够持续提高不同智能体的准确性和推理时效率，包括跨模态家族设置。实验表明，协作构建的记忆可以作为多样化LLM智能体的共享推理资源。

Conclusion: MemCollab成功解决了异构LLM智能体间共享记忆的挑战，通过对比不同智能体的推理轨迹构建智能体无关的记忆系统，实现了跨模型的知识重用和性能提升。
...
TL;DR: 研究人类在视觉谜题任务中如何通过创建和重用助手（中间构造）来形成高效可复用的抽象，以应对日益复杂的任务需求


<details>
  <summary>Details</summary>
Motivation: 研究人类在面对复杂任务时如何形成高效可复用的抽象，尽管存在未来不确定性，以及这种抽象过程如何帮助人们应对日益增长的任务需求

Method: 在线实验：参与者在视觉谜题任务中定义和重用助手；计算建模：使用具有库学习的程序归纳模型来估计搜索空间，分析人类决策时间和操作数量

Result: 早期参与者创建大量助手，注重完整性而非效率；随着经验积累，助手使用变得更加选择性和高效；助手访问使参与者能够解决原本困难或不可能解决的谜题；人类决策时间和操作数量与程序归纳模型估计的搜索空间相关

Conclusion: 在线库学习是人类问题解决的核心机制，使人们能够灵活地构建、优化和重用抽象，以应对日益增长的任务需求
...
<details>
  <summary>Details</summary>
Motivation: 当前NLP基准测试在LLM时代容易被误读，分数可能反映的是基准追逐、隐藏的评估选择或意外暴露测试内容，而非真实能力。封闭基准虽能缓解部分问题，但降低了透明度

Method: 提出奥林匹克式评估事件：1) 问题密封直到评估开始；2) 提交提前冻结；3) 所有参赛模型通过统一标准化框架运行；4) 评分后发布完整任务集和评估代码

Result: 该设计使强大的性能更难被"制造"，结果更容易被信任，同时保持透明度和可复现性

Conclusion: 奥林匹克式评估活动可作为传统基准测试的补充实践，提高LLM评估的可信度和透明度，使社区能够更好地从结果中学习
...
TL;DR: RelayS2S是一个混合架构，通过并行运行快速路径（端到端语音模型）和慢速路径（级联ASR-LLM管道），在保持低延迟的同时获得高质量响应。快速路径生成短前缀并立即流式传输，慢速路径生成高质量续接，通过验证器控制切换。


<details>
  <summary>Details</summary>
Motivation: 实时语音对话系统面临延迟与响应质量之间的根本矛盾。端到端语音模型响应迅速但语义较弱，级联管道响应质量高但延迟随模型规模增长。需要一种能兼顾低延迟和高响应质量的解决方案。

Method: 提出RelayS2S混合架构：1）快速路径使用双工端到端语音模型，在检测到话轮时立即生成短响应前缀并流式传输；2）慢速路径使用级联ASR-LLM管道，生成高质量续接；3）轻量级学习验证器控制切换，决定是否提交前缀或回退到慢速路径。

Result: RelayS2S在P90起始延迟方面与端到端模型相当，同时保留了99%的级联管道响应质量。随着慢速路径模型规模的扩大，优势更加明显。该架构无需修改组件架构，可作为现有级联管道的轻量级即插即用方案。

Conclusion: RelayS2S成功解决了实时语音对话系统中延迟与质量之间的矛盾，通过并行路径和智能切换机制，实现了低延迟和高响应质量的平衡，为现有系统提供了实用的改进方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动研究系统都需要人类干预优化，研究是否能让LLM自主优化自动研究过程本身，实现自我改进

Method: 提出双层自动研究框架：内层循环优化目标任务，外层循环通过生成和注入新的搜索机制（Python代码）来优化内层循环的搜索方式，两层使用相同的LLM

Result: 在Karpathy的GPT预训练基准上，外层循环使性能提升5倍（-0.045 vs -0.009 val_bpb），自主发现了组合优化、多臂老虎机和实验设计等机制，打破了内层循环的确定性搜索模式

Conclusion: 自动研究能够自我优化，这一核心原则意味着自动研究原则上可以优化任何可测量目标的任务，无需更强模型即可实现元级优化
...
### [76] [Beyond Hard Constraints: Budget-Conditioned Reachability For Safe Offline Reinforcement Learning](https://arxiv.org/abs/2603.22292)
*Janaka Chathuranga Brahmanage,Akshat Kumar*

Main category: cs.LG

TL;DR: 提出一种基于安全可达性分析的新离线安全强化学习方法，通过定义安全条件可达集来解耦奖励最大化与累积安全成本约束，避免不稳定的min/max对抗优化


<details>
  <summary>Details</summary>
Motivation: 现实世界任务需要在奖励最大化与安全约束之间取得平衡，传统方法存在不稳定min/max对抗优化问题，而现有可达性方法主要处理硬安全约束，很少扩展到累积成本约束

Method: 1) 定义安全条件可达集，解耦奖励最大化与累积安全成本约束；2) 基于此集设计无需不稳定min/max或拉格朗日优化的安全约束执行机制；3) 开发从固定数据集学习安全策略的离线安全RL算法

Result: 在标准离线安全RL基准测试和真实世界海上导航任务中，该方法匹配或优于最先进的基线方法，同时保持安全性

Conclusion: 提出的安全条件可达集方法有效解决了累积安全成本约束问题，避免了传统对抗优化方法的不稳定性，为离线安全RL提供了新的有效解决方案
...
### [77] [Efficient Embedding-based Synthetic Data Generation for Complex Reasoning Tasks](https://arxiv.org/abs/2603.22294)
*Srideepika Jayaraman,Achille Fokoue,Dhaval Patel,Jayant Kalagnanam*

Main category: cs.LG

TL;DR: 本文提出了一种基于嵌入空间的针对性采样管道，通过分析生成数据在嵌入空间的多样性和分布，发现特定邻域内示例密度与预测准确性之间的强相关性，从而提升合成数据生成的质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 合成数据生成（SDG）利用大语言模型（LLMs）来提升较小但更高效模型性能，但面临确保生成数据质量和多样性的关键挑战。现有方法在数据多样性和分布方面存在不足。

Method: 分析生成数据在嵌入空间的多样性和分布，发现示例密度与预测准确性的相关性，并基于此提出嵌入空间针对性采样管道来增强数据多样性。

Result: 在多个基准测试中，该方法一致提升了性能，证明了嵌入空间采样策略对合成数据生成质量和多样性的有效性。

Conclusion: 通过分析嵌入空间的数据分布特性，提出的针对性采样管道能够有效提升合成数据生成的质量和多样性，从而改善小规模语言模型的微调效果。
...
### [78] [Between the Layers Lies the Truth: Uncertainty Estimation in LLMs Using Intra-Layer Local Information Scores](https://arxiv.org/abs/2603.22299)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型经常表现出自信但错误的预测，需要可靠的不确定性估计。基于输出的启发式方法廉价但脆弱，而探测内部表示的方法有效但高维且难以迁移。

Method: 提出一种紧凑的、针对每个实例的不确定性估计方法，通过单次前向传播对内部表示的跨层一致性模式进行评分。

Result: 在三个模型上，该方法在分布内匹配探测性能（AUPRC差异≤-1.8%，Brier分数差异≤+4.9点）。在跨数据集迁移下持续优于探测（AUPRC增益达+2.86，Brier增益达+21.02）。在4位权重量化下保持稳健，平均改进+1.94 AUPRC点和+5.33 Brier点。

Conclusion: 该方法提供了一种轻量级、紧凑的方式来捕获LLMs中的可迁移不确定性，通过分析层间交互揭示了不同模型编码不确定性的差异。
...
### [79] [Scaling Attention via Feature Sparsity](https://arxiv.org/abs/2603.22300)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过局部窗口、核近似或令牌级稀疏性来降低Transformer长上下文计算成本，但这些方法会降低准确性。本文探索特征稀疏性这一正交方向，以在保持准确性的同时显著降低计算开销。

Method: 提出稀疏特征注意力(SFA)，将查询和键表示为k-稀疏编码，保持高维表达能力的同时降低注意力计算成本。开发FlashSFA内核，扩展FlashAttention直接在稀疏重叠上操作，无需生成密集分数矩阵。

Result: 在GPT-2和Qwen3预训练中，SFA与密集基线性能相当，速度提升达2.5倍，FLOPs和KV缓存减少近50%。在合成和下游基准测试中，SFA保持长上下文检索准确性和鲁棒性，优于特征多样性崩溃的短嵌入基线。
...
### [80] [Latent Semantic Manifolds in Large Language Models](https://arxiv.org/abs/2603.22301)
*Mohamed A. Mabrok*

Main category: cs.LG

TL;DR: 论文提出一个数学框架，将LLM隐藏状态解释为潜在语义流形上的点，其中token对应划分流形的Voronoi区域，并定义了表达性差距来衡量词汇离散化带来的语义失真。


<details>
  <summary>Details</summary>
Motivation: LLM在连续向量空间中进行内部计算，但输出离散token，这种基本不匹配的几何后果尚未被充分理解。需要建立数学框架来分析这种连续-离散转换的几何特性。

Method: 将LLM隐藏状态建模为黎曼子流形上的点，使用Fisher信息度量，token对应划分流形的Voronoi区域。定义表达性差距来衡量语义失真，并证明两个定理：有限词汇的率失真下界，以及通过余面积公式得到的表达性差距线性体积缩放定律。

Result: 在6个transformer架构（124M-1.5B参数）上验证，确认了普遍的沙漏形内在维度剖面、平滑曲率结构，以及线性差距缩放（斜率0.87-1.12，R^2 > 0.985）。边界邻近表示的硬核分布在不同规模模型中保持稳定。

Conclusion: 该几何框架为理解LLM的连续-离散转换提供了理论基础，揭示了词汇离散化的固有失真，对架构设计、模型压缩、解码策略和缩放定律具有重要启示。
...
### [81] [Research on Individual Trait Clustering and Development Pathway Adaptation Based on the K-means Algorithm](https://arxiv.org/abs/2603.22302)
*Qianru Wei,Jihaoyu Yang,Cheng Zhang,Jinming Yang*

Main category: cs.LG

TL;DR: 该研究利用K-means聚类算法分析大学生学业成绩、性格特征等数据，将学生分为四类群体，为不同特征组合的学生提供精准的职业指导建议。


<details>
  <summary>Details</summary>
Motivation: 随着信息技术发展，人工智能和机器学习在教育领域应用潜力巨大。现有职业指导方法多关注职业路径预测，但较少研究不同特征组合学生在特定职业方向的适配性。本研究旨在通过聚类分析为大学生提供更精准的职业指导。

Method: 研究分析了3000多名学生的CET-4成绩、GPA、性格特征和学生干部经历等数据，采用K-means聚类算法将学生分为四个主要群体。该算法通过最小化簇内平方误差，将特征相似的学生归为一类，确保同一簇内学生高度相似，不同簇间差异最大化。
...
### [82] [Sample Transform Cost-Based Training-Free Hallucination Detector for Large Language Models](https://arxiv.org/abs/2603.22303)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的幻觉问题是可信部署的主要障碍，需要准确、轻量且广泛适用的检测器。作者认为LLM与提示定义的分布复杂度可以作为幻觉的指标。

Method: 计算成对样本的token嵌入之间的最优传输距离，得到Wasserstein距离矩阵来衡量分布复杂度。基于该矩阵推导出两个互补信号：AvgWD（平均成本）和EigenWD（成本复杂度）。还通过教师强制方法将框架扩展到黑盒LLM。

Result: 实验表明，AvgWD和EigenWD与强不确定性基线方法竞争激烈，在不同模型和数据集上表现出互补行为，证明分布复杂度是LLM真实性的有效信号。

Conclusion: 分布复杂度是检测LLM幻觉的有效指标，提出的基于Wasserstein距离的训练免费检测器在准确性和适用性方面表现优异。
...
### [83] [Mitigating Premature Discretization with Progressive Quantization for Robust Vector Tokenization](https://arxiv.org/abs/2603.22304)
*Wenhao Zhao,Qiran Zou,Zhouhan Lin,Dianbo Liu*

Main category: cs.LG

TL;DR: ProVQ提出渐进量化方法解决VQ中过早离散化问题，通过量化难度动态调整实现从连续到离散的平滑过渡，在图像生成和蛋白质结构标记化任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有向量量化方法存在过早离散化问题，即在编码器捕获底层数据流形之前就强制离散化，这限制了模型对数据结构的有效学习。

Method: 提出渐进量化方法，将量化难度作为VQ训练中一个基本但被忽视的维度，通过课程学习方式从连续潜在空间平滑退火到离散空间，引导码本向良好扩展的流形发展。

Result: 在ImageNet-1K和ImageNet-100基准测试中改善了重建和生成性能，同时在蛋白质结构标记化任务上在StrutTokenBench排行榜上建立了新的性能上限。

Conclusion: ProVQ通过解决过早离散化问题，在多种模态上展现出广泛有效性，为生成建模提供了显著提升，特别是在复杂生物序列建模方面表现出色。
...
### [84] [A Multi-Modal CNN-LSTM Framework with Multi-Head Attention and Focal Loss for Real-Time Elderly Fall Detection](https://arxiv.org/abs/2603.22313)
*Lijie Zhou,Luran Wang*

Main category: cs.LG

TL;DR: 提出MultiModalFallDetector多模态深度学习框架，用于老年人实时跌倒检测，通过融合加速度计、陀螺仪和生理信号，结合多尺度CNN、自注意力机制和Focal Loss，在SisFall数据集上达到98.7%的F1分数，推理延迟低于50ms。


<details>
  <summary>Details</summary>
Motivation: 全球老龄化加剧对可靠健康监测系统的需求，特别是老年人跌倒检测。传统基于单模态加速度数据的方法误报率高，而传统机器学习方法需要大量手工特征工程。

Method: 提出MultiModalFallDetector多模态深度学习框架：1) 多尺度CNN特征提取器捕捉不同时间分辨率运动动态；2) 融合三轴加速度计、陀螺仪和四通道生理信号；3) 多头自注意力机制进行动态时间加权；4) 使用Focal Loss缓解严重类别不平衡；5) 引入辅助活动分类任务进行正则化；6) 从UCI HAR到SisFall数据集的迁移学习。

Result: 在SisFall数据集（60-85岁老年人真实模拟跌倒试验）上，F1分数98.7%，召回率98.9%，AUC-ROC 99.4%，显著优于传统机器学习和标准深度学习方法。在边缘设备上推理延迟低于50ms。

Conclusion: 提出的多模态深度学习框架在老年人跌倒检测中表现出色，高精度和低延迟使其适合老年护理环境的实时部署。
...
### [85] [Emergency Preemption Without Online Exploration: A Decision Transformer Approach](https://arxiv.org/abs/2603.22315)
*Haoran Su,Hanxiao Deng,Yandong Sun*

Main category: cs.LG

TL;DR: 本文提出了一种基于决策变换器的返回条件框架，用于优化应急车辆走廊，通过离线序列建模实现调度级紧急控制，无需在线环境交互。


<details>
  <summary>Details</summary>
Motivation: 应急车辆响应时间是生存结果的关键决定因素，但现有的信号优先策略仍然是反应性和不可控的，需要更智能、可控的优化方法。

Method: 将走廊优化建模为离线、返回条件的序列建模问题，使用决策变换器框架，通过单一目标返回标量实现调度级紧急控制，并扩展到多智能体设置，使用带图注意力的多智能体决策变换器进行空间协调。

Result: 在LightSim模拟器上，决策变换器在4x4网格上将平均应急车辆行驶时间减少了37.7%（88.6秒 vs 142.3秒），实现了最低的民用车辆延误（11.3秒/车）和最少的应急车辆停车次数（1.2次）。多智能体决策变换器在更大网格上进一步改进，在8x8网格上实现了45.2%的减少。

Conclusion: 返回条件框架为应急车辆走廊优化提供了有效的离线解决方案，通过单一目标返回标量实现平滑的调度控制，无需重新训练即可在应急车辆行驶时间和民用车辆延误之间进行权衡，扩展的多智能体版本进一步提升了大规模场景的性能。
...
TL;DR: ST-GDance++是一个可扩展的群体舞蹈生成框架，通过解耦空间和时间依赖关系，实现高效且碰撞感知的群体编舞生成，显著降低延迟并提高可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有群体舞蹈生成模型在交互场景中部署困难，因为双向注意力依赖导致计算复杂度随舞者数量和序列长度呈二次增长，效率低下且容易产生运动碰撞。需要有效建模密集的时空交互，但现有方法难以捕捉这种复杂性，导致可扩展性有限和多舞者协调不稳定。

Method: 提出ST-GDance++框架，将空间和时间依赖关系解耦：1）空间建模采用轻量级距离感知图卷积来捕捉舞者间关系并减少计算开销；2）时间建模设计扩散噪声调度策略和高效的时间对齐注意力掩码，支持长运动序列的流式生成，提高长时场景的可扩展性。
...
TL;DR: 该论文提出一个框架来克服气候模型ML模拟器的使用障碍，通过整合气候科学和机器学习视角，设计易于采用且可靠的模拟器来弥合两个领域间的差距。


<details>
  <summary>Details</summary>
Motivation: 气候模型对气候决策很重要，但计算和技术需求高；ML模拟器能降低计算成本，但实际使用面临多重障碍：可访问性有限、缺乏专业知识、对ML方法物理基础不足的不信任。

Method: 提出一个整合气候科学和机器学习视角的框架，设计易于采用的模拟器，这些模拟器针对明确定义的任务，并展示其可靠性。

Result: 研究发现，设计易于采用、针对明确定义任务且能证明可靠性的模拟器，为弥合气候科学和机器学习领域间的差距提供了有希望的途径。

Conclusion: 通过整合气候科学和机器学习视角，设计易于采用且可靠的ML模拟器，可以有效克服气候模型使用中的障碍，促进两个领域的融合。
...
TL;DR: AEGIS是一个医疗AI治理框架，将FDA和欧盟的监管要求转化为可执行程序，支持医疗AI系统的安全持续学习。


<details>
  <summary>Details</summary>
Motivation: 医疗AI系统需要治理框架来确保安全同时支持持续改进。FDA和欧盟已引入PCCP和PMS等机制来管理模型迭代更新，但需要具体可操作的治理框架来实现这些监管要求。

Method: 提出AEGIS治理框架，包含三个模块：数据集同化和重新训练、模型监控、条件决策。采用四类部署决策分类（批准、有条件批准、临床审查、拒绝）和独立的PMS警报信号。框架可跨不同临床场景实例化，在脓毒症预测和脑肿瘤分割两个案例中展示应用。

Result: 在脓毒症预测的11次模拟迭代中，AEGIS产生8次批准、1次有条件批准、1次临床审查和1次拒绝决策，覆盖所有四类决策。警报信号在第8和10次迭代发出，包括关键状态（无可用部署模型且已发布模型同时失效）。AEGIS在可观察性能下降前检测到漂移。
...
TL;DR: Delta-Aware Quantization (DAQ) 是一种无需数据的训练后量化框架，通过保护训练后获得的参数变化来保持模型性能，使用符号保持率和余弦相似度替代传统重建误差目标。


<details>
  <summary>Details</summary>
Motivation: 标准量化方法最小化重建误差，但忽略了基础模型，导致量化噪声不成比例地破坏编码训练后行为的小幅度参数变化(ΔW)，从而损害模型在训练后获得的能力。

Method: DAQ 使用两种ΔW感知的度量指标替代基于重建的目标：符号保持率（Sign Preservation Rate）和余弦相似度（Cosine Similarity），直接优化ΔW的方向保真度，仅需要基础模型和训练后模型的权重矩阵。

Result: 在FP8的初步研究中，DAQ恢复了在标准量化下丢失的特定风格能力，同时保持了通用性能。

Conclusion: DAQ通过直接保护训练后获得的参数变化方向，有效解决了标准量化方法破坏训练后模型能力的问题，为训练后量化提供了新的优化方向。
...
TL;DR: HAM层结合了RNN和自注意力的优势：RNN压缩整个序列，注意力仅补充RNN难以预测的最有价值信息，实现KV缓存的数据依赖增长和用户可控调节。


<details>
  <summary>Details</summary>
Motivation: RNN和自注意力在序列建模中各有优缺点：RNN将整个过去压缩为固定大小状态，效率高但长上下文性能下降；自注意力存储每个过去时间步，KV缓存随序列长度线性增长，检索能力强但内存和计算成本高。现有工作简单交替使用两者，未充分利用它们的互补机制。

Method: 提出混合关联记忆（HAM）层，结合RNN和自注意力的优势：RNN压缩整个序列，注意力仅补充RNN难以预测的最有价值信息。HAM层实现KV缓存的数据依赖增长，用户可通过单一连续阈值精确控制缓存增长速率。

Result: KV缓存增长速率与损失和性能之间存在平滑权衡关系。实验表明，混合架构在显著降低KV缓存使用量的情况下，相对于RNN和Transformer仍能提供强大、有竞争力的性能。

Conclusion: HAM层通过结合RNN的压缩能力和自注意力的精确检索能力，实现了更高效的序列建模，在KV缓存使用和性能之间提供了灵活可控的权衡。
...
TL;DR: 该论文提出了一种针对风功率爬坡事件预测的新方法，通过多数类欠采样和集成学习解决数据类别不平衡问题，在真实数据集上取得了85%准确率和88%加权F1分数的优异性能。


<details>
  <summary>Details</summary>
Motivation: 在低碳电力系统中，风功率爬坡事件预测对电网稳定性至关重要，但现有方法面临数据类别不平衡的挑战——爬坡事件仅占观测事件的不到15%，导致传统机器学习模型偏向多数类，预测性能受限。

Method: 提出一种新颖的多变量时间序列分类方法：1）数据预处理策略，从近期功率观测中提取特征并屏蔽不可用的爬坡信息；2）结合多数类欠采样和集成学习技术，专门针对类别不平衡问题优化风功率爬坡事件预测。

Result: 在真实世界数据集上的数值模拟表明，该方法在风功率爬坡事件预测方面表现优越，准确率超过85%，加权F1分数达到88%，显著优于基准分类器。

Conclusion: 该研究提出的结合欠采样和集成学习的方法能有效解决风功率爬坡事件预测中的类别不平衡问题，为电网控制室操作员提供可靠的实时预警，有助于及时启动系统稳定性评估和预防措施。
...
TL;DR: 研究探讨了在仅解码器语言模型中实现持久内存的方法，通过训练小型内存适配器使冻结的GPT-2获得跨会话的记忆能力，发现不同架构方法在内存保留效果上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 仅解码器语言模型（如GPT-2）是无状态的，每次前向传播后隐藏表示都会被丢弃，无法跨会话保持记忆。本研究旨在探索是否能在仅解码器架构中实现持久内存，特别是在没有交叉注意力机制的情况下，只能通过自注意力注入记忆。

Method: 将六种内存方法（前缀、并行交叉注意力、KV扩展、Hebbian记忆、上下文门控分支、基于槽的稀疏写入）适配到冻结的GPT-2模型中，只训练小型内存适配器θ_mem。写入规则共享，读取注入从解码器的交叉注意力改为自注意力的KV前缀或并行分支。在LoCoMo基准上进行测试。

Result: 在1倍容量下，三种具有强架构先验的方法（交叉注意力、Hebbian记忆、槽写入）实现了7-18%的保留内存分数和7-10的知识增益ΔK，而其他三种方法几乎失败（<0.4%）。在10倍容量下，所有六种方法都收敛，表明差距是架构性的而非根本性的。

Conclusion: 研究证明了持久潜在空间内存可以扩展到仅解码器Transformer架构，与Jeong (2026a)的编码器-解码器结果以及Jeong (2026b,c)的脑启发模块一起，确立了持久潜在空间内存作为跨越主要Transformer家族的通用范式。
...
TL;DR: 首次将保形风险控制应用于野火蔓延预测，为假阴性率提供有限样本保证，发现标准阈值仅捕获7-72%真实火势，而CRC方法能实现95%火势覆盖且仅标记15%像素


<details>
  <summary>Details</summary>
Motivation: 现有野火预测模型缺乏对漏报火势蔓延的正式保证，疏散规划者依赖没有正式保证的概率阈值，存在安全隐患

Method: 应用保形风险控制到野火蔓延预测，比较三种模型架构（LightGBM、Tiny U-Net、Hybrid ResGNN-UNet），提出感知偏移的三向CRC框架划分安全/监控/疏散区域

Result: 标准阈值仅捕获7-72%真实火势，CRC方法能实现假阴性率≤0.05的保证，空间模型+CRC达到约95%火势覆盖且仅标记约15%像素，比LightGBM效率高4.2倍
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大型语言模型的缺失值填补研究存在可扩展性挑战、跨模型比较有限、评估数据集小或领域特定等问题，且实验协议和缺失机制处理不一致，缺乏系统性基准测试。

Method: 采用零样本提示工程方法，对5个广泛使用的LLMs与6个最先进的填补基线方法进行比较，在29个数据集（包括9个合成数据集）上评估MCAR、MAR和MNAR三种缺失机制，缺失率最高达20%。

Result: 领先的LLMs（特别是Gemini 3.0 Flash和Claude 4.5 Sonnet）在真实开源数据集上持续优于传统方法，但在合成数据集上传统方法如MICE表现更好；LLMs的有效性依赖于语义上下文而非纯统计重建，且存在计算时间和经济成本显著更高的权衡。

Conclusion: LLMs作为语义驱动的填补器在复杂表格数据中展现出潜力，但其优势与预训练期间学习的领域特定模式密切相关，且需要在填补质量与计算成本之间进行权衡。
...
TL;DR: Chinchilla Approach 2存在系统性偏差，导致计算最优分配估计不准确，造成显著计算资源浪费；Approach 3能消除这些偏差，且通过Variable Projection方法可以解决其实现困难的问题。


<details>
  <summary>Details</summary>
Motivation: Chinchilla Approach 2作为拟合神经缩放定律的常用方法，其抛物线近似在计算最优分配估计中引入系统性偏差，即使在无噪声合成数据上也是如此。这些偏差导致实际训练中的参数分配不当和计算资源浪费。
...
Result: 在Llama 3 IsoFLOP数据上，Approach 2的偏差导致参数分配不足，相当于浪费了3.8×10²⁵ FLOP训练预算的6.5%和140万美元计算成本。模拟的多模态模型分配不当显示出更大的机会成本。改进后的Approach 3消除了这些偏差，且实现更稳定。

Conclusion: Chinchilla Approach 2存在显著偏差问题，而Approach 3通过Variable Projection方法可以成为更可靠的替代方案，既能消除偏差，又能解决传统实现中的数值不稳定和实现困难问题，为更丰富的缩放定律公式提供可扩展的替代方案。
...
TL;DR: 提出了一种风险感知的云边协同框架，用于延迟敏感的光伏功率预测，通过轻量级筛选模块评估不确定性，并在本地专家模型、边缘小模型和云端大模型之间智能路由，平衡预测精度、鲁棒性和系统效率。


<details>
  <summary>Details</summary>
Motivation: 边缘电网中的光伏功率预测需要在预测精度、天气驱动分布偏移下的鲁棒性以及严格延迟约束之间取得平衡。本地专用模型在常规条件下高效，但在罕见爬坡事件和未见天气模式时性能下降，而始终依赖云端大模型会产生显著的通信延迟和云端开销。

Method: 提出风险感知的云边协同框架，包含：1）针对常规情况的站点特定专家预测器；2）用于增强本地推理的轻量级边缘侧模型；3）云端大型检索模型，通过检索-预测管道在需要时提供匹配的历史上下文。轻量级筛选模块评估预测不确定性、分布外风险、天气突变强度和模型分歧，而Lyapunov引导的路由器在长期延迟、通信和云端使用约束下，选择性地将推理升级到边缘小模型或云端辅助分支。激活分支的输出通过自适应融合进行组合。

Result: 在两个真实世界光伏数据集上的实验表明，该框架在预测精度、路由质量、鲁棒性和系统效率之间取得了良好的整体权衡。

Conclusion: 提出的风险感知云边协同框架有效解决了延迟敏感光伏预测中的挑战，通过智能路由和自适应融合机制，在保证预测性能的同时满足了系统效率约束。
...
TL;DR: 该论文识别并实证表征了梯度提升中的"先动者偏差"——一种由顺序残差拟合导致的特征重要性路径依赖集中现象，这是SHAP特征排序在多共线性下不稳定的具体机制原因。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解释为什么在多共线性存在时，基于SHAP的特征排序会不稳定。作者发现梯度提升算法中，当相关特征竞争早期分割时，会产生自我强化的先动者优势，导致SHAP重要性集中在任意选择的特征上，而不是分布在相关特征组中。

Method: 提出了DASH（Diversified Aggregation of SHAP）方法，通过模型独立性来打破顺序依赖链。同时测试了简单的种子平均方法（Stochastic Retrain）。还开发了两个诊断工具：特征稳定性指数（FSI）和重要性-稳定性（IS）图，用于检测先动者偏差。

Result: 在rho=0.9时，DASH和种子平均方法都实现了稳定性0.977，而单一最佳工作流降至0.958，大型单一模型降至0.938。在乳腺癌数据集上，DASH将稳定性从0.32提高到0.93（提升0.61）。模型独立性被证明足以在线性机制中解决先动者偏差，在非线性数据生成过程中也是最有效的缓解措施。

Conclusion: 先动者偏差是SHAP特征排序在多共线性下不稳定的具体机制原因。通过模型独立性（如DASH方法）可以打破顺序依赖链，恢复稳定性。DASH提供的诊断工具使从业者能够在依赖特征排序之前审计解释可靠性。
...
TL;DR: COMPASS-Hedge算法首次在完全信息设置中同时实现对抗环境下的极小极大最优遗憾、随机环境下的实例最优遗憾、以及相对于基准策略的近似零遗憾，无需任何先验参数知识。


<details>
  <summary>Details</summary>
Motivation: 在线学习算法面临一个基本的三难困境：需要在对抗环境和随机环境之间平衡遗憾保证，同时提供相对于固定比较器的基准安全性。现有方法通常只能在一两个领域表现出色，无法同时满足所有三个要求而不牺牲最优速率或需要问题相关参数的先知知识。

Method: COMPASS-Hedge算法通过新颖的自适应伪遗憾缩放和基于阶段的激进策略相结合，加上比较器感知的混合策略，实现了参数自由的设计，无需环境性质或随机次优差距大小的先验知识。

Result: 该算法首次在完全信息设置中实现了"三世界最佳"保证：1) 对抗环境下的极小极大最优遗憾；2) 随机环境下的实例最优、差距依赖遗憾；3) 相对于指定基准策略的$\tilde{\mathcal{O}}(1)$遗憾（对数因子内）。

Conclusion: COMPASS-Hedge证明了基准安全性不必以牺牲最坏情况鲁棒性或随机效率为代价，为在线学习算法设计提供了新的理论框架，解决了长期存在的三难困境。
...
TL;DR: 该研究提出了一种基于神经正切核理论的波形反演统一框架，揭示了连续表示FWI缓解初始模型依赖性的机制，并提出了一种结合隐式神经表示和多分辨率网格的混合方法，在多个地球物理勘探模型上取得了优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统全波形反演方法对初始模型精度敏感，而基于隐式神经表示的连续表示FWI虽然能缓解这一问题，但其机制不明确且高频收敛速度较慢。需要从理论上理解CR-FWI的工作原理，并改进其性能。

Method: 1) 将神经正切核理论扩展到FWI领域，建立了基于波的NTK框架；2) 分析了波基NTK的特征值衰减行为，解释了CR-FWI的机制；3) 提出了多种具有定制特征值衰减特性的CR-FWI方法，包括结合INR和多分辨率网格的IG-FWI混合表示方法。

Result: 1) 揭示了波基NTK在初始化和训练过程中都不是常数，这是由FWI固有的非线性导致的；2) 波基NTK的特征值衰减行为解释了为什么CR-FWI能缓解初始模型依赖性且高频收敛较慢；3) 提出的IG-FWI方法在Marmousi、2D SEG/EAGE Salt和Overthrust、2004 BP模型以及更真实的2014 Chevron模型上，相比传统FWI和现有INR-based FWI方法表现出优越性能。

Conclusion: 通过建立基于波的NTK理论框架，本文为连续表示FWI提供了统一的理论理解，解释了其缓解初始模型依赖性的机制，并提出了一种新的混合表示方法IG-FWI，在鲁棒性和高频收敛速度之间取得了更好的平衡，在地球物理勘探应用中表现出优越性能。
...
TL;DR: 提出FAAR方法，针对NVFP4格式的非均匀数值网格进行自适应舍入优化，配合2FA微调方案，显著降低量化误差，在极低训练开销下提升LLM在超低比特量化下的性能。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备部署大型语言模型需要极低比特量化，NVFP4等超低精度格式虽能减少内存占用和加速计算，但现有量化方法使用传统舍入策略，未考虑NVFP4数值网格的非均匀性，导致次优舍入决策和量化误差放大。

Method: 提出格式感知自适应舍入（FAAR），将NVFP4的非均匀网格显式纳入优化过程，通过损失梯度指导自适应调整舍入决策。配合2阶段格式对齐（2FA）微调方案，逐层将LLM参数对齐到NVFP4数值空间。

Result: 在Llama3-1B上仅需4GPU小时训练开销，相比RTN方法，WikiText-2困惑度从14.28降至12.60；在Qwen3-1.7B上从23.06降至21.27。在各种零样本下游任务中持续优于最先进方法。

Conclusion: FAAR方法有效解决了NVFP4格式非均匀网格的量化挑战，通过自适应舍入优化和格式对齐微调，在极低训练成本下显著提升超低比特量化LLM的性能，为边缘部署提供实用解决方案。
...
### [101] [Neural Structure Embedding for Symbolic Regression via Continuous Structure Search and Coefficient Optimization](https://arxiv.org/abs/2603.22429)
*Fateme Memar,Tao Zhe,Dongjie Wang*

Main category: cs.LG

TL;DR: SRCO：一种基于嵌入的符号回归框架，将符号结构转化为连续可优化的表示空间，通过梯度优化提升搜索效率和准确性


<details>
  <summary>Details</summary>
Motivation: 传统符号回归方法依赖离散结构搜索（如遗传编程），存在计算成本高、性能不稳定、难以扩展到大规模方程空间等问题。需要一种更高效、稳定的方法来发现可解释的方程。

Method: SRCO框架包含三个关键组件：1）结构嵌入：使用传统符号回归算法生成大量探索性方程，训练Transformer模型将符号结构压缩到连续嵌入空间；2）连续结构搜索：在嵌入空间中使用基于梯度或采样的优化进行高效探索；3）系数优化：将符号系数作为可学习参数，应用梯度优化获得准确数值。

Result: 在合成和真实数据集上的实验表明，该方法在方程准确性、鲁棒性和搜索效率方面一致优于现有最先进方法。

Conclusion: 这项工作通过将符号方程发现与连续嵌入学习和优化相结合，为符号回归引入了一种新范式，显著提升了符号回归的性能和效率。
...
### [102] [Model Predictive Control with Differentiable World Models for Offline Reinforcement Learning](https://arxiv.org/abs/2603.22430)
...
<details>
  <summary>Details</summary>
Motivation: 现有离线RL方法在推理时直接应用训练好的策略，无法利用推理时的环境信息进行实时优化，限制了性能提升

Method: 设计可微分世界模型管道，支持通过想象轨迹进行端到端梯度计算，在推理时基于模型预测控制优化策略参数

Result: 在D4RL连续控制基准测试（MuJoCo运动任务和AntMaze）上，相比强基线方法获得了一致的性能提升

Conclusion: 利用推理时信息优化策略参数能有效提升离线强化学习性能，可微分世界模型框架为此提供了可行方案
...
### [103] [SkillRouter: Retrieve-and-Rerank Skill Selection for LLM Agents at Scale](https://arxiv.org/abs/2603.22455)
*YanZhao Zheng,ZhenTao Zhang,Chao Ma,YuanQiang Yu,JiHuan Zhu,Baohua Dong,Hangcheng Zhu*

Main category: cs.LG

TL;DR: LLM智能体生态系统中的技能路由问题：现有架构仅暴露技能名称和描述，但研究发现技能实现文本才是关键信号，移除会导致检索性能大幅下降。作者提出SkillRouter两阶段检索重排系统，在消费级硬件上实现74%的top-1路由准确率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体生态系统发展，可用技能数量已达数万级别，无法将所有技能注入智能体上下文，需要从大规模技能池中检索最相关技能。现有架构采用渐进式披露设计，仅暴露技能元数据而隐藏实现细节，但作者质疑这种假设的有效性。

Method: 通过系统性实证研究，在约80K技能和75个专家验证查询的基准上分析技能路由问题。研究发现技能实现文本是关键信号，基于此提出SkillRouter两阶段检索重排管道：0.6B编码器进行检索，0.6B重排器进行精排，总计1.2B参数。

Result: 移除技能实现文本会导致所有检索方法性能下降29-44个百分点，交叉编码器注意力分析显示91.7%的注意力集中在技能实现字段。SkillRouter达到74.0%的top-1路由准确率，在紧凑型和零样本基线中表现最强，可在消费级硬件上部署。

Conclusion: 技能实现文本对技能路由至关重要，现有仅依赖元数据的渐进式披露设计存在缺陷。SkillRouter证明了在保持模型紧凑性的同时实现高效技能路由的可行性，为大规模LLM智能体生态系统提供了实用解决方案。
...
### [104] [A Theoretical Framework for Energy-Aware Gradient Pruning in Federated Learning](https://arxiv.org/abs/2603.22465)
*Emmanouil M. Athanasakos*

Main category: cs.LG

TL;DR: 提出了一种考虑硬件能耗的梯度稀疏化方法CWMP，相比传统Top-K剪枝在联邦学习中建立了更好的性能-能耗帕累托前沿


<details>
  <summary>Details</summary>
Motivation: 联邦学习受限于边缘设备的通信和能耗限制。传统的Top-K幅度剪枝虽然减少了通信负载，但忽略了硬件现实——假设所有参数更新具有相同的传输和内存更新成本，没有考虑内存密集型与计算高效操作之间的硬件级差异。

Method: 将剪枝过程形式化为能量约束的投影问题，考虑后向传播阶段内存密集型与计算高效操作之间的硬件差异。提出成本加权幅度剪枝(CWMP)，根据参数更新幅度相对于其物理成本的比值来优先选择参数更新。证明CWMP是该约束投影问题的最优贪婪解，并提供其全局能量效率的概率分析。

Result: 在非独立同分布的CIFAR-10基准测试中，CWMP相比Top-K基线始终建立了更优的性能-能耗帕累托前沿。

Conclusion: CWMP通过考虑硬件能耗差异，为联邦学习中的梯度稀疏化提供了更优的能量效率解决方案，超越了传统幅度剪枝方法。
...
### [105] [Adversarial Vulnerabilities in Neural Operator Digital Twins: Gradient-Free Attacks on Nuclear Thermal-Hydraulic Surrogates](https://arxiv.org/abs/2603.22525)
*Samrendra Roy,Kazuma Kobayashi,Souvik Chakraborty,Rizwan-uddin,Syed Bahauddin Alam*

Main category: cs.LG

TL;DR: 神经算子模型对边界条件的稀疏对抗攻击极其脆弱，即使只有不到1%的输入扰动也能导致灾难性预测失败，而标准验证指标无法检测。


<details>
  <summary>Details</summary>
Motivation: 神经算子模型作为核能和能源系统数字孪生的预测核心，其对抗扰动的鲁棒性尚未被充分研究，这对于安全关键系统的部署至关重要。

Method: 使用无梯度差分进化算法对四种算子架构进行攻击，引入有效扰动维度d_eff和灵敏度幅度的双因素脆弱性模型，分析不同架构的脆弱性特征。

Result: 极稀疏的物理可行扰动（少于1%输入）能将相对L2误差从约1.5%增加到37-63%，100%的单点攻击通过z-score异常检测，无梯度搜索在梯度病态架构上优于基于梯度的方法。

Conclusion: 神经算子模型存在先前被忽视的攻击面，需要在部署前建立超越标准验证的鲁棒性保证，特别是对边界条件扰动的防御。
...
### [106] [Multimodal Training to Unimodal Deployment: Leveraging Unstructured Data During Training to Optimize Structured Data Only Deployment](https://arxiv.org/abs/2603.22530)
*Zigui Wang,Minghui Sun,Jiang Shu,Matthew M. Engelhard,Lauren Franz,Benjamin A. Goldstein*

Main category: cs.LG

TL;DR: 提出一种多模态学习框架，在训练时利用非结构化电子病历数据，但部署时仅需结构化数据，用于儿童语言发育迟缓的识别任务。


<details>
  <summary>Details</summary>
Motivation: 非结构化电子病历数据（如临床笔记）包含丰富的临床上下文信息，能显著提升模型学习效果，但由于其非结构化特性，在模型部署时往往不可用或不实用。需要一种方法既能利用这些数据训练，又能部署仅依赖结构化数据的模型。

Method: 提出多模态学习框架：使用BioClinicalBERT生成临床笔记的嵌入表示，同时编码人口统计学和医疗代码的结构化嵌入。采用基于笔记的教师模型和仅依赖结构化数据的学生模型，通过对比学习和对比知识蒸馏损失进行联合训练。

Result: 在3,466名语言发育迟缓儿童的队列中，笔记基础教师模型表现优异（AUROC = 0.985）。提出的模型达到AUROC 0.705，优于仅使用结构化数据的基线模型（0.656）。

Conclusion: 训练时融入非结构化数据能增强模型从结构化电子病历数据中识别任务相关信息的能力，从而构建可部署的仅依赖结构化数据的表型识别模型。
...
### [107] [MIOFlow 2.0: A unified framework for inferring cellular stochastic dynamics from single cell and spatial transcriptomics data](https://arxiv.org/abs/2603.22564)
...
TL;DR: MIOFlow 2.0是一个整合流形学习、最优传输和神经微分方程的框架，用于从离散的单细胞转录组数据推断连续的细胞轨迹，能够建模随机分支、种群变化和空间环境影响。


<details>
  <summary>Details</summary>
Motivation: 从离散的时间点单细胞转录组数据推断连续细胞轨迹对研究发育、再生和疾病至关重要。现有方法通常使用确定性插值，将细胞孤立处理，无法捕捉真实的生物过程，包括随机细胞命运决定、时间性增殖变化和空间环境影响。

Method: MIOFlow 2.0整合了三个核心过程：1）通过神经随机微分方程建模随机性和分支；2）使用基于不平衡最优传输初始化的生长率模型建模非保守种群变化；3）通过联合潜在空间统一基因表达与空间特征（如局部细胞类型组成和信号传导）。该方法在PHATE距离匹配自编码器潜在空间中操作，确保轨迹尊重数据的固有几何结构。

Result: 在合成数据集、胚状体分化和空间分辨的蝾螈脑再生数据上的验证表明，MIOFlow 2.0通过神经微分方程实现了表达性轨迹学习，优于现有的生成模型（包括无模拟流匹配）。该方法提高了轨迹准确性，并揭示了细胞转变的隐藏驱动因素，如特定的信号传导生态位。
...
### [108] [A Foundation Model for Instruction-Conditioned In-Context Time Series Tasks](https://arxiv.org/abs/2603.22586)
*Anish Saha,Konstantin Shmakov*

Main category: cs.LG

TL;DR: 基于T5编码器-解码器的时序基础模型，通过指令条件上下文学习实现多任务时序分析，无需任务特定微调


<details>
  <summary>Details</summary>
Motivation: 现有时序基础模型主要依赖隐式位置上下文、检索或任务特定目标，缺乏显式的指令条件演示。需要一种能够通过上下文学习在推理时适应多种时序任务的方法，而不需要更新模型参数。

Method: 使用基于分位数回归的T5编码器-解码器架构，采用结构化标记化方案编码历史示例和查询，包含目标序列、协变量、上下文和任务特定未来信息。使用分层Transformer，包含每个示例编码、示例级融合和跨示例注意力，使解码能够基于演示对进行条件化。

Result: 在多样化的数据集、频率和时间范围上，该方法在点预测和概率预测基准测试（包括fev-bench和GIFT-Eval）中优于强基础基线，同时在分类和异常检测任务上保持竞争力。

Conclusion: 提出的指令条件上下文学习时序基础模型能够通过多任务训练学习任务映射分布，在推理时更好地适应局部结构，实现无需任务特定微调的高性能多任务时序分析。
...
### [109] [Causal Discovery in Action: Learning Chain-Reaction Mechanisms from Interventions](https://arxiv.org/abs/2603.22620)
...
<details>
  <summary>Details</summary>
Motivation: 在一般动态系统中，因果发现具有挑战性，因为即使使用干预数据，如果没有强结构假设，底层因果图也可能无法识别。然而，许多现实世界系统表现出定向的、级联状结构，其中组件按顺序激活，上游故障抑制下游效应。

Method: 研究链式反应系统中的因果发现，证明因果结构可以通过阻断个体组件激活的干预来唯一识别。提出一个具有有限样本保证的最小估计器，实现指数级错误衰减和对数样本复杂度。

Result: 在合成模型和多样化的链式反应环境中的实验表明，该方法能够从少量干预中可靠地恢复因果结构，而观测启发式方法在具有延迟或重叠因果效应的机制中会失败。

Conclusion: 链式反应系统的定向级联结构使得因果结构可以通过阻断干预唯一识别，提出的估计器具有理论保证和实际有效性，为这类系统的因果发现提供了可靠方法。
...
### [110] [Transfer learning via interpolating structures](https://arxiv.org/abs/2603.22621)
*T. A. Dardeno,A. J. Hughes,L. A. Bull,R. S. Mills,N. Dervilis,K. Worden*

Main category: cs.LG

TL;DR: 该研究提出了一种通过中间结构实现高度异质结构间知识迁移的方法，解决了PBSHM中异质结构间迁移的挑战


<details>
  <summary>Details</summary>
Motivation: 尽管基于群体的结构健康监测（PBSHM）取得了进展，但在高度异质结构（异质群体）之间的知识迁移仍然是一个挑战。当前研究旨在解决这一挑战

Method: 提出通过中间结构桥接异质结构间信息差距的方法，通过改变材料属性和几何参数等，将一个结构连续变形为另一个结构。通过两个案例研究验证：案例1涉及模拟异质桥梁设计的参数化和迁移；案例2通过有限元模型链演示了"桥梁"和"飞机"简化物理表示之间的迁移

Result: 研究结果表明，在某些情况下，即使在高度异质的系统之间，也能实现正向知识迁移。这回答了"桥梁何时不是飞机？"的问题，虽然明显答案是"总是"，但研究显示在某些情况下可以实现正向迁移

Conclusion: 通过中间结构实现连续变形的方法可以有效解决PBSHM中异质结构间的知识迁移问题，证明了即使在高度不同的系统之间也能实现正向迁移
...
### [111] [Generalizing Dynamics Modeling More Easily from Representation Perspective](https://arxiv.org/abs/2603.22655)
*Yiming Wang,Zhengnan Zhang,Genghe Zhang,Jiawen Dan,Changchun Li,Chenlong Hu,Chris Nugent,Jun Liu,Ximing Li,Bo Yang*

Main category: cs.LG

TL;DR: 提出PDEDER预训练动力学编码器，通过最小化Lyapunov指数目标约束潜在空间中的混沌行为，实现跨复杂系统的通用动力学建模


<details>
  <summary>Details</summary>
Motivation: 现有神经动力学建模方法为每个复杂系统的观测数据单独训练模型，缺乏跨系统的泛化能力，需要一种通用的预训练方法来学习系统动力学

Method: 提出PDEDER预训练动力学编码器，通过最小化Lyapunov指数目标约束潜在空间混沌行为，结合重构和预测目标防止过平滑，使用152组真实和合成观测数据进行预训练

Result: 在12个动态系统的短/长期预测任务上评估，包括域内和跨域设置，实证结果表明PDEDER具有有效性和泛化能力

Conclusion: PDEDER通过预训练学习通用动力学表示，能够在潜在空间中更有效地建模复杂系统动力学，为跨系统的动力学学习提供了有前景的解决方案
...
### [112] [Vision-based Deep Learning Analysis of Unordered Biomedical Tabular Datasets via Optimal Spatial Cartography](https://arxiv.org/abs/2603.22675)
*Sakib Mostafa,Tarik Massoud,Maximilian Diehn,Lei Xing,Md Tauhidul Islam*

Main category: cs.LG

TL;DR: Dynomap是一个端到端的深度学习框架，通过动态特征映射将无序的表格数据转换为可学习的空间拓扑结构，使视觉架构能够有效处理非空间生物医学数据。
...
Method: 提出动态特征映射（Dynomap）框架，通过完全可微的渲染机制联合优化特征放置和预测，无需依赖启发式方法、预定义分组或外部先验知识，将高维表格向量转换为学习的特征图。

Result: 在多个临床和生物数据集上，Dynomap始终优于传统机器学习、现代深度表格模型和现有向量到图像方法。在液体活检数据中，将临床相关基因特征组织成连贯的空间模式，多类癌症亚型预测准确率提高达18%；在帕金森病语音数据中，聚类疾病相关声学描述符，准确率提高达8%。

Conclusion: Dynomap为连接表格和基于视觉的深度学习提供了一种通用策略，能够揭示高维生物医学数据中的结构化、临床相关模式。
...
### [113] [Coordinate Encoding on Linear Grids for Physics-Informed Neural Networks](https://arxiv.org/abs/2603.22700)
*Tetsuro Tsuchino,Motoki Shiga*

Main category: cs.LG

TL;DR: 该研究提出了一种基于PINN的方法，通过在线性网格单元上添加坐标编码层，解决了PINN训练收敛慢的问题，提高了训练效率和稳定性。


<details>
  <summary>Details</summary>
Motivation: PINN在求解偏微分方程时面临训练收敛速度显著缓慢的问题，这主要是由于谱偏差问题导致的。虽然PINN具有网格无关解、无监督学习和处理高维问题的优势，但训练效率低限制了其实际应用。

Method: 提出了一种基于PINN的方法，在线性网格单元上添加坐标编码层。通过网格单元分离局部域来提高训练收敛速度，使用轴独立的线性网格单元降低计算成本，并采用自然三次样条插值网格点间的编码坐标，保证模型导数函数的连续性。

Result: 数值实验结果表明，所提出的方法在性能表现和训练收敛速度方面都表现出有效性，能够实现高效稳定的模型训练。

Conclusion: 该方法通过坐标编码层和网格单元分离策略，有效解决了PINN训练收敛慢的问题，为基于物理定律的机器学习方法在求解偏微分方程方面提供了更高效的解决方案。
...
### [114] [Non-Adversarial Imitation Learning Provably Free of Compounding Errors: The Role of Bellman Constraints](https://arxiv.org/abs/2603.22713)
*Tian Xu,Chenyang Wang,Xiaochen Zhai,Ziniu Li,Yi-Chen Li,Yang Yu*

Main category: cs.LG

TL;DR: 本文重新审视了IQ-Learn等非对抗性Q学习模仿方法，发现它们实际上退化为行为克隆，存在二次依赖时间跨度的模仿差距下界，仍然遭受复合误差问题。作者提出了Dual Q-DM方法，通过原始-对偶框架和贝尔曼约束实现泛化，理论上能消除复合误差。


<details>
  <summary>Details</summary>
Motivation: 对抗性模仿学习（AIL）虽然能减轻行为克隆的复合误差，但存在训练不稳定问题。非对抗性Q学习模仿方法（如IQ-Learn）被认为能通过在线交互超越行为克隆，但本文发现这些方法实际上退化为行为克隆，仍然存在复合误差问题。

Method: 提出了Dual Q-DM方法：1）采用原始-对偶框架进行分布匹配；2）引入贝尔曼约束，将高Q值从已访问状态传播到未访问状态；3）实现超越演示的泛化能力。

Result: 理论证明Dual Q-DM等价于对抗性模仿学习，能够恢复演示之外的专家动作，从而消除复合误差。实验进一步验证了理论结果，表明Dual Q-DM是首个理论上保证消除复合误差的非对抗性模仿学习方法。

Conclusion: IQ-Learn等现有非对抗性Q学习模仿方法实际上退化为行为克隆，仍然存在复合误差。提出的Dual Q-DM方法通过原始-对偶框架和贝尔曼约束实现了真正的泛化，理论上能消除复合误差，为模仿学习提供了更稳定的替代方案。
...
### [115] [Behavioral Heterogeneity as Quantum-Inspired Representation](https://arxiv.org/abs/2603.22729)
*Mohammad Elayan,Wissam Kontar*

Main category: cs.LG

TL;DR: 提出量子启发的驾驶员表示方法，将驾驶员建模为密度矩阵形式的动态潜在状态，而非静态标签


<details>
  <summary>Details</summary>
Motivation: 现有方法将驾驶员异质性简化为标签或离散状态，压缩了固有的动态特性，需要更灵活的表示方法

Method: 使用量子启发的密度矩阵表示驾驶员动态潜在状态，通过非线性随机傅里叶特征嵌入行为观测，结合时间行为持续性和上下文相关配置文件激活进行状态演化

Result: 在TGSIM驾驶数据上评估，展示了如何提取和分析驾驶配置文件

Conclusion: 提出的量子启发表示方法能够更好地捕捉驾驶员行为的动态异质性，超越传统的静态分类方法
...
### [116] [Multitask-Informed Prior for In-Context Learning on Tabular Data: Application to Steel Property Prediction](https://arxiv.org/abs/2603.22738)
*Dimitrios Sinodinos,Bahareh Nikpour,Jack Yi Wei,Sushant Sinha,Xiaoping Ma,Kashif Rehman,Stephen Yue,Narges Armanfard*

Main category: cs.LG

TL;DR: 该研究提出了一种多任务学习框架，通过改进TabPFN模型来预测热轧过程中钢材的机械性能，解决了传统方法资源密集且无法充分利用性能间相关性的问题。


<details>
  <summary>Details</summary>
Motivation: 热轧过程中钢材机械性能的准确预测面临挑战，传统经验方法资源密集且缺乏适应性，现有方法未能充分利用关键机械性能之间的强相关性，错失了通过多任务学习提高预测精度的机会。

Method: 提出了一个多任务学习框架，通过两种互补方法增强TabPFN模型：1）目标平均法，提供与TabPFN单目标架构兼容的统一标量信号；2）任务特定适配器，在微调过程中引入任务特定监督。这些策略共同引导模型形成能够捕捉关键机械指标间跨属性关系的多任务感知先验。

Result: 在工业TSDR数据集上的大量实验表明，该多任务适应方法在多个评估指标上优于经典机器学习方法和最新的表格学习模型。与任务特定微调相比，该方法不仅提高了预测精度，还增强了计算效率。

Conclusion: 多任务感知先验适应使得表格数据的基础模型能够在TSDR中实现可扩展、快速和可靠的部署，适用于自动化工业质量控制和工艺优化。
...
### [117] [Caterpillar of Thoughts: The Optimal Test-Time Algorithm for Large Language Models](https://arxiv.org/abs/2603.22784)
*Amir Azarmehr,Soheil Behnezhad,Alma Ghafari*

Main category: cs.LG

TL;DR: 该论文提出将测试时计算建模为算法与马尔可夫链的交互，证明最优算法生成"毛毛虫树"结构，并基于此提出Caterpillar of Thoughts算法，在减少token生成的同时提高成功率。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型通过额外测试时计算（如采样、思维链、回溯等）能显著提升输出质量，但这些技术的理论理解有限，缺乏对如何最优利用固定计算预算的理论指导。

Method: 将测试时计算建模为算法与马尔可夫链的交互，允许算法回溯到任何先前观察到的状态。证明最优算法生成"毛毛虫树"结构（去除叶子后得到一条路径），并基于此提出Caterpillar of Thoughts算法。

Result: 理论证明回溯能将生成次数减少指数级，但有限形式的回溯在理论上已足够。实验表明CaT算法相比Tree-of-Thoughts在减少token生成的同时获得了更高的成功率。

Conclusion: 该研究为测试时计算提供了理论框架，证明了最优算法的结构特性，并基于此提出了更高效的Caterpillar of Thoughts算法，在理论和实践上都有重要意义。
...
### [118] [Transformers Trained via Gradient Descent Can Provably Learn a Class of Teacher Models](https://arxiv.org/abs/2603.22801)
*Chenyang Zhang,Qingyue Zhao,Quanquan Gu,Yuan Cao*

Main category: cs.LG

TL;DR: 论文证明单层简化注意力机制的Transformer能够学习并恢复多种教师模型（包括卷积层、图卷积层和统计学习模型）的参数，达到最优总体损失，并在温和假设下具有良好的分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在各种应用中取得了巨大成功，但其理论基础仍然很大程度上未被探索。为了理解Transformer在多种场景和任务中表现出的强大能力，作者从理论角度研究将Transformer作为学生模型来学习一类教师模型的能力。

Method: 使用单层Transformer，采用简化的"仅位置"注意力机制，作为学生模型来学习多种教师模型，包括带平均池化的卷积层、图卷积层以及各种经典统计学习模型（如稀疏token选择模型和组稀疏线性预测器）。分析的关键是识别各种学习任务共享的基本双线性结构。

Result: 证明单层Transformer能够成功恢复教师模型的所有参数块，从而达到最优总体损失。基于Transformer对教师模型的有效模仿，进一步证明在温和假设下，训练好的Transformer能够很好地泛化到广泛的分布外数据类别。

Conclusion: 研究为Transformer的成功提供了理论解释，表明其能够学习并模仿多种类型的教师模型，具有强大的泛化能力。识别出的双线性结构为建立统一的学习保证提供了基础，有助于理解Transformer在多样化任务中的表现。
...
### [119] [Sparser, Faster, Lighter Transformer Language Models](https://arxiv.org/abs/2603.23198)
...
<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型的扩展带来了前所未有的进展，但伴随巨大的计算成本。前馈层占用了大部分模型参数和执行FLOPs，因此通过利用这些层中的非结构化稀疏性来降低计算成本成为研究动机。

Method: 1) 引入新的稀疏打包格式和一组CUDA内核，与现代GPU的优化执行管道无缝集成；2) 使用简单的L1正则化诱导超过99%的稀疏度；3) 提供LLM稀疏性的定量研究。

Result: 实验表明，当与提出的内核结合时，这些稀疏度水平转化为显著的吞吐量、能源效率和内存使用效益，且这些效益随着模型规模的增加而增加。稀疏度对下游性能影响可忽略。

Conclusion: 通过释放所有代码和内核作为开源，促进采用并加速研究，将稀疏性确立为改善现代基础模型效率和可扩展性的实用轴心。
...
### [120] [Towards The Implicit Bias on Multiclass Separable Data Under Norm Constraints](https://arxiv.org/abs/2603.22824)
*Shengping Xie,Zekun Wu,Quan Chen,Kaixu Tang*

Main category: cs.LG

TL;DR: 该论文提出NucGD优化器，通过核范数约束在可分离多类数据上强制低秩结构，并分析随机优化动态如何影响收敛到最大间隔解


<details>
  <summary>Details</summary>
Motivation: 研究梯度算法在过参数化模型中诱导的隐式偏差机制，特别是优化几何如何影响多类可分离数据上的解

Method: 基于归一化最陡下降框架，设计NucGD几何感知优化器，通过核范数约束强制低秩结构；推导基于异步幂迭代的高效SVD-free更新规则；分析小批量采样和动量引入的梯度噪声对收敛的影响

Result: NucGD能够有效强制低秩结构，与新兴的低秩投影方法建立联系；异步幂迭代实现可扩展训练；实验揭示了不同梯度噪声水平如何调节向期望最大间隔解的收敛

Conclusion: 该工作为理解优化几何如何塑造多类可分离数据上的解提供了统一框架，NucGD及其高效实现为强制低秩结构提供了实用工具，同时深入分析了随机优化动态对隐式偏差的影响
...
TL;DR: ReVal：基于贝尔曼更新的价值强化学习方法，结合步长信号和轨迹级信号，支持回放缓冲区训练，提高LLM训练的数据利用效率
...
Method: 提出ReVal方法，基于贝尔曼更新的价值强化学习框架，结合捕捉内部一致性的步长信号和基于结果验证的轨迹级信号。该方法天然支持基于回放缓冲区的训练，能够高效重用历史轨迹。

Result: 在标准数学推理基准测试中，ReVal不仅收敛更快，最终性能也优于GRPO。在DeepSeek-R1-Distill-1.5B上，ReVal提高了训练效率，在AIME24上提升2.7%，在GPQA基准上提升4.5%。

Conclusion: 基于价值的强化学习是LLM训练中实用的替代方案，能够显著提高数据利用效率和最终性能。
...
### [122] [Confidence Calibration under Ambiguous Ground Truth](https://arxiv.org/abs/2603.22879)
*Linwei Tao,Haoyang Luo,Minjing Dong,Chang Xu*

Main category: cs.LG

TL;DR: 本文针对传统置信度校准方法在标注者存在真实分歧时的失效问题，提出了一系列考虑标注模糊性的后处理校准器，显著提升了真实标签的校准质量。


<details>
  <summary>Details</summary>
Motivation: 传统置信度校准假设每个输入只有一个真实标签，但这一假设在标注者存在真实分歧时失效。基于多数投票标签的后处理校准器在传统评估下看似校准良好，但实际上对底层标注者分布存在严重校准错误。

Method: 开发了一系列考虑模糊性的后处理校准器：1) Dirichlet-Soft：利用完整的标注者分布；2) MCTS S=1：每个样本仅需一个标注，匹配完整分布校准；3) LS-TS：仅使用投票标签，通过模型自身置信度构建数据驱动的伪软目标。

Result: 在四个真实多标注者分布基准测试中，Dirichlet-Soft相对于Temperature Scaling将真实标签ECE降低了55-87%，LS-TS在没有标注者数据的情况下将ECE降低了9-77%。MCTS S=1在所有基准测试中匹配完整分布校准。

Conclusion: 标注者分歧是置信度校准的重要挑战，提出的模糊感知校准方法能够显著改善真实标签校准质量，且在不同标注数据可用性条件下均有效。
...
### [123] [TreeTeaming: Autonomous Red-Teaming of Vision-Language Models via Hierarchical Strategy Exploration](https://arxiv.org/abs/2603.22882)
*Chunxiao Li,Lijun Li,Jing Shao*

Main category: cs.LG

TL;DR: TreeTeaming：一种基于树状进化探索的自动化红队框架，突破传统线性攻击策略限制，通过LLM驱动的编排器动态构建攻击策略树，在12个主流VLM上实现SOTA攻击成功率


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型红队方法受限于线性探索范式，只能在预定义策略集中优化，无法发现新颖多样的攻击漏洞。需要超越静态启发式方法，实现动态进化的漏洞发现

Method: 提出TreeTeaming框架：1）LLM驱动的编排器自主决策进化有潜力的攻击路径或探索多样化策略分支；2）动态构建和扩展策略树；3）多模态执行器执行复杂策略
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在从多元时间序列中学习潜在因子时缺乏可识别性保证，导致因子解释困难。需要开发能够提供可识别性保证的动态因子模型，以改善因子恢复和因果推断的稳定性。

Method: 提出iVDFM模型，将iVAE（可识别变分自编码器）的条件机制应用于驱动动态的创新过程而非潜在状态。采用线性对角动态保持可识别性，并通过伴随矩阵和Krylov方法实现可扩展计算。

Result: 在合成数据上展示改进的因子恢复能力，在合成结构因果模型上实现稳定的干预准确性，在真实世界基准测试中达到有竞争力的概率预测性能。

Conclusion: iVDFM能够从多元时间序列中学习具有可识别性保证的潜在因子，为时间序列分析、因果推断和预测任务提供了理论保证和实用价值。
...
TL;DR: VLGOR框架结合视觉-语言知识生成虚拟交互轨迹，通过离线强化学习提升语言指令驱动的智能体在未见任务上的泛化能力


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能理解语言指令，但缺乏对物理环境的直接感知能力，限制了其对环境动态的理解和在未见任务上的泛化能力

Method: 提出视觉-语言知识引导的离线强化学习框架，微调视觉-语言模型以基于初始视觉观察和高级指令预测未来状态和动作，生成时间连贯、空间合理的虚拟轨迹，并使用反事实提示增加轨迹多样性

Result: 在机器人操作基准测试中，VLGOR在需要新最优策略的未见任务上显著优于基线方法，成功率高出24%以上

Conclusion: 通过整合视觉和语言知识生成虚拟交互轨迹，能够有效增强智能体在语言指令驱动下基于视觉线索执行未见任务的能力
...
TL;DR: 该论文指出当前图基础模型（GFMs）存在架构适应性不足的问题，提出了一种能够在推理时自适应发现和混合任务特定图算子的框架，以解决固定架构在不同任务上的泛化限制。


<details>
  <summary>Details</summary>
Motivation: 当前领域无关的图基础模型依赖于固定的架构骨干，隐含假设单一的消息传递机制足以应对所有任务。作者认为架构适应性是真正图基础模型的必要条件，并发现现有方法对任务依赖的架构属性不鲁棒。

Method: 提出了一个框架，在推理时通过发现和混合任务特定的线性图算子来自适应有效的GNN架构，实现跨异构架构需求任务的零样本泛化，无需重新训练。

Result: 通过理论分析和受控合成实验证明，固定骨干的GFMs在架构需求与训练时不同的任务上存在"欠达"问题。在任意范围的合成任务和真实世界基准测试中验证了所提方法，相比现有领域无关GFMs表现出更好的性能和鲁棒性。

Conclusion: 架构适应性是图基础模型的关键要求，提出的自适应框架能够有效解决固定架构在不同任务上的泛化限制，为真正的零样本跨图泛化提供了新的解决方案。
...
Method: 比较了鲁棒性量化(RQ)和不确定性量化(UQ)两种方法，在多个基准数据集上进行实验，包括标准设置和分布偏移情况，并探索了两种方法的组合策略。

Result: RQ在标准设置和分布偏移下都能优于UQ，同时发现RQ和UQ具有互补性，两者的组合能产生更好的可靠性评估效果。

Conclusion: RQ是评估分类器预测可靠性的有效方法，不仅能与UQ竞争，还能与UQ互补结合，为可靠性评估提供了新的思路和更好的解决方案。
...
TL;DR: 本文质疑表格数据生成领域的进展认知，指出当前评估协议的局限性，重新评估深度概率电路作为简单基线模型的性能，发现其在成本更低的情况下能达到或超越SOTA模型，并证明当前进展饱和主要是由于不恰当的评估指标所致。


<details>
  <summary>Details</summary>
Motivation: 表格数据生成比文本和图像生成更具挑战性，因为其特征异构且样本量小。当前扩散模型被认为是SOTA，但本文质疑该领域的进展认知，指出当前评估协议存在局限性，需要重新审视简单基线模型的潜力。

Method: 1) 分析当前评估生成数据保真度的协议局限性，提倡替代方案；2) 重新评估分层混合模型（深度概率电路PCs）作为简单基线，PCs是决策森林的生成对应物，能原生处理异构数据并提供可处理的概率生成和推理；3) 通过严格的实证分析验证PCs的性能。

Result: 深度概率电路（PCs）以更低的成本实现了与SOTA模型竞争甚至更优的性能。实证分析表明，当前SOTA模型进展的明显饱和主要是由于使用了不恰当的评估指标，表格数据生成仍有很大改进空间。

Conclusion: 表格数据生成领域尚未达到饱和，当前SOTA模型的进展认知存在偏差，主要源于不恰当的评估指标。深度概率电路作为简单基线模型显示出强大潜力，表明该领域仍有大量工作要做才能生成真实的表格数据。
...
TL;DR: 该研究评估了三种机器学习气候模拟器在历史训练数据范围外的泛化能力，发现即使高性能的基础模型在面对未来"无类比"气候状态时也存在准确性与稳定性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 气候变化加速带来的非平稳性挑战了机器学习气候模拟器的泛化能力，特别是在超出历史训练数据范围的"无类比"未来气候状态下。当前评估面临数据污染问题，因为许多模型训练时已经包含了未来情景，掩盖了真正的分布外性能。

Method: 研究采用两种互补策略评估三种最先进架构（U-Net、ConvLSTM和ClimaX基础模型）的分布外鲁棒性：1）时间外推至近期气候（2015-2023）；2）跨不同排放路径的强迫变化。所有模型都限制在历史训练制度（1850-2014）。

Result: 分析揭示了准确性与稳定性之间的权衡：ClimaX基础模型虽然达到最低绝对误差，但在分布变化下表现出更高的相对性能变化，极端强迫情景下降水误差增加高达8.44%。这表明即使限制在历史训练动态中，高容量基础模型对外部强迫轨迹也很敏感。

Conclusion: 研究结果强调了情景感知训练和严格的分布外评估协议的必要性，以确保气候模拟器在变化气候下的鲁棒性。仅依赖历史训练数据不足以应对未来"无类比"气候状态。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习方法在预测性维护任务上表现出色，但缺乏通用的服务导向框架来捕捉工业物联网传感器数据中的复杂依赖关系。Transformer模型虽然具有强大的序列建模能力，但作为稳健的AI服务直接部署面临瓶颈：1）流式传感器数据存在多尺度时间相关性；2）可用于训练故障预测服务的数据集通常规模有限。

Method: 提出MsFormer，一种轻量级多尺度Transformer，包含多尺度采样模块和定制的位置编码机制来捕捉多流服务数据的序列相关性。为适应数据稀缺的服务环境，采用轻量级注意力机制，使用简单的池化操作替代自注意力。

Result: 在真实世界数据集上的广泛实验表明，该框架相比最先进方法取得了显著的性能提升。MsFormer在不同工业设备和操作条件下均表现优异，展示了强大的泛化能力，同时保持了高度可靠的服务质量。

Conclusion: MsFormer作为一种统一的AI服务模型，能够有效解决工业预测性维护中的多尺度时间相关性和数据稀缺问题，提供可靠的服务质量，具有强大的实际应用价值。
...
TL;DR: 本文提出了一种轻量级强化学习框架，通过引入分布级别的LOO-FID奖励和组合实例级奖励，优化自回归图像生成模型的质量和多样性，避免模式崩溃。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成中效果显著，但标准的极大似然估计训练无法直接优化样本质量和多样性。现有强化学习方法在扩散模型中存在输出多样性崩溃问题，而自回归模型的强化学习方法通常依赖实例级奖励，导致分布覆盖范围与质量之间的权衡。

Method: 提出轻量级RL框架，将基于token的自回归合成建模为马尔可夫决策过程，使用Group Relative Policy Optimization进行优化。核心创新是引入分布级别的Leave-One-Out FID奖励，利用特征矩的指数移动平均来显式鼓励样本多样性并防止模式崩溃。结合CLIP和HPSv2等实例级奖励确保语义和感知保真度，并使用自适应熵正则化项稳定多目标学习。

Result: 在LlamaGen和VQGAN架构上的大量实验表明，仅需几百次调优迭代就能在标准质量和多样性指标上取得明显改进。结果显示模型无需Classifier-Free Guidance也能生成有竞争力的样本，避免了其2倍的推理成本。

Conclusion: 提出的RL框架有效解决了自回归图像生成中质量与多样性的平衡问题，通过分布级和实例级奖励的组合优化，在保持语义保真度的同时防止模式崩溃，实现了高效且高质量的图像生成。
...
TL;DR: SpecXMaster：基于智能体强化学习的NMR分子光谱解释框架，直接从原始FID数据自动提取1H和13C谱的多重性信息，实现端到端的NMR谱图到化学结构的全自动解释。
...
Method: 提出SpecXMaster框架，采用智能体强化学习技术，直接从原始FID数据自动提取1H和13C NMR谱的多重性信息，构建端到端的自动化解释流程。

Result: 在多个公开NMR解释基准测试中表现出优越性能，并通过专业化学光谱学家的迭代评估不断优化改进。
...
TL;DR: 本文提出了DAK-UCB方法，一种考虑多样性的上下文老虎机算法，用于在线选择生成模型，同时兼顾保真度和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有生成AI模型选择方法仅基于保真度评分（如CLIP-Score），忽略了生成输出的多样性，可能导致响应缺乏多样性。需要一种同时考虑保真度和多样性的在线模型选择机制。

Method: 提出了多样性感知核化上置信界（DAK-UCB）方法，这是一种上下文老虎机算法。该方法将保真度和多样性相关指标整合到选择过程中，基于提示感知的多样性评分函数，该函数分解为先前生成轮次中提示-输出对的基于两样本的期望。具体使用联合核距离和核熵度量。

Result: 实验结果表明DAK-UCB在促进多样性感知模型选择的同时，能保持一系列提示生成的保真度。该方法在平衡保真度和多样性方面表现有效。

Conclusion: DAK-UCB方法成功解决了现有保真度选择方法忽略多样性的问题，为生成AI模型的在线选择提供了兼顾保真度和多样性的有效框架。
...
TL;DR: 本文证明了多组学习样本复杂度的最紧上界，提出基于二分图b-匹配扩展的单包含图预测算法，在组可实现设置下达到最优收敛率。


<details>
  <summary>Details</summary>
Motivation: 多组学习在公平机器学习中至关重要，但现有方法的样本复杂度界限不够紧致。需要开发更高效的算法来降低样本需求，同时保证在不同群体上的泛化性能。

Method: 扩展单包含图预测策略，采用二分图b-匹配的泛化形式。通过构建合适的图结构来捕捉不同群体间的学习关系，实现样本高效的多组学习。

Result: 1. 在组可实现设置下，算法达到log n/n的最优收敛率，并通过下界证明该速率在一般情况下是最优的；2. 当评估组与样本选择无关时，算法在组可实现设置下达到1/n的最优收敛率。

Conclusion: 提出的算法在多组学习中实现了样本复杂度的最优收敛率，为公平机器学习提供了理论基础和实用工具，特别是在组可实现设置下表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 研究学习过程中的制度变化问题，即学习者、其记忆状态和评估条件可能随时间演变的情况。需要建立能够处理这种变化的理论基础。

Method: 开发了一个以可容许传输、受保护核心保持和评估者感知学习演化为中心的框架。建立了可容许性的直接闭包结果，为真正多制度设置中的忠实固定本体约简开发了结构阻碍论证，并引入了受保护稳定性模板。

Result: 建立了关于评估者分解、态射、组合和语义可通约层之间的部分核级对齐的定理层结果。通过一个双制度工作示例展示了可容许性证书、受保护评估核心和制度变化成本。建立了首个核级兼容性结果和受控单调演绎见证。

Conclusion: 该论文为制度变化学习引入了一个结构化学习理论框架及其首个定理支持层，但不是一个完整的定量理论。这是一个基础性贡献，为后续研究奠定了基础。
...
### [136] [GEM: Guided Expectation-Maximization for Behavior-Normalized Candidate Action Selection in Offline RL](https://arxiv.org/abs/2603.23232)
*Haoyu Wang,Jingcheng Wang,Shunyu Wu,Xinwei Xiao*

Main category: cs.LG

TL;DR: GEM是一个用于离线强化学习的多模态动作选择框架，通过高斯混合模型演员和基于候选集的推理，在保持数据支持的同时提升决策质量。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习可以从固定数据集中学习强大的价值函数，但当数据集产生分支或多模态动作空间时，单模态策略提取会模糊竞争假设，产生数据支持弱的"中间"动作，即使有强大的批评家，决策仍然脆弱。

Method: GEM训练高斯混合模型演员，通过批评家引导、优势加权的EM风格更新，在保持不同组件的同时将概率质量移向高价值区域；学习可处理的高斯混合模型行为模型来量化支持；推理时执行基于候选集的选择：生成并行候选集，使用保守集成下界和行为标准化支持重新排序动作。

Result: GEM在D4RL基准测试中具有竞争力，并提供了一个简单的推理时预算旋钮（候选数量），可以在不重新训练的情况下用计算换取决策质量。

Conclusion: GEM提供了一个分析框架，使动作选择既多模态又明确可控，通过保持数据支持的同时提升决策质量，解决了离线强化学习中多模态动作空间的挑战。
...
### [137] [Neural ODE and SDE Models for Adaptation and Planning in Model-Based Reinforcement Learning](https://arxiv.org/abs/2603.23245)
*Chao Han,Stefanos Ioannou,Luca Manneschi,T. J. Hayward,Michael Mangan,Aditya Gilra,Eleni Vasilaki*

Main category: cs.LG

TL;DR: 该研究探索在模型强化学习框架中使用神经常微分方程和随机微分方程来建模完全和部分可观测环境中的随机动态，证明神经SDE能更有效捕捉转移动态的随机性，提高样本效率


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决强化学习中环境动态的随机性建模问题，特别是在完全和部分可观测环境下，传统方法难以有效捕捉随机动态，影响策略性能和样本效率

Method: 使用神经ODE和SDE建模环境动态；引入潜在SDE模型结合ODE和GAN训练的随机组件处理部分可观测性；利用逆模型进行策略适应；在随机连续控制基准上评估

Result: 神经SDE能更有效捕捉转移动态的随机性，在挑战性场景中实现高性能策略并提高样本效率；潜在SDE模型在部分可观测环境中表现优异，优于或匹配现有模型基和无模型方法

Conclusion: 该工作证明了动作条件潜在SDE在具有随机转移的环境中进行强化学习规划的有效性，为随机动态建模提供了有前景的解决方案
...
### [138] [A Learning Method with Gap-Aware Generation for Heterogeneous DAG Scheduling](https://arxiv.org/abs/2603.23249)
...
Method: 提出WeCAN框架：1）两阶段单次前向设计：单次前向传播生成任务-资源池评分和全局参数，然后通过生成映射构建调度方案；2）加权交叉注意力编码器：通过兼容性系数门控的任务-资源池交互建模；3）顺序空间分析：分析生成映射的可达顺序集，解释生成诱导差距机制；4）跳转扩展实现：使用分析参数化的递减跳转规则扩大可达顺序集。

Result: 在计算图和真实世界TPC-H DAG上的实验表明，WeCAN相比强基线改进了完工时间，推理时间与经典启发式算法相当，且快于多轮神经调度器。

Conclusion: WeCAN通过端到端强化学习框架有效解决了异构DAG调度问题，结合单次前向效率和扩展可达顺序集的设计，在保持高效推理的同时提升了调度质量。
...
### [139] [SafeSeek: Universal Attribution of Safety Circuits in Language Models](https://arxiv.org/abs/2603.23268)
*Miao Yu,Siyuan Fu,Moayad Aloqaily,Zhenhong Zhou,Safa Otoum,Xing fan,Kun Wang,Yufei Guo,Qingsong Wen*

Main category: cs.LG

TL;DR: 该论文提出了一个统一的AI安全可解释性框架，通过优化方法识别LLM中的功能完整安全电路，并在后门攻击和安全对齐两个关键场景中验证了其有效性。
...
Method: 提出了一个统一的安全可解释性框架，引入可微分二元掩码，通过梯度下降在安全数据集上提取多粒度电路，并集成安全电路调优，利用这些稀疏电路进行高效的安全微调。

Result: 在后门攻击场景中，识别出稀疏度为0.42%的后门电路，其消融使攻击成功率从100%降至0.4%，同时保留超过99%的通用功能；在安全对齐场景中，定位到包含3.03%头部和0.79%神经元的对齐电路，移除该电路使攻击成功率从0.8%飙升至96.9%，而在有用性微调中排除该电路可保持96.5%的安全保留率。

Conclusion: 该方法能够有效识别LLM中的功能完整安全电路，为理解和干预LLM的安全关键行为提供了系统化的可解释性框架，在安全归因和模型调优方面具有重要应用价值。
...
### [140] [A Comparative Study of Machine Learning Models for Hourly Forecasting of Air Temperature and Relative Humidity](https://arxiv.org/abs/2603.23282)
*Jiaqi Dong*

Main category: cs.LG

TL;DR: 该研究比较了7种机器学习模型在重庆复杂地形下进行气温和相对湿度小时预测的性能，发现XGBoost表现最佳。


<details>
  <summary>Details</summary>
Motivation: 准确预测气温和相对湿度对复杂地形城市管理至关重要，特别是在重庆这样的山地城市，需要评估不同机器学习模型在短期气象预报中的效果。

Method: 采用统一的数据预处理框架，包括滞后特征构建、滚动统计特征和时间序列验证，比较了XGBoost、随机森林、SVR、MLP、决策树、LSTM和CNN-LSTM等7种模型。
...
### [141] [Robustness Quantification for Discriminative Models: a New Robustness Metric and its Application to Dynamic Classifier Selection](https://arxiv.org/abs/2603.23318)
...
<details>
  <summary>Details</summary>
Motivation: 现有的鲁棒性量化方法虽然能评估分类器预测的可靠性，但应用范围有限：需要生成模型，且只能用于特定模型架构或离散特征

Method: 提出一种新的鲁棒性度量方法，适用于任何概率判别分类器和任何类型的特征，基于该度量区分可靠与不可靠预测，并开发动态分类器选择策略

Result: 新度量方法能够有效区分可靠和不可靠的预测，为动态分类器选择提供了新的技术基础

Conclusion: 提出的新鲁棒性度量方法扩展了可靠性评估的应用范围，为动态分类器选择提供了更通用的解决方案
...
TL;DR: CDT-III是一个基于中心法则的生物AI模型，通过虚拟细胞嵌入器架构预测DNA、RNA和蛋白质的复杂细胞响应，在蛋白质预测任务中表现出色，并能通过梯度分析预测基因敲除的临床副作用。


<details>
  <summary>Details</summary>
Motivation: 现有生物AI模型虽然能预测复杂的细胞响应，但其学习到的表征与它们试图捕捉的分子过程之间存在脱节。需要开发能够更好连接分子机制与预测结果的AI模型。

Method: 提出CDT-III模型，采用两阶段虚拟细胞嵌入器架构，模拟细胞的空间区室化：VCE-N在细胞核中建模转录过程，VCE-C在细胞质中建模翻译过程。模型覆盖完整的中心法则（DNA、RNA、蛋白质）。

Result: 在5个保留基因上，CDT-III达到RNA预测相关系数r=0.843，蛋白质预测r=0.969。蛋白质预测任务改善了RNA预测性能（从r=0.804提升到0.843）。蛋白质监督提高了DNA层面的可解释性，CTCF富集度增加30%。在模拟CD52敲除实验中，模型正确预测了29/29的蛋白质变化，并在无临床数据情况下重新发现了7个已知临床副作用中的5个。

Conclusion: CDT-III展示了机制导向AI在连接分子过程与细胞响应预测方面的潜力。蛋白质监督能正则化上游表征，提高模型可解释性。基于梯度的副作用分析方法仅需未扰动基线数据，可实现大规模基因筛查而无需新实验。
...
### [143] [Graph Energy Matching: Transport-Aligned Energy-Based Modeling for Graph Generation](https://arxiv.org/abs/2603.23398)
*Michal Balcerak,Suprosana Shit,Chinmay Prabhakar,Sebastian Kaltenbach,Michael S. Albergo,Yilun Du,Bjoern Menze*

Main category: cs.LG

TL;DR: GEM是一种用于图生成的基于能量的模型框架，通过结合传输对齐引导和能量优化，在保持基于能量模型优势的同时，达到了与离散扩散模型相当的生成质量。


<details>
  <summary>Details</summary>
Motivation: 离散能量模型在图生成任务中面临采样效率和质量问题，存在虚假局部极小值导致采样器陷入困境，与离散扩散模型存在保真度差距。需要开发既能保持能量模型优势（如可组合推理、约束满足）又能达到高质量采样的方法。

Method: GEM基于Jordan-Kinderlehrer-Otto方案的传输映射优化视角，学习一个置换不变势能函数，同时提供从噪声到数据的传输对齐引导，并在高数据似然区域细化样本。引入基于能量的切换采样协议，无缝桥接：(i) 快速梯度引导传输到高概率区域，以及(ii) 混合机制以探索学习到的图分布。

Result: 在分子图基准测试中，GEM匹配或超越了强大的离散扩散基线模型。除了样本质量外，相对似然的显式建模支持推理时的目标探索，促进了组合生成、属性约束采样和图之间的测地线插值。

Conclusion: GEM成功弥合了离散能量模型与扩散模型之间的保真度差距，同时保持了能量模型的优势，为图生成提供了既能高质量采样又支持可组合推理的框架。
...
### [144] [SortedRL: Accelerating RL Training for LLMs through Online Length-Aware Scheduling](https://arxiv.org/abs/2603.23414)
*Yiqi Zhang,Huiqiang Jiang,Xufang Luo,Zhihe Yang,Chengruidong Zhang,Yifei Shen,Dongsheng Li,Yuqing Yang,Lili Qiu,Yang You*

Main category: cs.LG

TL;DR: SortedRL通过基于输出长度的在线调度策略，优化强化学习训练中的rollout效率瓶颈，减少50%以上的训练等待时间，同时提升模型性能3.9%-18.4%。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型的强化学习训练中，rollout阶段（生成长轨迹）占用了高达70%的训练时间，主要由于自回归生成缓慢和rollout与策略更新之间的同步开销，这成为训练效率的主要瓶颈。

Method: 提出SortedRL在线长度感知调度策略：1）根据输出长度对rollout样本重新排序，优先处理短样本形成组进行早期更新；2）支持大rollout批次和灵活的更新批次；3）通过缓存机制控制离策略训练程度；4）配备专门的RL基础设施，通过状态控制器和rollout缓冲区管理rollout和更新。

Result: 在LLaMA-3.1-8B和Qwen-2.5-32B模型上的实验表明，SortedRL将RL训练等待时间比例减少超过50%，在相同数据量下相比基线获得3.9%到18.4%的性能提升，在逻辑谜题、AIME 24、Math 500和Minerval等数学挑战任务上表现优异。

Conclusion: SortedRL有效解决了大规模语言模型强化学习训练中的rollout效率瓶颈，通过智能调度策略显著提升训练效率，同时保持训练稳定性并提高模型性能，为大规模RL训练提供了实用解决方案。
...
### [145] [Similarity-Aware Mixture-of-Experts for Data-Efficient Continual Learning](https://arxiv.org/abs/2603.23436)
*Connor Mclaughlin,Nigel Lee,Lili Su*

Main category: cs.LG

TL;DR: 该论文提出了一种自适应混合专家框架，用于解决数据稀缺且任务重叠的持续学习问题，通过增量全局池化和实例级提示掩码来提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法通常假设每个任务有足够多的数据样本或任务之间不重叠，但在实际应用中，每个任务可能只有有限数据，且任务可能以任意方式重叠，这导致两个挑战：数据稀缺需要有效知识迁移，而任务重叠又容易造成负迁移。

Method: 提出基于预训练模型的自适应混合专家框架，包含两个创新组件：1) 增量全局池化 - 随时间逐步引入提示以减少提示关联噪声；2) 实例级提示掩码 - 将任务样本分解为与当前提示对齐的分布内样本和需要新提示的分布外样本。

Result: 在不同数据量和任务间相似度的实验中，该方法提高了样本效率，并显示出广泛的适用性。
...
### [146] [End-to-End Efficient RL for Linear Bellman Complete MDPs with Deterministic Transitions](https://arxiv.org/abs/2603.23461)
*Zakaria Mhammedi,Alexander Rakhlin,Nneka Okolo*

Main category: cs.LG

TL;DR: 提出了一种在具有线性Bellman完备性和确定性转移的MDP中高效学习ε-最优策略的算法，适用于有限或大动作空间


<details>
  <summary>Details</summary>
Motivation: 现有线性函数逼近的强化学习算法在具有线性Bellman完备性的MDP中，要么局限于小动作空间，要么需要特征空间的强预言机假设，缺乏既计算高效又适用于大动作空间的实用算法

Method: 针对具有确定性转移、随机初始状态和随机奖励的线性Bellman完备MDP，提出计算高效算法：对于有限动作空间完全端到端高效；对于大或无限动作空间仅需标准的argmax动作预言机

Result: 算法能够学习ε-最优策略，其样本和计算复杂度在时间范围、特征维度和1/ε上都是多项式级别的

Conclusion: 该工作为线性Bellman完备MDP提供了一种既统计可处理又计算高效的算法，填补了现有方法在动作空间规模方面的限制，为实际应用提供了可行的解决方案
...
### [147] [Byzantine-Robust and Differentially Private Federated Optimization under Weaker Assumptions](https://arxiv.org/abs/2603.23472)
*Rustem Islamov,Grigory Malinovsky,Alexander Gaponov,Aurelien Lucchi,Peter Richtárik,Eduard Gorbunov*

Main category: cs.LG

TL;DR: 提出Byz-Clip21-SGD2M算法，在联邦学习中统一解决差分隐私和拜占庭鲁棒性问题，提供收敛保证并放宽现有工作的假设条件。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护数据隐私，但梯度更新仍可能泄露敏感信息，且恶意服务器可能发起拜占庭攻击。现有方法存在假设不现实、需要额外数据集或缺乏收敛保证等局限性。

Method: 提出Byz-Clip21-SGD2M算法，结合鲁棒聚合、双动量和精心设计的裁剪技术，在标准L-光滑和σ-次高斯梯度噪声假设下工作。

Result: 理论证明高概率收敛保证，在无攻击时达到最先进收敛率，在拜占庭和差分隐私设置下改进效用保证。MNIST数据集上CNN和MLP模型的实验验证了有效性。

Conclusion: Byz-Clip21-SGD2M算法成功统一处理联邦学习中的差分隐私和拜占庭鲁棒性问题，放宽了现有工作的假设条件并提供理论保证。
