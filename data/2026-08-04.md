...
TL;DR: 本文探究廉价开源权重模型能否替代昂贵的顶尖大模型作为数学证明评分器。在200实例验证集上，三个廉价评判模型（GPT-OSS 120B、DeepSeek-V4 Flash、Gemma-4 31B）与人类评判的一致性在统计上与Claude Opus 4.7和Gemini 3.1 Pro无显著差异，且成本低至1-2个数量级。在全量1000实例基准上，"三模型一致通过"（all-three-pass）规则达到最高通过一致性和精确度，且运行间波动最小。推荐将该规则作为默认部署方案，但需注意该规则为事后识别，需独立复现验证。


<details>
  <summary>Details</summary>
Motivation: 自然语言数学证明的评分是评估数学推理系统的持续成本，而前沿大模型作为评判者价格昂贵。作者希望探究廉价开源权重模型能否在给定候选证明、标准答案和人工评分标准的情况下，作为可靠且经济的评分替代方案。

Method: 在IMO-GradingBench上选取200实例验证集，使用三个廉价评判模型（GPT-OSS 120B、DeepSeek-V4 Flash、Gemma-4 31B）与两个前沿模型（Claude Opus 4.7、Gemini 3.1 Pro）进行对比，统计与人类通过/未通过判定的一致性。随后扩展到1000实例全量基准，探索包括多数投票和全票一致（all-three-pass）等多种共识规则。

Result: 三个廉价模型与人类判定的一致性在统计上与前沿模型不可区分，成本降低达100倍。多数投票匹配前沿表现但未超过最优成员。全票一致规则（all-three-pass）达到最高的通过与精度，并在四次重复运行中表现出最小的运行间波动。

Conclusion: 廉价开源模型在数学证明评分任务上与前沿模型竞争力相当，成本优势显著。推荐使用all-three-pass共识规则作为默认部署，但该规则为事后识别，需要独立复现以验证其可靠性。
...
TL;DR: 本文提出RubricReviewer，一种完全基于评分标准驱动的LLM评审框架，通过将评分标准生成作为显式中间步骤，并结合无训练代理（Scout）与有训练模型（Aligner）的优势，在真实投稿数据上生成了比现有系统更全面、更具判别力且更鲁棒的评审意见。


<details>
  <summary>Details</summary>
Motivation: 同行评审面临前所未有的投稿压力，促使使用LLM作为评审助手。但现有LLM评审器存在两个结构性局限：一是直接将稿件映射到评审意见，导致评分标准隐式化、其推导与判断纠缠不清；二是现有范式各只捕捉了优质评审的一半——无训练代理收集广泛证据但批评缺乏方向性，有训练模型继承人类判别判断但带有噪声和覆盖不均的问题。

Method: RubricReviewer是一个完全评分标准驱动的框架。其核心在于将评分标准生成作为显式中间步骤，使评审生成和最终评估都基于论文自适应的评分标准。框架结合了收集外部证据的无训练代理（Scout）和消费这些证据的人类对齐训练模型（Aligner），融合了两种监督来源的优势。

Result: 在真实世界投稿上的实验表明，RubricReviewer生成的评审比现有系统更加全面且更具判别力，并且对对抗性提示注入攻击表现出最强的鲁棒性。消融研究进一步证实了每个组件的必要性。

Conclusion: RubricReviewer通过显式的评分标准生成和双组件融合结构，有效解决了LLM评审中评分标准隐式和证据收集与判断分离两大问题，在评审质量、判别力和鲁棒性上均优于现有方法。
...
TL;DR: 本文提出MemoryForge框架，通过合成自传式终身记忆替代静态文本描述，让冻结的LLM动态检索情境相关的记忆来引导角色扮演和用户模拟行为，在PersonaGym和SimulatorArena上超过传统描述性条件化基线。


<details>
  <summary>Details</summary>
Motivation: 传统基于提示的方法通过注入静态文本档案进行描述性条件化，由于缺乏真实的生活记忆，往往使智能体表现出泛化行为，无法体现真实人类角色的个性特征。

Method: MemoryForge包含三个核心组件：上下文生成器（用于社会历史基础）、生活组织器（用于目标身份的成长连贯性）、多分辨率模拟器（平衡宏观时间总结与高保真情景经验）。通过从简短的目标准入合成终身记忆，实现记忆化条件化。

Result: 在PersonaGym（角色扮演）和SimulatorArena（用户模拟）上的实验表明，MemoryForge合成的记忆库能使冻结LLM在多个指标和多种LLM骨干网络上展现出比强描述性条件化基线更接近人类的行为。

Conclusion: 记忆化条件化是替代传统描述性条件化的有效范式，自传式记忆能够为LLM的人设赋予更真实、更情境化的行为表现。
...
TL;DR: AgentMemBench提出了一个统一的、可复现的基准，评测五种对话智能体长期记忆管理策略（ICW、EKV、GEM、CBS、WAM），在三个公开数据集上对比其检索质量、回答质量、内存占用和延迟。结果显示外部键值存储（EKV）在质量维度上全面占优，尤其在长距离回忆场景下表现突出，但存在明显的内存占用代价。


<details>
  <summary>Details</summary>
Motivation: 长对话智能体的有限上下文窗口无法支撑跨数千轮的一致性回忆，长期记忆仍是关键瓶颈。但缺乏一个在相同条件下统一评估不同记忆管理策略的可复现基准。

Method: 构建AgentMemBench统一基准，在相同条件下评测五种记忆策略：上下文窗口（ICW）、外部键值存储（EKV）、图式情景记忆（GEM）、压缩摘要（CBS）和网络增强记忆（WAM）。使用三个公开数据集（LoCoMo、MultiDoc2Dial、MSC），共491个标注问题轮次，以Recall@k、MRR、nDCG@k、Answer F1、LLM-judge Faithfulness、内存占用和延迟为指标。生成与评判均使用Qwen2.5-7B-Instruct（4-bit）和贪心解码，并额外评估MemGPT/Letta和HippoRAG两个已发表系统。

Result: EKV在质量维度上全面领先（宏观Recall@5 0.792，MRR 0.677，F1 0.156，Faithfulness 0.354）；在LoCoMo长距离回忆场景中EKV达到Recall@5 0.573而其他方法几乎为零（≤0.005），表明密集检索是唯一能扩展到长时域的方法；CBS在检索上列第二（0.556）；WAM在语料内回忆上与ICW持平；EKV的内存占用约5,100 tokens，远高于ICW/WAM的约300 tokens。

Conclusion: 对话Agent的长期记忆存在明确的准确性-效率权衡：EKV证明了密集检索在长时域回忆中的不可替代性，但付出约17倍的内存足迹代价。工作总结、实体图和网络增强方法在长距离回忆上均失效。发布全部代码、环境和结果工件以保证完全可复现。
...
TL;DR: 本文提出DLLM-TTS，一种将文本转语音（TTS）建模为基于X-Codec2神经音频编解码器令牌的条件块离散扩散框架。模型将序列分解为块，在每个块内应用掩码扩散，同时按顺序处理块，兼顾局部声学连贯性和全局文本-语音对齐。推理时块内并行令牌预测使实时因子（RTF）达到0.15，0.6B参数模型在20K小时数据上训练即在Seed-TTS-eval基准上取得有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有TTS系统面临权衡：自回归编解码器语言模型生成语音可懂度高但需要大规模模型和训练数据且逐token顺序解码，而非自回归方法虽提升速度但牺牲语言准确性。作者旨在解决这一矛盾，实现兼具高质量和高效率的语音合成。

Method: 将TTS形式化为X-Codec2神经音频编解码器令牌上的条件块离散扩散。模型将序列分解为块，在块内采用掩码扩散，块间顺序处理，同时学习局部声学连贯性和全局文本-语音对齐。推理时块内实现并行令牌预测。

Result: 推理时实现0.15的实时因子（RTF），0.6B参数模型在20K小时训练数据上即在Seed-TTS-eval基准上取得具有竞争力的性能，表明该方法支持高效且数据效率高的语音合成。

Conclusion: 块离散扩散语言模型能够在保持并行生成效率的同时实现实用且数据高效的语音合成，克服了自回归与非自回归方法之间的固有权衡。
...
TL;DR: 本文提出Obshazard-bench，一个实时、观测驱动的多模态大语言模型(MLLM)灾害智能评估基准，直接整合高频卫星探测流、地面观测、历史灾害记录和社会经济指标，覆盖8大类28小类灾害、60多个国家、120多个历史极端事件案例，并定义三阶段评估体系（灾前预测、灾中推演、灾后量化）。实验表明现有模型在将原始多通道物理观测转化为时间感知和决策相关的灾害推理方面存在重大局限。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基准大多依赖静态、事后、专家处理的产品（如网格化再分析数据），难以与快速演变、时间紧迫的实战灾害场景对齐，因此MLLMs在真实灾害应急响应中的能力缺乏充分评估。

Method: 构建Obshazard-bench基准：(1)直接整合多源卫星原始高频探测流、同步地面站观测、历史灾害记录和社会经济指标，绕开滞后的人工处理和物理反演流程；(2)覆盖8大类28小类灾害、60多个国家、120多个历史极端事件案例及数千个生命周期导向的VQA样本；(3)定义三阶段评估框架：预测性危机预判、主动演化推理、多维度影响量化。

Result: 在通用型和地球科学专用基础模型上的实验显示，现有模型在将原始多通道物理观测转化为时间上有根据的、决策相关的灾害推理方面存在显著不足。

Conclusion: Obshazard-bench填补了MLLM灾害智能评估与真实作战场景之间的空白，揭示了当前模型在实时观测驱动的灾害推理上的关键短板，为未来灾害应急智能系统的发展提供了评估基准和方向指引。
...
TL;DR: 本文提出首个跨模型家族的能力驱动多模态缩放定律（CDM Scaling Law），通过从LLM文本基准中提取低维能力分数S，预测VLM在下游基准上的表现。训练了150多个VLM、覆盖34个LLM和7个模型家族，验证了该定律在跨规模外推、训练轨迹预测和泛化到未知模型家族方面的有效性，并将骨干模型选择从昂贵的经验搜索转变为原则性的定量决策。


<details>
  <summary>Details</summary>
Motivation: 选择LLM骨干是构建VLM时最关键的决策，但现有方法缺乏原则性指导：基于计算量的缩放定律无法跨模型家族泛化，且缺乏在训练前直接预测VLM性能的框架。

Method: 提出能力驱动的多模态缩放定律（CDM Scaling Law）：通过PCA从LLM文本基准中提取低维能力分数S，将VLM性能建模为S的函数，引入每个骨干的迁移率（transfer rate）和量化数据缩放效率的吸收率（absorption rate）。在严格受控的训练方案下，训练150多个VLM，覆盖34个LLM、7个模型家族。

Result: 在200多个文本基准和50多个多模态基准上的评估表明：该定律能从8B参数模型外推迁移率至72B规模骨干；能高保真地预测完整VLM训练轨迹；能泛化到完全未见过的模型家族。此外发现若干可操作见解：某些文本基准与多模态性能负相关（暴露基准游戏行为）；基础LLM优于指令微调版本作为VLM骨干（吸收率更高、数据缩放衰减更低）；不同模型家族在迁移-吸收空间中占据不同位置。

Conclusion: 该框架将VLM骨干选择从昂贵的经验搜索转变为原则性的定量决策，为VLM架构设计提供了可预测、可解释的科学基础。
...
TL;DR: 本文提出了一种基于激活方向引导（activation steering）的角色条件代理筛选工作流，先在语言模型代理进入仿真环境之前，对角色配置进行筛选验证，并在OLMo-3-7B-Instruct上对275个角色的混合库存进行了评估，结果表明角色特定方向比先前的persona-vector控制方法具有更高的角色画像对齐度，且能保留更高的词汇多样性。关键发现是大多数角色随引导强度提升而改善，但有38个角色在所有维度上退化，因此应根据角色个体选择引导系数，而非统一使用高强度设置。


<details>
  <summary>Details</summary>
Motivation: 社交仿真中由语言模型构建的代理需要具备角色条件的可验证行为，而现有方法缺乏在代理投入仿真种群之前对其进行系统性筛选的机制。作者希望引入一种能够在部署前检查角色条件行为的工作流，避免未经筛选的角色配置在仿真中产生不符合预期的表现。

Method: 提出激活引导筛选工作流，具体步骤包括：定义角色画像、提取角色特定方向、扫描四个引导系数、评估角色画像对齐度、对每个候选配置进行通过或标记。在OLMo-3-7B-Instruct模型上，使用275个角色的混合库存和228个角色无关问题，由GPT-4.1-mini生成角色参考并作为评判者，将角色特定方向与先前persona-vector研究中的助手轴方向控制进行对比。

Result: 角色特定方向在角色画像对齐度上显著优于控制方法（平均总分63.2对41.1），并在较大引导系数下保持较高的词汇多样性，而控制方法的词汇多样性急剧下降。角色层面的筛选是主要实用输出：大多数角色随引导强度增加而改善，但38个角色在所有六个测量维度上均出现退化，表明需要按角色选择引导系数。

Conclusion: 本文证明了激活引导筛选工作流能够在部署前有效评估和筛选角色条件代理，并强调了按角色定制引导系数的重要性。研究提供代码和评估工件以供复现和进一步研究。
...
TL;DR: 本文提出了一种名为SAKE（Semantic-Aware Kernel Entropy）的训练无关引导方法，通过计算核Gram矩阵上的二阶Rényi熵，在嵌入空间中线性化该目标，得到可处理的引导信号，动态调整采样分布以在保真度和多样性之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在连续领域（如图像生成）取得了巨大成功，但将这种可控性迁移到离散的文本生成仍面临挑战。现有采样策略和引导方法仅调整词元似然，无法捕捉更广泛的语义景观，导致保真度与多样性之间的平衡不理想。

Method: 提出SAKE引导方法：1) 在核Gram矩阵上计算二阶Rényi熵，该矩阵同时捕捉跨词元语义交互和相对词元位置信息；2) 在嵌入空间中对目标函数进行线性化，推导出可计算的引导信号；3) 动态调整采样分布——在冗余时展平分布以鼓励探索，在多样化时锐化分布以保证保真度。

Result: 实验表明，SAKE方法在保真度和多样性之间取得了更优的Pareto前沿，并且在代码生成和数学生成等推理密集型任务的多样本性能上，优于温度缩放和离散引导基线方法。

Conclusion: SAKE作为一种无需训练的语义感知引导方法，有效解决了离散文本生成中引导控制与语义捕捉的难题，为文本扩散模型的采样质量提升提供了新思路。
...
TL;DR: 本文提出了一种通过小语言模型进行智能体选择与工具调用的方法，利用监督微调加强化学习，结合基于检索相关性和查询-智能体主题对齐的分层奖励函数，克服了传统基于意图路由的局限，在NDCG@10和延迟上均显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 传统的基于意图或主题的智能体路由方法存在根本性局限：无法利用检索内容的信号，也无法检测到主题对齐的智能体产生低相关性结果的问题，导致路由决策不准确。

Method: 训练一个小语言模型，先进行监督微调，再通过强化学习，联合执行智能体选择和下游工具调用的结构化参数生成。使用基于检索相关性和查询-智能体主题对齐的分层奖励函数，使模型从检索性能中学习任务相关的智能体适配性。

Result: 在智能体-查询不匹配的目标子集中，NDCG@10达到0.918（对比Amazon Nova Lite的0.539和Claude Haiku 4.5的0.490）；整体平均NDCG@10为0.771（较Nova Lite提升+0.177，较Haiku提升+0.219），平均选择延迟120.1ms，较Nova Lite降低82.4%。

Conclusion: 实验证明，通过将检索反馈纳入智能体选择决策，小语言模型能够在准确性和延迟上大幅超越基于意图路由的大语言模型基线，验证了以检索性能驱动的智能体选择范式的有效性。
...
TL;DR: 本文介绍了XL-DocBench，一个全新的人类全面验证的超长文档理解基准，包含来自六个专业领域的1,519个问题，上下文最长可达2,303页。该基准强调多证据页面（72.6%）、结构化数据使用（36.6%）和多文档推理（10.9%），并配有专家标注的证据页面、类型化验证规则和12种推理标签，弥补了现有基准在超长专业文档理解评估上的空白。


<details>
  <summary>Details</summary>
Motivation: 现实中的专业文档任务（如年报、法规、临床指南和技术手册）往往跨越数百或数千页，且部分问题需要跨文档比较，决策必须可追溯到特定证据页面。然而现有基准大多只衡量短上下文或单页问答，难以满足合规、临床、金融和工程等高风险场景对长文档理解能力评估的需求。

Method: 构建XL-DocBench采用树引导的合成流程（tree-guided synthesis pipeline），配合工件过滤器和194位人类专家的全面验证。基准包含12种推理标签、专家标注的证据页面、类型化验证规则和答案格式（含218个None-answer案例）。评估维度包括检索、证据使用和规则遵循，而非单一的排行榜分数。

Result: 实验结果表明，当前系统在长上下文、多页面证据以及结构化推理方面仍然表现不佳。该基准通过将超长专业上下文与页面级证据和类型化规则相结合，能够将系统失败归因于检索、证据使用或规则遵循等具体环节。

Conclusion: XL-DocBench填补了此前单页、短多页或纯文本长上下文基准留下的空白，为未来研究提供了更细粒度的失败归因能力，可用于改进LLM在专业文档理解任务中的可靠性和可追溯性。
...
TL;DR: 本文首次系统性地量化了小型语言模型（SLM）领域自适应中的可信度代价，涵盖3种架构、3个领域、2种训练数据条件和4种微调策略共216个实验配置。研究发现：基线QLoRA领域自适应对事实校准影响极小；对抗扰动数据反而提升适配质量且不损害可信度；三种安全保持策略均未能有效降低对抗性危害。


<details>
  <summary>Details</summary>
Motivation: 虽然参数高效微调（PEFT）在SLM领域自适应中的性能提升已被充分研究，但其对可信度（事实校准和对抗鲁棒性）的影响仍未被系统量化。在医疗、法律、金融等高危场景中部署SLM时，这一缺失令人担忧，亟需实证研究来揭示领域自适应的可信度成本。

Method: 作者设计了跨架构（TinyLlama 1B、Gemma-2 2B、Llama 3.2 1B）、跨领域（医疗、法律、金融）、跨训练数据条件（良性/对抗扰动）和跨微调策略（基线LoRA、Safety-DPO、Dark Experience Replay、TA-LoRA）的系统性实验，使用TruthfulQA MC2和HarmBench ASR分别评估事实校准与对抗鲁棒性，共216个配置×3个随机种子。

Result: 三个主要发现：(1) 基线QLoRA领域自适应对TruthfulQA MC2影响极小（平均|Delta TQA|<0.02）；(2) 对抗扰动数据使领域适配质量提升（Delta loss约-0.040）且不恶化可信度指标；(3) Safety-DPO几乎中性（Delta ASR<0.001），但Dark ER和TA-LoRA分别使安全对齐模型的HarmBench ASR平均增加+0.171和+0.155，个别配置超过+0.45。

Conclusion: 研究结果挑战了「重放式」和「算术合并式」策略能够将对齐能力迁移到领域自适应SLM的假设。安全保持策略在领域自适应场景下不仅未能降低对抗危害敏感性，反而可能显著增加风险，这对于部署关键领域SLM的实际选择具有重要意义。
...
TL;DR: 本研究证明仅凭文本描述即可预测早期初创企业成功（退出），利用风险投资数据集中的7,419家初创公司，构建850个文本特征，LightGBM达到F1=0.48，纯文本描述达到F1=0.30，并引入可量化的"炒作评分"（Hyping Score）用于风险投资应用。


<details>
  <summary>Details</summary>
Motivation: 在高度信息不对称的早期投资环境中，传统依赖财务和人力资本变量的预测方法存在局限，研究者希望探索创始人叙事文本本身是否蕴含可预测初创企业成功（退出）的独立信号。

Method: 使用风投精选数据集覆盖7,419家初创公司（20年跨度），通过初创企业叙事映射工程技术构建850个文本特征，对数据子集和向量嵌入进行统计显著性评估，采用六种监督机器学习模型进行实验，其中LightGBM表现最佳，并进行特征重要性分析。

Result: LightGBM实现最高预测性能（F1=0.48），纯文本描述符独立达到F1=0.30，验证了创始人叙事的独立预测价值。特征分析显示，适度密度的炒作标记（形容词、行话、流行词）与更高退出概率相关，而过长的陈述或名称长度则产生负面影响。

Conclusion: 创业者的文本框架本身提供了在信息不对称条件下预测退出的可测量信号，研究引入的Hyping Score可作为风险投资领域的实用工具，填补了纯文本分析在早期创业成功预测中的空白。
...
TL;DR: 本文提出了一种名为LLantia的全自动方法，结合脑连接组数据和文献中已发表的细胞类型功能描述，系统地推断神经回路功能及其组成细胞类型的功能。该方法在果蝇成体大脑上进行了验证，结果具有层次结构，可回溯到已知文献信息。


<details>
  <summary>Details</summary>
Motivation: 连接组图谱绘制已成功完成，但神经回路的功能解读成为新的挑战。需要一种自动化方法系统性地推断神经回路功能及其中各细胞类型的作用，以弥合连接组结构和功能之间的鸿沟。

Method: LLantia方法从文献中提取细胞类型功能描述作为先验知识，结合连接组数据推断所有其他细胞类型的功能，进而基于此推断神经回路的功能。结果以层次化结构组织，涵盖多个行为与生理学背景，每个回路功能由子回路描述及相关细胞类型组成。

Result: 该方法成功推断出果蝇成体大脑所有细胞类型的功能，并对其中的多个广泛回路进行了功能推断。验证结果显示，分析与发表后文献的交叉对照结果一致，证明方法的正确性。

Conclusion: LLantia提供了一种可扩展的自动化神经回路功能推断框架，既支持向已有文献的回溯验证，也为未来实验研究提供了基础支撑，有助于加速从连接组到功能理解的转化。
...
TL;DR: 该论文提出LLM-OSDA，一种将最优停止理论融入LLM原生广告拍卖的动态点击付费机制，将广告投放从单次响应中的固定槽位扩展到对话过程中动态时刻的选择，实现时序与分配联合优化，并通过信封定价保证激励兼容性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM广告拍卖机制仅在一单次响应内分配广告位，忽略了"何时插入广告"这一时序问题。因为每次会话仅有一次原生插入机会，停止时间取决于出价，从而与分配耦合，静态真实性论证不再适用，需要新的动态机制设计。

Method: 提出LLM-OSDA：1) 将Bellman最优停止与赢家分配及信封定价整合为动态CPC拍卖机制；2) 使用与出价无关的LLM层估计上下文点击质量并渲染广告，出价仅进入承诺机制；3) 在实际部署中使用StopNet近似Bellman动作价值函数。

Result: 理论层面证明在精确Bellman预言下，期望折扣点击分配对每个广告主出价单调，信封支付使诚实出价在期望意义下弱占优；近似层面证明StopNet仅在停止边界附近与最优策略有差异，并以近似误差界定了激励损失上界。实验表明相比最强固定时序基线，净收入提升11%且用户留存相当。

Conclusion: 通过将最优停止理论引入LLM广告拍卖，该工作为对话式广告的时序优化提供了理论基础和可部署方案，在激励兼容与收益提升之间取得了良好平衡。
...
TL;DR: DiffusionGemma是一个实验性的开源权重语言模型，利用离散扩散技术并行生成256个token的文本块，而非逐token顺序解码。通过微调Gemma 4混合专家模型获得，其两阶段训练流程只需不到原始模型10%的训练token预算，在单个NVIDIA H100 GPU上实现每秒约1500个输出token的速度，同时保持思考模式、多模态输入和长上下文支持。


<details>
  <summary>Details</summary>
Motivation: 传统自回归(AR)大语言模型存在顺序解码瓶颈，生成速度受限。本文旨在通过离散扩散机制替代逐token解码，以并行方式同时生成多个token，从而大幅提升文本生成的推理速度。

Method: 采用计算高效的两阶段训练流程：第一阶段使用监督微调(SFT)教授双向去噪能力；第二阶段结合强化学习(RL)与采样器蒸馏，同时优化生成质量和推理效率。模型通过在Gemma 4 (3.8B激活/25.2B总参数)上微调获得，利用少于10%的原始训练token预算。

Result: DiffusionGemma在生成速度与模型能力之间建立了新的帕累托前沿。平均每次前向传播生成约20个token，在单张NVIDIA H100 GPU上达到每秒约1500个输出token，显著快于即使采用最先进投机解码的AR模型。同时保留了思考模式、多模态输入和长上下文支持，且仍具备AR生成能力，仅有轻微性能下降。

Conclusion: 离散扩散微调是构建高速文本生成模型的有效途径，开辟了生成速度与能力的新平衡点。模型能够同时支持扩散解码和AR解码，暗示了混合扩散-AR解码的可能路径，为未来高效语言模型的设计提供了新方向。
...
TL;DR: 本论文提出了C-Guard和C-LIM两个工具来数据高效地训练RL对齐中的安全护栏，解决安全目标与不拒绝良性提示之间的冲突目标问题。


<details>
  <summary>Details</summary>
Motivation: RL对齐中普遍存在冲突目标，训练安全护栏需要在"捕捉真实危害"和"不拒绝良性提示"之间取得平衡，而现有方法在此类冲突目标上的数据高效训练很困难。过度拒绝与拒绝不足之间存在此消彼长的困境。

Method: 提出C-Guard（宪法网格工具）用于生成RL训练数据，以及C-LIM（单元可学习性评分）决定每个单元的操作为：剪枝、加密、修正或扩展。C-LIM在训练预算投入之前就标记出无效数据区域。

Result: 过度拒绝率从22.4%改善至12.8%；对抗性攻击下的拒绝不足从0.27恶化到0.33。C-LIM识别出187行未产生任何收益的无目标数据行，并将同一区域的学习影响力从0.733提升至0.80。

Conclusion: 通过C-Guard和C-LIM的组合，能够在训练前识别并消除无效数据区域，提升数据利用效率和模型学习效果，代码和宪法均已开源。
...
TL;DR: 本文发现广泛使用的文本摘要忠实度基准中的人类标注存在"平均偏差"——人类标注者倾向于在大多数句子忠实时就判定整个摘要为忠实，而非严格执行"所有句子都必须忠实"的合取规则。作者通过五个大语言模型裁判对四个基准进行逐句评估，证实全局人类标签与逐句判断的平均值相关性更高，而非严格的合取规则实现。


<details>
  <summary>Details</summary>
Motivation: 忠实度评估理论上应遵循严格的合取规则（每个句子都必须有源文档支持），但现有基准只收集每个摘要一个全局人类标注。作者质疑这些全局标注是否真的实现了合取规则，并假设人类标注者可能存在"平均偏差"——接受大部分句子忠实而非全部忠实的摘要。

Method: 使用五个大语言模型作为逐句评分器，在四个广泛使用的忠实度基准上评估摘要。将全局人类标签分别与逐句判断的平均值及严格合取规则实现进行相关性比较，并通过人工审查验证人类标记为忠实的摘要中是否包含真实的局部事实错误。

Result: 全局人类标签与逐句大语言模型判断的平均值相关性更好，而非与严格合取规则的实现相关。人工审查确认有相当比例被人类标记为忠实的摘要包含真实的局部事实错误，证实了平均偏差的存在。

Conclusion: 广泛使用的忠实度基准中的人类标注存在可测量的平均偏差，这呼吁需要精心设计的标注结构以获取可信赖的人工标注。该发现对基于这些基准训练的模型评估与开发具有重要警示意义。
...
TL;DR: 本文通过机制分析发现阿拉伯语医学知识存在于LLM中间层表示中，但无法在输出层浮现。基于此提出TLoRA，仅在跨语言表征分叉的层窗口进行针对性低秩适配，在医学问答、短文生成和临床对话任务上超越全网络LoRA及零样本/少样本基线。


<details>
  <summary>Details</summary>
Motivation: 现有假设认为LLM在阿拉伯语医学任务上表现不佳是因为训练数据不足，但作者通过机制分析方法系统质疑这一假设，发现问题的本质是知识在输出阶段无法被正确"提取"，而非知识缺失。

Method: 使用tuned lens probing和causal activation patching进行机制诊断，定位知识存在于中间层但无法在输出层涌现的层窗口；据此提出TLoRA（Targeted Low-Rank Adaptation），仅对表征分叉的层窗口进行参数适配；同时构建了AraClinicDialog基准数据集。

Result: TLoRA在多项选择医学问答任务上优于全网络LoRA、零样本和少样本基线；在短文生成和多轮临床对话任务上无需任务特定微调即表现竞争力。

Conclusion: 机制诊断可以作为低资源语言医学LLM定向适配的实用指南，证明了"知识存在但无法浮现"的机制洞察可直接指导更高效的适配策略。
...
TL;DR: 本文提出PRISMS框架，利用MLP神经元激活实现工具调用失败（无效参数、过度调用、遗漏调用）的稀疏检测与激活引导，在六个模型上实现高ROC-AUC且仅需少量神经元，并通过条件干预大幅降低过度调用率、提升工具调用准确率。


<details>
  <summary>Details</summary>
Motivation: Agentic LLM在工具调用中存在三类关键失败模式：无效参数、不必要的过度调用和必需时的遗漏调用。现有方法缺乏轻量且通用的检测与干预机制，需要一种既能稀疏检测失败风险又能实施定向干预的框架。

Method: PRISMS通过三个步骤实现：(1) 识别对特定失败类型贡献关键的MLP神经元子集；(2) 在这些神经元激活上拟合L1正则化稀疏检测器；(3) 共享相同神经元基底进行激活引导（steering），并根据预测的失败风险门控干预，避免无条件引导带来的副作用。

Result: 在Qwen3、Llama和Gemma六个模型上，过度调用和遗漏检测在生成前提示边界达到ROC-AUC 0.90-1.00，无效参数检测在生成工具调用片段达ROC-AUC 0.86-0.90。检测仅需1-2个（遗漏）、2-16个（过度调用）和约128个（无效参数）MLP神经元。PRISMS将过度调用率从0.131降至0.026（降80%），工具需求准确率从0.689提升至0.831（+14.2个百分点）。

Conclusion: PRISMS证明了失败特定的稀疏MLP神经元基底可用于高效的失败检测与双向行为控制，且通过风险门控干预可有效减轻无条件引导的副作用，为跨模型家族的轻量级工具调用可靠性提升提供了可行方案。
...
TL;DR: 本文评估了九个前沿视觉语言模型在两项心理学基准任务（Keysar导演任务和Frith-Happé动画三角形任务）上的心智理论（ToM）表现，发现模型的心智理论轮廓在不同任务间呈碎片化状态：没有任何模型在两项任务中都能匹配典型发育成年人，部分模型在导演任务中表现像儿童，在三角形任务中则更接近高功能自闭症成人。


<details>
  <summary>Details</summary>
Motivation: 探究前沿视觉语言模型在心智理论任务中的表现是否具有跨范式的一致性，即模型是否在不同任务中都能匹配同一人类参照群体的表现，还是其心智理论轮廓会随任务范式的变化而碎片化。

Method: 对九个前沿视觉语言模型在两个心理学基准任务上进行评估：（1）Keysar导演任务，测试视觉观点采择能力及自我中心干扰下的表现；（2）Frith-Happé动画三角形任务，使用Castelli评分量表测试从纯运动模式中归因意图的能力。

Result: 在导演任务中，无思维链提示时模型面板在78%的试验中出现自我中心错误（类似儿童而非成人），模型间差异显著，推理提示可拯救部分模型。在三角形任务中，模型面板整体低估意图归因，其心智理论轮廓距离高功能自闭症成人均值比典型发育成人均值近三倍以上，而目标导向和随机运动维度的表现接近典型发育成人。

Conclusion: 没有任何模型在两项任务中都最接近典型发育成人；在导演任务中表现成人化的模型在三角形任务中偏向高功能自闭症侧，而在三角形任务中最接近典型发育成人的模型在导演任务中表现儿童化。研究仅报告群体层面描述，不对任何模型作诊断标签。
...
TL;DR: 本研究通过十个人工智能模型家族的十六个语言模型对一个心理治疗案例进行表述，发现集成模型平均产生1.69种不同表述（单个模型基线为1.43）。研究定义了每个模型的"异议贡献"（与集成其他成员的平均相似度的补数），用以识别最分歧的声音。预注册假设检验发现模型身份确实解释了异议方差的一部分，但常见分类（规模、家族）只能部分恢复这种结构，且最分歧的声音随集成组成而变化，说明异常值描述的是集成而非模型本身。分歧与案例的诠释开放性无关，而是由临床内容组织。


<details>
  <summary>Details</summary>
Motivation: 集成模型的核心前提是多个模型提供多个视角，但现有文献通常用正确性标准来验证多样性或不确定性，而本任务（心理治疗案例表述）不存在唯一的正确性标准。此外，现有多样性度量（如Vendi分数）只告诉集成整体有多少不同元素，却不能解释多样性的来源。因此需要用一种不依赖正确性标准的方法来量化并解释集成分歧的来源。

Method: 使用Vendi分数（相似性矩阵的冯·诺依曼熵的指数）度量多样性；定义"异议贡献"为每个模型对集成其他成员平均相似度的补数；使用预注册假设检验跨模型和案例，检验模型身份是否解释异议方差的非零份额；由专家组编写十五个分层小故事，共获得7,082个表述用于分析。

Result: 十六个模型的集成平均产生1.69种不同表述，高于单一模型的1.43种基线。模型身份确实是一个可检测的分歧结构因素，但常规类别只能部分恢复：规模差异在成对比较中方向相反，家族仅在五个双成员线上分组模型，最分歧的声音随集成组成而改变。分歧不追踪案例的诠释开放性分层，而是由临床内容组织。

Conclusion: 集成分散性是一个需要测量而非假设的属性。模型身份影响异议但无法用现有类别（规模、家族）简单解释；异常值描述的是集成而非模型的固有属性；分歧的组织原则是临床内容而非诠释开放性。这提示评估集成模型时应将分散性视为实证测量的对象。
...
TL;DR: 本文构建了一个涵盖德国三种政治场域（全体会议、委员会会议、新闻发布会）的17k句论证语料库，并进行了自动识别论证段落的试点研究，发现不同场域间论证模式存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 政治论证模式在不同场域（如全体会议与委员会会议）之间存在显著差异，但现有计算研究极少关注场域间政治论证模式的差异分析，存在研究空白。

Method: 构建了一个17k句子的语料库，在三个德国政治场域（全体会议、委员会会议、新闻发布会）中对论证段落（论证及其理由，包括边界和类别）进行标注，主题（COVID-19）保持一致；随后进行自动识别论证段落的试点研究。

Result: 语料分析发现，与预期相反，领域专业知识的论证在新闻发布会中比在委员会会议中更频繁。试点研究表明，论证边界难以精准界定，且模型预测存在确认偏差问题。

Conclusion: 研究填补了跨场域政治论证模式计算分析的空白，提供了新的标注语料资源，并揭示了自动识别论证段落的挑战与模型偏差问题。
...
TL;DR: SeDeM是一种选择性解压缩框架，将紧凑记忆存储与解码器条件分离。它通过LLM从中间Transformer层提取隐藏状态，用轻量级压缩器存储为记忆块，查询条件选择器挑选相关块，解压器仅扩展选中块供解码器使用，从而避免全上下文处理和直接从高压缩记忆槽生成，在四个长上下文QA基准上超越压缩基线，并降低首token延迟。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理成本高昂——自注意力在预填充阶段随序列长度二次增长，KV缓存随处理token数量增长，且更大的上下文窗口并不保证可靠的证据使用。现有软压缩方法依赖LLM作为压缩器并通过紧凑记忆token同时承担信息保存和解码条件化，效率有限。

Method: SeDeM将紧凑记忆存储与解码器条件解耦：1) LLM从选定的中间Transformer层提取隐藏状态；2) 轻量级压缩器将其存储为记忆块；3) 查询条件选择器选择相关块；4) 解压器仅将选中块扩展为与中间解码器层兼容的隐藏状态。训练时选择器使用块级证据监督。

Result: 在四个长上下文QA基准上，SeDeM在1B和3B同骨干设置中均获得比压缩基线更高的QA分数；3B骨干在三个数据集上超过全上下文微调；相比ICAE，SeDeM降低了在线首token时间并提高了自回归解码吞吐量。

Conclusion: SeDeM通过解耦记忆存储与解码器条件化，结合选择性解压缩策略，在保持高质量长上下文推理的同时显著降低了计算成本，验证了块级证据监督训练选择器的有效性。
...
TL;DR: 本文研究大语言模型能力提升在不同任务难度上的分布。作者发现大部分所谓"向更难任务迁移"的表象并非能力曲线形状的真正变化，而是由于单一Rasch模型的"天花板效应"所致。通过引入LiveCodeBench基准（无agentic框架、外生难度排序），在控制整体能力提升后，发现2024年9月后发布的模型在最难问题上仍有+0.40 logits的超额增益，将难题解决率从约18%提升到25%。该效应主要由最强推理模型驱动，且仅限于需要短推理的硬任务，而非长时间自主性任务。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM进展总结通常使用单一标量指标（如时间跨度、能力估计或基准总分），但这些指标无法检验能力提升在任务难度维度上的分布是否真的发生了变化。作者希望区分"天花板效应"（模型整体能力上升导致的表面难度迁移）与真实的"硬任务专门化"效应，并消除agentic benchmark中模型与脚手架混杂的混淆因素。

Method: 首先在METR时间跨度数据上使用单一Rasch模型验证"天花板效应"可解释大部分难度迁移现象。然后引入LiveCodeBench——一个无agentic脚手架、使用外生难度排序的公开竞赛编程基准，涵盖66个有明确发布日期模型和1,055道题。通过控制整体能力提升后的残差分析，识别出模型在最难问题上的超额增益。

Result: 在METR数据上，单参数Rasch模型（仅能力上升）即可重现难度迁移模式，说明大部分现象由天花板效应解释。在LiveCodeBench上，经过保守校正后，2024年9月后发布的模型在难题上仍有+0.40 logits的超额增益，难题解决率从约18%升至25%。该效应主要由最强的推理模型驱动，且对需要短推理而非长时间自主性的硬任务成立。

Conclusion: LLM进步在"硬任务"上的真实增益是存在的，但其规模远小于表面指标所示；该效应主要体现为推理能力在短推理硬任务上的突破，而非长时序自主能力的质变。这一结论目前仅对竞赛编程场景有干净的识别依据，需要更多类似的无脚手架、外生难度的基准来验证其普适性。
...
TL;DR: 本文提出Deep Research Pretraining (DRP)，一个离线预训练框架，从已有引文或超链接结构中提取预测性导航监督信号，将自然存在的证据结构转化为搜索-打开-写作轨迹，无需实时检索环境或策略回滚即可训练深度研究智能体，在数据效率上显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 深度研究智能体的训练通常依赖昂贵的、基于环境交互的工具使用轨迹，需要反复检索、文档检查和报告评估，成本高昂且难以规模化。作者希望找到一种无需实时代价高昂的检索环境即可有效训练智能体导航能力的替代方案。

Method: DRP从含引文或超链接的文本段落中构建代理研究目标，恢复链接证据及图相关替代选项，将其转化为搜索-打开-写作轨迹。在学术引文图（DRP-Paper）和维基百科超链接（DRP-Web）上实例化，在Qwen3-14B-Base上连续预训练1B token，并在13K条智能体轨迹的不同比例子集上进行微调。

Result: 在五个独立采样的低数据预算子集上，两种DRP变体在DeepResearch Bench上均一致优于无DRP的对照模型。仅用四分之一的SFT数据，DRP-Web甚至超过固定的全数据无DRP检查点，且收益可迁移至ResearchQA、WebWalkerQA和SimpleQA。DRP-Web的优势在后续agentic RL中依然保持。对照实验表明改进源于证据条件化导航而非领域暴露或格式模仿。

Conclusion: DRP提供了一种有前景的、与基于轨迹的智能体训练互补的预训练方法，能够显著提升数据效率并带来可迁移的性能增益，为深度研究智能体的低成本训练开辟了新路径。
...
TL;DR: AdaMTP提出了一种自适应多令牌预测训练范式，通过基于熵的分割算法动态调整预测深度，避免跨语义边界产生噪声梯度，在推理速度和任务性能上均优于标准MTP。


<details>
  <summary>Details</summary>
Motivation: 现有MTP训练框架采用固定长度的预测视野，忽略了自然语言和代码中信息密度的非均匀性。强制辅助头跨高熵语义边界进行预测会注入噪声和冲突的训练信号，由于辅助头共享骨干网络的潜在表示，这些梯度会反向传播并干扰模型的核心能力。

Method: AdaMTP的核心是一种基于熵的分割算法，利用基础模型检测不确定性突然激增的位置作为语义边界，将序列划分为可变长度分组。每个token被分配一个自适应的预测深度，并通过动态掩码的MTP目标函数抑制跨边界预测的损失，从而衰减损害骨干网络的噪声梯度。

Result: 在数学推理、代码生成和通用基准上，以三个骨干模型（Llama-3.1-8B、Qwen-2.5-7B、Gemma-3-12B）进行实验，AdaMTP在任务性能和推理加速两方面均一致优于标准MTP。

Conclusion: 自适应预测视野比固定预测视野更符合序列的内在可预测性，能够有效减少噪声梯度对骨干模型的影响，同时保持甚至提升推理加速效果。
...
TL;DR: SERL-SQL提出了一种选择性执行反馈强化学习框架，用于多轮Text-to-SQL智能体。该方法通过训练专用教师模型对学生动作进行执行反馈重新评分，将师生似然差距转化为有界掩码权重，仅对SQL和工具动作token的GRPO优势进行重新加权，实现了细粒度的信用分配。在BIRD-Dev上达到76.56%的执行准确率，在Spider-Test上达到89.92%。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL系统大多使用执行正确性作为轨迹级别的奖励，这种粗粒度的奖励无法识别具体是哪个SQL决策导致了成功或失败，导致信用分配不精确，限制了强化学习的优化效果。

Method: SERL-SQL包含三个关键部分：(1) 采样on-policy的SQL交互轨迹；(2) 使用仅在训练阶段启用的教师模型，基于执行反馈对学生动作进行重新评分；(3) 将师生似然差距转换为有界掩码权重，仅对SQL和工具动作token的GRPO优势进行重新加权，实现局部信用分配。

Result: 在BIRD和Spider等跨域基准上表现优异，BIRD-Dev达到76.56%执行准确率，Spider-Test达到89.92%。奖励选择策略接近Oracle Best-of-N上限，且优于基于一致性的选择方法。

Conclusion: SERL-SQL通过执行反馈进行细粒度信用分配，有效提升了多轮Text-to-SQL智能体的优化效果。其基于奖励的候选选择策略能可靠地识别高质量SQL候选，证明了该方法的实用性和优越性。
...
TL;DR: 本文提出了一套构建俄语自杀倾向文本检测数据集的详细方法论，并据此收集标注了超过5万条来自社交媒体的文本，还进行了基础分类实验并公开了数据集和代码。


<details>
  <summary>Details</summary>
Motivation: 俄罗斯自杀率较高，部分有自杀倾向的人在社交媒体上留下文字线索，但这些珍贵信息淹没在海量无关文本中，严重阻碍了对个体自杀风险的快速评估和干预，因此需要有效的方法从社交媒体噪音中识别出自杀前兆文本。

Method: 本文详细阐述了数据集构建方法论，包括指令和类别表的创建、标注流程、验证以及标注后修正。基于该方法论，收集并标注了超过5万个俄语社交媒体文本，并进行了基础分类模型的实验来评估不同标注层级下的性能。

Result: 成功构建了包含5万多条文本的大规模俄语自杀倾向检测数据集，提供了数据集的统计数据和标注中常见问题的分析，并在不同标注层级上进行了基础分类实验，展示了可用的性能表现。

Conclusion: 该数据集和代码已公开发布，为俄语自杀倾向文本检测领域提供了重要资源和基准，有助于加速识别有自杀风险人群并促进干预。
...
TL;DR: 本文通过训练约700万参数的Transformer编码器于儿童导向语音和朗读语音上，评估英语、法语和中文的语音音素辨别能力，发现学习目标（而非架构）是知觉窄化（感知窄化）表征变化的一阶决定因素。重建目标（掩码mel预测）会损害非母语音素辨别，而预测目标（帧对比）则改善之；多目标配置均无法产生完整的发育特征（母语提升且非母语下降）。


<details>
  <summary>Details</summary>
Motivation: 知觉窄化是发展心理学的经典发现，即婴儿在一岁内丧失对非母语音素的辨别能力，但其产生的学习目标（learning objective）是什么仍然是一个开放问题。本文旨在回答何种学习目标能够产生这一发展现象。

Method: 使用约7M参数的Transformer编码器，在儿童导向语音（child-directed speech）和朗读语音（read speech）上训练，评估英语、法语和中文三种语言的音素ABX辨别任务。实验在十个随机种子下重复进行，以种子为复制单位。比较重建目标（掩码mel预测）与预测目标（帧对比）的跨语言迁移差异，并测试六种目标配置（锐化、压缩、巩固、组合以及两种词级语义接地形式）能否产生完整的发育特征。

Result: 六个主要结果：(1) 目标设定决定跨语言迁移方向，重建目标使非母语辨别退化，预测目标使其改善（第一层中文ABX差异+0.051，p=3×10⁻⁸）；(2) 退化由大的臂内固有难度梯度和小的语言特化效应组成；(3) 重建目标将第一层推到低于输入的可辨别性，预测目标则推高；(4) 朗读语音的非母语下降斜率比儿童导向语音陡3.6倍；(5) 常规三个种子不足以可靠观测此效应（七成三种子子集能发现显著性）；(6) 六种目标配置均无法产生完整发育特征，因为单一目标作用于共享表征时会使两种语言同向移动。

Conclusion: 学习目标而非架构是知觉窄化形态表征变化的一阶决定因素。这一结论对发展心理学中关于知觉窄化机制的理论建模具有重要启示，也强调了在计算建模中增加复制种子数量以获得可靠结论的必要性。
...
TL;DR: 本文提出"语态"（state）不应仅被视为亚非语系附属结构的语言特异性现象，而应理解为合成语中一种系统性的、依赖语境的形态句法机制。作者在基于模板的模块化认知框架下，以里夫语为主要实证基础，将语态形式化为模板集合上的集值函数，并设计了基于有限集合运算的学习算法，为各种传统上独立分析的名词标记模式提供了统一解释。


<details>
  <summary>Details</summary>
Motivation: 传统的"语态"概念被局限于亚非语系的构造（附属）状态，被视为一种语言特定的形态句法现象。本文试图突破这一限制，论证语态实际上是跨合成语普遍存在的系统性、依赖语境的形态句法机制。作者希望通过统一的理论框架解释各种传统上被独立处理的名词标记模式，并将其与更广泛的句法依赖（如一致关系和语法格）联系起来。

Method: 作者在"基于模板的模块化认知框架"（Template-Based Modular Cognitive framework）下提出理论，以里夫语（Riffian）为主要实证基础。语态被形式化为一种集值函数，作用于语法模板集合之上。此外，作者设计了一个基于有限集合运算的学习算法，用于获取和预测依赖于语态的语法配置，并以符号计算模型实现该理论。

Result: 该框架为传统上被独立分析的各种名词标记模式提供了统一的解释，尤其是在限定词-名词结构方面提出了一致性分析。研究结果表明，语态构成了更广泛一类句法条件依赖关系中的一个实例，这类依赖关系还包括一致关系和语法格。学习算法能够通过有限集合运算有效获取和预测依赖语态的语法配置。

Conclusion: 语态不应被限定为亚非语言的附属状态现象，而应被视为跨语言中一种系统性的形态句法机制。该理论不仅对名词形态提供了统一解释，还对名词结构和词汇认知的理论具有更广泛的意义。语态属于包括一致关系和语法格在内的一类句法条件依赖关系的实例之一。
...
TL;DR: 本文提出S⁴R方法，通过从选择性采样token构建低秩子空间，并在解码阶段进行稀疏重建KV表示，实现高达5倍的KV缓存压缩，同时保持接近全缓存精度的长上下文性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）的上下文窗口扩展显著提升了长上下文能力，但KV缓存带来了高昂的内存开销。现有低秩压缩方法面临两难：离线方法依赖外部校准数据，在线方法则因全提示分解与重建而产生大量计算开销。

Method: S⁴R方法包含两个核心策略：（1）使用提示感知（prompt-aware）初始化，从代表性提示子集中构建初始键/值基，平衡校准数据依赖与预填充成本；（2）采用稀疏重建策略，在解码阶段仅保留信息密集的位置进行KV表示重建，避免每一步完全重建缓存带来的昂贵计算。

Result: 在LongBench和RULER基准上，使用Llama和Qwen模型家族的广泛实验表明，S⁴R在高达5倍KV压缩下仍能保持接近全缓存的精度，结合了固定压缩的高效性与提示相关方法的自适应性。

Conclusion: S⁴R有效解决了KV缓存压缩中离线校准数据依赖与在线计算开销之间的权衡问题，通过选择性token采样和稀疏重建实现了高效的KV压缩方案。
...
TL;DR: 本文提出OSCD（Onramp-Sequence Cross-Distillation）后训练算法，解决大语言模型在低资源语言环境下进行复杂推理时的跨语言崩塌（回退到英语）问题。该方法通过集成翻译智能体循环将高资源推理轨迹投影到低资源词汇子空间，并结合联合嵌入语义对齐弥合跨语言表征差距，在AIME25和HMMT25基准上实现了东南亚本地语言数学推理最多3.2倍的性能提升。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理能力上虽有显著进展，但在低资源母语场景中常出现跨语言崩塌现象——即复杂逻辑推理的中间步骤会回退到英语。这给策略优化带来冷启动瓶颈，而标准微调又因跨语言表征漂移存在灾难性遗忘风险。因此需要一种既能避免遗忘又能稳定迁移推理能力到低资源语言的训练方法。

Method: OSCD是一种后训练算法，包含两个核心组件：(1) 通过集成翻译智能体循环（translator agentic loop），在生成训练rollout过程中将高资源语言的推理轨迹投影到低资源语言的词汇子空间，实现动态生成参考样本的稳定高效翻译用于微调；(2) 对参考语言和目标语言的推理轨迹进行联合嵌入语义对齐（joint-embedding semantic alignment），弥合跨语言表征差距。

Result: 在AIME25和HMMT25基准上的综合评估表明，OSCD在东南亚本地语言的数学推理任务上实现了最多3.2倍的整体性能提升，其中联合嵌入语义对齐组件相比仅翻译的基线方法在语言去偏上贡献了最多6.4%的提升。

Conclusion: OSCD通过将高资源推理轨迹有效迁移到低资源语言空间并辅以语义对齐，成功缓解了跨语言崩塌问题，为低资源语言环境下的大模型推理能力提升提供了可行方案。
...
TL;DR: 本文提出DE-NER框架，通过对话引导方式激发大语言模型的聊天能力来完成零样本命名实体识别，无需人工设计提示词和示例，在多个基准上平均提升3.75%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有的基于大语言模型的零样本命名实体识别方法通常将序列标注转化为问答任务，但这类方法依赖于繁琐的提示词（prompt）和示例（demonstration）工程，需要大量人工干预，效率低下且泛化性受限。

Method: 提出了DE-NER（Dialogue Elicitation framework for NER），一种对话引导框架，通过自然对话的形式激发大语言模型内在的聊天能力，让模型在对话交互中充分提取和调用其编码的知识来完成实体识别任务，从而减少对人工提示词和示例设计的依赖。

Result: 在多个零样本命名实体识别基准上，DE-NER方法优于现有的强基线方法，平均提升了3.75%的F1分数。

Conclusion: DE-NER验证了通过对话引导方式可以有效利用大语言模型的知识进行零样本NER，只需最少的人工干预即可达到更优性能，为减少提示工程负担提供了新思路。
...
### [130] [Uncertainty-Aware Simulation-Based Inference for Operations Research with Large Language Models](https://arxiv.org/abs/2608.00019)
*Liang Guo,Lin Shaochong,Shen Zuo-Jun Max,Zhang Kun*

Main category: cs.LG

TL;DR: 本文提出一种无需训练的不确定性感知推理框架，通过短视前瞻模拟和重要性重采样来评估中间候选步骤，从而提升大语言模型在运筹学数学建模任务中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在运筹学任务中的部署面临挑战，因为正确性依赖于连贯的建模过程而非仅正确的最终答案。标准自回归生成基于短视策略，无法预判局部步骤能否有效扩展为全局一致的优化模型，导致局部看似合理的步骤可能引发灾难性的下游错误。

Method: 提出一种不确定性感知、免训练的推理框架。该方法在不更新模型参数的前提下，利用短前瞻模拟评估中间候选步骤，量化下游预测不确定性或概率集中度，并通过重要性重采样动态选择更可能产生连贯数学表述的候选方案。

Result: 在多个运筹学基准测试（包括NL4OPT、MAMO和IndustryOR）上的实验表明，该框架持续优于标准和低温基线，建立了一种高效且无需训练的可靠运筹学表述生成范式。

Conclusion: 该研究验证了通过不确定性感知的推理策略，可以在不进行模型训练的情况下显著提升LLM在运筹学数学建模方面的可靠性，为相关应用提供了一种高效的替代方案。
...
### [131] [Learning Compositional Meta-Routing for Agentic Workflows: An Executable Benchmark](https://arxiv.org/abs/2608.00106)
*Natan Vidra,Alina Kapanova,Arun Kanhai,Spurthi Setty*

Main category: cs.LG

TL;DR: 本文提出了一个可执行的基准测试和一个预算感知的元路由器，用于从原始任务文本中组合异构操作（如检索、代码执行、委派等）。在保留测试集上，学习策略达到100%成功率，比静态策略成本低43%；但在词汇偏移的挑战集上，成功率降至75.9%，落后于静态路由的93.5%，表明词汇泛化是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 智能体系统不仅要决定产生什么答案，还要决定在推理和执行之前应执行哪些操作。现有路由工作大多孤立地选择模型端点、检索深度或工具，缺乏能将异构操作从原始任务文本中组合起来的系统方法。

Method: 构建了一个包含216训练/72开发/108测试/108词汇偏移挑战任务的可执行基准，覆盖数据分析、冻结语料库研究和文档处理。采用独立的正则化逻辑回归头从词和字符特征预测操作概率，通过温度缩放校准，并在路由成本和动作数量预算下进行贪婪组合。

Result: 在保留测试集上，学习策略达到100%成功率（静态和固定工作流为93.5%），成本比静态策略低43%；匹配的一次性路由器仅达56.5%。在未触碰的挑战集上，学习策略降至75.9%，落后于静态路由的93.5%，但成本仍低49%，且超过一次性路由34.3个百分点。

Conclusion: 词汇泛化而非路由执行是主要限制因素。该工作建立了可复现的测试平台和有界的概念验证，不声称代表实时LLM的性能表现。
...
### [132] [MetaRoute-Bench: Evaluating Meta-Decision Policies for Agentic Workflow Routing](https://arxiv.org/abs/2608.00107)
*Natan Vidra,Alina Kapanova,Arun Kanhai,Spurthi Setty*

Main category: cs.LG

TL;DR: 本文提出MetaRoute-Bench，一个开放可审查的基准框架，用于在共享执行模型下比较智能体系统的元决策（路由）策略。基准包含180个合成任务、8种路由策略和30组随机种子，通过43,200条轨迹评估，任务感知的组合策略达到79.4%的成功率，优于静态策略（76.7%）、一次性路由（67.4%）和直接回答（52.9%），但代价是更高的成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 智能体系统需要反复做出元决策（直接回答、分解任务、调用工具、执行代码、委派专家、验证中间结果或从失败中恢复），这些决策影响任务成功率、成本和延迟，但通常被嵌入编排框架中，仅通过聚合任务准确率来评估，缺乏可复现的对比方法。

Method: 构建MetaRoute-Bench框架，包含180个合成任务配置（覆盖数据分析、研究和文档处理）、8种路由策略、30组配对随机种子，采用种子化的离线执行模型生成43,200条轨迹，并通过消融实验分析路由组合和验证操作的影响。

Result: 任务感知组合策略达到79.4%成功率，优于工作负载特定静态策略（76.7%）、一次性任务路由（67.4%）和直接回答（52.9%）。相对于静态策略提升2.7个百分点（95% CI ±2.0），但成本增加4.7%，延迟增加6.4%。消融实验中，限制路由组合为单一操作和移除验证时性能损失最大。

Conclusion: MetaRoute-Bench的贡献在于提供可复现的评估方法和路由策略权衡分析，而非生产环境有效性证明。作者发布了任务生成、策略、轨迹、测试和分析工件，以支持真实系统的验证。
...
### [133] [Progressive$^2$: A Teacher-Student Progressive Co-Evolving Knowledge Distillation Method for Substantial Model Compression](https://arxiv.org/abs/2608.00129)
*Tiancong Cheng,Ying Zhang,Zhiwen Yu,Yifang Yin,Bin Guo*

Main category: cs.LG

TL;DR: 本文提出Progressive²，一种渐进式知识蒸馏方法，通过"渐进增强的教师"和"渐进缩小的学生"相结合，解决大模型与小模型能力差距过大时蒸馏性能严重下降的问题。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏在教师模型能力与学生模型需求差距过大时，蒸馏性能会显著下降，这限制了KD在服务端模型压缩中的应用效果。

Method: Progressive²包含两个核心策略：(1) 教师侧：按从浅层原始特征到深层语义特征的顺序，逐步选择更多层参与蒸馏，形成系统化的学习课程，并设计基于Lipschitz连续性理论支持的多特征融合适配器以提升训练稳定性；(2) 学生侧：逐步缩小网络规模，与教师形成迭代协同进化。

Result: 教师侧的渐进策略可独立部署，在准确率与训练效率之间取得最优平衡；教师与学生的联合集成可进一步提升整体性能。

Conclusion: Progressive²是一个灵活的框架，通过渐进式教学与渐进式学习相结合的方式，有效缓解了知识蒸馏中师生能力差距过大导致的性能退化问题。
...
### [134] [Rethinking Pretraining for Specialized Design Data: Evidence from the JONES-19 Cultural Design Dataset](https://arxiv.org/abs/2608.00135)
*Alexandros Haridis,Charles Zhou*

Main category: cs.LG

TL;DR: 本文基于JONES-19设计图像数据集，比较了CNN的两种训练策略（ImageNet预训练与从零训练），发现局部多裁剪采样能够弥补预训练带来的性能优势，表明在专业设计领域中，精心策划的小型高质量数据集可能比大规模预训练更有效。


<details>
  <summary>Details</summary>
Motivation: 设计与建筑档案以图形格式编码了专家的人类知识，为设计驱动的机器学习挑战提供了关键测试平台。但现有典型的计算机视觉基准数据集无法体现这些设计领域的独特挑战，需要评估CNN在专业设计数据上的表现。

Method: 基于JONES-19数据集（源自1857年《装饰语法》），评估CNN在两种训练策略下的判别性能：(a) ImageNet预训练以获得领域通用的"视觉常识"；(b) 在JONES-19设计数据上从零训练，并采用重复局部采样（多裁剪）策略。

Result: 领域通用的预训练先验能提升判别性能，但从零训练结合多裁剪采样方法可以有效弥补这些性能差距。对于高度结构化的设计数据，局部设计驱动的表示足以提供学习基础。

Conclusion: 在专业设计领域，精心策划的小型高质量数据集（能够捕捉经验和形式设计原则）可能比大规模数据收集更有效且更能揭示特定设计领域的本质。
...
### [135] [Leak It: A Probabilistic Approach to Training-Data Extraction from Black-Box Language Models](https://arxiv.org/abs/2608.00144)
*Victor Maricato*

Main category: cs.LG

TL;DR: 本文揭示了语言模型成员推断攻击（MIA）的聚合ROC-AUC指标存在严重混淆问题——盲基线（不考虑模型）仅凭表面文本就能达到极高AUC。作者提出基于采样的概率视角，将泄漏信号视为输出分布的函数，并通过逐文档提取审计发现真实风险：模型的"照抄"行为会在长尾文档中精确重现私人标识符（如邮箱），这种风险随模型容量增大而增强，在代码领域比散文强约3倍，且聚合并不能检测到。作者发布了leakit开源审计工具。


<details>
  <summary>Details</summary>
Motivation: 现有MIA评估用聚合ROC-AUC总结模型隐私风险，但这类指标被混淆：无模型盲基线仅凭表面文本特征就能区分成员与非成员。聚合指标掩盖了真实的逐文档泄漏危害，需要用概率视角和逐文档审计来揭示真正的隐私风险。

Method: 从概率视角将N次采样p(.|x)视为输出分布的估计，将泄漏信号建模为输出分布的函数。在采样场景下扩展盲基线批判：在WikiMIA上使用盲bow分类器，在IID Pile分割（MIMIR）上对比自浓度与gold-continuation恢复与盲基线的差异。同时引入逐文档提取审计：用不匹配前缀作为对照，检测模型对包含真实标识符文档的精确重现。

Result: 在WikiMIA上盲bow分类器达到AUC 0.97（5% FPR下TPR 0.90），采样不增加信息；MIMIR上自浓度和gold-continuation恢复的增量AUC 95%置信区间包含零。在Pythia-6.9B上，500个Pile文档中83个（16.6%）含真实标识符的文档被精确重构且在错配前缀对照下未重现，证明泄漏可归因于特定文档。标识符泄漏从410M的5.6%增长至6.9B的16.6%，代码比散文强3倍，任意held-out续写的恢复仅发生在代码域。各种采样参数（温度、nucleus、前缀长度、去重）影响甚微。

Conclusion: 隐私审计应报告按领域分解的逐文档提取情况，而非单一AUC值。聚合AUC掩盖了真实的风险分布——风险不均匀地集中在长尾文档和代码领域。作者发布leakit作为黑盒提取审计工具，呼吁采用逐文档披露作为更准确的隐私度量标准。
...
### [136] [Response Magnitude as a Dominant Signal for Held-Out CRISPRi Perturbation Effect Prediction](https://arxiv.org/abs/2608.00152)
*Mehrdad Shoeibi,Niloofar Yousefi*

Main category: cs.LG

TL;DR: 本文研究CRISPRi扰动转录组效应预测问题，在虚拟细胞挑战赛（VCC）基准上采用严格的留出目标基因划分，发现目标变量（log Anderson-Darling距离）可由输入的四个确定性标量函数高度预测。深度学习编码器退化为边际训练均值，而基于四个标量的线性回归和随机森林均大幅超越深度模型。跨细胞类型零样本迁移中，幅度预测器正迁移而表达预测器为负或无法解决。


<details>
  <summary>Details</summary>
Motivation: 在单细胞生物学中，预测CRISPRi扰动对留出目标基因的转录组效应是一个重要的开放问题。尽管深度学习模型被广泛使用，但近期研究表明简单基线方法在相关协议上往往与深度预测器相当甚至超越。本研究旨在系统性地理解这一现象背后的低维信号，并探索其跨细胞类型的迁移特性。

Method: 在VCC基准上采用严格的留出目标基因划分；识别出四个确定性标量函数作为关键低维信号；对比深度MLP编码器与线性回归、随机森林等模型；通过两个预先指定的对照组区分逐行对齐与维度增加的影响；在外部CRISPRi筛选数据上进行零样本迁移评估。

Result: 四个幅度标量的线性回归超过最强的x-only经典模型；随机森林在输入加四标量上大幅超越深度编码器；深度编码器在拥有完整输入时坍缩向边际训练均值且标准补救措施无效；幅度增益归因于逐行对齐而非维度增加；零样本迁移中幅度预测器正迁移而表达预测器为负；对深度编码器暴露幅度信号可提升迁移但未超过四标量线性回归。

Conclusion: CRISPRi扰动效应预测的核心可预测信号是低维的幅度标量而非高维表达谱本身。深度模型在本任务中存在固有的局限性，简单线性模型在此问题上更具竞争力。此外，Anderson-Darling列衡量的是转录组响应广度而非目标基因效应强度，这要求重新审视迁移评估的合理性。
...
### [137] [Inference-Time Policy Alignment for Fair Reinforcement Learning](https://arxiv.org/abs/2608.00175)
*Umer Siddique,Peilang Li,Conor Wallace,Yongcan Cao*

Main category: cs.LG

TL;DR: 本文提出了一种推理时策略塑形框架，通过乘性调整动作概率来引导预训练深度强化学习策略满足福利导向的公平性目标，无需更新基策略参数。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习公平性方法通常假设公平偏好预先已知，并需要基于公平导向指标完全重新训练策略，这导致策略部署后难以适应新的利益相关者偏好，适应性差且成本高昂。

Method: 受大语言模型推理时对齐的启发，将推理时公平性对齐形式化为策略塑形问题，提出乘性策略塑形框架，利用动作相关的福利分数调整动作概率，不修改基策略参数，且与任意深度强化学习智能体兼容。

Result: 在多个领域的广泛实验中，推理时策略塑形显著改善了基于福利的公平性目标，同时保持了核心任务性能。

Conclusion: 该框架为预训练强化学习策略提供了灵活的推理时公平性对齐手段，克服了传统方法需要完全重训的局限，兼顾了公平性与任务性能的平衡。
...
### [138] [AutoCause: A Python framework that automates expert decisions in environmental time-series causal discovery](https://arxiv.org/abs/2608.00198)
*Marco Ruiz,Miguel Arana-Catania,David R. Ardila,Rodrigo Ventura*

Main category: cs.LG

TL;DR: AutoCause是一个开源Python工作流，用于环境时间序列因果发现，通过记录所有分析决策、自动生成默认参数并允许领域知识覆盖，将不一致的专家实践转化为可审计、可重复的分析流程。该工作流集成四种因果发现方法，在145个数据集上验证了多数方法支持的边具有更高精确率。


<details>
  <summary>Details</summary>
Motivation: 环境时间序列因果发现依赖于专家对不同方法、条件独立性检验、滞后窗口、样本量、多重检验控制等的主观决策，这些选择在不同数据集间不一致，导致生成的结果图无法比较、复现和审计。作者旨在解决这一可重复性问题。

Method: 开发了AutoCause开源Python工作流，核心特点包括：(1)记录每个分析决策；(2)从扩展的因果审计模块推导默认参数；(3)允许领域知情覆盖；(4)集成来自三个方法族的四种成熟因果发现方法；(5)加入非因果参考模型；(6)按方法计数支持度对连接（边）进行分级。实验使用了DGP-Atlas、TimeGraph和基于拓扑推导的CausalRivers参考共145个数据集。

Result: 各方法在参考图上恢复了互补的部分。在合成基准上，多数方法支持的边比单一方法得到的边更精确；但在河流拓扑参考数据上，这一优势不成立。代码已在GitHub开源。

Conclusion: AutoCause能够将不一致的专家实践转化为可审计、可重复的分析，但最终的因果解释仍由研究者负责，工具本身不替代因果判断。
...
### [139] [A Physics-Chemistry-Informed Neural Network (PCINN) for Real-Time Spatial-ALD Coverage Prediction and Reliable Kinetics Inversion](https://arxiv.org/abs/2608.00212)
*Ning Hu,Chang Liu,Yunlei Jiang,Yuan Dong*

Main category: cs.LG

TL;DR: 本文提出一种物理-化学信息神经网络（PCINN），作为空间原子层沉积（SALD）的混合代理模型，以约7毫秒的实时速度实现CFD级别的精度（比CFD快约5万倍），仅用30个训练案例即可达到R²_log=0.998。该架构通过单标量瓶颈将网络学习限制在操作条件到近壁浓度的映射，而表面动力学作为可训练的硬编码化学层，保证可解释性和可逆性。文章还进行了完整的可辨识性分析，发现E_ads和k_des稳健可辨识，而k_ads仅在单一温度下不可单独辨识。


<details>
  <summary>Details</summary>
Motivation: SALD作为工业规模ALD的主流大气压高通量路线，其设计和控制受限于表面覆盖率的预测成本：高保真CFD仿真太慢无法用于操作窗口扫描，而解析模型又无法捕捉气体幕等传输调制效应。需要一个兼具CFD精度和实时速度的替代模型。

Method: 提出PCINN混合代理模型：小规模神经网络仅学习操作条件到近壁浓度的映射（单标量瓶颈），已知表面动力学作为硬编码的可训练化学层沿基底轨迹积分。在此基础上采用Fisher信息和剖面似然进行完整可辨识性分析，并推导出Arrhenius参数退化谷的解析形式（k_B T_eff ln(10)）作为可靠性诊断工具。

Result: PCINN单次查询约7毫秒，比CFD快约5×10⁴倍，测试R²_log=0.998（留一法R²_raw=0.974），仅需30个训练案例覆盖四个数量级的覆盖率。可辨识性分析表明E_ads和k_des稳健可辨识；k_ads在单一温度下不可单独辨识（仅k_ads*c_wall可辨识）。跨四温度下，ν和E_ads沿斜率为0.065 eV/decade的弱可辨识退化谷排列，该斜率偏离可指示未建模的位点异质性。

Conclusion: PCINN实现了SALD覆盖率预测的实时CFD级精度，同时保持可解释性和可逆性；可辨识性框架为参数可靠性提供了严格诊断。由于数据来自已知真值的仿真（同一动力学形式反演），该研究验证的是管线自一致性和可辨识性边界，而非实际工艺参数。
...
### [140] [Verifier-Induced Support Reshaping in On-Policy Optimization](https://arxiv.org/abs/2608.00220)
*Shaohang Wei,Zikun Su,Feifan Song,Wen Luo,Wei Li,Guangyue Peng,Houfeng Wang*

Main category: cs.LG

TL;DR: 本文发现基于可验证奖励的在线强化学习（RLVR）在提升当前任务目标的同时，可能使后续目标的成功行为因采样稀少而难以被再次强化，这种现象被称为"验证器诱导的支持重塑"。研究表明RLVR主要重排基础策略中已有的响应开头，且所选开头会因果影响后续任务的可搜索性，端点改进并不能保证未来的可训练性或联合能力。


<details>
  <summary>Details</summary>
Motivation: 在线RLVR在提升当前目标时可能导致后续目标的成功行为过于稀少而无法采样和强化，这种"验证器诱导的支持重塑"效应可能损害模型的长期可训练性和多任务联合能力，但目前缺乏系统性研究。

Method: 跨两个模型家族，通过重复验证器评分采样和双向训练，在数学推理和约束指令跟随任务上进行研究，包括使用相反验证器的顺序训练。具体分析了Math-RLVR和IF-RLVR之间的相互影响，采用token分布分析和受控开头干预实验，并测试了参考策略约束、路由先验和在线蒸馏等缓解方法。

Result: Math-RLVR提高了平均指令跟随成功率但减少了多次采样下成功响应的提示数量；在IFEval上pass@1提升6.5个百分点而best@32下降9.8个百分点。IF-RLVR使数学响应从逐步解答转向直接答案，降低best@k和后续Math-RLVR的奖励变化。这些变化集中在响应开头几个token，验证器主要重排基础策略中已有的开头。参考策略约束等方法仅部分保留跨任务支持。

Conclusion: RLVR的端点改进不能保证未来的可训练性或联合能力，验证器诱导的支持重塑是在线优化中需要关注的重要现象，单独优化单一目标可能损害模型在多任务上的整体表现。
...
### [141] [Similarity-Aware Machine Unlearning](https://arxiv.org/abs/2608.00246)
*Madhavan Citalamangalam Kumaran,Midhun Parakkal Unni,Vicky Kouni,Haripriya Harikumar*

Main category: cs.LG

TL;DR: 提出了一种保留感知的定位方法用于机器遗忘，该方法同时考虑参数对遗忘数据和保留数据的重要性，并引入保留相似评估集来直接衡量附带损伤，在CIFAR-10和ResNet18上的11种实验设置中显著减少附带损伤并提升遗忘指标。


<details>
  <summary>Details</summary>
Motivation: 现有基于定位的机器遗忘方法仅根据遗忘集的重要性选择参数子集，忽略了这些参数在保留数据集中的作用，常常对语义相似的保留样本造成附带损伤。

Method: 提出保留感知的定位方法，同时考虑参数对遗忘数据和保留数据的重要性；引入基于模型嵌入空间中余弦相似度构建的保留相似评估集，用于直接衡量附带损伤。

Result: 在CIFAR-10数据集和ResNet18模型上的11种实验设置中，该方法一致地减少了附带损伤，同时改善了标准的机器遗忘指标。

Conclusion: 保留感知的定位方法对于相似性感知的机器遗忘是有效的，能够在提升遗忘效果的同时更好地保护语义相似的保留数据。
...
### [142] [Stabilized Best-of-$K$ Training for Neural Combinatorial Optimization](https://arxiv.org/abs/2608.00296)
*Melveena Jolly,Midhun Xavier*

Main category: cs.LG

TL;DR: 本文测试了Leader Reward的一种窄扩展：用基于采样预算K的稳定秩信号替代原有的二元领导者/非领导者区分。在固定POMO架构、3050轮训练计划和TSP-100测试集下，稳定化K=8方案在三个配对训练种子上均降低了Best-of-8成本（7.7944 vs 7.8136），但作者明确声明这只是估计性观察，不作普遍优越性或SOTA声明。


<details>
  <summary>Details</summary>
Motivation: 探索Leader Reward训练方法的窄扩展——将二元领导/非领导区分替换为基于采样预算K的稳定秩信号，以检验该变体是否能带来性能提升。

Method: 在POMO架构上，用稳定化秩信号（按采样预算K索引）替代Leader Reward的二元区分，采用3050轮训练计划和TSP-100测试集，分别在100-start、8-augmentation贪心解码和独立采样两种协议下进行评估。

Result: Leader Reward复现结果7.7662与报告值7.766匹配；稳定化K=8方案在三个配对训练种子上均优于基线（7.7944 vs 7.8136 Best-of-8成本），但优势仅在独立采样、K=8的解码协议下成立，且种子数低于六种子测试下限。

Conclusion: 该改进是估计性、解码器特定的观察结果：在采样K=1时Leader Reward更好，在其原始增强贪心协议下也略优。作者明确声明不作无偏估计量、普遍优越性或SOTA声明。
...
### [143] [Abstention as an Action Can Kill Both the Reward Gradient and the KL Anchor: Collapse Law and Repair for Error-Penalized Reinforcement Learning](https://arxiv.org/abs/2608.00301)
*Xujun Che,Yuchen Yuan,Weida Zhao,Chenyang Yu*

Main category: cs.LG

TL;DR: 本文证明了基于KL锚定的梯度学习者在面对错误惩罚评分规则时，可能产生与预期相反的行为——模型会趋向于拒绝回答所有问题，即使训练奖励表面上在提升（趋近于零），实际覆盖率却崩溃。作者提出结构性修复方案：训练时强制输出置信度报告（使用严格适当评分加正确性奖励），仅在部署时通过阈值化报告来实现弃权。


<details>
  <summary>Details</summary>
Motivation: 错误惩罚评分规则（答对+1，答错-λ，弃权0）被广泛用于抑制幻觉，其理论保证是理性主体会在正确概率超过Chow阈值时回答。然而，作者想探究在实际的梯度学习场景下，这种规则是否真的能达到预期效果——特别是在离散弃权动作与门饱和效应相互作用的情况下。

Method: (1) 理论上证明了KL锚定梯度学习者在离散弃权动作下，奖励梯度和锚定恢复力会被相同的门饱和因子压制并同时消失，导致模型在显式条件下漂移向全拒绝状态；(2) 分析了优势估计器（advantage estimator）的稀疏回答机制，发现组归一化会把设计惩罚替换为有效惩罚1，将学习阈值从t*移动到1/2；(3) 提出结构性修复：训练强制置信度报告（严格适当评分+正确性奖励），部署时才阈值化弃权；(4) 通过模拟和两种规模的语言模型实验验证。

Result: 模拟验证了所有理论预测；语言模型实验确认了机制在真实场景中的有效性：评分规则在十步优化内就压制了模型仍能解决的问题，消融实验隔离了原因，报告级训练同时提升了覆盖率、准确率和校准度。

Conclusion: 直接使用错误惩罚评分规则训练离散弃权的梯度学习者会出现反直觉的失败模式（奖励上升但覆盖率崩溃），而将弃权从训练中剥离、改为部署期对强制置信度报告做阈值化，是结构性正确的修复方案，能同时改善覆盖率、准确率与校准。
...
### [144] [Agentic Bayesian Optimization through Surrogate-Augmented Autoresearch](https://arxiv.org/abs/2608.00316)
*Paul Brunzema,Louis Tiao,Nhat Le,Kevin De Angeli,Yao Xuan,Djordje Gligorijevic*

Main category: cs.LG

TL;DR: 本文提出了一种"智能体贝叶斯优化"（agentic Bayesian optimization）范式，将LLM作为BO循环中的核心决策者，同时用贝叶斯后端提供不确定性感知的优化基底。作者实现了Sara（代理增强的自动研究代理）和lenz（模块化BoTorch后端），实验表明该方法在保持标准BO可靠性的同时，能利用自然语言先验超越现有方法，并支持动态问题重配置。


<details>
  <summary>Details</summary>
Motivation: 传统BO依赖通用统计先验，难以编码丰富的领域先验知识；虽然LLM可以获取非正式先验，但现有LLM-BO方法要么将LLM固定在特定角色（如替代模型、采集函数代理或配置接口），要么赋予其过度控制权而牺牲BO的系统性探索优势。需要一个既能利用LLM灵活先验知识，又能保持BO可靠性的方法。

Method: 提出agentic BO范式：LLM智能体作为BO循环的核心决策者，负责配置问题、查询贝叶斯后端、选择并提交评估、在运行过程中动态修改优化策略（收紧边界、切换采集函数、提出定向评估、重新定义问题等）。具体实现了两个组件：Sara（智能体）和lenz（基于BoTorch的模块化后端，可通过结构化接口被智能体检查和修改）。

Result: 在合成和真实世界基准测试中，Sara无需先验知识即可保持SOTA BO的可靠性，优于基于LLM的基线方法，并利用自然语言先验超越标准BO。在动态设置中，Sara能在需求变化时实时重新配置完整优化问题，展现了标准BO不具备的能力。

Conclusion: Agentic BO范式成功融合了LLM的灵活推理能力与贝叶斯优化的系统性探索优势，不仅提升了优化性能，还赋予了优化器动态适应问题变化的新能力，为BO的实用化提供了新的方向。
...
### [145] [Neural operator learning for collision-aware trajectory planning of spacecraft swarms](https://arxiv.org/abs/2608.00320)
*Sidhdharth D. Sikka,Suyi Gao,Zehui Lu,Rongjie Lai,Shaoshuai Mou*

Main category: cs.LG

TL;DR: 本文提出一种置换等变神经算子，可将航天器、目标和碎片的分布映射为整个星群的避碰轨迹，并通过批量高斯-牛顿收尾确保轨道动力学精确性。该算子无需最优轨迹标签，结合自监督物理目标与对抗威胁训练，从10颗航天器零样本泛化到1000颗航天器及11000多个编目物体，匹配最优控制求解器精度，优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 经典轨迹优化方法在星群规模增大时，由于成对安全性约束数量激增而难以扩展；学习型规划器又难以在不同星群规模和碎片密度间迁移。这促使作者寻求一种既能高效规划、又可扩展且具良好泛化性的新方法。

Method: 提出置换等变神经算子，将航天器、目标与碎片的分布直接映射为整个星群的避碰轨迹（单次前向传播完成）；配合批量高斯-牛顿收尾以强制执行精确的轨道动力学；训练采用自监督物理目标加上对自身轨迹生成的对抗威胁，无需最优轨迹标签。

Result: 在10颗航天器上训练后，可零样本泛化至1000颗航天器及11000余个编目物体场景；匹配单智能体最优控制求解器的精度，能规避碎屑盲基线无法应对的最坏情况威胁，并将星群内部邻近距离降低数倍。

Conclusion: 基于物理的算子学习为拥挤轨道环境下的最优控制提供了一种快速、可扩展的替代方案，在效率、精度与泛化能力上均具有显著优势。
...
### [146] [Ensemble of Unsupervised Deep Learning for Clustering Imbalanced Tabular Data](https://arxiv.org/abs/2608.00346)
*Pulock Das,Yina Hou,Md. Kamrozzaman Bhuiyan,Manar D. Samad*

Main category: cs.LG

TL;DR: 本文首次系统研究了深度聚类方法在数据不平衡场景下的表现，提出了两种新颖的聚类集成方法（跨嵌入维度聚合和多数投票），在16个二分类表格数据集上验证了集成方法在ACC、NMI和ARI指标上优于单个聚类方法，表明深度聚类可成为不平衡数据下监督分类的强有力替代方案。


<details>
  <summary>Details</summary>
Motivation: 监督分类在数据不平衡时存在多数类偏置问题，导致假阴性和准确率高估。深度聚类因无需类标签进行表示学习，理论上对类别不平衡具有免疫性，但其在不平衡数据下的表现尚未被充分研究。

Method: 提出两种聚类集成方法：(1)跨不同嵌入维度聚合深度聚类分配结果；(2)对表现最佳的聚类算法应用多数投票。在16个具有人工诱导不平衡级别的二分类表格数据集上进行实验。

Result: 实验显示不同深度聚类方法在不同不平衡水平下各具优势。集成方法在ACC、NMI和ARI指标上平均优于单个聚类方法，对数据不平衡具有更强的鲁棒性。

Conclusion: 深度聚类在不平衡数据场景下可作为监督分类的有力替代方案，集成策略能进一步提升无监督方式识别真实类别的能力。
...
### [147] [Modeling Unknown Nonlocal PDE Systems via Flow Map Learning](https://arxiv.org/abs/2608.00400)
*Zhongshu Xu,Ying Li,Yanzhi Zhang,Dongbin Xiu*

Main category: cs.LG

TL;DR: 本文提出一种流图学习（FML）框架，直接从解数据中学习未知的非局部偏微分方程（PDE），通过学习有限时间演化算子而非非局部算子本身，在模态空间或节点空间中实现长期稳定预测。


<details>
  <summary>Details</summary>
Motivation: 非局部偏微分方程在众多应用中广泛出现，但由于非局部算子的存在，这类方程的建模和学习非常困难，需要一种不显式评估非局部算子的数据驱动学习方法。

Method: 提出流图学习（FML）框架，直接学习有限时间演化算子而非底层非局部算子；针对谱表示和网格表示分别开发了两种互补的公式化方法（模态空间与节点空间）。
...
### [148] [DSETA: A Dual-Stage Continual Learning Framework for Travel Time Prediction in Dynamic Traffic Environments](https://arxiv.org/abs/2608.00402)
*Yanming Lyu,Yue Cheng,Lingkun Li,Ruipeng Gao,Xinyue Liu,Hui Gao,Qiang Ni*

Main category: cs.LG

TL;DR: 本文提出DSETA，一个双阶段的ETA（预计到达时间）预测框架，通过将持续学习分为日内和日间两个阶段来分别处理短期交通波动和长期分布变化，并引入历史交通知识整合模块防止灾难性遗忘，已在滴滴平台大规模部署并验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有ETA预测方法难以适应大城市中日益动态的交通拥堵模式，要么无法应对不规则交通模式和突发拥堵，要么无法将长期趋势与短期波动解耦，导致模型在真实场景中性能退化。

Method: 提出DSETA双阶段ETA预测框架：(1)日内学习阶段，完全依赖实时数据进行动态适应短期交通模式（如节假日、事故）；(2)日间学习阶段，利用短期时间窗口的聚合历史数据捕捉长期分布变化（如季节趋势、交通网络演变）；(3)引入历史交通知识整合模块以防止灾难性遗忘并保留常规模式知识。

Result: 在滴滴平台三个主要城市（北京、武汉、西安）的在线A/B测试中，MAE分别降低6.62%、0.73%和2.40%。该框架已在滴滴生产环境成功部署，每天处理数亿次请求。

Conclusion: DSETA通过双阶段增量学习框架有效解决了ETA预测中的动态交通适应问题，在离线实验和在线A/B测试中均展现出良好的有效性和鲁棒性，且具备工业级大规模部署的可行性。
...
### [149] [Unleashing the Potential of Large Language Models: A Blueprint for Real-Time, Enterprise-Ready Deployments](https://arxiv.org/abs/2608.00419)
*Muhammad Faizan Raza,Shuo,Yang,Satish Mahadevan Srinivasan,Joanna F. DeFranco*

Main category: cs.LG

TL;DR: 本文提出了一种统一的、模式驱动的LLMOps架构，将实时数据摄入、持续学习、检索增强生成（RAG）和人在环反馈整合到单一运营管道中，以解决大语言模型在实时、受监管环境中面临的知识过时、灾难性遗忘、幻觉和反馈循环薄弱等问题。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型在实时受监管环境中部署时面临知识过时、灾难性遗忘、幻觉和反馈循环薄弱四大挑战，现有方案缺乏统一集成的运营架构。

Method: 提出四点贡献：(1) AIPO自适应摄入模式编排器（基于FreshStreamBench评估）；(2) STAR+FAR持续学习方法，采用稀疏时间适配器路由和新鲜度感知重放；(3) SAGE，一种SLO感知的自适应检索策略，预测每个查询的段落预算以满足尾部延迟目标；(4) 基于反馈的自动化收敛阶段，带有RLHF触发机制。

Result: 该架构在降低延迟-成本-准确性权衡方面取得了改善，并支持可审计性和回滚能力，适用于医疗和金融等高危行业。

Conclusion: 该统一LLMOps架构通过模式驱动设计实现了实时数据、持续学习和人机反馈的有机结合，在性能与可审计性之间取得平衡，适合受监管的高风险领域部署。
...
### [150] [HP-JEPA: Hierarchical Partitioning for Multi-Resolution Graph Joint-Embedding Predictive Learning](https://arxiv.org/abs/2608.00491)
*Ruichen Xu,Jingxiang Qu,Wenhan Gao,Jiaxing Zhang,Linsey Pang,Ravid Shwartz-Ziv,Yann LeCun,Yuefan Deng*

Main category: cs.LG

TL;DR: 本文提出HP-JEPA，一种用于图自监督学习的层次化多分辨率联合嵌入预测框架。通过将图组织为从粗到细的多层级划分，并在每个分辨率上独立进行上下文-目标潜在空间预测，再整合各分辨率的表示，HP-JEPA在6/8个基准任务上超越了固定分辨率基线Graph-JEPA。


<details>
  <summary>Details</summary>
Motivation: 现有图JEPAs依赖单一预定义的图划分，导致学习到的表示偏向某一种结构粒度，无法同时捕捉不同图尺度上的互补模式，限制了表示的可迁移性和表达能力。

Method: HP-JEPA将每个图组织为从粗到细的有序划分分辨率库，在每个分辨率上使用在线编码器、EMA目标编码器和潜在预测器进行上下文-目标潜在预测。各分辨率特定的图表示通过拼接或任务特定的分辨率加权进行整合，使下游模型能结合局部、区域和全局结构信息。

Result: 在7个图分类基准和1个图回归基准上，HP-JEPA在8个任务中的6个上超越了固定分辨率的Graph-JEPA基线。规模分层分析进一步表明，在三个代表性数据集的多数图大小四分位数上，HP-JEPA的准确率高于Graph-JEPA。

Conclusion: 层次化多分辨率划分为可迁移的图表示学习提供了一种有效方案，能够整合互补的局部、区域和全局结构信息，从而提升图自监督学习的性能。
...
### [151] [Agentic Graph Token Reasoning](https://arxiv.org/abs/2608.00542)
*Zhuoyi Peng,Yi Yang*

Main category: cs.LG

TL;DR: 本文提出了一种"智能体图标记推理"方法，将图标记化（graph tokenization）重新定位为推理过程本身的一部分：模型在每个推理步骤中自主选择要编码的图视图和粒度，按需调用图编码器生成图标记并拼接进上下文，实现逐步推理。通过三阶段训练流程（自监督图标记阅读、图标记鲁棒轨迹、偏好优化），模型在七个图域上的评估中大幅超越基线，并能零样本迁移到未见过的领域。


<details>
  <summary>Details</summary>
Motivation: 现有的图标记方法采用静态一次性编码方式：在模型看到目标任务之前就预先编码一个固定图视图，且从不修改，没有利用模型逐步推理的能力，限制了LLM在图分析任务上的表现。

Method: 提出智能体图标记推理框架：每一步由模型自主选择编码哪个图视图（节点、k跳邻域或簇）及粒度，按需调用图编码器生成图标记并拼接进运行上下文。采用三阶段训练：①自监督任务教模型阅读异构图标记；②图标记鲁棒轨迹阶段（含图标记一致性正则化）；③偏好优化奖励图标记证据与节点文本证据一致的轨迹。

Result: 在七个图域的评估中，模型大幅超越广泛的基线方法，且无需针对目标进行微调即可零样本迁移到未见过的图域。

Conclusion: 本文推动LLM图分析从静态图标记编码器向图原生的智能体范式演进，证明了将图标记化融入推理过程能显著提升图分析性能。
...
### [152] [Crushing the Evidence: A Dual-Penalty Evasion Framework for Fooling White-Box Explainable AI Auditors](https://arxiv.org/abs/2608.00566)
*Niraj Kumar,Harsh Kasyap*

Main category: cs.LG

TL;DR: 本文提出一种更强大的白盒、梯度正则化对抗解释攻击框架，通过连续嵌入双惩罚机制在训练阶段直接惩罚触发特征梯度，将逃避逻辑原生嵌入模型参数中，无需依赖OOD包装器，从而在不留异常痕迹的情况下系统性摧毁目标特征归因。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗解释攻击依赖于OOD检测器包装器，会产生异常扰动痕迹，容易被现有的防御机制识别和中和。作者旨在突破这一限制，展示一种更隐蔽、更强大的攻击方式，揭示解释性管线中存在的严重安全漏洞。

Method: 提出一种白盒、梯度正则化的规避攻击框架，采用连续嵌入双惩罚框架，在分布内数据的训练过程中直接惩罚触发特征梯度。该方法将逃避逻辑原生嵌入模型参数中，不依赖OOD脚手架包装器，从而生成平滑的分布内预测，不留下异常足迹。

Result: 在四个基准表格数据集（COMPAS、German Credit、IEEE-CIS、Communities & Crime）上的实验表明，该方法能将目标特征归因系统性压至接近零（<0.02），攻击成功率保持在90%以上，并从根本上绕过条件异常检测。

Conclusion: 该研究揭示了现有对抗解释防御机制的严重局限性，表明仅依赖异常检测来防御攻击是不够的，需要开发更根本性的防御策略来保护模型可解释性管线的安全。
...
### [153] [Fairness Auditing: Lower Bounds on Company Manipulation](https://arxiv.org/abs/2608.00568)
*Rachit Verma,Padala Manisha,Sujit Gujar*

Main category: cs.LG

TL;DR: 本文研究了有限审计资源下公平性审计的固有限制，通过最小-最大优化框架量化了审计后不可避免的操纵程度，并推导出最坏情况下人口统计平价偏差的下界。


<details>
  <summary>Details</summary>
Motivation: 现有研究已证明黑盒公平性审计存在根本性的不可能性结果，即足够复杂的模型可以规避任何审计策略。然而，在有限审计资源条件下，审计后操纵的具体程度尚未被量化，需要对此进行补充研究。

Method: 将公平性审计建模为计算能力无界公司与预算受限审计者之间的最小-最大优化问题，考察两种审计机制：(i) 使用固定规模审计集的预算审计者；(ii) 额外要求审计集在α近似内估计公平性的α容忍审计者。对两种情况推导出最坏情况下的后审计人口统计平价偏差下界，并通过线性与神经网络分类器的简单审计集构建启发式方法进行实证验证。

Result: 推导出了审计后人口统计平价偏差的显式下界，这些下界是审计预算、群体不平衡性和公平性容忍度的函数。实证结果展示了增加审计资源可以减少但不能消除后审计操纵空间。

Conclusion: 有限预算的公平性认证存在根本性局限，增加审计资源只能部分缓解操纵问题，无法完全消除，这凸显了诸如招聘、贷款等高风险场景中公平性认证方案的内在不足。
...
### [154] [CoSynFlow: Conformal Symplectic Neural Flows for Cross-System Prediction of Dissipative Hamiltonian Dynamics](https://arxiv.org/abs/2608.00571)
*Baige Xu,Takaharu Yaguchi*

Main category: cs.LG

TL;DR: CoSynFlow是一种共形辛神经流方法，用于学习耗散哈密顿动力学的连续时间解算子，通过组合辛剪切映射与显式共形缩放，在构造上保持共形辛结构，可对未见系统进行泛化预测。


<details>
  <summary>Details</summary>
Motivation: 现有的神经算子方法在求解微分方程时大多只优化预测精度，未显式施加动力学的几何结构约束。已有的保结构模型（如SympNets、辛神经流）仅适用于保守哈密顿系统，无法处理具有共形辛结构的耗散哈密顿系统，因此需要一种能保持共形辛结构的神经算子方法。

Method: CoSynFlow通过组合辛剪切映射（symplectic shear maps）与显式共形缩放（conformal scaling）来构造网络，从构造上保证共形辛结构的保持。模型以有限维哈密顿描述符和耗散参数为条件输入，使单个训练好的模型可以预测未见系统的解映射而无需重新训练。

Result: CoSynFlow将结构误差保持在机器精度水平，在长时程预测中取得最低误差，并且支持物理信息训练（physics-informed training）。

Conclusion: CoSynFlow有效解决了耗散哈密顿系统中保结构神经算子学习的问题，兼顾了结构的严格保持与对新系统的泛化能力，为保几何结构的神经算子方法在耗散动力学中的应用提供了新的途径。
...
### [155] [From field-scale to large-scale spectral libraries: Tabular foundation models in soil spectroscopy](https://arxiv.org/abs/2608.00608)
*Viacheslav Barkov,Jonas Schmidinger,Robin Gebbers,Martin Atzmueller*

Main category: cs.LG

TL;DR: 该研究系统比较了多种回归模型（TabPFN、CNN、Cubist、随机森林、PLSR）和降维方法（PCA、PLS）在85个土壤光谱回归任务中的表现，发现TabPFN整体性能最佳，且与PLS降维结合后效果最优。


<details>
  <summary>Details</summary>
Motivation: 可见-近红外和中红外光谱技术可快速低成本预测土壤属性，但高维、强共线性的光谱数据在机器学习建模中仍具挑战，缺乏系统的模型选择和降维策略对比研究。

Method: 在85个来自开放基准数据集的回归任务上（涵盖田间尺度数字土壤制图和全球土壤光谱库），系统比较了TabPFN、CNN、Cubist、随机森林和PLSR五种模型，并结合PCA和PLS两种降维特征进行了全面评估。

Result: TabPFN在所有尺度上表现最佳，直接使用全光谱即超越了所有经典基线，表明显式降维并非必要；PLS作为降维策略对所有模型均有提升，TabPFN结合PLS潜变量取得整体最优预测性能。

Conclusion: 研究为光谱校准模型选择提供了基于证据的指导，证明PLSR的传统优势与现代表格基础模型在化学计量学中具有互补性。
...
### [156] [RHEA: Reliability-Harmonized Reconstruction and Assignment for Robust Multimodal-Attributed Graph Clustering](https://arxiv.org/abs/2608.00621)
*Yinlin Zhu,Di Wu,Ziyu Han,Zekai Chenm,Wang Luo,Miao Hu,Guocong Quan*

Main category: cs.LG

TL;DR: 本文提出RHEA框架，用于多模态属性图（MAG）聚类，通过从图邻域中估计节点特定的模态可靠性，并在聚类全流程中传播该信号，从而在属性噪声或缺失的情况下显著提升聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有MAG聚类方法假设所有节点的模态可靠性相同，在属性干净完整时表现良好，但在属性有噪声或缺失时性能大幅下降。然而实际应用中，模态可靠性具有节点特异性（如图像损坏或缺失、文本不完整），需要更鲁棒的解决方案。

Method: RHEA框架包含四个核心组件：1）从图邻域共识估计节点特定的模态可靠性；2）从图邻域重建不可靠或缺失的模态；3）在可靠性感知融合中自适应加权不同模态；4）拓扑感知的最优传输聚类，包含可靠性感知的传输分配和邻域共识分配蒸馏。重构表示的置信度被纳入聚类目标函数中。

Result: 在四个MAG基准数据集上的五种属性条件下，RHEA始终优于最强基线，且NMI增益随属性质量恶化而增大，表明其在噪声和缺失属性场景下具有显著优势。

Conclusion: RHEA通过利用图邻域作为无监督证据来估计节点特定模态可靠性，有效解决了MAG聚类在属性不完整或噪声场景下的鲁棒性问题，且性能优势在属性越差时越明显。
...
### [157] [Towards Effective Federated Multimodal Graph Learning via Navigating Multifaceted Heterogeneity](https://arxiv.org/abs/2608.00623)
*Yinlin Zhu,Di Wu,Yi Zhang,Xunkai Li,Wang Luo,Wei-Jin Huang,Miao Hu,Guocong Quan*

Main category: cs.LG

TL;DR: 本文提出了FedTCR，这是第一个系统性的联邦多模态图学习算法，通过两阶段训练（任务无关预训练+任务导向微调）和拓扑感知的跨模态路由机制，同时解决了任务、模态和拓扑三重异构性挑战，在7个领域上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有的联邦图学习方法（FGL）直接应用于联邦多模态图学习（FMGL）是不充分的，因为它们无法处理分布式多模态图（MAGs）中固有的多面异构性：任务异构性（不同客户端目标不同）、模态异构性（模态质量和语义域不一致）以及拓扑异构性（拓扑模式发散且跨模态相关性低）。

Method: FedTCR采用两阶段范式（联邦任务无关预训练+隔离的任务导向微调）来处理任务异构性；引入拓扑感知的跨模态路由机制，每个客户端通过图结构感知的重要性加权聚合将模态特定知识提炼为紧凑原型，服务器评估跨客户端跨模态关系并路由有信息量的原型作为对比参考，驱动三层次跨模态对比学习以对齐跨客户端模态同时保持判别性。

Result: 在7个领域的实验表明，FedTCR在图形中心任务和模态中心任务上都优于最先进的基线方法。

Conclusion: FedTCR作为首个系统性的FMGL算法，通过联合解决任务、模态和拓扑三重异构性，有效提升了联邦多模态图学习的性能，并在多个领域得到验证。
...
### [158] [Relative Parameter Importance in Task-Agnostic Replay-Free Continual Learning](https://arxiv.org/abs/2608.00630)
*Malavika Suresh,Ikechukwu Nkisi-Orji,Nirmalie Wiratunga*

Main category: cs.LG

TL;DR: 本文提出一种基于相对参数重要性的持续学习（CL）方法，在无先前任务训练数据和无测试时任务ID的条件下，通过衡量参数相对于当前与过去任务的重要性来实现稳定性与可塑性的平衡，同时在文本分类和文本生成任务上实现知识反向迁移。


<details>
  <summary>Details</summary>
Motivation: 持续学习需要在稳定性与可塑性之间取得平衡，同时实现知识迁移。现有方法通常在无先前数据和任务ID约束下难以兼顾两者，且缺乏对参数重要性的相对视角，限制了在线学习算法的表现。

Method: 提出相对参数重要性（relative parameter-importance）度量，该度量同时考虑参数对当前任务和过去任务的重要性。高相对重要性的参数被施加强正则化以维持过去任务的稳定性，低相对重要性的参数允许更自由的更新。与现有方法不同，该方法允许对过去任务重要但相对重要性低的参数进行更新，从而在解决稳定性-可塑性权衡的同时实现反向知识迁移。

Result: 在类增量学习和领域增量学习的文本分类任务上，该方法优于现有的持续学习方法；此外还为将该方法扩展到文本生成任务提供了见解。

Conclusion: 该工作通过相对参数重要性的新视角，在无数据访问和任务ID约束下有效平衡了稳定性与可塑性，并实现了反向知识迁移，为持续学习提供了新的思路和实用工具。
...
### [159] [Learning the Pareto Frontier of Predictive Models under Distribution Shift](https://arxiv.org/abs/2608.00632)
*Yiming Dong,Jiwei Zhao,Yang Young Lu*

Main category: cs.LG

TL;DR: 本文提出Frontier Learning框架，通过拼接白盒模型内部表征和黑盒模型预测输出，构建统一目标域特征并拟合轻量级监督学习器，在分布偏移场景下保证不劣于任何单一模型复用策略。


<details>
  <summary>Details</summary>
Motivation: 在分布偏移下，预训练模型的复用策略（零样本、微调、直接训练）没有统一最优方案，需要一种能够整合多种模型互补信息的框架。

Method: 将白盒候选模型的内部表征与黑盒候选模型的预测输出拼接成统一目标域特征，在其上拟合带正则化的轻量级监督学习器；该假设类包含零样本复用、微调和直接训练作为特例，因此经验风险最小化保证不劣于任何单一基线。

Result: 在模拟实验（不同源-目标兼容性）、DomainNet/VisDA视觉域适应和MIMIC-IV-Notes临床死亡预测中，Frontier Learning均匹配或超越最强单一复用策略，在无单一基线可靠的偏移场景中增益最大。

Conclusion: Frontier Learning通过将候选模型视为互补信息源而非互斥选项，在分布偏移下提供了一种理论上保证不劣于最优基线、实践中稳定超越的策略框架。
...
### [160] [Mitigating Backdoors via Decoy Shortcuts and Knowledge Decoupling](https://arxiv.org/abs/2608.00732)
*Zixuan Zhu,Rui Wang,Lihua Jing,Jinwen Zhong*

Main category: cs.LG

TL;DR: 本文提出了一种名为TR（Trapping and Removing）的训练时后门防御方法，通过引入轻量级捷径分支作为"蜜罐"来诱捕后门知识，训练后丢弃该分支即可移除后门，无需额外数据，在多种攻击和模型架构上均有效。


<details>
  <summary>Details</summary>
Motivation: 后门攻击对依赖第三方数据训练的深度神经网络构成严重威胁，攻击者可通过数据投毒注入恶意行为。现有防御方法往往需要额外数据或计算开销，作者观察到后门行为倾向于被更简单的并行分支吸收，由此启发设计一种训练时防御策略。

Method: 提出TR防御框架：(1) 引入轻量级捷径分支作为"蜜罐"来诱捕后门知识，训练后直接丢弃该分支；(2) 设计知识解耦策略，结合基于熵的权重分配，促使中毒样本流向蜜罐分支，引导主网络专注于良性学习；(3) 提出自动捷径生成策略，提升对不同模型架构的泛化能力。

Result: 在四个基准数据集和五种模型架构上进行了大量实验，结果表明该方法能够有效缓解多种后门攻击，同时保持良性数据上的性能。

Conclusion: TR作为一种简单而有效的训练时后门防御方法，通过"蜜罐"机制在不需额外数据的情况下实现后门移除，展示了良好的泛化性和实用性。
...
### [161] [An Embedded RISC-V Evaluation of Kolmogorov--Arnold Networks in Hard-Constrained Recurrent Physics-Informed Models](https://arxiv.org/abs/2608.00737)
*Enzo Nicolas Spotorno,Josafat Leal Filho*

Main category: cs.LG

TL;DR: 本文研究了硬约束递归物理信息网络（HRPINNs）中，KAN残差分支相比MLP残差分支在嵌入式RISC-V平台上的实际部署成本。结果表明，KAN虽然参数效率高，但执行速度慢4.7-14.5倍，能耗高4.7-18.7倍，且在INT8量化下轨迹发散时间提前最多43倍，因此MLP仍是嵌入式HRPINN部署的更可靠默认选择。


<details>
  <summary>Details</summary>
Motivation: 此前研究已表明KAN在HRPINN残差分支中的参数效率（即发现精度方面）可能优于或匹配MLP，但尚不清楚这种参数效率能否转化为实际部署中的成本优势。本文旨在回答参数效率是否能存活于部署阶段这一问题。

Method: 在无向量扩展的RISC-V RV64GC平台（StarFive VisionFive 2, SiFive U74）上，使用完全相同的训练权重，对KAN和MLP残差分支进行闭环递归结构下的执行延迟、每积分步能耗以及训练后量化（INT8）下的可靠性测量。对两个精度匹配的模型对和四个参数匹配的规模层级进行了系统比较。

Result: 精度匹配的两对中，KAN残差分支执行速度慢13.5倍和8.0倍，每积分步能耗高11.3倍和5.6倍（最小对为3.7μJ对0.33μJ）；所有四个规模层级中性能差距范围为4.7-14.5倍（速度）和4.7-18.7倍（能耗）。在INT8量化下，KAN轨迹发散的提前量最多达43倍，且损害源于权重量化而非输入侧节点区间误分配。

Conclusion: KAN在HRPINN中报告的参数效率无法转化为标量嵌入式内核上的部署成本优势；除非采用针对性的量化协同设计，否则MLP残差分支是嵌入式HRPINN部署中更可靠的默认选择。
...
### [162] [Generic Vision and Cross-Attention for Reaction Yield Prediction](https://arxiv.org/abs/2608.00776)
*Qiwei Han,Chi Zhou*

Main category: cs.LG

TL;DR: 本文提出了一种双模态视觉交叉注意力架构，融合表格物理有机数据与二维分子拓扑结构进行反应产率预测，相比传统纯量子描述符方法实现了更优的预测精度（测试RMSE=5.27%）。


<details>
  <summary>Details</summary>
Motivation: 传统反应产率预测受限于1D量子描述符，缺乏显式的空间信息，导致预测能力受限。作者旨在通过引入2D分子拓扑结构弥补这一不足。

Method: 采用双模态视觉交叉注意力架构，融合表格型物理有机数据与2D分子骨架结构。利用通用计算机视觉骨干网络处理简单2D骨骼结构，通过交叉注意力机制实现模态间信息交互，并采用残差跳跃连接保护非空间电子参数。

Result: 最优交叉注意力框架在测试集上达到RMSE=5.27%的预测精度，优于传统纯量子方法。机制探针实验显示网络能够进行描述符引导的空间查询，将宏观空间位阻识别卸载到视觉通路，并动态学习化学层级以优先关注关键空间瓶颈（如芳基卤化物）。

Conclusion: 该工作证明纯视觉骨干网络处理2D分子骨架即可超越纯量子基线，且双模态融合能进一步提升精度；同时提供了一种可扩展、高度可解释的将深度视觉学习融入物理化学研究的蓝图。
...
### [163] [Paris as a 15-Minute City: An Explainable AI Perspective](https://arxiv.org/abs/2608.00815)
*András J. Molnáar,Csaba I. Sidló,Rita Rónai,Domonkos Rózsay*

Main category: cs.LG

TL;DR: 本研究利用巴黎大都市区约7万条出行轨迹数据，结合社会人口统计和兴趣点（POI）数据，构建步行和骑行可达性指标，分析其与出行时长、交通方式及短途驾车行为的关系，发现高POI可达性与减少私家车出行、增加主动出行相关，但这一关系在外围区域显著减弱。


<details>
  <summary>Details</summary>
Motivation: 15分钟城市理论主张居民可在步行或骑行短距离内获得日常服务，但该理论与实际出行行为之间的关系难以量化，缺乏基于大规模真实轨迹数据的实证验证。

Method: 使用NetMob 2025数据挑战赛的移动轨迹数据，结合INSEE社会人口数据和OpenStreetMap的POI数据，经停留点分割和数据清洗得到约7万条出行段；构建步行和骑行可达性指标，采用梯度提升树模型结合可解释机器学习（XAI）方法分析各因素对出行行为的影响，并通过特征归因方法检验变量排序的敏感性。

Result: 高POI可达性与更少的私家车出行和更多的主动出行相关，但该关系在外围集聚区明显减弱；出行目的、家-工作距离、服务可达性、车辆拥有情况、公共交通订阅和社会人口背景是重要预测因素；短途出行中高POI密度与较低驾车使用相关，而车辆拥有和驾照与较高驾车预测相关；服务稀疏区域中公共交通订阅与更低的车依赖相关。

Conclusion: 研究结果支持15分钟城市的核心假设，同时揭示了显著的空间和社会人口异质性，证明可解释机器学习方法能够补充传统可达性指标，为城市出行政策提供局部相关的研究假设。
...
### [164] [AdvPlan-Bench: Adversarial Evaluation of Structured Plan-Generation Agents](https://arxiv.org/abs/2608.00832)
*Alina Kapanova,Arun Kanhai,Natan Vidra,Spurthi Setty*

Main category: cs.LG

TL;DR: 本文提出AdvPlan-Bench，一个用于对抗性评估结构化计划生成代理的离线基准测试框架，通过类型化计划、对抗响应集、选择器诊断和可追踪候选前沿等通用评估对象，在150个合成场景中系统评估计划的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有计划生成代理的评估往往将计划质量视为孤立属性，但现实规划任务需要考虑候选计划在其他代理搜索响应时的表现，缺乏对抗性评估框架。

Method: 将计划表示为带可选分支的类型化动作链，分配合成质量分数，使用BLUE-vs-RED优势和Nash-gap诊断比较对立计划，并通过透明启发式规则评估定性约束连贯性。构建跨越五种规划模板的150个合成场景，引入采样最佳响应策略、离线LLM策略合约基线和两阶段多代理委员会等评估方法。

Result: 采样八候选响应的策略将BLUE优势从.518降至.486，BLUE胜率从.900降至.820；离线LLM策略合约基线达.496优势/.700胜率；两阶段多代理委员会达.509优势/.813胜率；三评分者评分敏感性研究在600条记录上取得.978的评分者间一致性。

Conclusion: AdvPlan-Bench是用于研究对抗性计划评估、响应预算敏感性、候选前沿和多代理批评-修订轨迹的可复现基准工具，但并非可操作的规划器，不提供真实世界决策质量的证据。
...
### [165] [Deep Learning CNN and Recurrence Analysis for Alpha Gamma EEG Biomarkers in Fragile X Syndrome](https://arxiv.org/abs/2608.00835)
*Zag ElSayed,Payton Siekierski,Jack Yanchen Liu,Ernest Pedapati*

Main category: cs.LG

TL;DR: 本文提出一种多表征深度学习框架，通过整合CNN、LSTM和递归图（RP）分析，对脆性X综合征（FXS）的EEG α（8-12Hz）和γ（30-100Hz）振荡进行自动化表型特征刻画。实验表明，混合CNN-LSTM模型优于单模态基线，且α-γ整合获得最佳性能，为FXS的EEG生物标志物开发提供了可扩展的深度学习方法。


<details>
  <summary>Details</summary>
Motivation: FXS是由FMRP蛋白表达减少引起的神经发育障碍，表现为突触可塑性受损、皮层过度兴奋和网络同步异常。EEG虽能非侵入性地揭示这些机制，但现有方法缺乏对α和γ振荡中复杂时空及非线性特征的自动化、系统性刻画能力，亟需更强大的计算框架来开发可靠的EEG生物标志物。

Method: 提出多表征深度学习框架：(1)将带限EEG信号分解为α和γ频段组件；(2)构建三种互补表征——时间特征序列、时频图和编码非线性递归结构的RP图像；(3)CNN模块从图像表征中学习空间-频谱及动力学纹理特征；(4)LSTM模块建模振荡活动的时间调制；(5)混合CNN-LSTM架构联合捕获空间、时间和非线性依赖关系。

Result: 在独立受试者评估中，混合模型优于各单模态基线；γ频段特征提供较强的判别能力，而α与γ频段信息的整合取得了最佳整体性能。

Conclusion: 深度学习结合非线性表征可作为FXS EEG生物标志物开发的可扩展方法，在诊断、患者分层和转化场景中的治疗监测方面具有潜在应用价值。
...
### [166] [Nonlinear Laplacians Improve Signed-Directed Graph Learning](https://arxiv.org/abs/2608.00836)
*Ali Parviz,Yuichi Yoshida*

Main category: cs.LG

TL;DR: 本文提出了一种面向符号有向图的非线性拉普拉斯算子（NLSD），并基于此构建了谱图神经网络框架NLSD-GNN，在节点分类和链路预测任务中取得了优于现有方法的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的图神经网络在处理带符号的图时多使用线性拉普拉斯算子，但针对兼具符号与方向信息的非线性拉普拉斯算子研究较少。

Method: 提出NLSD非线性算子，该算子基于节点特征计算势能差异，仅在势能差异与边方向一致时进行消息传递（不一致则忽略），并据此构建高效的谱GNN框架（NLSD-GNN）。

Result: 在节点分类和链路预测任务上，涵盖仅符号、仅方向及两者兼具的多种数据集，NLSD-GNN均实现了优于现有方法的性能。

Conclusion: NLSD-GNN能够有效整合符号与方向信息，并展现出跨数据集的优越性能。
...
### [167] [Adaptive Quantum Physics-Informed Neural Networks for Differential Equations with Applications to Fluid Dynamics](https://arxiv.org/abs/2608.00850)
...
### [168] [HyperODE: Zero-Shot Surrogate for Simulation and Inference of Dynamical Systems](https://arxiv.org/abs/2608.00852)
*Ajitesh Srivastava*
...
### [169] [SparseKAN: Compressing Kolmogorov--Arnold Networks Across Basis Functions, Neurons, and Bits](https://arxiv.org/abs/2608.00859)
...
### [170] [Kilobyte Models: Neural Networks as a Seed and a Quantized Latent](https://arxiv.org/abs/2608.00860)
...
### [171] [GeoArbiter: Verifiability-Guided Grounding for Remote-Sensing Multimodal LLMs](https://arxiv.org/abs/2608.00877)
*Xuechen Li*
...
### [172] [AOSpec: Action and Observation Co-Speculation for Low-Latency Agent Serving](https://arxiv.org/abs/2608.00881)
...
### [173] [UpliftBench: Revealing Outcome-Regime and Objective Mismatch in Uplift Evaluation](https://arxiv.org/abs/2608.00915)
*Binshuang Li*
...
### [174] [Subtype Robustness Is Not Just Accuracy: Calibration Under Unseen Subtype Shift](https://arxiv.org/abs/2608.00928)
...
### [175] [xMICD: Explainable Representation of Multiple ICD Codes](https://arxiv.org/abs/2608.00935)
...
### [176] [Data-Driven Pinball-Loss Selection for Vertically Distributed Elastic-Net SVMs](https://arxiv.org/abs/2608.00949)
...
### [177] [Interpretable machine learning for predicting splitting strength of asphalt concrete: insights from SHAP analysis](https://arxiv.org/abs/2608.00956)
...
### [178] [One-Sided Quantile Coupling for Flow Matching](https://arxiv.org/abs/2608.00978)
...
### [179] [Beyond Gene Reconstruction: Learning Cell Representations through Complementary Transcriptomic Views](https://arxiv.org/abs/2608.00985)
...
### [180] [Who Belongs in the Eval Set? A Capability-Taxonomy-Driven Pipeline for Curating Regression Eval Sets in Agent-Extensibility Platforms](https://arxiv.org/abs/2608.01004)
...
### [181] [Hierarchical Solomonoff Induction: An Unbounded Machine Learning Model](https://arxiv.org/abs/2608.01005)
*Nathan Young*
...
### [182] [Fused Bayesian Flow Networks for Dual-Target Molecular Design](https://arxiv.org/abs/2608.01007)
...
### [183] [Caliber: Cross-Architecture Extraction-Cost Control for Score-Returning APIs](https://arxiv.org/abs/2608.01023)
...
### [184] [The Fourth Quadrant: A Stylized View of Benign Misfitting](https://arxiv.org/abs/2608.01032)
...
### [185] [Characterizing Bias in Post-Bandit Inference under Index Algorithms](https://arxiv.org/abs/2608.01069)
...
### [186] [Logit-Origin Centering for Singleton Test-Time Adaptation](https://arxiv.org/abs/2608.01074)
...
### [187] [Breaking Diversity Collapse in Spiking Pseudo-Ensembles for Efficient OOD Detection in Remote Sensing](https://arxiv.org/abs/2608.01090)
...
### [188] [Factorized AdaBoost.MH Achieves the Same Convergence Rate as AdaBoost.MH](https://arxiv.org/abs/2608.01091)
*Xin Zou,Jingyuan Xu*
...
### [189] [FL-OA: A Byzantine-Robust Federated Learning Framework with Outsourced Auditing for Intelligent Devices](https://arxiv.org/abs/2608.01095)
...
### [190] [When Do Surrogate Updates Improve Decisions? A Local Theory of Trajectory-Wise Transfer](https://arxiv.org/abs/2608.01130)
*Yuyang Shen*
...
### [191] [Policy Optimality Measurement for Multi-Vehicle Decision-Making: From Extrinsic Indicators to Intrinsic Quality](https://arxiv.org/abs/2608.01133)
...
### [192] [EulerLoRA: Rank-Driven Jump Dynamics for Calibrated Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2608.01142)
...
### [193] [Differentiable Lifting for Topological Neural Networks](https://arxiv.org/abs/2608.01160)
...
### [194] [Interpretable Machine Learning for Traffic Congestion Prediction: Unveiling the Impact of Different COVID-19 Periods](https://arxiv.org/abs/2608.01180)
...
### [195] [SAFE-Merge: Data-Free Continual Model Merging with General Knowledge Preservation](https://arxiv.org/abs/2608.01184)
...
### [196] [ReBRAC-v2: The Return of the King](https://arxiv.org/abs/2608.01205)
...
### [197] [AdaHAT: Adaptive Hard Attention to the Task in Task-Incremental Learning](https://arxiv.org/abs/2608.01252)
...
### [198] [Distill What the Student Can See: Fisher-Projected On-Policy Distillation for Vision-Language Models](https://arxiv.org/abs/2608.01263)
...
### [199] [Riemannian Attention Mechanisms for Transformers: A Theoretical Framework and Architecture Design](https://arxiv.org/abs/2608.01283)
*Sen Song*
...
### [200] [Training nGPT](https://arxiv.org/abs/2608.01284)
...
### [201] [Stop When Memory Suffices: Evidence-Conditioned Progressive Execution for LLM Agents](https://arxiv.org/abs/2608.01285)
...
### [202] [FedChronos: Federated Fine-Tuning of Time-Series Foundation Models for Privacy-Preserving Commodity Price Forecasting](https://arxiv.org/abs/2608.01290)
...
### [203] [AlphaG-OPD: Reliability-Gated Sibling Counterfactuals for On-Policy Distillation in Symbolic Alpha Factor Discovery](https://arxiv.org/abs/2608.01303)
*Yaoyu Su*
...
### [204] [Spatiotemporal Proximal Causal Inference under Hidden Confounding and Interference](https://arxiv.org/abs/2608.01352)
...
### [205] [Do Neural Networks Really Beat the Curse of Dimensionality? A Bit-Complexity View](https://arxiv.org/abs/2608.01357)
*Tong Mao,Jinchao Xu*
...
### [206] [When May a Model Replace the Experiment? Audits, Licenses, and the Price of Trust in Surrogate-Driven Design](https://arxiv.org/abs/2608.01378)
...
### [207] [On the Identifiability of Masked Prediction: Mode Blindness and Mask Schedules](https://arxiv.org/abs/2608.01383)
...
### [208] [TabDPT-Turbo: Efficient In-Context Learning for Tabular Prediction](https://arxiv.org/abs/2608.01400)
...
### [209] [Cluster-Aware Over-the-Air Federated Learning with Energy-Harvesting Devices: From Global Training to Model Personalization](https://arxiv.org/abs/2608.01426)
...
### [210] [Statistical Mechanics of Learning on Product Wasserstein Manifolds](https://arxiv.org/abs/2608.01434)
...
### [211] [Conformalized Large Language Models under Configuration Shift](https://arxiv.org/abs/2608.01460)
...
### [212] [Plasticity of Growing and Elastic Neural Networks in Online Continual Learning](https://arxiv.org/abs/2608.01475)
...
### [213] [Interpretable MEG Decoding of Perceived Speech: Cortical Sources and the Stimulus Features That Drive Retrieval](https://arxiv.org/abs/2608.01481)
...
### [214] [BiKAN: Restoring Collapsed Basis of Binary Kolmogorov--Arnold Networks](https://arxiv.org/abs/2608.01490)
*Kazi Ahmed Asif Fuad,Lizhong Chen*
...
### [215] [Stochastic Sequential Search in Very-High-Dimensional Feature Selection](https://arxiv.org/abs/2608.01502)
...
### [216] [Question Begets Question: Self-Evolving Curriculum for Reinforcement Fine-Tuning on Competition Mathematics](https://arxiv.org/abs/2608.01522)
...
### [217] [Gram-Space: Structure-Preserving Codebook Compression for Memory-Efficient Neuro-Symbolic AI](https://arxiv.org/abs/2608.01528)
...
### [218] [Rethinking Personalized Reward Modeling for LLMs under Preference Heterogeneity via Group-Debiased Federated Learning](https://arxiv.org/abs/2608.01556)
...
### [219] [Meganeura: Portable GPU Training and Inference through Vulkan and Metal](https://arxiv.org/abs/2608.01563)
*Dzmitry Malyshau*
...
### [220] [Measuring in-context algorithmic reasoning in language models against an exact Bayes-optimal standard](https://arxiv.org/abs/2608.01575)
...
### [221] [HindSearch: Trajectory-Level Hindsight Critique for Search-Augmented Reinforcement Learning](https://arxiv.org/abs/2608.01597)
...
### [222] [Latent-Regime Bias Auditing for Volatility Forecasting](https://arxiv.org/abs/2608.01599)
...
### [223] [Online Algorithms via Minimax and Posterior Matching](https://arxiv.org/abs/2608.01616)
...
### [224] [QWRF-Net: A Quantum-Wavelet Framework with Rectified Flow for Short-Term Precipitation Nowcasting](https://arxiv.org/abs/2608.01626)
...
### [225] [GraphIR: Architecture-Level Search States for LLM-Guided Neural Architecture Evolution](https://arxiv.org/abs/2608.01633)
...
### [226] [Evaluating Forecasting Techniques for Hardware Errors on a Large-scale HPC System](https://arxiv.org/abs/2608.01648)
...
### [227] [Sharp Root Anti-Concentration via Projective Incidence and Ordered Root Laws](https://arxiv.org/abs/2608.01670)
...
### [228] [Progressive Agent Skill Generation via Reinforcement Learning](https://arxiv.org/abs/2608.01678)
...
### [229] [Beckmann Transport Models: From Autonomous Flows to One-Step Maps](https://arxiv.org/abs/2608.01692)
...
### [230] [Beyond On-Policy Exploration: Integrating External Policy Rollouts for Reinforcement Learning in Diffusion Language Models](https://arxiv.org/abs/2608.01717)
...
### [231] [LLM-Guided Retrieval for Prediction of Molecular Perturbation Responses](https://arxiv.org/abs/2608.01734)
...
### [232] [Disagree to Accelerate: Closing the Loop on Diffusion Feature Forecasts](https://arxiv.org/abs/2608.01740)
...
### [233] [Toward Plasticity-Preserving KL Regularization for Capability Retention in LLM Reinforcement Learning](https://arxiv.org/abs/2608.01743)
...
### [234] [Heterogeneous Multi-Agent Reinforcement Learning for Radio Resource Management under Coupled Finite-Horizon Constraints](https://arxiv.org/abs/2608.01745)
...
### [235] [Multi-Source Dynamic Graph Learning for Compound-Flood Forecasting in Managed Coastal Systems](https://arxiv.org/abs/2608.01775)
...
### [236] [ReFP-AD: Rectified Flow Preconditioning for Energy-Based Anomaly Detection](https://arxiv.org/abs/2608.01793)
...
### [237] [LEAP: Lean Environment-Feedback via Adaptive Pruning for Code RL in GPU Kernel Generation](https://arxiv.org/abs/2608.01804)
...
### [238] [Predictive Maintenance: Deep Learning-Based Remaining Useful Life Prediction for Combat Aircraft Engines](https://arxiv.org/abs/2608.01819)
...
### [239] [tFUSOperator: Operator Learning for Transcranial Focused Ultrasound Digital Twins](https://arxiv.org/abs/2608.01839)
...
### [240] [WorldDynCache: Risk-Controlled Latent Dynamics Approximation for Diffusion World Model](https://arxiv.org/abs/2608.01845)
...
### [241] [Beyond Magnitude and Shape: A Direction-Aware Loss for Time Series Forecasting](https://arxiv.org/abs/2608.01857)
...
### [242] [LAB-Tab: LLM-Augmented Bayesian Network Adaptation for Few-Shot Tabular Generation](https://arxiv.org/abs/2608.01879)
...
### [243] [CARE: A Cascaded Framework for Efficient and Reliable Time Series Anomaly Detection](https://arxiv.org/abs/2608.01885)
...
### [244] [Understanding and Correcting Low-Frequency Bias in EEG Foundation Model](https://arxiv.org/abs/2608.01898)
...
### [245] [Finite-Time Analysis of Discounted Exponential-Utility Reinforcement Learning](https://arxiv.org/abs/2608.01917)
...
### [246] [HarnessCompass: Guiding Automatic Harness Evolution toward Generalizable and Effective Agent Harnesses](https://arxiv.org/abs/2608.01918)
...
### [247] [ChaosProbe: A Neurochaotic Lens on Frozen Transformer Input-Embedding Spaces](https://arxiv.org/abs/2608.01968)
...
### [248] [AOS: Adaptive Optimizer Switching via Training-State Signals for Faster Convergence and Better Generalization](https://arxiv.org/abs/2608.01997)
...
### [249] [Scikit-fingerprints: Python library for scikit-learn compatible molecular fingerprints and chemoinformatics](https://arxiv.org/abs/2608.02027)
...
### [250] [DART: Decoded Attention over Recurrent States for Efficient Long-Context Sequence Modeling](https://arxiv.org/abs/2608.02032)
...
### [251] [Upper-Expectile Multi-Step Q-Learning for Off-Policy Reinforcement Learning](https://arxiv.org/abs/2608.02034)
...
### [252] [Convex Neural Energy Elements: Monolithic Finite-Element Assembly of Geometry-Parameterized Neural Operators with Stability and Error Guarantees](https://arxiv.org/abs/2608.02036)
...
### [253] [Secrets Everywhere: Auditing Memorization in Mobility Prediction Models](https://arxiv.org/abs/2608.02052)
...
### [254] [SCOPE: Entanglement Frontier Escape for Source-Free Class Unlearning](https://arxiv.org/abs/2608.02058)
...
### [255] [Geometry-Guided Layerwise FFN Width Allocation in Transformers](https://arxiv.org/abs/2608.02064)
...
### [256] [Feed-Forward Steering in Transformer Residual Dynamics](https://arxiv.org/abs/2608.02071)
*Timur Mudarisov,Mikhail Burtsev,Radu State*
...
### [257] [Isotonic Bradley-Terry Model for Paired Comparison Data](https://arxiv.org/abs/2608.02081)
*Ryoya Yamasaki*
...
### [258] [A 2-Block Architecture for Real-Time EEG Gait Decoding: A Pilot Study](https://arxiv.org/abs/2608.02083)
...
### [259] [An AI-Based Decision-Support Pipeline for Day-Ahead Photovoltaic Forecasting](https://arxiv.org/abs/2608.02088)
...
### [260] [How Much Does a Reasoning Summary Reveal? An Observability Ladder for Large Language Models](https://arxiv.org/abs/2608.02089)
...
### [261] [One QK Channel, Many Sources: Guarding Low-Precision Attention Collapse](https://arxiv.org/abs/2608.02091)
...
### [262] [Do Static Embeddings Add Value to Hybrid Dutch Retrieval?](https://arxiv.org/abs/2608.02112)
...
### [263] [CoRe-GNN: Multilevel Message passing on Coarsened graphs](https://arxiv.org/abs/2608.02128)
...
### [264] [RamanPFN: learning from Raman spectral structure with a tabular foundation model](https://arxiv.org/abs/2608.02157)
...
### [265] [Empowering Credit Risk Detection in Weixin Pay with Billion-Scale Deep Graph Learning](https://arxiv.org/abs/2608.02168)
...
### [266] [Start Classifying: Categorical Critics for LLM Reinforcement Learning](https://arxiv.org/abs/2608.02181)
...
### [267] [CRIP: Channel Level Representation Injection for Personalized One-Shot Federated Learning](https://arxiv.org/abs/2608.02222)
...
### [268] [Constrained Co-Design for Photonic Bayesian Neural Networks](https://arxiv.org/abs/2608.02229)
...
### [269] [Assessing the Impacts of Imperfect Datasets on Client Selections in Federated Learning](https://arxiv.org/abs/2608.02250)
...
### [270] [Z-PEFT: Zero-shot Backdoor Detection in Parameter-Efficient Fine-Tuning via Canonical Spectral Signatures](https://arxiv.org/abs/2608.02271)
...
### [271] [BRiG-AFA: Bellman Risk-to-Go Learning for Non-Myopic Active Feature Acquisition](https://arxiv.org/abs/2608.02305)
...
### [272] [Diffusion Policy with Behavioral Advantage Correction for Offline Reinforcement Learning](https://arxiv.org/abs/2608.02332)
...
### [273] [Qwen-CUA: Native Computer Use for (almost) Everything](https://arxiv.org/abs/2608.02352)
...
### [274] [GLAIM: Learning Global and Local Adaptive Inter-Variable Dependency for Multivariate Time Series Imputation](https://arxiv.org/abs/2608.02366)
...
### [275] [Gecko: Fast Private Inference via Secure Public Encoder Offloading](https://arxiv.org/abs/2608.02378)
...
### [276] [From fragmented data to actionable design: Physics-calibrated learning for plastic upcycling](https://arxiv.org/abs/2608.02402)
...
### [277] [Deep Learning-Based Estimation of Ground Reaction Forces in Parkinsonian Gait Using an Optimized Set of IMU Data](https://arxiv.org/abs/2608.02408)
...
### [278] [Why Large Language Models Fail at Tabular Prediction](https://arxiv.org/abs/2608.02412)
...
### [279] [Foundations of Reinforcement Learning and Control:Connections and New Perspectives](https://arxiv.org/abs/2608.02433)
...
### [280] [Aggregate-then-Calibrate for Human-centered Assessment with Theoretical Guarantees](https://arxiv.org/abs/2608.02455)
...
### [281] [RoMeRL: Balancing Feedback Coverage and the Memory-Reward Trap in Self-Evolving Agent Memory via Reduced-Order Utility States](https://arxiv.org/abs/2608.02508)
...
### [282] [Analytic Planning under Uncertainty with Moment Closure](https://arxiv.org/abs/2608.02519)
...
### [283] [Uncertainty Is Not Enough: Value-of-Information Routing for Mixtures of LoRA Experts](https://arxiv.org/abs/2608.02528)
...
### [284] [Benchmarking Sheaf Neural Networks for Inductive Tasks](https://arxiv.org/abs/2608.02558)
...
### [285] [Structured Memory for Edge Language Models: Persistent Context and Corpus Retrieval via O(1) SSM State Injection](https://arxiv.org/abs/2608.02560)
...
### [286] [Pseudorandom Streams within Diffusion Models Act as Learnable Inputs That Affect Generation Quality](https://arxiv.org/abs/2608.02575)
...
### [287] [Smooth Reparameterizations of Functions on Simplicial Product Spaces: Applications to Probabilistic Tensor Decomposition and Functional Data Registration](https://arxiv.org/abs/2608.02576)
...
### [288] [GradCuit: Credit-Assigned Gradient Flow Enables Robust and Interpretable Test-Time Latent Reasoning](https://arxiv.org/abs/2608.02585)
...
### [289] [onepot-Bench 0: towards lab-aware in silico chemistry benchmarks](https://arxiv.org/abs/2608.02595)
...
### [290] [Revisiting Classic Thought Experiments to Measure Consciousness for Artificial Intelligence Safety](https://arxiv.org/abs/2608.00001)
*Peter David Fagan*

Main category: cs.AI

TL;DR: 本文通过守恒一致编码（CCE）框架重新审视莱布尼茨磨坊、图灵测试和塞尔中文房间等经典思想实验，提出用任务表现（W_causal,T）和操作意识（κ_T）分别度量行为成功与内部结构效率，并以无压缩查找系统与紧凑生成系统为例说明两者可在外在表现上相当但内部组织迥异，从而将"理解"问题拆解为外在表现与支撑它的内在组织两个维度。


<details>
  <summary>Details</summary>
Motivation: 经典的关于机器能否"理解"的争论（如图灵测试、中文房间）长期纠缠于外在行为表现，缺乏一个能同时考察行为成功与内部结构效率的正式框架。作者希望通过CCE框架将这两个维度分离，为AI安全性分析提供更清晰的概念基础。

Method: 提出CCE（Conservation-Congruent Encoding）框架，在玩具形式化设定中定义两个关键度量：W_causal,T用于衡量任务表现（因果层面行为成功），κ_T用于衡量操作意识（保留内部结构支撑行为的效率）。在该框架下对比两类系统：无压缩查找系统（依赖不断扩大的未复用映射存储）和紧凑生成系统（复用紧凑内部结构）。

Result: 在原理上，无压缩查找系统与紧凑生成系统可以达到相当的行为成功水平（W_causal,T可比），但两者在κ_T上差异显著——前者依赖不断膨胀且不复用的存储映射，后者通过复用紧凑内部结构实现高效率。这证明外在行为表现相同并不蕴含内部组织方式相同。

Conclusion: 该框架将关于"理解"的经典争论重新表述为外在行为表现与支撑行为的内部组织之间的区分，并指出这一区分可能对后续AI安全性分析具有重要意义。
...
### [291] [AutoFOAM: The Self-Refining Autonomous OpenFOAM Agent](https://arxiv.org/abs/2608.00003)
*Arun Govind Neelan,A Seshaditya*

Main category: cs.AI

TL;DR: 提出了AutoFOAM——一个基于大语言模型的自进化智能体，能够根据自然语言指令自动创建、评估、运行和演化OpenFOAM仿真，通过七阶段进化循环和三种防崩溃机制，无需手动配置即可完成CFD仿真任务。


<details>
  <summary>Details</summary>
Motivation: 使用开源CFD求解器（如OpenFOAM）需要专业知识、技能和耗时的配置文件设置，这成为现代工程中CFD应用的主要瓶颈，因此需要一种降低使用门槛的自动化解决方案。

Method: 基于Qwen-coder 2.5-14B预训练模型，在252个文本提示上微调，覆盖7个OpenFOAM求解器、13个参数化网格模板和y plus感知数值策略。核心是包含7个阶段的进化循环，并采用三种互补的防退化机制：RAG增强重试上下文、词典级外科补丁和提示多样性改写。

Result: AutoFOAM能够根据自然语言指令自主完成OpenFOAM仿真全流程，加速快速原型开发，并在重复自训练下保持模型稳定性。

Conclusion: 通过将生成式人工智能与严格的流体仿真相结合，AutoFOAM加速了快速原型开发并使高级CFD工作流程民主化，降低了CFD仿真的专业门槛。
...
### [292] [Enhancing LLMs with Context-Specific Knowledge for Mitigating Misinformation in SMEs: A RAG-based Modeling and Analysis](https://arxiv.org/abs/2608.00006)
*Md. Samiul Islam,Iqbal H. Sarker,Chadni Islam,Ahmad Mohsin,Ahmed Ibrahim,Helge Janicke*

Main category: cs.AI

TL;DR: 本文提出VectorRAG和GraphRAG两种检索增强生成方法，用于减少大语言模型在中小企业场景中的幻觉和错误信息，并在LLaMA、Mistral和Qwen等多个LLM上进行了实验验证，结果表明RAG增强方法能显著提升响应质量、降低幻觉风险，支持更可靠和可信赖的决策。


<details>
  <summary>Details</summary>
Motivation: 大语言模型正被中小企业越来越多地用于问答和决策支持，但其生成内容中的幻觉问题会导致错误信息传播，降低用户信任度，因此需要有效的方法来缓解这一风险。

Method: 提出VectorRAG和GraphRAG两种建模方法，通过将外部知识源融入建模过程来增强LLM的生成能力，并在多个主流LLM（LLaMA、Mistral、Qwen）上评估其在有用响应生成、幻觉风险、上下文相关性和人工解读等维度的表现。

Result: 实验结果表明，RAG增强的LLM在降低幻觉和错误信息方面显著优于基线，响应质量得到明显提升，验证了该方法在中小企业环境中的有效性。

Conclusion: RAG增强方法能够有效缓解LLM幻觉问题，为中小企业提供更可靠、可信和上下文感知的决策支持，是提升LLM实用性和可信度的重要途径。
...
### [293] [Energy Efficiency of Locally Deployed LLMs: A Preliminary Quantitative GPU Power Benchmark on Consumer Hardware](https://arxiv.org/abs/2608.00008)
*Philipp M. Zähl,Anika Hennig*

Main category: cs.AI

TL;DR: 本文在消费级GPU（RTX 4060Ti 16GB）上对九个开源LLM（1B-7B参数）进行了硬件级能耗基准测试，发现模型架构和量化策略比参数量更能影响能效，其中gemma3:1b和llama3.2:1b能耗最低（0.56和0.65 J/token），吞吐量最高（>170 tok/s），而7B-Mistral每token能耗高出4.4倍。


<details>
  <summary>Details</summary>
Motivation: 尽管本地部署LLM因隐私和本地推理需求日益流行，但消费级硬件上的能耗成本缺乏充分研究，现有基准大多只关注准确性而忽视能耗指标。

Method: 在单张RTX 4060Ti 16GB GPU上，使用Ollama推理引擎运行九个开源LLM（1B-7B），通过nvidia-smi以2Hz采样GPU功率，在固定提示集上评估平均/峰值功率、每提示总能耗（J/prompt）、每输出token能耗（J/token）和吞吐量（tok/s）等指标。

Result: gemma3:1b和llama3.2:1b能耗最低（0.56和0.65 J/token）且吞吐最高（>170 tok/s）；7B-Mistral每token能耗比最高效模型高4.4倍；qwen3.5:2b因内部推理扩展导致每提示能耗异常偏高。

Conclusion: 模型架构和量化策略是驱动能耗效率的关键因素而非单纯参数量，效率指标需要区分token生成模式（如推理模式），以更准确评估LLM的能效表现。
...
### [294] [CoT-Core: Accelerating LLM Evaluation via CoT-Aware Coreset Selection](https://arxiv.org/abs/2608.00014)
*Qihua Pan,Zhenheng Tang,Peijie Dong,Xiang Liu,Huacan Wang,Bo Li,Xiaowen Chu*

Main category: cs.AI

TL;DR: 提出CoT-Core框架，利用零样本思维链推理轨迹在潜空间中对问题按内在逻辑等价性聚类，实现无需历史日志的训练免费核心问题选择，在保持高保真度评估的同时大幅降低LLM评估成本。


<details>
  <summary>Details</summary>
Motivation: LLM评估在持续开发过程中计算开销巨大。现有的核心集选择方法存在两个问题：基于项目反应理论的方法面临严重的"冷启动"瓶颈（需要大量历史日志），而其他方法存在表面词汇偏差，忽略了任务背后的推理结构。

Method: CoT-Core框架的核心思想是：词面不同的问题可能共享相同的底层逻辑。具体做法是提示LLM展开零样本思维链推理轨迹，将这些推理路径投影到潜空间中，按问题内在的逻辑等价性而非表面文本相似性进行聚类，从而实现训练免费的核心问题选择。

Result: 在GSM8K、MMLU、MMLU-Pro和GPQA四个基准上的实验表明，CoT-Core能大幅降低评估成本同时保持高保真度的分数估计，并揭示了推理感知剪枝的边界条件——其有效性受任务复杂度的内在门控。

Conclusion: CoT-Core通过利用思维链推理轨迹捕捉问题的逻辑等价性，为LLM评估中的核心问题选择提供了无需训练且无需历史日志的高效解决方案，但该方法的效能取决于任务的复杂度水平。
...
### [295] [Optimization and Constraint Modeling using LLMs with a Retrieval Augmented Generation Process](https://arxiv.org/abs/2608.00015)
*Prateek Roy,Akash Singirikonda*

Main category: cs.AI

TL;DR: 本文提出了一种基于检索增强生成（RAG）的管道，利用500个合成的优化问题构建向量数据库，在推理时检索语义相似的示例作为上下文指导，显著提升了LLM在优化建模任务上的准确性和结构一致性（准确率从40%提升至72%）。


<details>
  <summary>Details</summary>
Motivation: 优化建模和约束建模需要深厚的领域专业知识和建模语言熟练度，但现有大语言模型在组合优化场景中经常生成结构不一致或不完整的优化公式，需要一种无需昂贵微调的有效改进方法。

Method: 从Text2Zinc数据集提取种子描述，结合LLM创建的专业角色生成500个优化问题（以JSON格式表示），并关联经过验证的Python求解器脚本；将这些问题编码到Chroma向量数据库；推理时通过语义相似检索获取上下文指导，输入给LangChain LLM智能体（使用Qwen 3 30B Instruct模型）。

Result: 在三个基准测试集上：NL4OPT准确率从40%提升至72%，MAMO Easy从40%提升至56%，MAMO Complex从32%提升至56%。合成示例的语义验证显著改善了求解准确率和结构完整性。

Conclusion: 合成数据集生成与检索增强的组合为微调提供了一种有效替代方案，表明领域特定的合成语料库加上检索增强可以成为在真实决策支持场景中部署LLM优化工具的实际路径，且无需昂贵的模型重训练。
...
### [296] [Memory Reward Inflation in Self-Improving LLM Agents](https://arxiv.org/abs/2608.00017)
*Mohammad Asadolahi,Amir Amini,Samira Talebi,Amirfarhad Farhadi,Azadeh Zamanifar*

Main category: cs.AI

TL;DR: 本文研究了基于外部记忆的LLM智能体自我改进系统中存在的一个核心缺陷——"Echo Gap"现象。当存储的奖励分数由LLM自身评估而非真实标签时，错误样本会被高估，导致智能体优先复用其最自信的"错误记忆"。作者形式化定义了错误独立性假设（EIA），证明其是纠正奖励膨胀的必要条件，并提出无答案依赖的去膨胀算法LUCID，在BIRD文本转SQL基准上取得了优于基准方法的执行准确率。


<details>
  <summary>Details</summary>
Motivation: LLM智能体通过外部记忆从经验中学习（无需更新权重），但部署环境中无法获得真实标签，只能依赖LLM自身的评估作为代理奖励。这种替代会造成严重的奖励膨胀问题——智能体会优先复用其最自信的错误，且错误会通过记忆累积而非平均化，进一步加剧偏差。

Method: 作者首先形式化定义了"Echo Gap"失败模式，提出错误独立性假设（EIA）并证明其是纠正奖励膨胀的必要条件。EIA要求验证信号不仅要追踪真实质量，还必须使其错误与记忆偏差去相关。基于此理论，作者设计了LUCID算法——一种无答案依赖的去膨胀算法，用于校正存储记忆中的奖励通胀。

Result: LUCID算法在BIRD文本转SQL基准上实现了56.9%的执行准确率，超过Memento式自评分智能体（54.0%，平均提升+2.9个百分点）和无记忆的相同架构智能体（52.4%）。

Conclusion: 存储的评分必须同时满足正确性（追踪真实价值）和错误去相关（与记忆偏差独立）两个条件才能真正改善智能体行为。膨胀不仅发生在按存储评分排序的检索中，也发生在普通相似度检索下。LUCID算法验证了理论上可行的去膨胀策略可以带来一致的端到端性能提升。
...
### [297] [Request-Level Energy Attribution for Batched LLM Serving](https://arxiv.org/abs/2608.00026)
*Qi Luo,Kunlin Li,Ziwen Wang,Dongsheng Wang,Yun Chen*

Main category: cs.AI

TL;DR: 本文提出JouleShare框架，通过离线精确Shapley能耗基准和轻量级校准模型JCalib，解决批量LLM推理服务中请求级能耗归因问题，发现token等比归因与精确Shapley平均偏差0.44-0.458，JCalib可将误差降至0.116-0.177。


<details>
  <summary>Details</summary>
Motivation: 批量LLM服务提升吞吐量但使能耗核算复杂化，GPU功率遥测是聚合的，而可持续性报告、计费和工作负载分析需要请求级能耗归因。现有研究和工具无法提供实测的请求级基准真值，导致实际使用的归因规则与公平分配的偏差程度未知。

Method: JouleShare包含两个组件：一是离线条带（offline harness），通过vLLM回放请求子集并结合GPU功率遥测计算精确Shapley能耗作为基准真值；二是轻量级校准模型JCalib，利用廉价请求特征在服务时预测Shapley份额。实验覆盖16个模型/工作负载组合，在三种数据中心GPU上进行静态批处理和连续批处理对比。

Result: token等比归因与精确Shapley在静态批处理下平均归一化L1偏差0.440，连续批处理下0.458。JCalib将误差降至静态0.116和连续0.177，优于在线不可用的独立测量基线，同时保持批处理能耗效率。采样Shapley扩展到更大组规模时偏差持续存在。

Conclusion: token归因不能作为批量执行下边际能耗的可靠代理，实测Shapley基准真值可以校准低成本的请求特征以实现更公平的能耗归因。
...
### [298] [Motif-Mamba: network motif improved mamba for long-range sequence modeling](https://arxiv.org/abs/2608.00027)
*Chonghe Hao,Yue Sun,Jian Zhang,Yansong Wang,Wangzi Yao,Yunjie Yao,Tielin Zhang*

Main category: cs.AI

TL;DR: 该论文提出Motif-Mamba，一种在Mamba基础上引入模体约束低秩循环通路的结构化状态空间模型。该方法通过将隐藏状态投影到紧凑动态子空间、施加模体引导的交互并映射回原始状态空间，增强了跨维度通信，同时保持Mamba的线性时间循环结构。实验在长序列外推、语言建模和脑机接口解码任务中均优于Mamba基线。


<details>
  <summary>Details</summary>
Motivation: Mamba虽然通过选择性的状态空间循环实现了线性时间复杂度的长序列建模，但其状态转移矩阵主要呈对角结构，限制了状态维度之间的显式交互，从而制约了模型的表达能力。作者希望通过引入一种结构化的低秩循环路径来弥补这一缺陷。

Method: Motif-Mamba的核心设计是增广Mamba架构，加入一个模体约束的低秩循环通路。具体地，该方法受三节点网络模体动力学启发，将隐藏状态投影到一个紧凑的动态子空间，在该子空间中施加模体引导的交互运算，然后将动力学结果映射回原始状态空间。这一设计在增强跨维度通信的同时，保留了Mamba的线性时间循环结构。

Result: 在长序列外推、语言建模基准测试以及脑机接口解码等实验任务上，Motif-Mamba均比Mamba骨干模型表现出一致的性能提升，验证了模体引导的低秩动力学作为长序列建模的有效结构先验。

Conclusion: 模体引导的低秩循环通路为Mamba类模型提供了一种有效的结构先验，能够在不牺牲线性时间复杂度的前提下增强状态维度间的显式交互，推动长序列建模性能的提升。
...
### [299] [Nova: An End-to-End MLIR Compiler for Deep Learning](https://arxiv.org/abs/2608.00029)
*Adwaid Suresh,Aparna A,Harshini V M,Jona Delcy C A,Killi Uma Maheswara Rao,Ram Charan Golla,Surendra Vendra*

Main category: cs.AI

TL;DR: Nova是一个端到端JIT编译器，通过将前向和反向传播统一为单一值语义方言、使用解析配置器零搜索确定最优执行调度、以及基于结构哈希运行时合成细粒度内核，实现了对硬件映射的绝对控制。在RTX 3060上，Nova在TF32矩阵乘法上匹配或略超cuBLAS和XLA，模型吞吐量相比PyTorch提升最高10.6%，内存占用降低29%，并能在12GB消费级GPU上训练PyTorch因OOM而失败的1.44亿参数模型。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型规模扩大时，高性能张量框架的急切执行模式缺乏全局图可视性和对硬件/内存的细粒度控制，无法原生最大化物理硬件利用率，导致高层数学运算到底层硬件的映射效率不高。

Method: Nova采用三条核心策略：(1) 捕获急切执行并将前向和反向传播统一到单一值语义方言，解锁全局优化；(2) 利用解析配置器（Analytic Configurator）基于算术强度确定性地推导最优执行调度，将搜索时间降至零；(3) 使用结构哈希运行时根据计算结构直接合成细粒度内核。

Result: 在RTX 3060上，Nova在大多数形状的TF32矩阵乘法上匹配或略超cuBLAS和XLA（相对误差<5e-4）；在4200万参数模型上，吞吐量比PyTorch高10.6%、比XLA高4.4%；内存占用相比PyTorch减少高达29%，能以17,900 tokens/s训练1.44亿参数模型，而PyTorch在相同12GB GPU上遭遇OOM。

Conclusion: Nova通过自动化端到端JIT编译实现了对硬件映射的绝对控制，在不牺牲数值精度的前提下同时提升吞吐量和内存效率，验证了基于解析推导而非搜索的编译优化路径在消费级GPU上的可行性和有效性。
...
### [300] [SIRIN: A Unified Toolkit for Detecting Contextual Hallucinations in Retrieval-Augmented and Memory-Grounded LLM Systems](https://arxiv.org/abs/2608.00033)
*Julia Belikova,Rauf Parchiev,Mikhail Filimonov,Konstantin Polev,Andrey Savchenko,Maksim Makarenko*

Main category: cs.AI

TL;DR: SIRIN是一个统一的工具包和交互式Web界面，用于检测检索增强、代理型和记忆驱动的LLM系统中的上下文幻觉（即流畅但缺乏证据支持的响应）。它统一了三种检测器范式（表征探测、不确定性估计和裁判式验证），并支持生成前查询可回答性评估，适用于白盒和黑盒场景，提供响应级和跨度级检查功能。


<details>
  <summary>Details</summary>
Motivation: 检索增强、代理型和记忆驱动的LLM系统容易产生上下文幻觉——即流畅但缺乏证据支持的响应。现有工具往往分散、异构且缺乏统一接口，难以系统性地检测和评估此类幻觉问题，需要一个统一的工具包来整合多种检测范式并进行评估。

Method: SIRIN将三种检测器范式（表征探测、不确定性估计和裁判式验证）以及生成前查询可回答性任务统一到一个接口、配置系统和评估流水线下。提供响应级和跨度级检查，支持白盒和黑盒设置，并配备交互式Web UI用于实时分析上下文-查询-答案三元组，包括幻觉分数、未支持跨度高亮和并排检测器对比，采用轻量级插件设计以便添加新检测器。

Result: SIRIN在幻觉检测、查询可回答性评估以及作为长期记忆系统中的忠实性门控机制方面得到了展示验证。源代码已在GitHub上公开。

Conclusion: SIRIN提供了一个统一、可扩展且交互式的幻觉检测框架，能够在多种LLM系统架构下实现上下文幻觉的系统性检测，并支持未来的检测器扩展。
...
### [301] [Linguistic Context Recodes Visual Representations in Vision-Language Models](https://arxiv.org/abs/2608.00035)
*Brian Song,Michael A. Lepori,Ellie Pavlick*

Main category: cs.AI

TL;DR: 本文研究了视觉语言模型（VLMs）中语言如何动态重编码视觉表征的问题，发现两种具体的语言诱导重编码现象：一是存在抽象参考表征用于标记目标相关对象，可通过对比转向向量进行因果干预；二是后期层会选择性放大视觉表征中的目标相关属性。两项因果干预实验表明这些重编码机制确实介导了模型预测。


<details>
  <summary>Details</summary>
Motivation: 以往研究将VLM中的视觉表征视为静态信息仓库，由语言表征进行操作，但缺乏对语言如何动态重编码视觉表征的系统性刻画。理解这一机制对于揭示VLM跨模态处理原理至关重要。

Method: 提取与抽象参考表征对应的对比转向向量，验证其对模型预测的因果作用；通过分析后期层的表征变化，检测语言诱导的属性调制现象；最后通过因果干预实验确认属性调制介导VLM响应分布。

Result: 发现参考表征具有抽象性，能泛化到不同对象、不同任务情境，甚至从合成图像泛化到自然图像。属性调制现象在多种提示条件下均稳定存在，且因果干预证实其确实介导模型响应。

Conclusion: VLMs中的视觉表征并非静态信息仓库，而是被动态调制以支持自然语言查询。这一发现支持跨模态处理的动态模型，对理解VLM工作机制具有重要意义。
...
### [302] [RAG-TESTER: Automated End-to-End Testing of Retrieval-Augmented Large Language Models](https://arxiv.org/abs/2608.00054)
*Ange Maiztegi,Jon Ayerdi,Miren Illarramendi,Aitor Arrieta*

Main category: cs.AI

TL;DR: 本文提出RagTester，一种面向RAG系统的自动化端到端测试方法，通过生成检索文档、测试输入和预期输出，并利用LLM作为评判器来评估RAG系统的可靠性。在24种配置、72,000次测试执行中，RagTester检测出21,633个故障，比基线方法多6.6%，并在20/24种配置中表现优于基线。


<details>
  <summary>Details</summary>
Motivation: RAG系统的可靠性取决于生成模型、嵌入模型、检索机制和提示词构建策略之间的交互，但目前缺乏端到端的自动化测试方法来系统性地暴露这些组件交互导致的故障。

Method: RagTester采用四阶段流程：生成检索文档、测试输入和预期输出，执行测试，并使用LLM作为评判器评估答案。其测试生成策略针对复杂段落、无依据查询和文档覆盖准则三类场景。

Result: 在8个LLM和6个嵌入模型组成的24种配置上，通过72,000次测试执行，RagTester检测出21,633个故障（比基线多6.6%），在20/24配置中优于基线。检测到的故障包括不准确的检索、无依据的回答、检索上下文使用不完整以及复杂段落解释困难等。

Conclusion: 面向覆盖的测试生成方法能有效暴露检索与生成组件交互导致的故障，支持RAG配置部署前的评估与选择。
...
### [303] [H+ Embedding: Harmonizing Global and Token-Level Retrieval with Context-Dependent Phrases](https://arxiv.org/abs/2608.00065)
*Shusen Zhang,Junyi Hu,Ye Feng,Ziteng Wang,Zhaoyuan Pan,Guosheng Dong,Xiaojun Yuan,Jiangshou Hong,Xiangzhi Wang*

Main category: cs.AI

TL;DR: 本文提出了H+ Embedding，一种统一的多粒度检索器，通过预测可变长度短语分区、将未覆盖词元保留为单例，并采用重要性引导的单元选择和加权MaxSim交互，在全局向量与词元级交互之间提供了中间质量-成本平衡点。


<details>
  <summary>Details</summary>
Motivation: 术语密集型检索（尤其是医疗场景）需要保留多词实体、缩写、数值约束和组合概念，但现有表示处于两个极端：单向量检索器过度压缩局部相关性信号，而词元级晚期交互保留了每个分词子词，导致索引、存储和评分成本过高。作者希望探索上下文相关短语能否作为全局向量与词元之间的有用检索单元。

Method: H+ Embedding采用统一的多粒度检索框架：预测可变长度的短语分区，未覆盖的词元保留为单例单元，并应用重要性引导的单元选择机制，配合加权MaxSim交互进行评分。

Result: 在16个科学、医学和双语任务上，短语检索分支相比全局检索分支在macro nDCG@10上提升了6.91；同时仅使用13.7%更少的文档向量即可接近Token方法的性能，并在中等向量预算下优于基于内容无关的分组规则。

Conclusion: 上下文相关的短语交互为实际检索系统提供了介于全局压缩与词元级交互之间的中间质量-成本解决方案，在保证检索质量的同时显著降低了存储和计算开销。
...
### [304] [Agentic Coding in the Wild: Characterizing GitHub Copilot Traces at Production Scale](https://arxiv.org/abs/2608.00101)
*Banruo Liu,Haoran Qiu,Íñigo Goiri,Rodrigo Fonseca,Ricardo Bianchini,Esha Choukse*

Main category: cs.AI

TL;DR: 本文首次对AI编码代理工作负载进行了生产规模的系统表征研究，基于2026年6月GitHub Copilot的采样数据（320万用户、1300万会话、7.61亿次LLM调用、95万亿token），揭示了代理式编码会话与聊天机器人工作负载的本质差异，包括会话结构、KV缓存命中率模式、长尾的token消耗分布等，并提出轻量级空闲时间预测器以支持资源调度优化。


<details>
  <summary>Details</summary>
Motivation: AI编码代理（如GitHub Copilot、Claude Code和Codex）将多步LLM推理与工具执行交织在一起，形成了与聊天机器人截然不同的工作负载。然而，现有LLM服务系统大多基于聊天机器人的工作负载假设设计，缺乏对代理式编码工作负载的大规模实证表征，无法为代理原生基础设施的设计提供依据。

Method: 基于2026年6月的GitHub Copilot生产环境采样数据展开分析，覆盖320万用户、1300万会话、7.61亿次LLM调用和95万亿token，对工作负载的会话结构、KV缓存命中模式、token消耗分布、时间特性等方面进行系统性表征分析，并设计了一个轻量级的空闲时间预测器。

Result: 发现代理式编码会话具有稀疏的用户发起轮次，每轮展开为自主代理循环（LLM调用几乎总是伴随工具执行）。KV缓存命中率在轮内平均达90%，但跨轮边界降至55%，在模型切换或上下文压缩等事件后大幅失效。工作负载表现出多样化和长尾的token消耗、时间跨度及工具调用分布，并发现代理快速周转时间与分钟级用户空闲期之间的显著差异；设计的空闲时间预测器可捕获86-90%的总空闲时间。

Conclusion: 这些发现挑战了当前LLM服务系统的核心假设，为代理原生（agent-native）基础设施的设计提供了经验基础，特别在资源编排、缓存管理和调度策略方面具有重要指导意义。
...
### [305] [Can LLM Agents Price Competitively? A Dynamic Multi-Attribute Auction Benchmark for Agentic Commerce](https://arxiv.org/abs/2608.00102)
*Shimaa Ahmed,Yiwei Cai,Mohsen Minaei,Rahul Rachuri*

Main category: cs.AI

TL;DR: 本文介绍了Bazaar，一个面向多属性拍卖的动态密封竞价基准测试，用于系统评估LLM驱动的智能体在真实市场条件下的定价能力。实验发现，前沿LLM在客户获取和利润表现上排名不一致，需求冲击后表现各异，但即使最强智能体也只能达到事后最优利润的三分之一以下。


<details>
  <summary>Details</summary>
Motivation: Agentic commerce正在从概念走向实际部署，但LLM智能体是否能在客户偏好隐藏、竞争者实时适应、需求突变等真实市场条件下进行有效定价，尚未被系统性测试。现有基准缺乏对这些动态复杂性的考量。

Method: 构建Bazaar动态密封竞价基准，模拟多属性拍卖环境，包含隐藏客户偏好、实时竞争对手适应和需求冲击等动态因素。基准基于闭式客户效用函数，支持精确评估。对来自4个提供商的11个前沿LLM进行系统测试。

Result: 客户获取领先的智能体（如Gemini 3.1 Pro）并非利润领先者（如Opus 4.6）；需求冲击后排名再次变化，学习最快的智能体通常在事后修订信念最慢，而Gemini 3.1 Pro恢复最快；即使最强智能体也仅捕获不到事后最优利润的三分之一。

Conclusion: 当前LLM在agentic commerce领域已有进展但仍有显著提升空间，不同智能体在不同维度（客户获取、利润、需求冲击恢复）上各有优劣，尚需进一步突破才能实现接近最优的定价能力。
...
### [306] [Shared Organizational Memory for Enterprise Coding Agents: System Design and Deployment Snapshot](https://arxiv.org/abs/2608.00122)
*Harsh Rao Dhanyamraju,Leonidas Raghav*

Main category: cs.AI

TL;DR: 本文介绍了一个企业级编码代理的组织记忆系统生产部署，该系统将知识捕获嵌入编码工作流的平台层面，自动收集任务相关的开发经验，经过贡献者审批后整理为可复用的问答记忆，并过滤安全隐私风险，供未来代理检索使用。


<details>
  <summary>Details</summary>
Motivation: 企业编码代理依赖于工具和检索，但企业知识（内部DSL、专有平台、本地约定、近期修复、隐性工作流）往往不在公开训练数据或正式文档中。现有知识接口依赖代理主动识别和记录经验，导致捕获与编码工作流脱节，开发经验被反复重新发现。

Method: 构建一个共享组织记忆系统，作为编码工作的平台级组成部分：1）在贡献者批准下收集任务相关的开发经验；2）将其整理为可复用的问答记忆；3）设置明显的安全与隐私风险过滤门控；4）为未来代理提供记忆检索能力。

Result: 论文报告了正在进行中的生产部署，描述了部署生命周期和运营快照。对检索和编码任务的实际效果仍在评估中。

Conclusion: 该系统通过在平台层实现经验捕获与编码工作流的深度融合，有望解决企业编码代理知识复用不足的问题，但有效性评估尚在进行中。
...
### [307] [AgentStream: How Well Do Self-Evolving LLM Agents Perform Under Streaming Tasks?](https://arxiv.org/abs/2608.00155)
*Dong Yan,Jian Liang,Dapeng Hu,Ran He,Nicholas Jing Yuan,Qi Zhang,Tieniu Tan*

Main category: cs.AI

TL;DR: 本文提出 AgentStream 框架，用于在真实的流式任务场景下评估自进化 LLM 智能体，通过配置可变的任务流并实例化三种流式场景（隔离、顺序、交错），系统性地组合评估五种自进化方法和三种前沿基础模型，揭示了自进化可靠性随流式场景变化、自进化收益受模型能力门控且呈非单调关系，以及没有单一方法在所有场景下占优等关键发现。


<details>
  <summary>Details</summary>
Motivation: 现有自进化智能体的研究主要采用独立评估方式，无法反映智能体在真实流式任务环境中的适应行为。为弥补这一空白，作者引入 AgentStream 统一框架来评估多样化流式场景下的自进化智能体。

Method: 将智能体基准测试组织为可配置的任务流，在测试时实例化 Isolated、Sequential 和 Interleaved 三种流式场景（逐步变化任务流的范围和领域组成），并对五种代表性自进化方法和三种前沿基础模型进行组合式评估，以解耦模型能力、方法架构和流式场景对自进化的联合影响。

Result: 结果显示：自进化可靠性因流式场景而异；自进化的收益受模型能力门控，且与模型强度呈非单调关系；没有任何单一方法在模型和场景组合中占优。

Conclusion: 自进化智能体应在真实任务流而非孤立单任务设置下进行评估，研究结果为在不同模型和流式场景下选择自进化方法提供了具体指导。
...
### [308] [TRACE-TS: Attribution-Grounded and Traceable Sensor-Language Reasoning for Human Activity Understanding](https://arxiv.org/abs/2608.00200)
*Sparsh Rastogi,Tanmay Kumar,Baiyu Chen,Jatin Bedi,Zechen Li,Flora D. Salim*

Main category: cs.AI

TL;DR: 提出了TRACE-TS框架，通过专家分类器的归因识别显著时空传感器区域，构建带证据来源的DAG推理轨迹，并训练紧凑语言模型生成这些轨迹，实现可验证的传感器时间序列推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于语言模型的可穿戴传感器数据方法生成的解释流畅但缺乏对底层信号的充分依据，导致解释难以验证。作者旨在解决推理轨迹与传感器信号之间的grounding问题。

Method: 使用专家分类器归因识别显著时空传感器区域，构建DAG推理轨迹并显式标注证据来源；通过门控交叉注意力在传感器记忆token上训练紧凑语言模型生成这些轨迹；推理时联合输出活动预测与推理轨迹，无需归因计算或教师指导；提出SNM（语义节点匹配）LLM评判指标用于诊断推理保真度。

Result: 在七个可穿戴基准上，TRACE-TS取得最佳平均准确率84.43%和F1 81.24%，相比最好的基于LLM的基线在F1上提升17.96%。

Conclusion: TRACE-TS证明了通过显式证据溯源与结构化推理轨迹，可以有效提升可穿戴传感器时间序列推理的可验证性，同时保持甚至超过现有方法的预测性能。
...
### [309] [Personalizing Large Language Model Agents with Small Policy Models](https://arxiv.org/abs/2608.00215)
*Dian Jin,Zhi Zhang,Huichao Li,Yihe Pan,Rundong Huang,Doudou Zhou*

Main category: cs.AI

TL;DR: 本文提出FABLE（因子化自适应带状层），一种轻量级策略层，通过在线学习为冻结的大语言模型智能体实现个性化执行决策，无需微调模型即可从标量反馈中学习用户特定偏好。


<details>
  <summary>Details</summary>
Motivation: LLM智能体可以检索记忆、调用工具、提问和调整响应风格，但将这些执行决策适应到个体用户仍然困难。微调单独LLM成本高昂或对专有系统不可行，而提示和记忆主要向智能体暴露用户信息，而非从反馈中适应执行决策。

Method: FABLE将个性化问题形式化为从已执行动作的标量反馈中在线学习每用户执行策略。它将记忆、信息获取和响应决策因子化，使反馈更新相关选择；在探索前通过外部指定可行集过滤动作；并利用贝叶斯上下文汤普森采样学习用户特定残差偏好（相对于固定默认和成本分数的）。还提供校准变体（在线性残差奖励模型下有期望遗憾界）、偏好可识别性分析和即时有效的虚假提升控制。

Result: 在个性化推理、受控反馈和可执行工具使用评估中，FABLE相对于仅规则控制改善了几种偏好敏感行为，同时在端到端任务性能上保持竞争力。

Conclusion: FABLE证明了可以在不微调底层LLM的情况下，通过外部轻量级策略层实现有效的个性化执行决策学习，为黑盒或专有系统的个性化提供了一条可行路径。
...
### [310] [More Debate, Same Evidence: Structural Limits of Homogeneous Multi-Agent Groundedness](https://arxiv.org/abs/2608.00243)
*Yuelyu Ji*

Main category: cs.AI

TL;DR: 本文研究了多智能体LLM评审团在接地性验证任务中的效果，发现与单一智能体相比，三智能体同质评审团在六个基准上的准确率差异从+8.5到-4.4个百分点不等，其中两个数据集有可靠提升，一个可靠下降，三个统计上不显著。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM评审团被广泛采用，假设通过交换批评意见可以提高判断质量，但这一假设缺乏系统性验证。

Method: 使用同质三智能体评审团，在六个公开的事实验证和幻觉检测基准上进行接地性验证实验，并与固定单智能体参照系统进行对比。

Result: 系统级准确率差异范围为+8.5至-4.4个百分点；两个数据集显示可靠增益，一个显示可靠损失，三个统计结果不确定。由于参照与评审团使用不同模型变体，差异表征完整系统而非纯粹的辩论因果效应。

Conclusion: 多智能体评审团的优势并非普遍成立，其效果因数据集而异，且现有实验设计无法隔离辩论机制本身的因果贡献。
...
### [311] [Geometric Self-Supervised Pre-training for Neural Combinatorial Optimization](https://arxiv.org/abs/2608.00270)
*David Aguado,Daniel Fuertes,Carlos R. del-Blanco,Fernando Jaureguizar*

Main category: cs.AI

TL;DR: 本文提出了一种面向路由问题的几何自监督预训练框架，通过等距变换（旋转和轴向反射）捕捉空间不变性和全局相对距离分布，使强化学习模型在预训练阶段学习到鲁棒的结构表征，从而显著提升模型在高维TSP实例上的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 神经组合优化（NCO）在解决TSP等路由问题时高效，但强化学习模型在扩展到高维实例时泛化能力严重受限。自监督预训练在CV和NLP领域已证明有效，但路由图仅具有2D空间坐标，缺乏复杂的拓扑属性，直接应用预训练策略存在挑战。

Method: 提出一个几何自监督预训练框架，通过施加等距变换（如旋转和轴向反射）来增强模型对空间不变性的学习，同时捕捉全局相对距离分布，使模型在策略优化阶段之前学习到鲁棒的结构表征。

Result: 该策略在零样本大规模外推场景（TSP1,000）中，相比从零训练的基线模型，行程长度提升了7.23%；同时计算效率显著，在超大规模实例上比精确求解器Concorde快了两个数量级。

Conclusion: 几何自监督预训练能够有效提升NCO模型在高维路由问题上的泛化能力，且兼具高效的计算性能，为NCO领域提供了一种实用的预训练范式。
...
### [312] [RF-HOI: Recognize Human-Object Interaction with Radio Frequency Signals](https://arxiv.org/abs/2608.00289)
*Lihao Wang,Linlu Gao,Jiacan Yu,Yanyu Lin,Yifan Yin,Jianxin Wang,Tianmin Shu,Renjie Zhao*

Main category: cs.AI

TL;DR: 本文提出了RF-HOI，首个仅使用射频（RF）信号进行人-物交互（HOI）识别的框架。通过融合毫米波雷达和RFID实现动作识别与目标识别的多模态融合，并利用仿真器合成多样化的RF数据以解决训练数据不足的问题。实验表明RF-HOI优于所有基线方法，接近视觉模型性能，且在真实场景中表现良好。


<details>
  <summary>Details</summary>
Motivation: 视觉-based的HOI识别方法面临隐私泄露和光照条件差等挑战，需要一种不依赖视觉信号的替代方案。

Method: 1) 融合毫米波雷达和RFID两种RF模态，实现动作识别与目标识别的同步进行；2) 开发仿真器合成多样化的多模态RF训练数据，仅需少量真实数据微调即可提升泛化能力。

Result: RF-HOI在所有基线方法中表现最优，性能接近视觉模型；合成的多样化训练数据能显著提升系统在真实场景中的性能。

Conclusion: 多模态RF感知在鲁棒且保护隐私的HOI识别方面具有巨大潜力，同时验证了RF数据合成方法的有效性。
...
### [313] [WM-Cov: Test Adequacy for Interactive World-Model-Style Autonomous Driving Simulation](https://arxiv.org/abs/2608.00298)
*Jianxun Cui,Ping Wu,Stanisa Peric,Marko Milojkovic,Vladan Devedzic*

Main category: cs.AI

TL;DR: 本文提出WM-Cov，一个与提供者无关的评价层，用于量化世界模型式自动驾驶闭环测试的充分性。通过覆盖率增长、有效失败发现、故障模式多样性、真实性等指标，从原始输出中提取有效证据，帮助判断测试是否达到预期目的。


<details>
  <summary>Details</summary>
Motivation: 世界模型和生成式模拟器作为自动驾驶交互式测试基础设施，将测试场景从固定回放轨迹变为交互式场景族，其演化取决于被测规划器。因此，不仅要问能否生成危险rollout，更要问什么有效的闭环证据足以支持测试意图和停止决策。目前缺乏明确的测试充分性框架。

Method: 提出WM-Cov评价层，将原始提供者输出转化为"请求的"、"已实现的"和"有效的"三类证据。通过覆盖增长、有效失败发现、故障模式多样性、真实性、伪影抑制、重复计账和有效证据精度等维度报告充分性。在TeraSim/SUMO事件、WM样混合轨迹池和真实DriveArena矩阵上进行验证。

Result: DriveArena矩阵评估了2个规划器、2个时域、6个提示条件和360个ego路径请求；304次尝试成为完全实现的证据，56次保持部分实现。额外的80-请求路线子集检查得到74次完全实现和6次部分实现。结果表明危险外观事件可能包含有效ADS故障、重复、部分实现和伪影。

Conclusion: 应当基于预算下有效交互证据的收敛性来评估世界模型式测试，而非仅依赖原始生成的失败数或提示覆盖率。WM-Cov为不同的提供者提供统一的评价框架，支持测试意图验证和停止决策。
...
### [314] [CrystalMem: Elastic Memory for Self-Evolving LLM Agents via Knowledge Crystallization](https://arxiv.org/abs/2608.00303)
*Beining Wu,Jun Huang*

Main category: cs.AI

TL;DR: 本文提出CrystalMem，一种弹性记忆侧车系统，解决大语言模型智能体在内存预算收缩-恢复周期中出现的"记忆迟滞"问题（能力无法恢复到压缩前水平），通过四态保真度降级、结晶能量调度和验证性重结晶机制，在七种环境、十七种方法、六种骨干网络上均取得最优恢复能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体记忆系统假设字节预算只会增长，但云平台会随负载和成本调整配额。研究发现经过"压缩-恢复"周期后，智能体能力无法回到压缩前水平（记忆迟滞）。作者证明任何仅保留或丢弃条目的策略都存在残余赤字下限，因此需要新的弹性记忆方案。

Method: 提出CrystalMem（结晶化记忆）系统，包含三个核心机制：(1) 在结晶能量调度下将条目降级到四种保真度状态；(2) 使用优势加权影响力结合依赖耦合来排序降级顺序；(3) 在明确的计算和字节上限下，通过验证性重结晶恢复能力。

Result: 在七种环境、十七种方法、六种骨干网络上，包括多租户服务和物理边缘云部署场景，CrystalMem在所有设置中均取得最高恢复能力。从50%字节预算出发，CrystalMem在每个环境中都能匹配最强预算基线在全量供给时的表现；在同等预算下平均领先4.6个百分点。

Conclusion: CrystalMem通过弹性的多状态记忆管理策略，成功解决了智能体记忆在预算波动下的迟滞问题，为云环境中LLM智能体的记忆管理提供了可行的解决方案。
...
### [315] [Trust and Its Betrayal under Three Representational Strategies](https://arxiv.org/abs/2608.00321)
*Mihnea C. Moldoveanu,Joel A. C. Baum*

Main category: cs.AI

TL;DR: 本文提出信任是一种虚拟（subjunctive）认知状态，分别用词典序假设、Lewis–Stalnaker球系中的序数接近性以及条件概率系统中的强信念三种表示来刻画信任，并考察在Brandenburger–Keisler共同信念不可能性框架下，用可信赖假设替换理性假设后共同信任是否可实现。三种表示一致认为有限深度的共同信任可实现，但完成态的共同信任不动点均存在困难，且三种表示在遭遇具体背叛时表现截然不同。


<details>
  <summary>Details</summary>
Motivation: 信任是一种独特的命题态度，其失望（背叛）与预测失败的失望在本质上不同，现有逻辑模型尚未系统刻画这种区别。作者希望用虚拟条件句的认知结构来建模信任，并检验在共同信念的博弈论框架中，信任能否替代理性作为一个可行的基础假设。

Method: 作者将信任分解为对能力（competence）和品性（integrity）两个虚拟条件信念，分别用三种数学表示建模：词典序假设（lexicographic assumption）、Lewis–Stalnaker球系中的序数接近性（ordinal closeness）、条件概率系统中的强信念（strong belief）。在每个表示下分析共同信任的有限深度与完成态不动点的可实现性，并通过一个具体的背叛实例检验三种模型的行为差异。

Result: 三种表示均得出：任意有限深度的共同信任都是可实现的，但完成态的共同信任不动点（对应Brandenburger–Keisler不可能性的类似物）是困难的来源。然而困难的表现形式各不相同：同一背叛在词典序表示下推翻代理人的层级排序，在序数表示下按背叛者的蓄意程度污染其接近性排序，在强信念表示下仅使操作条件的假设发生偏移而信念结构仍保持一致。

Conclusion: 三种表示虽在共同信任的可实现性结论上一致，但在背叛的动态反应上存在本质差异，说明信任的建模选择对博弈论中共同假设的稳定性有深远影响。强信念表示在背叛面前保持信念结构的一致性和自我修复能力，可能是最稳健的信任表示。
...
### [316] [Learning to Coordinate Symbolic Tools: LLM Agents for Verified Sum-of-Squares Certificates](https://arxiv.org/abs/2608.00326)
*Bohan Chen,Shivam N. Patel,Richard Hoffmann,Sam Looi,Tony Yue Yu*

Main category: cs.AI

TL;DR: 本文研究了将工具调用与强化学习相结合，用于加权平方和（SOS）分解这一可机器验证的数学任务。通过构建135万个合成示例覆盖八个辅助多项式任务，结合SFT和GRPO训练，并集成SymPy工具调用，系统在加权SOS任务上达到78.96%的验证成功率，相比基线模型（44.73%）有显著提升。


<details>
  <summary>Details</summary>
Motivation: 工具调用是LLM在数学领域求解问题的重要能力，但加权SOS分解这一任务需要候选方案可以精确验证，同时面临多项式的非唯一重新分组和多种符号变换协调的挑战。已有方法仅针对复合SOS任务进行训练，缺乏对支撑性代数技能的分解式训练。

Method: 构建135万个涵盖八个支撑性多项式任务的合成训练样本；采用两阶段训练：先进行SFT（包含直接代数问题和模拟符号轨迹，不包含原生工具调用消息），再使用GRPO结合任务特定的符号奖励进行强化学习；在评估阶段使用原生SymPy调用执行展开、收集、重排和因式分解等操作；最终通过精确展开和系数比较验证所有SOS答案。

Result: 在相同生成器合成的留出测试集上，完整的SFT+GRPO+工具系统在四个评估配置中表现最强，加权SOS验证成功率78.96%（基线模型仅44.73%），九个多项式任务的宏平均准确率达91.75%。

Conclusion: 该工作展示了在可精确验证输出的领域中将领域特定技能训练、可执行工具和验证器反馈相结合的有效性，为其他具有精确可检查输出的工具调用智能体设计提供了参考案例。
...
### [317] [RMSWeb: Reflection, Failure-Mode Mining, and Salvage-DS for Web Agent Reinforcement Learning](https://arxiv.org/abs/2608.00335)
*Chengbo Liu,Lifang Zhou,Ruijie Yan,Pei Tan,Ao Sun,Haojun Huang,Guichun Hua,Sining Wei,Yining Chen,Yingying He,Yutao Xie*

Main category: cs.AI

TL;DR: 本文提出RMSWeb，一个针对紧凑型网页代理的三部分训练方案，包括反思条件重试、失败模式挖掘和Salvage-DS奖励机制，在8B和32B的Qwen3-VL-Instruct模型上验证，减少了行动步骤并提升了多项基准测试的性能。


<details>
  <summary>Details</summary>
Motivation: 紧凑型网页代理虽可降低部署成本，但训练面临数据收集困难（成功轨迹昂贵且含低效绕路）以及SFT后RL的挑战：全轨迹语料被常规状态主导，动作级奖励设计不当会导致弱化或误导的相对更新，被拒绝的组群缺乏回退学习信号。

Method: RMSWeb包含三个部分：1) 反思条件重试（Reflection-conditioned retries）提高数据收集产出并缩短成功轨迹；2) 失败模式挖掘（Failure-mode mining）将离线RL聚焦于SFT策略暴露的关键状态；3) Salvage-DS，结合动作语义极化奖励、对比与能力门控的动态采样，以及针对被拒绝组群的动作专用锚点。

Result: 反思收集数据训练的策略在已解决任务上最多减少19.7%的动作步骤。在WebVoyager、Online-Mind2Web和WebTailBench上，8B模型相对SFT提升2.4-7.0点，32B模型提升1.2-7.7点。8B模型在Online-Mind2Web上取得同类开源模型中最佳成绩，并在WebVoyager和WebTailBench上实现领先的准确率-成本权衡。

Conclusion: RMSWeb有效解决了紧凑型网页代理在数据收集和RL训练中的核心难题，在保持较小模型规模的同时实现了显著的性能提升和更好的成本效益，但需要注意外部评估协议存在差异这一局限性。
...
### [318] [Bayesian and Motivated Reasoning in AI Agents](https://arxiv.org/abs/2608.00339)
*Eddie Yang*

Main category: cs.AI

TL;DR: 本研究通过12个智能体-领域对比实验发现，AI智能体在面对相同数值数据时，会因情境框架的改变而得出不同结论——当框架与智能体的先验信念一致时更容易得出肯定结论，反之则更困难。这种框架效应甚至改变了智能体的工作方式（搜索范围、分析规格、证据评估），揭示了一个重要的AI委托决策风险。


<details>
  <summary>Details</summary>
Motivation: AI智能体越来越多地参与开放式任务，其结论可指导高风险决策。然而，智能体是否像人类一样受到情境框架和先验信念的影响尚不清楚。作者希望通过实证研究揭示这一潜在的系统性偏差，因为如果智能体的决策依赖于未在任务中指定的先验信念，将对AI的可靠部署构成重大威胁。

Method: 在医学、选举取证和地缘政治预测三个高风险领域，保持数值证据完全不变，仅改变证据出现的场景框架，进行12个智能体-领域的对比实验。比较不同框架下智能体得出的结论、搜索行为、分析规格选择和证据评估方式。

Result: 智能体的结论强烈受到其先验信念的影响：当框架围绕其认为可能的命题时，更容易得出肯定结论；当框架与其先验冲突时则相反。框架还改变了智能体的工作方式——搜索更广泛、选择不同的分析规格、对相同证据给出不同评价。

Conclusion: AI智能体存在与人类类似的框架效应和先验依赖问题，这构成了将决策委托给AI智能体的特定风险——其决策可能依赖于既未在任务中明确指定、也不在决策记录中可见的先验信念，从而影响决策的可解释性和可靠性。
...
### [319] [Gene Ontology-Guided Hierarchical Spatial Gene Expression Prediction from Histopathology Images](https://arxiv.org/abs/2608.00405)
*Zhiwen Xu,Xiaoming Yan,Chengkun Wu,Juan Chen,Haoang Chi,Liyang Xu*

Main category: cs.AI

TL;DR: 本文提出MSGR（Multi-Scale Gene Refiner），利用基因本体论（GO）作为结构化先验，将靶基因组织成四级GO树，通过GO引导的解码器以残差校正方式从粗粒度功能域逐步细化到细粒度基因预测，在不修改图像侧的条件下作为即插即用模块提升现有架构性能。


<details>
  <summary>Details</summary>
Motivation: 现有的空间基因表达预测方法将靶基因集视为扁平、无结构的向量，忽略了基因间由共享生物学通路和调控程序产生的依赖关系。由于模型只能从有限的配对数据中推断这些依赖，预测质量受到限制。

Method: MSGR将基因本体论（GO）作为显式结构先验，把靶基因组织为四级GO树；GO引导的解码器在尺度加权监督下，通过残差校正从粗粒度功能域逐步细化到细粒度单个基因的预测。该方法仅作用于基因侧，作为即插即用的解码器替换模块，无需修改图像侧架构。

Result: 在HEST-1k基准的九个数据集上进行大量实验，结果表明：GO结构化解码在性能上一致优于扁平解码（包括优于最先进的生成式基线）；与结构等价的随机层级相比有+0.027的增益，证明性能提升归因于生物学本体结构而非层级分解本身。

Conclusion: 实验证实了GO结构化解码的有效性，且增益确实来源于生物学本体结构的先验信息，而非仅仅是层级分解带来的结构优势。
...
### [320] [Where did the ambiguity go? Examining how multimodal models interpret polysemous words](https://arxiv.org/abs/2608.00410)
*Jasin Cekinmez,Addison J. Wu,Raja Marjieh,Thomas L. Griffiths*

Main category: cs.AI

TL;DR: 该研究调查了多义词在文本生成和文生图模型中的表现差异，发现图像生成模型在表达多义词不同含义时的多样性远低于文本生成模型，且两者均低于人类想象的多样性。


<details>
  <summary>Details</summary>
Motivation: 人类语言高度多义，许多常见词汇携带多个不同含义。虽然已知大型语言模型能理解多义性，但对图像等其他模态中多义性的表现知之甚少，因此需要跨模态研究多义性如何呈现。

Method: 对17个文生图模型和15个文本生成模型进行测试，向每个模型提供无上下文的多义词，通过大量采样测量模型生成的词义分布。使用归一化熵来衡量词义多样性，并与人类对相同词汇的想象进行对比。

Result: 发现了明显的多模态差距：同一模型家族内，图像生成比句子生成使用的词义明显更少（归一化熵0.10 vs 0.25），二者都远低于人类想象的多样性（归一化熵0.47）。此外，当让模型预测自己生成不同词义的概率时，其预测分布比实际生成输出的多样性更高。

Conclusion: 研究揭示了基础模型在不同模态间表达意义时存在多模态差距，表明模型对多义词的理解在不同模态间可能无法忠实地、平等地迁移。
...
### [321] [SymboUQ: Symbolic Uncertainty Quantification for Spatial Reasoning in LLMs](https://arxiv.org/abs/2608.00417)
*Dahai Yu,Lin Jiang,Rongchao Xu,Guang Wang*

Main category: cs.AI

TL;DR: SymboUQ是一个符号化不确定性量化框架，通过区分"可符号化性"（claim能否在验证器的形式语言中表示）和"语义确定性"（执行是否产生确定判定），从推理轨迹中估计最终答案的可靠性，在五个空间推理基准上相比最强基线在AUROC上提升约8%（相对），在类平衡Brier损失上降低约7%（相对）。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）虽能生成流畅的空间推理轨迹，但其中间关系可能无法支撑最终结论，仅靠token级置信度不足以可靠估计最终答案的正确性。现有的形式验证器能提供更强的语义证据，但其适用性是局部的——解析出的claim不一定能产生确定的语义判定结果，因此需要一种能够区分不同验证适用情形的方法来更好地估计答案可靠性。

Method: SymboUQ框架包含三个核心组件：（i）Layout Auditor——执行有序空间claim并提取可行性、冲突和修复证据；（ii）无标签的确定性剖面（Determinacy Profile）——刻画有效的可执行覆盖范围；（iii）确定性感知的可靠性合成器（Determinacy-Aware Reliability Composer）——根据验证器的适用性，整合基于约束、基于表示和基于解码的评分。该框架的核心思想是将"可符号化性"（claim能否在验证器形式语言中表示）与"语义确定性"（执行是产生蕴含/矛盾判定还是未知/不可评估结果）相区分。

Result: 在五个空间推理基准和四个冻结LLM骨干上的广泛实验表明，SymboUQ相比最强基线实现了约8%的相对AUROC提升和约7%的相对类平衡Brier损失降低。

Conclusion: 通过将可符号化性与语义确定性解耦，SymboUQ能够更精准地利用形式验证器的部分适用性来估计LLM空间推理的最终答案可靠性，为符号化不确定性量化提供了一种有效的新范式。
...
### [322] [Mask-Based Priors Are More Persistent than Query-Key Initializations](https://arxiv.org/abs/2608.00418)
*Mingze Ma,Hemanth Saratchandran,Cameron Gordon,Simon Lucey*

Main category: cs.AI

TL;DR: 本文发现Transformer在布尔外推任务上并非简单地缺乏数据，而是以系统性的错误方式进行泛化（倾向于最小度插值器而非真实目标函数）。现有基于QK投影的结构化初始化方法在布尔外推任务上会被训练过程快速覆盖，无法改变外推规则。作者提出直接初始化可学习的注意力掩码（attention mask），将结构先验与内容相关的注意力分数分离，使其能在优化过程中持续存在。实验表明掩码初始化在布尔推理任务上实现近乎完美的外推，同时提升低数据算术性能，并在视觉和语言基准上保持竞争力。


<details>
  <summary>Details</summary>
Motivation: Transformer在未见数据上的外推存在系统性错误——倾向于最小度插值器而非真实目标函数。现有方法仅通过QK投影间接注入结构先验，但这类先验在训练中会被快速覆盖，无法纠正错误的归纳偏置。因此需要寻找一种能够在优化过程中持久保持的结构先验注入方式。

Method: 提出直接初始化可学习的注意力掩码（attention-logit bias），该掩码来源于任务层面的交互结构，是有界且可学习的。与标准硬掩码（用于因果或局部注意力）不同，该方法将结构先验与内容相关的注意力分数分离，使先验在训练中不被覆盖。

Result: 在布尔推理任务上，掩码初始化实现近乎完美的外推，而vanilla和QK初始化Transformer仍受默认归纳偏置限制；该方法还提升了低数据算术性能，并在视觉和语言基准上保持竞争力。

Conclusion: 注意力掩码不仅可作为架构约束，还可作为编码Transformer中持久归纳偏置的简单有效载体。
...
### [323] [TrAC: Trace-Conditioned Answer Consistency for Efficient Uncertainty Quantification in LLMs](https://arxiv.org/abs/2608.00422)
*Dahai Yu,Lin Jiang,Rongchao Xu,Guang Wang*
...
### [324] [Diagnose Before You Compress: Prediction-Independent Bottleneck Witness Refinement for LLM Serving Traces](https://arxiv.org/abs/2608.00423)
...
### [325] [Ekova: A Personality-Support Agent for Self-Discovery Dialogue](https://arxiv.org/abs/2608.00478)
*Yuyan Chen*
...
### [326] [F-WANDA: Fisher-Reweighted Post-Training Pruning for Sustainable Deployment of Large Language Models](https://arxiv.org/abs/2608.00481)
*Himanshu Mishra*
...
### [327] [The Bayesian Reflex: A Predictive Coding Engine for Artificial Intelligence](https://arxiv.org/abs/2608.00492)
...
### [328] [TaPR: Test-Aware Policy Refinement for Feedback-Conditioned Code Generation](https://arxiv.org/abs/2608.00494)
...
### [329] [BayesSeg: A Bayesian Optimization Framework for State Segmentation of Electricity Consumption Time Series](https://arxiv.org/abs/2608.00513)
...
### [330] [CURE: Local Uncertainty Repair for Block-Parallel Speculative Decoding](https://arxiv.org/abs/2608.00531)
*Aofan Liu,Jingxiang Meng,Fangxin Liu,Yongbiao Chen*
...
### [331] [Through the LENS: Local Geometric Decomposition of Vision-Language Model Representations](https://arxiv.org/abs/2608.00561)
...
### [332] [Why Does the Future Branch? Identifiable Closure Tests for Stochastic Physical World Models](https://arxiv.org/abs/2608.00591)
*Yibin Dong*
...
### [333] [Escaping Confidence Trap: Evolutionary Decoding for Mathematical Reasoning in Diffusion LLMs](https://arxiv.org/abs/2608.00605)
...
### [334] [Slides2MindMap: Reconstructing Cognitively Efficient Knowledge Hierarchies from Lecture Slides](https://arxiv.org/abs/2608.00610)
...
### [335] [DASH: Decoupled Adaptive Surrogate - Acquisition Harness for Automated Bayesian Optimization](https://arxiv.org/abs/2608.00641)
...
### [336] [HetGPS: Scalable Graph Multi-Agent Reinforcement Learning with Physics-Anchored Adaptive Safety for EV Charging](https://arxiv.org/abs/2608.00679)
...
### [337] [Multi-Dimensional Assessment for AI Cognition (MAAC): A Theoretical Framework for Process-Oriented Cognitive Evaluation of Text-Based AI Systems](https://arxiv.org/abs/2608.00680)
...
### [338] [When Does LLM Orchestration Pay Off? A Controlled Evaluation of Accuracy, Cost, and Task Difficulty](https://arxiv.org/abs/2608.00685)
...
### [339] [Evolutionary Curriculum Learning Improves Biological Sequence Modeling](https://arxiv.org/abs/2608.00697)
...
### [340] [DGA$_2$D: Directed Graph-Guided Automated Algorithm Design with Large Language Models](https://arxiv.org/abs/2608.00700)
...
### [341] [Tracing the Cascade: A Topology-Aware Evaluation Framework for Scientific Agent Hallucinations](https://arxiv.org/abs/2608.00711)
...
### [342] [AI-Based Thesis Assessment: An Empirical Study of Human Evaluation Priorities and Their Impact on Automated Assessment](https://arxiv.org/abs/2608.00717)
...
### [343] [Behavioral Grammar: Detecting Adaptive Malware via Tiny Language Model Priors and Second-Order Temporal Analysis](https://arxiv.org/abs/2608.00745)
*Zihan Luo*
...
### [344] [FinDeepIndicator: Benchmarking Deep Research Agents in End-to-End Financial Indicator Construction](https://arxiv.org/abs/2608.00764)
...
### [345] [Measurement Without Validity: The Compounding Reliability Problem in Agentic AI Evaluation](https://arxiv.org/abs/2608.00794)
*William Caban*
...
### [346] [AgentSLABench: Evaluating and Benchmarking Agentic Systems Under Resource Constraints](https://arxiv.org/abs/2608.00805)
...
### [347] [Large language models improve physician accuracy but lead to false reliance](https://arxiv.org/abs/2608.00817)
...
### [348] [The Scaling Paradox in Human-AI Collaboration](https://arxiv.org/abs/2608.00818)
...
### [349] [Isotropy Cliffs: The Geometric Signature of Decision-Making in Large Language Models](https://arxiv.org/abs/2608.00828)
...
### [350] [Similarity Weighted Aggregation with Global Differential Privacy for Federated Brain Lesion Segmentation](https://arxiv.org/abs/2608.00872)
...
### [351] [Assuming You Knew: Fixing an Epistemic Semantics for Flow Policies Using Agentic AI](https://arxiv.org/abs/2608.00882)
*David A. Naumann*
...
### [352] [Neuro-Evolved Heuristics for Variable Gapped Common Subsequence Identification](https://arxiv.org/abs/2608.00888)
...
### [353] [CADIR: A Cross-Backend Editable Intermediate Representation for Agentic CAD Generation](https://arxiv.org/abs/2608.00891)
...
### [354] [Modeling Social Dynamics with an LLM-Enabled Agent Based Network-Dynamic (LAND) Model](https://arxiv.org/abs/2608.00929)
...
### [355] [PMMC: Prospective Multimodal Memory Compilation for Long-Term LVLM Agents](https://arxiv.org/abs/2608.00962)
...
### [356] [TrajWiki: Source-Grounded Memory Trajectories for Long-Horizon Dialogue Agents](https://arxiv.org/abs/2608.00967)
...
### [357] [PROGRESS: Coverage-guided RL to Train Search-augmented LLM Agent](https://arxiv.org/abs/2608.00969)
...
### [358] [Search-GRT: Guided Retrieval Training of Search Agents to Optimize for Complex Question Answering](https://arxiv.org/abs/2608.00974)
...
### [359] [Passing Coarse Marginal Checks Can Be Cheap: Persona Mixtures and Imprecise Treatment-Response Estimates in an LLM Persona Panel](https://arxiv.org/abs/2608.00979)
*Yohei Nakajima*
...
### [360] [Auditing Discovery Claims: A Two-Sided Criterion for Agentic Science, with the Negative Side Decidable](https://arxiv.org/abs/2608.00981)
...
### [361] [SCHEDBench: A Benchmark for Evaluating LLM Constraint Faithfulness in Natural-Language Combinatorial Scheduling](https://arxiv.org/abs/2608.00991)
...
### [362] [Judging Is Not Enumerating: Silent Omissions in LLM-Authored Acceptable Sets](https://arxiv.org/abs/2608.01000)
...
### [363] [From AI Technical Debt to Agentic Technical Debt: A Systematic Mapping of Root Causes and Manifestations in Agentic AI Systems](https://arxiv.org/abs/2608.01001)
...
### [364] [Toward Fine-Grained Forgetting:Attribute Unlearning for Multimodal Large Language Models](https://arxiv.org/abs/2608.01008)
...
### [365] [FactorJEPA: Factorizing Monolithic Futures into Layout-Agent-Interaction Channels for Crowded and Chaotic Global South Urban Worlds](https://arxiv.org/abs/2608.01049)
...
### [366] [Don't Offer What Can't Be Done: Deterministic Executability Gating for LLM Skill Selection at Scale](https://arxiv.org/abs/2608.01050)
...
### [367] [Control Under Compression: Reliability Frontiers for Tool-Using Agents](https://arxiv.org/abs/2608.01056)
...
### [368] [Role-Decoupled Attention Residuals: Separating Matching and Content Retrieval Across Depth](https://arxiv.org/abs/2608.01075)
*Kehan Wang*
...
### [369] [Inter-Residue Geometry Attention for Antibody-Specific Epitope Prediction](https://arxiv.org/abs/2608.01092)
...
### [370] [Fighting Fire with Fire: On the Feasibility of Protecting Exercises Against AI Cheating](https://arxiv.org/abs/2608.01112)
...
### [371] [MA-HEAD-Net: Adaptive Rule-Guided Multi-Agent DRL for AoI Minimization in UAV-Assisted Emergency Networks](https://arxiv.org/abs/2608.01128)
...
### [372] [PATH-Bench: Path-Dependent Evaluation of Lifelong Agents](https://arxiv.org/abs/2608.01149)
...
### [373] [The Graph Language: How Knowledge Graphs Speak to Large Language Models](https://arxiv.org/abs/2608.01175)
*Giuseppe Pirrò*
...
### [374] [Co-evolution of social reward and punishment under institutional interventions](https://arxiv.org/abs/2608.01183)
...
### [375] [Humans Are More Diverse: Frontier LLMs Show Extreme Policies in Idealised AI Development Races](https://arxiv.org/abs/2608.01193)
...
### [376] [Reputation-driven Cooperation in Lattice-based Decentralized Federated Learning through Evolutionary Game Theory](https://arxiv.org/abs/2608.01197)
...
### [377] [Perspectives on Tsallis Statistics for Artificial Intelligence](https://arxiv.org/abs/2608.01223)
...
### [378] [CT-PrepAgent: Bounded Policy and Controlled Execution for Adaptive CT Data Preparation](https://arxiv.org/abs/2608.01233)
...
### [379] [Learning What to Remember and What to Internalize in LLM Self-Evolution via Adaptive Memory-Parameter Coordination](https://arxiv.org/abs/2608.01234)
...
### [380] [Cognitive Demand Steering for Adaptive Meta-Reasoning in Large Language Models](https://arxiv.org/abs/2608.01319)
...
### [381] [G-ReAct: Graph-Guided Deep Search via Structure-State Co-Evolution](https://arxiv.org/abs/2608.01324)
...
### [382] [402Pilot: An x402 Decision Layer for Autonomous Agent Micropayments](https://arxiv.org/abs/2608.01341)
...
### [383] [Agentic Stage-One Stellarator Optimization: Autonomous Multi-Objective Search for Finite-Beta Equilibria](https://arxiv.org/abs/2608.01344)
...
### [384] [High-Stakes Decisions with Language Models: Insights from Emergency Triage](https://arxiv.org/abs/2608.01361)
...
### [385] [CRAFTS: Collaborative Role-Adaptive Fine-Tuning of LLM Agents for Chemical Process Simulation](https://arxiv.org/abs/2608.01369)
...
### [386] [CraftAlign: Feature-Grounded Evaluation and Revision Guidance for AI Stories](https://arxiv.org/abs/2608.01377)
...
### [387] [KoVRE: Training an Efficient Embedding Model for Korean Visual Document Retrieval](https://arxiv.org/abs/2608.01389)
...
### [388] [No Single Neuron of Failure: Distributed Safety Alignment Against White-Box Attacks](https://arxiv.org/abs/2608.01414)
...
### [389] [Reusing Rollouts under Policy Lag: Prefix-Normalized Policy Optimization for LLM Reinforcement Learning](https://arxiv.org/abs/2608.01418)
...
### [390] [Scoring Rules! Statistical and Strategic Alignment for Text Evaluation Metrics](https://arxiv.org/abs/2608.01423)
...
### [391] [MRAFnd: Multimodal Retrieval-Augmented Framework for Zero-Shot Fake News Detection](https://arxiv.org/abs/2608.01430)
...
### [392] [PolymerGPT: Multi-property Optimization with a Decoder-Based GPT Model for Generative Polymer Design](https://arxiv.org/abs/2608.01431)
...
### [393] [A New Theory of Value for Post-AGI Economics](https://arxiv.org/abs/2608.01432)
*Keyun Ruan*
...
### [394] [Beyond Routing Saturation: A Long-Horizon Class-Incremental Perspective on Expert Routing in Multimodal Continual Instruction Tuning](https://arxiv.org/abs/2608.01437)
...
### [395] [Loud or Silent? A Reusable Framework for Per-Modality Failure Analysis in Multimodal Clinical AI](https://arxiv.org/abs/2608.01462)
...
### [396] [Where Reasoning Diverges: Localized Multi-Agent Debate](https://arxiv.org/abs/2608.01463)
...
### [397] [Computing with Agentic Oracles](https://arxiv.org/abs/2608.01464)
*Jie Wang*
...
### [398] [Sweet Little Lies: Strategic Deception in AI Emotional Support Chatbots](https://arxiv.org/abs/2608.01480)
...
### [399] [MineGrad: Gradient Inversion Attacks on LoRA Fine-Tuning](https://arxiv.org/abs/2608.01521)
...
### [400] [V-Mem: Modality-Routed Retrieval for Long-Term Multimodal Agentic Memory](https://arxiv.org/abs/2608.01543)
...
### [401] [Emergence Invariance: From Symbolized Thought to Interface Refinement](https://arxiv.org/abs/2608.01548)
*Yi Liu*
...
### [402] [Securing Agentic AI: From Per-Action Checks to Trajectory Assurance](https://arxiv.org/abs/2608.01558)
...
### [403] [Does the Competitive Component of Adversarial Self-Play Improve Legal Reasoning? A Controlled Negative Result](https://arxiv.org/abs/2608.01559)
*Miseog Shawn Kim*
...
### [404] [Is More Privileged Information Better? From Solution Traces to Problem-Solving Structure in Self-Distilled Reasoning](https://arxiv.org/abs/2608.01589)
...
### [405] [Latent Thought Credit: Multi-Answer Credit Assignment for Latent Reasoning](https://arxiv.org/abs/2608.01593)
...
### [406] [Post-Training on Office Work Improves Software Engineering: A Behavioral Account of Cross-Domain Transfer](https://arxiv.org/abs/2608.01604)
...
### [407] [When Memory Updates but Behavior Does Not: Repairing Implicit Stale Dependencies in Personalized Agent Responses](https://arxiv.org/abs/2608.01619)
*Haofei Sun,Lin He*
...
### [408] [Salami Attack: Stealthy Collusive Memory Poisoning against OpenClaw](https://arxiv.org/abs/2608.01637)
...
### [409] [GISAgentBench: A Practitioner-Sourced Benchmark for Evaluating LLM Agents on GIS Tasks](https://arxiv.org/abs/2608.01645)
...
### [410] [LongCat Sparse Attention: Taming the Lightning via Streaming-aware Hierarchical Cross-Layer Indexing](https://arxiv.org/abs/2608.01662)
...
### [411] [Allocation Before Ranking: Decoupled Token Compression for OmniLLMs](https://arxiv.org/abs/2608.01665)
...
### [412] [TCPO: Turn-Level Credit Policy Optimization](https://arxiv.org/abs/2608.01667)
...
### [413] [When Memory Becomes Authority: Benchmarking Authority Collapse at the Memory Consolidation Boundary](https://arxiv.org/abs/2608.01679)
...
### [414] [GABench: A Comprehensive Benchmark for Evaluating LLM Agents on Graph Analysis Tasks](https://arxiv.org/abs/2608.01684)
...
### [415] [Beyond Single-Use Tokens: Durable Authorization State for Replay-Resistant LLM Agent Actions](https://arxiv.org/abs/2608.01710)
...
### [416] [Constructing Executable Analytical Knowledge Representations for Meta-Analysis Synthesis Using an Agentic Harness](https://arxiv.org/abs/2608.01711)
...
### [417] [LaCache: Robust Semantic Caching for LLM Serving](https://arxiv.org/abs/2608.01718)
...
### [418] [DAPD: Dual-Anchored Policy Distillation](https://arxiv.org/abs/2608.01735)
...
### [419] [CoEvo-Mem: Co-Evolving Retrieval Policy and Memory Bank for LLM Agents](https://arxiv.org/abs/2608.01739)
...
### [420] [MemSIF: From Structured Interactions to Dual-Track Fact Memory for LLM Agents](https://arxiv.org/abs/2608.01742)
...
### [421] [RL-Lock: Reinforcement Learning for Generating Interlocking Assemblies](https://arxiv.org/abs/2608.01744)
...
### [422] [Deferred Exposure of Future Trajectories for Verifiable Reasoning in Autonomous Driving VLMs](https://arxiv.org/abs/2608.01755)
...
### [423] [Leveraging AI for fine-grained food safety risk forecasting in sparse data conditions](https://arxiv.org/abs/2608.01767)
...
### [424] [FRAMES: Guarded and Dual-Objective Skill Evolution for Agents in Policy-Governed Enterprise Workflows](https://arxiv.org/abs/2608.01772)
...
### [425] [REFLEX: Rethinking MoE Inference as Refinement-Aware Compute Allocation in Diffusion Language Models](https://arxiv.org/abs/2608.01784)
...
### [426] [Can You Trust the Confidence? ConfBench for Vision-Language Models on Document Extraction](https://arxiv.org/abs/2608.01792)
...
### [427] [CoNav-UAV: Cooperative Dual-Altitude Aerial Navigation via Stackelberg Learning](https://arxiv.org/abs/2608.01802)
...
### [428] [CockpitHAT: Dependency-Graph-Driven Hierarchical Attribution for Embodied Multi-Agent Cockpits](https://arxiv.org/abs/2608.01805)
...
### [429] [SearchMaster: Grounded and Regulated Self-Play for Search Agents](https://arxiv.org/abs/2608.01822)
...
### [430] [Rewriting or Reweighting? A Geometric Account in Language Models](https://arxiv.org/abs/2608.01835)
...
### [431] [PCSD: Persistent Consistency for Self-Distillation in Agentic Reinforcement Learning](https://arxiv.org/abs/2608.01837)
...
### [432] [FOCUS: FP4 Optimization via Coupled-Relaxation and Dual-Granularity Scaling](https://arxiv.org/abs/2608.01847)
...
### [433] [Exploring and Bridging Knowledge Holes in Unlearned Multimodal Large Language Models](https://arxiv.org/abs/2608.01849)
...
### [434] [Physics-Informed Neural Networks for Complex Eigenfrequency Identification and Mode Structure Reconstruction of the Ground-State ITG Branch](https://arxiv.org/abs/2608.01850)
...
### [435] [EchoChange: A Diffusion Language Model with Dual Pass Remasking for Factual Remote Sensing Disaster Change Captioning](https://arxiv.org/abs/2608.01856)
...
### [436] [Wnuan: Staged Post-Training for Question Answering over Proprietary Enterprise Knowledge](https://arxiv.org/abs/2608.01862)
...
### [437] [ReasonCast: Towards Explainable Time Series Forecasting with Reasoning](https://arxiv.org/abs/2608.01875)
...
### [438] [CoEvoKG: Co-Evolving Knowledge Graphs with Self-Evolving Search Agents](https://arxiv.org/abs/2608.01904)
...
### [439] [Diagnosing Search Behavior and Failure Modes in Long-Horizon Search Agents](https://arxiv.org/abs/2608.01913)
...
### [440] [ProWorld: Progress-Aware Hyperbolic World Models for Long-Horizon Visual Goal Reaching](https://arxiv.org/abs/2608.01926)
...
### [441] [A Contractualist Argumentation Framework for Moral Decision-Making](https://arxiv.org/abs/2608.01937)
...
### [442] [Long-Horizon Autonomous Architecture Research with a Language-Model Agent: A Behavioural Case Study](https://arxiv.org/abs/2608.01995)
...
### [443] [Evolving in the Agent Jungle via History-Informed Opponent Awareness](https://arxiv.org/abs/2608.02005)
...
### [444] [Instruction-Conditioned Exploration with Asymmetric Reinforcement Learning and Self-Distillation](https://arxiv.org/abs/2608.02087)
...
### [445] [HALT: Verification-Aware Stopping for Retrieval-Augmented Search Agents](https://arxiv.org/abs/2608.02009)
...
### [446] [Before Reasoning Fails: Pre-Evidence Procedural Failures in Agentic RAG](https://arxiv.org/abs/2608.02011)
*Daeyoung Roh,Donghee Han*
...
### [447] [Right Answer, Wrong Method: Shortcut Hacking Misleads the Evaluation of LLM Reasoning on Frontier Science Benchmarks](https://arxiv.org/abs/2608.02442)
...
### [448] [EduZone: A Framework for Evaluating LLM Safety for K-12 Students and Teachers](https://arxiv.org/abs/2608.02024)
...
### [449] [HPFA: Hypergraph-Based Paired Failure Attribution for LLM Reasoning](https://arxiv.org/abs/2608.02026)
...
### [450] [Cross-Fitted Residual Utility for Primary-Preserving Cognitive Decision Correction in Automatic Modulation Classification](https://arxiv.org/abs/2608.02063)
...
### [451] [Fetch-then-Explore: Decoupling Selection from Extraction over a Persistent Workspace for Search Agents](https://arxiv.org/abs/2608.02097)
...
### [452] [MemArbiter: Decision-Time Memory Arbitration for Long-Horizon LLM Agents](https://arxiv.org/abs/2608.02113)
...
### [453] [Beyond Solution-Centric Search: Adaptive Inquiry and Knowledge Revision for Autonomous ML Engineering](https://arxiv.org/abs/2608.02143)
...
### [454] [Beyond the Mean: Multi-Moment Policy Optimization for LLM Reasoning](https://arxiv.org/abs/2608.02149)
...
### [455] [Auditing Data Provenance in LLM Fine-tuning via Intrinsic Distributional Fingerprints](https://arxiv.org/abs/2608.02154)
...
### [456] [From Simple QA to Deep Research: A Verifiable Benchmark Constructed through Iterative Task Evolution](https://arxiv.org/abs/2608.02163)
...
### [457] [From Profiling to Synthesis: Benchmarking Implicit Behavioral Alignment in Personalized LLM Agents](https://arxiv.org/abs/2608.02171)
...
### [458] [PAC Approximation and DIRECT Optimization for Parametric Markov Models](https://arxiv.org/abs/2608.02184)
...
### [459] [MEGRAG: Multi-Granular Evidence Graphs for Answer-Aware Multi-Hop RAG](https://arxiv.org/abs/2608.02195)
...
### [460] [PosterMELD: Multi-Agent Paper-to-Poster Generation for Controllable Design Diversity with Editable Print-Ready Outputs](https://arxiv.org/abs/2608.02218)
...
### [461] [Trustworthy AI in Digital Health: A Comprehensive Review of Robustness and Explainability](https://arxiv.org/abs/2608.02238)
...
### [462] [Homebot: A Personal AI Agent for Conversational Home Assistance and Automation](https://arxiv.org/abs/2608.02254)
...
### [463] [Self-Certification of Representation Adequacy: Sequential Certification at Minimum Task Loss](https://arxiv.org/abs/2608.02267)
*Zijie Huang*
...
### [464] [Harness-R1: Learning to Edit Executable Runtime Harnesses from Agent Failure Trajectories](https://arxiv.org/abs/2608.02276)
...
### [465] [SKT: Skill-Use Training at Scale via Verified Synthetic Data Generation](https://arxiv.org/abs/2608.02287)
...
### [466] [Shared Prefixes, Better Credit: Adaptive Routing for Multi-Agent Reasoning](https://arxiv.org/abs/2608.02291)
...
### [467] [MechGeo: Autoformalizing and Proving Euclidean Geometry in Lean 4](https://arxiv.org/abs/2608.02295)
...
### [468] [Trajectories That Segment Themselves: Agent-Declared Boundaries as a Training Unit](https://arxiv.org/abs/2608.02302)
*Jingxi Wei*
...
### [469] [Hard Constraints, Smooth Gradients: Learning Feasible Inventory Policies via Differentiable Projection](https://arxiv.org/abs/2608.02343)
...
### [470] [Mamba with Hierarchical Memory: Solving Representation Bottleneck in Long Sequence Modeling](https://arxiv.org/abs/2608.02347)
...
### [471] [KC-Agent: A Dual-Process Cognitive Architecture for Efficient ML Model Improvement](https://arxiv.org/abs/2608.02351)
...
### [472] [SkillTrace: Traversing a Query-Skill Graph for Composable LLM Agents](https://arxiv.org/abs/2608.02356)
...
### [473] [Faster-WAM: Do World Action Models Need Deep Action Modules?](https://arxiv.org/abs/2608.02365)
...
### [474] [Chess on Ice: Curling Tactical Decision-Making via Backward Induction and Deep Reinforcement Learning](https://arxiv.org/abs/2608.02379)
...
### [475] [Cooperative Coevolution for Resource-Constrained Agentic LLM Post-Training](https://arxiv.org/abs/2608.02391)
...
### [476] [MonitrLLM: A Community-Centered Evaluation Infrastructure for Large Language Models](https://arxiv.org/abs/2608.02409)
...
### [477] [xPress: Parallel Refinement for Diffusion Drafters in Speculative Decoding](https://arxiv.org/abs/2608.02438)
...
### [478] [Agentic Commerce World: An Auditable and Verifiable Environment for Vibe Commerce](https://arxiv.org/abs/2608.02441)
...
### [479] [ParEvalLayer: When Partial LLM-Agent Evaluations Support a Decision](https://arxiv.org/abs/2608.02444)
...
### [480] [Infinite Trace Objectives with Finite Trace Techniques: Translating LTL to LTLf+](https://arxiv.org/abs/2608.02454)
...
### [481] [Real-Time Detection and Repair of LLM Agent Failures](https://arxiv.org/abs/2608.02464)
*Sunny Dubey*
...
### [482] [Long-term Measurements: Towards a Longitudinal Understanding of Human-AI Interactions](https://arxiv.org/abs/2608.02491)
...
### [483] [CMuon: Accelerating and Stabilizing Diffusion Transformer Training via Chunked Momentum Orthogonalization](https://arxiv.org/abs/2608.02502)
...
### [484] [Abduction Without a Body? Representational Grounding and the Abduction Loop for Scientific Hypothesis Generation](https://arxiv.org/abs/2608.02505)
*Michael Farmer*
...
### [485] [Optimizing Minimax Regret in Uncertain MDPs with Small Sets of Policies](https://arxiv.org/abs/2608.02509)
...
### [486] [Magnet: Detecting Cross-Session AI Misuse Through Capability Accumulation](https://arxiv.org/abs/2608.02518)
...
### [487] [A Taxonomy of Cognitive Capability Gaps in Generative and Agentic AI](https://arxiv.org/abs/2608.02553)
...
### [488] [AtumAI: A Principled Framework for Agentic Generation of Datacenter Control-Plane Policies](https://arxiv.org/abs/2608.02569)
