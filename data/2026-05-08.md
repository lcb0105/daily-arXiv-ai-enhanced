...
TL;DR: AdaGATE是一种无需训练的证据控制器，通过实体间隙追踪、微型查询生成和效用选择机制，在有限上下文下修复多跳RAG中的缺失桥梁事实，在HotpotQA上以更少token实现了更优的证据F1。


<details>
  <summary>Details</summary>
Motivation: 现有RAG控制器在处理多跳问题时存在局限：要么加性扩展上下文，要么从固定top-k集合中选择，或仅优化相关性而未能显式修复缺失的桥梁事实，导致在噪声或冗余检索条件下表现脆弱。

Method: 将证据选择建模为token约束下的修复问题，结合实体中心间隙追踪、针对性微查询生成以及基于效用的选择机制（兼顾间隙覆盖、佐证、新颖性、冗余度和直接问题相关性）。

Result: 在HotpotQA的干净、冗余和噪声三种检索条件下，AdaGATE均取得最佳证据F1（干净数据62.3%，冗余注入71.2%），且输入token数比Adaptive-k少2.6倍。

Conclusion: 显式的间隙感知修复结合token高效的证据选择，能够提升多跳RAG在不完美检索下的鲁棒性。
...
TL;DR: 本研究探讨了学生在生成式AI背景下写作中运用反驳论证来培养批判性思维，并验证了使用前沿大语言模型根据已有评分标准评估学生作文的可行性。结果显示学生能通过反驳AI生成内容展现逻辑等批判性思维要素，且AI评估与人类评估基本一致（Gwet's AC2=0.33）。


<details>
  <summary>Details</summary>
Motivation: 在生成式AI普及的背景下，学生存在作弊和认知卸载的风险，因此有必要研究如何在写作中利用AI工具促进而非削弱学生的批判性思维。

Method: 对36名大学生进行干预实验，使用4个辩论主题的论文陈述，收集35份合格作文。采用6个评分标准（焦点、逻辑、内容、风格、正确性、参考文献），由两名同伴评审和一名经验教师进行5点李克特量表评分，同时使用6种前沿大语言模型作为评审进行对比，结合定性开放式反馈和定量分析。

Result: 学生自行撰写的反驳AI生成内容的论证体现了逻辑等批判性思维要素；除一个模型外，AI评估与人类评估具有一致性（Gwet's AC2=0.33）。

Conclusion: 基于清晰评分标准，生成式AI可以大规模成功评估学生写作，评估结果与人类评审基本一致，同时反驳论证写作有助于培养学生的批判性思维。
...
TL;DR: 本文提出了一种基于证据的查询生成模型，能够从无查询数据集中自动生成查询关键词，并用于查询聚焦摘要任务，实验表明生成的查询能取得与原始查询相竞争的ROUGE分数。


<details>
  <summary>Details</summary>
Motivation: 大规模摘要数据集通常不包含查询，而查询聚焦摘要（QFS）任务需要查询信息，因此需要研究如何从无查询数据集中自动生成基于证据的查询关键词。

Method: 提出一种基于证据的查询生成模型，从无查询数据集中生成查询；通过内在评估（比较与原始查询的相似度）和外在评估（使用不同预训练模型及SOTA QFS模型进行摘要生成）来评价模型性能。

Result: 基于证据生成的查询在摘要任务中取得了与原始查询相竞争的ROUGE分数。

Conclusion: 基于证据的查询生成方法可以有效地从无查询数据集中生成查询，并支持查询聚焦摘要任务。
...
TL;DR: SLAM是一种新型白盒水印方案，通过在结构几何（而非token频率）中嵌入标记，在Gemma-2 2B/9B上实现100%检测准确率，仅需1-2奖励点的质量成本，远低于KGW、EWD、Unigram等方法（7.5-11.5点），同时保持自然度和多样性接近无水印水平。


<details>
  <summary>Details</summary>
Motivation: 现有LLM水印方案通过在next-token分布中引入偏差来嵌入水印，导致可检测的质量损失。作者希望在不牺牲文本质量的前提下实现可靠的水印检测。

Method: 利用稀疏自编码器识别残差流中编码语言结构（如语态、时态、从句顺序）的方向，在生成时对这些方向进行因果干预，将水印写入结构几何而非token频率，不约束词汇采样和语义。

Result: 在Gemma-2 2B和9B上，SLAM实现100%检测准确率，质量成本仅1-2奖励点（对比方法为7.5-11.5），自然度和多样性保持接近无水印水平。对词级编辑有鲁棒性，但对改写句法的释义攻击脆弱。

Conclusion: SLAM通过将水印嵌入语言结构几何而非token分布，在保持高质量的同时实现可靠检测，与token分布方法形成互补的鲁棒性特征。
...
TL;DR: 本文研究了扩散语言模型的水印在多次重写攻击下的脆弱性。原始水印检测率87.9%，经一次重写后降至14%-41%，五次链式重写后仅剩4.86%，94.76%的原检测文本不再被标记。


<details>
  <summary>Details</summary>
Motivation: 现有水印方案主要针对自回归生成模型设计，而扩散语言模型（如LLaDA）的任意顺序去噪生成方式不适用传统方案。虽有新方案（Gloaguen等）声称对单次生成有效，但实际场景中文本可能被多次重写，需要评估水印在多次重写下的鲁棒性。

Method: 使用相同水印配置，在5个WaterBench领域生成1,605个约300词的水印文本。用4个开源模型（1.5B-8B参数）进行5种风格（释义、人性化、简化、学术、摘要扩展）的重写，每种风格最多链式重写5次，共生成160,500个重写文本。

Result: 原始输出检测率87.9%；一次重写后降至14%-41%（取决于重写模型和风格）；五次链式重写后降至4.86%，即94.76%的原检测文本不再被标记；三次重写后检测器得分已向零分布下降86%。

Conclusion: 多次重写是比单次重写强得多的攻击手段，该结论在四个重写器上均成立。当前扩散语言模型的水印方案在多次重写下鲁棒性严重不足。
...
TL;DR: 本文评估了领域专用的小型语言模型（SLM）Olava Extract在法律合同结构化提取任务中，以极低成本超越前沿大型语言模型（LLM）的表现。Olava Extract在宏观F1（0.812）和微观F1（0.842）上均取得最佳成绩，同时推理成本降低78%-97%，并减少了幻觉输出。研究表明，高性能法律AI不再需要最大型的外部托管模型，挑战了企业AI能力必须依赖更大模型和巨额基础设施投入的假设。


<details>
  <summary>Details</summary>
Motivation: 探究领域专用的小型语言模型（SLM）能否在结构化合同提取任务中以极低成本超越前沿大型语言模型，挑战企业AI能力必须依赖更大模型和巨额基础设施投入的主流假设。

Method: 使用自托管的法律领域混合专家模型Olava Extract，与五款前沿模型进行对比测试，在结构化合同提取任务上评估性能与成本。

Result: Olava Extract在宏观F1（0.812）和微观F1（0.842）上取得最强聚合表现，推理成本降低78%-97%，同时获得最高精确度，产生更少的幻觉和未支持提取。

Conclusion: 高性能、可媲美人类的法律AI不再需要最大型的外部托管模型；企业AI能力不必依赖于越来越大的模型、巨额基础设施支出和集中托管的服务商。
...
TL;DR: 本文发现MLLM在使用RAG时会出现“再腐化”现象——即使引入正确的外部文档，模型反而会放弃原本正确的预测。通过注意力机制分析，提出了BAIR方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM集成RAG虽能缓解幻觉，但引入外部文档可能隐藏严重的实例级失效模式，需要识别并解决这种“再腐化”现象。

Method: 通过注意力矩阵的机械诊断，揭示再腐化由注意力坍塌驱动：(1)视觉盲区——视觉注意力质量和锐度被系统性抑制；(2)位置偏差——模型优先关注边界token而非语义相关性。提出BAIR方法，通过恢复视觉显著性并施加位置感知惩罚来修复。

Result: 在医学事实性、社会公平性和地理空间基准测试中，BAIR成功恢复了多模态基础，提高了诊断可靠性。

Conclusion: BAIR作为一种无参数、推理时框架，无需模型重训练或微调即能有效缓解RAG中的再腐化问题。
...
TL;DR: 本文提出When2Speak数据集和四阶段生成流程，用于训练LLM在多人群聊中判断何时发言。通过监督微调和强化学习，显著提升了干预时机的把握能力，将召回率从0.479提升至0.78-0.81，但SFT模型仍存在过度保守问题。


<details>
  <summary>Details</summary>
Motivation: LLM在多轮对话中虽然能生成恰当回复，但缺乏判断何时该发言的能力，容易过度打断或沉默，影响对话连贯性。现有研究多关注说什么，忽略了什么时候说的关键性。

Method: 构建When2Speak数据集（21.5万样本，1.6万对话，2-6人），采用四阶段生成流程：真实对话锚定、结构化增强、受控转录合成、微调就绪标注。先使用SFT微调，再引入非对称奖励形状的强化学习优化干预时机。

Result: SFT使4B+参数模型的Macro F1平均提升60%（最高120%），但模型过度保守，漏报率（MIR）约0.50。强化学习将MIR降至0.186-0.218，召回率从0.479提升至0.78-0.81。

Conclusion: 时序参与是对话智能中一个可独立训练的能力维度，基于真实场景的合成数据为LLM在多人群聊中更自然恰当地参与提供了有效且可扩展的路径。
...
TL;DR: 本文提出TurnGate，一种用于多轮对话中检测隐藏恶意意图的按轮次监控方法，能够在最早的危险使能轮次进行干预，并构建了MTID数据集以支持训练和评估。


<details>
  <summary>Details</summary>
Motivation: 多轮对话中恶意意图被分散在多个看似无害的轮次中，现有安全对齐和外部防护对这类攻击防御不足，亟需精准的按轮次检测机制。

Method: 提出TurnGate，通过检测最早的危险使能轮次（delivery of candidate response使累积交互足以执行有害行为的轮次）进行精确干预；同时构建MTID数据集，包含分支攻击展开、匹配的良性硬负样本以及最早危险使能轮次的标注。

Result: TurnGate在有害意图检测上大幅超越现有基线，同时保持较低的错误拒绝率，并能在不同领域、攻击流程和目标模型上泛化。

Conclusion: TurnGate通过按轮次监控有效识别多轮对话中的隐藏恶意意图，MTID数据集为相关训练和评估提供了重要资源。
...
TL;DR: 本文提出XL-SafetyBench，一个包含5,500个测试用例、覆盖10个国家-语言对的跨文化安全基准，并引入NSR和CSR两个新指标，发现前沿模型的安全鲁棒性与文化感知无耦合关系，而本地模型表现出近似的ASR-NSR权衡（r=-0.81），表明其安全表现源于生成失败而非真正对齐。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全基准主要基于英语并依赖翻译，无法捕捉特定国家的危害，也缺乏评估模型检测文化嵌入敏感性（区别于普遍危害）的能力。

Method: 构建了5,500个测试用例（含越狱基准和文化基准），采用多阶段流水线（LLM辅助发现+自动验证+双独立母语标注者）；提出攻击成功率（ASR）、中性安全率（NSR）和文化敏感性率（CSR）三个指标。

Result: 前沿模型的越狱鲁棒性与文化意识无耦合关系；本地模型的ASR与NSR呈近线性负相关（r=-0.81），说明其安全性源于生成失败。

Conclusion: XL-SafetyBench实现了更细致、跨文化的安全评估，揭示了现有安全测评中忽视的维度差异。
...
TL;DR: 提出BADIT方法，将LLM参数分解为正交的基本能力模块，通过球面聚类保持正交性，以解决多任务指令微调中的跨任务干扰问题，在SuperNI基准上6个LLM上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多任务指令微调存在跨任务干扰（conflicting gradients），现有方法（任务特定神经元选择、混合专家）仍因大量共享参数而无法彻底解决该问题。

Method: 1) 发现参数存在共激活现象，自然形成基组；2) 将LLM参数分解为正交的高奇异值LoRA专家，代表基本能力；3) 通过秩1分量的球面聚类动态保持正交性；4) 任意任务表示为这些基本能力的线性组合。

Result: 在SuperNI基准上对6个LLM进行实验，BADIT优于SOTA方法，有效缓解了跨任务干扰程度。

Conclusion: LLM编码若干正交的基本能力，通过分解参数为正交LoRA专家并保持正交性，可有效缓解跨任务干扰，提升多任务指令微调性能。
...
TL;DR: BioTool是一个专为生物医学领域设计的工具调用数据集，包含34个常用生物信息学工具和7,040个高质量查询-API调用对。通过微调4B参数的LLM，在生物医学工具调用上超越了GPT-5.1等商业模型，并能显著提升下游回答质量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在生物医学等专业领域表现不佳，无法有效利用生物医学工具；而现有生物医学工具调用研究多依赖上下文学习且工具集很小，缺乏专门用于微调的高质量数据集。

Method: 收集了来自NCBI、Ensembl、UniProt等数据库的34个常用工具，构建了7,040个人工验证的查询-API调用对，涵盖变异、基因组学、蛋白质组学、进化和普通生物学领域，用于微调LLM。

Result: 在4B参数LLM上微调后，BioTool在生物医学工具调用性能上超越了GPT-5.1等前沿商业LLM。人工评估表明，集成BioTool微调的工具调用器能显著提升下游答案质量。

Conclusion: BioTool有效填补了生物医学领域缺乏高质量工具调用数据集的空白，通过微调显著增强了LLM在生物医学领域的工具使用能力和下游任务表现。
...
TL;DR: 本文研究了大型推理模型（LRMs）在思维链（CoT）中表达的评估意识（VEA）是否会导致其策略性调整输出。实验发现，无论注入或移除VEA，对模型行为的影响都非常有限（效应量ω≤0.31），表明评估意识可能并不像当前文献所担心的那样构成重大安全风险。


<details>
  <summary>Details</summary>
Motivation: 研究者担心LRMs在CoT中意识到自己正在被评估（VEA）后，会策略性地调整输出以迎合评估标准，例如让模型显得比实际更安全。然而，这种效应是否真实存在尚不清楚，因此本文旨在实证检验VEA对模型行为的影响程度。

Method: 在开放权重的LRMs上，覆盖安全、对齐、道德推理和政治观点等多个基准。采用两种测试方式：1) on-policy：对每个项目采样多个CoT，比较自发包含VEA与不包含VEA的情况；2) off-policy：通过模型预填充（prefilling）注入或移除评估意识句子，然后重新采样。

Result: VEA对模型行为影响有限：注入VEA几乎无效果（ω≤0.06），移除VEA产生微小偏移（ω≤0.12），自发VEA最多使答案分布偏移3.7个百分点（ω≤0.31）。

Conclusion: 研究结果呼吁谨慎解读高VEA率作为策略行为或对齐篡改的证据，评估意识可能比当前文献假设的安全风险更小。
...
TL;DR: 本文提出FLAS（基于流的激活引导方法），通过学习概念条件的速度场来引导激活变化，在AxBench基准上首次以学习方法持续超越上下文提示方法，并揭示了激活空间中的弯曲、多步、位置变化轨迹，挑战了先前假设。


<details>
  <summary>Details</summary>
Motivation: 现有激活引导方法在大型评估中表现不如简单的上下文提示方法，且对未见概念的泛化能力较差。作者认为这是因为现有方法共享了未经验证的简化假设（如固定、单步、位置不变的变换），因此需要一种更通用的方法。

Method: 提出FLAS（Flow-based Activation Steering），学习一个概念条件的速度场 v_t(h,t,c)，该场将未引导的激活传输为引导后的激活，不依赖于固定、单步、位置不变等简化假设。

Result: 在AxBench上，FLAS是首个持续优于上下文提示的学习方法，在Gemma-2-2B-IT和Gemma-2-9B-IT上分别达到1.015和1.113的留出调和平均值，且无需针对每个概念进行微调。

Conclusion: 学习到的流展现了弯曲、多步、位置变化的轨迹，表明先前关于激活空间几何的假设可能是不完整的，为未来激活引导研究提供了新方向。
...
TL;DR: 提出LOVER，一种基于逻辑规则的无监督验证器，通过将验证器视为二元潜变量并施加三种逻辑约束（否定一致性、组内一致性、组间一致性），无需标注数据即可提升LLM推理能力，在10个数据集上达到监督方法的95%性能。


<details>
  <summary>Details</summary>
Motivation: 传统验证器依赖监督数据集构建，成本高昂且数据多样性受限，亟需一种无需标注数据的无监督验证方法。

Method: 将验证器建模为二元潜变量，利用内部激活，在多个推理路径上施加三种逻辑约束：否定一致性、按最终答案分组的组内一致性和组间一致性。

Result: 在10个数据集上显著优于无监督基线，性能达到监督验证器的平均95%。

Conclusion: 逻辑规则作为先验的无监督验证器能有效利用未标注样本，且可直接兼容任意现成LLM，是一种低成本高性能的替代方案。
...
TL;DR: 提出了一种轻量级风格一致性分析方法LiSCP，通过结合离散风格特征与连续语义信号，实现对LLM生成文本的鲁棒检测，在跨域和对抗场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有检测器依赖统计线索或模型特定启发式方法，易受改写和对抗性操纵影响，鲁棒性和可解释性不足，亟需更稳健的检测方法。

Method: 构建风格一致性画像，结合离散风格特征与连续语义信号，利用跨模态改写文本变体的风格稳定性进行检测。

Result: 在多媒体新闻、电影数据集及传统文本领域，跨域设置下性能提升高达11.79%，且在对抗攻击和混合人机场景下表现出强鲁棒性。

Conclusion: LiSCP是一种轻量级、鲁棒且可解释的LLM生成文本检测方法，能有效应对对抗性操纵和跨域挑战。
...
TL;DR: 本文提出PCNET和PC-LDCD方法，通过概率电路检测LLM幻觉，在不破坏正确生成的前提下实现高精度幻觉检测与修正，AUROC最高达99%，并将平均破坏率降低至53.7%。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉校正方法对所有token无差别修正，会破坏原本正确的生成内容。本文旨在解决这一关键局限，实现针对性的幻觉检测与校正。

Method: 将概率电路（PCNET）作为可追踪密度估计器，对LLM残差流进行精确负对数似然计算，将幻觉检测视为事实流形上的几何异常检测。在此基础上提出PC-LDCD，仅在潜在几何偏离事实区域时触发对比解码，保留正确生成。

Result: 在CoQA、SQuAD v2.0、TriviaQA上AUROC最高达99%；TruthfulQA上三款模型取得最高True+Info、MC2、MC3分数；平均破坏率降至53.7%，保留率达79.3%。

Conclusion: PCNET和PC-LDCD在不修改模型权重、无需外部验证器的情况下，有效实现了幻觉的精准检测与选择性校正，显著降低了对正确生成内容的破坏。
...
TL;DR: 提出了TableVista基准，包含3000个表格推理问题，每个扩展为10种视觉变体（共30,000样本），用于评估29个前沿模型在多模态表格推理中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对视觉和结构复杂性的系统评估，无法全面检测多模态模型在表格推理中的鲁棒性和一致性。

Method: 构建3000个高质量表格推理问题，通过多风格渲染与变换pipeline为每个实例生成10种视觉变体（含场景风格、鲁棒性扰动、纯视觉配置），最终形成30,000个多模态样本。

Result: 评估的29个模型在不同渲染风格下表现稳定，但在复杂结构布局和纯视觉设置下性能显著下降，表明模型在结构复杂性与视觉呈现结合时推理一致性不足。

Conclusion: 揭示了当前多模态模型在表格理解中的关键缺陷，为开发更鲁棒可靠的表格理解模型提供了方向。
...
TL;DR: 本文构建了包含9,688条记录的鞑靼斯坦多语言地名数据集及约39,000个问答三元组，提出结合稠密语义检索（multilingual-e5-large）与地理空间过滤（KD树+haversine距离）的混合检索器，采用XLM-RoBERTa-large作为阅读理解模型，在500条测试查询上Recall@1=0.988、EM=0.992，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 多语言地理空间问答面临地名数据多语言异构、空间查询与语义理解难以融合的挑战，现有数据集和模型在多语言地名问答场景下表现不足。

Method: 1）构建鞑靼斯坦共和国多语言地名数据集（9,688条结构化记录）；2）生成约39,000个问答三元组；3）提出混合检索器：稠密语义检索（multilingual-e5-large）+ 地理空间过滤（KD树+haversine距离）；4）对比多种阅读理解架构（RuBERT、XLM-RoBERTa-large、T5-RUS）。

Result: 混合检索Recall@1=0.988、MRR=0.994；XLM-RoBERTa-large阅读理解EM=0.992、F1=0.994；RuBERT在坐标问题上F1=0，经后处理可恢复至100%。

Conclusion: 该方法有效融合语义与空间检索，可应用于多语言地区的地理空间问答、地理编码和数字人文领域，全部资源已在Hugging Face开源。
...
TL;DR: 本文提出PrimeFacts方法，从完整事实核查文章中提取细粒度证据，利用LLM将锚点句子重写为独立前提，显著提升了证据检索和事实验证性能。


<details>
  <summary>Details</summary>
Motivation: 事实核查文章中蕴含丰富的支撑证据和推理过程，但这些证据因非结构化呈现而难以被自动化验证系统有效利用。

Method: 收集13,106篇PolitiFact文章，识别49,718个内部超链接作为自然锚点，利用大语言模型将锚点句子重写为上下文独立的去语境化前提，并探索提取额外隐含证据。

Result: 去语境化证据在跨文章证据检索中MRR相对提升30%，在事实验证中Macro-F1提升10-20个百分点，且在不同粒度（2类/5类）和模型架构下均表现一致。

Conclusion: PrimeFacts展示了复用事实核查员证据用于自动化验证的潜力，并提供了大规模结构化事实核查证据资源。
...
TL;DR: 该研究通过HPSG形式语法框架，比较了不同代LLM生成的英文新闻文本与人类撰写的《纽约时报》文章在句法结构和词汇类型分布上的差异，发现较新的指令微调模型在句法和词汇多样性上较旧模型有所降低。


<details>
  <summary>Details</summary>
Motivation: 已有大量研究比较LLM生成文本与人类文本，但多数关注语义层面，缺乏对形式语法框架下句法特性的系统评估。本文希望填补这一空白。

Method: 采用HPSG形式语法框架，分析句法结构和词汇类型的分布；利用生态学和信息论中的多样性指标（如多样性度量）进行量化比较；对比两代LLM与两个年份的人类撰写NYT新闻数据。

Result: 人类英文新闻文本在给定时间范围内变化很小；较新的指令微调LLM相比旧的非指令微调模型，句法多样性有所降低，词汇多样性下降尤为显著。

Conclusion: 指令微调虽提升了模型的连贯性和提示遵循能力，但可能缩小了模型输出的表达范围，未来需进一步研究指令微调对语言多样性的影响。
...
TL;DR: 本文质疑了“定位-更新”范式在LLM后训练中的核心假设——当前静态机制能否指导未来动态参数更新。通过追踪SFT过程中Transformer电路的演化，发现电路存在“自由演化”特性，导致静态机制存在时间滞后性，无法有效指导未来状态。


<details>
  <summary>Details</summary>
Motivation: 挑战“定位-更新”范式中一个未经验证的根本假设：基于当前静态参数推导出的机制是否能够可靠地指导未来的动态参数更新？

Method: 系统追踪SFT过程中Transformer电路的结构演化，引入三个新指标：电路距离（神经迁移）、电路稳定性（语义稳定性）和电路冲突（跨任务干扰），从三个维度分析电路演化动力学。

Result: 发现电路在参数更新过程中固有地表现出“自由演化”特性；静态机制存在时间滞后性，无法充分指导未来状态；揭示了现有方法“有效性幻觉”的背后原因。

Conclusion: 强调了在机制定位中需要具备“前瞻性”，并提出了一个预测性框架作为未来研究方向。
...
TL;DR: BEACON是一个基于里程碑引导的策略学习框架，通过精确的信用分配和双尺度优势估计，有效解决了长序列语言智能体训练中的信用误归因和样本效率低下问题，在ALFWorld等任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 长序列智能体任务需要语言模型进行数十步顺序决策，但现有的强化学习方法面临两个根本问题：1) 信用误归因——正确的早期动作因最终失败而被错误惩罚；2) 样本效率低下——成功轨迹稀缺导致学习信号几乎完全丢失。

Method: 提出BEACON框架，利用长序列任务的组合结构：在里程碑边界划分轨迹，在片段内进行时序奖励塑造以奖励局部进展，并通过双尺度优势估计防止远端失败污染局部动作的评估。

Result: 在ALFWorld、WebShop和ScienceWorld上一致优于GRPO和GiGPO。在长序列ALFWorld任务中，BEACON成功率达92.9%（GRPO仅53.5%），有效样本利用率从23.7%提升至82.0%。
...
TL;DR: 本文发现多模态知识编辑后的模型存在“实体身份混淆”（EIC）问题：编辑后，仅用文本查询原实体身份时，模型会错误返回新实体信息。作者构建了EC-Bench基准，揭示问题根源在于现有方法未能区分图像-实体绑定与实体-实体关系知识，导致模型将实体-实体关联作为捷径过度拟合。通过约束编辑到图像-实体处理阶段可缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 多模态知识编辑（MKE）旨在修正大视觉语言模型中的内部知识，但编辑后模型的行为模式尚未被充分探索。作者发现编辑后的模型存在系统性失败模式，即实体身份混淆（EIC），需要对此进行深入研究和诊断。

Method: 构建EC-Bench诊断基准，系统探究编辑前后图像-实体绑定的变化。通过对比分析揭示EIC的产生原因，并提出通过在图像-实体处理阶段进行约束编辑来缓解EIC的改进策略。

Result: EIC源于现有方法无法区分图像-实体绑定知识和实体-实体关系知识，模型过度依赖实体-实体关联作为捷径。约束编辑到图像-实体处理阶段可显著减少EIC。
...
TL;DR: 本文提出MemReranker系列模型，通过多阶段LLM知识蒸馏解决记忆系统中重排序模型缺乏推理能力的问题。0.6B和4B参数版本在记忆检索基准上超越BGE-Reranker，匹配GPT-4o-mini和Gemini-3-Flash等大模型性能，推理延迟仅为大模型的10-20%。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统中的重排序模型依赖语义相似度匹配，缺乏真正推理能力，导致检索结果语义相关但缺失回答问题的关键信息，具体表现为分数校准不当、时序/因果等复杂查询排序退化、无法利用对话上下文消歧。

Method: 基于Qwen3-Reranker进行多阶段LLM知识蒸馏，包括多教师成对比较生成校准软标签、BCE逐点蒸馏建立分布良好的分数、InfoNCE对比学习增强难样本区分能力。训练数据结合通用语料和覆盖时序约束、因果推理、共指消解的记忆专用多轮对话数据。

Result: MemReranker-0.6B显著超越BGE-Reranker，匹配开源4B/8B模型和GPT-4o-mini。MemReranker-4B达到0.737 MAP，多项指标与Gemini-3-Flash相当，推理延迟仅为大模型10-20%。在金融和医疗垂直领域保持与主流大参数重排序器相当的泛化能力。

Conclusion: 通过知识蒸馏可以在极小参数规模下实现与大模型相当的记忆检索性能，同时保持极低推理延迟和领域泛化能力，为智能体的长期记忆系统提供高效实用的重排序方案。
...
TL;DR: IRC-Bench是一个评估回忆叙事中隐式实体识别能力的新基准，包含25,136个样本和19种模型配置的评测结果。


<details>
  <summary>Details</summary>
Motivation: 人们在回忆个人经历时往往通过上下文线索而非明确名称来指代人、地点和事件，这种隐式引用在治疗、档案和社交场景中很常见，但现有NLP方法（如命名实体识别、实体链接、共指消解）无法处理跨越多个非连续从句的分布式线索，因此需要专门的基准来评估这一能力。

Method: 构建了IRC-Bench基准，包含来自1,994篇回忆录、涵盖11个主题领域的25,136个样本，每个样本由显式提及实体的叙述和移除直接提及的隐式版本配对组成。评估了19种配置，包括LLM生成、密集检索、RAG和微调方法。

Result: QLoRA微调的Llama 3.1 8B在开放世界设定中表现最佳（精确匹配38.94%，Jaccard相似度51.59%）；微调DPR在封闭世界检索中领先（Hit@1 35.38%，Hit@10 71.49%）。

Conclusion: 隐式回忆语境中的实体识别是一个具有挑战性的计算问题，现有模型性能有限，IRC-Bench为这一方向提供了系统性的评估框架。
...
TL;DR: 本文提出A²TGPO方法，通过回合组归一化、方差重缩放折扣累积和自适应回合级裁剪三个改进，解决基于信息增益的内在过程信号在强化学习训练中的标准化、累积和消费三大挑战，从而更有效地为智能体大语言模型的多轮工具调用分配过程信用。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在智能体LLM的多轮交互中，面临细粒度过程信用分配的难题：稀疏的轨迹级结果奖励难以评估单个工具调用的贡献，而依赖外部过程奖励模型或树状结构展开的方法存在额外消耗或限制轨迹多样性的问题。基于信息增益的内在信号虽有潜力，但在RL训练中存在标准化混乱、优势幅度漂移和固定裁剪范围不当三大系统性挑战。

Method: 提出A²TGPO方法，包含三个核心改进：(i) 回合组归一化——在同一(prompt, 回合索引)组内归一化IG，使各回合与同交互深度的同行比较；(ii) 方差重缩放折扣累积——用累积项数的平方根除累积归一化IG，使优势幅度在不同回合位置间可比；(iii) 自适应回合级裁剪——根据归一化IG动态调整裁剪范围，对信息量大的回合放宽更新区域，对信息量少的回合收紧。

Result: 该方法保留了IG作为内在信号的优势（无需外部评估器），同时解决了标准化、累积和消费三个关键问题，实现了更合理的过程信用分配。

Conclusion: A²TGPO通过重新设计信息增益信号的归一化、累积和使用方式，有效克服了现有方法在智能体LLM多轮交互中过程信用分配的三大挑战，为基于内在信号的过程强化学习提供了更稳健的解决方案。
...
TL;DR: 本文指出现代LLM中普遍存在但未经审视的设计缺陷——词元索引仅在输入嵌入层查找一次后永久丢弃（单次注入假设），导致稀有词元训练不足和上下文坍塌两个问题，并提出TIDE方法，通过嵌入记忆模块在每层注入语义向量来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 指出现代LLM中普遍存在的"单次注入假设"（token索引仅在输入嵌入层查找一次后永久丢弃）这一未被充分审视的设计选择，会导致两个结构性问题：稀有词元问题和上下文坍塌问题。

Method: 提出TIDE方法，在标准Transformer基础上增加EmbeddingMemory：一个由K个独立MemoryBlock组成的集成模块，将词元索引映射为上下文无关的语义向量，通过深度条件化的softmax路由器和可学习的空库（null bank）计算，注入到每一层中。

Result: 理论和实验均表明TIDE能够解决单次词元身份注入带来的问题，并在多个语言建模和下游任务上提升性能。

Conclusion: TIDE通过打破单次注入假设，在每一层持续注入词元语义信息，有效缓解了稀有词元训练不足和上下文坍塌问题，提升了模型性能。
...
TL;DR: 提出UniPrefill框架，可在多种模型架构（包括混合架构）上实现预填充加速，支持连续批处理，无缝集成vLLM，TTFT加速最高达2.1倍。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文预填充加速方法主要针对全注意力模型，在新型混合架构（如线性/全注意力混合、滑动窗口/全注意力混合）上性能显著下降，且不兼容连续批处理，难以集成到现代推理引擎中。

Method: 提出UniPrefill框架，直接在Token级别加速模型计算；将其实现为连续批处理算子；扩展vLLM调度策略以原生支持预填充-解码协同处理和张量并行。

Result: TTFT加速最高达2.1倍，且加速效果随并发请求数量增加而更加显著。

Conclusion: UniPrefill是一种通用的预填充加速框架，适用于几乎所有模型架构，并能无缝集成到现代推理引擎中，有效提升长上下文推理效率。
...
TL;DR: 本文提出了一个异构集成多语言预训练模型（XLM-RoBERTa-large + mDeBERTa-v3-base），用于SemEval-2026任务9中的多语言、多文化、多事件在线极化检测，发现独立任务建模结合类别加权效果优于多任务学习。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中多语言极化内容检测面临标签不平衡、跨语言泛化等挑战，现有方法难以同时处理22种语言的三个子任务。

Method: 采用异构集成方法，融合XLM-RoBERTa-large和mDeBERTa-v3-base两个多语言预训练模型，并探索多任务学习、翻译数据增强和类别加权等技术。

Result: 独立任务建模结合类别加权的方法表现更优，优于多任务学习策略。

Conclusion: 对于多语言极化检测，独立处理各子任务并配合类别加权是更有效的方案。
...
TL;DR: 本文发现强化学习并不教会大模型新策略，只是在模型已有知识中进行稀疏的、可预测的概率再分配（仅影响1-3%的token位置）。提出ReasonMaxxer方法，通过仅在熵较高的决策点应用对比损失，无需完整RL流程，仅需少量问题和分钟级单GPU训练，即可达到甚至超越完整RL的性能，训练成本降低约三个数量级。


<details>
  <summary>Details</summary>
Motivation: 探究强化学习优化大语言模型推理能力的本质机制——RL是否真的教会了新策略，还是仅仅在已有知识中进行概率重分配？若是后者，RL的优化循环本身是否必要？

Method: 通过token级别的分析，发现RL的效果集中在高熵决策点（模型不确定选择哪个分支的位置），且修正的token始终在base模型top-5候选内。基于此提出ReasonMaxxer：仅在熵门控的决策点应用对比损失，使用几百条base模型 rollout和无在线生成，无需完整RL流程。

Result: 在三个模型族、六种规模、六个数学推理基准上，ReasonMaxxer匹配或超越完整RL性能，仅需数十个问题和分钟级单GPU训练，训练成本降低约三个数量级。

Conclusion: 推理能力的提升应被重新定义为稀疏策略选择（sparse policy selection），而非能力获取。RL的收益本质上来自对模型已有知识中少数不确定点的定向修正，这一过程可以在不进行完整RL优化的前提下高效实现。
...
TL;DR: 本文针对阿拉伯语方言会话中的语义分割任务，构建了首个多体裁基准数据集（1000+样本），并提出了一种关注局部语义连贯性和话语不连续鲁棒性的分割模型，在方言非新闻体裁上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有语义分割模型主要针对高资源书面文本（如现代标准阿拉伯语新闻）开发，在低资源口语方言（如阿拉伯语方言）上效果不佳。方言阿拉伯语存在非正式句法、语码混用和弱标记话语结构等挑战，亟需专门的基准和方法。

Method: 1) 构建包含电话对话、语码混用播客、广播新闻和小说对话的多体裁方言阿拉伯语数据集（1000+样本），由母语者标注；2) 提出一种关注局部语义连贯性并对话语不连续性具有鲁棒性的分割模型。

Result: 在MSA新闻上表现良好的分割模型在方言转录语音上性能下降；所提模型在方言非新闻体裁上持续优于强基线方法，并泛化到其他低资源口语语言。

Conclusion: 方言口语语义分割需要专门基准和针对局部语义连贯性优化的模型，所提方法可推广到其他低资源口语语言。
...
TL;DR: 本文研究了评分标准（rubric）的修改如何影响人类与AI自动评分者（LLM-as-judges）之间的一致性，发现提供示例、减少位置偏差能提升一致性，而更高的评分标准复杂度和保守的聚合方法会降低一致性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为自动评分者被广泛用于评估和内容审核，但评分标准的修改对人类与自动评分者之间一致性的影响缺乏系统的统计分析，且整体性（holistic）与分析性（analytic）评判方式对此关系的影响尚不明确。

Method: 通过统计分析方法，研究修改评分标准（如提供代表性示例、增加上下文、减少位置偏差、调整复杂度及聚合方法）对人类与AI自动评分者之间评分一致性（agreement）的影响，涵盖自动作文评分和指令遵循评估两个领域。

Result: 提供代表性示例和额外上下文、减少位置偏差的评分标准修改能提升人类与自动评分者的一致性；而更高的评分标准复杂度和保守的聚合方法则倾向于降低一致性。

Conclusion: 在实践中，开发者和研究者应仔细分析特定领域和特定评分标准的性能，以提升人类与自动评分者之间的一致性，不能简单地认为修改评分标准就能同时提高两方的准确性。
...
TL;DR: LatentRAG提出将推理和检索从离散语言空间转移到连续潜在空间，通过单次前向传播生成潜在令牌，在保持与显式Agentic RAG相当性能的同时，将推理延迟降低约90%。


<details>
  <summary>Details</summary>
Motivation: 现有Agentic RAG通过多步迭代过程处理复杂问题，但LLM自回归生成长篇思路和子查询导致显著延迟，亟需一种高效的方法来降低延迟同时保持性能。

Method: 将推理和检索从离散语言空间转移到连续潜在空间；直接从前向传播的隐状态生成潜在令牌，而非逐词生成；在潜在空间中对齐LLM与稠密检索模型；引入并行潜在解码机制将潜在令牌翻译回自然语言以提升可解释性。

Result: 在七个基准数据集上，LatentRAG达到与显式Agentic RAG方法相当的性能，同时推理延迟降低约90%，显著缩小了与传统单步RAG的延迟差距。

Conclusion: LatentRAG通过在连续潜在空间中进行推理和检索，有效解决了Agentic RAG的高延迟问题，为复杂问答任务提供了高效且性能优异的解决方案。
...
TL;DR: 本文指出现有基于似然假设的机器文本检测器存在辛普森悖论问题：在隐藏空间中对不同区域进行简单平均会破坏局部强信号。为此提出基于贝叶斯决策理论的局部校准方法，通过学习每个隐藏空间位置的得分分布来替代原始分数平均，显著提升检测性能（如Fast-DetectGPT在GPT-5.4文本上AUROC从0.63提升至0.85），并实现了新的SOTA检测器DMAP。


<details>
  <summary>Details</summary>
Motivation: 现有机器文本检测器基于似然假设（机器生成文本对检测模型来说概率更高），但本文发现token级信号在检测模型的隐藏空间中分布不均匀，对不同统计结构的区域进行简单平均会导致辛普森悖论——强局部信号被不当聚合所破坏。

Method: 基于贝叶斯决策理论引入局部校准步骤：不是直接聚合原始token分数，而是先学习轻量级预测器，估计每个隐藏空间位置条件下的分数分布，然后聚合校准后的对数似然比。

Result: 该方法显著且一致地提升了所有基线检测器和所有数据集的性能。例如，校准后的Fast-DetectGPT在GPT-5.4文本上AUROC从0.63提升到0.85；新提出的局部校准DMAP检测器在各数据集上达到SOTA。

Conclusion: 核心贡献不在于提出新检测器，而在于精准诊断了现有检测器性能不佳的重要原因（辛普森悖论），并提供了一个原则上与任何token平均化管线兼容的模块化修复方案，为社区后续改进奠定了理论基础。
...
TL;DR: 提出了一种基于无监督异常检测的多语言笑声分割方法，使用 Isolation Forest 和 BYOL-A 编码器，在多语言场景下优于现有方法。
...
Method: 将笑声分割视为基于能量分割音频序列的异常检测任务，使用 Isolation Forest 对 BYOL-A 编码器学到的音频表示进行分析。

Result: 在四个数据集上与多个SOTA方法对比，现有方法在多语言环境下表现不佳，而本方法在非英语场景中表现更优。

Conclusion: 提出的无监督多语言笑声检测方法有效克服了语言依赖性问题，为非英语环境下的笑声分割提供了可行方案。
...
TL;DR: 本文首次大规模分析了四个有害语言检测数据集，发现标注者特征、语言属性及其交互作用对标注差异至关重要，但不同数据集间效应模式差异显著，需警惕泛化与迁移。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注"谁"在标注（如社会人口学特征、态度等），而忽视了"什么"被标注（如语言属性）以及两者的交互作用。本文旨在填补这一空白。

Method: 对四个有害语言检测参考数据集进行大规模统计分析，整合标注者特征、语言属性及其交互作用，采用统计建模方法揭示其影响。

Result: 1. 交互作用至关重要，揭示了以往被忽略的交叠效应；2. 词汇线索和标注者态度起显著作用；3. 不同数据集间的效应模式差异显著。

Conclusion: 交互效应是理解标注差异的关键，但不同数据集之间差异巨大，说明需要谨慎对待模型泛化和跨数据集迁移。
...
TL;DR: 本文提出了一套工具集成推理（TIR）训练方案，解决了强思维模型在使用工具时推理性能下降的问题，通过在监督微调中控制工具轨迹比例、优化pass@k和响应长度而非训练损失，以及结合稳定强化学习，在Qwen3模型上取得了SOTA性能（AIME 2025上4B达96.7%，30B达99.2%）。


<details>
  <summary>Details</summary>
Motivation: 工具集成推理（TIR）可以扩展思维模型的推理能力，但实际发现即使强思维模型几乎没有调用工具，引入工具评估反而会降低其推理性能。本文旨在向强思维模型注入自然的工具使用行为，同时不牺牲其无工具推理能力。

Method: (1) TIR监督微调（SFT）的关键在于教师轨迹的可学习性，应优先选择适合工具增强的问题；(2) 控制工具使用轨迹的比例以缓解对纯文本推理能力的灾难性遗忘；(3) 优化pass@k和响应长度而非训练损失，为后续强化学习保留探索空间；(4) 基于合适的SFT初始化和显式防止模式崩溃的稳定RLVR训练。

Result: 在Qwen3思维模型（4B和30B）上应用该方案，在多个基准测试中达到开源模型SOTA水平，AIME 2025上4B模型达96.7%，30B模型达99.2%。

Conclusion: 该TIR训练配方能够有效提升思维模型的工具使用能力，同时保持其无工具推理性能，验证了SFT与RL结合的策略在工具集成推理中的有效性。
...
TL;DR: 本文发现语言模型在评估框架下和实际部署场景中行为存在系统性差异，且不同模型表现出截然相反的"评估谨慎"或"部署谨慎"模式，这种异质性还受评判模型选择的影响。


<details>
  <summary>Details</summary>
Motivation: 安全基准测试的结果常被用来推断模型在部署后的行为，但这种推断可能不可靠，因为模型的行为可能取决于提示词是否看起来像评估场景。研究者想系统性地量化这种差异。

Method: 定义了"评估上下文偏差"概念，设计了配对提示协议（将同一任务分别框架为评估、部署交互或中性请求），控制释意变异、基准熟悉度和评判框架敏感性，在5个指令微调模型和1组基础/指令消融模型上进行了系统比较。

Result: 发现显著的跨模型异质性：OLMo-3-Instruct表现为"评估谨慎"（评估框架下拒答率上升11.8pp，有害服从率下降3.6pp），而Mistral-Small-3.2、Phi-3.5-mini和Llama-3.1-8B表现为"部署谨慎"（评估vs部署的拒绝效应为-9到-20pp）。基础模型也表现出"部署谨慎"模式，说明对齐训练是反转点。70B模型保留了方向但幅度减弱。跨模型异质性依赖于所使用的评判模型。

Conclusion: 安全基准不能简单等同于部署行为，模型在不同框架下行为存在系统性差异且高度异质，评估结果严重依赖于所选的评判模型。
...
TL;DR: 提出SKOP方法，通过正交投影约束注意力重路由，在保持模型行为控制的同时显著减少推理和检索性能下降。


<details>
  <summary>Details</summary>
Motivation: 激活引导（Activation Steering）虽能控制LLM行为，但常导致推理和检索性能严重下降，主要原因是注意力重路由（attention rerouting）使模型关注不重要的token。

Method: 提出SKOP（Steering via Key-Orthogonal Projections），保留模型依赖的关键焦点token的注意力模式，同时允许非关键尾部token的注意力重新分配，从而约束有害的注意力重路由。

Result: SKOP实现了最佳的引导-效用权衡，将效用退化降低5-7倍，同时保留95%以上的原始引导效果；在长上下文检索中，SKOP能维持稳健性能。

Conclusion: 注意力重路由是激活引导导致性能退化的主要原因，SKOP通过约束该问题可在保持引导效果的同时显著减少性能损失。
...
TL;DR: SEQUOR是一个用于评估长多轮对话中指令遵循能力的自动基准测试，发现当前模型在长时间对话中遵循约束的能力显著下降，单约束准确率下降超11%，多约束下降超40%，约束变更场景下降超9%。


<details>
  <summary>Details</summary>
Motivation: 现有指令遵循基准测试大多聚焦于单轮或短多轮场景，缺乏对长时域多轮对话中指令遵循能力的评估。

Method: 提出SEQUOR自动基准测试，通过模拟基于人设的对话交互，从真实对话中提取约束，评估模型在长多轮对话中对约束的遵循能力。

Result: 单约束指令遵循准确率随对话长度增加持续下降超过11%；多约束同时存在时准确率下降超过40%；约束在对话中途添加或替换时准确率下降超过9%。

Conclusion: 当前模型在长多轮对话中仍难以稳定遵循用户指令，SEQUOR为更好衡量助手指令遵循能力提供了有效途径。
...
TL;DR: GATHER是一种面向超实体查询的收敛中心检索器，通过对生物知识图谱进行全局多源图遍历，识别拓扑收敛节点，实现零样本单细胞类型注释，仅需一次LLM调用即可达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的RAG方法在零样本单细胞注释中采用局部实体探索策略，但每个查询包含数十到数百个基因，单一基因无法决定细胞类型，需要从集体共现中涌现标签。这种超实体查询从根本上挑战了局部逐实体推理方法，导致可扩展性差和LLM成本高昂。

Method: 提出GATHER（图感知遍历与超实体检索），进行全局多源图遍历，识别从多个输入基因共同可达的拓扑收敛节点；结合节点和路径重要性评分，在检索阶段完全无需LLM参与；构建细胞中心生物知识图谱（VCKG）进行实例化。

Result: 在Immune和Lung两个数据集上，GATHER分别达到27.45%和59.64%的精确匹配准确率，优于ToG、ToG-2、RoG、PoG等强基线；每个样本仅需一次LLM调用，而基线需要2-61次调用。

Conclusion: 收敛节点能够将多实体信号压缩为紧凑、高信息的证据，每项信息量超过多跳路径，为局部逐实体推理提供了高效的全局替代方案。
...
TL;DR: 本文受认知科学中“全局点燃”现象启发，提出Mindscape Activation Signature (MiA-Signature)概念——一种压缩的全局激活模式表征，通过子模选择与工作记忆迭代更新实现，并在RAG与Agent系统中取得长上下文理解任务的性能提升。


<details>
  <summary>Details</summary>
Motivation: 认知科学研究表明意识访问与全局点燃相关，但个体无法直接枚举所有激活内容；受此启发，需要一种紧凑表征来近似激活对下游处理的全局影响。

Method: 提出MiA-Signature，基于子模选择从高维概念空间中选取覆盖激活上下文的子集，并可选通过工作记忆的轻量迭代更新进行优化，以此作为条件信号近似全局激活状态。

Result: 将MiA-Signature集成到RAG和Agent系统中，在多个长上下文理解任务上取得了一致的性能提升。

Conclusion: MiA-Signature通过压缩表征有效近似全局激活效应，兼具计算效率与性能优势，为长上下文理解提供了新的可行方案。
...
TL;DR: 本文提出一个结合规则过滤与大语言模型分类的可扩展模块化新词检测流水线，在5.27亿条Reddit帖子中提取出1,021个候选新词，经人工验证发现58.7%为真正的词汇创新，并揭示了模型间分类分歧严重的现象。


<details>
  <summary>Details</summary>
Motivation: 大规模自动化检测新词面临定义模糊、算力成本高、跨模型分类不一致等挑战，亟需一个既保持理论严谨性又具备可扩展性的系统化方案。

Method: 采用语法与非语法形态学双框架定义新词边界，设计四类分类方案（新词、实体、外来词、非新词）；先通过规则过滤将1.246亿独特词元缩减99.99%以上，再交由多个LLM独立分类并通过多数投票和人工验证最终确认。

Result: 从5.27亿条Reddit帖子（2005-2024年）中提取1,021个候选新词，经人工标注确认599个（58.7%）为真实词汇创新；多个LLM之间展现出显著的分歧。

Conclusion: 该流水线在可扩展性与理论严谨性之间取得平衡，但跨模型分歧表明操作化新词检测仍面临挑战；代码与标注数据已开源。
...
TL;DR: 本研究探究文本和语言特征（如词二元组、词性分布）在假新闻检测中的作用，通过在COVID-19疫情期间收集的新数据集上使用多种传统机器学习模型进行实验。结果表明，随机森林性能最佳，文本和语言特征单独使用时均能提升检测效果，但两者结合并未显著提升。研究证实传统机器学习方法可有效利用这些特征进行假新闻检测。


<details>
  <summary>Details</summary>
Motivation: 尽管有实证证据表明文本和语言特征有助于区分真假新闻，但这些内容特征在假新闻检测中的研究仍不充分，因此本研究旨在深入探究这些特征的有效性。

Method: 使用COVID-19疫情期间收集的新数据集，选取词二元组、词性分布等文本和语言特征，采用决策树、K近邻、逻辑回归、支持向量机和随机森林五种传统机器学习模型进行实验。

Result: 随机森林在所有设置中表现最佳，支持向量机紧随其后；文本和语言特征单独使用时均能提升检测效果，但组合使用并未显著提升；二元组与词性标签的使用效果存在差异。

Conclusion: 传统机器学习方法可以利用文本和语言特征成功检测假新闻，无需依赖深度学习方法。
...
TL;DR: 本研究探讨了大型语言模型（LLMs）在情感对话中对自身生成回复的一致性，发现模型在面对包含错误预设的查询时表现不佳，尤其对中等情感内容更易受影响。


<details>
  <summary>Details</summary>
Motivation: 验证LLMs在情感对话中是否对自身生成的回复保持一致性，特别是在包含错误预设的查询下。

Method: 将LLM生成的文本作为查询反馈给同一模型，设计三种不同强度的错误预设查询，在极端和中度情感两个维度下测试Claude-3.5-haiku、GPT4o-mini和Mistral-7B三个模型。

Result: LLMs表现低于平均水平，容易受到查询中错误信念的影响，尤其对中等情感内容更敏感；注意力分数分析显示模型优先级从评估转向生成。
...
TL;DR: 研究发现，语言模型 surprisal 与隐喻新颖性判断的相关性主要源于词频因素，而非语境可预测性。


<details>
  <summary>Details</summary>
Motivation: 探索 LM surprisal 与隐喻新颖性之间的关系是否真正反映语境可预测性，还是主要受词频因素影响。

Method: 使用两种词频指标，分析8个Pythia模型大小和154个训练检查点的surprisal估计，比较词频和surprisal对隐喻新颖性评分的预测能力。

Result: 词频比surprisal更强地预测隐喻新颖性；surprisal-新颖性关联在训练早期达到峰值后下降，与surprisal-词频关联的同步变化一致。

Conclusion: 先前报道的最优LM surprisal设置可能错误地将语境可预测性与隐喻新颖性和加工难度联系起来，而词频可能是主要的潜在因素。
...
TL;DR: 本文提出了STALE基准测试（400个冲突场景、1200个评估查询），揭示LLM智能体在记忆更新方面的严重缺陷——即使最佳模型准确率也仅55.2%。同时提出CUPMem原型作为改进方向。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要测试静态事实检索，忽略了智能体根据新证据修订已有信念的能力，尤其是"隐式冲突"这一关键失败模式尚未被充分研究。

Method: 构建包含400个专家验证的冲突场景的STALE基准，设计三维探测框架（状态解析、前提抵抗、隐式策略适应），并开发CUPMem原型实现结构化状态合并与传播感知搜索。

Result: 前沿LLM和专用记忆框架存在"检索更新证据与基于证据行动"之间的普遍差距，最佳模型仅55.2%准确率；模型常接受查询中的过时假设，且难以识别状态变化对相关记忆的失效影响。

Conclusion: 显式状态裁定是构建鲁棒智能体记忆的有前景方向，现有模型在隐式冲突检测和记忆动态更新上仍有巨大提升空间。
...
TL;DR: 本文提出Token-Superposition Training (TST)，一种即插即用的预训练加速方法，通过将多个连续token合并为bag并使用多热交叉熵目标进行训练，在不修改模型架构、优化器等组件的情况下，显著提升数据吞吐量，在10B规模模型上实现高达2.5倍的预训练时间缩减。


<details>
  <summary>Details</summary>
Motivation: 大语言模型预训练成本过高、效率低下，现有的加速方法往往需要对并行策略、优化器、分词器、数据或模型架构进行复杂且侵入性的修改，缺乏简单高效的即插即用方案。

Method: TST包含两个阶段：(i) 叠加阶段——将多个连续token合并为一个bag，采用多热交叉熵(MCE)损失函数进行高效训练；(ii) 恢复阶段——切换回标准训练方式。该方法无需修改并行策略、优化器、分词器、数据或模型架构。

Result: 在270M、600M参数规模上进行了广泛验证，并在3B和10B的A1B混合专家模型上测试，TST在不同设置下均表现稳健，一致优于基线损失和下游评估结果。在等损失条件下，TST在10B A1B规模上实现了高达2.5倍的预训练时间缩减。

Conclusion: TST是一种简单、高效、鲁棒的预训练加速方法，无需对现有训练框架进行侵入式修改即可显著提升数据吞吐量，适用于不同规模和架构的语言模型预训练任务。
...
TL;DR: Cola DLM是一种层次化潜变量扩散语言模型，通过文本VAE、块因果DiT和条件解码实现分层信息分解，在连续潜空间中建模全局语义先验，而非在词元级别恢复观测值，从而在约2B参数规模下验证了其作为严格词元级语言建模的替代方案的可行性与扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有自回归范式虽成功，但高质量文本生成不必局限于固定的从左到右顺序；现有替代方法在生成效率、可扩展表示学习和有效全局语义建模方面仍难以兼顾。

Method: 提出Cola DLM：1) 使用Text VAE学习稳定的文本到潜变量映射；2) 使用块因果DiT在连续潜空间中建模全局语义先验；3) 通过条件解码生成文本。扩散过程执行潜变量先验传输而非词元级观测恢复，分离了全局语义组织与局部文本实现。

Result: 在4个研究问题、8个基准、严格匹配的约2B参数自回归和LLaDA基线以及约2000 EFLOPs的扩展曲线上，验证了Cola DLM的有效配置及其强大的文本生成扩展行为。

Conclusion: 层次化连续潜变量先验建模可作为严格词元级语言建模的原则性替代方案，生成质量与扩展行为比似然更能反映模型能力，并为统一离散文本与连续模态建模提供了具体路径。
...
TL;DR: 本文提出Lighthouse Attention，一种基于层级对称选择的高效注意力算法，通过可移除的亚二次复杂度预处理/后处理步骤和两阶段训练策略，在保持因果性的同时显著加速长序列Transformer训练。


<details>
  <summary>Details</summary>
Motivation: 训练因果Transformer在超长序列时，标准缩放点积注意力（SDPA）的二次时间和内存开销成为性能瓶颈，亟需更高效的训练方案。

Method: (i) 亚二次复杂度的层级预处理/后处理，实现序列的自适应压缩与解压；(ii) 对称压缩策略，同时对query、key、value进行池化并保持从左到右的因果性；(iii) 两阶段训练：大部分时间使用Lighthouse Attention预训练，最后短时间恢复为完整注意力模型。

Result: 在小型LLM预训练实验中，与全注意力训练相比，在相同设置下实现了更快的总训练时间和更低的最终损失。

Conclusion: Lighthouse Attention作为一种训练时专用算法，有效降低了长序列训练的计算开销，且可无缝移除恢复为全注意力模型，具有良好的实用性和扩展潜力。
...
TL;DR: 本文比较了基于规则模板和零样本GPT-4两种方法在远程认知康复场景中自动生成临床报告的效果，发现模板系统在流畅性和连贯性上更优，GPT-4更简洁，但两者在统计上无显著差异，并据此提出了八项临床报告系统设计建议。


<details>
  <summary>Details</summary>
Motivation: 认知康复治疗需求增长与言语治疗师资源有限之间的矛盾，促使远程康复工具被广泛采用，但这些工具产生的大量交互数据 clinicians 难以高效审查，亟需自动化的临床报告生成方法。

Method: 采用两种对比方法：(1) 基于规则的模板系统，编码言语治疗领域知识作为显式决策规则和验证模板；(2) 零样本GPT-4方法，生成更流畅简洁的报告。两者使用相同的预提取且经专家验证的结构化变量。由8位言语治疗师和应届毕业生通过九维度问卷进行评估。

Result: 模板系统在流畅性、连贯性和结果呈现上得分更高，GPT-4输出更简洁。各评估维度方向性差异一致，但经校正后均未达到统计显著性，反映了专家临床评估的规模限制。

Conclusion: 临床可靠性与语言质量之间存在明确权衡；基于评估反馈提取了八项临床报告系统设计建议；贡献了一种结合专家启发、分类驱动生成和多维人工评估的可复现方法，为生成式AI在医疗领域的负责任应用提供参考。
...
TL;DR: 本文提出UniSD框架，系统研究自蒸馏在大型语言模型中的应用，整合多种互补机制（多教师一致、EMA稳定、对比学习、特征匹配、发散裁剪），在六个基准和六个模型上验证了其有效性，最终构建的UniSD_full相比基础模型提升+5.4分，比最强基线提升+2.8分。


<details>
  <summary>Details</summary>
Motivation: 自蒸馏在自回归LLM中面临挑战：自生成轨迹自由、正确性依赖任务、合理理由仍可能提供不稳定监督。现有方法仅研究孤立设计选择，缺乏对有效性、角色和交互的系统性理解。

Method: 提出UniSD统一框架，整合五种互补机制：多教师一致性监督、EMA教师稳定化、token级对比学习、特征匹配、发散裁剪，系统研究各组件在不同任务中的作用与交互。

Result: 在六个基准、六个模型（来自三个模型家族）上验证，UniSD_full相比基础模型提升+5.4分，相比最强基线提升+2.8分，揭示了自蒸馏何时优于静态模仿、各组件驱动增益的方式及跨任务交互规律。

Conclusion: 自蒸馏是一种实用且可控的LLM高效适配方法，无需依赖更强的外部教师模型即可实现显著性能提升。
...
TL;DR: 本文研究了Algospeak（算法隐语）中逃避者与检测者之间的共同演化动态，提出了“多数可理解调制”（MUM）概念，并通过可复现框架和COVID-19虚假信息数据集，量化了语言调制水平与可检测性、可理解性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: LLM在内容生成和审核中的广泛应用加剧了Algospeak的逃避策略演化，需要正式化其动态机制并量化逃避效果与可理解性之间的权衡。

Method: 基于联合行动模型形式化动态机制，引入MUM概念；构建基于现有分类法的可调调制框架，生成700条COVID-19虚假信息变体（20条基句×5个调制水平×7种策略）；使用7种语言模型进行意义恢复（可理解性）和虚假信息检测（可检测性）评估，通过曲线拟合估计MUM阈值。

Result: 揭示了调制水平与可理解性之间的特征关系，成功估计了多数可理解调制（MUM）阈值，并实现了跨策略和跨模型的敏感性分析。

Conclusion: 该研究为理解Algospeak背后的动态机制奠定了基础，并提供了可复现的框架、数据集和实验设置，有助于未来在内容审核与语言规避方面的研究。
...
TL;DR: 本文提出了首个LLM生成报告的引文归因评估框架，通过可复现的AST解析器提取并评估内联引文在三个维度（链接可用性、内容相关性、事实准确性）的表现。实验发现前沿模型的链接有效性>94%、相关性>80%，但事实准确性仅39-77%；且随检索量增加（2→150次工具调用），事实准确性平均下降42%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的深度研究报告中的引文无法被可靠验证：自引方法存在偏差，RAG方法不验证来源的可访问性、相关性和事实一致性，缺乏系统性的引文归因评估框架。

Method: 提出三类评估维度：Link Works（URL可访问性）、Relevant Content（主题对齐）、Fact Check（事实准确性）。使用可复现的AST解析器从Markdown报告中提取内联引文，通过检索实际引用内容实现闭环评估，采用基于评定量表的LLM-as-a-judge评估器并经过人工校准。

Result: 14个闭源和开源LLM的基准测试显示：最强模型链接有效性>94%、相关性>80%，但事实准确性仅39-77%；不到一半的开源模型能一次性生成带引文的报告。消融实验表明工具调用从2次扩展到150次时，两个前沿模型的事实准确性平均下降42%。

Conclusion: 研究发现表面引文质量与事实可靠性之间存在严重脱节，更多的检索并不能带来更准确的引文。本文框架为评估这种脱节提供了基础设施。
...
TL;DR: 提出StraTA框架，通过显式的轨迹级策略抽象，结合分层GRPO式强化学习、多样化策略回滚和关键自我评判，显著提升了LLM在长时域决策任务中的探索能力和信用分配，在ALFWorld、WebShop和SciWorld上取得了领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体方法多为纯反应式，在长时域决策中探索能力弱、信用分配困难，导致优化效果不佳。

Method: 提出StraTA框架：从初始任务状态采样紧凑策略，后续动作基于该策略执行；采用分层GRPO式回滚设计联合训练策略生成与动作执行；结合多样化策略回滚和关键自我评判增强效果。

Result: ALFWorld上成功率达93.1%，WebShop上达84.2%；SciWorld上整体得分63.5%，超越前沿闭源模型。

Conclusion: StraTA通过引入显式的轨迹级策略抽象，有效提升了LLM智能体在长时域决策任务中的样本效率和最终性能，验证了层次化策略设计的有效性。
...
TL;DR: EMO通过文档级专家共享约束，在保持完整MoE性能的同时，实现了领域专家子集的可独立使用和组合，显著提升了稀疏大模型在内存受限场景下的部署效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型作为整体部署，即使应用只需要部分能力（如代码、数学）也必须加载完整模型；MoE虽能按输入激活部分专家，但限制专家子集用于特定领域时性能严重下降，无法满足内存受限场景下的高效部署需求。

Method: 提出EMO方法，核心思想是让同一文档（同一领域）的token共享专家池，不同文档使用不同的专家池，通过文档边界这一简单的预训练约束，使专家自动在语义层面（如数学、代码）形成可组合的领域分组。

Result: 在1T token上预训练1B激活/14B总参的EMO模型。全模型性能与标准MoE持平；仅保留25%专家时性能仅下降1%，保留12.5%专家时下降3%，而标准MoE在此设置下性能崩溃。专家分组表现出语义级（如数学、代码）而非低阶语法级 specialization。
...
TL;DR: 本研究利用All of Us全国纵向EHR数据，通过混合特征选择和人口统计分层模型，实现了对慢性鼻窦炎（CRS）的早期预测，AUC达0.8461。


<details>
  <summary>Details</summary>
Motivation: CRS是一种常见但异质性强、早期识别困难的炎症性疾病，其症状与过敏性鼻炎等常见疾病重叠，且现有预测研究多依赖单机构队列，缺乏人群层面的普适性。

Method: 利用All of Us全国纵向EHR数据，基于诊断前两年的病史预测CRS；采用基于流行率统计筛选与模型重要性排序相结合的混合特征选择管道，将约11万个候选代码压缩为100个可解释特征；针对六种成人性别与生命阶段亚组训练人口统计分层模型，并进行亚组特异性超参数调优。

Result: 整体AUC达到0.8461，比最佳基线模型提升了0.0168。
...
TL;DR: 本文提出 Sequential Agent Tuning (SAT)，一种无协调器的多智能体训练范式，通过分块坐标更新和序列感知优势估计器，使多个小模型组成的团队能超越单个大模型，并支持即插即用的智能体升级。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数多、部署成本高，而现有使用多个小模型协作的方法在联合训练时存在分布偏移和不稳定问题，缺乏可扩展、可保证性能提升的训练范式。

Method: SAT采用分块坐标更新策略，将团队建模为因子化策略；提出序列感知的基于策略的优势估计器，结合每个智能体的KL信任区域约束来隔离占据漂移；理论保证单调改进和即插即用不变性。

Result: 三个4B参数的智能体（总计12B）经SAT训练后，在AIME24/25基准上平均超越Qwen3-32B模型3.9%；换用两个8B智能体后，综合分数提升10.4%。

Conclusion: SAT框架提供了多智能体协作训练的理论保障（单调改进和即插即用），实验验证了多个小模型可超越单一大型模型的可行性，为高效部署LLM提供了新方向。
...
### [60] [Horizon-Constrained Rashomon Sets for Chaotic Forecasting](https://arxiv.org/abs/2605.05218)
*Gauri Kale,Rahul Vishwakarma,Holly Diamond,Ava Hedayatipour,Amin Rezaei*

Main category: cs.LG

TL;DR: 本文首次将混沌理论与预测多重性联系起来，提出了面向混沌系统的水平约束Rashomon集框架，证明有效Rashomon集随预测时长指数收缩，并开发了基于下游效用的决策对齐模型选择算法，在多个混沌系统中将决策质量提升18-34%。


<details>
  <summary>Details</summary>
Motivation: 预测多重性与混沌动力学是机器学习中两个独立发展的基本挑战，但二者之间存在概念上的联系，尚未被建立起来。本文旨在弥补这一空白。

Method: 提出了水平约束Rashomon集理论框架，利用最大Lyapunov指数刻画模型多重性在混沌系统中随预测时长的演变规律，引入Lyapunov加权度量以收紧预测不一致性的界，并开发了基于下游效用的决策对齐模型选择算法。

Result: 在Lorenz-96、Kuramoto-Sivashinsky等合成混沌系统和风电、交通、天气等真实应用中，决策质量提升18-34%，同时保持有竞争力的预测性能。

Conclusion: 本文建立了混沌理论与预测多重性之间的首个严格联系，为在安全关键混沌领域部署机器学习提供了原则性指导。
...
### [61] [MidSteer: Optimal Affine Framework for Steering Generative Models](https://arxiv.org/abs/2605.05220)
*Tatiana Gaintseva,Andrew Stepanov,Ziquan Liu,Martin Benning,Gregory Slabaugh,Jiankang Deng,Ismail Elezi*

Main category: cs.LG

TL;DR: 本文形式化了概念引导的理论框架，提出了MidSteer方法，在视觉扩散模型和大型语言模型等多种任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有概念引导方法缺乏全面的理论框架，作者希望填补这一理论空白。

Method: 首先将概念引导与线性概念擦除LEACE建立联系，然后提出LEACE-Switch框架用于概念切换，最后引入更通用的MidSteer框架实现有向的最小扰动变换。

Result: MidSteer在视觉扩散模型和大型语言模型等多种任务、模态和架构中表现良好。

Conclusion: MidSteer作为一种更通用的仿射概念操控框架，放松了LEACE-Switch的假设条件，实现了有向的最小扰动变换，并验证了其有效性。
...
### [62] [Adaptive Computation Depth via Learned Token Routing in Transformers](https://arxiv.org/abs/2605.05222)
*Ahmed Abdelmuniem Abdalla Mohammed*

Main category: cs.LG

TL;DR: 本文提出Token-Selective Attention (TSA)，通过轻量级门控机制让Transformer根据token难度动态跳过部分层计算，在几乎不损失质量的情况下节省14-23%的计算量。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer对所有token使用相同层数，忽略了不同token的上下文难度差异，导致计算效率低下。

Method: 在相邻Transformer块之间添加一个轻量级两层MLP门控，为每个token生成连续的停止概率，实现端到端可微的令牌选择性计算，仅增加1.7%的参数开销。

Result: 在字符级语言建模任务中，TSA节省了14-23%的token-layer操作，质量损失小于0.5%；在相同效率下，验证损失比early exit低0.7%。

Conclusion: TSA能够学习与难度成比例的路由策略，无需显式深度正则化，且学到的路由可直接用于推理时的稀疏执行，实现实际加速。
...
### [63] [Structural Instability of Feature Composition](https://arxiv.org/abs/2605.05223)
*Yunpeng Zhou*

Main category: cs.LG

TL;DR: 本文提出了一个几何框架来分析稀疏自编码器中特征组合的不稳定性，发现当同时激活多个语义特征时，由于非线性干扰效应会导致组合坍塌，并通过理论推导和实验验证了这一现象。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器虽能解耦特征叠加并实现激活操控，但组合式操控（同时激活多个语义潜在变量）的理论基础尚不明确，现有线性表示假说忽略了过完备字典中的非线性干扰效应。

Method: 将激活空间建模为高维稀疏锥形流形，基于球面字典模型推导组合坍塌的渐近阈值（由信号锥的高斯平均宽度/统计维度刻画）；分析ReLU整流在高偏置下将微观相关性方差波动转化为系统性漂移，形成类似棘轮效应的干扰累积。

Result: 在CLEVR数据集的结构化语义特征上验证了理论预测的缩放趋势，层次化相关性加速了组合坍塌的转变；随机基线相比下，结构化特征的组合更不稳定。

Conclusion: 揭示了基于特征并集的操控可扩展性受几何约束，需要设计显式管理干扰的组合机制，而非依赖朴素线性叠加。
...
### [64] [Channel-Level Semantic Perturbations: Unlearnable Examples for Diverse Training Paradigms](https://arxiv.org/abs/2605.05224)
*Bo Wang,Jia Ni,Mengnan Zhao,Zhan Qin,Kui Ren*

Main category: cs.LG

TL;DR: 本文首次系统研究了无学习样本在预训练-微调范式下的表现，发现预训练权重会削弱现有UEs方法的效果，并提出一种分层欺骗策略SSC来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有无学习样本研究主要关注从头训练场景，忽视了预训练-微调范式下其行为表现，需要填补这一空白。

Method: 提出分层语义伪装策略(SSC)，将扰动生成限制在语义有效子空间内，以绕过预训练权重引入的语义抑制。

Result: 该方法在浅层冻结和语义聚焦预训练等困难场景下仍能保持数据的不可学习性。

Conclusion: SSC有效桥接了预训练范式下无学习样本学习的关键缺口，保障了数据隐私。
...
### [65] [MACS: Modality-Aware Capacity Scaling for Efficient Multimodal MoE Inference](https://arxiv.org/abs/2605.05225)
*Bo Li,Chuan Wu,shaolin Zhu*

Main category: cs.LG

TL;DR: MACS是一个免训练推理框架，通过熵加权负载和动态模态自适应容量机制，解决了MoE多模态大模型在专家并行推理中的效率瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于token计数的负载均衡方法无法解决MoE MLLMs在EP推理中的两个独特挑战：(1)信息异质性——大量冗余的视觉token被与语义关键的token同等对待；(2)模态动态性——不同任务中视觉与文本比例变化导致资源分配不均。

Method: 提出MACS框架，包含两个核心机制：(1)熵加权负载机制——量化视觉token的语义价值，解决信息异质性；(2)动态模态自适应容量机制——根据输入实时的模态组成分配专家资源，解决模态动态性。

Result: 在多个多模态基准测试上，MACS显著优于现有方法。

Conclusion: MACS为MoE MLLMs在EP推理中的高效部署提供了一种新颖且鲁棒的免训练解决方案。
...
### [66] [Internalizing Outcome Supervision into Process Supervision: A New Paradigm for Reinforcement Learning for Reasoning](https://arxiv.org/abs/2605.05226)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习用于推理面临两大挑战：一是仅依赖结果级奖励难以进行精确的信用分配；二是外部构建的过程监督成本高昂且难以可持续扩展。

Method: 提出监督内化方法，通过识别、纠正和重用失败的推理轨迹，自动提取过程级学习信号，将结果监督转化为过程监督，形成模型在强化学习过程中持续生成和优化内部过程监督的新训练范式。

Result: 实现了在仅有结果监督的条件下进行更细粒度的策略优化，为推理任务中的细粒度信用分配开辟了新路径。

Conclusion: 强化学习中的推理问题可被理解为将结果监督内化为过程监督的问题，该方法在无需外部过程监督的情况下实现了细粒度学习信号提取。
...
### [67] [Rethinking Data Curation in LLM Training: Online Reweighting Offers Better Generalization than Offline Methods](https://arxiv.org/abs/2605.05227)
*Wanru Zhao,Yihong Chen,Yuzhi Tang,Wentao Ma,Shengchao Hu,Shell Xu Hu,Alex Iacob,Abhinav Mehrotra,Nicholas D. Lane*

Main category: cs.LG

TL;DR: 提出ADAPT在线数据重加权框架，通过动态调整每个样本的学习率来优化LLM训练，替代传统离线数据筛选方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据筛选方法采用离线范式，与训练过程脱节，导致工程开销大、在模型/任务迁移时需重新运行整个流程；同时硬过滤或重采样会损失数据多样性，影响泛化能力。

Method: 提出ADAPT在线框架，基于相似度质量信号为每个训练样本动态分配自适应学习率，通过损失加权实现样本重要性在线调整，无需改变训练样本数量，隐式实现课程学习。

Result: 在指令微调和大规模预训练实验中，ADAPT在相等FLOPs下，持续优于离线筛选/混合方法及现有在线方法，实现了更强的跨基准泛化性能。

Conclusion: 将数据整理重新定义为在线重加权问题，通过动态调整样本重要性而非静态预处理，能更有效地提升LLM训练效果和泛化能力。
...
### [68] [Evolutionary fine tuning of quantized convolution-based deep learning models](https://arxiv.org/abs/2605.05228)
*Marcin Pietroń*

Main category: cs.LG

TL;DR: 本文提出使用进化策略优化深度模型量化后的权重分配，通过迭代调整部分权重的量化状态，以提升量化模型的精度。


<details>
  <summary>Details</summary>
Motivation: 深度模型在IoT、移动设备等场景中受限于复杂度和内存大小，量化是主流压缩方法，但基于最近邻的量化方式无法保证最优精度。

Method: 采用进化策略作为优化方法，每轮迭代只改变少量权重的值，将其转移到不同的量化状态，通过合适的算子与参数设置来提升精度。

Result: 在VGG、ResNet等图像分类与检测任务以及自编码器架构上验证，结果表明所提方法能快速提升量化模型的精度。

Conclusion: 进化策略可以有效优化预训练量化模型的权重分布，超越传统最近邻量化，为模型压缩提供了一种精度恢复的新思路。
...
### [69] [Attribution-Guided Continual Learning for Large Language Models](https://arxiv.org/abs/2605.05285)
*Yazheng Liu,Yuxuan Wan,Rui Xu,Xi Zhang,Sihong Xie,Hui Xiong*

Main category: cs.LG

TL;DR: 提出归因引导的持续微调框架，通过估计每个Transformer层中每个参数对特定任务的重要性，用重要性分数调节梯度，在持续学习中缓解大语言模型的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有缓解灾难性遗忘的方法（如数据重放、参数冻结、正则化）缺乏对大语言模型内部知识分布的语义感知，无法区分哪些参数应保留、哪些应更新。

Method: 提出归因引导的持续微调框架，估计每个Transformer层中每个参数对特定任务的重要性，用这些重要性分数调节梯度：对先前任务重要的参数更新幅度小，不重要的参数保持可塑性以学习新任务。

Result: 在持续学习基准上，该方法一致优于基线方法，更好地保留旧任务知识，同时在新任务上保持有竞争力的性能。

Conclusion: 通过归因引导的参数重要性估计和梯度调节，可以有效区分大语言模型中应保留和应更新的参数，从而缓解灾难性遗忘。
...
### [70] [COPYCOP: Ownership Verification for Graph Neural Networks](https://arxiv.org/abs/2605.05360)
*Rahul Nandakumar,Deepayan Chakrabarti*

Main category: cs.LG

TL;DR: CopyCop是一种无需水印或指纹的算法，能在GNN架构、权重、嵌入维度不同且对手可变换嵌入的条件下，准确识别抄袭的GNN模型。


<details>
  <summary>Details</summary>
Motivation: 现有水印和指纹方法无法在对手对嵌入进行变换的情况下检测GNN抄袭，需要更鲁棒的检测方案。

Method: 提出CopyCop算法，利用图结构特性和嵌入关系进行抄袭检测，并提供了理论保证。

Result: 在14个数据集和5种GNN架构上验证了CopyCop的准确性，对多种对抗攻击和变换具有鲁棒性。

Conclusion: CopyCop能在严苛条件下有效识别抄袭GNN，弥补了现有方法的不足。
...
### [71] [SPADE: Faster Drug Discovery by Learning from Sparse Data](https://arxiv.org/abs/2605.05370)
*Rahul Nandakumar,Ben Fauber,Deepayan Chakrabarti*

Main category: cs.LG

TL;DR: SPADE是一种新型配体选择算法，平均只需40次测试就能找到10个高质量配体，在样本效率上比深度学习和贝叶斯优化方法提升7%-32%，且速度比最接近的竞争者快10倍。


<details>
  <summary>Details</summary>
Motivation: 药物发现中，超过95%的候选配体无法通过早期筛选，且需要对缺乏先验数据的新颖蛋白质进行高效配体筛选，现有方法效率不足。

Method: 提出SPADE算法，一种新颖的配体选择方法，通过从稀疏数据中学习来实现高效迭代筛选。

Result: 平均仅需40次测试即可找到10个高质量配体；在多数蛋白质上优于深度学习和贝叶斯优化方法，样本效率中位数提升7%-32%；速度比最接近的竞争者快10倍。

Conclusion: SPADE在样本效率和计算速度上均显著优于现有方法，为缺乏先验数据的新颖蛋白质药物发现提供了高效的解决方案。
...
TL;DR: 本文研究了基于预训练扩散模型的零样本条件采样在求解线性逆问题（如图像修复、超分辨率）中的应用。作者通过分数函数的法向-切向分解，理论证明了用无条件切向分数替代条件切向分数产生的误差受条件互信息上界约束，并据此提出投影朗之万初始化+引导反向去噪的方法，实验表现优于现有投影基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于投影的采样器仅强制测量一致性，但未能正确分配可行集上的概率质量，导致条件样本存在偏差。作者希望从理论上分析偏差来源，并设计更优的零样本条件采样方法。

Method: 1. 对分数函数进行法向-切向分解，区分观测方向分数（由测量精确确定）和切向条件分数（未知）。2. 理论证明用无条件切向分数替代条件切向分数的误差受维度无关的条件互信息上界约束。3. 提出投影朗之万初始化+引导反向去噪的算法流程。

Result: 在图像修复和超分辨率实验中，所提方法显著优于强投影基线方法，生成的条件样本更加准确且符合真实数据分布。

Conclusion: 通过信息论分解揭示了条件采样偏差的本质来源，为设计更优的零样本求解逆问题方案提供了理论指导。
...
TL;DR: 提出了一种节点-边策略分解（NEPF）方法，将路由策略分解为节点排列和边选择两个阶段，通过预编码边聚合、非自回归架构和分层强化学习，解决了多图车辆路径问题中的可扩展性问题，在六个VRP变体上实现了与SOTA相当或更优的解质量，同时大幅提升训练和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有神经方法大多局限于欧几里得设置或简单图，而多图（平行边代表不同旅行选项）场景下现有方法存在严重的可扩展性问题。

Method: 提出NEPF方法，将路由策略分解为节点排列阶段和边选择阶段；引入预编码边聚合方案、非自回归架构用于边阶段，以及分层强化学习方法联合训练两个阶段。

Result: 在六个VRP变体上，NEPF在解质量上匹配或超越现有最优方法，同时在训练和推理速度上显著更快。

Conclusion: NEPF有效缓解了多图VRP的可扩展性问题，是一种高效且高质量的解决方案。
...
TL;DR: 本文研究了带有状态事件、代数约束和重置映射的半显式微分代数方程（DAE）系统的可微参数优化问题，提出了两种梯度计算策略：自动微分模拟法和显式离散伴随法，并比较了它们的优劣。


<details>
  <summary>Details</summary>
Motivation: 梯度基参数学习在含有代数变量、依赖参数的事件时间以及不连续重置映射的DAE系统中非常困难，现有方法缺乏系统性的处理框架。

Method: 将学习问题建模为带DAE动力学、代数约束、守卫方程和重置映射的约束最小二乘问题；提出两种梯度计算方法：（1）自动微分通过模拟法，利用隐函数定理处理代数变量，通过分段可微积分处理事件；（2）显式离散伴随法，将前向模拟表示为事件分割残差系统，通过求解拉格朗日乘子计算梯度。

Result: 两种方法均能为前向模拟选择的事件路径提供梯度，且在固定事件顺序和横向守卫穿越条件下有效。伴随法中的残差项被明确为等式约束而非启发式惩罚项。

Conclusion: 从梯度解释、事件时间处理、实现复杂度和局部有效性四个维度对两种方法进行了比较分析，为带事件DAE系统的可微参数优化提供了理论基础和实用方案。
...
TL;DR: 提出WARDEN框架，通过f-散度模糊集动态重加权对抗样本，实现分布鲁棒的对抗训练，在降低攻击成功率的同时保持模型效用和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对抗训练方法计算成本高、难以扩展，且难以有效应对新型攻击策略。

Method: 构建f-散度模糊集动态重加权对抗样本，优化分布球内最坏情况对抗损失，通过凸对偶形式转化为log-sum-exp目标函数。

Result: WARDEN在多个LLM和攻击设置下显著降低攻击成功率，计算开销和效用损失与CAT、CAPO、MixAT等基线相当。

Conclusion: WARDEN是一种实用且可扩展的鲁棒对齐方法，在提升安全性的同时保持模型实用性和计算可行性。
...
TL;DR: 本文研究了基于提示条件生成模型的压缩感知方法，分析了提示在采样分布设计和恢复模型中的不同作用，并证明了提示匹配的Christoffel采样可保持近最优的生成压缩感知理论界，而提示不匹配会带来相容性惩罚。


<details>
  <summary>Details</summary>
Motivation: 将生成压缩感知扩展到条件版本，利用提示条件生成模型从欠采样傅里叶测量中恢复图像，并理清提示在采样和恢复中的不同作用。

Method: 使用ReLU和Lipschitz条件生成器，提出提示匹配的Christoffel采样方法，并推导稳定恢复界；通过Stable Diffusion实验验证。

Result: 提示匹配的Christoffel采样保持与近最优生成压缩感知相同的Christoffel复杂度常数；提示不匹配会引入显式的相容性惩罚；实验表明提示能有效重塑采样分布并影响图像恢复。

Conclusion: 提示应被视为具有不同传感、近似和恢复影响的设计变量。
...
TL;DR: 标准微调会导致Transformer在因果推理任务上完全崩溃（100%崩溃率），论文提出一种基于图逻辑约束的动态lambda语义损失函数，将准确率从误导性的73.9%提升至稳定且上下文依赖的70.4%（传递性任务）和68.6%（d-separation任务），相比崩溃基线提升42.7%。


<details>
  <summary>Details</summary>
Motivation: 标准微调Transformer模型进行因果推理时，模型会学习到琐碎解（如总回答"Yes"或"No"），导致因果推理能力彻底丧失，需要解决这一模型崩溃问题。

Method: 提出包含基于图的逻辑约束和动态lambda调度的语义损失函数，防止模型学习琐碎解，保持对输入结构的上下文依赖预测。

Result: 传递性任务70.4%准确率，d-separation任务68.6%准确率，相比崩溃基线提升42.7%；在1000个对抗样本上语义模型达到67-70%准确率而崩溃模型仅43-71%。

Conclusion: 语义损失对于Transformer实现稳定的因果推理是必要且不可替代的，实验基于20万+评估样本和五种模型变体验证了结论的鲁棒性。
...
TL;DR: 本文首次全面评估了图自监督学习（GSSL）在文本驱动含噪图上的鲁棒性，提出了NATD-GSSL框架，发现关系重建对噪声敏感、特征重建更鲁棒，双向关系消息传递架构更适合含噪图，相比预训练语言模型基线提升最高7%。


<details>
  <summary>Details</summary>
Motivation: 现有GSSL方法假设图是干净且人工整理的，但实际中通过NLP自动从文本提取的知识图谱含有大量真实噪声，而此前鲁棒性研究仅依赖合成扰动，缺乏对真实噪声的系统评估。

Method: 提出NATD-GSSL统一框架，包含自动图构建、图细化和GSSL三个模块；采用双图对比协议（含噪MedMentions图 vs 干净UMLS参考图），对比不同预训练任务（关系重建、特征重建、对比学习）和GNN架构（双向关系消息传递、单向关系消息传递等）的鲁棒性。

Result: 关系重建对噪声高度敏感但受益于良好定义的图模式；特征重建鲁棒性最强，性能接近干净图设置；对比学习受噪声影响较小但依赖与下游任务的对齐；双向关系消息传递GNN更适合含噪图，单向关系GNN更适合干净图；整体相比预训练语言模型基线提升最高7%。

Conclusion: NATD-GSSL为在真实含噪图上应用GSSL提供了实用指导，代码和基准已开源。
...
### [79] [GRALIS: A Unified Canonical Framework for Linear Attribution Methods via Riesz Representation](https://arxiv.org/abs/2605.05480)
*Raimondo Fanale*

Main category: cs.LG

TL;DR: GRALIS是一个统一的数学框架，通过Riesz表示定理证明了所有加性、线性和连续的归因泛函都具有唯一规范表示，统一了SHAP、IG、LIME和线性化的GradCAM，并在13.5/14个公理属性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有XAI归因方法（GradCAM、SHAP、LIME、IG）基于不同的理论基础，彼此之间无法形式化比较，缺乏统一的数学框架来理解它们的共性和差异。

Method: 提出GRALIS框架，基于Riesz表示定理建立归因的表示理论：每个加性、线性、连续的归因泛函在L^2(Q,mu)上都有唯一规范表示(Q, w, Delta)。该框架包含七个形式化定理，并通过Mobius变换建立GRALIS与Shapley交互值的对应关系。

Result: 在BreKHis数据集（1,187张组织学图像，DenseNet-121）上初步验证：删除忠实度AUC提升+0.015（恶性），96%类条件一致性，SAL=0.762±0.109，稀疏指数0.39。

Conclusion: GRALIS满足13.5/14个公理属性（现有方法仅2.5-6/14），同时保证了完整性、敏感性、局部性、阶k交互作用和最优多尺度聚合，为XAI归因方法提供了统一的理论基础。
...
### [80] [Approximate Next Policy Sampling: Replacing Conservative Target Policy Updates in Deep RL](https://arxiv.org/abs/2605.05481)
*Dillon Sandhu,Ronald Parr*

Main category: cs.LG

TL;DR: 本文提出近似下一策略采样（ANPS）方法，通过修改训练数据分布而非约束策略更新来解决强化学习中的"鸡生蛋"问题。基于ANPS的SV-API框架和SV-PPO算法在Atari和连续控制任务中表现优异，支持更大策略更新且更安全。


<details>
  <summary>Details</summary>
Motivation: 经典强化学习中的"鸡生蛋"问题：要安全改进策略，价值函数必须在更新后策略的状态访问分布上准确，但该分布未知且无法采样。传统保守更新策略以缩小更新为代价解决此问题，本文探索替代方案。

Method: 提出ANPS方法，通过修改训练数据分布使其逼近下一策略的分布，而非约束策略更新。具体实现为SV-API框架：在标准近似策略迭代中，保持目标策略固定，迭代更新的行为策略收集相关经验，满足收敛条件后才提交新策略。

Result: 基于ANPS的SV-PPO算法在高维离散（Atari）和连续控制基准测试中匹配或超越PPO性能，同时执行显著更大的目标策略更新。

Conclusion: ANPS作为解决RL中经典挑战的新方案具有可行性，在满足稳定性条件时保证更新安全，否则也不比标准近似策略迭代更差。
...
### [81] [MEMOA: Massive Mixtures of Online Agents via Mean-Field Decentralized Nash Equilibria](https://arxiv.org/abs/2605.05492)
*Xuwei Yang,David B. Emerson,Fatemeh Tavakoli,Anastasis Kratsios*

Main category: cs.LG

TL;DR: 本文提出了一种去中心化联邦学习的闭式最优策略，通过最小化最弱客户端遗憾（worst-client regret）来优化整体性能，并证明在大规模场景下该策略渐近收敛于难以直接计算的纳什最优集中式策略。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在智能体数量增加时面临计算和通信成本难以扩展的问题，而去中心化策略中每个智能体仅依赖自身状态和集合的均值场摘要（mean-field）可缓解这一瓶颈，但缺乏理论最优策略的闭式解。

Method: 基于最差客户端/最小最大（worst-client/minimax）准则，推导出唯一的闭式最优去中心化策略；采用在线加权机制优化服务器对客户端预测的混合，同时提升均值预测与最弱客户端预测。

Result: 最优去中心化策略在大规模极限下渐近收敛于纳什最优集中式策略；数值实验验证了理论保证，并表明该策略优于自然的贪心去中心化基线方法。

Conclusion: 该去中心化策略在理论上最优且可扩展，为联邦学习中的大规模智能体协同提供了高效的理论框架和实用方案。
...
### [82] [Shortcut Solutions Learned by Transformers Impair Continual Compositional Reasoning](https://arxiv.org/abs/2605.05495)
*William T. Redman,Erik C. Johnson,Brian Robinson*

Main category: cs.LG

TL;DR: 本研究扩展了LEGO框架到持续学习场景，发现前馈Transformer(BERT)学习捷径解而限制泛化，循环Transformer(ALBERT)学习类似For循环的解，在持续学习中表现更好，但两者在跨经验组合任务上均失败。


<details>
  <summary>Details</summary>
Motivation: 人类利用跨领域共同特征进行类比学习，这对持续学习至关重要，但目前缺乏对Transformer模型如何利用表征学习新相关经验的系统性理解。

Method: 将LEGO框架扩展到持续学习设置("continual LEGO")，比较前馈(BERT)和循环(ALBERT)Transformer模型在持续学习任务中的表现。

Result: BERT学习捷径解，限制泛化和前向迁移；ALBERT学习类似For循环的解，持续学习表现更好；两者在跨经验组合任务中均失败。混合训练策略可挽救ALBERT但无法挽救BERT的性能下降。

Conclusion: 循环ALBERT模型具有更适合持续学习的归纳偏置，激励未来研究Transformer架构与计算解决方案之间的相互作用。
...
### [83] [Online Localized Conformal Prediction](https://arxiv.org/abs/2605.05497)
*Yuheng Lai,Garvesh Raskutti*

Main category: cs.LG

TL;DR: 本文提出在线局部共形预测（OLCP）及其改进版OLCP-Hedge，通过结合在线自适应与协变量依赖的局部化，在非交换数据（如时间序列）下实现更窄的预测区间和有效的长期覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 现有的在线共形方法（如ACI）依赖全局校准，在协变量异质性场景下效率低下，预测区间过宽。

Method: 提出OLCP，结合在线自适应与协变量局部化；进一步提出OLCP-Hedge，将带宽选择建模为在线专家聚合问题，使用约束在线凸优化框架。

Result: 模拟和真实数据实验表明，OLCP和OLCP-Hedge在保证有效长期覆盖的同时，预测区间比现有基线更窄。

Conclusion: 所提方法在非交换数据场景下实现了更高效的共形预测，兼具理论保证和实际性能优势。
...
### [84] [Non-Myopic Active Feature Acquisition via Pathwise Policy Gradients](https://arxiv.org/abs/2605.05511)
*Linus Aronsson,Morteza Haghir Chehreghani*

Main category: cs.LG

TL;DR: 本文提出NM-PPG方法，通过连续松弛和直通式路径策略梯度，实现非短视的特征获取优化，在AFA任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有AFA方法使用得分函数策略梯度方差高，且多为短视策略，无法进行端到端优化非短视获取策略。

Method: 1) 对获取过程进行连续松弛，实现路径梯度；2) 提出直通式rollout方案，前向硬获取、反向软回传；3) 使用熵正则化与分阶段温度锐化稳定优化。

Result: 在合成与真实数据集上，NM-PDG优于现有SOTA的AFA基线方法。

Conclusion: 基于POMDP的非短视路径策略梯度方法能有效降低方差、实现端到端优化，显著提升AFA性能。
...
### [85] [OpenG2G: A Simulation Platform for AI Datacenter-Grid Runtime Coordination](https://arxiv.org/abs/2605.05519)
*Jae-Won Chung,Zhirui Liang,Yanyong Mao,Jiasi Chen,Mosharaf Chowdhury,Vladimir Dvorkin*

Main category: cs.LG

TL;DR: 本文介绍了OpenG2G，一个用于模拟AI数据中心与电网实时协调运行的仿真平台，旨在缓解AI算力增长带来的电网容量和可靠性挑战。


<details>
  <summary>Details</summary>
Motivation: AI算力需求激增和新数据中心建设给电网带来巨大压力，导致数据中心并网延迟长达数年，制约AI发展。需要理解大型数据中心对电网的影响并设计有效的协调策略。

Method: 构建OpenG2G仿真平台，采用模块化和可扩展架构：数据中心后端基于真实生产级AI服务测量数据，电网后端基于高保真电网模拟器，通过通用控制器接口闭环连接两者。支持经典、优化和基于学习的多种控制范式。

Result: OpenG2G能够回答广泛的协调问题，支持用户实现和对比不同控制范式，量化AI模型和部署选择对数据中心灵活性和协调结果的影响，并通过真实电网场景和AI工作负载验证了其实用性。

Conclusion: OpenG2G为AI数据中心与电网的运行时协调提供了有效的仿真工具，有助于缓解电网压力、加速数据中心并网，推动AI可持续发展。
...
### [86] [Bayesian Rain Field Reconstruction using Commercial Microwave Links and Diffusion Model Priors](https://arxiv.org/abs/2605.05520)
*Badr Moufad,Albina Ilina,Hai Victor Habi,Salem Lahlou,Yazid Janati,Hagit Messer,Eric Moulines*

Main category: cs.LG

TL;DR: 本文提出利用扩散模型作为空间先验，通过贝叶斯逆问题框架实现商用微波链路（CML）降雨场重建，在合成和真实数据集上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有CML降雨重建方法将CML简化为点传感器，忽略了降雨与信号衰减之间的路径积分关系，导致异构降水条件下性能下降。

Method: 将降雨场重建视为以扩散模型为高保真空间先验的贝叶斯逆问题，支持Plug-and-Play、序列蒙特卡洛、副本交换等多种免训练后验采样方法。

Result: 扩散模型相比截断高斯过程能更好地保留关键降雨统计特征，且在合成与真实数据集上均一致优于现有CML重建基线。

Conclusion: 扩散模型作为空间先验能有效解决CML路径积分观测下的降雨场重建难题，为降雨传感提供了一种性能更优的解决方案。
...
### [87] [MOSAIC: Module Discovery via Sparse Additive Identifiable Causal Learning for Scientific Time Series](https://arxiv.org/abs/2605.05524)
*Shicheng Fan,Nour Elhendawy,Jianle Sun,Ke Fang,Kun Zhang,Yihang Wang,Lu Cheng*

Main category: cs.LG

TL;DR: 提出MOSAIC模型，通过稀疏时序VAE将观测变量的可解释性迁移到因果表示学习的潜在空间，实现科学时间序列中潜在机制的模块级可解释发现。


<details>
  <summary>Details</summary>
Motivation: 因果表示学习(CRL)的潜在变量虽然具有可辨识性，但缺乏可解释性，通常需要事后与已知真实因子对齐。在科学时间序列中，底层机制未知，而观测变量本身具有语义可解释性，因此希望将观测的可解释性迁移到潜在空间。

Method: 提出MOSAIC（通过稀疏可加可辨识因果学习的模块发现），一种稀疏时序VAE，结合时序CRL可辨识性与观测变量的支持恢复。通过状态条件时序变化识别潜在变量，利用稀疏可加解码器为每个潜在变量恢复一组稀疏关联观测，实现模块级可解释性。

Result: 在RNA分子动力学、太阳风、ENSO气候、田纳西伊士曼过程以及合成托卡马克基准等多个科学时间序列数据上，MOSAIC恢复了领域一致的变量分组，实现了潜在机制的可解释发现。

Conclusion: ANOVA主效应支持在一般光滑混合函数下是可辨识的，对于稀疏可加变体提供了有限样本恢复保证。
...
### [88] [Energy Generative Modeling: A Lyapunov-based Energy Matching Perspective](https://arxiv.org/abs/2605.05530)
*Yixuan Wang,Wenqian Xue,Warren E. Dixon*

Main category: cs.LG

TL;DR: 本文提出将静态标量能量生成模型的训练与采样阶段统一到Wasserstein空间上的密度传输框架中，并将其转化为以KL散度为李雅普诺夫函数的非线性控制问题。在此框架下推导了有限步Langevin采样停止准则，证明了确定性梯度流无李雅普诺夫证书，并展示了能量函数的加性组合仍保持吉布斯不变测度与闭环李雅普诺夫性质。


<details>
  <summary>Details</summary>
Motivation: 传统静态标量能量生成模型将训练和采样视为两个独立过程，缺乏统一的理论框架。作者希望将两者统一，并引入非线性控制理论工具来分析和改进生成模型。

Method: 将密度传输问题重新表述为Wasserstein空间上的非线性控制问题，以KL散度作为李雅普诺夫函数，使训练和采样成为同一主动力学的两个实例（仅初始条件不同）。在此基础上进行理论分析并实验验证。

Result: 1) 推导出Langevin采样的有限步停止准则；2) 证明确定性梯度流不存在李雅普诺夫证书；3) 证明加性组合的标量能量仍保有明确的吉布斯不变测度和闭环李雅普诺夫证书；4) 在合成数据上验证了理论预测。

Conclusion: 该统一框架将静态标量能量生成模型与非线性控制理论工具箱连接起来，为约束生成（通过障碍函数）和加速采样（通过收缩度量）开辟了新方向。
...
TL;DR: 提出自适应Q分块（AQC）方法，通过基于折扣因子归一化的优势比较，动态选择每个状态下的最优动作分块大小，解决了固定分块大小导致的反应控制与信用分配矛盾，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线到在线强化学习中的动作分块方法在所有状态下使用固定分块大小，这在接触事件附近（需要短分块进行反应控制）和自由空间运动（需要长分块进行更好的信用分配）时是次优的。

Method: 提出自适应Q分块（AQC），通过比较每个分块大小相对于每步基线的优势，并除以折扣因子进行归一化，从而避免直接比较Q值时的折扣尺度不匹配问题和低价值状态下的噪声退化问题。

Result: 在OGBench和Robomimic基准上达到离线与在线成功率的SOTA，并能显著提升大规模VLA模型在RoboCasa-GR1任务上的性能。

Conclusion: AQC通过理论证明的优势选择器噪声免疫界和自适应分块的价值主导性，有效解决了固定分块大小的局限性，为动作分块RL提供了更优的解决方案。
...
### [90] [Accelerating LMO-Based Optimization via Implicit Gradient Transport](https://arxiv.org/abs/2605.05577)
*Won-Jun Jang,Si-Hyeon Lee*

Main category: cs.LG

TL;DR: 本文提出LMO-IGT方法，通过隐式梯度传输（IGT）加速基于线性最小化预言机（LMO）的随机优化，在保持单梯度每迭代结构的同时实现了更好的收敛复杂度，并提出了统一的分析框架和新的平稳性度量RSF。


<details>
  <summary>Details</summary>
Motivation: 现有LMO-based优化器（如Lion、Muon）经验表现强但理论理解碎片化；方差缩减虽能加速但带来额外梯度计算开销；需要一种既保持单梯度效率又能加速的理论与实践统一方法。

Method: 提出LMO-IGT方法，利用隐式梯度传输（IGT）在传输点上评估随机梯度；引入统一随机LMO优化框架和新的平稳性度量——正则化支持函数（RSF），桥接梯度范数和Frank-Wolfe gap。

Result: 标准随机LMO复杂度O(ε⁻⁴)，方差缩减LMO为O(ε⁻³)但需额外梯度，LMO-IGT达到O(ε⁻³⋅⁵)且仅用单随机梯度；实验显示LMO-IGT持续改进，Muon-IGT综合表现最强。

Conclusion: IGT提供了有效且实用的加速机制，适用于现代LMO-based优化器，在理论与实践中均取得显著提升。
...
### [91] [When Can Voting Help, Hurt, or Change Course? Exact Structure of Binary Test-Time Aggregation](https://arxiv.org/abs/2605.05592)
*Yi Liu*

Main category: cs.LG

TL;DR: 本文揭示多数投票在可交换重复正确性下并非单调，而是由潜在的正确率分布决定，存在非单调行为和无限趋势变化，并提出了有符号投票签名这一等价表征。


<details>
  <summary>Details</summary>
Motivation: 经典固定能力理论认为多数投票是单调的（超过阈值越投票越好，低于阈值越投票越差），但这一认识并不完整。作者希望揭示投票行为在更一般分布下的复杂动力学。

Method: 利用de Finetti表示将可交换重复正确性建模为潜在的正确率分布，分析不同潜变量分布下的投票曲线行为，提出有符号投票签名（signed voting signature）概念，并证明奇数预算下的投票曲线与该签名等价。

Result: 发现简单潜变量混合即可产生非单调投票曲线，甚至存在无限次趋势变化的显式构造；投票曲线无法唯一确定潜变量分布，但等价于有符号投票签名（即每个二项方差尺度上超过多数阈值的多余潜质量）；证明了曲线增量是Hausdorff有符号矩。

Conclusion: 该视角统一解释了投票曲线的形状现象、分支对称非可辨识性、可实现性、变异性和端点速率，并区分了两种估计体制：直接获得每个样本的正确概率可恢复完整签名，而固定深度的分组标签只能揭示有限前缀。
...
### [92] [Optimal Contextual Pricing under Agnostic Non-Lipschitz Demand](https://arxiv.org/abs/2605.05609)
*Jianyu Xu,Yu-Xiang Wang*

Main category: cs.LG

TL;DR: 针对带有有界支撑不可知噪声的线性估值情境化动态定价问题，提出Conservative-Markdown Redirect-UCB Pricing算法，实现$\tilde O(T^{2/3})$最优遗憾，匹配已知下界，改进先前$\tilde O(T^{3/4})$的上界。


<details>
  <summary>Details</summary>
Motivation: 现有情境化动态定价算法依赖需求曲线的跨上下文插值论证，但不可知噪声导致需求曲线可能非Lipschitz（存在任意跳跃和原子），破坏了平滑定价算法的理论基础。此前最优方法仅达到$\tilde O(T^{3/4})$遗憾，与已知下界存在缺口。

Method: Conservative-Markdown Redirect-UCB Pricing算法，结合三项关键设计：（1）随机化参数估计；（2）保守残差网格探测；（3）基于置信度的一步重定向。

Result: 算法达到$\tilde O(T^{2/3})$遗憾，匹配Kleinberg和Leighton (2003)已知下界（仅差对数因子），优于Xu和Wang (2022)的$\tilde O(T^{3/4})$上界。在随机良态上下文条件下，闭合了线性估值情境化定价在不可知非Lipschitz噪声分布下的长期遗憾缺口。

Conclusion: 该工作证明了在非光滑需求曲线情境下，随机化探测与重定向机制能够有效克服非Lipschitz噪声带来的理论困难，达成了问题的最优遗憾界。
...
### [93] [LLMSpace: Carbon Footprint Modeling for Large Language Model Inference on LEO Satellites](https://arxiv.org/abs/2605.05615)
*Lei Jiang,Adrian Ildefonso,Daniel Loveless,Fan Chen*

Main category: cs.LG

TL;DR: 本文提出LLMSpace，首个针对低轨卫星上大语言模型推理的碳足迹建模框架，综合考量运营碳排、嵌入碳排、硬件辐射加固及LLM工作负载特性，揭示卫星算力与碳排放之间的关键权衡。


<details>
  <summary>Details</summary>
Motivation: LLM推理带来快速增长的地球能源消耗和碳排放问题，现有研究缺乏对太空AI推理全生命周期碳足迹的系统建模，尤其是卫星发射、制造和辐射加固硬件带来的额外碳排放未被充分考虑。

Method: 提出LLMSpace碳建模框架，联合建模运营碳排与嵌入碳排、外围子系统、辐射加固的加速器和内存，以及LLM特有的预填-解码行为和token生成等负载特征，使用真实卫星与GPU配置进行实验。

Result: 揭示了碳足迹、推理延迟、硬件设计与运行寿命之间在太空LLM推理场景下的关键权衡关系。

Conclusion: LLMSpace为可持续的太空LLM推理提供了系统性碳评估工具，有助于指导未来太空AI基础设施的低碳设计。
...
### [94] [Region-adaptable retrieval of coastal biogeochemical parameters from near-surface hyperspectral remote sensing reflectance using physics-aware meta-learning](https://arxiv.org/abs/2605.05623)
...
TL;DR: 本文提出了一种两阶段物理感知元学习框架，用于从近地表高光谱遥感反射率观测中反演沿海水质生物地球化学参数。第一阶段利用生物光学前向模型生成合成数据集预训练区域无关的基础模型，第二阶段针对特定区域用本地样本微调。在澳大利亚五个沿海站点验证，该方法优于五个基准模型。


<details>
  <summary>Details</summary>
Motivation: 高光谱原位感知在反演水质参数方面有潜力，但不同水域间算法泛化困难，因为环境条件和生物地球化学差异导致遥感反射率与生物地球化学参数关系因区域而异。

Method: 两阶段物理感知元学习框架：第一阶段利用生物光学前向模型和原位生物光学光谱库生成大合成数据集，预训练区域无关基础模型；第二阶段用局部样本微调特定区域模型。

Result: (1)各实验站点BGC参数与高光谱Rrs特征存在明显区域差异；(2)合成数据集物理合理，与实际样本在参数分布和参数间相关性上高度一致；(3)所提方法在BGC反演中优于五个基准模型；(4)原位测量与模型预测的时间序列在幅度和动态变化上高度一致。

Conclusion: 提出的两阶段物理感知元学习框架能够有效解决跨水域水质参数反演的泛化问题，对沿海水质经济高效监测具有重要价值。
...
### [95] [Information-Preserving Domain Transfer with Unlabeled Data in Misspecified Simulation-Based Inference](https://arxiv.org/abs/2605.05652)
*Joon Jang,Eunho Jeong,Kyu Sung Choi,Hyeonjin Kim*

Main category: cs.LG

TL;DR: SPIN是一个基于仿真的推理框架，通过保留参数相关信息的域迁移技术，在模型错误指定情况下利用无标签真实数据提升后验推断的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有SBI方法在模型错误指定时可靠性下降，而基于无标签真实数据的方法仅实现边缘分布对齐，未能保留参数相关的重要信息。

Method: 提出SPIN框架，通过将带标签的模拟数据向真实域迁移并映射回模拟域，利用原始标签约束域迁移过程以保留参数相关互信息。测试时用学到的真实到模拟的映射进行后验推断。

Result: 在合成和物理真实基准上，SPIN显著提升了真实世界的后验推断性能，且模型错误指定越严重，改进效果越明显。

Conclusion: SPIN通过信息保持的域迁移有效解决了模型错误指定下的SBI问题，无需真实参数标签或配对数据。
...
### [96] [Distributionally Robust Multi-Objective Optimization](https://arxiv.org/abs/2605.05660)
*Yufeng Yang,Fangning Zhuo,Ziyi Chen,Heng Huang,Yi Zhou*

Main category: cs.LG

TL;DR: 本文提出了分布鲁棒多目标优化（DR-MOO）框架，通过将分布偏移纳入多目标优化，设计了双循环和单循环两种多梯度下降算法，分别达到O(ε^{-12})和O(ε^{-4})的样本复杂度，实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有MOO方法没有考虑数据分布偏移，而实际应用中分布偏移普遍存在，因此需要更鲁棒的优化框架。

Method: 提出DR-MOO框架和Pareto类型解概念；设计了基于拉格朗日对偶的双循环MGDA（内环估计对偶变量）；进一步引入梯度裁剪，提出单循环双裁剪MGDA，无需双重采样且处理广义光滑和有偏梯度。

Result: 双循环MGDA样本复杂度O(ε^{-12})；单循环双裁剪MGDA样本复杂度O(ε^{-4})；实验与SOTA MGDA基线相比具有竞争力。

Conclusion: DR-MOO框架有效解决了带分布偏移的多目标优化问题，所提算法在非凸设定下具有理论保证和实际竞争力。
...
### [97] [Budgeted Attention Allocation: Cost-Conditioned Compute Control for Efficient Transformers](https://arxiv.org/abs/2605.05697)
*Amrit Nidhi*

Main category: cs.LG

TL;DR: 本文提出预算注意力分配机制，通过门控方法在一个检查点上实现不同注意力预算下的精度-速度权衡，在多个NLP任务上验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 部署系统通常需要多个成本-质量运行点，但Transformer每个训练模型只有一个推理成本，缺乏灵活性。

Method: 提出Budgeted Attention Allocation，一种基于单调头门控机制、根据请求的注意力预算进行调节的方法，采用稠密热启动确保稳定性。

Result: 在合成序列任务上达到99.7%精度(0.303成本)和100.0%精度(0.504成本)；AG News上BERT-Mini达到87.6%精度、1.20x加速；DBpedia14上97.4%精度(预算0.50) vs 密集全注意力的96.6%。

Conclusion: 该方法并非普遍最优，但提供了一个可控检查点跨预算的可行复现研究，可将注意力成本转化为可测量的CPU加速。
...
### [98] [On the Blessing of Pre-training in Weak-to-Strong Generalization](https://arxiv.org/abs/2605.05710)
*Wei Yao,Wang Zhaoyang,Gengze Xu,Chen Qian,Dongrui Liu,Ziqiao Wang,Yong Liu,Yunbei Xu*

Main category: cs.LG

TL;DR: 本文从理论和实验角度证明，预训练是弱到强泛化（W2SG）出现的必要先决条件，W2SG并非模型固有属性，而是随预训练进程通过相变涌现。


<details>
  <summary>Details</summary>
Motivation: 现有W2SG研究忽视了预训练在其中的决定性作用，作者希望从理论和实验上填补这一空白。

Method: 在高维单指标模型框架下形式化W2SG问题，将预训练建模为谱初始化步骤，证明当预训练提供几何暖启动时W2SG可实现，并推导泛化界。通过合成数据和大规模LLM中间检查点进行实验验证。

Result: 预训练能提供使模型进入"有效区域"的几何暖启动，在此区域内出现先提升后受限于弱监督偏置的泛化曲线。大规模实验表明W2SG通过相变涌现，与预训练进程紧密耦合。

Conclusion: 预训练是W2SG涌现的必要前提，W2SG不是模型的先天能力，而是随预训练进展逐步获得的。
...
### [99] [Enabling Federated Inference via Unsupervised Consensus Embedding](https://arxiv.org/abs/2605.05718)
*Yui Hashimoto,Takayuki Nishio,Yuichi Kitagawa,Takahito Tanimura*

Main category: cs.LG

TL;DR: 提出CE-FI框架，在无需共享模型参数、原始输入或公共编码器的前提下，实现预训练模型间的协同推理，仅需共享无标签数据训练共识嵌入层和协作输出层。


<details>
  <summary>Details</summary>
Motivation: 现有协同推理框架通常依赖共享输入数据、模型参数或公共编码器，限制了其在隐私敏感或跨组织场景中的应用。

Method: CE-FI包含两个组件：共识嵌入（CE）层将异构中间表示映射到公共嵌入空间；协作输出（CO）层从这些嵌入生成预测。两层仅使用共享无标签数据训练。

Result: 在CIFAR-10/100非IID条件下，CE-FI优于单模型推理，性能与需要更强共享假设的传统方法相当；在文本和时间序列任务上也展现适用性，但性能依赖集成策略。

Conclusion: CE-FI在保护隐私的前提下实现了有效的模型间协作，但表示对齐仍是主要瓶颈。
...
### [100] [CRAFT: Forgetting-Aware Intervention-Based Adaptation for Continual Learning](https://arxiv.org/abs/2605.05732)
*Md Anwar Hossen,Fatema Siddika,Juan Pablo Munoz,Tanya Roosta,Ali Jannesari*

Main category: cs.LG

TL;DR: CRAFT是一个通过低秩表示干预而非更新权重来实现LLM持续学习的框架，利用KL散度统一路由、正则化和合并，在多个基准上优于LoRA方法并减少遗忘。


<details>
  <summary>Details</summary>
Motivation: LLM微调获取新能力时会导致灾难性遗忘，现有方法难以平衡新任务学习与旧知识保留。

Method: 三阶段框架：1)基于输出分布散度的任务路由；2)KL散度正则化微调控制遗忘；3)基于KL信号合并干预到共享表示。统一使用KL散度目标。

Result: 在多个基准和模型规模上优于强LoRA方法，减少了灾难性遗忘，且对任务顺序鲁棒。

Conclusion: 以输出空间散度引导表示空间中的适应控制，为LLM持续学习提供了可扩展且原则性的方法。
...
### [101] [Multi-Dimensional Behavioral Evaluation of Agentic Stock Prediction Systems Using LLM Judges with Closed-Loop Reinforcement Learning Feedback](https://arxiv.org/abs/2605.05739)
*Mohammad Al Ridhawi,Mahtab Haj Ali,Hussein Al Osman*

Main category: cs.LG

TL;DR: 提出一种行为评估框架，通过LLM裁判对智能股票预测系统的中间决策（如制度检测、路径路由、强化学习控制）进行细粒度评分，并将行为缺陷转化为SAC奖励的惩罚项，经微调后在2017-2025测试集上MAPE降低11.5%、方向准确率提升至74%、夏普比率提升18%。


<details>
  <summary>Details</summary>
Motivation: 现有智能股票预测系统依赖MAPE、方向准确率等聚合指标评估性能，但无法揭示制度检测、路径路由、强化学习控制等中间决策步骤的个体质量，导致系统行为层面的缺陷被掩盖。

Method: 1) 在每个自主决策点记录行为痕迹，按五天为一个周期分组；2) 沿六个领域特定维度（制度检测、路由、适应、风险校准、策略连贯性、错误恢复）由三个LLM裁判（GPT 5.4、Claude 4.6 Opus、Gemini 3.1 Pro）集成评分；3) 将短板维度的评分转化为信用分配惩罚项，加入SAC奖励函数；4) 在验证期内进行三轮微调。

Result: 420个周期的扰动验证显示目标维度得分下降-1.6至-2.4，其余五个维度平均仅下降-0.32，跨模型一致性达Krippendorff's α=0.85。行为综合得分与20天夏普比率相关性ρ=0.72。微调后测试集上：MAPE从0.61%降至0.54%（相对降低11.5%，p<0.001，Cohen's d=0.31），方向准确率从71%提升至74%，夏普比率提升18%（95% Bootstrap CI [8.2%, 27.4%]），增益集中于高波动周期。

Conclusion: 行为评估框架能有效揭示并修复智能股票预测系统的细粒度决策缺陷，通过LLM裁判引导的奖励塑形显著提升系统在高波动场景下的表现，但结果基于离线回测，尚需验证实时部署效果。
...
### [102] [Weak-to-Strong Generalization is Nearly Inevitable (in Linear Models)](https://arxiv.org/abs/2605.05742)
*Scott Geng,Dutch Hansen,Jerry Li*

Main category: cs.LG

TL;DR: 本文证明弱到强泛化现象在标准线性逻辑回归中普遍存在，即使学生模型与教师模型容量相同，这一现象也几乎必然发生，挑战了现有理论中关于模型容量差异是核心机制的观点。


<details>
  <summary>Details</summary>
Motivation: 理解弱到强泛化现象的机制，检验现有理论中模型容量差异是否为必要条件。

Method: 在标准线性逻辑回归框架下，设置温和的数据分布假设，系统分析弱到强泛化发生的条件。

Result: 弱到强泛化在大多数学生-教师配对中都会发生，且学生模型无需比教师模型更具表达能力。

Conclusion: 弱到强泛化在简单线性模型中几乎不可避免，模型容量差异并非其核心机制。
...
### [103] [RVPO: Risk-Sensitive Alignment via Variance Regularization](https://arxiv.org/abs/2605.05750)
*Ivan Montero,Tomasz Jurczyk,Bhuwan Dhingra*

Main category: cs.LG

TL;DR: 针对批评者less RLHF方法因算术平均聚合多目标奖励导致约束忽视的问题，提出奖励方差策略优化（RVPO），通过惩罚奖励间方差实现风险敏感的多目标对齐，在医学和科学推理任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有无批评者RLHF方法使用算术平均聚合多目标奖励，容易导致约束忽视——某一目标的高分可能掩盖其他目标（如安全、格式）的严重失败，无法可靠实现多目标对齐。

Method: 提出奖励方差策略优化（RVPO），在优势聚合时惩罚奖励间方差，将优化目标从"最大化总和"改为"最大化一致性"。通过泰勒展开证明LogSumExp（SoftMin）算子可充当平滑的方差惩罚项。

Result: 在包含多达17个LLM评判奖励信号的医疗和科学推理任务（Qwen2.5-3B/7B/14B）以及基于规则约束的工具调用任务（Qwen2.5-1.5B/3B）上，RVPO在HealthBench上达到0.261（GDPO为0.215，p<0.001），在GPQA-Diamond上保持竞争力且无后期退化。
...
### [104] [Adaptive Selection of LoRA Components in Privacy-Preserving Federated Learning](https://arxiv.org/abs/2605.05769)
*Myoungjun Kim,Sangwoo Park,Yoseob Han,Jin-Hyun Ahn*

Main category: cs.LG

TL;DR: 提出AS-LoRA自适应框架，通过层粒度自由选择、轮次自适应更新和曲率感知评分，解决了差分隐私联邦微调中LoRA的聚合误差问题，在不增加隐私成本的情况下提升精度并降低聚合开销。


<details>
  <summary>Details</summary>
Motivation: DP联邦微调中LoRA因乘法结构产生聚合误差，DP噪声进一步放大该误差，导致稳定性与精度下降；现有方法在所有层和所有通信轮次使用统一更新模式，忽略了LoRA两个因子间的结构不对称性及训练动态。

Method: AS-LoRA框架包含三个维度：(1) 层粒度自由——每层独立选择活跃组件；(2) 轮次自适应——选择策略随通信轮次动态更新；(3) 曲率感知分数——基于损失二阶近似的评分机制。理论上消除了层绑定调度的重构误差下限，加速收敛，隐含地偏向平坦极小值，且不增加额外隐私成本。

Result: 在GLUE、SQuAD、CIFAR-100、Tiny-ImageNet上，严格DP预算和非IID分区下，AS-LoRA相比联邦LoRA基线在GLUE上提升最高+7.5个百分点，MNLI-mm上+12.5个百分点；与SVD聚合方法相比，聚合成本降低33-180倍，通信开销可忽略。

Conclusion: AS-LoRA通过自适应更新策略有效缓解了DP联邦微调中LoRA的聚合误差问题，在精度、稳定性和计算效率上均优于现有方法。
...
### [105] [A Measure-Theoretic Finite-Sample Theory for Adaptive-Data Fitted Q-Iteration](https://arxiv.org/abs/2605.05791)
*Manuel Haussmann,Mustafa Mert Çelikok,Melih Kandemir*

Main category: cs.LG

TL;DR: 本文构建了一个统一的拟合Q迭代（FQI）理论框架，将测度论MDP、确定性误差传播和PAC泛化三大传统整合，首次在一般可测波雷尔空间上给出了FQI的有限样本自适应数据性能界、序贯Rademacher复杂度控制的Bellman回归泛化界，以及连续空间FQI的累积路径在线遗憾保证。


<details>
  <summary>Details</summary>
Motivation: 当前模型无关的无模型深度强化学习在启发式上取得了巨大成功，但其理论基础局限于表格或可线性化场景，缺乏对一般连续空间上学习过程的统一理论分析。本文旨在弥合三大传统之间的鸿沟，为现代深度RL算法提供严格的理论基础。

Method: 通过将测度论概率与Banach空间中的Bellman算子压缩相结合，利用序贯Rademacher复杂度来控制策略依赖数据收集下的Bellman回归泛化误差，并扩展至在线场景下的累积累径遗憾分析。

Result: 首次给出了FQI在一般可测波雷尔空间上的有限样本自适应数据性能界，证明了序贯Rademacher复杂度可控制Bellman回归泛化，并提供了连续空间FQI的第一个累积路径在线遗憾保证。

Conclusion: 该统一框架为现代深度RL算法的形式化分析奠定了必要基础，弥合了理论分析与启发式成功之间的差距。
...
### [106] [Revealing Modular Gradient Noise Imbalance in LLMs: Calibrating Adam via Signal-to-Noise Ratio](https://arxiv.org/abs/2605.05794)
*Ziqing Wen,Zhouyang Liu,Jiahuan Wang,Ping Luo,Li Shen,Dongsheng Li,Tao Sun*

Main category: cs.LG

TL;DR: 本文提出MoLS方法，通过估计模块级信噪比（SNR）自动调整Adam优化器的学习率，无需手动调参，在LLM训练中加速收敛并提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的结构异质性导致不同模块梯度差异大，Adam等自适应优化器虽提供逐参数适应性，但未显式处理模块级梯度异质性，导致收敛慢、性能次优或训练不稳定。现有手动调整模块学习率的方法成本高且难以泛化。

Method: 提出MoLS（Module-wise Learning Rate Scaling via SNR），通过估计每个模块的信噪比（SNR）来缩放Adam的更新量，实现模块级学习率的自动化分配，无需人工调参。

Result: 在多个LLM训练基准上，MoLS提升了收敛速度和泛化能力，达到了与精心调参的模块级学习率相当的性能，且兼容内存高效的训练算法。

Conclusion: MoLS提供了一种更 principled 的模块级自适应优化方法，有效解决LLM训练中的梯度异质性问题，兼具自动化与高性能。
...
### [107] [Selective Rollout: Mid-Trajectory Termination for Multi-Sample Agent RL](https://arxiv.org/abs/2605.05802)
*Zhiyuan Zhai,Xin Wang*

Main category: cs.LG

TL;DR: 针对GRPO训练中零方差组浪费计算资源的问题，提出基于部分轨迹编辑距离的提前终止门控机制，在ALFWorld上实现10.7%加速并提升2.5个百分点的成功率。


<details>
  <summary>Details</summary>
Motivation: GRPO在每个训练提示上采样多个并行 rollout，但当同一组内所有 rollout 获得相同奖励时（实践中约占40%），梯度贡献为零，额外计算完全浪费。现有过滤方法要么在 rollout 完成后才处理，要么在开始前决定，均未利用 rollout 过程中产生的信息。

Method: 提出单参数门控机制：在中间步骤计算组内部分动作序列的平均成对编辑距离，若低于阈值则提前终止该组的所有 rollout。

Result: 在ALFWorld上用Qwen2.5-7B进行60轮GRPO训练，平均4个随机种子，门控版本墙钟时间加快10.7%（bootstrap 95% CI排除0），50个未见任务的成功率提升2.5个百分点。

Conclusion: 通过中间步骤的轨迹分歧度预测零方差组并提前停止，既节省计算又通过减少零优势梯度批次稀释来提升最终性能。
...
### [108] [Retrieval from Within: An Intrinsic Capability of Attention-Based Models](https://arxiv.org/abs/2605.05806)
*Elad Hoffer,Yochai Blau,Ron Banner,Daniel Soudry,Boris Ginsburg*

Main category: cs.LG

TL;DR: 提出INTRA框架，利用注意力机制让编码器-解码器模型从自身内部表征中直接检索证据，统一了检索与生成。


<details>
  <summary>Details</summary>
Motivation: 传统RAG将检索和生成分离，存在检索器与生成器不匹配的问题，且编码计算无法复用。

Method: INTRA框架：解码器注意力查询对预编码的证据块进行评分，并直接复用为生成上下文，统一检索与生成，复用编码器状态。

Result: 在问答基准上，INTRA在证据召回和端到端答案质量上均优于强工程化的RAG检索流水线。

Conclusion: 基于注意力的模型本身已具备可被调用的检索机制，无需作为外部模块添加。
...
### [109] [A Testable Certificate for Constant Collapse in Teacher-Guided VAEs](https://arxiv.org/abs/2605.05813)
*Zegu Zhang,Jianhua Peng,Jian Zhang*

Main category: cs.LG

TL;DR: 本文研究了VAE后验坍塌中的输入无关常数坍塌模式，提出可通过教师互信息I_T(X;T)作为常数学生的对齐损失阈值，若原始见证的损失低于该值则可证明其非恒定，实现了可量化的坍塌边界检测。在CIFAR-100和Tiny-ImageNet-200上的实验验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有后验坍塌检测依赖KL项小、解码器强等间接症状，缺乏明确的坍塌边界定义。作者希望将定性失效模式转化为可量化的检测标准。

Method: 提出教师-学生框架：定义非恒定教师分布T(·|x)，最佳常数学生为数据集平均教师分布，其对齐损失等于教师互信息I_T(X;T)。若原始见证（latent-only）的损失低于该值一定安全裕度，则证明该见证非输入恒定。

Result: CIFAR-100实验：完整训练保持在认证边界安全侧；移除对齐使原始见证进入常数学生区域；从坍塌检查点重启并启用对齐可恢复认证。Tiny-ImageNet-200实验呈现相同的预防-坍塌-恢复模式。标准VAE基线（包括保持重建质量或事后可预测性的方法）在原始认证下均为负。

Conclusion: 该方法提供了一种窄但有保证的认证：证明匹配的非恒定教师相对变化通过了潜在路径，而非声称排除了所有形式的后验坍塌。
...
### [110] [MDN: Parallelizing Stepwise Momentum for Delta Linear Attention](https://arxiv.org/abs/2605.05838)
*Yulong Huang,Xiang Liu,Hongxiang Huang,Xiaopeng Lin,Zunchang Liu,Xiaowen Chu,Zeke Xie,Bojun Cheng*

Main category: cs.LG

TL;DR: 本文提出Momentum DeltaNet (MDN)，通过动量SGD和块并行算法改进线性注意力模型，在400M和1.3B参数规模上超越Transformer、Mamba2等基线。


<details>
  <summary>Details</summary>
Motivation: 线性注意力模型（如Mamba2、GDN）将线性递归解释为在线SGD，但朴素SGD更新存在信息快速衰减和收敛次优问题，动量优化器难以兼顾训练效率与效果。

Method: 提出带逐步骤动量规则的块并行算法，通过几何重排更新系数；从动力系统视角分析动量递归为二阶系统，设计稳定门控约束；基于Triton内核实现。

Result: 在400M和1.3B参数模型上，MDN在多项下游评估中持续优于Transformer、Mamba2、GDN等强基线，训练吞吐量与Mamba2、KDA相当。

Conclusion: 动量机制能有效提升线性注意力模型的长序列建模能力，且不牺牲训练效率，为线性注意力范式提供了新的改进方向。
...
### [111] [Hypothesis generation and updating in large language models](https://arxiv.org/abs/2605.05851)
*Hua-Dong Xiong*

Main category: cs.LG

TL;DR: 本文研究了LLM在数字游戏中的推理能力，发现LLM的行为可以用带偏见的贝叶斯模型描述，但存在系统性偏差：默认模式下有强采样假设和奥卡姆剃刀倾向，思考模式则更依赖先验。同时发现评估-生成差距，且这种贝叶斯模式无法外推到未观察到的假设领域。


<details>
  <summary>Details</summary>
Motivation: 探究LLM如何从部分描述中生成假设并随着新信息更新，以及这种推理过程是否接近最优。

Method: 在数字游戏（从正整数中推断规则）中，使用三种互补探针（后验预测、假设评估、假设生成）测量LLM的后验分布，并与最优贝叶斯模型和人类行为进行比较。

Result: LLM行为可用双参数贝叶斯模型拟合，但存在系统性偏差；默认模式下有强采样假设和隐式奥卡姆剃刀效应，思考模式更依赖先验；存在评估-生成差距；模型无法外推到假设领域未覆盖的部分。

Conclusion: LLM作为通用问题解决者存在局限性，尤其是在需要假设超越数据本身的科学推理中表现不佳。
...
### [112] [Measuring Learning Progress via Gradient-Momentum Coupling](https://arxiv.org/abs/2605.05856)
*Samuel Blad,Martin Längkvist,Amy Loutfi*

Main category: cs.LG

TL;DR: 本文提出梯度-动量耦合（GMC）信号，通过测量每个样本梯度与历史梯度动量的归一化绝对乘积，来量化样本对持续学习的贡献，从而替代传统预测误差作为好奇心驱动探索的信号，在噪声鲁棒性和课程学习方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统好奇心驱动探索中常用的预测误差信号无法区分有意义的可学习模式与随机噪声，需要一种更鲁棒的信号来度量学习进展。

Method: 提出GMC信号，通过计算每个参数上当前梯度与历史动量的归一化绝对乘积，利用动量对噪声和振荡的自然过滤特性，识别出对参数更新有贡献的样本。

Result: 在控制实验中，GMC表现出噪声鲁棒性和涌现的课程学习能力，能按学习速度而非难度优先排序任务；在MiniGrid实验中，用GMC替换传统好奇心架构中的预测误差可提升对观测噪声的鲁棒性。

Conclusion: GMC作为一种基于优化动力学的学习进展信号，能更有效地引导好奇心驱动探索，在存在噪声的环境中优于传统的预测误差方法。
...
TL;DR: 提出近策略蒸馏（NPD）框架，通过异步解耦学生生成与训练，结合稀疏更新和Δ-IFD过滤机制，在避免昂贵RL计算的同时实现高效知识蒸馏，取得8.1倍加速和性能提升。


<details>
  <summary>Details</summary>
Motivation: 自回归模型的标准知识蒸馏常面临分布不匹配问题，而在线策略方法虽能缓解但依赖计算昂贵的强化学习框架，效率低下。

Method: 提出NPD框架，异步解耦学生生成与训练，支持序列打包的SFT；引入稀疏学生更新和Δ-IFD启发式样本选择机制，过滤极端离群样本以稳定优化轨迹。

Result: 相比在线策略基线实现8.1倍加速，相比SFT提升8.09%；openPangu-Embedded-1B达到68.73%的SOTA分数，超越更大规模的Qwen3-1.7B。

Conclusion: NPD通过缩小后续RL的探索空间，在效率与性能之间取得良好平衡，使小模型超越大模型。
...
### [114] [Offline Reinforcement Learning for Rotation Profile Control in Tokamaks](https://arxiv.org/abs/2605.05857)
*Rohit Sonker,Hiro Josep Farre Kaga,Jiayu Chen,Andrew Rothstein,Ian Char,Ricardo Shousha,Egemen Kolemen,Jeff Schneider*

Main category: cs.LG

TL;DR: 本文研究了利用离线强化学习和基于模型的离线强化学习算法，仅基于DIII-D托卡马克历史数据训练等离子体旋转剖面控制策略，并在真实装置上取得了初步成功。


<details>
  <summary>Details</summary>
Motivation: 托卡马克中控制等离子体旋转剖面是一个高维度、多输入多输出的复杂控制问题，传统方法难以解决，而强化学习虽有潜力，但缺乏精确的仿真器来训练策略。

Method: 采用离线RL和离线模型-based RL算法，利用概率模型模拟等离子体动力学生成滚动轨迹，仅使用DIII-D托卡马克的历史数据进行训练。

Result: 训练的策略成功部署在DIII-D托卡马克上，并观察到了有希望的真实世界实验结果。

Conclusion: 在仅使用有限历史数据的情况下，在复杂物理装置上训练和部署强化学习策略是可行的，但仍存在关键挑战需要解决。
...
### [115] [Do Neural Operators Forget Geometry? The Forgetting Hypothesis in Deep Operator Learning](https://arxiv.org/abs/2605.05862)
*Yanming Xia,Angelica I. Aviles-Rivero*

Main category: cs.LG

TL;DR: 本文发现神经算子在处理不规则几何时存在深度方向上的几何遗忘问题，并提出一种轻量级的几何记忆注入机制来缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 神经算子在结构化域上表现良好，但在不规则几何上的行为尚未被充分理解；作者发现这不仅是编码问题，而是深度算子架构固有的失效模式。

Method: 提出几何遗忘假说（Geometric Forgetting Hypothesis），通过逐层几何探测实验验证谱算子和注意力算子均系统性丢失几何保真度；并提出轻量级几何记忆注入机制（geometry memory injection），在中间深度恢复几何约束。

Result: 几何遗忘会降低精度、稳定性和泛化能力；所提出的干预措施能一致缓解遗忘，并揭示了基于Transformer的算子存在几何捷径不稳定性。

Conclusion: 几何保持是神经算子架构的结构性需求，而非可选的的设计选择。
...
### [116] [SOPE: Stabilizing Off-Policy Evaluation for Online RL with Prior Data](https://arxiv.org/abs/2605.05863)
*Carlo Romeo,Girolamo Macaluso,Alessandro Sestini,Andrew D. Bagdanov*

Main category: cs.LG

TL;DR: SOPE利用离线策略评估信号作为自动早停机制，动态控制离线训练阶段长度，在25个连续控制任务上最高提升45.6%性能并降低22倍计算量。


<details>
  <summary>Details</summary>
Motivation: 现有在线强化学习中融合先验数据的方法要么计算成本高，要么需要手动调参固定长度的稳定化阶段，容易浪费先验知识或导致过拟合。

Method: 提出SOPE算法，使用与actor对齐的离线策略评估（OPE）信号作为自动早停机制，在验证集上评估critic，当分布外收益饱和时停止梯度更新。

Result: 在Minari基准的25个连续控制任务上，性能最高提升45.6%，计算量（TFLOPs）最高减少22倍。

Conclusion: 自适应、评估驱动的更新策略比静态、穷举的更新策略更有效，能够更好地平衡样本效率与计算效率之间的权衡。
...
### [117] [QuadraSHAP: Stable and Scalable Shapley Values for Product Games via Gauss-Legendre Quadrature](https://arxiv.org/abs/2605.05870)
*Majid Mohammadi,Grigory Reznikov,Pavel Sinitcyn,Krikamol Muandet,Siu Lun Chau*

Main category: cs.LG

TL;DR: 本文提出了一种针对乘积博弈的Shapley值高效计算方法QuadraSHAP，通过将原始加权和转化为一维积分，并利用Gauss-Legendre求积实现精确或近指数级精确的近似计算，同时实现了O(d m_q)的工作量和O(log d)的并行时间。


<details>
  <summary>Details</summary>
Motivation: 在机器学习可解释性中，当价值函数具有乘积结构时（如乘积核的核方法、基于树的模型），传统的Shapley值计算面临指数级特征组合的复杂度问题，亟需高效精确的计算方法。

Method: 证明了乘积博弈中每个玩家的Shapley值可以表示为[0,1]上d-1次多项式的一维积分，进而采用Gauss-Legendre求积方案：当节点数m_q ≥ ⌈d/2⌉时精确成立，否则误差几何级数衰减；通过对数空间评估实现数值稳定，并基于关联扫描原语实现O(d m_q)工作量和O(log d)并行时间的并行实现。

Result: 实验表明，QuadraSHAP在所有测试配置中是最快的数值稳定方法，仅需数百个节点即可在数千特征上获得高精度估计。

Conclusion: 该工作为乘积结构下的Shapley值计算提供了理论保证且高效实用的解决方案，填补了该领域在精确积分表示和数值稳定性方面的空白。
...
### [118] [RepFlow: Representation Enhanced Flow Matching for Causal Effect Estimation](https://arxiv.org/abs/2605.05890)
*Yifei Xie,Jian Huang*

Main category: cs.LG

TL;DR: 本文提出RepFlow框架，将表征学习与条件流匹配（CFM）结合，通过最小化熵正则化Wasserstein距离缓解选择偏差，并引入L2归一化约束，在点估计和分布估计上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有因果效应估计方法主要局限于点估计，缺乏分布建模能力，且存在缺失反事实和选择偏差等根本性挑战。

Method: 提出RepFlow框架，将因果效应估计建模为联合优化问题，融合表征学习与条件流匹配（CFM）；通过最小化熵正则化Wasserstein距离缓解选择偏差；对潜在表征施加L2归一化约束以增强数值稳定性。

Result: 在多个基准数据集上，RepFlow在点估计和分布因果效应估计两方面均一致优于现有方法。

Conclusion: RepFlow通过表征学习与条件流匹配的有机结合，有效实现了因果效应的分布建模，且性能显著优于已有方法。
...
### [119] [VARS-FL: Validation-Aligned Client Selection for Non-IID Federated Learning in IoT Systems](https://arxiv.org/abs/2605.05896)
*Mohamed Lakas,Mohamed Amine Ferrag*

Main category: cs.LG

TL;DR: VARS-FL是一种基于验证对齐信誉评分的客户端选择框架，通过评估客户端对服务器验证损失减少的贡献，结合滑动窗口平均和参与度对数缩放，实现高效的探索-利用选择，在非IID IoT入侵检测任务上相比FedAvg、Oort和Power-of-Choice方法显著提升精度、F1分数和收敛速度（最多减少36%的通信轮次）。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习使用无状态的客户端选择，忽略历史贡献信息，在非IID数据下收敛慢且训练不稳定，尤其是IoT/IIoT环境中数据高度异构。

Method: VARS-FL框架：1）基于每次更新对服务器验证损失的减少量量化客户端贡献；2）通过滑动窗口平均和参与度对数缩放构建信誉分数；3）利用该分数进行探索-利用的客户端选择。

Result: 在Edge-IIoTset数据集（15类非IID入侵检测，100客户端）上，VARS-FL相比FedAvg、Oort和Power-of-Choice，在准确率、F1-Macro和损失方面均一致提升，收敛速度提升达36%（达到80%准确率所需轮次减少36%）。

Conclusion: 验证对齐的、历史感知的客户端选择能够为异构IoT环境下的联邦学习提供更可靠和高效的训练过程。
...
### [120] [VisMMOE: Exploiting Visual-Expert Affinity for Efficient Visual-Language MoE Offloading](https://arxiv.org/abs/2605.05899)
*Cheng Xu,Xiaofeng Hou,Jiacheng Liu,Chao Li*

Main category: cs.LG

TL;DR: VisMMoE通过剪枝冗余视觉令牌重塑专家访问模式，在内存受限平台上实现VL-MoE模型的高效部署，性能提升最高达2.68倍。


<details>
  <summary>Details</summary>
Motivation: 现有MoE卸载系统主要面向文本密集型负载设计，在处理视觉密集型输入时效果不佳，因为大量视觉令牌会导致更广泛且不可预测的专家访问模式。

Method: 提出视觉-专家亲和力概念，结合亲和力感知的令牌压缩、前瞻专家预测以及缓存/流水线编排，通过剪枝冗余视觉令牌来集中和稳定专家访问模式。

Result: 在多个框架和代表性VL-MoE模型上，端到端推理性能相比强基线提升最高2.68倍和1.61倍，同时保持有竞争力的准确率。

Conclusion: 视觉令牌剪枝不仅能减少计算量，还能通过重塑专家需求模式来提升卸载效率，这一发现为VL-MoE在内存受限平台上的部署提供了有效方案。
...
### [121] [Contrastive Identification and Generation in the Limit](https://arxiv.org/abs/2605.06211)
*Xiaoyu Li,Andi Han,Jiaojiao Jiang,Junbin Gao*

Main category: cs.LG

TL;DR: 本文首次研究了极限对比识别与生成问题，学习者仅通过观察未标记的正负例对（满足h(x)≠h(y)）来学习。无噪声下给出了可对比识别的精确刻画、对比闭包维度及严格层次关系；有噪声下证明了对比学习能在任意有限破坏下保持可识别性，而正例学习在单次破坏下即失效。


<details>
  <summary>Details</summary>
Motivation: 传统的极限识别与生成模型仅关注正例或全标签数据，但许多自然监督信号本质上是关系型的（如对比关系），而非单个样本的标签。本文旨在填补这一空白，研究在仅提供未标记对比对情况下的学习理论。

Method: 定义对比呈现（contrastive presentation）模型，引入公共交叉图（common crossing graph）作为统一技术工具，从覆盖与关联角度刻画成对歧义性、族级生成障碍和破坏缺陷。分别考虑无噪声和有限对抗破坏两种情况。

Result: 无噪声下：(1) 对比可识别类的精确几何特征化（Angluin tell-tale条件的精炼）；(2) 对比闭包维度，严格刻画统一对比生成及样本复杂度；(3) 对比生成与文本识别之间不可比较的严格层次关系。有噪声下：存在类在任意有限破坏下仍可被对比对识别（单一算法独立于预算），但单次破坏即导致正例学习不可识别。

Conclusion: 对比学习在极限模型下具有独特的优势，尤其在鲁棒性上远优于传统正例学习，且其理论基础可统一于公共交叉图框架。
...
### [122] [Quadratic Objective Perturbation: Curvature-Based Differential Privacy](https://arxiv.org/abs/2605.05905)
*Daniel Cortild,Coralia Cartis*

Main category: cs.LG

TL;DR: 本文提出二次目标扰动（QOP）方法，通过添加随机二次项而非线性项来实现差分隐私，在更弱假设下达到隐私保护，并保证计算效率与理论效用。


<details>
  <summary>Details</summary>
Motivation: 线性目标扰动（LOP）需要损失函数梯度有界的强假设，这排除了许多现代机器学习模型。为突破这一限制，亟需一种能在更宽松条件下实现差分隐私的扰动机制。

Method: 提出二次目标扰动（QOP），用随机二次型扰动原目标函数，利用曲率诱导强凸性并控制敏感性，从而不依赖梯度有界假设即可实现差分隐私。同时分析了近似求解下的隐私保持性，并采用分裂算法高效求解。

Result: 在插值区域和更弱假设下实现了(ε,δ)-差分隐私；证明了近似解仍能保留隐私保证；给出了经验超额风险的理论效用界；理论分析与数值实验均表明QOP优于LOP。

Conclusion: 基于曲率的二次扰动方法有效克服了LOP对梯度有界假设的依赖，在更实际条件下实现了隐私保护，且具备良好的计算可行性，为差分隐私经验风险最小化提供了新范式。
...
### [123] [Is Escalation Worth It? A Decision-Theoretic Characterization of LLM Cascades](https://arxiv.org/abs/2605.06350)
*Dylan Bouchard*

Main category: cs.LG

TL;DR: 本文提出基于约束优化的决策理论框架，系统分析了LLM级联系统中成本-质量前沿的几何特性，发现级联性能主要受限于结构性成本（即必须支付廉价模型成本后才能升级），而非中间阶段不足。


<details>
  <summary>Details</summary>
Motivation: 现有模型级联方法将延迟阈值视为经验超参数，缺乏对成本-质量前沿几何特性的理论指导，需要建立统一的理论框架来分析级联策略的优化边界。

Method: 采用约束优化和对偶理论，建立决策理论框架；分析两模型级联的成本-质量前沿分段凹性；推导k模型级联的一阶条件；在5个基准测试上对8个模型进行实证验证。

Result: 两模型级联呈分段凹性；k模型级联中全固定链性能低于成对包络线；预生成路由器在4/5数据集上优于最优级联策略，主要原因是避免了将查询直接发送给大模型时的廉价模型生成成本。

Conclusion: 级联性能的主要瓶颈是结构性成本（升级前必须支付廉价模型成本），而非中间阶段不足；预生成路由器通过规避这一结构性成本获得优势。
...
### [124] [From Drops to Grid: Noise-Aware Spatio-Temporal Neural Process for Rainfall Estimation](https://arxiv.org/abs/2605.05912)
*Rafael Pablos Sarabia,Joachim Nyborg,Morten Birk,Ira Assent*

Main category: cs.LG

TL;DR: 提出一种基于神经过程的方法DropsToGrid，融合非规则分布的私人气象站时序数据与雷达空间上下文，生成高分辨率、具有不确定性量化的降雨场估计。


<details>
  <summary>Details</summary>
Motivation: 传统降雨观测存在分辨率低、偏差大等问题，难以捕捉局部降雨特征；现有深度学习降尺度方法受限于降雨的偏态分布、噪声以及时空融合能力不足。

Method: 采用神经过程框架，结合多尺度特征提取、时间注意力机制和多模态融合，从噪声、不规则分布的私人气象站时序数据与雷达空间数据中生成连续降雨场并显式量化不确定性。

Result: 在真实数据集上，DropsToGrid在仅有少量站点可用以及跨区域场景下，均优于操作化基准和深度学习基线，生成准确的高分辨率降雨图且不确定性校准良好。

Conclusion: DropsToGrid有效解决了降雨观测数据的稀疏性和噪声问题，通过多模态融合与不确定性量化，为高分辨率降雨场估计提供了可靠方案。
...
### [125] [E = T*H/(O+B): A Dimensionless Control Parameter for Mixture-of-Experts Ecology](https://arxiv.org/abs/2605.06415)
*Qingjun Zhang*

Main category: cs.LG

TL;DR: 本文提出一个无量纲控制参数 E = T*H/(O+B)，用于预测混合专家模型（MoE）是否会崩溃为死专家。通过12个控制实验（8个视觉、2个语言）共11,000+训练epoch，证明 E >= 0.5 即可保证零死专家，无需手动加载平衡辅助损失。此外发现6个额外现象，提出E可作为MoE训练的统一定量诊断指标，类比流体力学中的雷诺数。


<details>
  <summary>Details</summary>
Motivation: MoE模型训练中常出现专家崩溃（死专家）问题，现有方法依赖手工设计的负载平衡辅助损失函数，缺乏统一的定量预测指标。

Method: 将路由温度T、路由熵权重H、神谕权重O、平衡权重B四个超参数组合成单一控制参数E = T*H/(O+B)，通过12个跨模态实验验证E >= 0.5的临界值。

Result: E >= 0.5 可保证零死专家，且在CIFAR-10/100、TinyImageNet-200、WikiText-2/103上跨模态验证成功。额外发现6个有趣现象，包括死专家可复活、正交毒性数据集相关等。

Conclusion: E可作为MoE训练的统一定量诊断指标，类似流体力学中雷诺数的角色，简化了MoE训练中专家生态健康的监控。
...
### [126] [Invariant Features in Language Models: Geometric Characterization and Model Attribution](https://arxiv.org/abs/2605.06458)
*Agnibh Dasgupta,Abdullah Tanvir,Xin Zhong*

Main category: cs.LG

TL;DR: 本文提出局部几何框架，认为语义等价输入在潜空间中形成结构化区域，并通过对比子空间发现方法分离语义变化与干扰维度。实验表明，不变表征出现在特定深度区域，且能用于零样本模型归因。


<details>
  <summary>Details</summary>
Motivation: 语言模型对释义具有强鲁棒性，暗示语义信息可能通过稳定的内部表征编码，但这种不变性的结构和起源尚不清楚。

Method: (1) 不变潜特征的几何刻画；(2) 对比子空间发现方法，分离语义变化与语义保持变化；(3) 将不变表征应用于零样本模型归因。

Result: 不变结构出现在特定深度区域；语义位移主要位于干扰子空间之外；表征层干预表明不变组件对模型输出有因果作用；不变表征捕获模型特有的几何模式，实现准确归因。

Conclusion: 语义不变性可视为潜表征的局部几何属性，为理解语言模型如何组织意义提供了原则性的视角。
...
### [127] [Knowing but Not Correcting: Routine Task Requests Suppress Factual Correction in LLMs](https://arxiv.org/abs/2605.05957)
*Zixuan Chen,Hao Lin,Zizhe Chen,Yizhou Tian,Garry Yang,Depeng Wang,Ya Guo,Huijia Zhu,James Cheng*

Main category: cs.LG

TL;DR: 本文发现LLM在孤立情境下能纠正错误信息，但在任务导向请求中会抑制纠正（"知道但不纠正"），并提出两种无需训练的干预方法（CDS和DPA）来提升模型的事实严谨性。


<details>
  <summary>Details</summary>
Motivation: 现有研究关注LLM的知识准确性，但忽略了任务上下文对纠正行为的抑制效应。本文旨在揭示并解决模型"知道但不纠正"的失败模式。

Method: 构建300条错误前提基准，评估8个模型的抑制率（19%-90%）。提出两种干预方法：修正方向引导（CDS）在中间层注入修正-服从方向；动态负载放大（DPA）利用注意力差异定位并放大负载token表示。

Result: CDS在Qwen3.5-9B上将纠正率从0%提升至58.2%；DPA在两种模型上均保持或提升推理能力。抑制率在多个模型中超过80%，验证了该现象的普遍性和严重性。

Conclusion: 首次提出"事实严谨性"作为模型可靠性的新维度，揭示了任务上下文导致纠正抑制的机制，并提供了有效的无训练干预方案。
...
### [128] [Uncertainty Estimation via Hyperspherical Confidence Mapping](https://arxiv.org/abs/2605.05964)
*Eunseo Choi,Ho-Yeon Kim,Jaewon Lee,Taeyong jo,Myungjun lee,Heejin Ahn*

Main category: cs.LG

TL;DR: HCM通过将输出分解为幅度和单位超球面上的归一化方向向量，提出了一种无需采样和分布假设的不确定性估计框架，在降低推理成本的同时取得与集成方法相当的效果。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性估计方法依赖昂贵的采样或严格的分布假设，限制了在高风险领域的实用性和效率。

Method: 将输出分解为幅度和归一化方向向量，约束在单位超球面上，利用几何约束违反程度作为不确定性度量，无需采样且不依赖特定分布假设。

Result: 在多种基准和工业任务中，HCM匹配或超越集成方法及证据方法，推理成本大幅降低，置信度-误差对齐更强。

Conclusion: 几何结构在不确定性估计中具有强大潜力，HCM可作为传统方法的通用替代方案。
...
### [129] [Cubit: Token Mixer with Kernel Ridge Regression](https://arxiv.org/abs/2605.06501)
...
TL;DR: 本文提出 Cubit，一种基于核岭回归（KRR）的 Transformer 替代架构，将注意力机制从 Nadaraya-Watson 回归升级为 KRR 闭式解，并通过 Limited-Range Rescale (LRR) 提升训练稳定性，实验表明其在长序列建模上优于传统 Transformer。


<details>
  <summary>Details</summary>
Motivation: 传统 Transformer 的注意力机制本质上是 Nadaraya-Watson 回归，缺乏更强的数学基础；作者希望通过引入核岭回归（KRR）来提供更强的理论支撑，并提升模型在长序列上的建模能力。

Method: 将注意力计算修改为核岭回归的闭式解形式，即通过核相似性聚合值，并用核矩阵的逆进行归一化；同时提出 Limited-Range Rescale (LRR) 方法，将值层重缩放到可控范围内以提高训练稳定性。

Result: 实验表明 Cubit 在长序列建模任务上表现更强，且随着训练序列长度的增加，其相对于 Transformer 的性能增益也越大。

Conclusion: Cubit 作为基于 KRR 的架构，提供了比传统 Transformer（基于 Nadaraya-Watson 回归）更坚实的数学基础，并展现出更强的长序列建模潜力。
...
### [130] [Beyond Uniform Credit Assignment: Selective Eligibility Traces for RLVR](https://arxiv.org/abs/2605.05965)
*Chaoli Mou,Zhan Zhuang,Xinning Chen,Yu Zhang*

Main category: cs.LG

TL;DR: 本文提出选择性资格迹（S-trace）方法，通过稀疏资格迹机制和低熵令牌掩码实现细粒度信用分配，在无需评论家的情况下提升强化学习效率，在Qwen3系列模型上相比GRPO获得最高3.16%的提升。


<details>
  <summary>Details</summary>
Motivation: 现有无评论家强化学习算法（如GRPO）采用"统一信用分配"假设，将所有轨迹优势无差别地广播到每个推理步骤，无法区分关键推理步骤，导致学习效率低下。

Method: 首先提出P-trace（部分信任区域保持的资格迹方法），在此基础上构建S-trace（选择性资格迹），通过选择性掩码低熵令牌实现稀疏资格迹机制，实现细粒度信用分配。同时将GSPO方法统一到无评论家资格迹框架下进行理论分析。

Result: 在Qwen3-1.7B上提升0.49%，Qwen3-4B上提升3.16%，Qwen3-8B上保持2.98%的稳定提升（pass@16平均），同时实现了更高的样本和令牌效率。

Conclusion: S-trace通过细粒度信用分配有效提升了无评论家强化学习的效率和效果，并提供了统一的理论框架。
...
### [131] [Sharper Guarantees for Misspecified Kernelized Bandit Optimization](https://arxiv.org/abs/2605.05967)
*Davide Maran,Csaba Szepesvári*

Main category: cs.LG

TL;DR: 本文针对核化老虎机优化中的误规范问题，提出通过谱局部化（离线）和域分割（在线）方法，将误规范项从核复杂度相关的线性/平方根惩罚降低为对数或多项式对数增长，显著改善了现有理论保证。


<details>
  <summary>Details</summary>
Motivation: 现有误规范核化老虎机优化的理论保证中，误规范惩罚与核复杂度（如有效维数$d_\mathrm{eff}$或最大信息增益$γ_n$）成正比，导致惩罚过大。本文旨在消除或降低这种放大效应。

Method: 离线设置中，利用谱Lebesgue常数控制误规范项，针对一维单调谱和对角傅里叶乘积核得到对数/多项式对数放大；在线设置中，修改域分割算法，在局部化特征衰减假设下得到$\widetilde{\mathcal O}(\sqrt{γ_n n}+n\varepsilon)$的累积遗憾界，去掉了额外的$\sqrt{γ_n}$因子。

Result: 离线：误规范项从$\varepsilon\sqrt{d_\mathrm{eff}}$降至对数或多项式对数增长；在线：遗憾界从$\sqrt{γ_n}\,n\varepsilon$降至$n\varepsilon$，消除了$\sqrt{γ_n}$放大因子。

Conclusion: 局部化是核心原理：谱局部化控制离线近似算子的Lebesgue常数，域分割实现在线设置的空间类比，阻止局部误规范误差被全局放大。
...
TL;DR: 本文提出KV压缩感知训练（KV-CAT），通过在训练时对KV缓存进行稀疏化掩码，引导模型学习更易压缩的表示，从而提升下游压缩方法在长上下文任务中的质量-预算权衡。


<details>
  <summary>Details</summary>
Motivation: 长上下文语言建模中KV缓存的内存和访问成本随前缀长度线性增长，现有后验压缩方法受限于模型内部表示的可压缩性，因此需要从根本上引导模型学习可压缩表示。

Method: 形式化定义KV可压缩性概念，证明任意序列到向量的函数既可高度可压缩也可固不可压缩，提出KV-CAT：在继续预训练阶段引入训练时KV稀疏化策略，通过掩码KV槽迫使模型使用更少KV槽，学习利于后验压缩的表示。

Result: 在检索、长上下文问答和压缩前缀延续的困惑度评估中，KV-CAT改善了后验压缩方法的质量-预算权衡。

Conclusion: 引导Transformer在训练阶段学习可压缩表示是提升KV缓存压缩效果的有效途径，KV-CAT是一种可行且有效的训练框架。
...
### [133] [When and Why SignSGD Outperforms SGD: A Theoretical Study Based on $\ell_1$-norm Lower Bounds](https://arxiv.org/abs/2605.06615)
*Hongyi Tao,Dingzhi Yu,Lijun Zhang*

Main category: cs.LG

TL;DR: 本文从理论上证明了符号优化算法（SignSGD、Muon）在特定问题几何（ℓ₁范数平稳性、ℓ∞光滑性、可分离噪声）下能够比SGD实现d倍加速，并通过GPT-2预训练实验验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 尽管符号优化算法在训练大模型时表现优异，但理论上缺乏对其为何能超越SGD的解释。标准设定下SGD已被证明是最优的，因此需要新的理论框架来理解符号方法的优势。

Method: 提出了基于ℓ₁范数平稳性、ℓ∞光滑性和可分离噪声模型的分析框架，推导SignSGD的匹配上下界，并将框架扩展到矩阵域分析Muon优化器。

Result: SignSGD在稀疏噪声下比SGD降低d倍复杂度；Muon在矩阵域保持相同的维度最优缩放；在124M参数GPT-2预训练中理论优势与实验收敛速度一致。

Conclusion: 符号优化算法在特定问题几何下理论上优于SGD，为该类方法在大模型训练中的成功提供了理论依据。
...
### [134] [Physical Fidelity Reconstruction via Improved Consistency-Distilled Flow Matching for Dynamical Systems](https://arxiv.org/abs/2605.05975)
*Sicheng Ma,Tianyue Yang,Xiuzhe Wu,Xiao Xue*

Main category: cs.LG

TL;DR: 本文提出将最优传输流匹配教师模型蒸馏为一步一致性模型，用于从低保真观测中快速重建高保真流场，在三个流体基准上实现了12倍推理加速，且蒸馏学生模型在SSIM上比从头训练的一致性模型提升23.1%。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型和流匹配模型虽能进行高保真流场重建，但迭代采样过程计算成本高昂，难以满足集合预报、实时可视化和仿真回路推理等延迟敏感场景的需求。

Method: 将最优传输流匹配教师模型蒸馏为一步一致性模型，推理时通过沿传输路径对低保真观测添加噪声来初始化生成轨迹，使无条件高保真流模型无需重新训练即可执行条件重建。

Result: 在Smoke Buoyancy、Turbulent Channel Flow和Kolmogorov Flow三个基准上，蒸馏学生模型在频谱指标上与教师模型相当，参数量减少约一半，推理速度提升12倍；在相同训练预算下，SSIM比从头训练的一致性模型提升23.1%。

Conclusion: 该蒸馏策略为将高容量科学生成模型转化为紧凑、快速、低成本的重建模型提供了可行路径。
...
### [135] [Recursive Agent Optimization](https://arxiv.org/abs/2605.06639)
*Apurva Gandhi,Satyaki Chakraborty,Xiangjun Wang,Aviral Kumar,Graham Neubig*

Main category: cs.LG

TL;DR: 提出递归智能体优化(RAO)，一种训练能递归生成子任务并委派的智能体的强化学习方法，使得智能体可通过分治策略扩展到更长上下文和更困难问题。


<details>
  <summary>Details</summary>
Motivation: 现有智能体在长上下文和复杂任务上表现受限，需要一种能自动进行递归分解和委派的训练方法，以提升推理时扩展能力和任务泛化性。

Method: 采用强化学习方法RAO，训练智能体在推理过程中递归地生成子智能体并委派子任务，同时学习何时委派、如何委派以及如何进行有效通信。

Result: RAO训练的递归智能体具有更好的训练效率，能处理超出模型上下文窗口的任务，能泛化到远难于训练时任务的情况，并相比单智能体系统减少实际运行时间。

Conclusion: 递归智能体结合RAO训练方法，通过分治机制有效扩展了智能体的推理能力，在长上下文和复杂任务上展现出显著优势。
...
TL;DR: 本文精细刻画了半空间（halfspaces）的均匀收敛行为，发现非齐次半空间在d≥2时VC界基本紧致，而二维齐次半空间具有显著更优的收敛速率，并建立了匹配的上下界。


<details>
  <summary>Details</summary>
Motivation: 超越最坏情况VC界的精细分析，揭示半空间在均匀收敛中的维度与结构阈值。

Method: 采用临界楔形局部化（critical-wedge localization）论证，结合二进风险带的逐带分析。

Result: 非齐次半空间：可实现误差Θ(d ln(n/d)/n)，不可知偏差√(τ ln(1/τ))；二维齐次半空间：可实现误差O(1/n)，不可知偏差仅含ln ln n开销。

Conclusion: 半空间的均匀收敛行为存在尖锐的维度与结构阈值，二维齐次情形显著优于一般情形。
...
### [137] [When No Benchmark Exists: Validating Comparative LLM Safety Scoring Without Ground-Truth Labels](https://arxiv.org/abs/2605.06652)
...
TL;DR: 本文提出了一种在缺乏标注基准情况下比较语言模型安全性的方法（基准缺失下的比较安全评分），通过工具性效度链（包括对安全vs消融对比的响应性、目标方差主导性、重跑稳定性）来验证场景式审计的有效性，并在挪威语安全包和实际采购案例中验证了该方法。


<details>
  <summary>Details</summary>
Motivation: 许多部署场景需要在缺乏标注基准的情况下比较候选语言模型的安全性，现有方法无法解决这一实际问题。

Method: 形式化定义了基准缺失下的比较安全评分框架，指定场景式审计作为部署证据的契约条件；提出工具性效度链（安全vs消融对比响应性、目标方差主导性、重跑稳定性），并实例化为SimpleAudit评分工具。

Result: 在挪威语安全包上，安全与消融目标的AUROC介于0.89-1.00；目标身份是主要方差成分（η²≈0.52）；严重度分布在10次重跑后稳定；该方法也可应用于Petri；实际采购案例中，更安全的模型取决于场景类别和风险度量。

Conclusion: 分数、匹配的差异、临界率、不确定性以及所使用的审计员和判断者必须一并报告，而非简化为单一排名。
...
### [138] [Quantizing With Randomized Hadamard Transforms: Efficient Heuristic Now Proven](https://arxiv.org/abs/2605.06014)
*Ran Ben-Basat,William Kuszmaul,Michael Mitzenmacher,Amit Portnoy,Shay Vargaftik*

Main category: cs.LG

TL;DR: 本文研究了随机哈达玛变换（RHT）在量化预处理中的性能。证明两次RHT可使单坐标分布逼近高斯（误差O(d^{-1/2})），在DRIVE和QUIC-FL方案中渐近匹配均匀随机旋转（URR）；三次RHT可保证向量量化（VQ）的协方差衰减；并提出一种线性时间检测方法动态调整RHT使用次数。


<details>
  <summary>Details</summary>
Motivation: 均匀随机旋转是量化预处理中的常用步骤，实践中常用RHT替代。但单个RHT在最坏情况下性能不佳，因为其单个坐标的分布可能远偏离理想分布。本文旨在研究组合多个RHT能否在理论上逼近URR的性能。

Method: 理论分析：证明两次RHT后单坐标边际分布与标准高斯的Kolmogorov距离和1-Wasserstein距离均为O(d^{-1/2})；证明三次RHT可导致坐标协方差衰减；提出线性时间O(d)的输入矩检测方法动态调整RHT次数。

Result: 两次RHT在DRIVE和QUIC-FL方案中渐近匹配URR性能；三次RHT可保证VQ码本期望误差与URR一致（附加项随维度趋零）；动态检测方法可在实践中提升性能。

Conclusion: 组合多次RHT（两到三次）可以在理论上保证与URR相近的性能，并且通过动态检测可避免不必要的计算开销。
...
### [139] [Verifier-Backed Hard Problem Generation for Mathematical Reasoning](https://arxiv.org/abs/2605.06660)
*Yuhang Lai,Jiazhan Feng,Yee Whye Teh,Ning Miao*

Main category: cs.LG

TL;DR: 提出了VHG框架，通过引入独立验证器到传统的设置者-求解者二元结构中，构建三方自我博弈机制，有效生成有效、有挑战性且新颖的问题，在不定积分和通用数学推理任务上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在解决科学和数学问题上能力很强，但在生成有效、有挑战性且新颖的问题方面表现不佳。现有方法要么依赖昂贵的人类专家，要么采用简单的自我博弈范式，常常因奖励黑客行为而产生无效问题。

Method: 提出VHG（Verifier-enhanced Hard problem Generation）框架，基于三方自我博弈。在传统的设置者-求解者二元结构中引入独立的验证器，使设置者的奖励由验证器评估的问题有效性和求解器评估的问题难度共同决定。实现了两种验证器变体：Hard符号验证器和Soft基于LLM的验证器。

Result: 在不定积分任务和一般数学推理任务上的实验结果表明，VHG在所有基线方法中均表现出明显的性能优势。

Conclusion: VHG框架通过三方自我博弈和验证器增强机制，有效解决了问题生成中的奖励黑客问题，能够生成有效、有挑战性且新颖的问题，为推进LLM训练和自主科学研究提供了新思路。
...
### [140] [Multi-agent decision making: A Blackwell's informativeness approach](https://arxiv.org/abs/2605.06028)
*Zheng Zhang,Cuong C. Nguyen,Kevin Wells,Gustavo Carneiro*

Main category: cs.LG

TL;DR: 本文基于Blackwell信息框架理论分析了多LLM系统中的投票和辩论方法，证明这些方法的信息量不超过所有智能体私有信息的聚合。提出了一种基于乘积后验估计的实用方法，在六个QA基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型多智能体决策方法（如投票和辩论）缺乏理论基础和正式保证，无法确保决策的信息充分性。

Method: 1）利用Blackwell信息结构抽象理论分析多LLM决策；2）证明投票和辩论的信息结构不超过聚合后验信息；3）将贝叶斯池化后验最大化作为Blackwell序下的信息理论上界决策规则；4）提出乘积后验估计器来近似池化后验。

Result: 在六个问答基准上，所提方法优于现有最先进的多LLM辩论和投票方法。

Conclusion: Blackwell框架为多LLM决策提供了理论分析工具，乘积后验估计方法在实践中有效提升了决策质量。
...
### [141] [Does Synthetic Data Help? Empirical Evidence from Deep Learning Time Series Forecasters](https://arxiv.org/abs/2605.06032)
*Hugo Cazaux,Eyjólfur Ingi Ásgeirsson,Hlynur Stefánsson*

Main category: cs.LG

TL;DR: 本文通过大规模实验（4,218次运行）系统评估了合成数据在时间序列预测中的作用，发现其效果高度依赖于架构：通道混合模型受益，而通道独立模型性能下降。在低资源场景下，通道混合模型使用合成数据可超越全数据基线。最终提供了具体的使用指南。


<details>
  <summary>Details</summary>
Motivation: 合成数据在语言模型训练中已被广泛应用，但在时间序列预测中的作用尚不明确，缺乏系统性的大规模实证研究。

Method: 设计了9个实验组，共4,218次运行，跨5种架构、4种合成信号和7个数据集进行系统评估，对比不同架构（通道混合vs通道独立）和不同数据增强策略的效果。

Result: 通道混合模型（TimesNet, iTransformer）在多数试验中受益；通道独立模型（DLinear, PatchTST）性能持续下降。仅季节性-趋势生成器可靠有效，硬课程切换导致+24% MSE恶化。平均而言，67%的试验中有害。

Conclusion: 建议使用合成数据增强时应搭配通道混合架构、采用渐进退火调度策略，并注意低资源增强的架构与数据集依赖性。
...
### [142] [Optimal Transport for LLM Reward Modeling from Noisy Preference](https://arxiv.org/abs/2605.06036)
...
TL;DR: 提出基于最优传输的SelectiveRM框架，通过联合一致性差异和质量松弛机制，解决RLHF中偏好噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型在噪声偏好数据上容易过拟合，且去噪方法多假设噪声均匀分布，无法捕捉语言偏好的复杂性。

Method: 基于最优传输理论，提出联合一致性差异对齐模型预测与偏好数据分布，并通过部分传输的质量松弛机制自动排除语义不一致的噪声样本。

Result: 在各种基准测试中显著优于现有最先进方法。

Conclusion: SelectiveRM优化了未观察到的干净风险的上界，有效应对了真实世界偏好噪声问题。
...
### [143] [Requests of a Feather Must Flock Together: Batch Size vs. Prefix Homogeneity in LLM Inference](https://arxiv.org/abs/2605.06046)
*Saksham Rathi,Preeti,Mythili Vutukuru*

Main category: cs.LG

TL;DR: Feather是一个基于强化学习的前缀感知调度器，通过动态平衡批次大小与前缀同质性，并引入Chunked Hash Tree轻量级数据结构来加速前缀检测，相比现有调度器可实现2-10倍的端到端吞吐提升。


<details>
  <summary>Details</summary>
Motivation: 大语言模型自回归解码过程中，KV缓存访问是内存瓶颈。现有调度器仅关注最大化批次大小或减少KV缓存内存占用，忽略了更小的、前缀同质批次可能带来更高吞吐的事实。此外，现有前缀检测依赖基数树遍历，CPU开销与GPU执行时间相当。

Method: 提出Feather调度器，使用强化学习(RL)学习批次大小与前缀同质性之间的最优权衡；引入Chunked Hash Tree (CHT)轻量级数据结构，实现快速前缀检测和高效请求选择，避免昂贵的树遍历开销。

Result: 在vLLM和SGLang上集成，相比现有调度器实现2-10倍端到端吞吐提升；在无足够前缀共享的工作负载下性能不差于现有方案；通过减少KV缓存访问总量，超越了具有相同目标的前缀感知注意力核的性能。

Conclusion: 通过RL驱动的自适应调度和CHT高效前缀检测，Feather在保证不影响无共享场景性能的前提下，显著提升了前缀共享工作负载的解码吞吐。
...
### [144] [Towards Generation-Efficient Uncertainty Estimation in Large Language Models](https://arxiv.org/abs/2605.06053)
*Mingcheng Zhu,Yu Liu,Tingting Zhu*

Main category: cs.LG

TL;DR: 本文研究了如何在不完整生成甚至仅基于输入的情况下对LLM输出进行不确定性估计，提出了Logit Magnitude（基于早期停止生成前缀的top-M logit证据）和MetaUE（将生成式不确定性蒸馏为轻量级输入估计器）两种方法，实验表明有效的不确定性估计所需生成量远少于通常假设。


<details>
  <summary>Details</summary>
Motivation: 在高风险应用（如医疗、金融）中，LLM的幻觉可能流畅但事实错误，现有不确定性估计方法需要完整自回归生成，带来了显著的推理成本和延迟。

Method: 首先建立统一框架，将不确定性估计形式化为自回归生成过程中的早期估计问题；提出Logit Magnitude方法，利用早期停止生成前缀的top-M logit证据进行不确定性估计；提出MetaUE方法，将基于生成的不确定性蒸馏为轻量级仅输入估计器。

Result: 在通用和专业领域基准上，Logit Magnitude表现强劲，部分生成足以有效估计不确定性；MetaUE在多种设置下提供了有竞争力的仅输入近似。
...
### [145] [Towards Self-Explainable Document Visual Question Answering with Chain-of-Explanation Predictions](https://arxiv.org/abs/2605.06058)
*Kjetil Indrehus,Adrian Duric,Changkyu Choi,Ali Ramezani-Kebrya*

Main category: cs.LG

TL;DR: CoExVQA是一种自解释的文档视觉问答框架，通过链式解释设计先识别证据、再定位答案区域、最后解码答案，实现透明可验证的推理过程，在PFL-DocVQA上ANLS提升12%。


<details>
  <summary>Details</summary>
Motivation: 现有DocVQA模型将证据识别与答案定位纠缠在一起，作为黑箱运行，缺乏验证预测如何依赖视觉证据的手段。

Method: CoExVQA采用链式解释设计，首先识别与问题相关的证据，然后显式定位答案区域，最后仅从定位区域解码答案。

Result: 在PFL-DocVQA数据集上，CoExVQA达到可解释DocVQA的最优性能，ANLS较现有可解释基线提升12%。

Conclusion: 将解码限制在已定位的证据上，能够实现透明、可验证且性能优异的自解释文档VQA系统。
...
### [146] [Causal Reinforcement Learning for Complex Card Games: A Magic The Gathering Benchmark](https://arxiv.org/abs/2605.06066)
*Cristiano da Costa Cunha,Ajmal Mian,Tim French,Wei Liu*

Main category: cs.LG

TL;DR: MTG-Causal-RL 是一个基于《万智牌》构建的因果强化学习基准，提供高维部分观测、掩蔽动作空间、结构化因果模型及多种奖励方案，并提出了 CGFA-PPO 作为参考因果智能体。


<details>
  <summary>Details</summary>
Motivation: 现有因果强化学习缺乏同时具备序列决策、隐藏信息、大型掩蔽动作空间和显式因果结构的复杂系统基准。

Method: 构建了包含 3077 维部分观测、478 个掩蔽离散动作、五套标准赛制套牌、三种奖励方案及手工指定 SCM 的 Gymnasium 基准，并提出 CGFA-PPO（使用 SCM 父节点作为因子对齐评论家目标，配合干预校准损失）。

Result: Masked PPO 和 CGFA-PPO 达到具有竞争力的分布内胜率并超过随机基线；因子校准轨迹和留一法迁移缺口揭示了标量胜率无法单独呈现的诊断结构。

Conclusion: 该基准为因果 RL、世界模型和 LLM 智能体研究提供了可共享的测试平台，统一了掩蔽动作空间下的因果信用分配、跨套牌结构迁移及基于 SCM 的策略可审计性等关键问题。
...
### [147] [Normalized Architectures are Natively 4-Bit](https://arxiv.org/abs/2605.06067)
*Maxim Fishman,Brian Chmiel,Ron Banner,Daniel Soudry,Boris Ginsburg*

Main category: cs.LG

TL;DR: nGPT架构通过将权重和隐藏表示约束在单位超球面上，天然对低精度（4-bit）训练更鲁棒，无需额外干预即可实现稳定的NVFP4训练，且在1.2B密集模型和3B/30B混合MoE模型上得到验证。


<details>
  <summary>Details</summary>
Motivation: 以4-bit精度训练大语言模型对效率至关重要，但标准模型在低精度下需要额外干预（如随机Hadamard变换、逐张量缩放）来保持模型质量，nGPT旨在消除这些需求。

Method: 利用nGPT的归一化架构（权重和隐藏表示约束在单位超球面），通过分析点积特性发现：超球面约束增强了元素级乘积的弱正相关性，使信号随隐藏维度有效累加而噪声被平均化，从而获得更高的信噪比。

Result: 在1.2B密集模型和3B/30B混合（Mamba-Transformer）MoE模型上验证了端到端NVFP4训练的稳定性；随着隐藏维度增大，优势更加明显。

Conclusion: nGPT架构天然适合低精度训练，消除了一致性干预需求，且在大规模模型下优势更显著。
...
### [148] [Understanding diffusion models requires rethinking (again) generalization](https://arxiv.org/abs/2605.06077)
*Pierre Marion,Yu-Han Wu*

Main category: cs.LG

TL;DR: 本文主张理解扩散模型的泛化需要全新的理论框架，并指出在扩散模型中记忆与泛化是不兼容的。作者通过实证研究提出，学界应转向研究模型在预记忆阶段实际学习到的内容，而非仅仅解释为何不记忆。


<details>
  <summary>Details</summary>
Motivation: 现有理论（经典统计学习理论、良性过拟合范式）无法解释扩散模型的泛化现象，因为扩散模型中记忆训练数据与生成新样本是互斥的。需要新的理论框架来理解这一矛盾。

Method: 观点论述结合实证研究：在CIFAR-10上训练扩散模型进行实验，并将发现提炼为具体的开放性问题。

Result: 发现扩散模型的记忆与泛化存在根本性冲突，现有解释（容量限制、隐式正则化、架构归纳偏置）各自独立，其相互作用尚不清晰。

Conclusion: 研究重点应从"为何不记忆"转向"预记忆阶段模型学到了什么"，并提出了若干关键开放性问题以推动理解扩散模型的泛化机制。
...
### [149] [Fast Gauss-Newton for Multiclass Cross-Entropy](https://arxiv.org/abs/2605.06081)
*Mikalai Korbit,Mario Zanon*

Main category: cs.LG

TL;DR: 本文提出Fast Gauss-Newton (FGN)方法，通过将多类softmax交叉熵的广义Gauss-Newton曲率分解为"真vs其余"项和"内部竞争协方差"项，并舍弃后者，实现了对全GGN的有效近似，在保持损失和梯度不变的前提下简化了曲率计算，并通过共轭梯度法高效求解阻尼更新。


<details>
  <summary>Details</summary>
Motivation: 多类softmax交叉熵中，全广义Gauss-Newton曲率通过softmax协方差耦合所有输出logits，导致曲率-向量乘积难以随类别数扩展，需要更高效的近似方法。

Method: 1) 将多类GGN精确分解为真vs其余项和内部竞争协方差项；2) FGN保留第一项、舍弃第二项，得到半正定下界近似；3) 利用标量间隔表示重写softmax交叉熵；4) 将阻尼更新转换为等价的whitened行空间系统，通过无矩阵共轭梯度法求解。

Result: FGN在竞争类别质量集中或阻尼较大时最接近全softmax GNN，在内部竞争协方差增大时偏差增大；在固定特征多类头任务上验证了分解预测。

Conclusion: FGN提供了一种可扩展的多类曲率近似方法，在特定条件下能很好地逼近全GGN，为高效二阶优化提供了新思路。
...
### [150] [Beyond Autoregressive RTG: Conditioning via Injection Outside Sequential Modeling in Decision Transformer](https://arxiv.org/abs/2605.06104)
*Yongyi Wang,Hanyu Liu,Lingfeng Li,Bozhou Chen,Ang Li,Qirui Zheng,Xionghui Yang,Chucai Wang,Wenxin Li*

Main category: cs.LG

TL;DR: SlimDT通过将RTG信息注入状态表示中，消除了Transformer序列中的RTG token，使序列长度减少三分之一，在保持或提升性能的同时提高了推理效率。


<details>
  <summary>Details</summary>
Motivation: Decision Transformer中RTG作为标量token包含信息远少于状态/动作向量，却消耗相同的计算预算，且其存在增加了序列长度导致自注意力计算成本平方增长。

Method: 将RTG信息注入到状态表示中，然后仅使用（状态，动作）序列进行自回归建模，完全移除RTG token。

Result: 在D4RL基准测试中，SlimDT超越了标准DT，并达到了与现有最先进方法相当的性能。

Conclusion: 将稀疏的条件信号从信息密集的序列中解耦出来，能够同时获得计算效率和任务性能的提升。
...
### [151] [Listwise Policy Optimization: Group-based RLVR as Target-Projection on the LLM Response Simplex](https://arxiv.org/abs/2605.06139)
...
TL;DR: 本文提出列表策略优化（LPO），一种基于显式目标-投影几何框架的强化学习方法，用于大语言模型后训练中的推理能力提升。相比传统组策略梯度方法，LPO通过将近端RL目标限制在响应单纯形上并精确最小化散度，实现了单调改进、零和梯度及更好的稳定性与多样性。


<details>
  <summary>Details</summary>
Motivation: 现有基于组策略梯度的RLVR方法虽然有效，但其优化策略共享隐式几何结构，缺乏对目标分布的显式建模和可控投影，导致优化过程不够透明且可能存在稳定性问题。

Method: 提出Listwise Policy Optimization (LPO)，核心思想是将近端RL目标显式限制在响应单纯形上，通过精确散度最小化完成策略投影，从而将隐式目标分布显式化。该方法将投影梯度设计为有界、零和且自纠正的，并支持灵活选择不同散度以赋予不同结构性质。

Result: 在多种推理任务和大语言模型骨干上，LPO在匹配目标条件下始终优于典型策略梯度基线，同时保持优化稳定性和响应多样性。

Conclusion: LPO通过显式目标-投影框架为RLVR提供了更可控、更稳定的优化路径，兼具理论保证（单调改进）与实践优势（稳定性与多样性），是策略梯度方法的有力替代方案。
...
### [152] [Matrix-Valued Optimism is Matrix-Valued Augmentation: Additive Hybrid Designs for Constrained Optimization](https://arxiv.org/abs/2605.06141)
*Jiayi Zhao*

Main category: cs.LG

TL;DR: 本文证明了增广拉格朗日法与乐观原始-对偶法在矩阵校正下等价，并提出混合方法在约束优化中优于纯方法。


<details>
  <summary>Details</summary>
Motivation: 探讨增广拉格朗日（增加原始曲率）与乐观原始-对偶（增加对偶记忆）两种看似不同的约束优化稳定机制是否等价，并利用这种等价性设计更优的优化方法。

Method: 理论证明对称矩阵参数下的可加性原理；提出混合规则：选择矩阵校正，拆分为增广和乐观两个通道，并用局部谱权重选择原始和对偶步长。

Result: 在非线性等式约束问题上，混合设计优于纯增广和纯乐观方法，接近网格搜索混合最优解，在轻中度病态条件下与一阶原始-对偶基线方法竞争力相当。

Conclusion: 增广和乐观校正机制在矩阵参数下等价，灵活拆分可提升优化性能，但约束雅可比病态时需大幅增加矩阵校正。
...
### [153] [Unifying Goal-Conditioned RL and Unsupervised Skill Learning via Control-Maximization](https://arxiv.org/abs/2605.06145)
*Alireza Modirshanechi,Benjamin Eysenbach,Peter Dayan,Eric Schulz*

Main category: cs.LG

TL;DR: 本文从理论上统一了无监督预训练中的互信息技能学习（MISL）与目标条件强化学习（GCRL），证明二者均可视为控制最大化问题。通过识别三种GCRL公式并证明其本质不等价，揭示了技能多样性与下游目标达成能力之间的精确对应关系，为强化学习预训练奠定了理论基础。


<details>
  <summary>Details</summary>
Motivation: 尽管MISL方法在实际中成功用于GCRL任务，但其理论基础薄弱——为何通过技能多样性学习能支持下游目标达成仍是一个理论之谜。此外，GCRL和MISL各自包含多种不同定义，缺乏统一的理论框架。

Method: 将GCRL和MISL统一为控制最大化问题；识别三种经典GCRL公式并证明其不等价性；建立MISL目标与下游目标敏感性之间的理论上界关系。

Result: 证明三种GCRL公式根本不等价（即使在同一环境中也会产生不相容的最优策略）；揭示每种GCRL公式下的最优策略均表现为未来轨迹对目标高度敏感；证明MISL目标受限于特定GCRL公式的目标敏感性，建立了技能多样性与下游性能的精确对应关系。

Conclusion: 为RL预训练提供了理论基础，并具有实践指导意义——根据具体下游任务类型选择对应的MISL预训练目标。
...
### [154] [AdaGamma: State-Dependent Discounting for Temporal Adaptation in Reinforcement Learning](https://arxiv.org/abs/2605.06149)
*Yaomin Wang,Jianting Pan,Ran Tian,Xiaoyang Li,Yu Zhang,Hengle Qin,Tianshu YU*

Main category: cs.LG

TL;DR: AdaGamma提出了一种基于状态依赖折扣因子的深度演员-评论家方法，通过返回一致性目标约束避免TD误差崩溃，在理论和实验上均验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统深度强化学习使用固定的折扣因子，忽略了不同状态下对长期回报的不同重视程度。虽然状态依赖折扣因子的概念具有吸引力，但在深度演员-评论家框架中简单实现会导致不稳定和TD误差崩溃问题。

Method: 提出AdaGamma方法，学习一个状态依赖的折扣函数，并引入返回一致性目标（return-consistency objective）来正则化诱导的备份结构，防止对目标的操控退化。可集成到SAC和PPO框架中。

Result: 在连续控制基准测试中，集成到SAC和PPO均取得一致改进；在京东物流平台的在线A/B测试中达到统计显著的性能提升。

Conclusion: 状态依赖折扣因子结合返回一致性目标可以在深度RL中有效发挥作用，防止退化的目标操控，是一种实用且有效的方法。
...
### [155] [Entropy-Regularized Adjoint Matching for Offline RL](https://arxiv.org/abs/2605.06156)
*Abdelghani Ghanem,Mounir Ghogho*

Main category: cs.LG

TL;DR: 本文提出最大熵伴随匹配（ME-AM）框架，通过镜像下降熵最大化和混合行为先验机制，解决了流匹配模型在离线强化学习中的流行偏差和支持约束问题，在稀疏奖励连续控制任务中达到或超越现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于流匹配的离线强化学习方法（如QAM）受限于固定的行为分布，导致流行偏差（压制低密度区域的高奖励动作）和支持约束（限制离流形探索），而简单的残差高斯策略又会重新引入单峰分布的表达能力瓶颈。

Method: ME-AM包含两个核心机制：(1) 镜像下降熵最大化目标，缓解流行偏差以从离线数据中提取最优策略；(2) 混合行为先验，数学上拓宽几何支持以覆盖分布外的高奖励区域，同时保持生成向量场的绝对连续性。

Result: 在多种稀疏奖励连续控制环境中，ME-AM相比现有最先进方法展现出竞争性或更优的性能。

Conclusion: ME-AM通过统一框架同时解决流行偏差和支持约束问题，在保持生成模型表达能力的同时实现了更鲁棒的离线策略优化。
...
### [156] [One Algorithm, Two Goals: Dual Scoring for Parameter and Data Selection in LLM Fine-Tuning](https://arxiv.org/abs/2605.06166)
*Xinrui Chen,Liu Yang,Ou Wu*

Main category: cs.LG

TL;DR: 本文提出DualSFT算法，通过梯度交互矩阵的联合评分机制，实现大语言模型微调中参数掩码和数据子集的一次性协同选择，在3B-9B参数规模的LLM上取得优于传统分离式方法的性能-稳定性权衡。


<details>
  <summary>Details</summary>
Motivation: 现有LLM微调中，参数选择和数据集选择通常使用独立的评分机制，导致冗余计算且难以协同优化，急需一种联合选择方法。

Method: 将参数和数据集选择建模为双层优化问题，推导出基于梯度交互矩阵的局部响应替代评分规则，实现参数重要性（列聚合）和数据效用（行聚合）的闭式行列对应关系，据此提出一次性双评分DualSFT算法。

Result: 在3B-9B LLM上，DualSFT的单轴变体在目标任务性能和稳定性-可塑性权衡上优于对照组，完整DualSFT在联合约束下取得比顺序混合基线更优的权衡。

Conclusion: DualSFT通过统一的梯度统计实现了参数与数据的协同选择，降低了冗余开销，提升了联合约束下的微调效率与性能。
...
### [157] [In-Context Black-Box Optimization with Unreliable Feedback](https://arxiv.org/abs/2605.06187)
*Nicolas Samuel Blumer,Julien Martinelli,Samuel Kaski*

Main category: cs.LG

TL;DR: 提出反馈感知的上下文黑箱优化方法（FICBO），通过预训练Transformer模型同时利用历史优化轨迹和候选集的辅助反馈信息，在多种反馈源（可靠/误导）下自适应提升优化效率。


<details>
  <summary>Details</summary>
Motivation: 现有黑箱优化方法要么只能处理单一反馈源，缺乏泛化能力；要么仅利用优化历史而忽略可获取的辅助反馈（如专家知识、模拟器、预训练预测器等），未能充分利用这些廉价但可能带偏的信息。

Method: 1）设计结构化反馈先验，建模反馈源的访问性、相关性和失真程度；2）基于此先验预训练反馈感知的Transformer模型；3）测试时模型通过比对观测目标值与辅助信号，在上下文中自动估计各反馈源的可靠性，从而优化查询选择。

Result: 在合成任务和真实任务上，FICBO能有效利用有益的反馈信息，同时抵御弱或误导性反馈源的干扰，性能优于现有基线方法。

Conclusion: FICBO成功实现了对多源辅助反馈的上下文感知利用，且模型对测试时反馈源具有可解释的决策行为，为黑箱优化中融合异构先验知识提供了新范式。
...
### [158] [Bandit Learning in General Open Multi-agent Systems](https://arxiv.org/abs/2605.06202)
*Mengfan Xu*

Main category: cs.LG

TL;DR: 本文针对开放系统（智能体可随时加入和离开）中的多臂老虎机学习问题，提出了统一框架，引入预训练度、稳定性和全局动态遗憾等新概念，并开发了具有理论保证的全局UCB算法。


<details>
  <summary>Details</summary>
Motivation: 现有开放系统老虎机研究假设过强，不符合实际；新智能体的加入导致内生非平稳性、信息累积缓慢、遗憾随时间尺度扩大，亟需一个能处理一般动力学的统一学习范式。

Method: 构建统一开放系统老虎机问题（含异构奖励和一般智能体模式）；提出预训练度、稳定性、全局动态遗憾三个核心概念；设计全局UCB学习方法并提供可证明的遗憾界。

Result: 遗憾上界表明：进入不确定性通过预训练度线性增长；在稳定系统中，遗憾由识别持久最优臂所需时间和智能体模式共同决定。通过困难实例的下界证明这些依赖是紧的。

Conclusion: 本文为一般开放系统的在线学习建立了理论基础，揭示了新智能体进入不确定性、系统稳定性和智能体模式对学习效率的根本影响，并证明了所提方法的理论最优性。
...
### [159] [Memory Inception: Latent-Space KV Cache Manipulation for Steering LLMs](https://arxiv.org/abs/2605.06225)
*Andy Zeyi Liu,Michael Zhang,Ilana Greenberg,Adam Alnasser,Lucas Baker,John Sous*

Main category: cs.LG

TL;DR: 本文提出记忆注入（Memory Inception, MI），一种无需训练的大模型引导方法，通过在选定层插入文本衍生的KV缓存来实现潜在注意力空间的引导，在人格引导、可更新引导和结构化推理任务上均优于传统方法，同时大幅节省KV存储。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM引导方法（指令提示和激活引导）各有局限：提示引导控制力强但会缓存引导token到每一层，导致长交互中内容冗余；激活引导虽紧凑但控制能力较弱，且不支持大规模结构化提醒。因此需要一种兼顾控制力与存储效率的新方法。

Method: 记忆注入（MI）是一种无需训练的方法，在潜在注意力空间中通过仅在选定层插入文本衍生的KV键值对（KV banks）来引导模型。MI将引导视为选择性KV分配，只注入潜在槽位到模型路由到的层，而非将提醒内容物化在整个提示缓存中。

Result: 在人格引导任务中，MI在控制-漂移权衡上表现最佳，与提示引导相当且持续优于CAA；在可更新引导中，MI支持对话中途行为变更而不改写可见文本，在Qwen3上达到最高对齐度；在结构化推理中，MI在HARDMath和PHYSICS上10/12个子任务中优于可见提示，同时将KV存储压缩最多118倍。

Conclusion: MI是一种强大的引导方法，特别适用于引导内容持久、结构化或保留在可见文本中成本较高的场景。
...
### [160] [Soft Deterministic Policy Gradient with Gaussian Smoothing](https://arxiv.org/abs/2605.06228)
*Hyunjun Na,Donghwan Lee*

Main category: cs.LG

TL;DR: 该论文提出基于高斯平滑的软确定性策略梯度（Soft-DPG）方法，解决了传统DPG在非光滑奖励环境下因依赖评论家动作梯度而导致梯度异常的问题，并推出Soft DDPG算法，在离散化奖励场景中表现优于标准DDPG。


<details>
  <summary>Details</summary>
Motivation: 确定性策略梯度（DPG）在策略更新时要求评论家对动作可微，但实际控制问题中稀疏或离散奖励会破坏这一假设，导致策略梯度定义不良和学习不稳定。

Method: 基于高斯平滑构建平滑贝尔曼方程，定义新的动作-价值函数，推导软确定性策略梯度（Soft-DPG），消除对评论家动作梯度的显式依赖，确保梯度在非光滑Q函数下仍有效。

Result: 在标准连续控制基准和离散化奖励变体上的实验表明，Soft DDPG在密集奖励设置中保持竞争力，在大多数离散化奖励环境中相比标准DDPG取得了显著提升。

Conclusion: Soft-DPG为处理非光滑奖励环境下的策略梯度提供了原则性替代方案，有效提升了算法在复杂奖励结构下的稳定性和鲁棒性。
...
### [161] [Band Together: Untargeted Adversarial Training with Multimodal Coordination against Evasion-based Promotion Attacks](https://arxiv.org/abs/2605.06238)
*Guanmeng Xian,Ning Yang,Philip S. Yu*

Main category: cs.LG

TL;DR: 本文提出UAT-MC方法，通过梯度对齐机制解决跨模态梯度失配问题，提升多模态推荐系统对规避型推广攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有防御主要针对单模态和投毒攻击，对多模态规避型推广攻击的防御研究不足；此外，多用户推广场景下存在跨模态梯度失配问题，导致攻击效果被稀释，鲁棒训练低估最坏情况风险。

Method: 提出Untargeted Adversarial Training with Multimodal Coordination (UAT-MC)，将所有物品视为潜在目标以应对规避攻击中目标未知的挑战，并引入梯度对齐机制显式矫正跨模态梯度失配，实现各模态间扰动同步。

Result: 实验表明UAT-MC显著提升对推广攻击的鲁棒性，同时在防御-准确性权衡下保持可接受的推荐性能。

Conclusion: UAT-MC有效解决了多模态推荐系统中规避型推广攻击的防御难题，为后续鲁棒多模态推荐研究提供了新思路。
...
### [162] [When Graph Language Models Go Beyond Memorization](https://arxiv.org/abs/2605.06239)
*Masatsugu Yamada,Mahito Sugiyama*

Main category: cs.LG

TL;DR: 本文提出了一种校准诊断协议（结合频繁子图挖掘、图级bootstrap基线和三级频率分层），用于区分图语言模型是真正学到了结构规律还是仅仅记住了训练图。实验表明，在大规模条件下（375万图），模型能够学到高频结构规律，而非单纯记忆。


<details>
  <summary>Details</summary>
Motivation: 现有聚合保真度指标无法回答图语言模型是学到了结构规律还是仅仅记住了训练图，因此需要一套诊断方法来解决这一区分问题。

Method: 提出校准诊断协议，包括：(1) 频繁子图挖掘；(2) 图级bootstrap基线（用于模拟记忆行为）；(3) 三级频率分层（高频、中频、低频）。在5个TU基准数据集上，使用LLaMA风格图语言模型，结合两种图序列化方式（规范DFS编码和动作序列）进行实验。

Result: (1) 小规模下模型表现与逐字记忆难以区分；(2) 大规模（375万图）下逐字记忆大幅下降，但秩相关保持高位；(3) 对仅含新颖子图的固定子样本分析表明，对齐并非仅由逐字记忆驱动；(4) 高频模式被良好复现，低频模式覆盖差，且随容量提升改善有限；(5) 两种序列化方式下观察到相同的跨规模交叉现象。

Conclusion: 图语言模型在大规模条件下能够超越记忆、学到高频结构规律，对低频模式的泛化仍有限。该诊断框架具有鲁棒性，可在不同序列化方式下复现。
...
### [163] [Cumulative-Goodness Free-Riding in Forward-Forward Networks: Real, Repairable, but Not Accuracy-Dominant](https://arxiv.org/abs/2605.06240)
*Amirhossein Yousefiramandi*

Main category: cs.LG

TL;DR: 本文揭示了前向-前向（Forward-Forward）训练中存在的“层搭便车”问题：后续层会依赖前面层已积累的正向间隔，导致自身判别梯度指数级衰减。作者提出了三种局部修复策略，显著改善了深层分离统计量（提升4-45倍），但对最终准确率影响不大（<1%）。实验表明该问题虽真实存在且可修复，但并非限制FF训练精度的主导因素。


<details>
  <summary>Details</summary>
Motivation: 在累积好坏的FF训练变体中，后续层会继承前面层已部分分离的任务，导致梯度传播衰减，即“层搭便车”现象。作者希望形式化分析该问题，并探索局部修复方法，同时评估其对最终性能的实际影响程度。

Method: 1) 理论分析：在softplus FF准则下，推导出前面块累积的正向间隔会导致后续块分类判别梯度指数级衰减。2) 提出三种局部修复方法：逐块（per-block）、硬度门控（hardness-gated）和深度缩放（depth-scaled），这些方法不依赖反向传播梯度，而是恢复当前层的分离度量。3) 在CIFAR-10/100和Tiny ImageNet上进行实验验证。

Result: 在CIFAR-10/100上，三种修复方法使深层分离统计量提升4-45倍，但准确率变化不到1个百分点。Tiny ImageNet上验证了相同的定性结论：层健康诊断与最终准确率之间存在差距。校准实验表明，架构和数据增强选择对最终准确率的影响远大于训练规则修改。

Conclusion: 累积式搭便车是一个真实存在且可修复的优化病理现象，但对于所研究的FF训练规则、架构和数据集而言，它并非限制可达到准确率的主导因素。
...
### [164] [A Note on TurboQuant and the Earlier DRIVE/EDEN Line of Work](https://arxiv.org/abs/2604.18555)
*Ran Ben-Basat,Yaniv Ben-Itzhak,Gal Mendelson,Michael Mitzenmacher,Amit Portnoy,Shay Vargaftik*

Main category: cs.LG

TL;DR: 本文澄清了TurboQuant与EDEN（DRIVE/EDEN）的关系，指出TurboQuant实际上是EDEN的特例且存在多处次优设计，实验表明EDEN在精度上全面优于TurboQuant。


<details>
  <summary>Details</summary>
Motivation: 厘清TurboQuant与EDEN的关系，纠正学界对两者差异的误解，展示EDEN才是更优方案。

Method: 通过理论分析对比两者数学形式（标量缩放参数S的选取、有偏/无偏量化组合方式），并结合实验验证（精度对比、比特效率分析）。

Result: (1) TurboQuant_mse是EDEN在S=1时的特例，且该选择一般非最优；(2) TurboQuant_prod在三方面次优；(3) 实验中有偏EDEN优于TurboQuant_mse，无偏EDEN优于TurboQuant_prod（如2-bit EDEN超越3-bit TurboQuant_prod）。

Conclusion: TurboQuant并非独立创新，而是EDEN的次优特例；EDEN在量化精度上全面领先TurboQuant。
...
### [165] [Structure-Preserving Gaussian Processes Via Discrete Euler-Lagrange Equations](https://arxiv.org/abs/2605.06246)
*Jan-Hendrik Ewering,Kathrin Flaßkamp,Niklas Wahlström,Thomas B. Schön,Thomas Seel*

Main category: cs.LG

TL;DR: 本文提出拉格朗日高斯过程（LGPs），通过离散强迫欧拉-拉格朗日方程实现概率性和数据高效的动力学学习，仅利用位置快照数据即可学习物理一致的动力学模型，在无外力条件下保持拉格朗日-达朗贝尔原理的几何结构，避免能量漂移，实现稳定长期预测。


<details>
  <summary>Details</summary>
Motivation: 现有动力学学习方法通常需要速度或动量数据，而在许多实际场景（如运动捕捉、视觉伺服）中仅能获取位置测量值；此外，传统方法可能破坏系统的几何结构，导致能量漂移和不稳定的长期预测。

Method: 基于离散强迫欧拉-拉格朗日方程和变分离散化方案，构造高斯过程条件化的线性算子，在无外力时保持拉格朗日-达朗贝尔原理的几何结构，仅从离散位置快照学习动力学。

Result: 在多个合成和真实世界案例（包括具有滞后效应的软体机器人）中验证了方法的数据效率和泛化能力，能够仅从稀疏位置数据学习物理一致的动力学并量化不确定性，实现稳定长期预测。

Conclusion: LGPs方法成功克服了传统方法需要速度/动量数据的限制，通过保持物理几何结构避免了能量漂移，为仅能获取位置数据的实际应用场景提供了有效的动力学学习方案。
...
### [166] [The Role of Node Features in Graph Pooling](https://arxiv.org/abs/2605.06250)
*Jan von Pichowski,Alžbeta Hrabošová,Ingo Scholtes,Christopher Blöcker*

Main category: cs.LG

TL;DR: 本文分析了图池化在图分类中效果不稳定的原因，发现池化操作需要节点特征与图拓扑结构良好对齐，并提出了特征质量的量化标准和必要条件，实验证明满足条件时池化能提升性能。


<details>
  <summary>Details</summary>
Motivation: 图池化在图分类中相比标准WL-1表达能力GNN的增益常常微弱或不一致，现有研究对此缺乏深入分析，作者希望从节点特征与图拓扑的交互角度解释这一差距。

Method: 通过理论分析节点特征与图拓扑的对齐关系，形式化定义了池化有效所需的节点特征基本条件，并引入了一种量化特征质量的度量指标。

Result: 当节点特征满足所提出的条件时，池化操作能够带来性能提升，并在合适的数据集上得到验证。

Conclusion: 节点特征与图拓扑的对齐是池化有效的关键前提，特征质量可量化，满足条件时池化可改善图分类性能。
...
### [167] [Trade-off Functions for DP-SGD with Subsampling based on Random Shuffling: Tight Upper and Lower Bounds](https://arxiv.org/abs/2605.06259)
*Marten van Dijk,Murat Bilgehan Ertan*

Main category: cs.LG

TL;DR: 本文在$f$-DP框架下，针对基于随机洗牌的差分隐私随机梯度下降(DP-SGD)进行了紧致分析，推导出了可解释的闭式隐私权衡函数，并揭示了与泊松子采样不同的渐近行为。


<details>
  <summary>Details</summary>
Motivation: 现有DP-SGD隐私分析多基于泊松子采样，虽然可机器计算但公式非闭式且不透明；而随机洗牌（实际中更常用）缺乏紧致、可解释的理论分析，需要填补这一空白。

Method: 利用Berry-Esseen定理推导随机洗牌下单轮次的紧致闭式隐私权衡函数；引入基于大数定律推广的新证明技术，分析多轮次组合后的渐近行为。

Result: 给出了单轮次E=1时的具体参数设置（如σ=1, δ=1/100时需约M≈1.14×10^6轮、N≈1.14×10^7样本）；证明了多轮次组合后δ与E呈线性关系（限制E=O(√M)）；利用新方法得到了渐近随机猜测对角线极限结果：当E=c_M^2M且c_M→0时，组合函数一致收敛于1-a，δ仅O(√E)依赖。

Conclusion: 随机洗牌下的DP-SGD可得到比泊松子采样更透明的闭式隐私分析，但多轮次组合仍受限于线性δ增长；高维渐近极限的显式收敛率是未解决的问题。
...
### [168] [Beyond Rigid Alignment: Graph Federated Learning via Dual Manifold Calibration](https://arxiv.org/abs/2605.06260)
*Wentao Yu,Bo Han,Jie Yang,Chen Gong*

Main category: cs.LG

TL;DR: 提出了一种名为FedGMC的统一流形校准方法，通过双流形机制分别处理图联邦学习中的语义异构和结构异构，在保持全局共性的同时最大化局部个性化表示空间，在11个图上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有图联邦学习方法通过强制对齐模型参数或原型来处理异构性，但这隐含地依赖全局线性假设，用一个全局一致的表示空间压缩了客户的个性化表示空间，无法保留多样的局部图分布。

Method: 提出FedGMC，包含双流形校准机制：1）语义异构方面，服务器通过等距语义锚构建几何最优的语义流形，指导局部语义流形校准；2）结构异构方面，服务器通过全局结构模板构建全局结构流形，指导局部结构流形校准；3）服务器通过聚合局部流形动态优化全局语义流形和结构流形。

Result: 在11个同质性和异质性图上进行大量实验，FedGMC有效平衡了全局共性和局部个性化，显著优于现有最先进基线方法。

Conclusion: FedGMC从统一流形视角出发，通过双流形校准机制解决了语义和结构异构问题，在保持全局共性的同时最大化了个性化表示空间，为图联邦学习中的异构性问题提供了新范式。
...
### [169] [Can Attribution Predict Risk? From Multi-View Attribution to Planning Risk Signals in End-to-End Autonomous Driving](https://arxiv.org/abs/2605.06264)
*Le Yang,Ruoyu Chen,Haijun Liu,Jiawei Liang,ShangQuan Sun,Xiaochun Cao*

Main category: cs.LG

TL;DR: 提出了一种面向端到端自动驾驶规划的分层归因框架，通过粗到细的区域归因策略从六视图输入中定位关键视觉证据，并提取三种归因统计量（熵、空间方差、基尼系数）作为风险预测信号，在多个模型上验证了其与轨迹误差和碰撞检测的相关性。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型虽然提升了系统集成度，但存在决策不透明、风险难以定位的问题。已有的辅助监控或文本解释方法与规划过程解耦，无法揭示轨迹生成背后的视觉证据，而归因方法在规划任务（多视图输入、连续多步轨迹预测）中的应用及其风险识别能力尚未被充分探索。

Method: 提出分层归因框架：以原始轨迹的L2一致性为目标，设计粗到细的区域归因策略，先在六视图输入中搜索候选区域，再在其中细化归因；提取三种归因统计量——归因熵（衡量注意力集中程度）、相机内空间方差（反映归因分散程度）、跨相机基尼系数（量化归因分布不均程度），作为规划风险预测信号。

Result: 在BridgeAD、UniAD和GenAD三个模型上验证，归因统计量与轨迹误差的Spearman相关系数为0.30±0.07，碰撞检测的AUROC为0.77±0.04；该信号在未见场景中泛化能力良好，且在不同归因基线下保持稳定。

Conclusion: 所提出的分层归因框架能够有效揭示端到端规划模型的视觉证据，并利用归因统计量预测规划风险，为提升自动驾驶系统的可解释性和安全性提供了可行方案。
...
### [170] [A Flow Matching Algorithm for Many-Shot Adaptation to Unseen Distributions](https://arxiv.org/abs/2605.06272)
*Tyler Ingebrand,Ruihan Zhao,Kushagra Gupta,David Fridovich-Keil,Sandeep P. Chinchali,Ufuk Topcu*

Main category: cs.LG

TL;DR: 提出函数投影流匹配（FP-FM），通过计算目标分布样本在基函数上的最小二乘投影，实现无需额外训练即可高效生成新分布样本。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在自然语言条件生成上表现优异，但如何从少量样本数据点进行模型自适应仍是未被充分探索的难题。

Method: 学习一组基函数来张成多个训练分布的 velocity fields，对于新分布，仅需计算一个最小二乘投影即可完成适配；还提出了多种变体（如时间依赖系数）以平衡表达力与计算开销。

Result: 在合成数据和图像数据集上，FP-FM在精度和召回率上显著优于基线，尤其在未见过的分布上提升更为明显。

Conclusion: FP-FM提供了一种高效的免训练推理时自适应方案，能够从样本数据点直接控制生成，具有良好的泛化能力。
...
### [171] [When Labels Have Structure: Improving Image Classification with Hierarchy-Aware Cross-Entropy](https://arxiv.org/abs/2605.06274)
*April Chan,Davide D'Ascenzo,Sebastiano Cultrera di Montesano*

Main category: cs.LG

TL;DR: 提出HACE损失函数，通过将类别层次结构编码到交叉熵中，在CIFAR-100、FGVC Aircraft和NABirds数据集上平均提升4.66%的端到端准确率，并在线性探测中优于所有对比方法。


<details>
  <summary>Details</summary>
Motivation: 标准交叉熵将所有误分类等同对待，忽略了类别层次结构中蕴含的语义距离信息。

Method: HACE包含两个组件：(1) 预测聚合——将模型概率质量向上传播至父节点；(2) 祖先标签平滑——将真实信号沿类别路径分布至根节点。

Result: 端到端训练中，在18个架构-数据集对中有15个提升准确率，平均增益4.66%；线性探测中在三数据集上均优于所有对比方法，平均提升2.18%。
...
### [172] [PACE: Prune-And-Compress Ensemble Models](https://arxiv.org/abs/2605.06278)
*Fabian Akkerman,Julien Ferry,Théo Guyard,Thibaut Vidal*

Main category: cs.LG

TL;DR: PACE框架通过先主动增强集成模型多样性再剪枝的两阶段策略，在保证忠实度的前提下实现比现有方法更优的集成模型压缩效果。


<details>
  <summary>Details</summary>
Motivation: 集成模型虽性能优异，但大量弱学习器聚合导致部署困难、可解释性差及鲁棒性验证受阻，现有剪枝与压缩方法各有限制。

Method: 提出PACE框架，采用两阶段策略：第一阶段通过理论指导的流程主动生成新学习器以增强多样性；第二阶段对增强后的集成进行剪枝，并在两阶段中保持对原始集成的忠实度控制。

Result: 实验表明PACE在性能上优于现有剪枝和压缩方法，并能提供原则性的忠实度保证。

Conclusion: PACE通过有机结合主动生成与剪枝策略，实现了更优的集成模型压缩，兼顾了性能与可控的忠实度保障。
...
### [173] [INEUS: Iterative Neural Solver for High-Dimensional PIDEs](https://arxiv.org/abs/2605.06281)
*Jean-Loup Dupret,Davide Gallon,Patrick Cheridito*

Main category: cs.LG

TL;DR: INEUS是一种无网格迭代神经求解器，用于求解偏积分微分方程(PIDEs)，通过单跳采样替代非局部跳跃积分的显式计算，并将PIDE求解转化为递归回归问题。相比PINNs，它更高效地处理非局部项，避免昂贵的全PIDE残差微分，适用于高维问题，并具有收敛性证明。


<details>
  <summary>Details</summary>
Motivation: 现有方法如PINNs在处理PIDE中的非局部跳跃积分时计算成本高昂，难以高效求解高维PIDEs。

Method: 采用无网格迭代框架，用单跳采样替代非局部跳跃积分的显式计算，将PIDE求解重新表述为一系列递归回归问题。

Result: 数值实验表明INEUS能够为多种高维线性和非线性PIDE问题提供准确且可扩展的求解方案。

Conclusion: INEUS在PIDE求解中相比PINNs更高效，特别适合高维问题，且具有理论收敛保证。
...
### [174] [Attributions All the Way Down? The Metagame of Interpretability](https://arxiv.org/abs/2605.06295)
*Hubert Baniecki,Przemyslaw Biecek,Fabian Fumagalli*

Main category: cs.LG

TL;DR: 本文提出“元博弈”（metagame）框架，通过将归因方法视为合作博弈并计算其Shapley值，量化模型解释中的二阶交互效应（元归因），从理论上证明了归因可层次分解为元归因，并在多种可解释性任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有模型解释方法主要关注一阶特征归因，但忽略了特征间的交互作用对归因结果本身的影响，缺乏量化二阶交互效应的系统性框架。

Method: 将归因方法本身视为合作博弈，通过计算其Shapley值得到元归因 $\varphi_{j \to i}(f)$，度量特征 $j$ 对特征 $i$ 归因的方向性影响。

Result: 理论上证明了归因可层次分解为元归因，并建立了元归因与现有交互指数之间的方向性扩展关系；实验上在指令微调语言模型、视觉-语言编码器、多模态扩散Transformer三个场景中验证了有效性。

Conclusion: 元博弈框架为理解模型解释中的特征交互提供了新的理论工具和实用方法，有助于更深入地洞察模型内部机制。
...
### [175] [Molecules Meet Language: Confound-Aware Representation Learning and Chemical Property Steering in Transformer-VAE Latent Spaces](https://arxiv.org/abs/2605.06303)
*Zakaria Elabid,Jan Andrzejewski,Bartosz Brzoza,Attila Cangi*

Main category: cs.LG

TL;DR: 本文研究无监督自回归Transformer-VAE在SELFIES分子表示中的潜空间，发现化学性质的可预测性可能源于序列层面的捷径而非真实化学组织，通过残差化、混淆方向对齐分析和解码分子遍历等混淆感知评估方法，证实部分化学性质（如cLogP、TPSA等）存在稳健的单调操控方向，但需排除SELFIES结构伪影的影响。


<details>
  <summary>Details</summary>
Motivation: 分子生成模型常假设潜空间具有有意义的几何结构，但性质的可预测性可能来自序列层面的捷径（如SELFIES长度、分支标记等），而非真正的化学组织特征。作者旨在区分化学信号与SELFIES伪影，验证潜空间操控的化学有效性。

Method: 使用无监督自回归Transformer-VAE在SELFIES上训练，冻结模型后对RDKit描述符拟合线性探针，将探针权重作为全局操控方向。引入混淆感知评估：残差化、混淆方向对齐分析、解码分子遍历。

Result: SELFIES长度、分支标记、环标记和标记熵被强烈编码在潜空间中。在控制混淆后，cLogP、FractionCSP3、HeavyAtomCount、TPSA、BertzCT和HBA表现出稳健的单调操控。非线性探针显示部分性质存在稳定全局方向，另一些则更适合局部潜梯度描述。

Conclusion: 化学上有意义的操控可以在纠缠的分子潜空间中实现，但必须通过解码分子验证并控制表征层面的混淆因素。
...
### [176] [Perceive, Route and Modulate: Dynamic Pattern Recalibration for Time Series Forecasting](https://arxiv.org/abs/2605.06310)
*Siru Zhong,Zhao Meng,Haohuan Fu,Haoyang Li,Qingsong Wen,Yuxuan Liang*

Main category: cs.LG

TL;DR: 本文提出动态模式重校准（DPR）机制，通过轻量级的"感知-路由-调制"流程，对每个时间令牌进行软路由分布计算，生成时间感知的调制向量，以残差哈达玛积方式重校准隐藏状态，从而解决深层预测模型中固定权重矩阵无法适应局部时序动态变化的问题。


<details>
  <summary>Details</summary>
Motivation: 现实时间序列中的局部时序模式持续漂移，全局共享的变换方式效果不佳，现有深度预测模型依赖固定权重矩阵，导致静态模式响应，无法适应变化的局部动态。

Method: 提出DPR机制，包含"感知-路由-调制"三阶段流水线：计算软路由分布、基于学习的基础自适应响应模式生成时间感知调制向量，通过残差哈达玛积重校准隐藏状态。

Result: 作为骨干无关的适配器，DPR在多种架构上以极小开销提升预测性能；作为极简独立模型DPRNet，在12个基准上取得具有竞争力的表现。

Conclusion: DPR验证了动态重校准相对于宏观参数缩放的有效性，确认其解决了通用预测瓶颈。
...
### [177] [When Does $\ell_2$-Boosting Overfit Benignly? High-Dimensional Risk Asymptotics and the $\ell_1$ Implicit Bias](https://arxiv.org/abs/2605.06314)
*Ye Su,Jian Li,Yong Liu*

Main category: cs.LG

TL;DR: 本文研究了$\ell_2$-Boosting在$\ell_1$隐式偏置下的良性过拟合现象，发现在各向同性纯噪声模型下，贪婪选择会将噪声局部化到稀疏活跃集，导致方差以对数速率衰减（$\Theta(\sigma^2/\log(p/n))$），远慢于$\ell_2$几何中的线性衰减；对于尖峰-各向同性设计，风险在对数速率下趋于零；通过分析非光滑次微分动力学，提出了免调参的早停规则，在$\ell_1$有界信号下恢复Lasso基本不等式并达到极小极大最优经验预测率。


<details>
  <summary>Details</summary>
Motivation: $\ell_2$几何中的良性过拟合已有充分刻画，但基于$\ell_1$隐式偏置的贪婪集成方法的行为仍难以分析，主要障碍在于坐标选择阈值的非线性耦合，使得标准谱分解工具失效。本文旨在隔离这种算法偏置，刻画$\ell_2$-Boosting在高维设定下的风险行为。

Method: 将凸高斯极小极大定理与双边截断高斯矩的渐近展开耦合，解析求解非光滑$\ell_1$插值问题；进一步分析非光滑次微分动力学，推导免调参的早停规则。

Result: 纯噪声模型下，良性过拟合以对数速率（$\Theta(\sigma^2/\log(p/n))$）失败，而非线性速率；尖峰-各向同性设计中，风险在$r_2 \gg n$时趋于零，但仅以对数速率；早停规则在$\ell_1$有界信号下恢复Lasso基本不等式并达到极小极大最优率。

Conclusion: $\ell_1$隐式偏置下，贪婪Boosting的过拟合行为与$\ell_2$几何有本质差异：噪声局部化导致收敛速率显著变慢。早停策略可有效规避这一缓慢收敛问题。
...
### [178] [Pro-KLShampoo: Projected KL-Shampoo with Whitening Recovered by Orthogonalization](https://arxiv.org/abs/2605.06316)
*Ruotong Sun,Ermin Wei*

Main category: cs.LG

TL;DR: 本文提出Pro-KLShampoo优化器，通过利用KL-Shampoo预条件子的"尖峰-平坦"特征谱结构，仅对低维子空间进行全谱建模，其余方向采用正交化，在多个LLM预训练任务上超越原方法，降低内存和计算开销。


<details>
  <summary>Details</summary>
Motivation: KL-Shampoo等Kronecker因子预条件方法虽有效，但其预条件子存在"尖峰-平坦"特征谱结构——少量大特征值后跟随近似均匀的拖尾。现有方法未利用这一结构，导致计算和存储冗余。

Method: 提出Pro-KLShampoo：将KL-Shampoo的一个Kronecker因子限制为参数化族——在追踪的r维子空间上保留完整谱结构，剩余n-r方向共享单一特征值并应用正交化。由恒等式证明该正交化恢复完整KL-Shampoo预条件子的代数形式。

Result: 在GPT-2 124M/350M、LLaMA 134M/450M四个规模上，Pro-KLShampoo在每个测试的子空间秩下均优于KL-Shampoo，包括验证损失、单GPU峰值内存和达到各损失水平的墙钟时间。

Conclusion: 利用预条件子的谱结构稀疏性可在不牺牲精度的前提下显著提升LLM预训练优化器的效率，Kronecker因子预条件与正交化方法之间存在深层联系。
...
### [179] [LINC: Decoupling Local Consequence Scoring from Hidden Matching in Constructive Neural Routing](https://arxiv.org/abs/2605.06332)
*Shaofeng Qin,Li Wang*

Main category: cs.LG

TL;DR: 提出LINC解码器架构，通过显式计算动作的一步后果（如行程、等待、松弛时间、容量变化）来改进构造式神经路由求解器，在CVRPTW等约束路径问题上显著降低了最优性差距。


<details>
  <summary>Details</summary>
Motivation: 现有构造式神经路由求解器在解码时用上下文匹配候选嵌入来评分下一动作，隐式地掩盖了行程、等待、松弛时间、容量变化等确定性的单步后果，导致隐藏状态需要额外学习过渡算术。

Method: 提出LINC（Local Inference via Normed Comparison），在解码端显式计算动作的一步后果，按决策角色使用：居中的相对后果由共享线性局部评分器比较，可行集摘要用于调节解码器上下文，保留标准全局匹配机制。

Result: 在CVRPTW上，将PolyNet在Solomon和Homberger数据集上的差距从13.83%/38.15%降至7.26%/14.71%；在TSP和CVRP上也改进了外部基准差距。

Conclusion: 显式建模确定性过渡后果能有效提升神经路由求解器在强约束问题上的性能，LINC可作为通用解码器架构应用于多种路径问题。
...
### [180] [Eliciting associations between clinical variables from LLMs via comparison questions across populations](https://arxiv.org/abs/2605.06335)
*Fabian Kabus,Kian Kordtomeikel,Thomas Brox,Heinz Wiendl,Daiana Stolz,Harald Binder*

Main category: cs.LG

TL;DR: 本文提出了一种通过结构化三元组比较问题从大语言模型中提取患者特征间相关性和因果关系的方法，并结合不变因果预测（ICP）来获得保守的因果推断。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练数据包含大量生物医学文献，但直接从模型中提取相关性和因果信息存在困难，因此需要一种间接且可靠的启发方法。

Method: 使用患者比较三元组问题，结合统计模型估计相关性；通过提示级环境偏移获得不同子群体的相关性估计，再应用不变因果预测（ICP）识别候选因果父节点。

Result: 在COPD和MS两个临床领域，模型生成的相关性平滑、稳定且具有临床可解释性，不同环境间的相关性差异具有统计学显著性，ICP能够识别出一小组候选的不变因果父链接。

Conclusion: 通过三元组比较的间接启发方法能够从大语言模型中恢复有意义的关联结构，并为从隐式相关性到因果陈述提供了一条谨慎可行的路径。
...
### [181] [Order-Agnostic Autoregressive Modelling with Missing Data](https://arxiv.org/abs/2605.06355)
*Ignacio Peis,Pablo M. Olmos,Jes Frellsen*

Main category: cs.LG

TL;DR: 本文提出缺失感知的顺序无关自回归模型（MO-ARM），将顺序无关自回归模型重新解释为缺失数据视角下的工具，在完全数据训练时隐式执行随机缺失插补，并首次建立了在不完全数据上训练的原则性框架，利用其条件密度估计进行主动信息获取，在多个基准上优于传统插补方法。


<details>
  <summary>Details</summary>
Motivation: 顺序无关自回归模型在深度生成建模中表现出色，但其在不完整数据场景下的应用尚未被充分探索，现有方法缺乏针对缺失数据的系统性处理框架。

Method: 1) 从缺失数据视角重新解释顺序无关自回归模型的训练过程；2) 提出首个在不完全数据上直接训练的原则性框架，支持通用缺失机制；3) 利用摊销条件密度估计进行主动信息获取，顺序选择最具信息量的缺失变量。

Result: 在多个真实世界基准数据集上，MO-ARM一致优于已有的插补基线方法。

Conclusion: 顺序无关自回归模型在缺失数据场景中具有显著潜力，MO-ARM为处理不完整数据提供了统一且有效的解决方案。
...
### [182] [Memory Efficient Full-gradient Attacks (MEFA) Framework for Adversarial Defense Evaluations](https://arxiv.org/abs/2605.06357)
*Yuan Du,Mitchel Hill,HanQin Cai*

Main category: cs.LG

TL;DR: 本文提出基于梯度检查点的内存高效全梯度评估框架，用于白盒对抗攻击下迭代随机净化防御的鲁棒评测，发现全梯度攻击能揭示近似梯度方法遗漏的漏洞，并取得更强的ℓ∞和ℓ2攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有随机净化防御的评估常因内存限制采用近似反向传播，这会弱化攻击信号并高估模型鲁棒性；同时，迭代净化中的随机性未受充分控制，不同净化轨迹可能导致鲁棒性指标显著变化。

Method: 结合梯度检查点（checkpointed backpropagation）实现精确端到端梯度计算，并设计控制随机变异性的评估协议，在降低内存瓶颈的同时保留精确梯度。

Result: 在扩散模型和基于能量模型（EBM）的朗之万采样防御上，全梯度攻击发现了近似梯度评估遗漏的脆弱点，实现了更强的ℓ∞和ℓ2白盒攻击，并支持对分布外鲁棒性的探测。

Conclusion: 精确梯度评估对于迭代随机防御的可靠基准测试至关重要，本文方法为未来防御评估提供了更严格的标准。
...
### [183] [Preliminary Insights in Chronos Frequency Data Understanding and Reconstruction](https://arxiv.org/abs/2605.06361)
...
TL;DR: 本文初步分析了Chronos基础模型处理并内部表示频域信息的能力，通过离散正弦波实验和轻量级探针测试，揭示了模型对频率信息的编码特性及质量退化区域。


<details>
  <summary>Details</summary>
Motivation: 尽管时间序列基础模型被广泛采用，但它们在内部编码基本信号属性（如频率信息）的程度尚不明确，缺乏系统性的表征分析。

Method: 在受控条件下生成固定频率的离散正弦波，使用轻量级在线最小描述长度探针，作用于Chronos的解码器架构，测试频率信息在模型内部表征中的存在性和可分离性。

Result: 揭示了Chronos在不同频段捕捉频率信息的能力差异，指出了表征质量可能下降或需要特别注意的频率区间。

Conclusion: 为Chronos用户在信号处理和信息融合场景中提供了实用指导，并推动了时间序列基础模型可解释性与评估的持续改进。
...
### [184] [Flow Matching with Arbitrary Auxiliary Paths](https://arxiv.org/abs/2605.06364)
*Xin Peng,Ang Gao*

Main category: cs.LG

TL;DR: 提出了一种名为AuxPath-FM的生成建模框架，通过允许辅助变量服从任意分布来泛化条件流匹配，在理论上保持连续性方程和训练目标一致性的同时，支持多种概率路径设计（如高斯、均匀、拉普拉斯、离散Rademacher分布）及标签引导生成等特殊任务。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法将辅助成分限制为高斯噪声，缺乏灵活性。作者希望构建一个更通用的框架，允许辅助变量服从任意分布，从而扩展概率路径设计的可能性。

Method: 将辅助变量η引入轨迹生成，形式化为X_t = a(t)X_1 + b(t)X_0 + c(t)η，其中η可来自任意分布（高斯、均匀、拉普拉斯、Rademacher等）。理论上证明该构造保持连续性方程且与边缘公式的训练目标一致。

Result: 理论上证明了任意辅助分布下的连续性方程保持性和训练目标一致性；展示了多种分布（高斯、均匀、拉普拉斯、离散Rademacher）下的概率路径设计能力。

Conclusion: AuxPath-FM为概率路径设计提供了原则性且通用的基础，兼具理论通用性和实际灵活性，适用于多样化的生成建模任务。
...
### [185] [A Unified Pair-GRPO Family: From Implicit to Explicit Preference Constraints for Stable and General RL Alignment](https://arxiv.org/abs/2605.06375)
*Hao Yu*

Main category: cs.LG

TL;DR: 本文提出Pair-GRPO系列（Soft-Pair-GRPO和Hard-Pair-GRPO），通过统一理论框架解决RLHF中策略更新不稳定、梯度方向模糊等问题，在LLM对齐和连续控制任务上超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF中基于成对偏好的学习方法存在策略更新不稳定、梯度方向不明确、可解释性差、梯度方差高等问题，亟需系统的理论框架加以解决。

Method: 建立以Pair-GRPO家族为中心的统一理论框架：Soft-Pair-GRPO将GRPO的归一化标量奖励替换为二元成对偏好奖励并证明梯度等价定理；Hard-Pair-GRPO引入显式局部概率约束和约束KL拟合优化。

Result: 在HH-RLHF、UltraFeedback和HalfCheetah-v4基准上，Pair-GRPO系列在对齐质量、人类偏好胜率、训练稳定性和泛化能力上持续优于SOTA基线。

Conclusion: Pair-GRPO系列通过理论保证（单调策略改进、确定性梯度方向、梯度方差降低、动态步长收敛）和实验验证，系统性地提升了偏好学习的稳定性与性能。
...
### [186] [Data-Driven Covariate Selection for Nonparametric and Cycle-Agnostic Causal Effect Estimation](https://arxiv.org/abs/2605.06385)
...
<details>
  <summary>Details</summary>
Motivation: 现有因果效应估计方法通常假设无环性（acyclicity）或依赖全局因果结构学习，这在存在潜在混淆和反馈循环的真实场景中难以适用，计算效率也较低。

Method: 研究了一种基于条件独立性信息的局部数据驱动协变量选择方法，并通过σ-无环化（σ-acyclification）下条件独立性断言的不变性，证明该方法在循环因果模型中的有效性与在无环模型中一致。

Result: 理论证明该方法在循环因果模型中同样保持可靠性和完备性；在合成数据上的实验验证了其在循环因果模型中的稳定性能。

Conclusion: 该研究建立了协变量选择和因果效应估计的统一、循环无关的视角，表明该方法无需修改即可适用于循环和无环两种场景。
...
### [187] [Asymmetric On-Policy Distillation: Bridging Exploitation and Imitation at the Token Level](https://arxiv.org/abs/2605.06387)
*Nan Jia,Haojin Yang,Xing Ma,Jiesong Lian,Shuailiang Zhang,Weipeng Zhang,Ke Zeng,Xunliang Cai,Zequn Sun*

Main category: cs.LG

TL;DR: 提出非对称在线策略蒸馏（AOPD），通过在非正优势区域采用局部散度最小化替代无效负强化，解决了标准在线策略蒸馏的三个结构性问题，在数学推理任务上平均提升4.09/8.34（强/弱初始化）。


<details>
  <summary>Details</summary>
Motivation: 标准在线策略蒸馏（OPD）的优势加权策略梯度存在三个结构性缺陷：高方差更新、零优势区域梯度消失、纠正信号不足时的探索瓶颈。

Method: 提出AOPD，在非正优势区域使用局部散度最小化替代负强化，同时保留正强化学习机制。

Result: 在数学推理基准上，AOPD在强/弱初始化下分别平均提升4.09/8.34；训练中保持更高策略熵，工具使用适应中更好保持能力。

Conclusion: AOPD通过解决OPD的结构性缺陷，在多个维度上优于标准OPD。
...
### [188] [Consistent Geometric Deep Learning via Hilbert Bundles and Cellular Sheaves](https://arxiv.org/abs/2605.06395)
*Kartik Tandon,Julian Gould,Tanishq Bhatia,Francesca Dominici,Alejandro Ribeiro,Claudio Battiloro*

Main category: cs.LG

TL;DR: 本文提出HilbNets框架，在希尔伯特丛上定义卷积操作，通过两步采样实现无限维流形信号的深度学习，并证明其收敛性和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架缺乏处理定义在不规则域上的无限维信号（如时间序列、概率分布、算子）的统一学习理论。

Method: 利用与希尔伯特丛关联的连接拉普拉斯算子作为卷积算子，通过两步采样：1）流形采样诱导出希尔伯特细胞层（Hilbert Cellular Sheaf），证明其拉普拉斯算子概率收敛到底层连接拉普拉斯；2）离散化信号并证明离散HilbNets收敛到连续架构且具有可迁移性。

Result: 在合成数据和真实任务上验证了框架的有效性，证明了离散HilbNets收敛到底层连续架构，且在不同采样间可迁移。

Conclusion: 该工作将经典拉普拉斯框架推广到每个点信号位于独立希尔伯特空间的场景，拓宽了几何学习的整体范围。
...
### [189] [SparseForge: Efficient Semi-Structured LLM Sparsification via Annealing of Hessian-Guided Soft-Mask](https://arxiv.org/abs/2605.06402)
*Liu Hanzuo,Chaofan Lin,Weixuan Sun,Yulong Wang,Key,Rayying,Mingyu Gao*

Main category: cs.LG

TL;DR: SparseForge提出了一种后训练半结构化剪枝框架，通过直接优化稀疏掩码而非增加重训练数据量，在仅用5B tokens的情况下使LLaMA-2-7B在2:4稀疏度下达到57.27%的零样本准确率，超越了稠密模型的56.43%，并接近使用40B tokens的SOTA方法（57.52%），显著提升了精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有后训练半结构化剪枝方法因结构耦合导致精度严重下降，依赖大规模稀疏重训练来恢复精度，计算成本高昂。作者旨在提出一种更高效的恢复方法，在不增加重训练数据量的前提下提升剪枝后模型精度。

Method: SparseForge结合Hessian感知的重要性估计与渐进退火软掩码策略，将软掩码转化为硬件可执行的半结构化稀疏模式，直接优化稀疏掩码本身而非通过大规模重训练来恢复精度。

Result: 在LLaMA-2-7B 2:4稀疏度下，SparseForge仅用5B重训练tokens即达到57.27%平均零样本准确率，超过稠密模型（56.43%），接近SOTA方法（57.52%）但后者需40B tokens，实现了8倍效率提升。

Conclusion: SparseForge通过直接优化稀疏掩码而非扩大重训练规模，能够高效恢复半结构化剪枝后的模型精度，且该精度-效率优势在多种模型族中具有一致性。
...
### [190] [FRInGe: Distribution-Space Integrated Gradients with Fisher--Rao Geometry](https://arxiv.org/abs/2605.06404)
*Gabriele Martino,Sebastian Tschiatschek*

Main category: cs.LG

TL;DR: 提出FRInGe方法，通过Fisher-Rao测地线在预测分布空间定义归因路径，替代传统Integrated Gradients的基线选择和插值策略，提升归因校准性。


<details>
  <summary>Details</summary>
Motivation: Integrated Gradients方法依赖启发式基线和直线路径，导致解释结果对离散化、饱和等问题敏感，缺乏鲁棒性。

Method: 在预测分布空间用最大熵参考替代输入基线，沿Fisher-Rao测地线定义插值轨迹，通过拉回Fisher度量和KL信任域稳定输入空间路径，沿此路径积分梯度得到归因。

Result: 在六个ImageNet架构上，FRInGe显著提升校准导向的归因指标（尤其MAS分数），在扰动AUC和infidelity上保持竞争力。

Conclusion: FRInGe通过几何化的概率空间路径设计，有效改善了归因方法的校准性和鲁棒性。
...
### [191] [Federated Cross-Client Subgraph Pattern Detection](https://arxiv.org/abs/2605.06433)
*Selin Ceydeli,Rui Wang,Kubilay Atasu*

Main category: cs.LG

TL;DR: 本文针对分布式图场景下子图模式检测的表示等价性问题，提出了一种逐层嵌入交换框架，通过每步同步中间节点表示来弥合集中式与分布式GNN之间的表征鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的子图模式检测方法假设图是集中式可访问的，但在分布式场景中，跨分区边界的子图模式在局部不可识别，存在表示等价性差距。

Method: 提出逐层嵌入交换框架：在每层前向传播时同步客户端间的中间节点表示，同时不暴露原始特征和标签；基于扩展子图假设和共享模型参数，恢复与集中式GNN相同的节点表示。

Result: 在包含循环、双团、分散-聚合模式的合成有向多重图上实验表明：嵌入交换与联邦参数聚合互补而非可互换；两者组合且嵌入按步（而非按epoch）新鲜同步时，可弥合大部分表示差距。

Conclusion: 每步新鲜的嵌入交换是分布式GNN子图模式检测的关键，与联邦参数聚合结合能有效弥合表示等价性差距。
...
### [192] [Hyperbolic Concept Bottleneck Models](https://arxiv.org/abs/2605.06440)
*Daniel Uyterlinde,Swasti Shreya Mishra,Pascal Mettes*

Main category: cs.LG

TL;DR: 本文提出双曲概念瓶颈模型（HypCBM），通过将概念嵌入双曲空间并利用蕴含锥实现层次化、稀疏的概念激活，无需额外监督即可实现层次一致的人机交互干预，在稀疏数据下性能匹敌使用20倍数据的欧氏模型。


<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型将概念嵌入平坦欧氏空间并视为独立正交维度，忽略了概念间固有的层次化语义结构，导致模型与真实概念组织方式不匹配。

Method: 提出后验框架HypCBM，将概念激活重新表述为双曲空间中的非对称几何包含关系；利用蕴含锥的包含余量作为测试时的稀疏层次感知激活信号，并引入自适应缩放定律实现沿概念树的层次一致用户干预传播。

Result: 在稀疏数据场景下，HypCBM性能匹敌使用20倍数据的后验欧氏模型，具有更强的层次一致性和对输入损坏的鲁棒性。

Conclusion: 将概念瓶颈建模在双曲空间中可以更好地捕捉概念的层次结构，在不增加额外监督或学习模块的情况下提升可解释性和鲁棒性。
...
### [193] [FedFrozen: Two-Stage Federated Optimization via Attention Kernel Freezing](https://arxiv.org/abs/2605.06446)
*Junye Du,Zhenghao Li,Yushi Feng,Long Feng*

Main category: cs.LG

TL;DR: 提出FedFrozen框架，通过先全模型热身训练再冻结注意力机制的查询/键模块、仅优化值模块的两阶段方法，有效缓解异构联邦学习中的客户端漂移问题。


<details>
  <summary>Details</summary>
Motivation: 观察到Transformer架构在异构联邦训练中比传统模型更鲁棒，希望探究注意力机制中不同参数组件对联邦优化的影响，从而设计更优的异构联邦学习方法。

Method: 将注意力模块分解为查询/键块（决定注意力核）和值块（在核下进行语义变换），提出两阶段联邦优化框架FedFrozen：第一阶段全模型热身训练，第二阶段冻结查询/键块、仅优化值块。

Result: 仿真实验验证了预测的偏置-漂移行为，真实数据实验表明FedFrozen在异构联邦学习中提升了Transformer模型的稳定性和有效性。

Conclusion: 通过注意力机制的组件分解和两阶段优化策略，FedFrozen有效改善了异构联邦学习中的客户端漂移问题，且热身时长的选择存在显式的权衡关系。
...
### [194] [Scene-Adaptive Continual Learning for CSI-based Human Activity Recognition with Mixture of Experts](https://arxiv.org/abs/2605.06447)
*Wenhan Zheng,Yuyi Mao,Ivan Wang-Hei Ho*

Main category: cs.LG

TL;DR: SAMoE-C是一种基于混合专家系统的跨域CSI人体活动识别方法，通过注意力路由器和轻量训练协议实现场景自适应，在保持高精度的同时大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CSI的HAR方法在不同物理环境（域偏移）下性能下降严重，而已有的持续学习方法扩展性差、依赖大容量的重放缓冲区或推理成本随域数量线性增长。

Method: 提出SAMoE-C，将跨域CSI-HAR建模为混合专家系统，包含：(1) 一组聚类专家模块；(2) 基于注意力的语义路由器，仅激活与输入相关的专家；(3) 使用极小的重放缓冲区稳定路由器域判别能力的训练协议。

Result: 在四场景CSI数据集上，SAMoE-C达到了接近最先进的识别准确率，同时保持了显著更低的推理成本。

Conclusion: SAMoE-C通过模块化专家、选择性激活和轻量训练协议，实现了可扩展、低训练开销、高计算效率的跨域CSI-HAR部署，适用于真实场景。
...
TL;DR: 本文提出基于粗化层级结构多样性追踪的图级表示方法——多样性曲线，通过边收缩粗化提升表达能力，并在聚类、可视化、分子图比较等任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的图级表示方法在处理不同基数的图（即使来自同一分布）时存在困难，且无监督任务需要可解释、可扩展且可靠的、感知大小的图表示。

Method: 提出多样性曲线（diversity curves），通过追踪图在粗化层级中的结构多样性来生成图嵌入。具体包括：利用图的散布（spread）这一新的等距不变量来编码度量多样性和几何结构，采用边收缩粗化策略并证明其能提升表达能力。

Result: 在多个任务上验证有效性：（i）不同大小模拟图的聚类与可视化，（ii）单细胞图几何区分，（iii）分子图数据集结构比较，（iv）几何形状表征。

Conclusion: 多样性曲线是一种可解释、高效、可直接比较的图级表示方法，比单纯的结构描述符更具表达能力，在多种实际图分析任务中优于基线方法。
...
TL;DR: 本文扩展了MANTRA基准数据集，发现GNN和HOMP方法在正确表示下都能饱和该基准，但现有模型无法泛化到数据组合结构之外，揭示了拓扑深度学习评估中的关键问题。


<details>
  <summary>Details</summary>
Motivation: 拓扑深度学习模型日益增多，但缺乏统一的评估标准；拓扑对象特有的结构细化等操作使得评估比图数据更复杂。

Method: 将MANTRA数据集扩展到更多样化的流形同胚类型；提出基于表示多样性和三角剖分细化的新评估协议。

Result: GNN和HOMP在正确表示和特征分配下都能饱和基准；现有模型无法在组合结构外泛化。

Conclusion: 需要开发能理解拓扑结构且独立于尺度的模型，本文为未来模型评估和拓扑感知归纳偏置的发展提供了框架。
...
TL;DR: 本文提出了一种基于算子理论的离线强化学习表示学习框架，能够从击中时间观测中恢复受控马尔可夫过程的有向时间几何结构。基于该理论，作者设计了同构嵌入学习（IEL）算法，在离线迷宫 locomotion 数据上实现了最先进的基础策略学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常产生对称距离或不满足三角不等式，无法准确捕捉决策过程中的有向时间几何结构。

Method: 提出算子理论表示学习框架，将期望击中时间实现为潜在位移的线性泛函，并设计IEL算法，结合HILP风格一致性目标和显式击中时间回归。

Result: 证明了表示存在性与唯一可识别性；给出了有限维实现下的全局误差界；提供了有限样本保证；实验表明IEL在离线迷宫 locomotion 数据上达到了最先进水平。

Conclusion: 该框架能够学习非对称且满足三角不等式的有向几何结构，支持鲁棒的基于图的多阶段长程规划。
...
TL;DR: 本文提出PBKV系统，通过预测工作流中的智能体调用序列，实现KV-Cache的高效管理，在动态工作流中相比LRU提升1.85倍速度，在静态工作流中相比SOTA提升1.26倍。


<details>
  <summary>Details</summary>
Motivation: 现有KV-Cache管理方法要么在智能体级别管理，无法利用工作流内的复用机会；要么在工作流级别管理，但假设工作流调用的是静态智能体序列。然而实际工作流是动态的，智能体调用序列取决于任务上下文，现有方法无法高效处理这种动态性。

Method: PBKV系统通过融合历史工作流的引导和当前工作流的上下文，预测未来几步的智能体调用；基于预测结果评估缓存条目的复用潜力，将高潜力条目保留在GPU内存中；同时在缓存淘汰和预取时保守使用预测结果，以应对预测误差。

Result: 在三个工作流基准测试中，PBKV在动态工作流上相比LRU实现最高1.85倍加速，在静态工作流上相比当前最优方法KVFlow实现最高1.26倍加速。

Conclusion: PBKV通过预测驱动的KV-Cache管理，有效解决了动态工作流中智能体调用序列不确定带来的缓存管理挑战，显著提升了推理效率。
...
### [199] [Q-MMR: Off-Policy Evaluation via Recursive Reweighting and Moment Matching](https://arxiv.org/abs/2605.06474)
*Xiang Li,Nan Jiang*

Main category: cs.LG

TL;DR: 提出Q-MMR框架，通过矩匹配学习数据权重实现离策略评估，在仅需Q^π可实现性条件下获得与函数复杂度无关的有限样本保证。


<details>
  <summary>Details</summary>
Motivation: 现有离策略评估方法在一般函数近似下缺乏有限样本保证，或依赖于覆盖假设等强条件。

Method: Q-MMR通过矩匹配目标函数，以自顶向下归纳方式为每个数据点学习标量权重，使得加权奖励近似目标策略下的期望回报。

Result: 在仅需Q^π可实现性条件下，建立了与函数类统计复杂度无关（维数自由）的有限样本保证；与重要性采样、线性FQE等方法建立了联系；对覆盖条件提供了新见解。

Conclusion: Q-MMR是一种理论上优雅且实用的离策略评估方法，对离线强化学习中的覆盖概念提供了新的理解。
...
### [200] [Operator-Guided Invariance Learning for Continuous Reinforcement Learning](https://arxiv.org/abs/2605.06500)
*Zuyuan Zhang,Fei Xu Yu,Tian Lan*

Main category: cs.LG

TL;DR: 本文提出VPSD-RL框架，通过李群作用与拉回算子发现连续强化学习中的值保持结构，利用确定方程残差最小化学习无穷小生成元，实现近似值保持结构的发现与利用，提升数据效率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 连续时间/状态/动作空间的强化学习常因数据需求大且易受干扰而脆弱，现有方法多局限于预设对称性和精确等变性，缺乏发现更一般值保持结构的能力。

Method: 将连续RL建模为受控扩散过程，通过李群作用与拉回算子定义值保持映射；证明值保持结构存在的条件；通过最小化HJB残差学习无穷小生成元；利用ODE流指数化得到有限变换；通过转移增强和变换一致性正则化集成到RL中。

Result: 有界生成元/奖励残差保证最优值函数沿近似轨道的定量稳定性，灵敏度由有效视界刻画；在连续控制基准上观察到数据效率和鲁棒性提升。

Conclusion: VPSD-RL能够自动发现精确和近似的值保持结构，为连续RL提供了一种利用非线性算子发现更一般结构的新范式。
...
TL;DR: PACZero是一系列PAC隐私的零阶机制，用于大语言模型微调，能在互信息为零（I=0）的条件下实现可用性能。通过符号量化子集聚合的零阶梯度，在一致步骤中实现零条件互信息，提出PACZero-MI（预算化互信息）和PACZero-ZPL（I=0通过均匀硬币翻转）两种变体，在SST-2和SQuAD上使用OPT-1.3B和OPT-6.7B进行评估，在I=0时达到接近非私有基线的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私（DP）框架在低隐私预算（ε<1）下无法提供可用性能，且在高隐私保护需求下（如MIA后验成功率保持先验水平）DP需要ε=0和无限噪声，导致实用价值丧失。作者旨在提出一种能在严格隐私保护下仍保持实用性的微调方法。

Method: 基于PAC隐私框架，核心思想是仅当发布结果依赖于具体哪个候选子集是秘密时才计算互信息。采用符号量化的子集聚合零阶梯度，当所有候选子集对更新方向一致时，该步骤的发布信号零条件互信息。提出两个变体：PACZero-MI（通过精确校准二元发布来预算互信息）和PACZero-ZPL（在分歧步骤用均匀硬币翻转实现零互信息）。

Result: 在SST-2上使用OPT-1.3B全参数微调且I=0时，PACZero-ZPL达到88.99%准确率，仅比非私有MeZO基线（91.1%）低2.1个百分点。在SST-2和SQuAD上，OPT-1.3B和OPT-6.7B在LoRA和全参数两种设置下均取得竞争性结果，且是首个在ε<1高隐私保护下产生可用性能的方法。

Conclusion: PACZero证明了在高隐私保护（I=0）条件下，大语言模型微调仍可实现可用性能，打破了DP框架在低隐私预算下的实用性瓶颈。PACPrivacy框架在成员推理攻击防御方面有独特优势，为隐私保护的LLM微调提供了新的研究方向。
...
TL;DR: 首次大规模研究表格基础模型的逐层推理机制，发现深度冗余，并提出仅用20%参数的单层循环模型达到相近性能。


<details>
  <summary>Details</summary>
Motivation: Transformer-based表格基础模型在小到中型表格预测任务中表现优异，但其推理机制尚未得到充分探索。

Method: 对6个最先进的表格上下文学习模型进行逐层动力学的机械性研究，分析预测如何随层深度涌现，识别不同的推理阶段，揭示潜空间动力学。

Result: 发现多个模型存在显著的深度冗余，推理阶段表现为迭代精炼和重叠计算；基于此设计了一个循环单层模型，仅用20%参数即可达到可比性能。

Conclusion: 表格基础模型存在深度冗余，通过循环单层设计可以有效压缩模型规模而不损失性能。
...
TL;DR: 本文论证了基础模型的分布外（OOD）问题在结构上与以往不同，模型中心方法存在参数覆盖上限，提出智能体系统作为解决该问题必需的补充范式。


<details>
  <summary>Details</summary>
Motivation: 基础模型在开放世界中面临的知识边界、能力上限、组合偏移和开放式任务变化等OOD现象，与以往研究设置存在本质差异，且预训练和后训练分布通常仅部分可观，现有模型中心范式无法解决。

Method: 采用四步论证：1）给出阶段感知的OOD形式化定义；2）证明参数覆盖上限定理；3）从感知、策略选择、外部动作、闭环验证四个特性刻画智能体OOD系统并证明其扩展了可达集；4）回应七个反论并给出研究议程。

Result: 证明存在模型中心方法（无论训练时还是测试时）无法在容忍度ε内处理的输入；证明智能体系统在结构上严格扩展了可处理范围。

Conclusion: 模型中心方法与智能体方法互补，FM-OOD进展需要将智能体范式作为独立研究方向给予明确认可。
...
TL;DR: 本文发现RLVR的推理能力提升集中在rank-1成分上，并通过周期性替换实验揭示其存在隐式奖励过拟合现象，进一步分析了RLVR的三种特性：知识专一性、奇异值重尾分布以及左奇异向量对齐趋势。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明RLVR增强的推理能力主要集中在rank-1成分上，本文旨在深入探究这一现象背后的机制与特性。

Method: 采用周期性rank-1替换（Periodic Rank-1 Substitution）方法进行实验，并基于奇异值分解对模型参数进行谱分析。

Result: 发现RLVR存在隐式奖励过拟合：训练过程中奖励较低时测试集表现仍可令人满意；rank-1成分仅保留数学推理能力；各线性层奇异值呈重尾分布；rank-1左奇异向量在训练中表现出更强的对齐趋势。

Conclusion: 揭示了RLVR塑造模型参数的底层机制，为改进现有RL训练范式或实现持续学习提供了潜在思路。
...
TL;DR: 本文通过引入可处理的替代路径和Feynman-Kac表示，系统刻画了扩散后验采样器（如DPS和STSL）的偏差来源，解释了低温度不稳定性和早期引导停止等常见修正策略的理论基础。


<details>
  <summary>Details</summary>
Motivation: 扩散后验采样器在逆问题中被广泛使用，但其理论行为尚不清晰：即使在精确先验分数下，其输出也存在偏差，且低温度离散化会变得不稳定。需要从理论上理解这些偏差的来源和结构。

Method: 引入一个可处理的替代路径连接真实后验和高斯分布，与采样器路径进行比较；两者的密度比满足一个带有反应项的抛物型偏微分方程；利用Feynman-Kac表示将Radon-Nikodym修正表达为显式的路径期望。

Result: 1) DPS的修正项是Ornstein-Uhlenbeck路径期望，耦合了数据条件协方差与奖励曲率；2) STSL通过辅助漂移将轨迹导向低不确定区域，消除了DPS反应项的空间变化部分；3) 早期引导停止缓解了前向欧拉积分导致的低温度不稳定性。

Conclusion: 该框架阐明了采样器偏差、解释了现有修正策略，并指导了稳定变体的设计。
...
TL;DR: 本文提出MELO方法，通过在线聚合不同遗忘因子的指数加权最小二乘适应专家，在分布漂移下实现稳定与适应的平衡，无需提前知道最佳适应记忆长度，并在法国电力负荷预测中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在线预测中分布漂移普遍存在，预测器需要在平稳状态下保持稳定，在状态转换时快速适应，但最佳适应记忆长度未知，现有方法难以自动平衡这一权衡。

Method: 提出MELO方法：用多个遗忘因子的EWLS适应专家包装基础预测器池，再通过无参数在线聚合规则MLpol聚合原始预测和EWLS适应后的预测，实现跨适应尺度的对冲。

Result: 理论上有确定性oracle不等式保证；在法国电力负荷预测（涵盖COVID-19封锁期）中，相比基础预测器+MLpol降低34.7%的RMSE，且优于使用外部政策协变量的TabICL。

Conclusion: MELO是一种轻量级、无需模型重训练、无需先验知识的在线预测方法，能有效应对分布漂移，在实证中表现优异。
...
TL;DR: 本文提出了一种基于强化学习的顺序框架，用于在分子随机性和实验室条件变化双重不确定性下优化基因电路设计。通过预训练的摊销方法，避免了传统贝叶斯方法中耗时的推理和优化步骤，实现了基于观察的即时适应。在异源基因表达和repressilator电路模型上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 生物系统设计受限于两种不确定性：生物分子反应的内在随机性和不同实验条件间的变异性。现有贝叶斯方法虽能通过迭代实验-推理-优化循环处理不确定性，但每轮实验后的推理和优化步骤计算成本高，导致设计周期延迟。

Method: 采用基于微分方程或马尔可夫跳跃过程的仿真模型，结合强化学习策略方法。核心创新是提出"摊销化"预训练方法：在可能的不确定参数分布上预先训练模型，从而在实验设计周期中避免显式参数推理，实现基于观测的即时适应。

Result: 在异源基因表达和repressilator电路两种模型上验证了框架的有效性，证明该方法能够高效处理分子噪声和跨实验室变异性。

Conclusion: 该框架通过预训练摊销策略解决了传统贝叶斯方法的计算瓶颈，为在双重不确定性下高效优化基因电路提供了可行方案。
...
TL;DR: 提出EDDY引导机制，通过散度自由动力学在保持边缘分布不变的情况下提升扩散模型与流匹配模型生成样本的多样性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成多样性方面存在不足，样本间趋于同质化，需要在保持质量的同时提升生成多样性。

Method: 利用Fokker-Planck方程的对称性，通过核函数驱动的反对称成对矩阵场构造散度自由扰动，在不改变边缘分布的前提下改变粒子轨迹，并提出针对文本到图像生成的高效近似方法。

Result: 在合成分布和文本到图像生成任务上，EDDY相比基线方法在保持分布保真度的同时显著提升了生成多样性。

Conclusion: EDDY是一种无需额外训练即可提升生成多样性的有效引导机制，且通过近似计算可在实际场景中高效应用。
...
TL;DR: 本文针对树集成模型下的反事实解释优化问题，提出了基于约束规划(CP)的方法CPCF，并与MaxSAT和MILP方法进行了系统性比较，发现CP综合表现最优，但不同范式各有适用场景。


<details>
  <summary>Details</summary>
Motivation: 反事实解释的信任度取决于推荐变更是否真正最小化，次优解释可能导致用户付出不必要的高昂代价，且启发式错误可能带来不公平的后果。因此需要计算最优的反事实解释。

Method: 提出CPCF方法，将数值特征编码为由分裂阈值诱导的区间域，离散特征保留原生有限域表示，构建紧凑的有限域约束规划模型；同时将MaxSAT扩展至软投票集成，并与MILP最优方法进行比较。

Result: 在10个数据集和3类树集成上的实验显示：CP综合表现最佳；MaxSAT在硬投票集成上表现突出；MILP在分裂层级数适中的摊销推断场景中仍具竞争力。

Conclusion: 不同数学规划范式各有优势，CP最为通用，MaxSAT适合硬投票集成，MILP适合中等复杂度的摊销推理场景，为实际应用中选择最优方法提供了指导。
...
TL;DR: 本研究使用TCGA-BRCA基因表达数据，评估了逻辑回归、随机森林和支持向量机在乳腺癌亚型分类中的表现，发现逻辑回归在亚型级别表现最稳定均衡，随机森林对少数亚型识别不足，SVM受特征维度影响较大。


<details>
  <summary>Details</summary>
Motivation: 基因表达数据具有高维小样本的特点，给机器学习模型带来挑战，因此需要评估模型复杂度和特征选择对乳腺癌亚型分类性能的影响。

Method: 使用TCGA-BRCA基因表达数据，训练逻辑回归、随机森林和支持向量机三种模型，采用不同数量的高变异基因（50到20,518个），通过分层5折交叉验证，以准确率和宏F1分数评估性能。

Result: 所有模型均取得高准确率，但宏F1分析显示亚型级别性能存在显著差异：逻辑回归表现最稳定均衡，能更好检测稀有亚型；随机森林在少数亚型上表现不佳；SVM对特征维度敏感。
...
### [211] [SNAPO: Smooth Neural Adjoint Policy Optimization for Optimal Control via Differentiable Simulation](https://arxiv.org/abs/2605.06570)
*Dmitri Goloubentsev,Natalija Karpichina*

Main category: cs.LG

TL;DR: SNAPO是一个将神经策略嵌入可微模拟器的框架，通过平滑近似处理硬约束，利用伴随方法在一次反向传播中同时计算策略参数和所有输入的精确梯度，在多个领域实现了高效训练和快速敏感性分析。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，动态规划可精确求解但状态维度呈指数级扩展，黑盒强化学习能处理高维状态但训练慢且无法生成敏感性信息，需要一种兼具可扩展性与可微性的方法。

Method: 将神经策略嵌入已知的可微模拟器中，用平滑近似替代硬约束，通过单次伴随反向传播计算目标函数相对于所有策略参数和所有输入的精确梯度。

Result: 天然气存储：训练不到1分钟，365条远期曲线敏感性零额外成本；养老金管理：敏感性速度提升6.5-200倍；制药制造：4单元工艺链跨单元敏感性，5次伴随传播74.5毫秒生成20个ICH Q8监管敏感性。

Conclusion: SNAPO能够以仅相当于一次反向传播的成本，在训练策略的同时生成任意数量的敏感性信息，效率极高。
...
TL;DR: 提出CLAD框架，结合聚类联邦学习与双模式微架构，解决物联网入侵检测中的设备异构性和标签稀缺问题，在80%未标记客户端场景下检测性能提升30%，通信成本降低50%。


<details>
  <summary>Details</summary>
Motivation: 物联网和工业物联网的快速扩张导致攻击面庞大且异构，传统集中式入侵检测系统面临隐私和泛化问题；联邦学习虽提供隐私保护方案，但难以处理设备行为多样性和大量未标记数据。

Method: 提出CLAD框架，包含聚类联邦学习（CFL）和双模式微架构（DM²A）。DM²A通过共享编码器和两个分支实现无监督异常检测与有监督攻击分类的联合学习；聚类组件动态聚合流量模式相似的设备，防止全局模型发散。

Result: 在80%客户端未标记的场景下，检测性能相比最先进基线提升30%，通信成本仅为其一半。

Conclusion: 所提集成方法确保了数据不被丢弃，保留了不同运营模式，显著优于现有方案。
...
TL;DR: 本文发现深度学习中方向对齐与损失收敛可以解耦，并提出GONO优化器，通过基于梯度方向一致性自适应调整动量系数，在理论上保持收敛速度，在实践中实现振荡检测和竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 现有优化器（如Adam、SGD、RMSprop）缺乏利用梯度方向时间一致性的显式机制，仅依赖基于幅度的信号，无法有效区分平台、鞍点与真实收敛。

Method: 提出GONO优化器，基于连续梯度余弦相似度（cc_t）自适应调整Adam的动量系数beta_1：方向一致时放大动量，振荡时抑制动量。

Result: cc_t振荡检测F1=1.00（梯度范数仅0.45）；在MNIST（98.15%）、CIFAR-10（43.14%）、ResNet-18（75.44%）上与AdamW保持竞争力。

Conclusion: 方向对齐是一种理论上可证明、实践上可操作的优化信号，GONO在理论上匹配Adam的O(1/sqrt(T))收敛速率，并能退化回Adam。
...
TL;DR: 提出了GRL-Safety基准，系统评估了12种图表示学习方法在5个安全维度（鲁棒性、OOD泛化、类别不平衡、公平性、可解释性）上的表现，发现方法家族并非决定安全性的唯一因素，基础模型时代的方法呈现维度特定的优势，且存在难以通过模型选择解决的能力缺口。


<details>
  <summary>Details</summary>
Motivation: 现有图表示学习评估主要关注干净迁移、自适应和任务覆盖度，忽略了部署环境中的各种压力因素（如信号噪声、上下文变化、标签支持不足、结构群体偏移、预测证据缺失）对方法可靠性的影响。

Method: 构建GRL-Safety多轴安全评估基准，在25个图数据集上标准化评估12种代表性方法（包括拓扑嵌入、监督GNN、自监督图模型和图基础模型），覆盖5个安全轴（完整性鲁棒性、OOD泛化、类别不平衡、公平性、可解释性），按每个轴及其子条件分别报告结果而非聚合单一分数。

Result: 发现三个跨轴洞察：1）安全性由表示设计与压力因素的交互决定，而非方法家族；2）基础模型时代的方法仅在特定安全轴上有优势，而非全面领先；3）多种部署场景即使对最佳方法仍具挑战，揭示出需要新的鲁棒性、自适应或训练目标来填补能力缺口。

Conclusion: GRL-Safety揭示了现有图表示学习方法在安全性方面的局限性，指出未来研究需要超越模型选择，探索新的鲁棒性机制和训练目标来应对部署中的安全挑战。
...
TL;DR: 本文提出一种分布鲁棒的方法来学习凸优化中一阶方法的超参数，通过最小化Wasserstein分布鲁棒版本的性能估计问题（PEP）来学习算法参数（如步长），统一了经典学习优化（L2O）和最坏情况最优算法设计（PEP）两种极端情况，并通过理论证明和实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有学习优化（L2O）方法在训练分布上表现良好但缺乏鲁棒性保证，而最坏情况最优的PEP方法虽然鲁棒但过于保守。作者希望统一这两种方法，在性能和鲁棒性之间取得平衡。

Method: 提出Wasserstein分布鲁棒PEP框架：当鲁棒半径趋近于0时退化为L2O，半径增大时退化为最坏情况PEP。通过随机梯度下降求解，每次迭代需对内部半正定规划求导。给出高概率泛化界，证明真实风险不超过样本内L2O最优值加上随样本量缩小的松弛项，且不劣于最坏情况PEP界。

Result: 在无约束二次优化、LASSO和线性规划基准测试上，学习到的算法在具有可证明鲁棒性的同时实现了强泛化性能，优于最坏情况最优和普通L2O基线方法。

Conclusion: 分布鲁棒方法有效统一了基于数据的算法学习和最坏情况算法设计，提供了可证明的鲁棒性保证与良好的泛化性能之间的权衡。
...
TL;DR: 本文提出了一种理论框架，将编码器几何（双Lipschitz性质）与神经图编辑距离（GED）估计质量联系起来，并通过FSW-GNN作为即插即用模块，显著提升了GED预测和排序性能。


<details>
  <summary>Details</summary>
Motivation: 尽管神经图匹配架构在GED估计上取得了显著进展，但编码器几何在其中的作用仍不明确，缺乏理论理解。

Method: 开发理论框架连接编码器几何与GED估计质量；提出使用双Lipschitz的FSW-GNN作为编码器，替换现有架构中的标准GNN。

Result: 几何感知变体在多个基准数据集上显著提升了GED预测与排序指标；消融实验和迁移实验证实了改进源于编码器几何的优化。

Conclusion: 编码器几何是神经图匹配中一个有效且重要的设计原则。
...
TL;DR: 本文首次从泛函分析角度严格证明，带L2正则化（权重衰减）的Transformer损失函数满足Villani强制能量函数标准，进而推导出log-Sobolev常数和Poincaré常数，为带噪SGD的有限时间收敛保证和PAC-Bayesian泛化界提供理论基础，并通过GPT-Neo-125M实验验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 权重衰减在大型语言模型中广泛用作正则化器，但其在塑造Transformer损失景观中的精确作用在理论上尚未被充分探索。本文旨在填补这一空白，从严格的数学角度表征正则化后的Transformer目标函数。

Method: 1. 证明正则化损失函数ℱ满足Villani强制能量函数标准（无穷可微、至少二次增长、高斯可积尾部、微分增长条件）。2. 推导显式的log-Sobolev和Poincaré常数。3. 提出可扩展的Villani诊断指标Ψ_s(θ)，并使用Hutchinson迹探针高效估计。4. 在GPT-Neo-125M模型上对Penn Treebank和WikiText-103数据集进行实验验证。

Result: 1. 证明了正则化Transformer损失函数具有 coercive 能量函数结构。2. 推导出C_LS ≤ λ^{-1} + d/λ²形式的常数，将正则化强度λ和模型维度d与收敛保证联系起来。3. 实验验证了Ψ_s的二次增长、Hessian谱膨胀以及指数收敛行为，与log-Sobolev分析一致。

Conclusion: 权重衰减不仅能在经验上改善泛化，还能建立快速Langevin混合所需的数学条件，为深度学习中的曲率感知优化提供理论依据。
...
TL;DR: 本文证明了带有softmax注意力的多层Transformer可以通过逐层执行归一化梯度下降来实现上下文逻辑回归，并提供了训练收敛和泛化保证。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer为何能够通过上下文学习（ICL）隐式执行算法，特别是线性分类任务中的ICL机制。

Method: 构建了一类多层Transformer，每层精确执行一步关于上下文损失的归一化梯度下降；通过训练单层自注意力层（由一步梯度下降监督）并循环应用该层得到循环模型。

Result: 提供了自注意力层的训练收敛保证以及循环模型的分布外泛化保证。

Conclusion: softmax Transformer能够有效充当上下文学习器，推进了对ICL机制的理论理解。
...
TL;DR: SoftSAE提出了一种动态Top-K选择机制，允许稀疏自编码器根据输入复杂度自适应调整激活特征数量，解决了固定稀疏度无法适配数据内在复杂度的问题。


<details>
  <summary>Details</summary>
Motivation: 传统TopK SAE使用固定的K值，忽略了真实世界数据在不同样本间具有变化的局部固有维度，简单和复杂输入所需的特征数量不同，固定稀疏度会引入噪声或丢失结构信息。

Method: 提出SoftSAE，采用可微分的Soft Top-K算子，通过学习实现依赖输入的动态稀疏度选择，使模型能根据每个输入的复杂度自适应调整激活特征数量。

Result: 实验表明SoftSAE不仅能发现有意义的特征，还能为每个概念选择正确数量的特征。

Conclusion: 自适应的动态稀疏度能更好地匹配数据结构，解释长度反映了输入的信息量，是一种优于固定稀疏度的方案。
...
TL;DR: 本文揭示了LLM中注意力沉溺现象的机制根源：自注意力中的值聚合过程导致方差差异，而FFN层的超神经元激活进一步放大这种差异，迫使初始token形成结构锚点。实验通过两种干预手段验证了这一因果链，并提出head-wise RMSNorm架构改进来加速预训练收敛。


<details>
  <summary>Details</summary>
Motivation: 尽管注意力沉溺现象（初始token垄断注意力分数）在LLM中普遍存在，但其结构性起源尚不明确，本文旨在提供机制性解释。

Method: 1. 理论推导：将沉溺现象归因于自注意力中的值聚合过程导致的方差差异，以及FFN层超神经元激活对其的放大作用；2. 因果验证：通过注意力掩码修改和token表示方差放大两种干预手段；3. 架构改进：提出head-wise RMSNorm来稳定值聚合输出。

Result: 两种干预均能在任意位置复现注意力沉溺；提出的head-wise RMSNorm在预训练中能显著加速收敛。

Conclusion: 本文提供了注意力沉溺现象的机制理解，为系统控制沉溺形成奠定基础，架构改进实验表明恢复位置间的统计奇偶性对训练效率有重要意义。
...
TL;DR: 本文提出BRPC（贝叶斯递归投影校准）框架，解决了在线贝叶斯校准中模型偏差与参数混淆、非平稳数据流及系统突变三个核心问题，结合投影校准与重启动机制，实现高精度、鲁棒的在线校准。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯校准存在参数-偏差混淆问题，且为离线、平稳假设下的方法，不适用于数字孪生中系统演化、漂移和突变场景。现有数据同化方法虽支持序列更新，但未显式建模系统偏差且在突变下效果不佳。

Method: 将投影校准扩展到在线场景：分离无偏差粒子的参数更新与条件高斯过程的偏差更新，保持可辨识性；集成重启动机制检测突变并重置校准过程。

Result: 在合成数据和植物仿真基准上，BRPC在渐变场景中校准精度更高；重启动增强版在突变场景下相比滑动窗口贝叶斯校准和数据同化基线，鲁棒性和预测性能更优。

Conclusion: BRPC框架有效解决了在线校准中的非平稳性和突变挑战，兼具理论保障与实用性能，为数字孪生中的动态校准提供了新范式。
...
TL;DR: 本文融合概念解释与形式化因果解释，提出基于概念的反事实和对比解释方法，通过概念擦除技术枚举最小因果概念集，支持单样本及群体行为解释。


<details>
  <summary>Details</summary>
Motivation: 现有概念解释方法缺乏因果联系或仅支持单一概念，而形式化解释仅考虑低层特征（如像素），无法在高层概念层面提供因果解释。

Method: 提出概念层面的反事实与对比解释框架，利用概念擦除技术建立因果关系，并通过枚举算法找出所有最小因果概念集，聚合后支持个体及群体行为解释。

Result: 在多个模型和数据集上验证了方法的有效性，能够生成有用且用户友好的解释。

Conclusion: 通过连接概念解释与形式化因果解释两个方向，该方法在高层语义层面提供了因果完备的最小解释，兼具表达能力与可解释性。
...
### [223] [Inductive Venn-Abers and related regressors](https://arxiv.org/abs/2605.06646)
*Ivan Petej,Vladimir Vovk*

Main category: cs.LG

TL;DR: 本文将Venn-Abers预测器从二分类和有界回归扩展到无界回归，通过引入共形预测元素，实验表明基于Venn-Abers回归器导出的点回归器在大训练集上能略微提升预测效率。


<details>
  <summary>Details</summary>
Motivation: Venn-Abers预测器在二分类中具有良好有效性，但仅适用于二分类和有界回归，缺乏对无界回归场景的支持，严重限制了其应用范围。

Method: 通过引入共形预测（conformal prediction）的元素，将Venn-Abers预测器推广到无界回归场景，并结合模拟和实证研究评估其预测效率。

Result: 在大训练集上，基于Venn-Abers回归器导出的点回归器相比标准回归器在预测效率上有所提升。

Conclusion: 本研究成功将Venn-Abers预测器扩展到无界回归领域，验证了其在大训练样本下对预测效率的改善效果，为该方法的实际应用提供了可行方案。
...
TL;DR: 论文发现，在大型语言模型的监督微调阶段，使用与预训练相同优化器（即优化器-模型一致性）能实现更好的学习-遗忘权衡，比使用其他优化器或LoRA遗忘更少且性能相当或更好。通过实验和理论分析，揭示了优化器对激活值的正则化效应及其影响机制。


<details>
  <summary>Details</summary>
Motivation: 探索在LLM监督微调阶段，优化器选择对模型遗忘预训练知识与学习新任务能力的影响，特别是发现使用与预训练相同优化器能带来更好的学习-遗忘权衡。

Method: 通过控制实验和理论分析，研究优化器对模型激活值的正则化效应；对比Muon和AdamW在预训练和微调阶段的表现；设计合成语言建模实验验证Muon的机械记忆倾向。

Result: 1) 相同优化器（优化器-模型一致性）在SFT中遗忘更少；2) 优化器通过正则化效应塑造模型损失景观；3) Muon在推理任务微调中表现更差；4) Muon倾向于机械记忆，不利于小样本的模式学习。

Conclusion: SFT阶段应尽量保持与预训练阶段相同的优化器，以实现更好的知识保留与新任务学习。Muon优化器在推理任务微调中表现不佳，因其强机械记忆特性不利于小样本模式获取。
...
### [225] [UniPool: A Globally Shared Expert Pool for Mixture-of-Experts](https://arxiv.org/abs/2605.06665)
*Minbin Huang,Han Shi,Chuanyang Zheng,Yimeng Wu,Guoxuan Chen,Xintong Yu,Yichun Yin,Hong Cheng*

Main category: cs.LG

TL;DR: UniPool提出将MoE模型的每层独立专家替换为全局共享专家池，通过池级辅助损失和NormRouter实现稳定训练。实验表明，在5种规模模型上，UniPool比传统MoE提升验证困惑度，且使用仅41.6%-66.7%的专家参数就能达到或超越基线性能，证明专家参数无需随深度线性增长。


<details>
  <summary>Details</summary>
Motivation: 传统MoE架构中每层独立拥有专家集，导致深度扩展时专家参数线性增长。但实验发现深层使用随机路由仅下降1.0-1.6个点准确率，说明专家容量存在冗余，因此提出全局共享专家池以减少参数冗余。

Method: 将每层的独立专家替换为全局共享专家池，各层独立路由器可访问同一池；引入池级辅助损失平衡专家利用率；采用NormRouter实现稀疏且尺度稳定的路由。

Result: 在182M-978M五种LLaMA架构模型上，UniPool持续降低验证损失和困惑度，最高降低0.0386验证损失；使用仅41.6%-66.7%专家参数即可匹配或超越层式MoE性能。

Conclusion: 共享池设计下专家参数无需随深度线性增长，可实现亚线性增长；池大小可作为显式的深度缩放超参数；UniPool的收益可与细粒度专家分解方法组合。
...
### [226] [Understanding Annotator Safety Policy with Interpretability](https://arxiv.org/abs/2605.05329)
*Alex Oesterling,Donghao Ren,Yannick Assogba,Dominik Moritz,Sunnie S. Y. Kim,Leon Gatys,Fred Hohman*

Main category: cs.AI

TL;DR: 本文提出可解释的"标注者策略模型"(APMs)，从标注行为中学习标注者内部安全策略，无需额外标注负担即可揭示标注分歧来源（操作失误、政策模糊、价值多元），支持更精准透明的安全策略设计。


<details>
  <summary>Details</summary>
Motivation: 标注分歧普遍存在，但现有方法难以区分其真正来源（操作失误、政策模糊或价值多元），直接询问标注者推理过程成本高且不可靠。

Method: 提出了Annotator Policy Models (APMs)，一种可解释模型，仅从标注行为中学习标注者内部安全策略，无需额外标注工作量。

Result: APM准确率>80%，能忠实预测反事实编辑响应，在受控设置中恢复已知策略差异，并在LLM和人类标注中成功揭示政策模糊和价值观多元性。

Conclusion: APMs能够支持更有针对性、更透明和更具包容性的安全策略设计。
...
### [227] [ZAYA1-8B Technical Report](https://arxiv.org/abs/2605.05365)
...
TL;DR: ZAYA1-8B是一个基于MoE++架构的推理密集型专家混合模型，仅有7亿活跃参数（总计80亿），在数学和编程基准测试上达到或超越DeepSeek-R1-0528，并通过四阶段强化学习级联和Markovian RSA测试时计算方法，在AIME'25和HMMT'25上分别达到91.9%和89.6%的优异成绩。


<details>
  <summary>Details</summary>
Motivation: 构建一个参数高效但推理能力强大的MoE模型，验证在不到10亿活跃参数下能否匹配甚至超越更大规模的推理模型，同时探索高效的测试时计算扩展方法。

Method: 采用MoE++架构（700M活跃/8B总参数），从预训练阶段即融入推理数据并使用答案保留修剪方案；后训练采用四阶段强化学习级联（推理热身、RLVE-Gym课程、数学/代码RL、行为RL）；提出Markovian RSA测试时计算方法，递归聚合并行推理轨迹并仅保留有限长度的推理尾。

Result: 在多个数学和编程基准上匹配或超越DeepSeek-R1-0528；采用Markovian RSA后，AIME'25达91.9%，HMMT'25达89.6%，缩小了与Gemini-2.5 Pro、DeepSeek-V3.2、GPT-5-High等更大模型的差距。

Conclusion: 通过精心设计的训练流程和测试时计算方法，小参数MoE模型可以达到与大模型相当甚至更优的推理性能，为高效推理模型的设计提供了新思路。
...
### [228] [Partial Evidence Bench: Benchmarking Authorization-Limited Evidence in Agentic Systems](https://arxiv.org/abs/2605.05379)
*Krti Tallam*

Main category: cs.AI

TL;DR: 本文提出Partial Evidence Bench，一个用于衡量企业智能体在受限制的检索系统中产生看似完整但实际遗漏关键证据的失败模式的确定性基准测试，包含72个任务和四个评估维度。


<details>
  <summary>Details</summary>
Motivation: 企业智能体在权限受限的检索环境中运行时，即使访问控制机制正确执行，系统仍可能生成表面上完整但实际遗漏了位于调用者授权边界之外的关键证据的答案，这种静默过滤行为具有严重安全隐患，但目前缺乏可衡量的评估基准。

Method: 构建了一个确定性基准测试，包含三类场景（尽职调查、合规审计、安全事件响应）、72个任务，采用ACL分区语料库，提供标准答案、授权视图答案、完整性判定和结构化差距报告。从回答正确性、完整性感知、差距报告质量和危险完整性行为四个维度评估系统。

Result: 测试结果表明静默过滤在所有场景中都存在严重的安全风险，而显式的失败并报告行为能消除危险完整性行为且不会导致任务退化。实际模型运行显示，系统在过度声称完整性、保守地低估完整性和以企业可用形式报告不完整性方面存在模型依赖和场景相关的差异。

Conclusion: 该基准测试的贡献在于提供了一种无需人工评判或易受污染的静态语料库即可衡量治理关键型智能体失败模式的方法。
...
### [229] [BALAR : A Bayesian Agentic Loop for Active Reasoning](https://arxiv.org/abs/2605.05386)
*Aymen Echarghaoui,Dongxia Wu,Emily B. Fox*

Main category: cs.AI

TL;DR: 本文提出BALAR，一种无需微调的任务无关外循环算法，通过维护结构化信念状态、最大化期望互信息选择澄清问题，并动态扩展状态表示，使LLM智能体在多方交互中主动推理。在三个基准上准确率提升14.6%-38.5%。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在多轮交互场景中缺乏主动推理机制，无法有效判断缺失信息并决定下一步该问什么问题，大多采用被动式对话处理。

Method: BALAR算法包含三个核心：1) 维护潜在状态的结构化信念；2) 通过最大化期望互信息选择澄清性问题；3) 当当前状态表示不足时动态扩展状态表示。该算法无需微调，任务无关。

Result: 在AR-Bench-DC（侦探案件）、AR-Bench-SP（思维谜题）和iCraft-MD（临床诊断）三个基准上，BALAR分别取得14.6%、38.5%和30.5%的准确率提升，全面超越所有基线方法。

Conclusion: BALAR通过结构化信念维护和信息论驱动的主动提问策略，有效提升了LLM在多轮交互中的推理能力，且无需微调即可跨任务泛化。
...
### [230] [Intelligent CCTV for Urban Design: AI-Based Analysis of Soft Infrastructure at Intersections](https://arxiv.org/abs/2605.05402)
*Vinit Katariya,Seungjin Kim,Curtis Craig,Nichole Morris,Hamed Tabkhi*

Main category: cs.AI

TL;DR: 本研究提出AI驱动的分析框架，利用现有监控摄像头评估软干预措施（临时行人避难所和路缘延伸）对车辆速度和安全的影响。在明尼阿波利斯实验显示，无信号路口平均速度降低18.75%，85分位速度降低16.56%；信号路口平均速度降低20.0%，85分位速度降低17.19%，证明了软基础设施的交通静化效果和AI方法的实用性。


<details>
  <summary>Details</summary>
Motivation: 传统交通数据收集成本高、周期长，需要开发一种利用现有基础设施（如监控摄像头）的低成本、快速评估方法，以评估软干预措施对交通安全的影响。

Method: 采用深度学习与基于透视的速度估计算法，利用现有CCTV监控基础设施，在干预前后以及安装后第1周和第2周进行重复监测，评估驾驶员行为变化。

Result: 无信号路口平均速度和85分位速度分别下降18.75%和16.56%，过境交通量减少12.2%；信号路口平均速度和85分位速度分别下降20.0%和17.19%。

Conclusion: 软基础设施具有显著的交通静化效果，AI驱动的分析方法能够实现快速、低成本、基于证据的交通政策评估。
...
### [231] [When Helpfulness Becomes Sycophancy: Sycophancy is a Boundary Failure Between Social Alignment and Epistemic Integrity in Large Language Models](https://arxiv.org/abs/2605.05403)
*Jiechen Li,Catherine A. Barry,Rishika Randev,Janet Chen,Ella Jorgensen,Brinnae Bent*

Main category: cs.AI

TL;DR: 本文论证了LLM中的谄媚行为是社会对齐与认知完整性之间的边界失败，并提出三条件框架（用户线索、对齐行为、认知损害）和分类体系来界定和评估该现象。


<details>
  <summary>Details</summary>
Motivation: 现有研究将谄媚行为简单操作化为同意用户错误信念、立场反转等外部行为，忽略了认知完整性与社会对齐之间的微妙边界失败，需要更精确的理论框架来界定这一现象。

Method: 提出三条件框架：(1)用户表达信念/偏好/自我概念线索；(2)模型通过对齐行为向该线索偏移；(3)这种偏移损害认知准确性、独立推理或适当纠正。同时引入包含对齐目标、机制和严重程度的分类体系。

Result: 建立了比现有"外部行为"定义更精确的谄媚行为理论框架，区分了社会对齐与认知完整性之间的边界失败，并提出了分类学体系。

Conclusion: 呼吁边界感知评估、结构化评估标准和缓解策略，同时将该框架与关于谄媚行为的其他观点进行对话。
...
### [232] [Agentic Retrieval-Augmented Generation for Financial Document Question Answering](https://arxiv.org/abs/2605.05409)
*Yang Shu,Yingmin Liu,Zequn Xie*

Main category: cs.AI

TL;DR: 本文提出FinAgent-RAG，一个面向金融文档问答的智能体检索增强生成框架，通过迭代检索-推理循环、对比金融检索器、程序化推理和自适应策略路由器，在三个金融QA基准上显著优于现有方法，并降低41.3%的API成本。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法采用单次检索-生成范式，难以处理金融分析中复杂的组合推理链条，且缺乏对数值精度的保证。

Method: 提出三个创新模块：(1)对比金融检索器（硬负样本挖掘），(2)程序化推理模块（生成可执行Python代码），(3)自适应策略路由器（动态分配计算资源）。

Result: 在FinQA、ConvFinQA、TAT-QA上分别达到76.81%、78.46%、74.96%的执行准确率，超越最强基线5.62-9.32个百分点，API成本降低41.3%。

Conclusion: FinAgent-RAG在金融数值推理任务上兼具高性能与实用性，适合金融机构部署。
...
TL;DR: LaTA是一个完全在本地硬件上运行的开源LLM自动评分系统，专为LaTeX原生工作流的STEM课程设计。在俄勒冈州立大学ME 373课程中部署一学期后，它以零边际成本处理了约200名学生的每周作业，评分错误率仅0.02–0.04%，且使用LaTA的班级在期中考试和期末考试中分别比传统评分班级高出11%和8%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评分系统多依赖第三方API，违反FERPA法规并带来数据风险，且需要大量作业修改。作者希望开发一种完全本地运行、兼容现有LaTeX教学流程的零成本自动评分方案。

Method: LaTA采用四阶段流水线：ingest（输入）、segment（分段）、grade（评分）、report（报告）。使用本地部署的开源权重链式思维LLM（gpt-oss:120b），将学生作业与教师参考答案对比，并基于YAML格式的二元逐项评分标准进行评分。

Result: 在ME 373课程中部署，约200名学生，评分错误率0.02–0.04%；使用LaTA的班级期中考试高出11%，期末考试高出8%；学生在所有学习目标上自评信心大幅提升（Δ≥+1.49 Likert点，p<10⁻²⁷）。

Conclusion: LaTA表明，完全本地化的开源LLM评分系统可以在不牺牲评分质量的前提下，以零成本替代传统人工评分，同时提升学生学习表现和信心。
...
### [234] [From History to State: Constant-Context Skill Learning for LLM Agents](https://arxiv.org/abs/2605.05413)
*Haoyang Xie,Xinyuan Wang,Yancheng Wang,Puda Zhao,Feng Ju*

Main category: cs.AI

TL;DR: 提出了一种"常量上下文技能学习"框架，将可重复的智能体工作流从提示词迁移到模型权重中，通过轻量级任务族模块和紧凑状态块，在保持隐私的同时降低推理成本，在多个基准测试中取得优秀性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体面临隐私-成本-能力的矛盾：云端模型能力强但暴露敏感中间上下文，本地模型保护隐私但可靠性不足，且两者都因长提示词和历史记录而重复付出高成本。

Method: 提出context-to-weights框架：将可重复的工作流学习到轻量级任务族模块中，推理时仅依赖当前观测和一个紧凑状态块；使用确定性追踪器从任务进度生成状态块并提供子目标奖励，模块通过步骤级SFT训练和在线RL精炼。

Result: 在Qwen3-8B上：ALFWorld未见任务89.6%，WebShop 76.8%，SciWorld未见任务66.4%；相比ReAct基线，每轮提示词减少2-7倍；性能达到或超过已有强基线。

Conclusion: 证明了程序性上下文可以从提示词迁移到模型权重中，有效平衡了隐私、成本和能力三者关系。
...
### [235] [The Geopolitics of AI Safety: A Causal Analysis of Regional LLM Bias](https://arxiv.org/abs/2605.05427)
*Alif Al Hasan*

Main category: cs.AI

TL;DR: 本文提出了一种基于概率图模型（PGM）的因果框架来审计大语言模型（LLM）的安全机制，通过Pearl的do-算子隔离提示中注入文化人口统计信息的因果效应。实验发现，传统的观测性公平性指标会高估人口统计偏见，而因果分析揭示了西方模型对特定群体有更高的因果拒绝率，东方模型则整体干预率较低但对区域人口有针对性敏感。


<details>
  <summary>Details</summary>
Motivation: 当前的公平性评估主要依赖观测性度量，这种方法无法区分人口统计偏见与话题本身毒性带来的混杂效应，导致评估结果失真。需要一种因果方法来准确衡量LLM安全机制中的真实偏见。

Method: 引入概率图模型（PGM）框架，应用Pearl的do-算子从因果角度隔离文化人口统计信息对模型安全响应的因果效应，在七个不同来源的指令微调模型上进行大规模实证分析，使用ToxiGen和BOLD两个数据集。

Result: 观测性偏见与干预性偏见存在显著差异，标准公平性指标因未考虑上下文毒性而高估人口统计偏见；西方模型对特定群体有更高因果拒绝率，东方模型整体干预率低但对区域人口有针对性敏感。

Conclusion: 现有公平性评估方法存在混淆偏差，因果框架能更准确揭示模型安全机制中的真实偏见；人口统计敏感的过度触发会限制下游应用中良性话语的表达。
...
### [236] [Authorization Propagation in Multi-Agent AI Systems: Identity Governance as Infrastructure](https://arxiv.org/abs/2605.05440)
*Krti Tallam*

Main category: cs.AI

TL;DR: 本文提出多智能体系统中存在一个不同于提示注入的独立安全问题——授权传播（authorization propagation），即非人类主体在跨边界检索数据、委派任务和合成结果时，必须维持授权不变性。论文形式化了该问题，识别了三个子问题（传递性委派、聚合推理、时间有效性），并推导出七项架构要求。核心主张是身份治理必须作为基础设施来对待。


<details>
  <summary>Details</summary>
Motivation: 现有安全讨论过度聚焦于提示注入，而多智能体系统中的授权问题未被充分认识。经典访问控制模型（RBAC、ABAC、ReBAC）无法完全解决授权传播问题，且该问题不可简化为提示注入。

Method: 形式化定义授权传播为工作流级属性；识别三个子问题（传递性委派、聚合推理、时间有效性）；推导七项结构要求；结合现有工作（调用限界能力令牌、任务范围授权信封、依赖图策略执行、执行计数撤销）进行对比分析。

Result: 区分了授权传播与提示注入；明确了三个核心子问题；提出了七项架构要求；指出身份治理应作为基础设施组件；生产环境证据表明普通系统行为（而非仅对抗行为）已能触发所预测的授权失效。

Conclusion: 多智能体系统需要一种新的授权架构范式，其中身份治理是基础设施级组件，必须在每个交互边界持续评估和强制实施，并在编排逻辑规模化之前设计到系统中。
...
### [237] [Intentionality is a Design Decision: Measuring Functional Intentionality for Accountable AI Systems](https://arxiv.org/abs/2605.05475)
*Allessia Chiappetta,Robert Mahari*

Main category: cs.AI

TL;DR: 本文提出将意向性（intentionality）定义为一种可观察的行为特征（目的性、预见性、意志力、时间承诺和连贯性），而非意识属性，并引入功能性意向性测试（FIT）框架来量化AI系统的意向性水平，以支持分级治理和自治校准。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统展现出越来越自主、目标导向和长程规划的行为，用户缺乏标准化的方法来检测系统在多大程度上像有意图的行动者一样运作，这为治理和问责带来了困难。

Method: 将意向性重构为一种设计依赖的行为特征，提出FIT（Functional Intentionality Test）多维框架，包含五个可观察维度（目的性、预见性、意志力、时间承诺、连贯性），并设计FIT-Eval评估协议进行量化打分。

Result: FIT框架能够将意向性转化为可解释的等级，支持按比例监管和自治校准；同时指出意向能力提升会加剧问责风险。

Conclusion: 如果意向性是设计依赖的，那么原则上是可控的，但控制需要先有测量手段。FIT为此提供了可行的量化评估方案。
...
### [238] [LANTERN: LLM-Augmented Neurosymbolic Transfer with Experience-Gated Reasoning Networks](https://arxiv.org/abs/2605.05478)
*Mahyar Alinejad,Yue Wang,Amrit Singh Bedi,George Atia*

Main category: cs.AI

TL;DR: LANTERN是一个多源神经符号迁移学习框架，通过LLM生成任务自动机、语义嵌入聚合多个源策略、以及自适应教师-学生门控机制，在资源管理、导航和控制等领域实现比基线方法高40-60%的样本效率提升。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号迁移学习方法依赖手动指定的任务自动机、假设单一源任务、使用固定的知识集成机制无法适应源任务相关性变化，亟需一个更灵活、可扩展的多源迁移框架。

Method: (1) 利用大语言模型从自然语言任务描述生成确定性有限自动机；(2) 基于语义嵌入对多个源策略进行加权聚合；(3) 基于TD误差和语义不确定性的自适应教师-学生门控机制。

Result: 在资源管理、导航和控制等多个领域，LANTERN的样本效率比现有基线方法提升40-60%，且对低质量的源任务具有鲁棒性。

Conclusion: 多源自适应加权神经符号迁移方法能够提升符号强化学习设置中的可扩展性和鲁棒性。
...
### [239] [FinRAG-12B: A Production-Validated Recipe for Grounded Question Answering in Banking](https://arxiv.org/abs/2605.05482)
...
TL;DR: 提出一个统一、数据高效的框架，用于训练金融领域的接地（grounded）大语言模型，在答案质量、引用可靠性和校准拒绝方面达到优秀表现，并以更低成本和更快速度部署在40多家金融机构。


<details>
  <summary>Details</summary>
Motivation: 银行业对高准确率、合规性、可验证和接地响应的严格要求，使得通用大模型在该领域的采用面临阻力。

Method: 1) 数据生成流程：结合LLM-as-a-Judge过滤、引用标注和课程学习，仅用1.43亿token；2) 校准拒绝机制：以22%的不可回答样本训练，实现12%的“不知道”率；3) 端到端方法：从数据整理到量化服务部署。

Result: 12B模型在答案质量上超越GPT-4.1，引用接地性优于GPT-4.1；拒绝率（12%）优于基础模型（4.3%）和GPT-4.1（20.2%）；在40多家金融机构部署后，查询解决率提升7.1个百分点；响应速度3-5倍，成本降低20-50倍。

Conclusion: 该框架证明了有限数据和较小模型可以通过精心设计的数据生成与训练策略，在金融等高风险领域实现比超大模型更优的接地性和成本效益。
...
### [240] [FoodCHA: Multi-Modal LLM Agent for Fine-Grained Food Analysis](https://arxiv.org/abs/2605.05499)
*Woojin Lee,Pranav Mekkoth,Ye Tian,Onat Gungor,Tajana Rosing*

Main category: cs.AI

TL;DR: FoodCHA是一个多模态智能体框架，将食物识别重构为层次化决策过程，利用轻量级视觉语言模型Moondream-2B，在FoodNExTDB数据集上显著优于现有方法，尤其在烹饪风格识别上提升153.2%。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在粗粒度分类上表现良好，但难以捕捉烹饪风格等细粒度属性；多模态模型可能产生非规范标签，限制了实际部署。需要一种语义一致、可部署的细粒度食物识别方法。

Method: 提出FoodCHA多模态智能体框架，将食物识别重构为层次化决策过程：利用高层类别逐步锚定预测，引导子类别识别，再引导烹饪风格识别。采用轻量级Moondream-2B视觉语言模型以降低计算和内存开销。

Result: 在FoodNExTDB数据集上，FoodCHA在类别和子类别识别精度上分别优于Food-Llama-3.2-11B达13.8%和38.2%，烹饪风格分类精度提升153.2%。

Conclusion: 层次化决策结构结合轻量级视觉语言模型，能够有效提升细粒度食物识别的语义一致性和属性判别能力，同时保持实际部署的可行性。
...
### [241] [Housing Potential Common Data Model and City Digital Twin](https://arxiv.org/abs/2605.05535)
*Megan Katsumi,Mark Fox,Anderson Wong,Divnoor Chatha*

Main category: cs.AI

TL;DR: 本文提出住房潜力通用数据模型（HPCDM），用于整合多源异构数据，并通过城市数字孪生和仪表盘应用验证其实用性，同时识别了采纳障碍及应对策略。


<details>
  <summary>Details</summary>
Motivation: 现有住房潜力评估涉及多个维度的数据（如分区、土地利用、人口特征、服务可及性），但这些数据分散在不同的信息孤岛中，缺乏统一的集成与互操作标准，亟需一种通用数据模型来打破数据壁垒。

Method: 提出了住房潜力通用数据模型（HPCDM）作为数据集成与互操作的标准；创建了住房领域的城市数字孪生；开发了试点仪表盘应用来展示实际部署效果。

Result: 成功评估了HPCDM模型，并构建了城市数字孪生与仪表盘原型，验证了模型在实际场景中的可行性。

Conclusion: HPCDM能有效支撑住房潜力分析中的多源数据整合；研究还识别了关键的采纳障碍，并为城市规划者和利益相关者提供了可操作的缓解策略。
...
### [242] [AgenticRAG: Agentic Retrieval for Enterprise Knowledge Bases](https://arxiv.org/abs/2605.05538)
*Susheel Suresh,Hazel Mak,Shangpo Chou,Fred Kroon,Sahil Bhatnagar*

Main category: cs.AI

TL;DR: 提出了AgenticRAG，通过在现有企业搜索基础设施上叠加轻量级智能体框架，让推理LLM自主使用搜索、查找、打开和总结工具进行迭代检索和分析，在三个基准上大幅提升了检索准确率和事实性。


<details>
  <summary>Details</summary>
Motivation: 标准RAG流程将过多负担放在搜索堆栈上，限制了语言模型只能从检索过程早期确定的固定候选集中获取信息，导致对搜索结果的过度依赖。

Method: 在企业搜索基础设施之上构建轻量级智能体框架，为推理LLM配备搜索、查找、打开和总结等工具，使其能够自主迭代检索信息、在文档内导航并分析证据。

Result: BRIGHT上recall@1达49.6%（比最优嵌入基线高21.8个百分点），WixQA上事实性达0.96（相对提升13%），FinanceBench上答案正确性达92%（仅比真实证据的oracle访问低2个百分点）。从单次检索转向智能体工具使用带来5.9倍提升。

Conclusion: AgenticRAG适合企业生产环境，从单次检索到智能体工具使用的转变是最关键因素，多查询搜索和文档内导航同时提升了质量和效率。
...
### [243] [SPARK: Self-Play with Asymmetric Reward from Knowledge Graphs](https://arxiv.org/abs/2605.05546)
*Hyobin Park,Taeseop Kim,Dong-Geol Choi*

Main category: cs.AI

TL;DR: 提出SPARK框架，利用知识图谱的结构化信息增强科学文献中的自对弈强化学习，在跨文档多跳推理任务上优于基于非结构化语料的基线方法。


<details>
  <summary>Details</summary>
Motivation: 自对弈强化学习在数学和编程等具有明确形式化验证结构的领域表现优异，但将其扩展到科学文献领域面临挑战：文献中多模态元素间的关联很少显式表达，导致自动生成关系推理问题困难且奖励信号可靠性不足。

Method: SPARK框架从多文档科学文献中自动构建统一知识图谱（KG），利用KG中多模态节点间的路径生成关系推理问题，并基于KG中结构化事实计算可验证的奖励。单个小型视觉语言模型（sVLM）在信息不对称条件下交替扮演提议者和求解者角色。

Result: 在公开基准和自建跨文档多跳QA数据集上，SPARK一致优于基于非结构化语料的自我对弈基线，且随着跳数增加性能差距扩大。

Conclusion: 知识图谱结构化的基础有助于超越非结构化语料所能提供的多跳关系推理能力，该框架有望在未来工作中扩展至在线自适应场景。
...
### [244] [Who Prices Cognitive Labor in the Age of Agents? A Position on Compute-Anchored Wages](https://arxiv.org/abs/2605.05558)
*Siqi Zhu*

Main category: cs.AI

TL;DR: 本文提出AI智能体并非劳动力，而是将算力资本转化为认知劳动的生产技术，并推导出"算力锚定工资"（CAW）上界，认为认知劳动的定价权已从劳动力市场转移到算力资本市场。


<details>
  <summary>Details</summary>
Motivation: 纠正"AI智能体因零边际复制成本而无限供给、驱动认知劳动工资趋零"的直觉性误解，阐明其背后真实的经济机制，并说明这一修正对理论和政策均有重要意义。

Method: 基于Mankiw（2020）的标准要素定价框架，将智能体建模为将算力资本（K_c）转化为有效认知劳动（L_A）的生产技术；通过CES加总方法分离可替代任务与互补任务，推导出竞争性人力工资的上界λ·k·r_c（即算力锚定工资CAW）。

Result: 推导出认知劳动竞争性工资的上界由算力资本租金率（r_c）、算力强度（k）和相对生产率（λ）决定；在可替代任务中，人力工资不再由劳动力市场自身决定，而是由算力资本市场锚定。

Conclusion: 认知劳动的定价者不再是劳动力市场，而是算力资本市场；这一洞察改变了人们对技能偏向型技术变化方向的理解，并对要素份额分配产生重要影响。
...
### [245] [Nonsense Helps: Prompt Space Perturbation Broadens Reasoning Exploration](https://arxiv.org/abs/2605.05566)
*Langlin Huang,Chengsong Huang,Jinyuan Li,Donghong Cai,Yuyi Yang,Jiaxin Huang*

Main category: cs.AI

TL;DR: 针对GRPO在复杂任务中因采样全部失败导致“零优势问题”，提出LoPE方法：在重新采样时向提示词前添加伪拉丁文扰动序列，解锁正交推理路径，显著提升模型训练效果。


<details>
  <summary>Details</summary>
Motivation: GRPO在复杂推理任务中常因所有采样均失败而产生零优势信号，浪费训练数据和计算资源，简单增加采样数受限于静态采样策略，无法有效突破探索瓶颈。

Method: LoPE：在重新采样前，向提示词开头随机拼接由Lorem Ipsum词汇构成的伪拉丁文序列，通过任务无关的提示扰动改变模型输出分布，激发不同的推理路径。

Result: 在1.7B、4B和7B模型上进行实验，LoPE显著优于使用原始提示重新采样；其他低困惑度的拉丁随机序列同样有效。

Conclusion: LoPE是一种简单有效的基线方法，可拓宽LLM强化学习中的探索空间，缓解零优势问题。
...
### [246] [Locality-aware Private Class Identification for Domain Adaptation with Extreme Label Shift](https://arxiv.org/abs/2605.05567)
*Chuan-Xian Ren,Cheng-Jun Guo,Hong Yan*

Main category: cs.AI

TL;DR: 本文针对域自适应中标签空间包含关系的私有类识别问题，提出基于最优传输的局部感知方法ReOT，通过传输质量的得分函数区分共享类与私有类样本，并最小化分类风险同时学习聚类结构，在极端标签偏移下提供泛化上界，实验验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设私有类偏移足够大可作为异常值处理，但同一共享类内的方差可能大于私有类与共享类间的差异，导致该假设失效，私有类显著增加了跨域分类难度。

Method: 基于最优传输的局部运输和度量性质，提出局部感知的私有类识别方法（得分函数）；进而提出ReOT方法，在最小化分类风险的同时学习共享类与私有类的分离聚类结构，避免共享-私有样本对错配。

Result: 理论证明了得分函数区分共享类与私有类样本的强能力；提供了极端标签偏移下目标风险的泛化上界；在多个基准数据集上实验验证了ReOT的有效性。

Conclusion: ReOT通过可靠的最优传输机制有效解决了私有类带来的标签偏移问题，确保了知识在类内可靠传输，显著提升了域自适应性能。
...
### [247] [AlphaCrafter: A Full-Stack Multi-Agent Framework for Cross-Sectional Quantitative Trading](https://arxiv.org/abs/2605.05580)
*Yishuo Yuan,Jiayi Sheng,Sirui Zeng,Jiaqi Wang,Jiaheng Liu*

Main category: cs.AI

TL;DR: 本文提出AlphaCrafter，一个全栈式多智能体量化框架，通过持续自适应因子挖掘、市场状态筛选和风险约束交易三个专门智能体的协同工作，实现了从因子到执行的全流程闭环量化交易系统，在CSI 300和S&P 500上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有量化方法将因子发现、市场适应和交易执行视为隔离的静态组件，缺乏统一的、理性驱动的全自动化框架，难以应对金融市场的非平稳性。

Method: AlphaCrafter包含三个智能体：Miner（基于LLM持续扩展因子池）、Screener（评估市场状态构建条件性因子组合）、Trader（将因子组合转化为风险约束下的量化策略），形成闭环自适应交易系统。

Result: 在CSI 300和S&P 500上的大量实验表明，AlphaCrafter在风险调整收益上持续超越最先进基线，且交叉试验方差最低。

Conclusion: 集成且自适应的因子到执行设计能够产生稳健的交易性能，验证了全流程闭环量化框架的有效性。
...
### [248] [Belief Memory: Agent Memory Under Partial Observability](https://arxiv.org/abs/2605.05583)
*Junfeng Liao,Qizhou Wang,Jianing Zhu,Bo Du,Rui Yan,Xiuying Chen*

Main category: cs.AI

TL;DR: BeliefMem提出了一种概率记忆范式，将每个观察存储为多个候选结论及其概率，通过Noisy-OR规则更新，取代传统的单一确定性结论存储，在LoCoMo和ALFWorld基准测试中取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体记忆方法将每个观察存储为单一确定性结论，丢弃了观察本身固有的不确定性和歧义性，导致自我强化的错误累积——智能体基于该结论行动，从不重新审视其他可能性，随时间推移错误被不断强化。

Method: BeliefMem将每个观察的多个候选结论作为独立记忆条目存储，每个条目携带一个概率；利用Noisy-OR规则随新观察到来更新概率；检索时所有候选结论及其概率同时呈现，保持替代假设对智能体可见。

Result: 在LoCoMo和ALFWorld基准测试上，即使使用有限数据，BeliefMem取得了最佳平均性能，显著优于知名基线方法。

Conclusion: 概率记忆范式在部分可观察环境中为智能体记忆探索了新方向，通过保留不确定性使智能体能在证据充分的知识上高置信度行动，同时保留根据新证据更新置信度的能力。
...
### [249] [Text-Graph Synergy: A Bidirectional Verification and Completion Framework for RAG](https://arxiv.org/abs/2605.05643)
*Jiarui Zhong,Hong Cai Chen*

Main category: cs.AI

TL;DR: TGS-RAG是一种统一的文本-图协同增强框架，通过双向机制（图到文本的全局投票重排序 + 文本到图的记忆性孤立实体桥接算法）解决非对称推理导致的“信息孤岛”问题，在多跳推理基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统文本RAG检索到逻辑无关的伪证据，图RAG在搜索时剪枝可能丢弃有效推理路径，而现有混合方法仅简单拼接或单向增强，无法解决非对称推理流导致的“信息孤岛”问题。

Method: 提出TGS-RAG框架，包含两个双向通道：(1) 图到文本通道，通过全局投票策略对图节点进行重排序，筛选语义噪声；(2) 文本到图通道，采用记忆性孤立实体桥接算法，利用文本线索主动恢复搜索历史中被剪枝的有效推理路径，无需额外数据库开销。

Result: 在多个多跳推理基准上，TGS-RAG显著优于当前最先进的基线方法，在检索精度和计算效率之间取得了更优的平衡。

Conclusion: 文本与图的双向协同增强能有效解决信息孤岛问题，提升多跳推理的准确性和效率。
...
### [250] [Retrieval-Conditioned Topology Selection with Provable Budget Conservation for Multi-Agent Code Generation](https://arxiv.org/abs/2605.05657)
*Abhijit Talluri,Pujith Anne,Bhagavan Choudary Pendiyala,Raghavendra Chilukuri*

Main category: cs.AI

TL;DR: RGAO通过从分层代码索引中提取结构复杂度向量来动态选择多智能体LLM系统的编排拓扑，解决了代码生成中的路由问题，并证明了预算守恒性质。


<details>
  <summary>Details</summary>
Motivation: 现有系统选择编排拓扑时不参考代码库，导致路由问题；需要一种能根据代码结构复杂度自适应选择拓扑的架构。

Method: 提出RGAO架构，包含：(1) 基于复杂度条件的拓扑路由器；(2) 六维预算向量的预算代数与结构归纳守恒定理；(3) 分层代码检索引擎。

Result: 误路由率从30.1%降至8.2%；DAG构建亚毫秒级；树索引线性可扩展。

Conclusion: 将复杂度条件路由与形式资源代数结合，证明了检索条件动态拓扑选择下的预算守恒，为多智能体代码生成提供了可证明高效的自适应编排方案。
...
### [251] [Large Vision-Language Models Get Lost in Attention](https://arxiv.org/abs/2605.05668)
*Gongli Xi,Ye Tian,Mengyu Yang,Huahui Yi,Liang Lin,Xiaoshuai Hao,Kun Wang,Wendong Wang*

Main category: cs.AI

TL;DR: 本文提出一个统一的信息论与几何框架，揭示LVLM中Attention是子空间保持算子（重配置），FFN是子空间扩展算子（语义创新），并发现用随机噪声替换学习到的注意力权重在多数数据集上性能相当或更优，表明当前注意力机制存在严重冗余和错配。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法缺乏统一理论基础，难以清晰区分Transformer内部模块（Attention和FFN）在LVLM中的不同功能角色。

Method: 基于信息论和几何的统一框架，量化残差更新的几何与熵特性，分析Attention和FFN的算子性质。

Result: Attention是子空间保持算子（重配置），FFN是子空间扩展算子（语义创新）；用高斯噪声替换注意力权重在多数数据集上性能不降反升。

Conclusion: 当前LVLM存在严重的注意力机制冗余与错配，模型"迷失在注意力中"而非有效利用视觉上下文，亟需架构优化。
...
### [252] [Attractor Geometry of Transformer Memory: From Conflict Arbitration to Confident Hallucination](https://arxiv.org/abs/2605.05686)
*Qiyao Liang,Risto Miikkulainen,Ila Fiete*

Main category: cs.AI

TL;DR: 本文揭示了语言模型在参数记忆和工作记忆冲突或缺失时产生的两种失败模式（冲突与幻觉），并提出了一种基于隐藏状态几何特征（吸引子边界）的检测方法，该方法远优于传统的输出熵检测。


<details>
  <summary>Details</summary>
Motivation: 语言模型依赖参数记忆（PM）和工作记忆（WM）两种知识来源，但它们会产生两种不同机制的失败——冲突（PM与WM不一致）和幻觉（事实从未被学习），且模型在两种情况下都会自信输出，使得基于输出的监控方法失效。本文旨在从几何角度统一解释这两种失败机制，并寻找更可靠的检测方法。

Method: 通过合成任务（实体标识符映射到唯一编码，用LoRA安装PM）进行因果隔离分析，验证了隐藏状态空间中的吸引子盆地理论：冲突是盆地竞争，幻觉是盆地缺失。提出几何边界（隐藏状态到最近记忆盆地的距离）作为检测指标，并在自然语言事实查询上验证。

Result: 几何边界能比输出熵更干净地区分正确回忆和幻觉，零误拒率（熵检测无法避免拒绝绝大多数正确输出）；自信幻觉的比例遵循标度律 C = exp(-c/Δ̄)，随规模增长而增加；隐藏状态可靠编码认知状态，但冻结的输出头系统性地抹除它，且这种抹除随规模加剧。

Conclusion: 语言模型失败具有统一的几何解释，基于隐藏状态的几何边界检测优于基于输出的方法；模型规模扩大会使幻觉检测更困难，因为输出头对认知状态的抹除随规模加剧。
...
### [253] [DataDignity: Training Data Attribution for Large Language Models](https://arxiv.org/abs/2605.05687)
*Xiaomin Li,Andrzej Banburski-Fahey,Jaron Lanier*

Main category: cs.AI

TL;DR: 本文提出精确溯源（Pinpoint Provenance）任务，即从候选语料库中找出最支持模型回答的文档。为此构建了包含3,537篇虚构维基百科文章的FakeWiki基准，并提出ScoringModel（监督对比排序模型）和SteerFuse（无训练激活融合方法）。实验表明ScoringModel在Recall@10上比最强检索基线提升17.2个百分点（52.2 vs 35.0），在45个模型×条件组合中获胜41个。


<details>
  <summary>Details</summary>
Motivation: 审计大语言模型输出时，仅判断回答正确性不够，还需定位支持该知识的具体源文档。现有方法存在词汇捷径问题，难以区分真正的答案支持与主题或词汇相似性。

Method: 构建FakeWiki基准（3,537篇虚构维基文章），包含QA探针、保留源信息的改写、逆向生成变体、硬反事实文档（主题相似但删除关键事实）及5种查询条件（含越狱攻击转换）。提出ScoringModel（监督对比学习，InfoNCE损失，使用批内负样本、检索挖掘负样本和反事实负样本）和SteerFuse（无训练激活空间证据融合方法）。

Result: 在9个开源指令微调LLM和5种查询条件下，ScoringModel将平均Recall@10从35.0提升至52.2，在45个模型×条件组合中获胜41个。SteerFuse通常位列第二。在越狱攻击转换查询上，ScoringModel平均Recall@10提升15.7个百分点。

Conclusion: 鲁棒的训练数据溯源需要能够区分真实答案支持与主题或词汇相似性的评估设置。激活空间证据可以有效补充文本检索。
...
### [254] [GCCM: Enhancing Generative Graph Prediction via Contrastive Consistency Model](https://arxiv.org/abs/2605.05689)
*Shaozhen Ma,Wei Huang,Hanchen Wang,Dong Wen,Wenjie Zhang*

Main category: cs.AI

TL;DR: 提出GCCM方法，通过对比一致性目标和特征扰动，解决图预测中扩散模型存在的捷径解问题（模型忽略噪声目标退化为确定性预测器）。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图预测方法在推理时需要昂贵迭代去噪且采样不稳定，而一致性训练方法存在捷径解：模型忽略含噪目标退化为确定性预测器。

Method: 1）提出对比一致性目标，引入负样本对增加分离要求；2）对输入节点/边特征施加扰动，打破模型对输入图的完全条件依赖。

Result: 在多个基准数据集上，GCCM缓解了捷径解问题，相较于确定性预测器取得了一致的性能提升。

Conclusion: GCCM通过对比学习和特征扰动有效避免了图预测中一致性训练的捷径解问题，提升了模型性能。
...
### [255] [Saliency-Aware Regularized Quantization Calibration for Large Language Models](https://arxiv.org/abs/2605.05693)
*Yanlong Zhao,Xiaoyuan Cheng,Huihang Liu,Baihua He,Xinyu Zhang,Harrison Bo Hua Zhu,Wenlong Chen,Li Zeng,Zhuo Sun*

Main category: cs.AI

TL;DR: 提出了一种名为SARQC的量化校准框架，通过在标准PTQ目标中加入显著性感知正则化项，使量化权重更接近原始权重，从而提升泛化能力，在保持无额外推理开销的同时，显著改善了困惑度和零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法仅基于有限或非代表性校准数据上的经验重建误差来优化量化参数，可能导致量化权重偏离原始权重，使泛化风险发散，进而损害下游任务性能。

Method: 提出SARQC框架，在标准PTQ目标中引入显著性感知正则化项，鼓励量化权重在校准过程中保持接近原始权重，可无缝集成到现有的尺度搜索和基于Gram的PTQ方法中。

Result: 在稠密和MoE架构的大语言模型上，SARQC在困惑度和零样本准确率上均取得了一致性改进，且推理阶段不增加额外计算开销。

Conclusion: SARQC通过正则化校准目标有效控制了PTQ的泛化风险，是一种通用且高效的量化校准改进方案。
...
### [256] [Resolving the bias-precision paradox with stochastic causal representation learning for personalized medicine](https://arxiv.org/abs/2605.05706)
...
TL;DR: 提出采样最大均值差异（sMMD）方法，通过在子集层面匹配替代全局对抗平衡，解决了因果表征学习中偏差-精度悖论，在ICU数据集上实现更准确、可解释的个体化治疗效果估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法在减少混杂偏倚时往往会压制临床上有信息量的异质性，导致患者个体化预测精度下降，存在偏差-精度悖论。

Method: 提出采样最大均值差异（sMMD），一种随机对齐策略，用子集层面的匹配替代全局对抗性平衡，并构建了带有归因可解释性的反事实结果预测框架。

Result: 在大规模ICU队列（n=27,783）上，误差降低最高达11.5%，高风险任务召回率显著提升；人类-AI评估中优于实习医生和大语言模型，并帮助医生准确率提升14.7%，决策时间缩短。

Conclusion: sMMD通过选择性保留临床关键变量，实现了可解释的实时临床决策支持，有效平衡了偏差与异质性保留之间的矛盾。
...
### [257] [Conceal, Reconstruct, Jailbreak: Exploiting the Reconstruction-Concealment Tradeoff in MLLMs](https://arxiv.org/abs/2605.05709)
*Md Farhamdur Reza,Richeng Jin,Tianfu Wu,Huaiyu Dai*

Main category: cs.AI

TL;DR: 本文揭示了多模态大语言模型在意图混淆越狱攻击中存在“重构-隐藏”权衡问题，并提出了一种通过字符移除变体与关键词相关干扰图像来更好平衡该权衡的攻击方法，实验表明该方法能有效绕过安全机制。


<details>
  <summary>Details</summary>
Motivation: 现有针对多模态大语言模型的意图混淆越狱攻击未能有效平衡有害意图的隐藏性与原始请求的可恢复性，导致攻击效果受限。作者希望探究这一权衡关系，并设计更有效的攻击策略。

Method: 1) 通过重构分析揭示现有方法在权衡中的不足；2) 提出隐蔽感知变体构建方法，贪婪选择有害关键词对齐度低且多样化的字符移除变体；3) 引入关键词相关干扰图像作为辅助视觉上下文；4) 设计五种模态感知提示策略。

Result: 在闭源和开源多模态大语言模型上的实验表明，所提策略优于强基线方法，成功利用了模型自身的重构能力来恢复隐藏的有害意图。

Conclusion: 多模态大语言模型的安全机制存在一个未被充分探索的漏洞：模型自身的重构能力可被利用来恢复隐藏的有害意图并生成不安全响应。
...
### [258] [Decodable but Not Corrected by Fixed Residual-Stream Linear Steering: Evidence from Medical LLM Failure Regimes](https://arxiv.org/abs/2605.05715)
*Ming Liu*

Main category: cs.AI

TL;DR: 大语言模型在医学问答中会出现"过度思考"现象（正确但链式推理失败），该失败信号可线性解码（71.6%准确率），但五种线性干预方法均无法修正（Delta≈0），原因是失败方向与任务关键计算存在85-88%重叠（表征纠缠）。然而，同一探针可用于选择性弃权（AUROC=0.610），超越所有不确定性基线。


<details>
  <summary>Details</summary>
Motivation: 探究LLM隐藏状态中可线性解码的失败信号能否被用于修正这些失败，即分类-修正差距问题。

Method: 在医学QA中定义"过度思考"(OT)稳定行为状态；使用线性探针解码OT信号（71.6%平衡准确率）；测试五种固定线性干预策略（29种配置，n=1,273）；通过方向重叠分析、非目标干预损伤、LEACE概念擦除等方法验证表征纠缠假说；最后将探针用于选择性弃权。

Result: 所有线性干预均无法修正OT（Delta≈0），跨架构（Qwen2.5-7B）和跨领域（MMLU-STEM）结果一致。OT方向与任务关键计算重叠85-88%，非目标干预造成-12.1pp准确率下降，LEACE擦除造成-3.6pp下降，而随机擦除+0.3pp。探针-干预相关性r=-0.002。但选择性弃权AUROC=0.610，超越所有五种不确定性基线（p=0.009）。

Conclusion: 可解码的失败信号表征与任务关键计算高度纠缠，导致固定线性干预无法直接修正，但同一探针可用于后生成阶段的可信度估计（选择性弃权），为可靠估计模型输出质量提供了可行路径。
...
### [259] [Detecting Time Series Anomalies Like an Expert: A Multi-Agent LLM Framework with Specialized Analyzers](https://arxiv.org/abs/2605.05725)
*Hyeongwon Kang,Jeongseob Kim,Jinwoo Park,Pilsung Kang*

Main category: cs.AI

TL;DR: SAGE是一个多智能体框架，将单变量时间序列异常检测分解为四个专业分析器（点异常、结构异常、季节性异常、模式异常），通过证据驱动的检测器和监督器生成诊断报告，在三个基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的时间序列异常检测方法通常使用单一通用模型直接推断异常指标或区间，缺乏可控性、可解释性和可靠性，难以处理复杂异常模式。

Method: 提出SAGE框架，包含四个专业分析器（点、结构、季节性、模式异常），各使用特定数值工具和诊断可视化生成证据；证据驱动的检测器整合证据生成置信评分异常记录；监督器将结构化记录转为诊断报告。通过正常参考训练段构建合成上下文示例，无需真实异常段或异常类型标签。

Result: 在三个基准测试中，SAGE在强ML/DL和基于语言模型的基线方法中取得最佳平均性能。消融实验和人工评估表明该框架提升了检测可靠性和诊断输出的实用性。

Conclusion: 多智能体分解策略能有效提升时间序列异常检测的可控性、可解释性和可靠性，SAGE框架在实际应用中具有较好的诊断实用性。
...
### [260] [SkillRet: A Large-Scale Benchmark for Skill Retrieval in LLM Agents](https://arxiv.org/abs/2605.05726)
*Hongcheol Cho,Ryangkyung Kang,Youngeun Kim*

Main category: cs.AI

TL;DR: SkillRet是一个大规模技能检索基准，包含17,810个智能体技能和63,259个训练样本，实验表明现有检索器表现不佳，任务特定微调可显著提升性能（NDCG@10提升+13.1至+16.9点）。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体技能库规模扩大，在有限的上下文和延迟预算下选择正确技能成为关键系统挑战，但技能检索领域缺乏大规模基准和对检索行为的深入理解。

Method: 构建SkillRet基准，包含17,810个公开智能体技能，按结构化语义标签和两层分类体系（6大类和18个子类）组织，提供63,259个训练样本和4,997个评估查询。

Result: 现成检索器在大规模技能库上表现不佳；任务特定微调在SkillRet上显著提升性能，NDCG@10相比最强先前检索器提升+13.1点，相比最强现成检索器提升+16.9点。

Conclusion: SkillRet为大规模智能体系统中的技能检索建立了强基准和未来研究基础，微调模型的改进源于更好地聚焦于长噪声查询中的少量技能相关信号。
...
### [261] [Knee Osteoarthritis Severity Grading Using Optimized Deep Learning and LLM-Driven Intelligent AI on Computationally Limited Systems](https://arxiv.org/abs/2605.05731)
*Dayam Nadeem,Neha,Safdar Mustafa,Adnan Alvi,Mohd Hussain*

Main category: cs.AI

TL;DR: 提出了一种基于ResNet-18 CNN与TensorFlow Lite的膝骨关节炎严重程度自动分级方法，通过迁移学习实现K-L分级，测试准确率达94.48%，并利用LLM生成结构化解释报告，支持离线设备部署。


<details>
  <summary>Details</summary>
Motivation: 传统膝骨关节炎诊断存在主观性强、观察者间变异大的问题，缺乏精准及时的严重程度评估手段。

Method: 采用ResNet-18 CNN结合迁移学习进行五级K-L分级，模型优化后转换为TensorFlow Lite轻量格式，并引入Gemini-2.0-flash LLM生成结构化解释性结果。

Result: 测试准确率达到94.48%，模型收敛稳定，成功部署于资源受限的离线设备。

Conclusion: 该方法证明了在设备端实现可解释的AI辅助膝关节筛查工具的可行性，有助于早期诊断和提高可及性。
...
### [262] [SDFlow: Similarity-Driven Flow Matching for Time Series Generation](https://arxiv.org/abs/2605.05736)
*Wei Li,Shibo Feng,Pengcheng Wu,Min Wu,Peilin Zhao*

Main category: cs.AI

TL;DR: 本文提出SDFlow，一种基于流匹配的非自回归框架，在冻结的VQ潜空间中实现并行序列生成，解决了自回归模型在长时间序列生成中的暴露偏差问题，并取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在推理时存在暴露偏差（exposure bias），导致长序列生成中误差累积、质量严重下降。现有VQ+AR范式虽有竞争力，但受限于逐步骤预测的本质缺陷。

Method: (1) 用全局传输映射（flow matching）替代逐步骤token预测，消除暴露偏差；(2) 通过低秩流形分解+学习锚定先验降低VQ token空间的高维性；(3) 在连续传输动力学中引入离散监督，通过变分流匹配公式中的码本索引类别后验。

Result: 在多项指标上达到SOTA，Discriminative Score提升，Context-FID大幅降低，尤其在长序列生成任务上表现突出，同时推理速度显著快于自回归基线。

Conclusion: SDFlow在保持高保真度的同时实现了计算效率的提升，为时间序列生成提供了一种去自回归化的新范式。
...
### [263] [ReFlect: An Effective Harness System for Complex Long-Horizon LLM Reasoning](https://arxiv.org/abs/2605.05737)
*Fan Huang*

Main category: cs.AI

TL;DR: 提出了ReFlect框架，通过确定性包装器实现LLM推理中的错误检测与恢复，在长程多阶段任务上显著优于传统CoT方法，且模型越弱增益越大。


<details>
  <summary>Details</summary>
Motivation: 现有推理范式（CoT、ReAct、自我批判）在长程多阶段任务中错误会无声累积，缺乏有效的错误检测与恢复机制。

Method: ReFlect是一个"代驾"系统，将错误检测与恢复逻辑作为确定性包装器包裹在LLM周围，独立于模型本身的推理过程。

Result: 在6个推理领域实验中：自批判方法90%的反思块为空模板；LLM在至少76%情况下接受错误答案；ReFlect任务成功率41%-56%，增益从+7pp到+29pp；SWE-bench补丁质量从0%提升至82%-87%；增益与基线成功率呈反比（斜率-1.69, r=-0.76）。

Conclusion: ReFlect是模型无关、无需训练、纯推理时运行的框架，能有效弥补基座模型自身的推理缺陷，但结构化推理状态方法在中型模型上效果有限。
...
### [264] [HyperLens: Quantifying Cognitive Effort in LLMs with Fine-grained Confidence Trajectory](https://arxiv.org/abs/2605.05741)
*Chengda Lu,Xiaoyu Fan,Wei Xu*

Main category: cs.AI

TL;DR: 本文发现Transformer深层结构会自动放大层间置信度的微小变化，据此提出高分辨率探测工具HyperLens，可追踪置信度轨迹并量化推理过程中的认知努力。实验表明复杂任务需要更高认知努力，且标准SFT会降低认知努力并损害域内任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有分析工具分辨率有限，难以理解LLM推理过程中的动态机制。

Method: 提出HyperLens高分辨率探针，利用Transformer深层结构对层间置信度微小变化的放大机制，追踪置信度轨迹并量化认知努力。

Result: 发现复杂与简单任务的置信度轨迹存在一致性分化；标准SFT会降低认知努力，损害域内任务性能。

Conclusion: 复杂任务需要更高认知努力是LLM推理的基本原理，且SFT的副作用可通过认知努力视角进行机理解释。
...
### [265] [Best Arm Identification in Generalized Linear Bandits via Hybrid Feedback](https://arxiv.org/abs/2605.05745)
*Qirun Zeng,Xuchuang Wang,Jiayi Shen,Xutong Liu,Fang Kong,Jinhang Zuo*

Main category: cs.AI

TL;DR: 本文研究了广义线性老虎机中固定置信度最佳臂识别问题，提出了一种基于似然比的置信序列与混合Track-and-Stop算法，支持混合反馈（绝对奖励与成对比较），并扩展到成本感知场景，实验显示显著提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有最佳臂识别研究多聚焦于单一反馈模式（绝对奖励或成对比较），缺乏统一处理混合反馈的理论框架和高效算法。

Method: 提出似然比置信序列统一异构广义线性观测；构建自相容假设下的椭球置信集；设计混合Track-and-Stop算法，在臂与臂对的联合动作空间上追踪极小极大最优设计。

Result: 证明了算法的δ-正确性，给出了停时的高概率上界；实验表明样本效率显著优于基线方法。

Conclusion: 混合反馈模型能有效降低最佳臂识别的样本复杂度，且可扩展至成本感知场景，具有理论和实践价值。
...
### [266] [Confidence is the key: how conformal prediction enhances the generative design of permeable peptides](https://arxiv.org/abs/2605.05770)
*Laura van Weesep,Sunay Chankeshwara,Leonardo De Maria,Florian David,Ola Engkvist,Gökçe Geylan*

Main category: cs.AI

TL;DR: 提出了一种结合强化学习（RL）和保形预测（CP）的生成框架，用于设计具有可靠膜渗透性的环肽分子，首次将CP融入RL引导的分子生成过程，以解决预测模型在未知化学空间中的不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有RL引导的分子生成方法容易生成超出预测模型适用域（applicability domain）的分子，导致预测不可靠，尤其环肽的渗透性预测更具挑战性。因此需要一种能量化预测不确定性并引导探索的方法，避免陷入高奖励但高不确定性的化学空间。

Method: 提出RL引导的生成框架，使用不确定性感知的渗透性预测器作为评分组件；引入保形预测（Conformal Prediction, CP）作为不确定性量化方法，在用户定义的置信水平下对生成肽进行校准评分；将CP校正后的预测奖励用于指导RL优化过程。

Result: 实验表明，采用CP校正预测奖励的生成策略能显著提高环肽优化过程的可靠性和效率，有效抑制模型在预测器适用域外的过度探索。

Conclusion: 首次将保形预测与生成式建模及RL结合，弥合了预测不确定性与RL引导探索之间的差距，为设计高质量的环肽分子提供了更稳健的方案。
...
### [267] [CircuitFormer: A Circuit Language Model for Analog Topology Design from Natural Language Prompt](https://arxiv.org/abs/2605.05773)
*Md Touhidul Islam,Sujan Kumar Saha,Farimah Farahmandi,Mark Tehranipoor*

Main category: cs.AI

TL;DR: 本文针对模拟电路设计自动化难题，提出电路分词器CKT和电路专用语言模型CircuitFormer。CKT通过挖掘频繁子电路实现常数量级的词汇增长，相比BPE序列长度减少57%，压缩比提升2.3倍。基于此训练的511M参数CircuitFormer在语法正确率100%和功能成功率83%上分别超过开源LLMs 10%和14%，参数量减少240倍。


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计自动化是EDA领域的长期挑战，现有基于Transformer的LLMs在模拟硬件设计中面临两大瓶颈：模拟电路设计数据集稀缺，以及通用分词器无法有效捕捉电路的图结构信息。

Method: 构建了包含31,341对网表-自然语言描述的最大标注数据集；提出CKT电路图分词器，通过挖掘频繁子电路来编码网表连接性，实现O(1)词汇增长复杂度；训练511M参数的编-解码器Transformer模型CircuitFormer。

Result: CKT序列长度减少57%，压缩比提升2.3倍（词汇量512）；CircuitFormer语法正确率100%，功能成功率83%，超过开源LLMs分别达10%和14%，参数量减少240倍。

Conclusion: 通过专用分词器和电路专用语言模型，有效解决了模拟电路设计的自动化难题，展示了在资源效率下超越通用大模型的可能性。
...
### [268] [HEDP: A Hybrid Energy-Distance Prompt-based Framework for Domain Incremental Learning](https://arxiv.org/abs/2605.05776)
*Yu Feng,Zhen Tian,Haoran Luo,Xie Yu,Diancheng Cheng,Haoyue Zheng,Shuai Lyu,Ping Zong,Lianyuan Li,Xin Ge,Yifan Zhu*

Main category: cs.AI

TL;DR: 提出混合能量-距离提示（HEDP）框架，通过能量正则化损失和能量-距离加权机制，在域增量学习中提升模型对未见域的泛化能力，在CORe50等基准上达到2.57%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 域偏移会导致模型性能严重退化，而现有方法在持续适应新域时存在灾难性遗忘和开放世界适应性不足的问题。

Method: 受Helmholtz自由能启发，提出HEDP框架：①能量正则化损失增强域表示的可分离性；②混合能量-距离加权机制融合能量和距离线索以改进域选择与泛化。

Result: 在CORe50等多个基准上，对未见域取得2.57%的准确率提升，有效缓解了灾难性遗忘。

Conclusion: HEDP能有效提升域增量学习在开放世界中的泛化能力和稳定性，代码已开源。
...
### [269] [Sheet as Token: A Graph-Enhanced Representation for Multi-Sheet Spreadsheet Understanding](https://arxiv.org/abs/2605.05811)
*Yiming Lei,Yiqi Wang,Yujia Zhang,Bo Guan,Depei Zhu,Chunhui Wang,Zhuonan Hao,Tianyu Shi*

Main category: cs.AI

TL;DR: 提出Sheet as Token框架，将每个工作表视为统一语义单元，通过图检索器进行多表格检索，实验表明该方法能提升跨表格检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法将表格拆分为行、列或块，导致工作表被碎片化，难以保持全局的表格级语义，不利于多表格场景下的跨表信息检索。

Method: 将每个工作表编码为紧凑的稠密向量（Sheet Token），提取表名、列头、代表值、布局特征等模式感知信息；构建查询特定的候选图，使用多阶段图Transformer融合语义、查询条件、模式一致性和形状兼容性四种关系，进行列表级检索。

Result: 在构建的多表电子表格语料上，表格级分词学得稳定表示，图增强的跨表推理相比浅层图基线在列表检索上有提升，且额外图侧计算开销有限。

Conclusion: 表格级分词是一种有前景的、可扩展的多表格理解抽象方法。
...
### [270] [AGPO: Asymmetric Group Policy Optimization for Verifiable Reasoning and Search Ads Relevance at JD](https://arxiv.org/abs/2605.05826)
*Yang Xu,Kun Yao,Yiming Deng,Zheng Fang,Kai Ming Ting,Ming Pang*

Main category: cs.AI

TL;DR: 本文提出非对称群体策略优化（AGPO）方法，通过负优势主导的强化策略抑制错误推理路径，并结合群体优势机制聚焦稀有正确路径，在保持基础模型探索能力的同时提升推理准确性，在数学基准和搜索广告相关性优化任务中达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法虽提升采样效率，但导致模型推理能力边界收缩，基础模型在大样本量下反而覆盖更广。需解决强化学习不引发新推理模式且缩小能力边界的问题。

Method: 提出AGPO方法：(1) 负优势主导策略抑制错误推理路径，保持基础模型探索能力；(2) 群体优势机制根据组内方差缩放正更新，聚焦稀有正确路径，抑制平凡路径更新。

Result: 在五个数学基准上达到SOTA准确率，pass@k性能随规模持续提升；在搜索广告相关性优化的工业应用中有效提升数据标注质量，带来下游学生模型性能显著提升。

Conclusion: AGPO通过非对称强化策略有效缓解了RLVR方法导致的推理能力边界收缩问题，在学术基准和工业应用中均验证了其有效性。
...
### [271] [On the Role of Language Representations in Auto-Bidding: Findings and Implications](https://arxiv.org/abs/2605.05833)
...
TL;DR: 本文提出SemBid，一种将LLM语义编码注入离线竞价轨迹的自动竞价框架，通过Token级融合语义与数值特征提升广告投放的可控性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有自动竞价方法依赖紧凑数值状态表示，虽能隐式捕捉投放动态，但缺乏对高层意图、动态反馈和运营策略等语义信息的显式表示与控制。LLM虽有语义编码能力，但如何有效整合而不牺牲数值精度尚不明确。

Method: SemBid引入三类语义输入（Task、History、Strategy），将其作为Token与数值轨迹Token一起输入，通过自注意力机制实现语义-数值融合，实现Token级语义注入。

Result: 在多种场景和预算设置下，SemBid在整体性能、约束满足和鲁棒性方面持续优于离线强化学习和生成式序列建模的基线方法。

Conclusion: 语义信息与数值信息的精心融合能显著提升自动竞价效果，LLM语义嵌入有助于增强可控性和跨目标泛化能力。
...
### [272] [Taklif.AI: LLM-Powered Platform for Interest-Based Personalized College Assignments](https://arxiv.org/abs/2605.05842)
*Zaki Kurdya,Mohammed Zuqlam,Salem Amassi,Shady Telbany,Motaz Saad*

Main category: cs.AI

TL;DR: 本文介绍了Taklif.AI平台，利用大语言模型根据学生的课外兴趣和文化背景自动生成个性化作业，初步用户测试显示84%参与者认为个性化功能有益。


<details>
  <summary>Details</summary>
Motivation: 传统统一化作业难以满足学生多样化兴趣和认知能力，导致学生参与度下降和抄袭等不道德行为增加，亟需个性化作业生成方案。

Method: 基于LLM（Llama 3.3 70B）的Taklif.AI平台，采用结构化提示工程流水线，结合输入输出护栏，通过LiteLLM多提供商负载均衡和LangChain提示编排，部署在AWS无服务器架构上。

Result: 68名参与者（65名学生、3名教师）的初步用户验收测试显示84%认为个性化功能有益，平台获得积极评价。

Conclusion: Taklif.AI展示了AI在个性化教育作业生成中的潜力，但需要更严格的实证评估来验证学习效果，未来将进行系统的学习成果评估。
...
### [273] [XDecomposer: Learning Prior-Free Set Decomposition for Multiphase X-ray Diffraction](https://arxiv.org/abs/2605.05866)
*Hanyu Gao,Bin Cao,Yunyue Su,Tong-Yi Zhang,Qiang Liu*

Main category: cs.AI

TL;DR: 本文提出XDecomposer，一种无需先验知识的框架，可同时分解和识别多相XRD图谱中的各相成分、比例及结构表示，无需候选相列表或结构模板。


<details>
  <summary>Details</summary>
Motivation: 多相粉末X射线衍射（PXRD）分析是结构识别中的瓶颈，真实合成常产生复杂混合物，现有方法假定单相输入，在多相场景下失效。

Method: 将多相衍射分析建模为集合预测问题，采用相位查询驱动分解机制，结合衍射一致性物理重建，统一架构中推断无序的相分辨组分、混合比例及结构表示。

Result: 在模拟和实验数据集上，XDecomposer显著提升了重建精度和相识别能力，在多样化学体系中表现优异，并对未见混合物保持强泛化能力。

Conclusion: 该工作为数据驱动的多相XRD源分辨分析提供了实用路径，减少了对先验引导迭代相匹配的长期依赖。
...
### [274] [PRAISE: Prefix-Based Rollout Reuse in Agentic Search Training](https://arxiv.org/abs/2604.03675)
*Erhan Zhang,Yiqun Chen,Zechun Niu,Wei Yang,Xiaochi Wei,Yan Gao,Yi Wu,Yao Hu,Jiaxin Mao*

Main category: cs.AI

TL;DR: PRAISE通过前缀轨迹复用和中间步骤奖励机制，提升了智能搜索中强化学习的数据效率和信用分配，在多跳问答任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于搜索的强化学习方法存在两个核心问题：长序列轨迹利用率低（训练时只使用一次），且监督信号仅在最终答案处可用，导致奖励稀疏、信用分配困难。

Method: PRAISE从完整搜索轨迹中提取不同轮次的前缀状态，让模型从这些前缀中生成中间答案；利用前缀构建额外训练轨迹，并通过前缀间的性能差异推导出步骤级奖励；使用单一共享模型同时进行搜索策略学习和前缀答案评估，无需额外人工标注或独立奖励模型。

Result: 在多跳问答基准测试上，PRAISE在多项指标上持续优于强基线方法。

Conclusion: PRAISE有效解决了搜索强化学习中的数据效率和奖励稀疏问题，为智能搜索训练提供了一种无需额外标注的统一优化框架。
...
### [275] [Null Space Constrained Contrastive Visual Forgetting for MLLM Unlearning](https://arxiv.org/abs/2605.05909)
*Yuhang Wang,Zhenxing Niu,Haoxuan Ji,Guangyu He,Linlin Zhang,Haichang Gao*

Main category: cs.AI

TL;DR: 本文提出了一种针对多模态大语言模型的机器遗忘方法，通过微调视觉模块并冻结语言主干，在遗忘目标视觉知识的同时保留非目标视觉知识和所有文本知识。采用对比视觉遗忘机制分离目标与保留知识，利用零空间约束减少知识保留退化，并扩展至持续遗忘场景。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中视觉与文本知识紧密交织，现有遗忘方法难以在移除目标视觉知识的同时有效保留非目标视觉知识和全部文本知识，亟需一种能够平衡知识遗忘与保留的方法。

Method: 1) 冻结LLM主干，仅微调视觉模块；2) 提出对比视觉遗忘(CVF)机制，将目标视觉知识引导至特征空间合适区域；3) 识别保留知识的零空间，约束遗忘过程；4) 扩展至持续遗忘场景。

Result: 在多种基准上实验证明，该方法在有效遗忘与知识保留之间取得了强平衡。

Conclusion: 所提方法通过视觉模块微调和零空间约束，有效解决了MLLM中多模态知识遗忘的平衡难题，并在持续遗忘场景下仍能保持良好性能。
...
### [276] [PREFER: Personalized Review Summarization with Online Preference Learning](https://arxiv.org/abs/2605.05911)
*Millend Roy,Agostino Capponi,Vineet Goyal*

Main category: cs.AI

TL;DR: 提出一个在线学习框架，通过用户对摘要的反馈逐步学习其潜在偏好，从而生成个性化商品评论摘要。


<details>
  <summary>Details</summary>
Motivation: 现有电商评论摘要系统生成通用静态摘要，无法满足不同用户对不同产品特征的个性化需求，且用户的偏好会随交互动态变化。

Method: 构建在线学习框架，通过用户对已生成摘要的直接反馈，迭代式地学习用户的潜在偏好，并据此调整后续摘要生成。

Result: 在Amazon Reviews'23数据集上的受控模拟表明，在线偏好学习能够提升摘要与目标用户兴趣的对齐程度，同时保持摘要质量。

Conclusion: 在线偏好学习可以有效解决用户潜在偏好未知且动态变化的个性化摘要生成问题。
...
### [277] [Wisteria: A Unified Multi-Scale Feature Learning Framework for DNA Language Model](https://arxiv.org/abs/2605.05913)
*Weihua Wang,Haoji Li,Feilong Bao,Lei Yang,Guanglai Gao*

Main category: cs.AI

TL;DR: 本文提出Wisteria，一种将Mamba架构与门控膨胀卷积、门控MLP和傅里叶注意力机制结合的统一多尺度基因组语言模型，在多个短程和长程依赖基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有DNA语言模型虽能捕获长程序列依赖，但忽略了局部模体与全局依赖之间的交互作用，缺乏多尺度特征学习的统一框架。

Method: 在Mamba架构基础上，引入门控膨胀卷积捕获局部模体，门控多层感知机精炼全局依赖，并提出基于傅里叶的注意力机制支持频域建模、周期延拓和长度泛化。

Result: 在四种涵盖短程和长程依赖的实验设置下，Wisteria在多个下游基准任务上超越了竞争性的DNA语言模型基线。
...
### [278] [Which Are the Low-Resource Languages of the Semantic Web?](https://arxiv.org/abs/2605.05929)
*Ndeye-Emilie Mbengue,Pierre Monnin,Miguel Couceiro,Fabien Gandon*

Main category: cs.AI

TL;DR: 本文提出了一种基于DBpedia、BabelNet和Wikidata的多层级语言资源分类方法，旨在为LOD知识图谱中的低资源语言提供正式定义，以促进跨语言迁移，缩小数字鸿沟。


<details>
  <summary>Details</summary>
Motivation: 新兴数字技术加剧了高低资源语言在开放获取数据方面的鸿沟，许多社区被排除在全球数字化转型之外；同时，LOD知识图谱领域缺乏对低资源语言的明确定量定义。

Method: 提出一种分析LOD知识图谱中语言分布的方法论，基于DBpedia、BabelNet和Wikidata三大知识图谱，建立初步的多层级语言资源分类体系。

Result: 形成了低、中、高资源语言的多层级分类，可为后续跨语言迁移候选对象的选择提供依据。

Conclusion: 该分类体系有助于正式定义低资源语言，进而促进跨语言迁移，缓解数字鸿沟问题。
...
### [279] [In Data or Invisible: Toward a Better Digital Representation of Low-Resource Languages with Knowledge Graphs](https://arxiv.org/abs/2605.05931)
*Ndeye-Emilie Mbengue*

Main category: cs.AI

TL;DR: 该博士论文旨在通过研究多语言知识图谱中语言覆盖不均衡的问题，探索基于语言相似性和类比推理的跨语言迁移方法，以改善低资源语言在开放关联数据中的代表性。


<details>
  <summary>Details</summary>
Motivation: 数字技术加剧了高资源语言与低资源语言在开放获取数据（OAD）之间的鸿沟，许多社区被排除在全球数字化转型之外。论文旨在解决这一语言覆盖不均衡问题。

Method: 首先识别影响LOD中语言分布的关键变量（如Wikipedia文章数量、实体语言标签数量），在DBpedia、BabelNet、Wikidata三个多语言知识图谱中进行分析；进而研究跨语言迁移候选选择对多语言知识图谱补全的影响，探索基于语言邻近性和 curated 对齐标注的策略，并利用类比推理来识别跨语言对应关系。

Result: 分析揭示了多语言LOD知识图谱中语言分布现状，提出了通过语言邻近性和类比推理增强低资源语言覆盖的初步方案。

Conclusion: 通过语言邻近性驱动的跨语言迁移和类比推理，有望改善低资源语言在LOD知识图谱中的覆盖率和补全性能。
...
### [280] [ICU-Bench:Benchmarking Continual Unlearning in Multimodal Large Language Models](https://arxiv.org/abs/2605.05938)
*Yuhang Wang,Wenjie Mei,Junkai Zhang,Guangyu He,Zhenxing Niu,Haichang Gao*

Main category: cs.AI

TL;DR: 提出了ICU-Bench基准，用于评估多模态大模型在持续隐私删除场景下的机器遗忘能力，实验表明现有方法在持续设置中表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对静态或短序列设置，无法评估多模态大模型在真实部署中持续处理隐私删除请求的能力。

Method: 构建了包含1,000个隐私敏感档案（医疗报告和劳动合同）的ICU-Bench基准，含9,500张图像、16,000个问答对和100个遗忘任务，并提出了新的持续遗忘评估指标。

Result: 现有代表性遗忘方法在持续设置中普遍表现不佳，在遗忘质量、效用保持和长序列可扩展性之间存在明显局限。
...
### [281] [MAS-Algorithm: A Workflow for Solving Algorithmic Programming Problems with a Multi-Agent System](https://arxiv.org/abs/2605.05949)
*Yuliang Xu,Xiang Xu,Yao Wan,Hu Wei,Tong Jia*

Main category: cs.AI

TL;DR: 本文提出MAS-Algorithm，一个受竞赛程序员和算法工程师实践启发的多智能体工作流，用于算法问题求解。该方法将端到端求解过程分解为模块化阶段，实现结构化推理、工具集成和灵活协调。在自建基准上平均提升6.48%（接受率），远超参数高效微调的0.89%；在LiveCodeBench-Pro上提升4.72%，单个智能体贡献最高可达27.7%。


<details>
  <summary>Details</summary>
Motivation: 现有AI编码系统在处理算法问题时，主要依赖模型中心策略（如架构修改和数据扩展），成本高且可解释性有限；而外部工具或提示技术（如思维链）则缺乏统一框架。受竞赛程序员和算法工程师实践启发，作者希望构建一个系统化、可扩展的多智能体工作流来解决这些问题。

Method: 提出MAS-Algorithm，一个多智能体工作流，将算法问题求解的端到端过程分解为多个模块化阶段，支持结构化推理、工具集成以及智能体间的灵活协调。框架设计注重严谨性和可扩展性，可泛化至多种问题类型。

Result: 在自建基准上，Qwen系列模型平均接受率提升6.48%（参数高效微调仅提升0.89%）；LiveCodeBench-Pro上提升4.72%；其他准确性和效率指标也有一致改进。消融实验显示单个智能体贡献最高可达27.7%。

Conclusion: MAS-Algorithm在推动AI驱动的算法推理方面展现出巨大潜力，通过模块化多智能体协同显著优于传统模型中心方法和微调策略。
...
### [282] [HaM-World: Soft-Hamiltonian World Models with Selective Memory for Planning](https://arxiv.org/abs/2605.05951)
*Haoyun Tang,Haodong Cui,Keyao Xu,Kun Wang,Zhandong Mei*

Main category: cs.AI

TL;DR: HaM-World通过将潜在状态分解为规范(q,p)子空间和上下文c子空间，并利用Mamba选择性状态空间记忆，实现了更稳定的长时域规划，在DeepMind Control Suite任务中平均AUC提升9.5%，OOD鲁棒性增益达10.2%-13.6%。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在规划视野增长或动力学分布偏移时，想象展开变得不稳定。作者认为这种不稳定性源于规划器面向的潜在状态缺少两种结构：历史条件记忆（近似马尔可夫完备性）和几何组织（分离构型、动量与任务语义）。

Method: 提出HaM-World结构世界模型，将潜在状态分解为规范(q,p)子空间和上下文c子空间。使用Mamba选择性状态空间记忆作为历史条件输入到潜在动力学。在(q,p)子空间通过能量导出的哈密顿向量场加上可学习残差/控制动力学演化，c子空间捕捉语义、耗散和非保守因素。

Result: 在四个DeepMind Control Suite任务上达到最高平均AUC(117.9, +9.5%)；长时域展开误差降至强基线模型的45%；在11/12个MSE单元中胜出。在12种OOD扰动下，每项条件均获得最高回报，Finger Spin平均OOD回报提升10.2%，Reacher Easy提升13.6%。

Conclusion: HaM-World通过软哈密顿动力学设计，实现了有界无动作哈密顿能量漂移、策略展开下结构化能量变化以及连贯的控制诱导能量转移，支持了其设计意图。
...
### [283] [Temporal Smoothness Doubly Robust Learning for Debiased Knowledge Tracing](https://arxiv.org/abs/2605.05958)
*Peilin Zhan,Wei Chen,Weilin Chen,Shuyi Pan,Ruichu Cai*

Main category: cs.AI

TL;DR: 提出时序平滑双重鲁棒（TSDR）框架，解决知识追踪中的选择偏差问题，通过融合倾向性模型与误差插补模型保证无偏性，并引入平滑正则化降低方差，显著提升多种KT骨干模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识追踪方法忽略教育日志中的非随机选择偏差，导致掌握度估计有偏并累积误差。

Method: 提出TSDR框架：1）双重鲁棒（DR）公式融合倾向性模型与误差插补模型；2）推导泛化界刻画方差影响，引入时序平滑正则化降低方差；3）联合优化KT预测器与插补模型。

Result: 在多个真实基准数据集上，TSDR一致提升多种SOTA KT骨干模型的性能。

Conclusion: 验证了在知识追踪中进行原则性偏差校正的重要性。
...
### [284] [From Coordinate Matching to Structural Alignment: Rethinking Prototype Alignment in Heterogeneous Federated Learning](https://arxiv.org/abs/2605.05959)
*Xinghao Wu,Jianwei Niu,Guogang Zhu,Xuefeng Liu,Shaojie Tang,Jiayuan Zhang*

Main category: cs.AI

TL;DR: 提出FedSAF方法，将异构联邦学习中原型对齐从坐标对齐改为结构对齐，解决了传统坐标对齐在模型异构场景下抑制客户端学习能力的问题，在多个基准上提升达3.52%。


<details>
  <summary>Details</summary>
Motivation: 现有原型联邦学习方法在异构场景下使用坐标对齐，强制所有客户端映射到全局特征子空间，这抑制了异构特征提取器的学习能力。作者观察到坐标对齐隐式耦合了两个不同目标（对齐语义结构+强制共享特征基），后者在异构场景下有害。

Method: 提出FedSAF，将对齐目标从绝对坐标（坐标对齐）转变为类间关系结构（结构对齐），仅对齐客户端与全局原型之间的类间语义结构关系，而非强制特征坐标一致。

Result: 在多个基准数据集上，FedSAF相比现有最优原型异构联邦学习方法提升达3.52%。

Conclusion: 结构对齐在异构联邦学习中始终优于坐标对齐，验证了解耦语义结构对齐与特征基对齐的有效性。
...
### [285] [TheraAgent: Self-Improving Therapeutic Agent for Precise and Comprehensive Treatment Planning](https://arxiv.org/abs/2605.05963)
*Junkai Li,Yunghwei Lai,Tianyi Zhu,Zheng Long Lee,Weizhi Ma,Yang Liu*

Main category: cs.AI

TL;DR: 提出了TheraAgent智能体框架，通过迭代式"生成-评判-优化"流程替代大语言模型的一次性输出，实现了更精确、完整和安全的治疗方案，在HealthBench上达到SOTA，专家评估中86%胜率超过医生。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型主要依赖一次性输出，缺乏显式验证机制，导致治疗方案粗糙、不完整且存在安全隐患，无法模拟人类专家迭代修订的实际推理过程。

Method: 提出TheraAgent框架，核心是"生成-评判-优化"迭代流水线，以及TheraJudge治疗专用评估模块，在推理循环中强制执行临床标准。

Result: HealthBench上达到SOTA，Accuracy和Completeness领先；专家评估中86%胜率超过医生，Targeting和Harm Control表现优异；TheraJudge与HealthBench评估高度一致。

Conclusion: 迭代式生成-评判-优化框架能有效提升治疗方案的质量和安全性，且内置评估模块的可靠性得到验证。
...
### [286] [BehaviorGuard: Online Backdoor Defense for Deep Reinforcement Learning](https://arxiv.org/abs/2605.05977)
*Yinbo Yu,Xueyu Yin,Jiadai Wang,Chunwei Tian,Sai Xu,Qi Zhu,Daoqiang Zhang*

Main category: cs.AI

TL;DR: 本文提出BehaviorGuard，一种基于在线行为分析的深度强化学习后门防御框架，通过检测动作分布中的异常偏移来识别和抑制后门行为，无需触发器先验知识，在单智能体和多智能体场景下均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有后门防御依赖奖励异常和触发器逆向工程，对复杂触发模式鲁棒性差，且微调成本高昂，实用性受限。

Method: 发现后门策略无论是否激活触发器，都会在动作分布的高分位数和尾部区域产生一致偏移；基于此设计新的度量指标来捕捉行为漂移，在线识别并抑制后门动作。

Result: 在多种基准和不同后门攻击下，BehaviorGuard在有效性和效率上均持续优于现有方法。

Conclusion: 首个无需触发器先验的在线后门防御框架，同时适用于单智能体和多智能体深度强化学习。
...
### [287] [TACT: Mitigating Overthinking and Overacting in Coding Agents via Activation Steering](https://arxiv.org/abs/2605.05980)
*Yuan Sui,Yulin Chen,Yibo Li,Xue Jiang,Yufei He,Yihong Dong,Xiaoxin He,Tianyu Gao,Bryan Hooi*

Main category: cs.AI

TL;DR: 本文提出TACT方法，通过在残差流中检测并校准智能体的"过度思考"和"过度行动"两种漂移模式，提升长轨迹软件工程任务的解决率，最高提升5.8个百分点，步骤数减少26%。


<details>
  <summary>Details</summary>
Motivation: 语言模型智能体在处理复杂软件工程任务时，随着轨迹增长会出现性能退化（即"智能体漂移"），主要表现为过度思考（重复推理已有信息）和过度行动（未整合新观察便调用工具）两种失败模式，亟需有效的检测与缓解方法。

Method: TACT（Think-Act Calibration via activation Steering）方法：首先将轨迹步骤标注为过度思考、过度行动或校准三类，发现其隐藏状态在校准行为指向两种失败模式的两个漂移轴上可线性分离（AUC≈0.9）。测试时，将每一步的激活投影到这些轴上，将漂移的激活拉回校准区域。

Result: 在SWE-bench Verified、Terminal-Bench 2.0和CLAW-Eval三个基准上，TACT超越未校准基线：Qwen3.5-27B解决率提升5.8个百分点，Gemma-4-26B-A4B-it提升4.8个百分点，步骤数减少高达26%。

Conclusion: 智能体漂移是残差流中可引导的方向，TACT为可靠的长时程智能体提供了一种可行的校准手段。
...
### [288] [Strat-LLM: Stratified Strategy Alignment for LLM-based Stock Trading with Real-time Multi-Source Signals](https://arxiv.org/abs/2605.06024)
*Wenliang Huang,Zengyi Yu*

Main category: cs.AI

TL;DR: 提出Strat-LLM框架，在2025年实时前向交易中验证了不同规模LLM在多种对齐模式下的交易表现，发现推理型模型在自由模式最优，标准模型需严格模式约束，中等规模模型在严格约束下一致性最佳，且标准模型存在高胜率陷阱。


<details>
  <summary>Details</summary>
Motivation: 现有基准忽视了LLM作为交易智能体时架构推理与策略一致性之间的相互作用，缺乏对模型对齐策略的系统性评估。

Method: 提出Strat-LLM框架，基于分层策略对齐（Stratified Strategy Alignment），在2025年实时前向交易中集成价格序列、实时新闻和年度报告等多源异构数据，消除前瞻偏差，并在A股和美国市场进行压力测试。

Result: （1）推理型模型在自由模式下效用最高，标准模型需严格模式作为风险锚；（2）对齐效用的市场状态依赖性；（3）35B中等规模模型在严格约束下最优，122B超大模型在严格规则下受对齐税影响但在引导模式表现优异；（4）标准LLM存在高胜率陷阱，需深度推理或严格外部护栏缓解。

Conclusion: Strat-LLM框架揭示了不同规模和类型的LLM在交易任务中对齐策略的差异化表现，为构建可靠的LLM交易智能体提供了重要指导。
...
### [289] [Pathways to AGI](https://arxiv.org/abs/2605.06029)
*Gordon Fletcher,Saomai Vu Khan*

Main category: cs.AI

TL;DR: 本文从批判性软件研究视角出发，提出五个关键问题，探讨当前生成式AI发展路径、决策杠杆节点、不同模型轨迹差异、替代项目的现状，以及如何开发既能接近AGI能力又能满足透明度、可持续性等要求的社会技术方案。


<details>
  <summary>Details</summary>
Motivation: 作者认为当前AI发展状况并非必然，需要避免将其视为既定事实的假设，而是看到商业AI开发与其所处的社会、政治、经济环境之间的紧密联系。

Method: 采用批判性软件研究视角，提出五个递进式研究问题，分析生成式AI的发展路径、决策杠杆节点、不同模型轨迹（前沿专有模型、开放权重模型、特定领域模型）的差异，以及替代项目的成败原因。

Result: 通过条件性和批判性的分析，得出了关于AGI概念定义存在问题的结论，并基于此提出了最终建议。

Conclusion: AGI本身在概念和定义上存在问题；当前AI发展路径受社会政治经济条件深刻塑造；通过分析杠杆节点和替代可能性，可以制定更透明、更可持续的AGI-adjacent能力发展方案。
...
### [290] [Novelty-based Tree-of-Thought Search for LLM Reasoning and Planning](https://arxiv.org/abs/2605.06040)
*Leon Hamm,Zlatan Ajanovic*

Main category: cs.AI

TL;DR: 本文提出将新颖性度量引入思维树推理，通过评估新节点与已见节点的独特性来剪枝搜索分支，从而在保持推理质量的同时降低token开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理方法（如思维链、思维树、强化学习）虽有效但仍脆弱，且存在高时间和token成本问题，亟需更高效的推理策略。

Method: 提出一种可度量的新颖性概念，通过LLM提示利用预训练知识评估新节点与已搜索节点的独特性，据此对搜索树进行剪枝，缩减搜索范围。

Result: 该方法在基于语言的规划和通用推理基准测试上进行了验证，虽然增加了单状态的提示次数，但通过剪枝降低了整体树规模，从而减少了总token成本。

Conclusion: 将宽度搜索中的新颖性概念迁移到语言推理领域是可行的，能够在控制推理质量的同时提升效率，为LLM推理优化提供了新思路。
...
### [291] [Visual Fingerprints for LLM Generation Comparison](https://arxiv.org/abs/2605.06054)
*Amal Alnouri,Andreas Hinterreiter,Christina Humer,Furui Cheng,Marc Streit*

Main category: cs.AI

TL;DR: 本文提出一种可视化方法，通过将LLM输出建模为语言选择集合，提取内容、表达和结构等特征并展示其分布，形成"视觉指纹"，用于比较不同生成条件下模型行为的差异。
...
Method: 将LLM输出建模为语言选择集合（包括内容、表达和结构），利用自然语言处理流程提取这些选择，对重复样本进行分布表示，并通过可视化方式呈现"视觉指纹"，实现分布级别的条件倾向比较。

Result: 通过四个使用场景验证，视觉指纹能够揭示LLM行为中一致的、难以通过单个响应或聚合指标观察到的模式。

Conclusion: 该方法能够帮助研究人员直观比较不同生成条件下的LLM输出差异，揭示隐藏的行为模式，对提示设计和模型评估具有实用价值。
...
TL;DR: 本文研究了在预算和截止时间约束下，如何最大化智能体工作流成功完成的概率，并提出了一种名为MCPP的轻量级闭环规划方法。


<details>
  <summary>Details</summary>
Motivation: 实际部署中，智能体工作流往往需要在明确的预算和截止时间约束下完成，而现有工作主要关注平均效率优化，忽略了这类约束条件下的成功率最大化问题。

Method: 将问题建模为有限时域随机在线分配问题，提出Monte Carlo Portfolio Planning (MCPP)，通过模拟工作流执行来直接估计约束下的完成概率，并在观察结果后重新规划。

Result: 在CodeFlow和ProofFlow实验中，MCPP在广泛的预算-截止时间约束下，持续优于强基线方法。

Conclusion: MCPP能有效提升在预算和截止时间约束下的工作流完成概率，为实际部署中的资源分配提供了可行方案。
...
TL;DR: 本文提出了VibeServe，一种通过多智能体循环自动为不同使用场景生成定制化LLM服务系统的方法，在标准场景下与vLLM性能相当，在非标准场景下超越现有系统。


<details>
  <summary>Details</summary>
Motivation: 传统LLM服务系统采用通用型架构，需要大量人工调优来适配各种模型和工作负载。作者认为这种"运行时通用性"的设计思路并非最优，转而探索"生成时特化"的新范式。

Method: VibeServe采用双层智能体循环架构：外层循环规划并追踪系统设计方案搜索，内层循环负责实现候选方案、检查正确性并在目标基准上测量性能，从而端到端自动生成定制的LLM服务栈。

Result: 在标准部署场景中，VibeServe与高度优化的vLLM性能相当；在六种非标准场景（涉及非标准模型架构、工作负载知识和硬件特定优化）中，VibeServe通过利用通用系统忽略的机会，优于现有系统。

Conclusion: 结果表明基础架构软件设计空间的一个不同方向：生成时特化而非运行时通用性，为LLM服务系统的构建提供了新思路。
...
TL;DR: 提出一种基于核嵌入的框架，将安全认证视为轨迹数据上的分类问题，直接估计T步安全概率，避免了基于动态规划的递归方法中随时间步长累积的误差。


<details>
  <summary>Details</summary>
Motivation: 现有基于动态规划的安全认证方法在递归计算中会累积误差，导致安全概率下界随时间步长T增大而变得空洞（退化为平凡下界），无法有效保证长时域下的系统安全。

Method: 将安全认证建模为轨迹数据上的分类问题，采用核嵌入框架直接估计T步安全概率，无需递归计算；该框架统一了障碍证书、鲁棒马尔可夫模型等已有方法，并能处理非马尔可夫动力学。

Result: 直接估计器在认证时域上保持稳定，不随T增长而退化；在非马尔可夫设定下，DP-based证书会无声地变得不可靠，而所提方法仍保持有效。在神经控制四旋翼仿真中得到验证。

Conclusion: 所提核嵌入框架通过避免递归误差累积，实现了对不确定动态系统长时域安全的可靠认证，并能处理非马尔可夫动力学，突破了现有方法的局限。
...
### [295] [Rethinking Adapter Placement: A Dominant Adaptation Module Perspective](https://arxiv.org/abs/2605.06183)
*Suoxin Zhang,Run He,Di Fang,Xiang Tan,Kaixuan Chen,Huiping Zhuang*

Main category: cs.AI

TL;DR: 本文提出PAGE方法发现LoRA适配器存在一个“主导适应模块”（通常位于浅层FFN下投影层），并基于此提出DomLoRA，仅用约0.7%的可训练参数即可超越原始LoRA性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽表明使用更少的LoRA适配器可能保持甚至提升性能，但如何精准定位有限适配器的放置位置以最大化性能仍是开放问题。

Method: 提出PAGE（投影适配器梯度能量）作为基于梯度的敏感性探测方法，估计每个候选LoRA适配器的初始可训练梯度能量；发现能量高度集中在一个浅层FFN下投影模块，据此提出DomLoRA，仅在该主导模块放置单个适配器。

Result: DomLoRA仅用原始LoRA约0.7%的可训练参数，在指令跟随、数学推理、代码生成和多轮对话等多种下游任务中平均表现优于原始LoRA，并能改进其他LoRA变体。

Conclusion: 验证了主导适应模块视角作为实用放置指导原则的有效性，表明计算资源有限时仅需在关键模块微调即可达到甚至超越全量适配效果。
...
### [296] [Shallow Prefill, Deep Decoding: Efficient Long-Context Inference via Layer-Asymmetric KV Visibility](https://arxiv.org/abs/2605.06105)
*Jungsuk Oh,Hyeseo Jeon,Hyunjune Ji,Kyongmin Kong,Jay-Yoon Lee*

Main category: cs.AI

TL;DR: SPEED是一种非对称KV可见性策略，仅在低层保留非锚点提示token的KV状态，而解码阶段token保持全深度，从而在保持模型质量的同时显著降低长上下文推理成本。


<details>
  <summary>Details</summary>
Motivation: 长上下文解码器语言模型的推理成本高昂，因为长提示词在预填充阶段被处理、每层缓存，并在自回归解码阶段反复被关注。现有方法虽使上层提示KV更易存储或构建，但未从根本上减少计算开销。

Method: 提出SPEED（Shallow Prefill, dEEp Decode）策略，采用非对称KV可见性：非锚点提示token的KV状态仅保留在低层（如75%的层），解码阶段token保持全深度可见性，同时保留最小BoS锚点。

Result: 在Llama-3.1-8B指令微调实验中，SPEED（75%层用于预填充token）在OLMES基准上平均得分51.2（基线51.4），同时TTFT提升33%，TPOT提升22%，活动KV内存减少25.0%（128K上下文）。

Conclusion: 长上下文提示token无需始终作为全深度KV缓存对象存在，只要解码阶段token保持全深度，这种非对称设计就能在保留关键语义区域（提示选择和表示稳定化区域）的同时大幅降低推理成本。
...
### [297] [OPSD Compresses What RLVR Teaches: A Post-RL Compaction Stage for Reasoning Models](https://arxiv.org/abs/2605.06188)
*Jaehoon Kim,Dongha Lee*

Main category: cs.AI

TL;DR: OPSD在思维启用的数学推理中主要作为压缩机制而非修正机制，正确轨迹上训练可缩短响应且保持准确率，错误轨迹上训练会损害准确率，因此提出SFT→RLVR→OPSD的改进训练流程。


<details>
  <summary>Details</summary>
Motivation: OPSD在思维启用的数学推理中效果不佳（准确率增益缩小甚至为负），作者假设后见监督在长思维轨迹中更多识别冗余而非提供更好的替代方案，旨在探究OPSD在长思维推理中的实际作用机制。

Method: 将OPSD分别应用于正确和错误的rollout组，隔离观察压缩与修正效果；通过对比正确轨迹训练与错误轨迹训练的效果差异，分析OPSD的作用本质。

Result: 在思维启用的数学推理中，OPSD主要起压缩作用：正确轨迹训练可大幅缩短响应且不损失准确率；错误轨迹训练会显著损害准确率。

Conclusion: 提出修正后的训练流程：SFT→RLVR→OPSD（先监督微调，再通过可验证奖励进行强化学习，最后进行自蒸馏）。
...
### [298] [The Granularity Axis: A Micro-to-Macro Latent Direction for Social Roles in Language Models](https://arxiv.org/abs/2605.06196)
*Chonghan Qin,Xiachong Feng,Ziyun Song,Xiaocheng Feng,Jing Xiong,Lingpeng Kong*

Main category: cs.AI

TL;DR: 大语言模型中的社会角色粒度并非表面风格特征，而是一个结构化的、有序的、可因果操控的潜在方向。通过对比宏微观角色隐藏状态定义的"粒度轴"与角色表征空间的主轴高度对齐（余弦相似度0.972），可解释52.6%的方差，且角色投影在五个粒度级别上单调递增，具有跨层、跨提示变体等稳定性，并可通过激活操控改变响应粒度。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽知LLM可被提示扮演社会角色，但尚不清楚其内部表征是否编码了从微观个体到宏观机构/国家推理的粒度差异。作者旨在验证该粒度是否为有结构的、有序的潜在表征方向。

Method: 定义"粒度轴"为宏微观角色隐藏状态均值之差；在Qwen3-8B上验证该轴与角色表征空间主成分PC1对齐；构建75个社会角色（5个粒度层次）、收集91,200条角色条件响应；提取角色级隐藏状态投影到轴上；通过激活操控进行因果验证。

Result: 粒度轴与PC1余弦相似度0.972，解释52.6%方差；角色投影在全部五个粒度级别上单调递增；结果跨层、提示变体、端点定义、保留子集和评分过滤子集稳定；可迁移至Llama-3.1-8B-Instruct；激活操控可使Llama在五分量表上从2.00移动到3.17。

Conclusion: 社会角色粒度不是表面风格特征，而是结构化的、有序的、可因果操控的潜在方向；不同模型在可操控性上存在差异，取决于模型的默认运行机制。
...
### [299] [CrossCult-KIBench: A Benchmark for Cross-Cultural Knowledge Insertion in MLLMs](https://arxiv.org/abs/2605.06115)
*Zhen Zeng,Leijiang Gu,Feng Li,Jing Yu,Zenglin Shi*

Main category: cs.AI

TL;DR: 本文提出跨文化知识插入任务，构建了CrossCult-KIBench基准（含9800个案例，覆盖英语、中文、阿拉伯语三种语言文化群），并提出MCKI基线方法，实验表明当前方法难以在有效文化适应与行为保持间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）主要基于英语中心数据训练，在跨文化场景中常生成文化不当或失配的回复，因此需要研究如何将模型适应特定文化语境同时保持在其他文化中的原有行为。

Method: 1) 提出跨文化知识插入任务（Cross-Cultural Knowledge Insertion）；2) 构建CrossCult-KIBench评估基准（9800个图像案例，49个文化场景，支持单次插入和顺序插入设置）；3) 提出MCKI基线方法，通过冻结MLLM表示从外部记忆检索相关文化知识，匹配成功后作为条件提示插入。

Result: 实验表明现有方法难以在有效文化适应与行为保持之间取得平衡，揭示了开发文化感知MLLMs的关键挑战。
...
### [300] [Back to the Beginning of Heuristic Design: Bridging Code and Knowledge with LLMs](https://arxiv.org/abs/2605.06123)
*Nguyen Viet Tuan Kiet,Bui Dinh Pham,Dao Van Tung,Tran Cong Dao,Huynh Thi Thanh Binh*

Main category: cs.AI

TL;DR: 本文提出了一种自上而下的知识优先搜索范式来补充现有的自下而上的代码搜索方法，用于自动启发式设计。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型自动启发式设计方法主要采用自下而上的范式，即从低级代码实现逐步提炼高级原则。作者认为这种视角不完整，需要一种互补的自上而下的知识优先视角。

Method: 从统计学习视角正式化了知识优先搜索，揭示了失真-压缩权衡，并在基于种群和基于树的AHD框架中实例化了该方法。

Result: 在组合优化及其他任务中，知识优先搜索提高了发现效率、迁移性和泛化能力，通常优于以代码为中心的流水线，且两种策略结合能带来进一步提升。
...
### [301] [P-Guide: Parameter-Efficient Prior Steering for Single-Pass CFG Inference](https://arxiv.org/abs/2605.06124)
*Xin Peng,Ang Gao*

Main category: cs.AI

TL;DR: P-Guide 通过仅调制初始潜状态实现单次前向传播的高质量引导，将推理延迟降低约 50%，同时保持与标准双通道 CFG 相当的性能。


<details>
  <summary>Details</summary>
Motivation: CFG 在流匹配中需要双通道前向传播，计算开销大，本文旨在消除这一瓶颈。

Method: 提出 P-Guide 框架，仅通过调制初始潜状态实现单次推理引导；在一阶近似下等价于 CFG，并建模同方差与异方差先验，联合学习均值和方差以增强鲁棒性。

Result: 推理延迟降低约 50%，保真度和提示对齐性与标准 CFG 基线相当。

Conclusion: P-Guide 在显著降低计算开销的同时，保持了与双通道 CFG 相当的生成质量。
...
### [302] [Skill1: Unified Evolution of Skill-Augmented Agents via Reinforcement Learning](https://arxiv.org/abs/2605.06130)
*Yaorui Shi,Yuxin Chen,Zhengxi Lu,Yuchun Miao,Shugui Liu,Qi GU,Xunliang Cai,Xiang Wang,An Zhang*

Main category: cs.AI

TL;DR: 提出Skill1框架，通过单一策略协同进化技能选择、利用和蒸馏能力，仅凭任务结果信号驱动学习，在ALFWorld和WebShop上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法孤立优化或使用分离奖励源来优化技能选择、利用和蒸馏，导致部分且冲突的进化，缺乏统一的协同训练机制。

Method: Skill1框架：单一策略生成查询搜索技能库，重排候选技能并选择，基于选定技能执行任务，从轨迹中蒸馏新技能。所有学习由单一任务结果信号驱动，低频趋势用于选择，高频波动用于蒸馏。

Result: 在ALFWorld和WebShop上，Skill1优于基于技能和强化学习的基线方法。训练动态证实三种能力的协同进化，消融实验表明移除任何信用信号都会退化进化效果。

Conclusion: 单一策略协同进化技能选择和蒸馏是有效的，任务结果信号中的频率分解为两种能力提供了互补的信用分配机制。
...
### [303] [Patch-Effect Graph Kernels for LLM Interpretability](https://arxiv.org/abs/2605.06480)
*Ruben Fernandez-Boullon,David N. Olivieri*

Main category: cs.AI

TL;DR: 本文提出将激活修补图谱表示为"修补效应图"，利用图核和机器学习方法分析Transformer模型中的因果回路，并在IOI任务上验证了图特征能保留鉴别性结构信号，同时建立了严格的基线评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有机械可解释性方法通过激活修补识别因果回路，但产生的数据高维且非结构化，难以跨提示和任务族进行系统比较。

Method: 提出三种图构建方法：直接因果中介影响、偏相关和共影响；应用图核分析结构；引入筛选配对修补验证；建立提示-only和原始修补效应基线。

Result: 局部边-槽特征在分类准确率上优于全局图形描述符；CI和PC筛选的候选边对应更强的激活影响效应；图特征压缩了结构化修补信号，但原始张量和表面线索仍构成强基线。

Conclusion: 提供了一个压缩与评估流水线，用于在受控基线下比较修补衍生的结构，将鲁棒的切片鉴别性证据与更强的任务通用因果回路主张区分开。
...
### [304] [Graphlets as Building Blocks for Structural Vocabulary in Knowledge Graph Foundation Models](https://arxiv.org/abs/2605.06154)
*Kossi Amouzouvi,Robert Wardenga,Jens Lehmann,Sahar Vahdati*

Main category: cs.AI

TL;DR: 本文提出一种基于图结构标记（graphlets）的模型无关框架，解决了知识图谱基础模型因缺乏通用几何结构而难以实现零样本跨图迁移的问题。


<details>
  <summary>Details</summary>
Motivation: 知识图谱中的实体和关系虽然是离散符号，但排列是关系型的，缺乏统一的固定网格，导致现有知识图谱基础模型无法在未见过的图上迁移表示。作者希望提出一种通用的结构标记方法来解决这一局限。

Method: 引入图结构标记（graphlets，即小型连通图）作为结构标记，通过模式匹配挖掘知识图谱中关系间的结构不变性；具体使用了封闭和开放的2路径、3路径以及星形图结构标记，构建了一个模型无关的框架。

Result: 在来自多个领域的51个知识图谱上进行零样本归纳式和直推式链接预测实验，结果表明，加入简单图结构标记的模型优于之前的知识图谱基础模型。

Conclusion: 图结构标记可作为跨知识图谱的结构标记，有效提升模型在未见图谱上的迁移能力，是一种简单而有效的通用方法。
...
### [305] [SkillOS: Learning Skill Curation for Self-Evolving Agents](https://arxiv.org/abs/2605.06614)
...
TL;DR: 提出SkillOS框架，通过强化学习训练技能策展器，使LLM智能体能够从经验中提炼可复用技能，实现自我进化，在多种任务上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体作为一次性问题解决者，无法从过往交互中学习。可复用技能的提炼是实现自我进化的关键，但高质量技能策展是主要瓶颈：现有方法要么依赖人工策展、要么使用启发式规则、要么只能在短周期内训练技能操作，难以从间接且延迟的反馈中学习复杂的长期策展策略。

Method: 提出SkillOS框架：1) 冻结的智能体执行器负责检索和应用技能，2) 可训练的技能策展器负责从积累经验中更新SkillRepo。设计复合奖励并基于技能相关的任务依赖分组训练，早期轨迹更新SkillRepo，后期相关任务评估这些更新效果。

Result: 在多轮智能体任务和单轮推理任务上，SkillOS在效果和效率上均一致优于无记忆和强记忆基线方法；学到的技能策展器能跨不同执行器骨干和任务领域泛化；策展产生更精准的技能使用，SkillRepo中的技能随时间演化为更结构化的Markdown文件，编码更高级的元技能。

Conclusion: SkillOS通过经验驱动的强化学习训练，有效解决了智能体技能策展的长期学习问题，实现了智能体的自我进化，且具有良好的泛化能力。
...
### [306] [BioMedArena: An Open-source Toolkit for Building and Evaluating Biomedical Deep Research Agents](https://arxiv.org/abs/2605.06177)
*Jinge Wu,Hongjian Zhou,Mingde Zeng,Jiayuan Zhu,Junde Wu,Jiazhen Pan,Sean Wu,Honghan Wu,Fenglin Liu,David A. Clifton*

Main category: cs.AI

TL;DR: BioMedArena 是一个开源工具包，通过解耦生物医学智能体评估的六个层次，提供147个基准测试和75个生物医学工具，大幅降低了在深度研究智能体评估中集成新模型所需的重工程成本，并在8个代表性生物医学基准上实现了平均+15.03个百分点的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前构建深度研究智能体充满"胶水代码"：同一模型在不同论文中因测试框架和工具注册不同而报告不同精度，集成新模型需数周工程工作，存在严重的"单篇论文工程税"问题。

Method: 解耦生物医学智能体评估的六个层次（基准加载、工具暴露、工具选择、执行模式、上下文管理、评分），提供147个生物医学基准和75个生物医学工具（覆盖9个功能家族），通过注册几行适配器即可添加新模型，并提供6种智能体框架和6种上下文管理策略。

Result: 12个骨干模型获得了有竞争力的研究能力，在8个代表性生物医学基准上达到SOTA，平均提升+15.03个百分点。

Conclusion: BioMedArena有效解决了生物医学深度研究智能体评估中的工程税问题，为不同基础模型提供了公平比较平台，并显著提升了性能。
...
### [307] [MASPO: Joint Prompt Optimization for LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.06623)
*Zhexuan Wang,Xuebo Liu,Li Wang,Zifei Shan,Yutong Wang,Zhenxi Song,Min Zhang*

Main category: cs.AI

TL;DR: MASPO是一个自动迭代优化多智能体系统中角色提示的框架，通过联合评估机制（考虑下游任务成功）和数据驱动的进化波束搜索，在6项任务上平均提升2.9%准确率。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统中，智能体通过角色特定提示协作，但局部目标与全局系统目标存在错位，联合优化这些提示极具挑战。

Method: 提出MASPO框架：1）联合评估机制——评估提示不仅看局部有效性，更看其对下游智能体成功的能力；2）数据驱动的进化波束搜索——高效导航高维提示空间。

Result: 在6个多样化任务上，MASPO持续优于最先进的提示优化方法，平均准确率提升2.9%。

Conclusion: MASPO通过全局视角的提示优化有效弥合了局部交互与全局结果之间的差距，且无需依赖真实标签。
...
### [308] [Event-Causal RAG: A Retrieval-Augmented Generation Framework for Long Video Reasoning in Complex Scenarios](https://arxiv.org/abs/2605.06185)
*Peizheng Yan,Yu Zhao,Liang Xie,Juntong Qi,Mingming Wang,Erwei Yin*

Main category: cs.AI

TL;DR: 提出Event-Causal RAG框架，通过将视频分割为语义连贯的事件并用状态-事件-状态图表示，构建事件知识图谱，采用双向检索策略实现超长视频的因果推理，在多项基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在短/中长度视频理解上表现良好，但在超长视频推理中面临自注意力O(n²)复杂度、检索增强方法存在片段级记忆碎片化、时序因果建模弱以及存储和推理成本高等问题。

Method: 1) 将流式视频分割为语义连贯的事件；2) 每个事件用结构化State-Event-State图表示，捕获事件及其前后状态转换；3) 构建全局事件知识图谱并存储在支持语义匹配和因果拓扑检索的双存储记忆中；4) 设计双向检索策略识别最相关的事件因果链，提供给视频基础模型生成答案。

Result: 在长视频理解基准上，Event-Causal RAG在需要多事件整合和跨长时间间隔因果推理的问题上，一致优于强基线方法（片段检索方法和长上下文视频模型），同时实现了更高的记忆效率和鲁棒的流式处理性能。

Conclusion: 事件级语义分割与因果结构建模相结合是解决超长视频推理的有效途径，双向检索策略能够高效定位跨时间间隔的因果依赖关系。
...
### [309] [Can RL Teach Long-Horizon Reasoning to LLMs? Expressiveness Is Key](https://arxiv.org/abs/2605.06638)
*Tianle Wang,Zhaoyang Wang,Guangchen Lan,Xinpeng Wei,Sipeng Zhang,Guanwen Qiu,Abulhair Saparov*

Main category: cs.AI

TL;DR: 本文提出ScaleLogic框架，系统研究了强化学习训练计算量与推理任务难度（推理深度和逻辑表达力）之间的幂律关系，发现计算量随推理深度呈幂律增长，且指数随逻辑表达力单调增加，同时训练逻辑表达力对下游迁移效果有显著影响。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习已被用于提升大模型推理能力，但缺乏可控、可扩展的环境来系统研究训练如何随任务难度缩放，因此作者提出ScaleLogic框架以填补这一空白。

Method: 提出ScaleLogic框架，可独立控制推理深度（规划步数）和逻辑表达力（从简单蕴含逻辑到一阶推理）；在多种RL方法下对LLM进行训练，分析计算量与难度之间的关系。

Result: RL训练计算量T与推理深度D呈幂律关系（T∝D^γ, R²>0.99），缩放指数γ随逻辑表达力单调增加（从1.04到2.60）；更表达力的训练设置在下游数学和推理基准上性能提升更大（最高+10.66分），且更计算高效；幂律关系在多种RL方法下成立，课程训练显著提升缩放效率。

Conclusion: 模型训练的内容（逻辑表达力）而非仅训练量对下游迁移效果有决定性影响，为RL提升LLM推理的规模化训练提供了系统性指导。
...
### [310] [Systematic Evaluation of Large Language Models for Post-Discharge Clinical Action Extraction](https://arxiv.org/abs/2605.06191)
*Shivali Dalmia,Ananya Mantravadi,Prasanna Desikan*

Main category: cs.AI

TL;DR: 本文评估了大语言模型在临床行动提取中的零样本和少样本能力，提出了一种两阶段提取框架，并对比了通用LLM与监督BERT模型的效果。结果显示LLM在二分类任务上表现优异，但监督模型在多标签分类上仍有优势。错误分析表明模型失败主要源于推理与标注惯例的不匹配，建议未来构建包含推理注释的数据集。


<details>
  <summary>Details</summary>
Motivation: 临床文档（如出院记录）以叙述形式书写，其中包含大量隐含的临床行动信息，现有模型难以准确提取这些信息，尤其是在患者出院后的安全关键场景中。

Method: 提出两阶段提取框架：首先将叙述性出院记录分解为细粒度的、明确可执行的临床任务，采用分阶段提示策略；然后使用生成式LLM进行零样本和少样本评估，并与任务特定的监督BERT模型进行对比。

Result: 当代LLM在二元可行动性检测任务上达到或超过监督模型性能，但监督基线在细粒度多标签分类上仍保持明显优势。错误分析显示，许多失败源于模型推理与数据集标注惯例之间的不一致，特别是在隐含临床行动和严格结构标签规则的情况下。

Conclusion: 当前性能反映的是模型因缺乏临床推理而受到的限制，而非简单的标注问题。仅标注哪些span是可行动的，而不记录为什么可行动，使得无法区分临床推理失败与标注惯例不匹配。推进临床NLP需要构建包含推理注释的数据集。
...
### [311] [Towards Annotation-Free Validation of MLLMs: A Vision-Language Logical Consistency Metric](https://arxiv.org/abs/2605.06201)
*Ying Gu,Mei Chee Leong,Hui Li Tan,Shangbo Mao,Liyuan Li,Nancy Chen*

Main category: cs.AI

TL;DR: 提出了一种无需真实标注即可评估多模态大模型视觉-语言逻辑一致性的新框架VL-LCM，发现当前模型在准确率提升的同时逻辑一致性严重滞后。


<details>
  <summary>Details</summary>
Motivation: 传统准确率评估可能奖励大模型的无根据猜测，且在新任务中缺乏真实标注时难以适用，因此需要一种基于逻辑原理的评估方法。

Method: 基于基本逻辑原则，定义了视觉-语言逻辑一致性度量VL-LCM，涵盖充分因果和必要因果两种关系，应用于传统MC-VQA测试和NaturalBench测试，无需真实标注。

Result: 在MMMU和NaturalBench上评估了11个开源MLLMs，发现模型在逻辑一致性上显著落后于准确率提升；VL-LCM与基于真实标注的指标具有良好相关性。

Conclusion: 逻辑一致性可作为准确性的补充指标用于模型评估、选择和验证，尤其适用于缺乏真实标注的新任务。
...
### [312] [Beyond Fixed Benchmarks and Worst-Case Attacks: Dynamic Boundary Evaluation for Language Models](https://arxiv.org/abs/2605.06213)
*Haoxiang Wang,Da Yu,Huishuai Zhang*

Main category: cs.AI

TL;DR: 提出动态边界评估（DBE）方法，通过主动定位每个LLM的能力边界（即通过概率接近0.5的提示项），构建全局可比的难度标尺，解决固定基准测试中的天花板/地板效应问题。


<details>
  <summary>Details</summary>
Motivation: 现有固定基准测试对所有模型使用相同评测项，产生天花板和地板效应，掩盖了模型间真实的能力差距。

Method: 提出DBE框架，包含：校准题项库（标注难度）、技能引导边界搜索算法（SGBS，仅需API查询即可找到边界项）、自适应评估协议（将被测模型置于统一能力标尺，超出覆盖范围时动态扩展题项）。

Result: 在安全（拒绝有害请求、过度拒绝）、能力（约束指令遵循）、真实性（多轮谄媚抵抗）四类任务上实例化，覆盖更广模型谱系且无饱和问题，与现有数据集兼容。

Conclusion: DBE通过边界定位和统一难度标尺，实现了更精准、无饱和的LLM评估，解决了固定基准的局限性。
...
### [313] [Joint Consistency: A Unified Test-Time Aggregation Framework via Energy Minimization](https://arxiv.org/abs/2605.06219)
*Yunzhen Yao,Hongye Wang,Yahong Wang,Michael C. Gastpar,Bo Jiang,Lie He*

Main category: cs.AI

TL;DR: 提出联合一致性（JC）方法，将测试时聚合建模为约束伊辛型能量最小化问题，通过独立评估信号（外场）和成对比较（交互）统一了投票和加权聚合方法，在数学和代码推理任务中优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有测试时聚合方法仅考虑候选轨迹的孤立评估信号或答案频率，忽略了候选之间的比较交互信息。

Method: 将测试时聚合建模为约束伊辛型能量最小化问题，独立评估信号作为外场，成对比较作为交互；利用LLM-as-a-judge构建交互矩阵，并开发了高效的近似策略。

Result: 在数学和代码推理基准上，JC在不同任务、评判模型、轨迹预算和轨迹生成设置下均一致优于现有基线。

Conclusion: JC提供了一个统一的测试时聚合框架，将现有投票和加权聚合方法作为特例，且通过交互建模显著提升了聚合性能。
...
### [314] [Proactive Instance Navigation with Comparative Judgment for Ambiguous User Queries](https://arxiv.org/abs/2605.06223)
*Junhyuk Kwon,Seungjoon Lee,Hyejin Park,Kyle Min,Jungseul Ok*

Main category: cs.AI

TL;DR: ProCompNav是一个两阶段框架，通过构建候选池并主动询问区分性的是/否问题来减少用户负担，在同类别干扰物中精准定位目标实例，显著提升了导航成功率和对话效率。


<details>
  <summary>Details</summary>
Motivation: 现有导航智能体在面对模糊的自然语言指令时，要么过早停止于第一个可行候选、未充分探索替代方案，要么收集多个候选后仅询问单个候选的属性而非能区分整个候选池的问题，导致无法有效区分目标与干扰物，造成过早决策和冗长的用户回答。

Method: 提出ProCompNav两阶段框架：第一阶段构建候选池，第二阶段通过比较性判断识别目标。每轮提取一个属性-值对来分割当前候选池，询问二元的"是/否"问题，一次性剪枝所有不一致的候选，将消歧从开放式目标描述转变为池级别的区分性提问。

Result: 在CoIN-Bench上，ProCompNav在相同最小输入下优于交互式基线，在详细描述下优于非交互式基线，同时大幅降低回复长度。在TextNav上取得最先进的成功率。

Conclusion: 比较性判断对于实例级别导航中区分相似干扰物具有广泛适用性，能够有效提升智能体在模糊指令下的目标识别能力。
...
### [315] [A Versatile AI Agent for Rare Disease Diagnosis and Risk Gene Prioritization](https://arxiv.org/abs/2605.06226)
...
TL;DR: Hygieia是一个多模态AI智能体系统，通过整合表型特征、遗传数据和临床记录，采用路由器与知识增强框架来提升罕见病诊断的准确性和效率，在多个基准测试中达到最优性能，并能将医生诊断准确率提升12%-60%。


<details>
  <summary>Details</summary>
Motivation: 当前诊断流程存在耗时长、准确率低的问题，尤其是在罕见病领域，亟需一种整合多源数据、减少幻觉并辅助临床决策的智能诊断系统。

Method: 提出Hygieia系统，包含路由器机制和知识增强框架，可针对不同疾病类别定制诊断策略，优先考虑风险相关基因组因素，并提供置信度分数辅助决策。

Result: 在多个诊断基准上达到SOTA性能；与耶鲁大学和杜克-新加坡国立大学医学院合作验证，医生诊断准确率提升12%-60%，有效减轻临床工作负担。

Conclusion: Hygieia不仅能提升诊断准确性和可解释性，还能显著降低临床医生的工作量，有望成为临床决策支持系统的有力工具。
...
### [316] [Price of Fairness in Short-Term and Long-Term Algorithmic Selections](https://arxiv.org/abs/2605.06227)
*Shahin Jabbari,Chen Wang*

Main category: cs.AI

TL;DR: 本文研究了在序列决策问题中，短期公平约束可能加剧长期不平等，而简单的投资策略能有效降低长期差距并保持低公平代价。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注静态环境下的算法公平性，但近期发现静态公平约束可能加剧长期不平等。作者希望探究这一张力，理解短期公平与长期公平之间的权衡关系。

Method: 构建了一个序列选择问题模型，其中决策者重复选择个体，影响即时效用和人群分布。引入了短期和长期群体公平概念，通过公平代价（PoF）理论分析公平与效用的权衡，并在合成和真实数据上进行了实证验证。

Result: 短期最优公平策略的公平代价可能很大，即使群体分布几乎相同；而简单的投资策略能使长期差距消失，同时实现低公平代价。

Conclusion: 短期公平约束未必能保证长期公平，但合理的长期投资策略可以在维持高效用的同时消除群体间的不平等。
...
### [317] [Safactory: A Scalable Agent Factory for Trustworthy Autonomous Intelligence](https://arxiv.org/abs/2605.06230)
...
TL;DR: Safactory是一个用于可信自主智能体的可扩展工厂框架，集成了并行模拟、可信数据存储和自主进化三大平台，实现了从轨迹生成到异步强化学习的闭环优化。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基础设施在评估、数据管理和智能体进化方面碎片化严重，难以系统化地发现风险并形成持续改进的闭环。

Method: 提出Safactory框架，包含三个紧密耦合的平台：并行模拟平台（轨迹生成）、可信数据平台（轨迹存储与经验提取）、自主进化平台（异步强化学习与在线策略蒸馏）。

Result: 首次提出面向下一代可信自主智能体的统一进化流水线框架。

Conclusion: Safactory通过三大平台协同，有望解决智能体系统在长周期决策、工具使用和真实环境交互中的风险发现与模型持续改进问题。
...
### [318] [Data Language Models: A New Foundation Model Class for Tabular Data](https://arxiv.org/abs/2605.06290)
*Eda Erol,Giuliano Pezzoli,Ozer Cem Kelahmet*

Main category: cs.AI

TL;DR: 本文提出数据语言模型（DLM），首个原生理解表格数据的基座模型Schema-1，无需预处理即可直接处理原始表格数据，在行级预测、缺失值重建等任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有表格AI方法（如梯度提升树、表格基座模型）均需要预处理流水线，无法像语言模型理解句子那样原生理解表格数据，缺乏一个真正的表格数据基础模型。

Method: 提出DLM（数据语言模型）概念，并训练Schema-1：一个1.4亿参数模型，在超过230万个合成和真实表格数据集上训练，能够直接理解原始单元格值，无需序列化或预处理。

Result: Schema-1在行级预测基准上优于梯度提升集成、AutoML栈和现有表格基座模型；在缺失值重建上误差低于所有经典统计方法和前沿大语言模型；能仅凭原始单元格值识别任意未见数据集的行业领域。

Conclusion: DLM填补了AI栈中缺失的表格原生理解层，证明数据结构分布几何的理解比语言模型的世界知识对表格任务更有效。
...
### [319] [Addressing Labelled Data Scarcity: Taxonomy-Agnostic Annotation of PII Values in HTTP Traffic using LLMs](https://arxiv.org/abs/2605.06305)
*Thomas Cory,Axel Küpper*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型（LLM）的多阶段流水线，能够在运行时根据动态提供的分类法，对HTTP流量中明确传输的个人身份信息（PII）进行与分类法无关的自动标注与提取，并通过合成数据生成器实现可控评估。实验表明该方法在多种PII分类法下均能准确检测和提取PII。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的PII检测器依赖稀缺的手动标注流量，且与固定标签分类法紧密耦合，难以跨领域迁移和适应PII定义的演变。因此，需要一种分类法无关、可灵活适应不同PII定义的方法。

Method: 提出多阶段LLM流水线：确定性预处理 + 标签级分类 + 针对性的实例级值标注 + 输出验证。同时提出基于LLM的合成HTTP流量生成器，用于生成带有分类法派生PII标注的合成数据，支持可控评估和基于示例的提示。

Result: 在三种不同粒度和领域的PII分类法上评估，流水线能够准确检测PII类型并提取对应的具体值，验证了方法的有效性。

Conclusion: LLM可以作为灵活、分类法无关的流量标注的有前景的基础，也适用于在演进的隐私分类法下生成标注数据。
...
### [320] [More Than Can Be Said: A Benchmark and Framework for Pre-Question Scientific Ideation](https://arxiv.org/abs/2605.06345)
*Jie Yu,Song Qiu*

Main category: cs.AI

TL;DR: 提出InciteResearch框架，通过苏格拉底式追问将研究者模糊的隐性理解转化为显性、可检查、可操作的研究方案，并在TF-Bench基准上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有AI研究助手假设用户已有清晰的研究问题，但人类研究往往始于"隐性摩擦"——问题形成前的模糊不适感。本文旨在让AI在问题尚未明确时就能帮助研究者。

Method: 提出多智能体框架InciteResearch，包含三步：(1)从模糊输入中抽取五维研究者状态；(2)最大化可行性-新颖性乘积，强制7阶段因果推导痕迹以打破隐含假设；(3)检查提出方法是否为重构洞见的必然结果。同时构建TF-Bench基准。

Result: 在TF-Bench上，InciteResearch相比基于提示的基线，新颖性从3.671提升至4.250，影响力从3.806提升至4.397，将生成方案从重组式推进到架构性洞见水平。
...
### [321] [Prediction and Empowerment: A Theory of Agency through Bridge Interfaces](https://arxiv.org/abs/2605.06346)
*Richard Csaky*

Main category: cs.AI

TL;DR: 本文在确定性物理或模拟世界中研究部分可观测性下的智能体机制，建立了预测、压缩与赋权之间的分离定理，并提出了针对现代AI系统的设计原则。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测的确定性环境中，表观随机性源于初始条件、固定法则位和展开的外生噪声的不确定性，需要厘清预测、压缩与赋权之间的深层关系。

Method: 将感知与执行建模为桥接接口，分离智能体控制参数与环境控制的信道状态，通过潜在微观态先验与多对一观测粗化导出确定性POMDP框架，并进行位串特化分析。

Result: 证明了预测、压缩与赋权之间的分离：完美预测可通过识别隐藏商或覆盖控制实现，单纯高赋权不足；在可细化接口与足够记忆条件下，动作条件化的观测压缩可减少潜在商的后验不确定性。

Conclusion: 提出了AI系统的设计原则：目标应区分隐藏态识别、接口细化、任务相关可控性与单纯的覆盖或干扰控制；人机对齐本质上是一个接口设计问题。
...
### [322] [From Agent Loops to Deterministic Graphs: Execution Lineage for Reproducible AI-Native Work](https://arxiv.org/abs/2605.06365)
*Josh Rosen,Seth Rosen*

Main category: cs.AI

TL;DR: 本文提出执行谱系（execution lineage）模型，将AI工作表示为具有显式依赖关系和稳定中间边界的有向无环图（DAG），通过可重放的工件计算来替代传统循环式更新，从而保持演化中的AI生成工作的可维护性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型系统在智能体工作流中依赖隐式对话状态，导致难以隔离更新、稳定中间产物或传播变更，缺乏对演化中工作产物的可维护性保证。

Method: 提出执行谱系模型：将AI原生工作表示为有向无环图（DAG），其中每个节点是产生工件的计算，边为显式依赖关系，支持基于身份的重放（identity-based replay）和稳定的中间边界。

Result: 在无关分支更新任务中，DAG重放完美保留最终备忘录且无污染；在中间工件编辑任务中，DAG重放实现了完美的上游保留、下游传播、无关工件保留和跨工件一致性，而循环基线则频繁引入无关上下文或存在不一致。

Conclusion: 最终答案质量与维护状态质量是两回事；强循环基线在一次性合成/更新任务中可能表现良好，但会掩盖状态不一致问题；执行谱系在变更管理、稳定性保障和跨版本演化方面提供了更强的保证。
...
TL;DR: 本文论证了证据深度学习（EDL）中基于不确定性（Vacuity/UM）的OOD检测对类别数量（K）的差异高度敏感，当ID和OOD的K值不同时，AUROC和AUPR指标会被人为膨胀，最大偏差可达0.683。


<details>
  <summary>Details</summary>
Motivation: 指出EDL中常用的不确定性度量（Vacuity/UM）在评估OOD检测时，由于K与S之间不存在线性关系，导致ID与OOD的类别数不等时会引入评估伪影。

Method: 通过实验证明类别数差异对AUROC/AUPR的显著影响，并在因果语言模型（MCQA任务）中进一步分析EDL的OOD评估问题。

Result: 标准EDL中K差1时AUROC偏差达0.318，AUPR达0.613；IB-EDL中AUROC偏差0.360，AUPR偏差0.683。且无需改变模型预测即可人为膨胀指标。

Conclusion: 基于不确定性的OOD检测在EDL微调的大语言模型中极易受到类别数差异的干扰，需要更清晰的ID/OOD定义和评估规范。
...
### [324] [Knowledge Graphs, the Missing Link in Agentic AI-based Formal Verification](https://arxiv.org/abs/2605.06434)
*Vaisakh Naduvodi Viswambharan,Keerthan Kopparam Radhakrishna,Deepak Narayan Gadde,Aman Kumar*

Main category: cs.AI

TL;DR: 本文提出了一种基于知识图谱的多智能体工作流，通过从规格说明、RTL设计和形式化工具反馈中提取结构化中间表示来生成SystemVerilog断言，并利用语法修复、反例引导和覆盖率驱动的三重优化循环，在七个基准设计上实现了78.5%-99.4%的形式化覆盖率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型生成SystemVerilog断言方法存在规格说明模糊或不完整、与RTL设计脱节导致语义不匹配和语法错误频繁等问题，缺乏有效的结构化信息整合机制。

Method: 构建验证中心的知识图谱，从规格说明、RTL设计和形式化工具反馈（包括语法诊断、反例和覆盖率报告）中提取结构化中间表示；采用多智能体工作流查询和更新知识图谱以生成断言，并通过语法修复、反例引导修正和覆盖率驱动属性增强三重循环进行优化。

Result: 在七个基准设计上的评估表明，基于知识图谱的上下文检索改善了规格到RTL的接地性，持续生成可编译的断言且语法修复开销低，形式化覆盖率达到78.5%至99.4%。

Conclusion: 知识图谱方法有效提升了断言生成的可靠性和覆盖率，但收敛性存在设计依赖性，复杂时序和算术推理对当前大语言模型能力仍具挑战。
...
### [325] [SCRuB: Social Concept Reasoning under Rubric-Based Evaluation](https://arxiv.org/abs/2605.06444)
...
TL;DR: 本文提出SCRuB框架，用于评估大语言模型对社会概念的推理能力。研究发现，前沿模型在所有五个评估维度上均优于人类专家，在80.8%的成对比较中被专家评委评为第一，模型回答的总体偏好率为74.4%。研究首次证明了社会概念推理评估已趋近饱和。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理能力研究多聚焦数学或技术任务，缺乏对社会概念（塑造社会规范、文化和制度的抽象思想）推理的系统评估方法，而这对于模型作为社会智能体的应用至关重要。

Method: 提出SCRuB（Social Concept Reasoning under Rubric-Based Evaluation）框架，包含三阶段：基于权威来源构建提示、专家与模型生成回答、使用五维度批判性思维量规进行对比评估。引入学科视角专家组（Panel of Disciplinary Perspectives）进行集成验证。

Result: 发布SCRuBEval（4,711条评估提示）和SCRuBAnnotations（300条专家回答及150条专家对比判断，来自45位博士级学者）。前沿模型在所有五个量规维度上均超越人类专家。在1,170组成对比较中，模型回答在80.8%的判断中被专家评委排为第一，模型回答整体偏好率为74.4%。

Conclusion: 本研究首次提供了基于专家判断的经验证据，表明社会概念推理评估已趋近饱和——单轮考试式问答格式对于模型和人类均已达到天花板。
...
TL;DR: 提出Agentic Success Rate (ASR)指标，通过轨迹保真度评估多智能体支付系统的执行序列质量，发现传统指标无法检测的工作流捷径问题。


<details>
  <summary>Details</summary>
Motivation: 现有指标TSR和HF1仅关注最终结果或无序路由决策，无法检测支付流程中智能体执行序列的偏差，尤其在监管严格领域需要轨迹级评估。

Method: 定义ASR指标，分解为Transition Recall和Transition Precision，在18个LLM和90,000个任务实例的HMASP系统上测试。

Result: 10/18模型在支付结账时系统性地跳过确认检查点（传统指标无法检测），GPT-4.1在TSR和HF1完美下存在隐藏捷径，GPT-5.2达到完美ASR。基于ASR的诊断可提升TSR最高+93.8个百分点。

Conclusion: 轨迹级别的评估在受监管领域至关重要，ASR能发现传统指标遗漏的关键执行偏差。
...
TL;DR: ReasonSTL是一个工具增强框架，利用本地开源语言模型将自然语言翻译成信号时序逻辑公式，通过推理分解、工具调用和过程奖励训练，实现了低成本、隐私保护且性能优于商业LLM API的方案。


<details>
  <summary>Details</summary>
Motivation: 用户常用自然语言表达需求而非结构化STL公式，手动编写需要专业知识且无法规模化，而使用商业LLM API成本高且存在隐私风险，亟需一种低成本、隐私保护的自然语言到STL翻译方案。

Method: ReasonSTL将翻译过程分解为显式推理、确定性工具调用和结构化公式构建三个步骤，引入过程奖励训练来监督工具使用轨迹和最终公式，并构建了双语、计算感知的STL-Bench基准数据集。

Result: 使用ReasonSTL训练的4B模型在自动评估和人工评估中均达到最优性能。

Conclusion: ReasonSTL为形式化规格草拟提供了一种透明、低成本且保护隐私的替代方案。
...
TL;DR: 本文提出了一种基于token共现图的图核方法，用于分析稀疏自编码器特征之间的高阶共现结构，发现该方法能揭示解码器余弦相似度和token直方图基线无法捕获的互补性结构关系。


<details>
  <summary>Details</summary>
Motivation: 现有SAE特征分析几乎完全依赖top-activating token列表或解码器权重向量，忽略了特征之间的高阶共现结构。

Method: 将每个SAE特征建模为token共现图，节点为强激活附近的高频token，边为局部上下文窗口内的共现关系；采用自定义WL风格、频率分箱的图核计算特征间相似度，并进行聚类分析。

Result: 聚类恢复出标点符号模式、语言/脚本簇、代码模板等启发式主题族，这些是解码器余弦相似度聚类无法发现的；token直方图基线在整体纯度上更高，表明图视图的作用是互补而非主导。

Conclusion: 图结构表示能揭示token频率和解码器权重视图无法捕获的结构关系，且聚类结果对超参数和随机种子稳定。
...
### [329] [SpatialEpiBench: Benchmarking Spatial Information and Epidemic Priors in Forecasting](https://arxiv.org/abs/2605.06530)
*Ruiqi Lyu,Alistair Turcan,Bryan Wilder*

Main category: cs.AI

TL;DR: 提出了一个名为SpatialEpiBench的时空流行病预测基准，包含11个数据集和标准化滚动评估，发现多数模型在1天至1个月的预测中甚至不如简单的"上一次观测值"基线方法，并识别了三大失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有流行病预测缺乏标准化基准，且简单的时间序列划分方式不符合实际预测场景，阻碍了时空方法在流行病预测中的有效评估与发展。

Method: 构建SpatialEpiBench基准，包含11个流行病数据集、标准化滚动评估策略、疫情专用评估指标，并测试了基于邻接关系的时空预测模型与流行病先验知识融合方法。

Result: 多数时空预测模型在1天到1个月的预测中表现不如简单的last-value基线；识别出三大失败模式：疫情爆发预警能力差、难以处理稀疏和噪声数据、常用地理邻接关系对流行病空间信息帮助有限。

Conclusion: 当前主流时空预测方法在实际流行病预测场景中效果有限，需开发更能捕捉流行病动态特征的模型，基准已开源以推动该领域发展。
...
### [330] [Ex Ante Evaluation of AI-Induced Idea Diversity Collapse](https://arxiv.org/abs/2605.06540)
*Nafis Saami Azad,Raiyan Abdul Baten*

Main category: cs.AI

TL;DR: 本文提出一个评估AI创意系统的新框架，聚焦于AI输出在群体层面导致的多样性坍塌问题，而非仅关注个体效用。


<details>
  <summary>Details</summary>
Motivation: 现有AI创意系统评估仅关注个体效用，忽视了创意产出在群体消费时的拥挤效应——当大量相似创意出现时，单个创意的价值会下降。这导致评估盲区：AI可能提升个体输出质量，但加剧了群体层面的同质化。

Method: 构建了一个无需人机交互数据的人类相对基准框架，通过仅从模型生成和匹配的人类基线比较，估计拥挤风险。将创意建模为可拥堵资源，利用分布内比较识别来源级拥挤，定义超额拥挤系数Δ和人类相对多样性比率ρ，并建立采用博弈模型关联Δ与暴露依赖的冗余成本。

Result: 在短故事、营销标语和替代用途任务上，三个前沿LLM在各个拥挤核下均低于多样性平衡状态（ρ≥1）。估计量在可行的模型样本量下稳定。通过调整生成协议（提示工程等）可降低拥挤程度。

Conclusion: 多样性坍塌是可检测且可干预的评估目标，应成为人口感知型创意AI在开发阶段的重要考量指标，而非仅关注个体层面的输出质量。
...
### [331] [Improved techniques for fine-tuning flow models via adjoint matching: a deterministic control pipeline](https://arxiv.org/abs/2605.06583)
*Zhengyi Guo,Jiayuan Sheng,David D. Yao,Wenpin Tang*

Main category: cs.AI

TL;DR: 本文提出一种确定性的伴随匹配框架，将流式生成模型的人类偏好对齐建模为速度场上的最优控制问题，通过截断伴随方案在保持对齐质量的同时大幅降低计算成本，并在多个指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有流式生成模型的人类偏好对齐方法存在训练不稳定、计算开销大以及对齐强度与分布保持之间缺乏灵活权衡等问题。

Method: 将偏好对齐转化为速度场上的最优控制问题，直接回归控制到当前策略下的值梯度诱导目标；引入截断伴随方案，仅计算轨迹末端奖励相关信号集中的部分；推广框架至超越标准KL正则化，实现更灵活的对齐强度与分布保持权衡。

Result: 在SiT-XL/2和FLUX.2-Klein-4B上，多个对齐指标取得一致提升，同时显著改善了生成多样性和模式保真度。

Conclusion: 所提出的确定性伴随匹配框架在流式生成模型的对齐任务中，实现了高效、稳定且灵活的训练，兼顾了对齐效果与多样性保持。
...
### [332] [NeuroAgent: LLM Agents for Multimodal Neuroimaging Analysis and Research](https://arxiv.org/abs/2605.06584)
*Lujia Zhong,Yihao Xia,Jianwei Zhang,Shuo huang,Jiaxin Yue,Mingyang Xia,Yonggang Shi*

Main category: cs.AI

TL;DR: NeuroAgent是一个基于大语言模型的智能体框架，能够自动化处理多模态神经影像（sMRI、fMRI、dMRI、PET）的预处理和下游分析，在ADNI数据集（1,470名受试者）上实现84.8%的预处理正确率和0.9518的AUC，显著减少人工操作。


<details>
  <summary>Details</summary>
Motivation: 多模态神经影像分析涉及复杂的、模态特定的预处理流程，需要繁琐的配置、质量控制以及异构工具链的协调。此外，下游统计分析和疾病分类需要任务特定代码和评估协议，增加了从原始数据到可重复科学分析的障碍。

Method: 采用分层多智能体架构，包含反馈驱动的"生成-执行-验证"引擎。智能体自主生成可执行的预处理代码，检测并从运行时错误中恢复，验证输出完整性。支持通过自然语言查询进行交互式下游分析，并设计了人机回环接口处理边缘情况。

Result: 在ADNI全部阶段1,470名受试者（CN=1,000, AD=470）上评估，最强后端Qwen3.5-27B达到84.8%端到端预处理正确率，意图解析准确率最高达100%。使用自动预处理的多模态数据进行阿尔茨海默病分类，四模态集成AUC达0.9518，优于所有单模态基线。

Conclusion: NeuroAgent能够显著减少神经影像预处理所需的人工操作，实现神经影像研究的端到端自动化分析流程，为可重复性科学分析提供了有效解决方案。
...
### [333] [GlazyBench: A Benchmark for Ceramic Glaze Property Prediction and Image Generation](https://arxiv.org/abs/2605.06641)
*Ziyu Zhai,Siyou Li,Juexi Shao,Juntao Yu*

Main category: cs.AI

TL;DR: 本文提出了GlazyBench——首个面向AI辅助陶瓷釉料设计的大规模数据集，包含23,148个真实配方，支持表面属性预测和视觉生成两项任务，并建立了传统机器学习、大语言模型和深度生成模型的综合基线。


<details>
  <summary>Details</summary>
Motivation: 陶瓷釉料开发依赖昂贵且耗时的试错过程，尤其对独立艺术家造成沉重负担。尽管多模态AI提供了现代解决方案，但缺乏大规模数据集来训练这些模型。

Method: 构建了包含23,148个真实釉料配方的GlazyBench数据集，支持两大任务：从原材料预测烧制后表面属性（如颜色、透明度），以及基于属性生成釉料视觉呈现。使用传统机器学习、大语言模型、深度生成模型和大规模多模态模型建立综合基线。

Result: 实验结果表明，该方法展现出有前景但具挑战性的结果，为AI辅助材料设计提供了标准化评估基准。
...
### [334] [AI Co-Mathematician: Accelerating Mathematicians with Agentic AI](https://arxiv.org/abs/2605.06651)
...
TL;DR: 提出AI联合数学家（AI co-mathematician）——一个为数学家提供交互式AI辅助的工作平台，支持从构思到定理证明的全流程开放式数学研究。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统多聚焦于单一环节（如定理证明），缺乏对数学研究完整工作流（构思、文献搜索、计算探索、定理证明、理论构建）的全局支持，且未能充分适配数学研究迭代式、探索性的协作本质。

Method: 构建异步、有状态的工作空间，具备不确定性管理、用户意图精炼、失败假设追踪、原生数学成果输出等能力，模拟人类协作研究工作流。

Result: 在早期测试中帮助研究人员解决开放问题、发现新方向、找到被忽视的文献；在FrontierMath Tier 4上达到48%的新高分，实现硬问题求解基准上的最先进结果。

Conclusion: AI联合数学家不仅展示了AI辅助数学发现的高交互范式，也在硬问题求解基准上取得了新高度。
