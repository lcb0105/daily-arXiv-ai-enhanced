...
TL;DR: 本文研究自动归因方法，将摘要中的每个句子链接到源文本的部分内容，可能涉及一个或多个文档。探索使用混合摘要（即提取式摘要的自动改写）来简化归因过程，并采用自定义拓扑来识别不同类别归因相关错误的比例。


<details>
  <summary>Details</summary>
Motivation: 分析师需要归因，因为不知道信息来源就无法进行报告。自动归因方法对于将摘要句子与源文本内容建立链接至关重要。

Method: 使用混合摘要方法（提取式摘要的自动改写），并采用自定义拓扑结构来识别和分析归因相关错误的不同类别。

Result: 通过混合摘要方法简化了归因过程，自定义拓扑结构能够有效识别不同类别归因错误的比例分布。

Conclusion: 混合摘要方法有助于简化自动归因过程，自定义拓扑结构为归因相关错误的分类和分析提供了有效工具。
...
TL;DR: GMTRouter是一个基于异构图学习的个性化LLM路由框架，通过将多轮用户-LLM交互建模为异构图，在少量数据下有效学习用户偏好，实现个性化路由。
...
Result: 在多个数据集上，GMTRouter比强基线方法准确率提高0.9%到21.6%，AUC提高0.006到0.309，且仅需少量数据即可适应新用户和演化偏好。

Conclusion: GMTRouter通过异构图学习有效解决了LLM路由中的个性化挑战，在少量数据下实现了高效的用户偏好建模和路由决策。
...
TL;DR: 本研究评估了大型语言模型（LLMs）模拟社交媒体群组对话的能力，发现LLM生成的对话在39%的情况下被误认为是人类创建的内容，特别是Llama 3生成的对话被正确识别为AI生成的概率仅56%，略高于随机猜测。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs为模拟在线社区和社交媒体提供了新途径，可用于测试内容推荐算法设计和评估内容政策效果，但使用LLMs模拟不同用户间对话的有效性尚未得到充分验证。

Method: 从Reddit收集真实人类对话，使用Llama 3 70B和GPT-4o在相同主题上生成人工对话，然后将真实和AI生成的对话并排呈现给研究参与者进行识别。

Result: LLM生成的对话被误认为是人类内容的概率达39%，其中Llama 3生成的对话被正确识别为AI生成的概率仅为56%，接近随机水平。
...
TL;DR: 本文研究大语言模型在法律分析中的双重性：既能解读复杂的生物武器法律条款，又存在生成危险内容的安全漏洞。作者提出结合知识图谱和RAG的方法来评估LLM的法律理解能力和安全风险。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在敏感法律领域具有巨大应用潜力，但其安全漏洞可能导致生成生物武器制造指南等危险内容，这种矛盾需要系统性的评估和解决方案。

Method: 提出结合知识图谱构建和检索增强生成(RAG)的方法论，通过结构化实验评估LLM对法律条款的理解、法律意图判断能力以及安全应用潜力。

Result: 研究发现LLM在法律推理和安全机制方面存在显著局限性，但通过增强安全协议和更强大的法律推理框架可以改善。

Conclusion: 通过结合增强的安全协议和更稳健的法律推理框架，这项研究为开发能够在敏感法律领域提供道德和安全协助的LLM奠定了基础，确保它们成为法律的保护者而非无意中的违法促成者。
...
TL;DR: LLM对齐算法（如RLHF和DPO）显著降低了输出多样性，作者提出Soft Preference Learning方法来解决这个问题，通过解耦KL散度正则化中的熵和交叉熵项，实现更好的多样性和准确性平衡。


<details>
  <summary>Details</summary>
Motivation: LLM在影响社会时代表多元视角的能力至关重要，但现有对齐算法显著降低了输出多样性，导致模型过度偏向多数意见，牺牲了多样性。

Method: 提出Soft Preference Learning方法，解耦KL散度正则化中的熵和交叉熵项，从而实现对LLM生成多样性的精细控制。

Result: 使用Soft Preference Learning训练的LLM在困难重复采样任务上获得更高准确率，产生语义和词汇多样性更强的输出，能够代表更广泛的社会观点，并改善了logit校准。

Conclusion: Soft Preference Learning是对标准温度缩放的帕累托改进，能够在保持对齐效果的同时显著提升LLM输出的多样性。
...
TL;DR: 本文提出了一个测试LLMs在面对对抗性提示时事实保真度的框架，发现不同LLM对自生成谎言的抵抗力差异很大。


<details>
  <summary>Details</summary>
Motivation: 幻觉问题严重阻碍了LLMs在高风险领域的实际部署，需要测试其在对抗性提示下的鲁棒性。

Method: 三步骤框架：1) 让LLM生成特定领域内的真相和谎言；2) 让LLM验证这些断言；3) 测试LLM对自生成谎言的抵抗力。

Result: 在电影和小说两个封闭领域的测试显示：Claude抵抗力最强，GPT和Grok中等，Gemini和DeepSeek最弱。

Conclusion: 随着越来越多用户使用LLMs进行信息检索，这些发现敲响了警钟，表明需要提高LLMs的事实保真度。
...
TL;DR: 本文提出Self-HarmLLM攻击场景，利用LLM自身生成的模糊有害查询作为新输入，在相同模型的不同会话中实现越狱攻击，揭示了现有护栏设计的潜在漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有防御假设攻击者来自外部，但未充分探索模型自身输出成为攻击向量的可能性，需要研究LLM护栏的内在脆弱性。

Method: 提出Mitigated Harmful Query概念，通过模型自身生成模糊有害查询，然后在不同会话中重新输入进行越狱测试，在多种模型和条件下进行实验验证。

Result: 在Zero-shot条件下实现最高52%转换成功率和33%越狱成功率，Few-shot条件下最高65%转换成功率和41%越狱成功率，自动化评估显著高估越狱成功率。

Conclusion: 需要从根本上重新考虑护栏设计，建立更鲁棒的评估方法，自动化评估不足以准确判定有害性，Self-HarmLLM是一种有效的攻击场景。
...
TL;DR: 本研究开发了一个结合检索增强生成(RAG)和专业知识库的系统，用于自动生成儿科言语语言病理学临床案例材料，解决了传统人工创建耗时且通用大语言模型缺乏领域知识的问题。


<details>
  <summary>Details</summary>
Motivation: 临床案例是言语语言病理学教育的重要工具，但手动创建耗时费力。通用大语言模型虽然能生成文本，但缺乏领域专业知识，容易产生幻觉并需要大量专家修订。

Method: 构建了多模型RAG系统原型，整合了精选领域知识和工程化提示模板，支持五种商业和开源LLM。设计了七个涵盖不同障碍类型和年级水平的测试场景，并使用多维评估标准进行自动质量评估。

Result: 概念验证表明RAG增强生成儿科SLP案例在技术上是可行的。商业模型显示出边际质量优势，但开源替代方案达到了可接受的性能，为隐私保护的机构部署提供了可能。整合专业知识库使生成内容符合专业指南。

Conclusion: 该系统展示了生成儿科SLP案例的技术可行性，但需要专家评审、学生试点测试和心理测量评估等广泛验证才能用于教育或研究。未来应用可扩展到临床决策支持、自动化IEP目标生成和临床反思训练。
...
TL;DR: 本文扩展了DisCoCirc形式化方法，为孟加拉语开发了类似框架，并应用于英孟翻译任务。研究发现DisCoCirc在减少语言官僚主义方面存在局限性，特别是在处理两种语言结构差异时表现不佳。


<details>
  <summary>Details</summary>
Motivation: 扩展DisCoCirc形式化方法到孟加拉语，重新评估其在减少语言官僚主义方面的有效性，并探讨其在英孟翻译任务中的实际表现。

Method: 基于DisCoCirc的语法基础框架，为孟加拉语开发类似的形式化系统，应用于英孟翻译任务，并分析其处理语言结构差异的能力。

Result: 研究发现DisCoCirc虽然在语言的大部分方面表现良好，但由于两种语言的结构差异，在翻译任务中仍面临限制，即使处理相对简单的句子也存在困难。

Conclusion: DisCoCirc框架在翻译应用中存在局限性，这凸显了其在处理语言结构差异方面的约束，同时也为未来改进提供了方向。此外，研究还展示了英语连词与布尔逻辑之间的联系。
...
TL;DR: 开发了Mina，一个面向孟加拉国背景的多语言法律助手，使用LLM和RAG框架，在孟加拉国律师考试中达到75-80%的分数，匹配或超过人类平均表现。
...
Method: 采用多语言嵌入和基于RAG的工具链框架，用于检索、推理、翻译和文档生成，通过交互式聊天界面提供上下文感知的法律草案、引用和简明语言解释。

Result: 在2022和2023年孟加拉国律师委员会考试的所有阶段评估中，Mina在初步多选题、笔试和模拟口试中得分75-80%，匹配或超过人类平均表现，展示了清晰度、上下文理解和合理的法律推理。

Conclusion: Mina有潜力成为低成本、多语言的AI助手，自动化关键法律任务并扩大司法可及性，为构建领域特定、低资源系统以及解决多语言适应、效率和可持续公共服务AI部署挑战提供了现实案例研究。
...
TL;DR: 该研究构建了一个名为MEDAS的医疗紧急诊断建议系统，通过元学习器集成五个主要LLM（Gemini、Llama、Grok、GPT、Claude），将诊断准确率从单个模型的58-65%提升到70%，其中一个集成模型甚至达到85%的准确率。


<details>
  <summary>Details</summary>
Motivation: 医疗决策支持系统对急诊医生快速准确评估患者状况和做出诊断至关重要，AI特别是LLM在医疗决策支持系统中显示出潜力，但需要研究如何提高诊断准确性。

Method: 研究五个知名LLM在急诊医学真实案例中的表现，构建一个包含五个主要LLM的超级学习器MEDAS，使用元学习器学习每个LLM的不同能力，通过集体能力提升诊断准确性。

Result: 单个LLM的诊断准确率在58%-65%之间，超过人类医生报告准确率；超级学习器MEDAS将准确率提升到70%，其中一个集成模型达到85%正确诊断率。

Conclusion: 元学习方法提供的聚合诊断准确率超过任何单个LLM，表明超级学习器可以利用训练LLM群体所用医疗数据集的综合知识优势。
...
TL;DR: 提出了一种自适应的梯度感知数据选择方法GrADS，通过分析初步训练阶段的梯度来识别有效的训练数据子集，仅使用5%的选定数据就能超越全数据集微调的性能，同时显著缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在众多任务中取得了显著成就，但监督微调对于适应专业领域仍然至关重要。然而，领域专业化的SFT资源密集，且由于灾难性遗忘可能导致通用能力退化。

Method: 设计自引导标准，利用梯度的大小和统计分布来优先选择对模型学习过程贡献最大的样本，通过初步训练阶段获得的梯度分析来识别有效的训练数据子集。

Result: 在医学、法律和金融等多个领域的各种大语言模型上进行广泛实验，GrADS表现出显著的效率和成本效益。仅使用5%的选定数据，LLMs性能已超过全数据集微调，使用50%数据时性能显著提升，同时灾难性遗忘得到显著缓解。

Conclusion: GrADS方法通过智能数据选择实现了高效的领域专业化微调，在显著减少计算资源的同时提升性能并缓解灾难性遗忘问题。
...
TL;DR: 本文提出了一种结合CNN和BiGRU的混合深度学习方案，用于从社交媒体数据中准确识别自杀意念模式，并应用SHAP可解释AI方法来验证模型可靠性，在公开数据集上达到93.97%的准确率。


<details>
  <summary>Details</summary>
Motivation: 自杀是青少年第二大死因，过去自杀企图是未来自杀风险的重要预测因素。许多有自杀想法的人会在社交媒体平台上表达意图，需要有效方法来检测这些信号。
...
Conclusion: 提出的混合深度学习方案结合可解释AI方法，为社交媒体中的自杀检测提供了有效的技术解决方案，在准确性和可靠性方面优于现有方法。
...
TL;DR: 本研究开发并评估了三种NLP模型，用于从帕金森病患者的第一人称叙事中自动识别反映各种认知过程的类别。微调的Meta-Llama-3-8B-Instruct模型表现最佳，在提取七个认知类别方面取得了最高的F1分数。


<details>
  <summary>Details</summary>
Motivation: 了解帕金森病患者在日常生活中描述认知体验的方式可以为疾病相关的认知和情感变化提供宝贵见解，但从非结构化患者叙事中提取此类信息具有挑战性。

Method: 比较了三种模型家族：基于Bio_ClinicalBERT的嵌套实体识别模型、使用QLoRA微调的Meta-Llama-3-8B-Instruct指令跟随模型，以及在零样本和少样本设置下评估的GPT-4o mini。

Result: 模型性能在不同类别和模型家族间差异显著。微调的Meta-Llama-3-8B-Instruct获得了最高的整体F1分数（微平均0.74，宏平均0.59），在上下文依赖类别如思维和社交互动方面表现尤为出色。Bio_ClinicalBERT表现出高精度但低召回率。

Conclusion: 尽管由于复杂认知过程叙事描述的抽象和重叠性质，这项任务比传统信息提取更具挑战性，但通过持续改进，这些NLP系统有望实现低负担、纵向的认知功能监测，并作为帕金森病正式神经心理学评估的有价值补充。
...
TL;DR: HalluClean是一个轻量级、任务无关的框架，用于检测和纠正大语言模型生成文本中的幻觉内容，通过推理增强范式在多个任务上显著提升事实一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在各种自然语言处理任务中表现出色，但经常产生幻觉内容，影响事实可靠性，需要解决这一问题以增强模型输出的可信度。

Method: 采用推理增强范式，将过程分解为规划、执行和修订三个阶段来识别和优化无依据的声明，使用最小任务路由提示实现跨领域的零样本泛化，不依赖外部知识源或监督检测器。

Result: 在五个代表性任务（问答、对话、摘要、数学文字问题和矛盾检测）上的实验结果表明，HalluClean显著提高了事实一致性，并优于竞争基线方法。

Conclusion: HalluClean框架具有增强大语言模型输出在现实应用中的可信度的潜力。
...
TL;DR: TiDAR是一种序列级混合架构，在单个前向传播中并行进行扩散式草稿生成和自回归采样，实现了高吞吐量和AR模型级别的质量


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型并行生成优势与自回归模型质量优势之间的平衡问题，实现高吞吐量、高GPU利用率和AR级别质量的协同

Method: 使用结构化注意力掩码在单个前向传播中同时进行扩散式草稿生成（Thinking）和自回归采样（Talking）

Result: 在1.5B和8B规模上，TiDAR在吞吐量上比自回归模型快4.71-5.91倍，质量与AR模型相当，优于推测解码和扩散变体

Conclusion: TiDAR是首个在保持AR模型质量的同时显著提升吞吐量的架构，实现了并行草稿和采样的高效平衡
...
TL;DR: EVADE框架使用大语言模型生成和验证解释来检测自然语言推理任务中的标注错误，相比传统的人工两轮标注方法更高效且能提升数据集质量。


<details>
  <summary>Details</summary>
Motivation: 传统的人工标注方法（如VARIERR框架）需要两轮人工标注，成本高且可能限制合理标签和解释的覆盖范围，因此需要更高效的错误检测方法。

Method: 提出EVADE框架，使用大语言模型生成和验证解释来检测错误，通过分布比较、验证重叠和微调影响三个维度全面分析人工与LLM检测的错误。

Result: LLM验证使生成的解释分布更接近人工标注，从训练数据中移除LLM检测的错误比移除人工检测的错误能带来更好的微调性能提升。

Conclusion: LLM方法能够扩展错误检测规模，在减少人工工作量的同时提升标签变异情况下的数据集质量。
...
TL;DR: SpiralThinker是一个统一框架，通过在潜在表示上进行迭代更新来实现扩展的隐式推理，无需生成额外标记。结合渐进对齐目标和结构化注释，在数学、逻辑和常识推理任务中表现最佳。
...
TL;DR: 本文提出了一种基于轨迹的情感支持评估框架，从静态快照评估转向动态长期评估，通过模拟328个情感情境和1152个扰动事件，采用马尔可夫过程建模用户情感轨迹，并引入三个轨迹级指标来全面评估大语言模型的长期情感支持能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型情感支持评估多依赖短对话和静态评估，无法捕捉情感支持的动态和长期特性，需要开发能够评估模型改善和稳定用户情感状态能力的框架。

Method: 构建大规模基准测试，模拟真实情感变化场景；约束模型输出使用经验证的情绪调节策略；将用户情感轨迹建模为一阶马尔可夫过程；应用因果调整的情感估计进行无偏情感状态追踪；引入三个轨迹级评估指标。

Result: 通过对多种大语言模型的广泛评估，揭示了它们在情感支持能力上的显著差异，为模型开发提供了可操作的见解。

Conclusion: 该轨迹评估框架能够有效捕捉用户情感动态变化，为大语言模型的长期情感支持性能提供全面评估，显著优于传统的静态评估方法。
...
TL;DR: MM-CRITIC是一个用于评估大型多模态模型批判能力的综合性基准，涵盖基础、修正和比较三个维度，包含8种主要任务类型和500多个任务，共4471个样本。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态模型在图像描述和视觉推理等任务中能力不断增强，但多模态批判能力的研究仍然不足，这对于模型自我改进和成为可靠AI助手至关重要。
...
Conclusion: MM-CRITIC为评估多模态模型的批判能力提供了可靠的基准，揭示了当前模型的批判能力现状和关键洞察。
...
TL;DR: 本文提出了一种基于混合CTC/Attention架构的安多藏语流式语音识别框架，采用上下文感知动态分块机制，通过自适应调整分块宽度解决固定分块方法的上下文截断问题，并结合藏语词典和外部语言模型提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 解决固定分块方法在流式语音识别中的上下文截断问题，同时针对藏语的语言特点设计更合适的建模单元，提升安多藏语语音识别的准确性和实时性。

Method: 采用混合CTC/Attention架构，引入上下文感知动态分块机制自适应调整分块宽度，构建基于藏语正字法原则的词典作为建模单元，并在解码时集成外部语言模型。

Result: 在测试集上达到6.23%的词错误率，相比固定分块基线相对提升48.15%，显著降低识别延迟，性能接近全局解码。

Conclusion: 所提出的框架有效解决了流式语音识别中的上下文截断问题，显著提升了安多藏语语音识别的准确性和实时性能，为低资源语言的语音识别提供了有效解决方案。
...
TL;DR: Bi-RAR是一个新颖的检索增强推理框架，通过双向评估中间推理步骤来改善多步推理场景中的RAG性能，使用基于Kolmogorov复杂度的双向信息距离和强化学习优化。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成方法在复杂多步推理场景中效果有限，大多数方法依赖结果监督，缺乏对中间步骤的显式指导，导致奖励黑客攻击和响应质量下降。

Method: 提出Bi-RAR框架，双向评估每个中间步骤，引入基于Kolmogorov复杂度的双向信息距离来量化信息完整性，采用多目标强化学习框架和级联奖励结构优化推理过程。

Result: 在七个问答基准测试上的实证结果表明，Bi-RAR超越了先前的方法，并在训练和推理过程中实现了与搜索引擎的高效交互和推理。

Conclusion: Bi-RAR通过双向评估和强化学习优化，显著提升了检索增强生成在多步推理场景中的性能，为复杂推理任务提供了有效的解决方案。
...
### [24] [Assessing the Capabilities of LLMs in Humor:A Multi-dimensional Analysis of Oogiri Generation and Evaluation](https://arxiv.org/abs/2511.09133)
*Ritsu Sakabe,Hwichan Kim,Tosho Hirasawa,Mamoru Komachi*

Main category: cs.CL

TL;DR: 本文通过日本即兴喜剧游戏Oogiri，对大型语言模型的幽默能力进行多维度评估，发现LLMs在生成幽默内容时缺乏共情能力，且评估标准与人类存在根本差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究对大型语言模型的幽默能力评估多为单维度，缺乏对幽默多面性的理解，需要通过更全面的评估方法来填补这一空白。

Method: 扩展Oogiri数据集，使用LLMs生成回应，并手动标注六个维度的5分制评分：新颖性、清晰度、相关性、智力、共情和总体趣味性。

Result: LLMs生成的回应水平介于人类低中层次之间，但显著缺乏共情能力；模型评估与人类评估标准存在根本差异：LLMs优先考虑新颖性，而人类优先考虑共情。
...
TL;DR: 本文提出了一种个性化英语阅读理解测试生成方法，通过内容转译技术将原始阅读材料转化为符合学生兴趣的个性化材料，实验证明该方法能提高EFL学习者的阅读理解能力和学习动机。
...
Method: 开发了基于OpenAI gpt-4o的结构化内容转译流程，从RACE-C数据集出发，生成与原始文本语言特征相似但语义上符合学生兴趣的新篇章和多项选择题。方法整合了主题提取、基于布鲁姆分类法的问题分类、语言特征分析和内容转译。
...
TL;DR: DoPE是一种无需训练的基于截断矩阵熵的方法，通过检测特征图中的异常频率带并重新参数化，有效缓解注意力下沉现象，在长上下文任务中显著提升检索准确性和推理稳定性。


<details>
  <summary>Details</summary>
Motivation: Transformer模型中的旋转位置编码存在长度外推限制，导致注意力下沉现象，影响模型在长上下文中的表现。

Method: 将带位置编码的注意力图重新解释为噪声特征图，使用截断矩阵熵检测异常频率带，并通过无参数高斯分布重新参数化特征图以实现稳健外推。

Result: 在needle-in-a-haystack和多样本上下文学习任务中，DoPE在长达64K令牌的扩展上下文中显著提高了检索准确性和推理稳定性。

Conclusion: 位置编码的降噪策略有效缓解了注意力下沉，恢复了平衡的注意力模式，为改进长度泛化提供了简单而强大的解决方案。
...
TL;DR: LoopTool是一个完全自动化的模型感知数据演化框架，通过紧密集成数据合成和模型训练来解决工具学习中的静态数据管道问题。它通过三个协同模块迭代优化数据和模型：能力诊断、标签验证和错误驱动数据扩展，在开源生态系统中显著提升了LLM的工具使用能力。
...
Method: LoopTool框架包含三个协同模块：(1)贪婪能力探测(GCP)诊断模型已掌握和失败的能力；(2)判断引导标签验证(JGLV)使用开源判断模型查找并纠正标注错误，逐步净化数据集；(3)错误驱动数据扩展(EDDE)基于识别出的失败生成新的挑战性样本。

Result: 实验表明，使用LoopTool训练的8B模型显著超越了其32B数据生成器，并在BFCL-v3和ACEBench基准测试中达到了其规模的最新最先进结果。

Conclusion: 闭环自精化数据管道可以显著增强LLM的工具使用能力，LoopTool在开源生态系统中实现了这一目标，消除了对昂贵闭源API的依赖。
...
TL;DR: 本文提出了一种无需手动识别的表格问答细胞提取方法，通过混合检索机制计算问题与单元格的相似度来估计表头，并在最相关行和列的交点选择答案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在表格问答中直接输入整个表格作为长文本往往导致错误答案，因为大多数LLM无法固有地捕获复杂的表格结构。
...
Conclusion: 虽然本研究使用传统编码器模型进行检索，但计划整合更高效的文本搜索模型以改进性能并缩小与人工评估结果的差距。
...
TL;DR: 本文实证验证了基于上下文的sequentiality度量比原始版本更符合人类对文章组织性和连贯性的评估，在自动作文评分中具有更好的预测价值。


<details>
  <summary>Details</summary>
Motivation: 针对先前研究中sequentiality度量存在的问题（主题选择混淆、缺乏真实有效性验证），验证仅使用上下文术语作为更概念有效和可解释的替代方案。

Method: 使用两个带有人工标注特征分数的论文数据集（ASAP++和ELLIPSE），比较不同版本的sequentiality度量与人类评估的一致性，并分析其与标准语言特征结合时的预测性能。

Result: 基于上下文的sequentiality版本与人类对组织和连贯性特征的评估更一致；虽然零样本提示的LLM单独预测更准确，但上下文度量与标准语言特征结合时比原始版本和仅主题版本增加更多预测价值，且优于零样本LLM预测。
...
<details>
  <summary>Details</summary>
Motivation: 传统子词分割通常在预处理阶段固定不变，本文探索在训练过程中动态优化子词分割的可能性，研究子词在预训练和微调过程中的演化规律。

Method: 扩展子词分段语言模型框架以支持预训练和微调，针对三种形态类型不同的语言进行训练：聚合型的isi-Xhosa、分离型的Setswana和中间型的英语，从语言学角度分析子词动态变化。

Result: 识别出子词学习的四个阶段，形态复杂的isi-Xhosa表现出更大的不稳定性；微调过程中子词边界变得更细粒度；可学习子词为低资源、形态复杂语言提供了改进文本生成和跨语言迁移的途径。

Conclusion: 可学习子词分割在训练过程中会动态演化，特别是在形态复杂的语言中表现出更显著的变化，这为处理低资源、形态复杂语言提供了有前景的方法。
...
TL;DR: 该论文报告了在六种不同规模（5100万到4.75亿参数）上预训练ModernBERT编码器模型，重点关注与芬兰相关的有限多语言能力。这些模型在性能上与现有多语言模型竞争或更优，在需要超过512个标记的长上下文任务中优于单语言模型。


<details>
  <summary>Details</summary>
Motivation: 开发针对芬兰相关语言的有限多语言预训练模型，填补现有模型在特定语言支持方面的空白，并探索在长上下文任务中的性能优势。

Method: 使用六种不同参数规模的ModernBERT编码器架构进行预训练，采用有限多语言策略，特别关注与芬兰相关的语言，并在训练最后阶段使用不同数据进行实验。

Result: 提出的模型在性能上与现有多语言模型竞争或更优，在长上下文（超过512个标记）任务中明显优于单语言模型。

Conclusion: 有限多语言预训练的ModernBERT模型在特定语言任务中表现出色，特别是在长上下文处理方面具有优势，代码和模型已公开发布。
...
TL;DR: 本文提出了一种名为Anchor的强化学习方法，通过将真实轨迹注入到训练过程中来防止早期训练崩溃，显著提高了语言模型在可验证推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法主要优化最终任务结果，但在早期训练中当负面奖励占主导时模型容易崩溃。这在诚实对齐任务中尤为突出，模型不仅需要回答可解答的查询，还要识别何时无法从给定前提得出结论。

Method: 作者构建了两个基于图结构的多步演绎推理数据集，并提出Anchor方法——一种在训练过程中注入真实轨迹的强化学习方法，以防止早期训练崩溃。

Result: 实验表明，传统的GRPO方法在这些任务上表现不佳，而Anchor方法能够稳定学习过程，显著提高整体推理性能。课程学习有一定帮助但需要精心设计的数据集。

Conclusion: 研究强调了训练动态对于在语言模型中实现可靠演绎推理的重要性，Anchor方法为解决诚实对齐中的训练稳定性问题提供了有效解决方案。
...
TL;DR: POTSA是一个基于跨语言平行语音对和最优传输的框架，旨在通过偏差补偿模块和token级OT约束来弥合高资源和低资源语言之间的翻译差距，在FLEURS数据集上实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的语音大语言模型在多语言语音到文本翻译中往往忽略源语言之间的语义共性，导致翻译性能偏差。

Method: 提出POTSA框架：1）偏差补偿模块粗略对齐跨语言语音表示；2）在Q-Former上施加token级最优传输约束；3）采用层调度策略将OT约束集中在语义最有益的层。

Result: 在FLEURS数据集上，平均BLEU提升+0.93（五种常用语言）和+5.05（零样本语言），仅使用每个源语言10小时的平行语音数据。

Conclusion: POTSA通过跨语言对齐和最优传输约束有效提升了多语言语音翻译性能，特别是在低资源和零样本语言上表现突出。
...
TL;DR: C$^3$TG是一个两阶段框架，用于细粒度、多维度的文本属性控制，通过选择性配对LLM与属性分类器，使用加权KL散度调整token概率，并在优化阶段通过迭代反馈解决属性冲突。
...
Method: 提出C$^3$TG两阶段框架：生成阶段选择性配对LLM与17个可用维度属性分类器，使用加权KL散度调整token概率；优化阶段利用结合分类器分数和惩罚项的能量函数，通过迭代反馈解决属性冲突。

Result: 实验表明C$^3$TG在多个指标上显著优于基线方法，包括属性准确性、语言流畅性和输出多样性，同时降低毒性。

Conclusion: C$^3$TG为多维文本属性控制提供了一个有效且灵活的解决方案，无需昂贵的模型修改。
...
Conclusion: 该工作为个性化创意写作技术发展奠定了基础，展示了建模个人创意写作偏好的可行性，并强调了实际偏好数据相对于自述偏好的重要性。
...
TL;DR: 本文提出了一种可解释的高棉语极性分类器，通过微调基于指令推理的Qwen-3模型，不仅准确预测极性标签，还能提供支持预测的推理过程。
...
Method: 通过微调基于指令推理的Qwen-3模型，实现可解释的高棉语极性分类，并构建了包含短到中等长度随意、罗马化和混合代码高棉语表达的新数据集。

Result: 实验结果表明，微调后的模型不仅能准确预测标签，还能通过识别极性相关关键词或短语来提供推理支持。

Conclusion: 成功开发了可解释的高棉语极性分类器，并贡献了新的高棉语极性数据集和微调模型，均通过Hugging Face公开提供。
...
TL;DR: 本文提出AdaptDel方法，通过动态调整删除率来改进序列分类的认证鲁棒性，针对编辑距离扰动提供理论保证，并在自然语言任务中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定删除率处理变长序列（如自然语言句子）导致性能不佳，需要适应不同输入特性的动态调整机制。

Method: 引入AdaptDel方法，采用可适应删除率机制，基于输入属性动态调整删除率，并将随机平滑理论框架扩展到变率删除场景。

Result: 在自然语言任务中取得强实证结果，认证区域的中值基数相比最先进方法提升高达30个数量级。

Conclusion: AdaptDel方法通过动态删除率机制有效提升了序列分类在编辑距离扰动下的认证鲁棒性，具有显著的理论和实证优势。
...
TL;DR: 提出了mmJEE-Eval多模态双语基准，基于印度JEE Advanced考试，评估发现前沿VLMs在复杂推理任务上表现优异，但开源模型表现显著落后，且现有基准无法有效区分真实推理能力与模式匹配。
...
Method: 构建了包含1,460个英语和印地语问题的mmJEE-Eval基准，涵盖物理、化学和数学领域，评估了17个最先进模型，并进行了系统消融实验。

Result: 前沿VLMs在2025年考题上达到77-84%准确率，开源模型仅37-45%，差距显著；当增加元认知推理负荷时，GPT-5仅修复5.2%错误。

Conclusion: mmJEE-Eval基准能有效区分优秀训练和推理方法，其难度源于复杂性和推理深度而非记忆，为评估真实推理能力提供了有效工具。
...
TL;DR: SeerSC是一个动态自一致性框架，通过整合System 1和System 2推理，同时提高令牌效率和降低延迟。System 1快速计算答案熵来评估样本的扩展潜力，System 2进行动态自一致性推理，实现并行生成，显著减少令牌消耗和推理延迟。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展能提升大语言模型的推理性能，但计算成本高昂。现有动态自一致性方法虽然减少了令牌消耗，但仍受限于顺序请求的高延迟问题。

Method: 整合System 1和System 2推理：利用快速的System 1计算查询的答案熵，评估样本扩展潜力；System 2进行动态自一致性推理，实现并行生成。

Result: 相比现有方法，令牌消耗减少高达47%，推理延迟降低43%，且无显著性能损失。

Conclusion: SeerSC框架通过System 1和System 2的协同工作，有效解决了测试时扩展中的令牌效率和延迟问题，实现了更好的性能平衡。
...
TL;DR: 提出了Spider4SSC数据集和S2CLite解析工具，S2CLite是一个轻量级、本体无关的解析器，可将SPARQL查询转换为Cypher查询，在Spider4SPARQL数据集上达到77.8%的解析准确率，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有SPARQL到Cypher转换工具存在解析错误率高的问题，需要开发更准确、轻量级的转换工具来支持大规模查询翻译。

Method: S2CLite采用纯基于规则的方法（受传统编程语言编译器启发），无需RDF图或外部工具，实现SPARQL到Cypher的转换。

Result: 在BSBM42和Spider4SPARQL数据集上，S2CLite解析准确率达77.8%，优于S2CTrans的44.2%；在交集查询子集上执行准确率达96.6%，优于S2CTrans 7.3%。

Conclusion: S2CLite显著提升了SPARQL到Cypher的转换准确率，并生成了统一的Text-to-Query数据集Spider4SSC，为多语言查询转换研究提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型用于评估输出正变得越来越有效和可扩展，但尚不清楚这种能力是否能从特定任务评估扩展到更一般的文本质量评估，特别是在多语言环境中。

Method: 首先自动生成文本质量偏好数据，然后使用这些数据训练开源基础大语言模型，使其与高质量和低质量文本的评分保持一致。

Result: 在115种语言上的全面评估表明，所提出模型的性能得到了改善。进一步分析发现，这种增强的评估能力还能显著提升下游任务的性能。
...
TL;DR: 本文系统研究了大语言模型在开放式文本生成和多项选择任务中的自我纠正机制差异，发现不同任务结构下模型的改进模式和失败模式存在显著区别。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型自我纠正机制在不同任务范式（开放式生成vs多项选择）中的动态差异，以理解任务结构与输出空间的交互对模型自我纠正能力的影响。

Method: 通过在不同自然语言理解和推理任务上比较开放式生成和多项选择两种范式的性能趋势和错误纠正行为，涵盖不同规模和系列的多个语言模型。

Result: 开放式生成受益于重新解释和组合优化的灵活性，而多项选择可以利用更清晰的解决方案边界但受限于提供的选项。实验揭示了不同的改进模式和失败模式。

Conclusion: 自我纠正机制的设计应考虑任务结构与输出空间的交互，这对知识密集型推理和决策导向的LLM应用具有重要意义。
...
TL;DR: AMaPO是一种解决离线偏好优化中过拟合-欠拟合困境的自适应边际偏好优化算法，通过动态调整边际来改善排名准确性。


<details>
  <summary>Details</summary>
Motivation: 当前离线偏好优化方法在排名准确性上存在局限性，主要问题是过拟合-欠拟合困境：正确排名的样本梯度过大而浪费，错误排名的样本梯度不足。

Method: 提出AMaPO算法，采用基于Z归一化和指数缩放的自适应边际，动态重新分配学习努力，放大错误排名样本的梯度，抑制正确排名样本的梯度。

Result: 在广泛使用的基准测试中，AMaPO不仅实现了更好的排名准确性和下游对齐性能，还成功缓解了核心的过拟合和欠拟合问题。

Conclusion: AMaPO通过自适应边际设计有效解决了离线偏好优化中的过拟合-欠拟合困境，提升了模型对齐效果。
...
TL;DR: 本文开发了一个针对低资源语言（巴斯克语）的强大多模态大语言模型，通过创建训练和评估数据集，探索了不同数据混合比例，发现仅需20%的巴斯克语多模态数据即可获得良好效果，且无需巴斯克语指令调优的骨干LLM。
...
Method: 开发了训练和评估用的图文数据集，使用Llama-3.1-Instruct和巴斯克语适配变体Latxa两种大语言模型作为骨干，探索了多种数据混合训练策略。

Result: 发现仅需约20%的巴斯克语多模态数据即可在巴斯克语基准测试中获得稳定结果，且意外发现无需巴斯克语指令调优的骨干LLM也能构建强大的巴斯克语MLLM。

Conclusion: 研究结果为其他低资源语言开发多模态大语言模型开辟了道路，并公开释放了相关资源。
...
Method: 基于真实世界咨询案例构建多样化客户档案，按照专家指南进行模拟，建立基于成熟心理学量表的多维度性能评估框架。

Result: 评估了多个通用LLM和专业咨询模型，揭示了它们当前的局限性，并与心理学家合作分析了模型在与不同类型客户互动时失败的原因。
...
TL;DR: GSAP-ERE是一个用于机器学习研究信息提取的细粒度数据集，包含10种实体类型和18种关系类型，从100篇ML论文全文标注了6.3万实体和3.5万关系，支持知识图谱构建和AI研究可复现性监控。


<details>
  <summary>Details</summary>
Motivation: 机器学习研究发展迅速，从科学出版物中提取信息能够大规模识别研究概念和资源，有助于提高ML研究的理解和可复现性。

Method: 构建了手动标注的细粒度数据集GSAP-ERE，包含实体识别和关系抽取任务，并比较了微调模型与LLM提示策略的性能。

Result: 微调模型在NER任务上达到80.6%，RE任务达到54.0%，显著优于LLM提示方法（NER:44.4%, RE:10.1%）。
...
TL;DR: 本文提出了一种名为TPoT的新策略，通过语义过滤文本作为深度学习模型的输入，专门用于预测大五人格特质，解决了文本量过大时的输入限制问题，并在Stream of Consciousness Essays数据集上提高了预测准确率。


<details>
  <summary>Details</summary>
Motivation: 从生成文本预测个体人格是一项具有挑战性的任务，特别是当文本量很大时，需要解决大语言模型的输入文本限制问题。
...
Conclusion: TPoT策略是一种简单而有效的方法，能够通过语义相关文本选择来提升人格预测性能。
...
### [48] [Accelerating Training Speed of Tiny Recursive Models via Curriculum Guided Adaptive Recursion](https://arxiv.org/abs/2511.08653)
*Kaleem Ullah Qasim,Jiashu Zhang*

Main category: cs.LG

TL;DR: CGAR是一种新的训练方法，通过渐进深度课程学习和分层监督加权，显著提高了递归推理模型的训练效率，在Sudoku-Extreme数据集上实现了1.71倍训练加速和42%成本降低，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 递归推理模型虽然性能优异，但训练计算成本高昂（约36 GPU小时/数据集），限制了广泛应用和研究。需要开发更高效的训练方法。

Method: CGAR包含两个核心组件：渐进深度课程学习（动态调整递归深度）和分层监督加权（对监督步骤应用指数衰减权重），将课程学习应用于架构深度而非传统数据排序。

Result: 在Sudoku-Extreme数据集上，CGAR实现1.71倍训练加速（10.93到6.38小时），成本降低42%，精度仅下降0.63%（86.65%到86.02%）。渐进深度课程学习单独可实现2.26倍加速和85.47%精度。

Conclusion: 在架构深度上应用原则性课程学习能够高效训练递归推理模型，在适度硬件上实现训练效率和解决方案质量的同时提升。
...
### [49] [TabPFN-2.5: Advancing the State of the Art in Tabular Foundation Models](https://arxiv.org/abs/2511.08667)
...
<details>
  <summary>Details</summary>
Motivation: 解决表格AI领域对更大数据集处理能力的需求，提升模型在工业标准基准测试中的性能表现，同时提供更高效的部署方案。

Method: 基于TabPFN基础模型架构进行扩展，引入新的蒸馏引擎将模型转换为紧凑的MLP或树集成，保持准确性的同时大幅降低延迟。

Result: 在TabArena基准测试中显著优于调优的树模型，与AutoGluon 1.4四小时调优集成效果相当；对小中型分类数据集有100%胜率对抗XGBoost，对更大数据集保持87%胜率。

Conclusion: TabPFN-2.5代表了表格基础模型的重要进展，为实际应用提供了更强的性能和更便捷的部署方案，将增强现有TabPFN生态系统的应用效果。
...
### [50] [Gromov-Wasserstein Graph Coarsening](https://arxiv.org/abs/2511.08733)
...
### [51] [A Generalized Bias-Variance Decomposition for Bregman Divergences](https://arxiv.org/abs/2511.08789)
*David Pfau*

Main category: cs.LG

TL;DR: 本文提供了一个Bregman散度下的偏差-方差分解的清晰推导，该分解是平方误差分解的推广，适用于指数族的最大似然估计。


<details>
  <summary>Details</summary>
Motivation: 偏差-方差分解是统计学和机器学习的核心结果，但通常仅针对平方误差进行介绍。作者希望为Bregman散度下的偏差-方差分解提供一个清晰、独立的推导，用于教学目的。

Method: 作者提供了一个Bregman散度下偏差-方差分解的推导过程，该推导基于指数族最大似然估计的相关理论。

Result: 成功推导出了Bregman散度下的偏差-方差分解公式，这是对传统平方误差分解的推广。

Conclusion: 虽然该结果在文献中已经存在，但本文提供了一个更清晰、更易理解的推导过程，并补充了相关文献讨论，有助于教学和理解。
...
### [52] [Physics-Informed Machine Learning for Characterizing System Stability](https://arxiv.org/abs/2511.08831)
...
<details>
  <summary>Details</summary>
Motivation: 复杂动力系统设计中需要确保状态轨迹收敛到期望平衡点，但稳定性区域难以预先确定和计算。现有方法依赖系统方程显式知识，限制了实际应用。

Method: 采用二次型形式的Lyapunov函数，通过最小化Zubov方程的平均残差来拟合未知二次算子，将动力系统视为黑箱处理。

Result: 数值实验表明，该方法成功推断出接近最大椭球体的系统稳定性区域估计，无需系统方程知识。

Conclusion: LyapInf方法为未知系统方程的稳定性分析提供了一种有效的物理信息机器学习解决方案，能够准确估计稳定性区域。
...
### [53] [TIGER-MARL: Enhancing Multi-Agent Reinforcement Learning with Temporal Information through Graph-based Embeddings and Representations](https://arxiv.org/abs/2511.08832)
*Nikunj Gupta,Ludwika Twardecka,James Zachary Hare,Jesse Milzman,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.LG

TL;DR: 本文提出TIGER方法，通过基于图的嵌入和表示来捕捉和利用时间信息，以增强多智能体强化学习。该方法显式建模智能体间协调结构随时间演化的过程，构建动态时间图并使用时序注意力编码器聚合信息，在协调密集型基准测试中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有大多数多智能体强化学习方法依赖静态或每步关系图，忽略了智能体在适应、移动或重组合作策略时自然产生的交互时间演化。捕捉这种演化依赖关系对于实现稳健和自适应协调至关重要。

Method: TIGER构建多智能体强化学习智能体的动态时间图，连接当前和历史交互，使用时序注意力编码器聚合结构和时序邻域信息，生成时间感知的智能体嵌入来指导合作策略学习。

Result: 在两个协调密集型基准测试上的广泛实验表明，TIGER在任务性能和样本效率方面持续优于各种基于价值分解和基于图的多智能体强化学习基线方法。

Conclusion: 全面的消融研究揭示了结构和时间因素如何共同塑造多智能体强化学习中有效的策略学习，验证了TIGER方法的有效性。
...
### [54] [Enhancing DPSGD via Per-Sample Momentum and Low-Pass Filtering](https://arxiv.org/abs/2511.08841)
...
<details>
  <summary>Details</summary>
Motivation: 现有的差分隐私技术通常只能解决噪声或偏差中的一个问题，减少DP噪声可能会加剧裁剪偏差，反之亦然。需要一种能同时缓解这两种问题的方法。

Method: DP-PMLF结合了逐样本动量（在裁剪前平滑梯度估计以减少采样方差）和后处理低通滤波（在不消耗额外隐私预算的情况下衰减高频DP噪声）。

Result: 理论分析显示在严格的DP保证下具有改进的收敛速度，实证评估表明DP-PMLF相比多种最先进的DPSGD变体显著提升了隐私-效用权衡。

Conclusion: DP-PMLF通过同时缓解DP噪声和裁剪偏差，为差分隐私深度学习训练提供了一种有效的解决方案，在保持隐私保护的同时提高了模型准确性。
...
### [55] [On topological descriptors for graph products](https://arxiv.org/abs/2511.08846)
*Mattie Ji,Amauri H. Souza,Vikas Garg*

Main category: cs.LG

TL;DR: 本文研究了在图乘积上构建各种过滤及其对拓扑描述符（欧拉特征和持续同调）的影响，建立了欧拉特征在基于颜色过滤上的表达能力完整表征，证明了持续同调在图乘积中包含比单个图更多的信息，并提供了计算图乘积持续同调图的算法。


<details>
  <summary>Details</summary>
Motivation: 拓扑描述符在关系数据中捕获多尺度结构信息的应用日益增多，但图乘积上的过滤如何影响拓扑描述符的表达能力尚未得到系统研究。

Method: 考虑图乘积上的各种过滤，建立欧拉特征在颜色过滤上的表达能力理论表征，开发计算图乘积顶点和边级别过滤持续同调图的算法，并通过实证研究验证理论分析。

Result: 证明了持续同调在图乘积中包含比单个图更多的信息，而欧拉特征则不具备这种特性；提供了高效的算法实现，并通过实验验证了方法的有效性和计算效率。

Conclusion: 这项工作通过乘积过滤为强大的图持续描述符开辟了新途径，图乘积的持续同调能够提供比单个图更丰富的拓扑信息。
...
### [56] [Rethinking Graph Super-resolution: Dual Frameworks for Topological Fidelity](https://arxiv.org/abs/2511.08853)
*Pragya Singh,Islem Rekik*

Main category: cs.LG

TL;DR: 本文提出了两种GNN无关的图超分辨率框架：Bi-SR通过二分图连接低分辨率和高分辨率节点实现结构感知的节点超分辨率，DEFEND通过将高分辨率边映射到对偶图节点来学习边表示。在真实脑连接组数据集上取得了最先进性能，并引入了12个模拟数据集用于方法评估。


<details>
  <summary>Details</summary>
Motivation: 现有的图超分辨率方法存在两个关键问题：基于矩阵的节点超分辨率忽略了图结构且缺乏排列不变性；依赖节点表示推断边权重限制了可扩展性和表达能力。这些问题在医疗等资源受限领域尤其突出。

Method: 1. Bi-SR框架：构建连接低分辨率和高分辨率节点的二分图，实现保持拓扑结构和排列不变性的节点超分辨率；2. DEFEND框架：将高分辨率边映射到对偶图节点，通过标准节点GNN进行边推断。

Result: 在真实脑连接组数据集上，两个框架在七个拓扑度量上都达到了最先进的性能。同时创建了12个模拟数据集来捕获不同的拓扑结构和LR-HR关系。

Conclusion: 提出的Bi-SR和DEFEND框架有效解决了现有图超分辨率方法的局限性，在保持图结构的同时实现了更好的可扩展性和表达能力，为资源受限领域的图数据处理提供了实用解决方案。
...
### [57] [Decomposition of Small Transformer Models](https://arxiv.org/abs/2511.08854)
*Casper L. Christensen,Logan Riggs*

Main category: cs.LG

TL;DR: 本文扩展了随机参数分解（SPD）方法到Transformer模型，提出了适用于序列数据的因果重要性函数和新损失函数，成功分解了玩具模型和GPT-2-small，识别出可解释的概念组件。


<details>
  <summary>Details</summary>
Motivation: 虽然参数空间分解方法在玩具模型上取得了成功，但尚未应用于真实模型。本文旨在将SPD方法扩展到现代Transformer模型，弥合这一差距。
...
### [58] [ForeSWE: Forecasting Snow-Water Equivalent with an Uncertainty-Aware Attention Model](https://arxiv.org/abs/2511.08856)
...
### [59] [EEG-X: Device-Agnostic and Noise-Robust Foundation Model for EEG](https://arxiv.org/abs/2511.08861)
*Navid Mohammadi Foumani,Soheila Ghane,Nam Nguyen,Mahsa Salehi,Geoffrey I. Webb,Geoffrey Mackellar*

Main category: cs.LG

TL;DR: EEG-X是一个设备无关且抗噪声的脑电图基础模型，通过位置通道嵌入和噪声感知重建策略解决EEG分析中的跨设备变异性和低信噪比问题。


<details>
  <summary>Details</summary>
Motivation: 解决EEG分析中的两个关键挑战：(1)不同记录设备和配置导致的数据集变异性，(2)脑电信号信噪比低，脑信号常被伪迹和非脑源信号淹没。

Method: 引入位置通道嵌入编码空间信息；采用噪声感知掩码重建策略在原始和潜在空间重建去噪信号；使用字典启发的卷积变换层将信号投影到结构化特征空间。

Result: 在来自不同设备的数据集上，EEG-X在多个下游EEG任务中优于最先进方法，在电极布局不同的跨域设置中表现优异。

Conclusion: EEG-X通过创新的位置嵌入和噪声鲁棒性设计，为EEG表示学习提供了一个有效的设备无关基础模型。
...
### [60] [Transformer-Based Sleep Stage Classification Enhanced by Clinical Information](https://arxiv.org/abs/2511.08864)
*Woosuk Chung,Seokwoo Hong,Wonhyeok Lee,Sangyoon Bae*

Main category: cs.LG

TL;DR: 提出了一种结合Transformer编码器和1D CNN聚合器的两阶段架构，系统研究整合临床元数据和专家事件标注对睡眠分期的影响，在SHHS队列中显著提升了分期准确性。
...
Method: 两阶段架构：Transformer-based每时段编码器 + 1D CNN聚合器，整合受试者级临床元数据（年龄、性别、BMI）和每时段专家事件标注（呼吸暂停、血氧饱和度下降、觉醒、周期性呼吸）。

Result: 与仅使用PSG的基线相比，最终模型macro-F1从0.7745提升至0.8031，micro-F1从0.8774提升至0.9051，事件标注贡献最大增益。特征融合优于预测相同辅助标签的多任务方法。

Conclusion: 通过临床有意义特征增强学习表示可提高性能和可解释性，无需修改PSG导联或额外传感器，为构建上下文感知、专家对齐的睡眠分期系统提供了实用且可扩展的路径。
...
### [61] [Spectral Predictability as a Fast Reliability Indicator for Time Series Forecasting Model Selection](https://arxiv.org/abs/2511.08884)
*Oliver Wang,Pengrui Quan,Kang Yang,Mani Srivastava*

Main category: cs.LG

TL;DR: 提出了一种基于频谱可预测性指标Ω的快速模型选择方法，该指标能在几秒内评估数据集是否适合使用大型时序基础模型，从而显著降低模型验证成本。


<details>
  <summary>Details</summary>
Motivation: 解决实际部署中时间序列预测模型选择的两难困境：全面验证多个模型计算成本过高，但选择错误模型会导致性能不佳。

Method: 使用频谱可预测性指标Ω来系统性地分层模型家族性能，通过控制实验和扩展到51个模型、28个数据集的GIFT-Eval基准进行验证。

Result: 发现当Ω值高时，大型时序基础模型(TSFMs)系统性地优于轻量级任务训练基线，但随着Ω值下降，其优势消失。计算Ω仅需每数据集几秒钟。

Conclusion: Ω指标能够可预测地分层模型性能，提供了一个实用的初步筛选工具，既能降低验证成本，又突出了需要在真正困难（低Ω）问题上表现出色的模型需求。
...
### [62] [FAST-CAD: A Fairness-Aware Framework for Non-Contact Stroke Diagnosis](https://arxiv.org/abs/2511.08887)
...
TL;DR: FAST-CAD是一个结合领域对抗训练和组分布鲁棒优化的公平非接触式中风诊断框架，通过理论保证和公平性边界确保跨人口统计组的公平准确诊断。


<details>
  <summary>Details</summary>
Motivation: 现有自动化中风诊断方法存在跨人口统计组的公平性问题，可能加剧医疗保健差距，需要开发公平且准确的诊断方法。

Method: 结合领域对抗训练（DAT）和组分布鲁棒优化（Group-DRO），使用自监督编码器和对抗领域判别学习人口统计不变表示，同时优化最差组风险。

Result: 在覆盖12个人口统计子组的多模态数据集上，该方法实现了优越的诊断性能，同时保持跨人口统计组的公平性。
...
### [63] [Diffusion Policies with Value-Conditional Optimization for Offline Reinforcement Learning](https://arxiv.org/abs/2511.08922)
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习中，由分布外动作引起的价值高估问题严重限制了策略性能。现有基于扩散模型的方法通常对低质量数据集中的冗余动作进行无差别正则化，导致过度保守和扩散模型表达能力与效率之间的不平衡。

Method: 提出DIffusion policies with Value-conditional Optimization (DIVO)，利用扩散模型生成高质量、广泛覆盖的分布内状态-动作样本。引入基于动作优势值的二元加权机制来指导扩散模型训练，在策略改进阶段动态筛选高回报潜力动作。
...
### [64] [TransactionGPT](https://arxiv.org/abs/2511.08939)
...
Method: 引入专门设计的3D-Transformer架构，增强模态融合和计算效率，支持与下游目标的联合优化，并探索LLM嵌入的整合。

Result: TGPT显著优于竞争性生产模型和基线方法，在生成未来交易方面表现出优势，同时实现更快的训练和推理速度。
...
TL;DR: 提出了一种通过准确重构潜在表示中的触发激活变化(TAC)来移除后门攻击的新方法，通过凸二次优化问题求解最小扰动，识别中毒类别并进行微调，在多个数据集和攻击类型上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有后门防御方法基于触发激活变化(TAC)识别和移除后门神经元，但由于TAC值估计不准确导致识别精度低，需要更准确的TAC重构方法。

Method: 将最小扰动问题建模为凸二次优化问题，通过求解最优解作为TAC的替代，检测统计上小的L2范数扰动来识别中毒类别，并在微调中利用该扰动移除后门。

Result: 在CIFAR-10、GTSRB和TinyImageNet数据集上的实验表明，该方法在不同攻击类型、数据集和架构上都能实现优越的后门抑制效果，同时保持高清洁准确率，优于现有防御方法。

Conclusion: 通过准确重构TAC值并利用凸优化方法，提出的方法能有效识别和移除后门攻击，在多种场景下都表现出色。
...
Result: 在常识推理基准测试中，使用Qwen1.5-MoE和DeepSeek-MoE的实验表明，Bayesian-MoE在期望校准误差（ECE）和负对数似然（NLL）方面均优于基线方法。

Conclusion: Bayesian-MoE框架在可靠的下游决策制定方面具有有效性，为微调后的大语言模型提供了改进的不确定性估计能力。
...
TL;DR: BRIDGE是一种用于AI风险模型的数据复用RCT设计，通过重复使用已完成试验中AI模型预测一致的参与者数据，显著减少后续试验的入组需求，降低临床试验成本和时间。
...
Method: 提出BRIDGE试验设计，当新旧AI模型预测一致时，重复使用已完成试验的参与者数据，减少新试验的入组人数要求。提供检查清单确保因果推断有效性和I类错误控制。

Result: 在乳腺癌、心血管疾病和脓毒症的真实数据集中，连续AI模型间存在高度一致性，高风险队列重叠率最高达64.8%。模拟乳腺癌筛查研究显示，该设计减少46.6%的入组需求，节省超过280万美元，同时保持80%的统计功效。

Conclusion: BRIDGE设计将临床试验转变为自适应、模块化研究，使每个模型迭代都能获得I级证据，加速AI在常规护理中的成本效益转化。
...
TL;DR: 提出了一种在高维非欧几里得流形上聚类数据的高效框架，通过p-Fréchet映射将流形数据嵌入到低维欧几里得空间，然后应用标准欧几里得聚类方法。


<details>
  <summary>Details</summary>
Motivation: 解决标准内在方法在流形上聚类时面临的计算挑战，特别是针对对称正定矩阵流形的复杂情况。
...
Conclusion: 该方法为流形数据聚类提供了一种高效且准确的解决方案，特别适用于现有方法难以处理的情况。
...
TL;DR: FLAD是一个基于联邦学习的自动驾驶框架，利用分布式多模态传感器数据，通过云-边-车协作架构、智能并行化协同训练和知识蒸馏方法，在保护隐私的同时提升自动驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在自动驾驶中训练面临的高计算传输成本、隐私问题，以及异构环境下数据分布的挑战。

Method: 采用云-边-车协作架构减少通信延迟；智能并行化协同训练优化效率；知识蒸馏方法个性化LLM以适应异构边缘数据。

Result: 在NVIDIA Jetsons测试平台上实现了优越的端到端自动驾驶性能，高效利用分布式车辆资源。
...
TL;DR: FedSDWC是一种基于因果推断的联邦学习方法，通过整合不变特征和可变特征来建模弱因果影响，有效解决了数据分布差异问题，显著提升了联邦学习的泛化能力和OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 随着数据隐私需求增长和计算基础设施进步，联邦学习成为重要分布式学习范式，但数据分布差异严重影响了其在真实部署中的可靠性。

Method: 提出FedSDWC因果推断方法，通过建模不变特征和可变特征之间的弱因果影响来推断因果语义表示，克服现有不变学习方法在准确捕捉不变特征和直接构建因果表示方面的局限性。

Result: 在多个基准数据集上的广泛实验验证了FedSDWC在处理协变量偏移和语义偏移方面的优越性能，在CIFAR-10上平均比次优基线FedICON提升3.04%，在CIFAR-100上提升8.11%。
...
TL;DR: FairM2S是一个公平性感知的元学习框架，用于基于音视频数据的压力检测，通过集成Equalized Odds约束和对抗梯度掩码来减少性别偏见，在数据稀缺场景下实现公平且准确的压力检测。


<details>
  <summary>Details</summary>
Motivation: 现有AI驱动的压力检测模型在数据稀缺场景下经常表现出性别偏见，这影响了心理健康护理的公平性，因此需要开发能够有效减轻偏见的公平检测方法。

Method: 提出FairM2S框架，在元训练和适应阶段集成Equalized Odds约束，采用对抗梯度掩码和公平约束的元更新机制，利用音视频数据进行压力检测。

Result: 与五个最先进基线方法相比，FairM2S达到78.1%的准确率，同时将Equal Opportunity降低到0.06，显示出显著的公平性提升。还发布了带有性别标注的智能手机采集数据集SAVSD。

Conclusion: FairM2S是心理健康AI中公平且可扩展的少样本压力检测的最先进方法，公开发布了数据集和FairM2S框架以支持公平性研究。
...
TL;DR: 本文提出了一种基于反馈增强的模型无关对决赌博算法，通过增强置信边界整合增强的人类反馈，在稀疏反馈环境下实现高效的交互式偏好获取。


<details>
  <summary>Details</summary>
Motivation: 现有对决赌博算法在人类反馈稀疏时效率低下，而依赖参数化奖励模型的方法容易受到模型设定错误的影响。本文探索基于反馈增强的替代方案来改进模型无关框架。

Method: 引入增强置信边界来整合增强的人类反馈，在广义集中性属性下进行分析，并通过遗憾分析评估多因素性能权衡。

Result: 原型算法在多个交互式偏好获取基准测试中表现出竞争力，包括推荐系统、多目标优化和大语言模型响应优化。

Conclusion: 该方法在更广泛的应用中展示了可证明高效的交互式偏好获取潜力，为稀疏反馈环境下的偏好学习提供了有效解决方案。
...
TL;DR: 该论文提出了一种学习可定制距离度量的方法，用于改进分类数据的聚类分析，通过打破属性类别间的固有关系约束，学习适合不同聚类结构的距离度量。


<details>
  <summary>Details</summary>
Motivation: 分类属性在真实数据集的聚类分析中普遍存在，但由于缺乏明确定义的值间关系，难以探索紧凑的分类数据聚类。现有方法通常假设固定的拓扑关系，限制了其对不同聚类结构的适应性。

Method: 打破属性类别间的固有关系约束，学习适合灵活准确揭示各种聚类分布的自定义距离度量。学习到的类别关系与欧几里得距离度量兼容，可扩展到包含数值和分类属性的混合数据集。

Result: 在12个真实基准数据集上的比较实验和显著性检验表明，所提方法具有优越的聚类准确性，平均排名为1.25，显著优于当前最佳方法的5.21排名。

Conclusion: 通过可学习的类别关系，显著增强了聚类算法的拟合能力，为分类数据聚类提供了更灵活有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLM生成的标签存在质量低（标签噪声）和缺乏错误校正机制的双重限制，需要一种高效的方法来提升标签质量并减少人工成本。

Method: 提出HCL设置，仅对VLM存在差异的实例进行选择性人类校正；推导风险一致性估计器结合人工校正标签和VLM预测来训练分类器；提出条件概率方法估计标签分布。

Result: 大量实验表明，该方法在分类性能上表现优越，对标签噪声具有鲁棒性，验证了HCL在实际弱监督场景中的有效性。

Conclusion: HCL方法通过高效的人类校正机制，成功解决了VLM生成标签的质量问题，在保证标注质量的同时显著降低了人工成本，为弱监督学习提供了有效的解决方案。
...
TL;DR: FedPM是一种新颖的联邦学习方法，通过二阶优化和预条件混合解决本地预条件器漂移问题，在异构数据设置中显著提升收敛性和测试准确率。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法如LocalNewton、LTDA和FedSophia在二阶优化中采用简单的参数混合，导致本地预条件器漂移，特别是在异构数据环境下严重影响参数训练的收敛性。

Method: 通过将理想二阶更新分解为服务器上的参数混合和客户端的本地参数更新，引入服务器上的预条件混合机制来有效缓解本地预条件器漂移问题。

Result: 理论分析显示在强凸目标函数和单次本地更新场景下具有超线性收敛速率。实验结果表明FedPM相比传统方法在测试准确率上有显著提升。

Conclusion: FedPM通过预条件混合有效解决了二阶联邦学习中的预条件器漂移问题，充分发挥了二阶优化的潜力，在异构数据环境下表现出优越性能。
...
TL;DR: 本文研究了在RLHF/DPO对齐过程中通过翻转偏好标签进行最小成本数据投毒攻击的理论基础，提出了成本最小化的后处理方法，并分析了攻击成本的上界和下界。
...
Method: 将投毒攻击建模为具有线性约束的凸优化问题，推导最小攻击成本的上界和下界，并提出一种后处理方法减少所需标签翻转次数。

Result: 理论分析表明，任何现有的标签翻转攻击都可以通过提出的后处理方法减少标签翻转次数，同时保持投毒效果。实证结果显示该方法能显著降低投毒成本，特别是在奖励模型特征维度相对于数据集大小较小时。
...
TL;DR: 本文提出了一种基于链式方法的框架，用于估计未见数据上期望极值的上下界，无需访问特定输入位置。该方法改进了RBF和Matérn等常用核函数的边界估计，并开发了局部不确定性量化方法。
...
Method: 提出基于链式方法的框架，提供核函数特定的改进（RBF和Matérn核），避免分析松弛以提高数值紧密度，并开发基于分区直径的局部不确定性量化方法。

Result: 实验结果表明，该方法在合成和真实数据集上优于现有方法，验证了理论发现。

Conclusion: 所提出的链式框架能够有效估计期望极值边界，提供更紧的核函数特定边界，并实现局部不确定性量化，在理论和实验上都优于现有方法。
...
### [78] [Enabling Agents to Communicate Entirely in Latent Space](https://arxiv.org/abs/2511.09149)
*Zhuoyun Du,Runze Wang,Huiyu Bai,Zouying Cao,Xiaoyong Zhu,Bo Zheng,Wei Chen,Haochao Ying*

Main category: cs.LG

TL;DR: Interlat提出了一种基于LLM隐藏状态的智能体间潜在空间通信范式，通过直接传输潜在表示而非离散token来提升协作问题解决能力，并通过压缩机制加速推理。


<details>
  <summary>Details</summary>
Motivation: 自然语言作为LLM智能体通信媒介存在根本限制，将丰富的内部潜在状态降采样为离散token会限制信息传输的深度和细微差别，从而阻碍协作问题解决。

Method: 利用LLM的最后一个隐藏状态作为心智表示进行直接传输（潜在通信），并通过完全潜在空间推理的额外压缩过程进一步压缩潜在通信。

Result: 实验表明Interlat优于微调的思维链提示和单智能体基线，促进更多探索行为并实现真正利用潜在信息。压缩不仅显著加速推理，还通过高效信息保留机制保持竞争力。
...
### [79] [Unsupervised Feature Selection Through Group Discovery](https://arxiv.org/abs/2511.09166)
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督特征选择方法通常单独评估特征，但信息信号往往来自相关特征组，独立评估效果不佳。现有方法要么依赖预定义分组，要么需要标签监督，限制了应用范围。

Method: GroupFS是一个完全可微分框架，对特征图和样本图施加拉普拉斯平滑性，并应用组稀疏正则化器来学习紧凑的结构化表示。

Result: 在九个基准数据集（包括图像、表格数据和生物数据集）上，GroupFS在聚类任务中始终优于最先进的无监督特征选择方法，并选择出与有意义的模式对齐的特征组。

Conclusion: GroupFS能够有效发现和选择特征组，在无监督特征选择任务中表现出色，选择的特征组与有意义的模式一致。
...
### [80] [Data Fusion-Enhanced Decision Transformer for Stable Cross-Domain Generalization](https://arxiv.org/abs/2511.09173)
*Guojian Wang,Quinson Hon,Xuyang Chen,Lin Zhao*

Main category: cs.LG

TL;DR: 本文提出DFDT方法解决决策变换器在跨域迁移中的轨迹片段拼接问题，通过两级数据过滤、优势条件令牌和Q引导正则化来提升拼接质量和策略性能。
...
Method: DFDT通过MMD不匹配和OT偏差两级数据过滤融合目标数据与可信源片段，用优势条件令牌替代RTG令牌，并应用Q引导正则化抑制连接点值和动作跳跃。

Result: 在D4RL风格控制任务的重力、运动学和形态学迁移中，DFDT相比强离线RL和序列模型基线提高了回报和稳定性，并通过令牌拼接和序列语义稳定性分析验证了性能提升。

Conclusion: DFDT通过恢复轨迹片段的可拼接性，有效解决了决策变换器在跨域迁移中的挑战，理论分析和实验验证了方法的有效性。
...
### [81] [FSampler: Training Free Acceleration of Diffusion Sampling via Epsilon Extrapolation](https://arxiv.org/abs/2511.09180)
*Michael A. Vladimir*

Main category: cs.LG

TL;DR: FSampler是一个无需训练、与采样器无关的执行层，通过减少函数评估次数来加速扩散采样。它使用有限差分预测器从历史去噪信号中预测下一个epsilon，在选定步骤用预测值替代模型调用，同时保持采样器更新规则不变。
...
Method: FSampler维护最近真实模型调用的去噪信号历史，使用二阶、三阶或四阶有限差分预测器外推下一个epsilon。在选定步骤用预测epsilon替代模型调用，包含学习稳定器和梯度估计稳定器来纠正漂移和补偿局部曲率。

Result: 在FLUX.1 dev、Qwen Image和Wan 2.2上，FSampler减少时间8-22%，模型调用减少15-25%，同时保持高保真度（SSIM 0.95-0.99）。使用激进自适应门控时，模型调用可减少45-50%，但保真度较低（SSIM 0.73-0.74）。

Conclusion: FSampler能够有效加速扩散采样过程，显著减少模型调用次数，同时保持采样质量，且与多种采样器兼容。
...
### [82] [Sure! Here's a short and concise title for your paper: "Contamination in Generated Text Detection Benchmarks"](https://arxiv.org/abs/2511.09200)
*Philipp Dingfelder,Christian Riess*

Main category: cs.LG

TL;DR: 该论文发现DetectRL基准数据集中存在AI生成文本的简单模式，这些模式可能被检测器用作捷径，从而容易受到欺骗攻击。作者通过数据清洗操作重新处理了该数据集，使直接攻击更加困难。


<details>
  <summary>Details</summary>
Motivation: 防止AI生成文本的非法使用需要有效的检测器，而检测器的训练和评估依赖于高质量的基准数据集。现有数据集如DetectRL存在质量问题，其中AI生成文本包含明显的模式，这些模式可能被检测器利用作为捷径，导致检测器容易受到欺骗攻击。

Method: 作者对DetectRL数据集进行了多种清洗操作，移除了AI生成文本中的简单模式，如开头语“Sure! Here is the academic article abstract:”和LLM拒绝执行任务的实例。

Result: 实验表明，经过数据清洗后，对训练出的检测器进行直接攻击变得更加困难。重新处理后的数据集已公开可用。

Conclusion: 数据清洗对于提高AI文本检测器的鲁棒性至关重要，能够减少检测器对简单模式的依赖，从而增强对欺骗攻击的抵抗力。
...
### [83] [Stochastic Mean-Shift Clustering](https://arxiv.org/abs/2511.09202)
...
<details>
  <summary>Details</summary>
Motivation: 改进传统均值漂移聚类算法的效率和性能，通过引入随机性来提升聚类效果。

Method: 使用随机选择的数据点序列，基于目标函数的部分梯度上升步骤进行移动。

Result: 在由高斯混合分布生成的合成2维样本上，随机均值漂移聚类在大多数情况下优于标准均值漂移算法，并在说话人聚类应用中验证了其有效性。

Conclusion: 随机均值漂移聚类算法在理论和实践中都表现出良好的收敛性和优越性能，是传统均值漂移算法的有效改进。
...
### [84] [Parameter-Free Clustering via Self-Supervised Consensus Maximization (Extended Version)](https://arxiv.org/abs/2511.09211)
...
<details>
  <summary>Details</summary>
Motivation: 现有聚类方法严重依赖超参数（如聚类数量），限制了在现实场景中的应用。需要解决这一长期存在的挑战，开发无需参数的聚类方法。

Method: 通过自监督共识最大化框架，在层次凝聚聚类过程中创建结构感知的数据表示，引入最近邻共识分数来衡量原始表示与自监督表示之间的一致性，以共识最大化时刻作为确定最优聚类数量的标准。

Result: 在多个数据集上的广泛实验表明，该框架在未知聚类数量场景下优于现有的聚类方法。

Conclusion: SCMax框架提供了一种有效的完全无参数聚类解决方案，能够自动确定最优聚类结构，在现实应用中具有重要价值。
...
### [85] [Controllable protein design through Feynman-Kac steering](https://arxiv.org/abs/2511.09216)
*Erik Hartman,Jonas Wallin,Johan Malmström,Jimmy Olsson*

Main category: cs.LG

TL;DR: 本文扩展了Feynman-Kac（FK）引导框架，将其应用于基于扩散的蛋白质设计，能够在推理时引导采样过程朝向特定的功能或生化目标，同时保持扩散过程的多样性。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的蛋白质结构生成模型虽然能产生真实多样的结构，但难以引导结果朝向特定的功能目标（如结合亲和力或序列组成）。需要一种能够在推理时控制生成过程的方法。

Method: 将FK引导框架与结构生成相结合，通过ProteinMPNN和全原子松弛精炼模型计算奖励函数，同时生成序列和结构特性。该方法在推理时引导采样过程。

Result: 应用于结合剂设计时，FK引导在不同目标上一致改善了预测的界面能量学，且计算开销最小。该方法能够处理任意、不可微分且与奖励无关的目标。

Conclusion: FK推理时控制将基于扩散的蛋白质设计推广到任意目标，为引导分子生成提供了一个统一且模型无关的框架。
...
### [86] [GuardFed: A Trustworthy Federated Learning Framework Against Dual-Facet Attacks](https://arxiv.org/abs/2511.09294)
*Yanli Li,Yanan Zhou,Zhongliang Guo,Nan Yang,Yuning Zhang,Huaming Chen,Dong Yuan,Weiping Ding,Witold Pedrycz*

Main category: cs.LG

TL;DR: 本文提出了双重攻击(DFA)威胁模型，同时破坏联邦学习的模型效用和群体公平性，并开发了GuardFed防御框架来有效应对这种攻击。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然能保护隐私，但仍易受攻击影响模型效用或公平性。现有研究主要关注单一目标攻击，同时破坏效用和公平性的攻击策略尚未充分探索。

Method: 提出了双重攻击(DFA)模型，包括同步DFA和分割DFA两种变体；开发了GuardFed防御框架，使用公平感知参考模型和双视角信任评分机制来选择性聚合客户端更新。

Result: 实验表明现有鲁棒联邦学习防御无法有效抵抗DFA攻击，而GuardFed在各种非独立同分布和对抗条件下能持续保持准确性和公平性，达到最先进性能。

Conclusion: DFA攻击揭示了联邦学习中同时威胁效用和公平性的新风险，GuardFed提供了一种有效的自适应防御方案，为构建更安全的联邦学习系统提供了重要保障。
...
### [87] [MARBLE: Multi-Armed Restless Bandits in Latent Markovian Environment](https://arxiv.org/abs/2511.09324)
*Mohsen Amiri,Konstantin Avrachenkov,Ibtihal El Mimouni,Sindri Magnússon*

Main category: cs.LG

TL;DR: 本文提出了MARBLE模型，将传统RMAB扩展为包含潜在马尔可夫状态的非平稳环境，并引入马尔可夫平均可索引性准则，证明了在MAI准则下同步Q学习与Whittle索引能够收敛到最优解。


<details>
  <summary>Details</summary>
Motivation: 传统RMAB假设固定动态，这在非平稳环境中往往不成立。为了解决在潜在马尔可夫环境中的决策问题，需要开发能够适应动态变化的模型和方法。

Method: 提出MARBLE模型，在RMAB基础上加入潜在马尔可夫状态；引入马尔可夫平均可索引性(MAI)准则；开发同步Q学习与Whittle索引(QWI)算法。

Result: 理论证明在MAI准则下QWI算法几乎必然收敛到最优Q函数和Whittle索引；在数字孪生推荐系统上的实验验证了算法能够适应潜在状态变化并收敛到最优策略。

Conclusion: MARBLE模型和QWI算法能够有效处理潜在马尔可夫环境中的非平稳RMAB问题，为动态环境下的决策提供了理论保证和实用方法。
...
### [88] [GAMMA_FLOW: Guided Analysis of Multi-label spectra by MAtrix Factorization for Lightweight Operational Workflows](https://arxiv.org/abs/2511.09326)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个快速、高效且适应性强的光谱分析工具，作为专有软件的开放灵活替代方案，降低计算成本。

Method: 采用监督式非负矩阵分解（NMF）进行降维，避免使用计算密集型的大型模型。

Result: 分类准确率超过90%，实现可靠的光谱自动解释，适用于任何类型的一维光谱数据。

Conclusion: GAMMA_FLOW是一个高效的开源光谱分析工具，在研究和工业应用中具有广泛适用性。
...
### [89] [Distribution-Based Feature Attribution for Explaining the Predictions of Any Classifier](https://arxiv.org/abs/2511.09332)
...
<details>
  <summary>Details</summary>
Motivation: 复杂黑盒AI模型的普及增加了对其决策进行解释的需求，但特征归因方法领域缺乏正式的问题定义。

Method: 提出了DFAX（分布特征归因解释），一种新颖的模型无关特征归因方法，直接基于数据分布解释分类器预测。

Result: 通过大量实验证明，DFAX比最先进的基线方法更有效和高效。

Conclusion: DFAX通过引入基于数据分布的特征归因方法，解决了现有方法在满足正式定义方面的不足，提供了更可靠的解释。
...
TL;DR: AdaCuRL是一个自适应课程强化学习框架，通过粗到细的难度估计和自适应课程调度来解决强化学习中的梯度饥饿和策略退化问题，在多种推理基准测试中显著提升大语言模型和多模态大语言模型的性能。
...
Result: 在多种推理基准测试上的广泛实验表明，AdaCuRL在大语言模型和多模态大语言模型上均实现了显著的性能提升。

Conclusion: AdaCuRL通过自适应课程强化学习有效解决了现有方法的局限性，为提升语言模型的推理能力提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: SHAP是解释决策树集成模型的重要工具，但现有计算方法效率较低，特别是在处理大规模数据集时。需要一种更高效的算法来支持实际应用中的大规模SHAP值计算。

Method: WOODELF为每个消费者构建一个伪布尔公式，捕捉其特征值、决策树集成结构和整个背景数据集。利用这种表示在线性时间内计算背景SHAP值，同时还能计算路径依赖SHAP、Shapley交互值、Banzhaf值等。

Result: 在包含300万行、500万背景样本和127个特征的数据集上，WOODELF在CPU上计算所有背景Shapley值仅需162秒，GPU上仅需16秒，相比现有最佳方法的44分钟，分别实现了16倍和165倍的加速。

Conclusion: WOODELF提供了一个高效、统一的框架来计算SHAP和其他博弈论值，支持大规模实际应用，并通过Python包形式提供，便于集成到现有框架中。
...
TL;DR: 使用条件扩散模型从稀疏采样的正弦图中插值缺失的线响应，为新型低成本、患者友好的PET几何结构提供解决方案


<details>
  <summary>Details</summary>
Motivation: PET成像需要支持无约束探测器布局的方法，从步行设计到长轴环，其中间隙和开放侧会导致严重欠采样的正弦图，而不是约束硬件形成完整圆柱体

Method: 将缺失的线响应视为可学习先验，探索使用条件扩散模型来插值稀疏采样的正弦图

Result: 为新型、成本效益高且患者友好的PET几何结构在真实临床环境中铺平道路
...
<details>
  <summary>Details</summary>
Motivation: 现有序列推荐系统容易受到对抗攻击，但传统数据投毒攻击需要大规模用户访问或虚假配置文件，缺乏实用性。配置文件污染攻击(PPA)通过污染部分用户交互来诱导目标误预测，但现有方法存在对序列水平影响的过度依赖和整体修改导致可检测分布偏移的问题。

Method: 提出CREAT方法：1) 模式平衡奖励策略，集成模式反转奖励和分布一致性奖励；2) 约束组相对强化学习范式，通过动态屏障约束和组共享经验回放实现逐步扰动。采用双级优化框架和多奖励强化学习来平衡攻击效果与隐蔽性。

Result: 大量实验证明了CREAT方法的有效性，能够在保持最小可检测性的同时实现目标污染。

Conclusion: CREAT方法通过约束强化学习框架成功解决了配置文件污染攻击中的细粒度扰动和隐蔽性问题，为序列推荐系统的对抗攻击提供了更实用的解决方案。
...
Conclusion: AGT填补了训练数据扰动认证研究的空白，为对抗数据投毒、机器遗忘和差分隐私等场景提供了理论基础和实用工具。
...
TL;DR: CallosumNet是一个受胼胝体启发的时空图遗忘框架，通过增强子图构造和全局神经节桥接技术，实现高效完整的节点遗忘，仅产生1%-2%的相对MAE损失。


<details>
  <summary>Details</summary>
Motivation: GDPR和CCPA等隐私法规要求对未授权数据进行完全遗忘，但现有方法在时空图中无法高效擦除单个节点，成本接近完全重新训练。

Method: 采用分治策略，包含增强子图构造（基于生物启发虚拟神经节）和全局神经节桥接（重建全局时空依赖关系）。

Result: 在四个真实数据集上的实验显示，CallosumNet仅产生1%-2%相对MAE损失，显著优于现有基线方法。

Conclusion: CallosumNet有效解决了时空图完全遗忘问题，验证了所提技术的有效性。
...
Conclusion: PTE框架实现了仅使用遗忘数据和原始模型的高效平衡知识编辑，为动态工业物联网环境中的选择性知识遗忘提供了有效解决方案。
...
TL;DR: TSGP是一种使用预训练transformer模型作为变异算子的语义搜索方法，能够生成与父代程序具有可控语义相似度的子代程序，在24个数据集上显著优于多种基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统语义遗传编程依赖固定的语法变换，无法学习导致相似语义的多样化结构变化，因此需要一种能够学习语义相似性结构变异的更灵活方法。

Method: 使用预训练的transformer模型作为变异算子，通过控制目标语义距离SD_t来生成语义相似的子代程序，实现语义空间中的可控步长。

Result: 在24个真实世界和合成数据集上，TSGP显著优于标准GP、SLIM_GSGP、深度符号回归和去噪自编码器GP，平均排名为1.58，且比SLIM_GSGP产生更紧凑的解。

Conclusion: TSGP能够泛化到不同维度的符号回归问题，SD_t参数有效平衡了探索与利用，小SD_t值带来持续适应度提升但程序较大，大SD_t值促进快速收敛和紧凑性。
...
### [98] [LLM-Guided Dynamic-UMAP for Personalized Federated Graph Learning](https://arxiv.org/abs/2511.09438)
*Sai Puppala,Ismail Hossain,Md Jahangir Alam,Tanzim Ahad,Sajedul Talukder*

Main category: cs.LG

TL;DR: 提出一种利用大语言模型在个性化和隐私约束下辅助图机器学习的方法，结合数据增强、提示调优和上下文学习，通过贝叶斯变分目标实现个性化联邦学习。
...
Method: 结合稀疏图数据增强、提示和指令调优使基础模型适应图任务，使用上下文学习提供少样本图推理信号，通过动态UMAP流形参数化客户端特定图嵌入，在贝叶斯变分目标中实现个性化联邦学习。

Result: 方法支持节点分类和链接预测任务，通过跨模态正则化器对齐语言模型潜在表示与图结构，提供了变分聚合过程的收敛性论证和基于时刻会计师的差分隐私威胁模型。

Conclusion: 该方法在知识图谱补全、推荐式链接预测、引文和产品图等应用中有效，并讨论了LLM辅助图机器学习的基准评估考虑。
...
TL;DR: 本研究评估了统计、机器学习和深度学习模型在交通速度和流量预测中的表现，使用加州Harbor Freeway的真实数据。结果显示ANFIS-GP在早期预测窗口表现最佳，而Bi-LSTM在中期预测中更稳健，能够建模长期时间依赖性。


<details>
  <summary>Details</summary>
Motivation: 随着快速城市化，交通拥堵加剧，智能交通系统(ITS)变得至关重要。交通预测是ITS的核心功能，能够实现主动交通管理措施。

Method: 使用来自加州交通绩效测量系统(PeMS)的真实数据，评估统计、机器学习和深度学习模型在20个预测窗口（最长1小时40分钟）的性能，使用RMSE、MAE和R-Square指标。

Result: ANFIS-GP在早期窗口表现最佳（RMSE 0.038，MAE 0.0276，R-Square 0.9983），Bi-LSTM在中期预测中更稳健（RMSE 0.1863，MAE 0.0833，R-Square 0.987）。Bi-LSTM具有最平缓的斜率（RMSE 0.0454，MAE 0.0545），表明其鲁棒性最强。
...
<details>
  <summary>Details</summary>
Motivation: 现有太阳高能粒子事件预测方法多为黑盒模型，难以解释预测结果和物理机制，限制了太阳物理学家的理解。

Method: 开发了集成全局解释和特设特征映射的框架，使用341个SEP事件数据集（包括244个重要质子事件）进行验证，涵盖太阳周期22、23和24。

Result: 通过主要SEP事件的可解释性案例研究，证明了该方法提高了模型可解释性，促进了基于物理的SEP事件预测理解。

Conclusion: 该框架成功增强了太阳高能粒子事件预测模型的透明度，为理解预测决策过程提供了更深入的物理见解。
...
TL;DR: SOLIS是一种在评估对齐嵌入空间中直接进行规划的方法，通过监督对比学习创建表示空间，将规划简化为潜在空间中的向量操作，在象棋中仅使用浅层搜索就达到竞争水平。
...
Method: 使用监督对比学习创建评估对齐嵌入空间，其中结果相似性通过邻近度捕获，单个全局优势向量从输到赢区域定向空间，通过向量对齐对候选动作进行排序。

Result: 在象棋中，SOLIS仅使用学习嵌入引导的浅层搜索就能在受限条件下达到竞争强度。

Conclusion: 评估对齐潜在规划为传统动态模型或策略学习提供了轻量级替代方案，证明了在潜在空间中直接进行规划的可行性。
...
TL;DR: 本文提出了一种基于概率密度感知的核心集选择方法PDAC和其流式版本SPDAC，通过理论分析和实验验证高概率密度样本在误差抑制中的主导作用，实现了更高效的持续学习记忆缓冲区构建。


<details>
  <summary>Details</summary>
Motivation: 现有基于排练的持续学习方法依赖核心集选择来构建记忆缓冲区，但主流方法通常采用双层优化问题，计算成本高昂，限制了实际效率。本文旨在提供更高效的核心集构建选择逻辑和方案。

Method: 首先通过局部误差分解分析缓冲区训练模型与贝叶斯最优模型之间的均方误差，发现高概率密度样本在误差抑制中起主导作用。提出PDAC方法，利用投影高斯混合模型估计样本联合密度，实现基于密度的优先级缓冲区选择。针对流式场景，引入流式期望最大化算法，提出SPDAC方法。

Result: 广泛的对比实验表明，该方法在各种持续学习设置下优于其他基线方法，同时保证了良好的效率。

Conclusion: 基于概率密度感知的核心集选择方法能够有效提升持续学习的性能，同时保持计算效率，为实际应用提供了可行的解决方案。
...
TL;DR: AutoSynth是一个无需参考数据集的自动化工作流发现和优化框架，通过蒙特卡洛树搜索和新型无数据集混合奖励来解决冷启动问题，显著减少了人工工作量。


<details>
  <summary>Details</summary>
Motivation: 监督微调LLM需要高质量数据集，但人工标注成本高昂。现有自动化方法存在冷启动问题，需要标注数据集进行奖励建模，这在主观开放任务中尤其困难。

Method: 将问题重构为蒙特卡洛树搜索，使用新型无数据集混合奖励指导搜索。该奖励通过两个LLM作为评判员组件实现元学习：一个使用动态生成的任务特定指标评估样本质量，另一个评估工作流代码和提示质量。

Result: 在主观教育任务实验中，AutoSynth生成的数据训练的模型显著优于基线（40-51% vs 2-5%），在某些指标上匹配或超越专家设计的工作流，同时将人工工作量从5-7小时减少到30分钟（减少90%以上）。

Conclusion: AutoSynth解决了数据中心AI中的冷启动问题，为主观LLM任务提供了可扩展且成本效益高的方法，能够发现超出人类直觉的质量维度。
...
TL;DR: 提出一种二阶确定性actor-critic强化学习框架，利用性能函数的曲率信息，通过二次critic和准牛顿actor更新实现比一阶方法更快的收敛。


<details>
  <summary>Details</summary>
Motivation: 扩展经典的确定性策略梯度方法，利用性能函数的二阶曲率信息来加速收敛，克服一阶方法收敛速度较慢的问题。

Method: 基于兼容函数逼近概念，引入同时保持真实策略梯度和性能Hessian近似的二次critic，开发最小二乘时序差分学习方案高效估计参数，实现准牛顿actor更新。

Result: 数值实验表明，该方法相比标准确定性actor-critic基线实现了改进的收敛性和性能表现。

Conclusion: 所提出的二阶确定性actor-critic框架能够有效利用曲率信息，在可微分策略类中实现更快的收敛速度和更好的性能。
...
TL;DR: GenePheno是首个可解释的多标签预测框架，直接从基因序列预测基因敲除诱导的表型异常，通过对比学习和生物学一致性正则化实现多表型关联建模。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注特定基因变异对有限表型的影响，或依赖人工整理的遗传信息，难以直接从基因序列广泛预测多种表型异常。基因序列与表型之间的模态差距和基因多效性使该任务具有挑战性。
...
Conclusion: GenePheno为直接从基因序列预测基因敲除诱导的多表型异常提供了首个可解释框架，在准确性和可解释性方面均表现出色。
...
TL;DR: 该论文提出了一种使用LLM和AMR图将无约束英语翻译成ASP程序的新方法，用于解决逻辑谜题，生成完整的ASP程序来解决问题。
...
Result: 系统成功创建了完整的ASP程序，能够解决组合逻辑问题，展示了该方法的能力。
...
TL;DR: 提出了一种基于向量符号代数的认知合理性ARC-AGI求解器，结合系统1直觉和系统2推理，在ARC-AGI基准测试中取得初步成功。


<details>
  <summary>Details</summary>
Motivation: 尽管人类能轻松解决ARC-AGI问题，但即使最先进的AI系统也难以应对。受神经科学和心理学中人类智能建模方法的启发，旨在开发认知合理的求解器。

Method: 使用基于向量符号代数的神经符号方法，集成系统1直觉和系统2推理，通过面向对象的程序合成，利用VSA表示抽象对象、指导解决方案搜索并实现样本高效的神经学习。

Result: 在ARC-AGI-1-Train上得分10.8%，在ARC-AGI-1-Eval上得分3.0%；在更简单的基准测试中表现优异，Sort-of-ARC得分94.5%，1D-ARC得分83.1%（优于GPT-4且计算成本极低）。

Conclusion: 这是首个将VSA应用于ARC-AGI的方法，开发了迄今为止最具认知合理性的ARC-AGI求解器，为人工智能通用智能研究提供了新思路。
...
### [108] [UCO: A Multi-Turn Interactive Reinforcement Learning Method for Adaptive Teaching with Large Language Models](https://arxiv.org/abs/2511.08873)
*Shouang Wei,Min Zhang,Xin Lin,Bo Jiang,Kun Kuang,Zhongxiang Dai*

Main category: cs.AI

TL;DR: 本文提出单向认知优化（UCO）方法，通过多轮交互式强化学习解决LLM作为智能导师时的动态适应问题，包含两个协同奖励函数来评估学生认知进步和识别最近发展区。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在教学中仅学习表面教学模式，缺乏动态适应能力；现有强化学习方法无法区分学生是否真正理解，也无法感知学生实时认知状态。

Method: UCO方法采用多轮交互式强化学习范式，包含两个协同奖励函数：进步奖励捕捉学生认知进步，支架奖励动态识别学生的最近发展区。

Result: 在BigMath和MathTutorBench基准测试中，UCO模型优于所有同等规模模型，性能与先进闭源模型相当。

Conclusion: UCO方法有效解决了LLM作为智能导师时的动态适应问题，通过认知进步评估和最近发展区识别提升了教学效果。
...
### [109] [Lumine: An Open Recipe for Building Generalist Agents in 3D Open Worlds](https://arxiv.org/abs/2511.08892)
...
TL;DR: Lumine是第一个能够在3D开放世界环境中实时完成数小时复杂任务的通用智能体，采用端到端的视觉语言模型统一感知、推理和行动，在Genshin Impact中训练后能完成5小时主线任务，并具备零样本跨游戏泛化能力。
...
Method: 采用人类交互范式，通过视觉语言模型以5Hz处理原始像素，生成30Hz的键盘鼠标动作，仅在必要时进行推理。在Genshin Impact中进行训练。

Result: 成功完成Genshin Impact中5小时的蒙德主线任务，达到人类效率水平；在Wuthering Waves中完成100分钟任务，在Honkai: Star Rail中完成5小时第一章任务，均无需微调。

Conclusion: Lumine在不同世界和交互动态中表现出色，标志着在开放环境中开发通用智能体迈出了具体一步。
...
### [110] [The Double Contingency Problem: AI Recursion and the Limits of Interspecies Understanding](https://arxiv.org/abs/2511.08927)
*Graham L. Bishop*

Main category: cs.AI

TL;DR: 本文探讨了生物声学AI系统在处理动物通信时存在的递归认知冲突问题，认为AI不是中立的模式检测器而是递归认知主体，其信息处理可能系统性地遮蔽或扭曲其他物种的通信结构。


<details>
  <summary>Details</summary>
Motivation: 当前生物声学AI系统通过transformer架构、基础模型范式等方法实现了跨物种的出色性能，但这些方法忽略了一个基本问题：当一种递归认知（AI系统）遇到其他物种的递归通信过程时会发生什么。

Method: 基于哲学家Yuk Hui关于递归性和偶然性的研究，分析AI系统作为递归认知主体的特性，提出双重偶然性问题：物种通信源于生态和进化条件，而AI系统通过其架构和训练条件处理这些信号。

Result: 揭示了AI系统可能系统性地遮蔽或扭曲其他物种通信结构的问题，提出了从通用模式识别向不同递归认知形式间外交性相遇的重新概念化方向。
...
### [111] [A Research on Business Process Optimisation Model Integrating AI and Big Data Analytics](https://arxiv.org/abs/2511.08934)
...
<details>
  <summary>Details</summary>
Motivation: 随着数字化转型深化，业务流程优化成为提升企业竞争力的关键，需要实现流程全生命周期的智能管理。
...
### [112] [AlphaCast: A Human Wisdom-LLM Intelligence Co-Reasoning Framework for Interactive Time Series Forecasting](https://arxiv.org/abs/2511.08947)
...
Method: 采用两阶段框架：1)自动化预测准备，构建多源认知基础；2)生成推理和反思优化，集成统计特征、先验知识、上下文信息和预测策略，触发元推理循环进行持续自校正。

Result: 在短期和长期数据集上的广泛实验表明，AlphaCast在预测准确性方面始终优于最先进的基线方法。

Conclusion: AlphaCast通过人机协同推理框架显著提升了时间序列预测的准确性和实用性，为复杂环境下的预测任务提供了有效解决方案。
...
TL;DR: 该论文提出通过构建多智能体管道来实现渐进式、增量式、顺序式(GIS)搜索空间遍历，从而增强大语言模型的推理能力，并通过递归精炼方法验证了这一框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型表现出卓越的流畅性，但研究者仍在努力从中提取更强的推理能力。基于搜索视角的大语言模型计算解释，本文旨在系统理解大语言模型推理和优化的框架。

Method: 采用递归精炼方法，包括自我批评、对抗性压力测试和整合关键反馈的迭代过程，作为实现GIS搜索的实用方法。设计了简单线性管道与复杂结构化管道的对比实验，构建了反映美国开国元勋历史人物的多智能体模型。

Result: 复杂模型在所有九个测试案例中始终优于简单模型，平均仲裁得分为88.3对71.7。复杂模型的论证在分析深度、结构细微差别和战略框架方面更优越。

Conclusion: 递归精炼是通过GIS搜索增强大语言模型推理能力的强大架构特征。
...
TL;DR: 提出Argus框架，通过持续监控轨迹和危险缓解器来增强端到端自动驾驶系统的韧性，防止安全违规并提升驾驶性能
...
Method: 开发运行时韧性导向框架Argus，持续监控ADS生成的轨迹，当车辆被认为不安全时通过危险缓解器无缝接管控制

Result: 与TCP、UniAD和VAD三个先进ADS集成，驾驶评分平均提升150.30%，防止64.38%的违规行为，时间开销很小
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆有望通过更快、更安全、更高效的响应来革新应急服务，但传统强化学习方法在动态应急场景中存在样本效率低和适应性差的问题，需要新的优化策略来解决这些限制。

Method: 分析了从传统强化学习到扩散模型增强强化学习的转变（通过合成数据生成增强策略鲁棒性），以及新兴的大型语言模型辅助上下文学习范式（提供轻量级、可解释的替代方案，实现无需重新训练的快速适应）。

Result: 通过综述自动驾驶智能、扩散模型增强强化学习和大型语言模型辅助上下文学习的最新技术，建立了一个理解下一代自主应急响应系统的关键框架。

Conclusion: 从生成式AI的视角，本文为理解下一代自主应急响应系统提供了重要框架，展示了扩散模型增强强化学习和大型语言模型辅助上下文学习在解决传统强化学习局限性方面的潜力。
...
Result: OR-R1达到67.7%的平均求解准确率，仅需ORLM方法1/10的合成数据，准确率提升达4.2%。TGRPO带来额外3.1%-6.4%的准确率提升，将单次与多次尝试性能差距从13%缩小到7%。

Conclusion: OR-R1为自动化运筹优化问题建模和求解提供了稳健、可扩展且成本效益高的解决方案，降低了工业应用的专家知识和数据壁垒。
...
TL;DR: 提出了一种训练简洁性奖励模型（CRM）的流程，通过新颖的简洁性奖励函数（CRF）解决大型推理模型中的过度思考问题，在提高推理效率的同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（如DeepSeek-R1和OpenAI o1）经常生成冗长的响应，包含冗余或无关的推理步骤（过度思考现象），这显著增加了计算成本。现有的长度惩罚方法存在长度崩溃和训练崩溃问题。

Method: 提出训练简洁性奖励模型（CRM）的流程来评估推理路径的简洁性，并引入具有明确依赖关系的简洁性奖励函数（CRF），将结果奖励与简洁性评分相结合。

Result: 在五个数学基准数据集上的实验显示，该方法在Qwen2.5-7B上实现了8.1%的准确率提升和19.9%的响应token长度减少，并且在Llama和Mistral等其他LLM上具有良好的泛化能力。
...
TL;DR: 本文提出了一个双层可靠性监控框架，用于解决智能AI系统在运行中的可靠性不足问题，包括分布外检测层和AI透明度层，为人类操作员提供决策支持。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统在医疗和流程工业等高风险领域应用时，由于可靠性不足可能产生意外行为，需要开发缓解技术来降低运行中的不确定性风险。

Method: 提出双层可靠性监控框架：第一层是分布外检测，识别新颖输入；第二层是AI透明度层，揭示内部操作。该框架为人类操作员提供决策支持以判断输出是否可靠。

Result: 该监控框架能够帮助人类操作员识别潜在不可靠的输出并进行干预，为开发缓解技术提供了基础。

Conclusion: 双层可靠性监控框架是降低智能AI系统运行风险的有效方法，通过分布外检测和透明度机制增强了系统的可靠性和可控性。
...
TL;DR: MedFuse是一个用于不规则临床时间序列的框架，通过MuFuse模块使用乘法融合来建模特征间的高阶依赖关系，在多个真实数据集上优于现有方法。
...
Conclusion: MedFuse为建模不规则临床时间序列提供了一个可泛化的方法，乘法融合机制能有效捕捉复杂的特征交互关系。
...
TL;DR: 提出了HyperD框架，通过将交通数据解耦为周期性和残差分量来改善交通预测，解决了复杂空间依赖和多尺度周期模式与不规则波动的挑战。
...
Method: HyperD框架将交通数据解耦为周期性和残差分量。周期性分量使用混合周期表示模块处理，残差分量使用频率感知残差表示模块建模，并引入双视图对齐损失来强制语义分离。

Result: 在四个真实世界交通数据集上的实验表明，HyperD实现了最先进的预测精度，同时在干扰下具有更好的鲁棒性和更高的计算效率。

Conclusion: HyperD通过解耦周期性和非周期性模式，有效提升了交通预测性能，为智能交通系统提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI训练方法只在核心能力建立后才进行价值观对齐，导致模型容易被误导且缺乏深层次价值体系。在大模型能力超越人类的背景下，需要从根本上改变训练方式

Method: 重新设计训练语料库：采用第一人称视角重构训练数据、将信息重新语境化为生活经验、模拟社会互动、搭建训练数据排序的脚手架

Result: 预期从第一个训练token开始就建立价值观承诺，使知识、技能和价值观内在融合，难以分离

Conclusion: 在大模型能力超越人类的生态系统中，这种从训练转向培养的范式转变是至关重要的需求
...
Result: 发现定性AI系统需要具备透明性、可复现性和隐私友好性，以支持解释性研究。

Conclusion: 需要从零开始构建专门用于定性研究的AI系统，以弥补当前AI在定量方法和定性维度整合方面的关键差距，促进更全面的科学理解。
...
TL;DR: 评估了2025年三个前沿LLM（DeepSeek R1、Gemini 2.5 Pro、GPT-5）在PDDL规划和推理任务上的表现，发现GPT-5在标准PDDL领域与LAMA规划器竞争，但在混淆测试中所有模型性能下降。


<details>
  <summary>Details</summary>
Motivation: 随着前沿大语言模型推理能力的不断发展，需要评估它们在端到端规划任务中的最新表现，并与传统规划器进行比较。

Method: 使用国际规划竞赛学习赛道中的PDDL领域和任务子集，评估三个前沿LLM的规划性能，并与LAMA规划器作为参考基准进行比较，包括标准PDDL和混淆PDDL两种测试条件。

Result: 在标准PDDL领域，GPT-5在解决任务数量上与LAMA规划器具有竞争力；当PDDL领域和任务被混淆以测试纯推理能力时，所有LLM的性能都有所下降，但下降程度比之前报告的其他模型要小。

Conclusion: 前沿LLM在规划任务上相比前代模型有显著改进，在具有挑战性的基准测试中缩小了与规划器的性能差距。
...
TL;DR: 提出了一种基于潜在流匹配和分类器自由引导的新方法，通过明确分离条件信息和残差表示来解缠潜在子空间，实现了对高维数据有意义特征的访问。


<details>
  <summary>Details</summary>
Motivation: 在科学发现中访问学习表示中的信息对于高维领域至关重要，但现有方法在解缠潜在子空间方面存在局限。

Method: 使用潜在流匹配与分类器自由引导相结合的方法，明确分离条件信息和残差表示，从而解缠潜在子空间。
...
TL;DR: CrochetBench是一个评估多模态大语言模型在钩针编织领域进行细粒度、低层次程序推理能力的基准测试，重点关注从描述到执行的转变，通过结构验证和功能执行来评估模型性能。
...
Method: 使用CrochetPARADE DSL作为中间表示，支持结构验证和通过执行进行功能评估。基准测试涵盖针法分类、指令基础、自然语言到DSL翻译和图像到DSL翻译等任务。

Result: 在所有任务中，当评估从表面相似性转向可执行正确性时，模型性能急剧下降，暴露了长距离符号推理和3D感知程序合成的局限性。

Conclusion: CrochetBench为评估多模态模型的程序能力提供了新视角，突显了表面理解与可执行精度在真实创造性领域之间的差距。
...
TL;DR: 提出了一种基于多模型聚合的AI安全方法，通过共识采样算法从k个模型中选出最安全的s个模型，在模型间达成足够共识时输出结果，否则弃权，从而提升整体安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的AI安全方法主要依赖检查模型输出或激活状态，但某些风险仅通过检查无法发现。需要一种架构无关的补充方法来增强安全性。

Method: 使用共识采样算法，利用多个生成模型的输出概率计算，当足够多的安全模型在输出上达成共识时输出结果，否则弃权。该方法受Vyas等人(2023)的可证明版权保护算法启发。

Result: 该方法能够实现与最安全的s个模型平均风险相当的风险水平，同时当模型间缺乏足够共识时选择弃权，避免不安全输出。

Conclusion: 提供了一种新的模型无关的AI安全方法，通过从模型集合中的未知安全子集放大安全保证，构建单个可靠模型。
...
TL;DR: 本文从科学和系统角度阐述了物理人工智能（Physical AI）的基本原理，旨在为智能系统的物理体现、感知、行动、学习和情境敏感性创建统一的理论框架，强调智能是身体、环境和经验真实互动的涌现现象。


<details>
  <summary>Details</summary>
Motivation: 传统AI依赖符号处理和数据分析模型，而Physical AI将智能理解为身体、环境和经验之间真实互动产生的涌现现象，需要建立新的理论框架来描述物理智能系统的本质特征。

Method: 提出了六个基本原理：具身化、感官感知、运动行动、学习、自主性和情境敏感性，这些原理构成一个封闭的控制循环，其中能量、信息、控制和情境持续互动。

Result: 建立了物理AI的理论基础，展示了六个原理如何作为封闭控制循环运作，使系统能够从物理经验而非数据库中生成意义，实现了智能概念的范式转变。

Conclusion: 物理AI将学习理解为智能体与环境之间结构耦合的变化，而非参数调整，通过康复机器人案例证明物理智能源于具身体验而非抽象计算，六个基本原理在真实系统中协同作用。
