...
Motivation: 尽管大语言模型在各种任务中表现出色，但其内部机制仍然不透明。本文旨在开发一种简单有效的方法来识别编码特定技能的神经元，以增强模型的可解释性。

Method: 基于先前通过软提示训练识别"技能神经元"的工作，将分析扩展到涉及多种技能的复杂场景。通过将神经元激活与外部标签和模型置信度等辅助指标相关联，无需手动标记聚合即可发现可解释的任务特定行为。

Result: 在开放式文本生成和自然语言推理任务上进行了实证验证，证明该方法能够检测到不仅驱动已知技能，还能揭示BigBench算术推理中先前未识别的捷径的神经元。

Conclusion: 该方法简单、轻量且广泛适用，能够有效识别大语言模型中编码特定技能的神经元，增强模型的可解释性，并发现模型推理过程中的捷径行为。
...
TL;DR: 本文研究了在大型语言模型预训练中整合多种元数据的方法，发现细粒度的文档质量指标等元数据能加速训练，并提出了元数据追加和可学习元标记等新方法。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅关注URL这一种元数据信号，但其他形式的元数据可能带来更大收益，因此需要探索更广泛的元数据类型及其在预训练中的价值。

Method: 研究多种元数据类型，引入元数据追加作为辅助任务，使用可学习元标记配合掩码损失训练，并通过探针分析潜在表示。

Result: 发现细粒度信息编码的元数据能有效加速预训练，元数据追加和可学习元标记都能提高训练效率，元数据能塑造质量感知的潜在结构。
...
TL;DR: 研究探讨捷克语AI生成诗歌与人类创作诗歌的感知差异，发现参与者难以区分两者，且存在作者偏见影响审美评价。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注英语AI诗歌，而捷克语作为形态复杂、资源较少的斯拉夫语言，其AI诗歌的感知情况尚不明确。

Method: 通过让捷克母语者识别和评价AI与人类创作的诗歌，使用逻辑回归分析识别准确性与审美评价的关系。

Result: 参与者识别准确率仅45.8%，处于随机水平；审美评价显示强烈作者偏见，认为AI诗歌质量较差，但实际AI诗歌评分相当或更高。

Conclusion: AI能在捷克语等复杂低资源语言中生成令人信服的诗歌，读者对作者身份的信念与诗歌审美评价密切相关。
...
TL;DR: ToolOrchestra方法训练小型编排器来协调智能工具，通过强化学习优化结果、效率和用户偏好，产生的8B模型Orchestrator在Humanity's Last Exam等任务上超越GPT-5性能且成本更低。


<details>
  <summary>Details</summary>
Motivation: 解决深度复杂问题（如Humanity's Last Exam）对大型语言模型来说既概念挑战又计算昂贵，需要更高效智能的解决方案。

Method: ToolOrchestra方法训练小型编排器管理其他模型和各种工具，使用强化学习结合结果、效率和用户偏好感知的奖励机制。

Result: Orchestrator在HLE上获得37.1%得分，超越GPT-5（35.1%）且效率提高2.5倍；在tau2-Bench和FRAMES上大幅超越GPT-5，仅使用约30%成本。
...
TL;DR: 本文通过系统评估大语言模型在不同难度任务上的泛化能力，发现跨难度泛化有限，训练数据难度单一无法在所有难度上获得一致改进。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在不同难度任务上的泛化能力，解决现有研究中关于训练数据难度选择（容易或困难）对模型性能影响的不一致结论。

Method: 使用数千个不同LLM的输出和项目反应理论（IRT）对六个数据集中的示例进行难度排序，构建仅基于LLM能力的客观难度评级，进行大规模细粒度分析。

Result: 跨难度泛化通常有限，无论是训练容易数据还是困难数据，都无法在整个难度范围内实现一致的性能改进。

Conclusion: 在LLM的训练和评估数据中包含不同难度范围非常重要，在难度方面走捷径存在风险。
...
TL;DR: 论文通过实验发现，算法效率提升很大程度上依赖于计算规模，特别是LSTM到Transformer的转换贡献了主要效率增益，而小规模模型的算法进展比之前假设的要慢得多。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解释2012-2023年间AI训练FLOP效率提升22,000倍的现象，发现传统小规模消融实验只能解释不到100倍的增益，存在显著差距。

Method: 进行扩展实验，比较LSTM和Transformer在不同规模下的计算最优扩展规律，并使用实验外推和文献估计方法。

Result: 发现算法效率增益与计算规模密切相关，LSTM到Transformer的转换贡献了主要效率提升，总共解释了6,930倍的效率增益。

Conclusion: 算法效率的衡量具有强烈的参考依赖性，小规模模型的算法进展比预期慢得多，效率增益与计算规模紧密相关。
...
TL;DR: 研究发现深度视觉变换器(ViT)存在Cliff-Plateau-Climb三阶段模式，[CLS]令牌的作用逐渐被补丁令牌的分布式共识取代。通过信息扰乱指数分析，发现ViT-L比ViT-B多出的层主要增加信息扩散而非提升性能，建议精心校准深度比单纯增加参数更重要。


<details>
  <summary>Details</summary>
Motivation: 深度视觉变换器往往表现不如浅层模型，这挑战了常见的扩展假设。研究旨在通过系统实证分析理解ViT中表示随深度演化的模式。

Method: 对ViT-S、ViT-B和ViT-L在ImageNet上进行系统实证分析，使用信息扰乱指数量化信息混合模式，分析[CLS]令牌与补丁令牌的作用变化。

Result: 识别出Cliff-Plateau-Climb三阶段演化模式，发现更好的性能与[CLS]令牌的逐步边缘化相关，ViT-L比ViT-B多出的10层主要增加信息扩散而非提升任务性能。

Conclusion: 变换器架构在这种机制下可能更受益于精心校准的深度执行清晰的阶段转换，而不是简单地增加参数数量。信息扰乱指数为现有模型提供了有用的诊断工具，并为未来架构设计提供了潜在目标。
...
### [8] [Escaping the Verifier: Learning to Reason via Demonstrations](https://arxiv.org/abs/2511.21667)
*Locke Cai,Ivan Provilkov*

Main category: cs.LG

TL;DR: RARO通过对抗性逆强化学习从专家演示中学习推理能力，无需任务特定验证器，在多个推理任务上显著优于基线方法。
...
Method: 建立策略（生成器）和相对主义批评器（判别器）之间的对抗交互：策略学习模仿专家答案，批评器学习比较和区分策略与专家答案，通过强化学习联合训练。

Result: 在Countdown、DeepMath和Poetry Writing等评估任务上显著优于无验证器基线方法，展现出与可验证任务上RL相同的稳健扩展趋势。
...
### [9] [Visualizing LLM Latent Space Geometry Through Dimensionality Reduction](https://arxiv.org/abs/2511.21594)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言任务中表现出色，但其内部机制难以解释。本文旨在通过降维方法系统分析Transformer模型的内部状态几何结构，支持可复现的可解释性研究。

Method: 在Transformer块中的多个点捕获逐层激活，通过主成分分析（PCA）和均匀流形逼近（UMAP）进行系统分析，在GPT-2和LLaMa模型上进行实验。

Result: 发现了注意力机制和MLP组件输出在中间层的明显分离，识别了初始序列位置潜在状态的高范数，可视化了潜在状态的逐层演化，展示了GPT-2位置嵌入的高维螺旋结构和LLaMa的序列级几何模式。

Conclusion: 该方法支持对Transformer内部结构的系统分析，有助于推进可复现的可解释性研究，相关代码已开源。
...
### [10] [Through the telecom lens: Are all training samples important?](https://arxiv.org/abs/2511.21668)
...
<details>
  <summary>Details</summary>
Motivation: 电信AI应用中数据量大、噪声多、标注成本高，但标准工作流假设所有训练样本贡献相等。下一代系统需要准确、高效且可持续的AI模型，因此需要重新评估样本重要性假设。

Method: 执行跨周期的样本级梯度分析以识别模型学习中的影响和冗余模式，基于此提出样本重要性框架，选择性优先处理有影响力的数据并减少计算。

Result: 在三个真实世界电信数据集上的实验表明，该方法在保持性能的同时减少了数据需求和计算开销。

Conclusion: 该方法通过优化样本选择策略，在不影响准确性的前提下实现了计算和能源使用的优化，推动了电信领域可持续AI的发展目标。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多基准测试中表现优异，但其规划和状态推理能力仍不明确，本研究旨在直接评估这些能力。
...
Conclusion: 当前大语言模型在规划方面存在显著局限性，需要开发维护显式状态和执行结构化搜索的机制。
...
TL;DR: 本文提出了一个将系统动力学和结构方程建模整合到统一数学框架中的方法，用于支持负责任AI/ML的发展。
...
Method: 将系统动力学和结构方程建模结合到一个共同的数学框架中，用于从分布生成系统、开发方法并比较结果。
...
TL;DR: ViLoMem是一个双流记忆框架，通过分别编码视觉分心模式和逻辑推理错误，使MLLMs能够从成功和失败经验中学习，在六个多模态基准测试中显著提高准确率并减少重复错误。


<details>
  <summary>Details</summary>
Motivation: 现有基于轨迹的记忆方法存在简洁性偏差，逐渐丢失重要领域知识，且仅记录单模态行为轨迹，无法保留视觉注意力和逻辑推理如何共同促成解决方案，这与人类多模态整合的语义记忆不匹配。

Method: 引入ViLoMem双流记忆框架，分别编码视觉分心模式和逻辑推理错误，采用增长-精炼原则逐步积累和更新多模态语义知识，保留稳定可泛化策略同时避免灾难性遗忘。

Result: 在六个多模态基准测试中，ViLoMem持续提高pass@1准确率，显著减少重复的视觉和逻辑错误，消融实验证实了具有明确分心-幻觉分离的双流记忆的必要性。

Conclusion: 错误感知的多模态记忆对于终身和跨领域智能体学习具有重要价值，ViLoMem通过双流记忆设计有效解决了现有记忆方法的局限性。
