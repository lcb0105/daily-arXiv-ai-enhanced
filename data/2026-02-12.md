...
TL;DR: 提出一个LLM驱动的框架，通过分解审稿意见、神经符号模块检测问题、遗传算法优化模板生成针对性反馈，显著提升审稿质量


<details>
  <summary>Details</summary>
Motivation: 同行评审对科学质量至关重要，但依赖简单启发式（懒惰思维）降低了标准。现有研究将懒惰思维检测视为单标签任务，但审稿片段可能同时存在多个问题，包括清晰度和具体性问题。目前缺乏基于指导原则的可操作反馈
...
TL;DR: LT-Tuning提出了一种新的潜在思维调整框架，通过上下文-预测-融合机制和渐进式课程学习，解决当前潜在推理方法中的特征崩溃和不稳定问题，实现更鲁棒的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前显式思维链（CoT）方法将模型思维限制在离散词汇空间，而现有的潜在空间推理方法存在特征崩溃和不稳定性问题，主要源于循环使用隐藏状态作为输入嵌入时的分布不匹配，或依赖辅助模型时的对齐问题。

Method: 提出Latent Thoughts Tuning（LT-Tuning）框架，引入上下文-预测-融合机制，联合利用上下文隐藏状态和词汇嵌入空间的预测语义指导，而不是仅依赖原始隐藏状态。结合渐进式三阶段课程学习流程，实现潜在思维和显式思维模式之间的动态切换。

Result: 实验表明，该方法优于现有的潜在推理基线，有效缓解了特征崩溃问题，实现了鲁棒的推理准确性。

Conclusion: LT-Tuning通过重新定义潜在思维的构建和部署方式，为大型语言模型提供了更灵活、更稳定的连续潜在空间推理能力，克服了传统方法的局限性。
...
TL;DR: 该研究探讨语言模型是否共享计算机制来处理心智理论和语用推理，以验证其是否发展出可跨任务重用的"社会世界模型"（功能整合假说）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言模型是否发展出可跨任务重用的"社会世界模型"，即对心理状态的表征是否能在不同任务中被重新利用，这有助于理解人工智能系统中社会认知的涌现。
...
Conclusion: 该研究贡献了新的心智理论定位数据、功能定位技术的改进方法，以及对人工智能系统中社会认知涌现的实证见解，支持语言模型可能发展出可跨任务重用的社会世界模型。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在没有明确计算成本奖励的情况下，通过推理时间扩展（如指令调优模型和大型推理模型）能否自发产生资源理性行为，即优化性能的同时考虑计算约束。

Method: 研究方法包括：1）引入变量归因任务，让模型根据候选变量、输入输出试验和预定义逻辑函数推断决定结果的变量；2）通过改变候选变量数量和试验次数系统操纵任务复杂度；3）比较指令调优模型和大型推理模型在不同复杂度下的表现。

Result: 研究发现：1）随着复杂度增加，两种模型都表现出从暴力搜索到分析策略的转变；2）指令调优模型在XOR和XNOR函数上表现下降，而大型推理模型保持稳健；3）模型能根据任务复杂度调整推理行为，即使没有明确的成本奖励。

Conclusion: 结论表明资源理性是推理时间扩展本身的涌现特性，模型能够自发地根据任务复杂度优化推理策略，这为理解大型语言模型的推理能力提供了重要见解。
...
TL;DR: 本文创建了首个大规模交通拦截数据集，包含来自不同社区视角的尊重度评分和自由文本解释，开发了视角感知建模框架来预测个性化尊重评分并生成特定解释。


<details>
  <summary>Details</summary>
Motivation: 交通拦截是最常见的警民互动之一，尊重是这些互动的核心维度，影响公众信任和合法性感知。然而，对尊重的解释具有主观性，受到生活经验的影响，因此社区特定视角至关重要。目前缺乏从多个社区视角标注的大型数据集来系统研究这些感知差异。

Method: 1) 基于程序正义理论、LAPD培训材料和实地调查开发领域特定的评估标准；2) 引入标准驱动的偏好数据构建框架，用于视角一致的对齐；3) 提出视角感知建模框架，从交通拦截记录中预测个性化尊重评分，并为警官和司机生成特定标注者的解释。

Result: 在所有三个标注者群体（警察相关、司法系统受影响、无关联的洛杉矶居民）中，该方法提高了评分预测性能和解释对齐度。视角感知框架使执法部门能够更好地理解不同社区的期望。

Conclusion: 该研究为执法部门提供了理解不同社区期望的重要工具，有助于建立公众信任和程序合法性。视角感知框架能够预测个性化尊重评分并生成特定解释，为改善警民互动提供了数据驱动的见解。
...
<details>
  <summary>Details</summary>
Motivation: 探索多语言语音模型中不同编码器层的音素预测演化，特别关注低资源语言Campidanese Sardinian，了解中间层是否比最终层提供更准确的音素表示。

Method: 使用预训练的Wav2Vec2模型，应用层解码策略分析音素级预测在编码器层间的演化；通过截断上层Transformer层比较不同层的性能；进行细粒度对齐分析评估预测质量。

Result: 截断上层Transformer层能改善音素错误率，最佳性能出现在最终层前两层；中间层预测能更好地保持音段身份、避免过度生成、减少特定类型的音系错误；发现回归错误现象。

Conclusion: 中间层编码更准确的音素表示，深层可能过度泛化或抽象化声学细节；建议将早期层探测作为ASR模型的诊断工具，特别在低资源环境中标准评估指标可能无法捕捉语言学意义行为时。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自我解释方法存在超参数敏感性问题，导致解释不可靠。需要一种更稳定、可泛化的自我解释技术，能够跨任务和模型家族工作。

Method: 训练轻量级适配器（仅需d_model+1个参数的标量仿射适配器）来解释冻结的语言模型内部状态。适配器在可解释性数据上训练，学习生成稀疏自编码器特征标签、识别主题和解码推理中的隐含实体。

Result: 训练后的适配器在70B规模上生成稀疏自编码器特征标签的准确率达到71%（比训练标签高8%）；主题识别召回率达到94%（基线仅1%）；能够解码多跳推理中的隐含实体。仅偏置向量就贡献了85%的改进，且简单适配器比复杂替代方案泛化更好。

Conclusion: 自我解释能力随模型规模提升而改善，无需修改被解释的模型。轻量级适配器方法为可靠、可扩展的模型自我解释提供了有效途径。
...
Method: 使用美国大陆1210万个样本（2017-2023年），对Google AlphaEarth的64维嵌入进行综合可解释性分析，结合线性、非线性和基于注意力的方法，并开发基于检索增强生成的地表智能系统。

Result: 嵌入维度能映射到特定的地表属性，完整嵌入空间能高保真重建大多数环境变量（12/26变量R²>0.90；温度和海拔R²=0.97）。基于此构建的地表智能系统在LLM评估中获得3.74±0.77的加权分数。

Conclusion: 卫星基础模型嵌入是具有物理结构的表示，可用于环境和地理空间智能的运营化应用。
...
Method: 引入包含时间限制证据和人工标注子声明证据范围的新数据集，评估两种证据对齐设置：子声明对齐证据（SAE）和重复声明级证据（SRE），分析不同数据集和领域的表现

Result: 分解仅在证据粒度细且严格对齐时带来显著性能提升；依赖重复声明级证据（SRE）的标准设置不仅无法改进，反而会降低性能；在存在噪声子声明标签时，保守的"弃权"策略比激进但错误的预测更能减少错误传播

Conclusion: 未来的声明分解框架必须优先考虑精确的证据合成，并校准子声明验证模型的标签偏差，证据对齐的质量是分解方法成功的关键因素
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的后门攻击构成重大安全风险，但触发器操作的内部机制仍不清楚。本研究旨在理解语言切换后门的工作机制，特别是触发器如何影响模型行为。

Method: 使用GAPperon模型家族（1B、8B、24B参数），这些模型在预训练期间注入了导致输出语言切换的触发器。采用激活修补技术定位触发器形成到早期层（模型深度的7.5-25%），并识别哪些注意力头处理触发器信息。

Result: 核心发现：触发器激活的注意力头与自然编码输出语言的注意力头在模型各尺度上显著重叠（Jaccard指数在0.18到0.66之间）。这表明后门触发器并未形成孤立电路，而是利用了模型现有的语言组件。

Conclusion: 后门检测方法可以受益于监控已知功能组件而非搜索隐藏电路，缓解策略可能利用注入行为与自然行为之间的纠缠关系。这些发现对后门防御具有重要意义。
...
TL;DR: 该研究提出了首个法语金融文档多模态评估基准Multimodal Finance Eval，包含1,204个专家验证问题，涵盖文本提取、表格理解、图表解读和多轮对话推理。评估发现当前VLM在文本和表格任务上表现良好（85-90%准确率），但在图表解读（34-62%）和多轮对话推理（约50%）方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLM）在通用文档理解任务上表现良好，但在专业非英语领域（特别是金融领域）的可靠性尚未充分探索。金融文档包含密集的监管文本、数值表格和视觉图表，提取错误可能带来实际后果，因此需要专门评估。

Method: 1. 创建Multimodal Finance Eval基准数据集，包含1,204个专家验证问题，来自真实投资说明书、KIDs和PRIIPs文档
2. 问题类型涵盖文本提取、表格理解、图表解读和多轮对话推理
3. 评估6个开源VLM模型（8B-124B参数）
4. 使用LLM-as-judge协议进行评估

Result: 1. 模型在文本和表格任务上表现良好（85-90%准确率）
2. 图表解读任务表现较差（34-62%准确率）
3. 多轮对话推理存在严重问题：早期错误会传播到后续轮次，导致准确率降至约50%，且与模型规模无关
4. 当前VLM在定义明确的提取任务上有效，但在交互式多步骤金融分析中表现脆弱

Conclusion: 当前VLM在法语金融文档理解中，对文本和表格等结构化信息处理能力较强，但在图表理解和多轮对话推理方面存在明显不足。Multimodal Finance Eval基准为这一高风险领域提供了具有挑战性的评估标准，有助于推动相关技术进步。
...
TL;DR: 本文提出了一种基于特征激活覆盖度(FAC)的数据多样性度量方法，以及相应的数据合成框架FAC Synthesis，通过识别种子数据集中缺失的特征并生成包含这些特征的合成样本来提升大语言模型的下游性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用基于文本的指标来衡量后训练数据的多样性，但这些指标只能捕捉语言变化，对决定下游性能的任务相关特征提供信号较弱。需要一种更有效的多样性度量方法来指导数据合成。

Method: 1. 提出特征激活覆盖度(FAC)作为在可解释特征空间中度量数据多样性的指标；2. 开发FAC Synthesis框架：首先使用稀疏自编码器识别种子数据集中缺失的特征，然后生成明确反映这些特征的合成样本。

Result: 实验表明，该方法在各种任务上一致地提高了数据多样性和下游性能，包括指令跟随、毒性检测、奖励建模和行为引导。有趣的是，发现了跨模型家族（LLaMA、Mistral、Qwen）共享的可解释特征空间，实现了跨模型知识转移。

Conclusion: 该工作为探索大语言模型的数据中心化优化提供了坚实且实用的方法论，通过特征空间中的多样性度量指导数据合成，显著提升模型性能。
...
TL;DR: 利用焦虑词汇词典分析社交媒体数据，发现焦虑水平在一天中呈现规律变化（早上8点最高，中午最低），周末最低，周中最高，过去时态焦虑最高，未来时态最低，第三人称代词相关帖子焦虑程度更高。
...
Method: 使用新创建的词汇-焦虑关联词典，分析美国和加拿大大量社交媒体数据（推文），通过时间分析、时态分析和代词分析来探索焦虑模式。

Result: 焦虑水平在一天中呈现系统性波动：早上8点最高（与体内皮质醇水平高峰期一致），中午最低；周末焦虑最低，周中最高；过去时态句子焦虑最高，未来时态最低；第三人称代词相关帖子比第一、二人称焦虑更高；主语代词相关帖子比宾语代词焦虑更高。

Conclusion: 社交媒体焦虑分析揭示了焦虑的时间模式和语言特征，不仅显示了人们何时感到焦虑，还表明不同类型的关注焦点（未来、过去、自我、外部等）与焦虑程度相关，为理解焦虑情绪提供了有价值的见解。
...
TL;DR: EVOKE是一个英语和韩语情感词汇的平行数据集，包含1427个韩语词和1399个英语词，提供多对多翻译、语言特定情感词识别以及多义情感词和情感隐喻标注。
...
Method: 系统性地收集和标注英韩情感词汇，包括：1) 收集1427个韩语词和1399个英语词；2) 标注819个韩语和924个英语形容词和动词；3) 标注每个词的多重含义及其关系；4) 识别多义情感词和情感相关隐喻；5) 建立多对多翻译映射。

Result: 创建了迄今为止最全面、系统且理论无关的英韩情感词汇数据集，包含详细的语义标注和跨语言映射，数据集已在GitHub上公开可用。

Conclusion: EVOKE数据集为跨语言情感研究提供了有价值的资源，支持情感科学、心理语言学和自然语言处理等多个领域的研究，允许研究者根据自身需求和理论视角灵活使用该资源。
...
TL;DR: NeSyS框架通过结合LLM的语义先验和符号规则的逻辑一致性，解决了LLM作为世界模型时的幻觉问题，同时保持表达能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为世界模型时经常产生幻觉，特别是在需要严格遵守确定性转换规则的角落案例中。符号世界模型虽然逻辑一致但缺乏语义表达能力。需要一种方法同时具备表达能力和鲁棒性。

Method: 提出神经符号协同框架，将LLM的概率语义先验与可执行的符号规则相结合。通过交替训练两种模型，使用对方无法充分解释的轨迹。符号世界模型直接修改LLM的输出概率分布，神经世界模型仅在符号规则未覆盖的轨迹上进行微调。

Result: 在ScienceWorld、Webshop和Plancraft三个交互环境中，NeSyS在预测准确性和数据效率方面均优于基线方法，训练数据减少50%而准确率不损失。

Conclusion: NeSyS成功结合了神经和符号方法的优势，为构建既具有表达能力又逻辑一致的世界模型提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统Chain-of-Thought (CoT) 提示在复杂任务中存在瓶颈：线性文本序列无法有效处理视觉信息，推理历史不可变，修正错误需要冗长下游修正或完全重生成，增加了token消耗和认知负担，尤其在几何和SVG设计等高维领域，文本表达缺乏视觉指导。

Method: 提出Canvas-of-Thought (Canvas-CoT) 方法：利用HTML Canvas作为外部推理基底，支持原子级的DOM基础CRUD操作，实现原地状态修订而不破坏上下文；集成基于渲染的批判循环作为硬约束验证器，提供显式视觉反馈。

Result: 在VCode、RBench-V和MathVista数据集上的广泛实验表明，Canvas-CoT显著优于现有基线方法，建立了上下文高效多模态推理的新范式。

Conclusion: Canvas-CoT通过引入外部视觉推理基底和渲染批判机制，解决了传统CoT在复杂多模态任务中的局限性，为高维视觉推理任务提供了更精确、高效的解决方案。
...
TL;DR: 该研究提出了一个面向鲁棒性的知识编辑去毒化评估框架，发现现有方法存在伪去毒化问题，且去毒效果在多目标、跨语言场景下存在局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识编辑的去毒化方法评估主要依赖自动毒性分类器，假设降低的毒性分数反映了真正的行为抑制，但这种评估方式可能掩盖了方法的真实可靠性问题。

Method: 提出了一个鲁棒性导向的评估框架，从三个维度评估知识编辑去毒化：优化鲁棒性、组合鲁棒性和跨语言鲁棒性，识别伪去毒化等失效模式。

Result: 发现伪去毒化是常见失效模式，毒性降低源于退化的生成行为而非真正的不安全内容抑制；多目标联合编辑时去毒效果下降；跨语言去毒化仅在特定模型-方法组合下有效。

Conclusion: 基于知识编辑的去毒化方法仅在特定模型、有限去毒目标和部分语言下具有鲁棒性，需要更全面的评估框架来确保其真实可靠性。
...
TL;DR: GRU-Mem通过引入文本控制的门控机制改进MemAgent，解决长上下文推理中内存爆炸和缺乏退出机制的问题，实现更稳定高效的推理。


<details>
  <summary>Details</summary>
Motivation: 现有MemAgent在处理长上下文时存在两个关键缺陷：1) 内存可能因无差别更新而爆炸式增长，即使在没有证据的文本块上也会更新；2) 循环缺乏退出机制，导致在收集足够证据后仍进行不必要的计算。

Method: 提出GRU-Mem，引入两个文本控制的门控机制：更新门控制内存更新，退出门控制循环退出。通过端到端强化学习训练，使用两个奖励信号r_update和r_exit分别奖励正确的更新和退出行为。
...
<details>
  <summary>Details</summary>
Motivation: 构建智能体时最重要的两个因素：敏锐的推理能力和快速可靠的执行能力。需要在保持前沿智能水平的同时实现计算效率，为现实工业环境部署复杂智能体提供高密度基础。

Method: 采用稀疏混合专家架构，结合3:1滑动窗口/全注意力交错优化和多令牌预测来降低延迟；设计了可扩展的强化学习框架，结合可验证信号和偏好反馈，支持大规模离策略训练下的稳定自我改进。
...
Conclusion: Step 3.5 Flash通过重新定义效率边界，为在现实工业环境中部署复杂智能体提供了高密度基础，实现了前沿智能水平与计算效率的有效平衡。
...
TL;DR: KPO方法使用在线因果卡尔曼滤波来稳定强化学习中的重要性采样比率，解决了大语言模型训练中的高方差问题，在数学推理任务上取得了优异效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的强化学习面临高方差的重要性采样比率问题，现有方法要么使用固定的序列级比率，要么单独调整每个token的比率，忽略了序列中token间的时序关系，导致训练不稳定甚至崩溃。

Method: 提出在线因果卡尔曼滤波策略优化（KPO），将期望的重要性采样比率建模为跨token演化的潜在状态，应用卡尔曼滤波器基于过去token的状态在线自回归更新该状态，从而在保留token级局部结构变化的同时平滑噪声尖峰。

Result: 在具有挑战性的数学推理数据集上，KPO相比现有最先进方法取得了更优的结果。
...
TL;DR: UMEM框架联合优化LLM智能体的记忆提取与管理，通过语义邻域建模和GRPO奖励机制提升记忆泛化能力，在多项基准测试中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体方法主要优化记忆管理，而将记忆提取视为静态过程，导致智能体积累实例特定噪声而非鲁棒记忆，泛化能力差
...
Conclusion: UMEM通过联合优化记忆提取与管理，结合语义邻域建模，有效解决了LLM智能体记忆泛化问题，实现了更鲁棒的自演化能力
...
Method: 使用词汇级别的单字交叉熵和词频统计来衡量重叠度，在10个零样本基准测试、4个预训练数据集（8.5B到60B tokens）和模型规模（400M到3B参数）范围内进行控制实验。

Result: 结果显示词汇级别的单字交叉熵与基准测试性能之间存在稳健的负相关关系，表明广泛使用的基准测试受到训练和评估数据之间词汇重叠的强烈影响。具有相似词汇单字交叉熵的更大预训练子集能产生更好的下游结果。

Conclusion: 许多标准基准测试相对于预训练语料库只是弱分布外，简单的词汇重叠统计就能预测基准测试性能，词汇频率统计在塑造基准分数中发挥额外作用。
...
TL;DR: 该研究评估了基于Transformer的语言模型在格鲁吉亚语分裂作格对齐任务上的表现，发现模型在作格标记上表现最差，在通格标记上表现最好，性能与三种格形式在训练数据中的频率分布相关。


<details>
  <summary>Details</summary>
Motivation: 评估语言模型在格鲁吉亚语这种罕见的分裂作格对齐系统上的表现，格鲁吉亚语使用主格、作格和与格的不同排列来标记主语和宾语，这种系统在自然语言处理研究中相对较少被探索。

Method: 采用基于树库的方法，使用Grew查询语言生成最小对比对，创建包含370个句法测试的数据集，涵盖7个任务（每个任务50-70个样本），每个样本测试三种名词形式。评估了5个编码器模型和2个解码器模型，使用词级和/或句子级准确率指标。

Result: 无论具体句法结构如何，模型在正确分配作格方面表现最差，在正确分配主格方面表现最好。性能与三种形式（主格>与格>作格）的整体频率分布相关。作格的特定角色加上训练数据的缺乏可能是导致该格表现不佳的原因。

Conclusion: 该研究揭示了语言模型在罕见句法系统上的局限性，特别是数据稀缺的低资源语言。公开的数据集和方法为未来在基准测试有限的语言上进行句法评估提供了有趣的研究方向。
...
TL;DR: LoCoMo-Plus是一个评估LLM对话系统认知记忆能力的基准测试，专注于评估模型在语义断连的线索-触发场景下保留和应用潜在约束的能力。


<details>
  <summary>Details</summary>
Motivation: 现有对话记忆基准主要关注表面事实回忆，但在真实对话中，合适的回应往往依赖于用户状态、目标或价值观等未明确查询的隐式约束。需要评估模型在这种语义断连场景下的认知记忆能力。

Method: 提出了LoCoMo-Plus基准测试，专注于评估线索-触发语义断连下的认知记忆。同时提出了基于约束一致性的统一评估框架，替代传统的字符串匹配指标和显式任务类型提示。

Result: 实验表明，认知记忆对多种骨干模型、基于检索的方法和记忆系统仍然具有挑战性，揭示了现有基准未能捕捉到的失败情况。

Conclusion: LoCoMo-Plus基准和约束一致性评估框架能够有效评估LLM对话系统在隐式约束保留和应用方面的认知记忆能力，填补了现有评估方法的空白。
...
TL;DR: Macaron是一个多语言基准测试，通过模板优先的方法将推理类型与文化因素解耦，包含11,862个实例，覆盖20种语言和文化背景，评估显示推理模式模型表现最佳，而开放权重模型在本地语言上表现显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有多语言基准测试存在两个问题：翻译的数据集保持英语中心场景，而文化优先的数据集又缺乏对所需推理类型的控制。需要创建一个能够同时考虑推理类型和文化因素的基准测试。

Method: 采用模板优先的方法，使用100个语言无关模板覆盖7种推理类型和22个文化方面。由母语标注者创建情景对齐的英语和本地语言多项选择题，并系统推导出真/假问题。

Result: Macaron包含11,862个实例，覆盖20个国家/文化背景、10种文字和20种语言（包括低资源语言）。在21个多语言LLM的零样本评估中，推理模式模型表现最强，英语和本地语言接近同等水平，而开放权重模型在本地语言上表现大幅下降，在真/假任务上常接近随机猜测。文化基础的数学和计数模板始终最难。

Conclusion: Macaron基准测试成功解耦了推理类型和文化因素，揭示了多语言LLM在文化基础推理方面的能力差异，特别是推理模式模型在跨语言一致性方面的优势。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在专业领域表现不足，传统监督微调会导致灾难性遗忘，而持续预训练计算成本过高，需要高效的训练后适应方法

Method: 提出强化课程预对齐方法，早期采用部分输出约束安全引入新领域概念，随着模型熟悉度增加，逐步过渡到完整生成优化

Result: 在多个专业领域和通用基准测试中验证了RCPA的有效性，实现了领域知识获取与通用多模态能力保持的平衡

Conclusion: RCPA为构建高性能且领域自适应的视觉语言模型提供了实用路径，解决了领域适应中优化崩溃的问题
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在内容生成失真（幻觉）问题，核心原因是模型缺乏对自身存储知识的认知，无法像人类一样表达超出知识边界的问题状态。现有研究主要关注白盒模型，而黑盒模型（仅提供API访问）的知识边界表达方法尚未充分探索。

Method: 基于知识蒸馏框架，设计深度学习模型LSCL。以黑盒LLM的输入问题、输出答案和token概率作为输入，构建输入与模型内部知识状态之间的映射关系，实现对黑盒LLM知识边界的量化和表达。

Result: 在多个公共数据集和主流黑盒LLM上的实验表明，LSCL能有效帮助黑盒LLM准确表达知识边界，在准确率和召回率等指标上显著优于现有基线模型。针对不支持token概率访问的黑盒LLM，提出了自适应替代方法，性能接近LSCL且优于基线模型。

Conclusion: LSCL方法成功解决了黑盒大语言模型的知识边界表达问题，为缓解幻觉问题提供了有效解决方案，并针对不同API限制提供了适应性方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然表现出强大能力，但对输入上下文微小变化敏感，影响可靠性。传统指标如准确率和困惑度无法评估局部预测鲁棒性，因为归一化的输出概率可能掩盖模型内部状态对扰动的潜在弹性

Method: 引入Token Constraint Bound (δ_TCB)指标，量化LLM在主导下一个token预测显著变化前能承受的最大内部状态扰动。该指标与输出嵌入空间几何结构内在相关，提供模型内部预测承诺稳定性的洞察

Result: 实验表明δ_TCB与有效提示工程相关，并在上下文学习和文本生成过程中发现了困惑度指标遗漏的关键预测不稳定性。δ_TCB提供了分析LLM预测上下文稳定性的原则性补充方法

Conclusion: δ_TCB为分析和潜在改进LLM预测的上下文稳定性提供了原则性、互补的方法，有助于评估模型对输入扰动的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法常受到噪声和冲突更新信号的困扰，导致优化过程不稳定，需要更有效的稳定优化机制。

Method: 提出C-MOP框架：1) BACS利用批次信息挖掘硬负样本、锚点和边界对来精确表征正负提示样本的典型表示和决策边界；2) MGSC引入具有时间衰减的文本动量机制，从迭代中波动的梯度中提取持久共识。
...
TL;DR: 该研究评估了大型语言模型在系统综述和元分析中的证据提取能力，发现当前LLMs在需要变量、角色、统计方法和效应量之间稳定绑定的复杂结构化提取任务上表现不佳，无法满足自动化元分析的要求。


<details>
  <summary>Details</summary>
Motivation: 系统综述和元分析需要将叙述性文章转换为结构化、基于数值的研究记录。尽管大型语言模型发展迅速，但尚不清楚它们是否能满足这一过程的结构化要求，该要求强调在文档间保持角色、方法和效应量归属的一致性，而非仅仅识别孤立实体。

Method: 提出了一个结构化的诊断框架，将LLM证据提取评估为一系列模式约束查询的进展，这些查询具有递增的关系和数值复杂性。使用手动整理的跨五个科学领域的语料库，以及统一的查询套件和评估协议，在单文档和长上下文多文档输入两种模式下评估了两个最先进的LLM。

Result: 跨领域和模型，单属性查询表现中等，但一旦任务需要在变量、角色、统计方法和效应量之间进行稳定绑定时，性能急剧下降。完整的元分析关联元组提取可靠性接近零，长上下文输入进一步加剧了这些失败。下游聚合放大了上游的微小错误，导致语料库级统计不可靠。
...
TL;DR: FinMMEval 2026是首个多语言多模态金融大语言模型评估框架，包含三个相互关联的任务：金融考试问答、多语言金融问答和金融决策制定。


<details>
  <summary>Details</summary>
Motivation: 当前金融自然语言处理虽然已有进展，但现有基准测试大多是单语言、纯文本且局限于狭窄子任务，缺乏全面评估金融LLMs在多语言和多模态环境下的能力。

Method: 通过建立三个相互关联的任务：1) 金融考试问答，2) 多语言金融问答(PolyFiQA)，3) 金融决策制定，形成一个全面的评估套件，衡量模型在不同语言和模态下的推理、泛化和行动能力。

Result: 该实验室提供了首个多语言多模态金融LLM评估框架，公开了数据集和评估资源以支持可重复研究。

Conclusion: FinMMEval 2026旨在促进稳健、透明和全球包容的金融AI系统发展，填补了现有金融LLM评估的空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理大规模语料库时搜索延迟高，无法有效处理语义变化，需要一种既能快速搜索又能处理语义松弛的算法。

Method: 基于后缀数组的字符串匹配方法，采用磁盘感知设计实现快速精确查找，结合动态语料感知剪枝来抑制查询长度引起的搜索空间指数增长。

Result: 在FineWeb-Edu语料库（1.4T tokens）上，相比infini-gram、infini-gram mini和SoftMatcha等方法，实现了显著更低的搜索延迟。能够识别现有方法未发现的训练语料库中的基准污染。

Conclusion: 该方法实现了超快速的大规模语料库搜索，有效处理语义变化，在搜索速度和准确性方面优于现有方法，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 自闭症患者的时间性障碍（如与社会环境不同步和不可预测性）被认为是核心特征，严重影响人际关系。现有研究存在三个局限：1）以缺陷为基础的医学模型主导；2）质性研究样本量小；3）计算研究缺乏现象学基础。

Method: 整合三种方法：研究A-使用跨诊断时间体验评估工具对自闭症个体进行结构化现象学访谈；研究B-对为此构建的自闭症自传叙事语料库进行计算分析；研究C-使用叙事流测量方法复制计算研究，评估自闭症自传的现象学真实性感知。

Result: 访谈显示自闭症组与对照组最显著差异在于体验的不可预测性。计算分析结果与此一致：自闭症叙事中的时间词汇显著更负面，特别是"即时性与突发性"类别。异常值分析识别出与感知不连续性相关的词汇（不可预测地、突然地、急剧地）为高度负面。叙事流计算分析发现语料库中的自闭症叙事在量化上更接近自传故事而非虚构故事。

Conclusion: 自闭症个体经历的时间挑战主要涉及生活体验的不可预测性，源于生活体验的内容本身，而非自闭症叙事建构方式。研究成功弥合了现象学与计算方法之间的鸿沟。
...
TL;DR: LoRA-Squeeze：一种改进标准LoRA的方法，通过后处理压缩或训练中动态调整秩，先学习高秩表达再压缩，优于直接学习低秩适配器
...
Method: 先使用较高源秩进行微调，重构或高效近似全权重更新矩阵，然后使用随机奇异值分解(RSVD)创建压缩后的低目标秩LoRA模块。包括后处理压缩和训练中动态秩退火两种变体。

Result: 在13个文本和10个视觉语言任务上的实验表明，后处理压缩通常能产生优于直接训练目标秩的低秩适配器，特别是允许少量目标秩微调步骤时。动态秩退火变体始终实现最佳的LoRA大小-性能权衡。

Conclusion: LoRA-Squeeze通过先学习高秩表达再压缩的方法，有效解决了标准LoRA的秩选择问题，提供了更好的性能-效率权衡，且方法简单高效。
...
<details>
  <summary>Details</summary>
Motivation: 自发语言中的细微变化是认知衰退的最早指标之一。识别可解释的痴呆语言标记可以支持透明且临床基础良好的筛查方法。

Method: 使用DementiaBank Pitt Corpus的自发语言转录，采用三种语言表征：原始清理文本、结合词汇和语法信息的词性增强表征、以及纯词性的句法表征。使用逻辑回归和随机森林模型，在两种协议下评估：转录级训练-测试分割和防止说话者重叠的受试者级五折交叉验证。通过全局特征重要性检查模型可解释性，并使用Mann-Whitney U检验和Cliff's delta效应量进行统计验证。

Result: 所有表征下模型均获得稳定性能，句法和语法特征即使在缺乏词汇内容时仍保持强大的区分能力。受试者级评估产生更保守但一致的结果，特别是词性增强和纯词性表征。统计分析显示功能词使用、词汇多样性、句子结构和语篇连贯性存在显著组间差异，与机器学习特征重要性结果高度一致。

Conclusion: 结果表明抽象语言特征在临床现实评估中捕捉到早期认知衰退的稳健标记。通过将可解释机器学习与非参数统计验证相结合，本研究支持使用基于语言的特征进行透明可靠的语言基础认知筛查。
...
Method: 将问题形式化为经典的基于梯度优化问题。首先提出简单算法实现给定（冻结）语言模型的端到端可微分性，然后通过梯度下降找到优化的提示词。核心见解是将语言模型视为在标记分布序列上操作的函数（而非传统的标记序列函数）。

Result: 实验和消融研究表明，DLM驱动的反转方法能够可靠且高效地优化长度为10和80的提示词，以产生长度为20的目标输出，适用于多个白盒语言模型（开箱即用）。

Conclusion: 该方法成功解决了语言模型可逆性问题，通过将语言模型重新定义为分布序列上的函数，实现了基于梯度优化的提示词搜索，为语言模型的可控生成提供了新途径。
...
TL;DR: 本文提出了一种基于条件掩码扩散的嵌入反演方法，通过迭代去噪并行恢复所有token，而非顺序自回归生成，仅需8次前向传播即可实现高效反演。


<details>
  <summary>Details</summary>
Motivation: 传统的嵌入反演方法通常采用顺序自回归生成，效率较低。本文旨在开发一种更高效的嵌入反演方法，能够并行恢复所有token，减少计算开销。

Method: 将嵌入反演问题建模为条件掩码扩散过程，通过自适应层归一化将目标嵌入作为条件输入到掩码扩散语言模型中，仅需8次前向传播即可完成反演，无需访问目标编码器。

Result: 在三个嵌入模型的32-token序列上，该方法实现了81.3%的token准确率和0.87的余弦相似度，证明了其有效性。

Conclusion: 条件掩码扩散方法为嵌入反演提供了一种高效且准确的并行解决方案，相比传统自回归方法具有显著优势。
...
TL;DR: 该论文提出了一个基于图思维（GoT）的框架，通过多级感知建模对话过程，预测高层次沟通意图和低层次言语行为，用于全双工交互系统。
...
Method: 1. 将对话过程建模为多级感知；2. 通过图思维（GoT）对对话行为进行推理；3. 使用分层标注方案形式化意图到行动的路径；4. 开发高质量语料库，将可控、事件丰富的对话数据与人工标注标签配对；5. GoT框架将流式预测结构化为演化图，使Transformer能够预测下一个言语行为、生成决策的简明理由并动态优化推理。

Result: 在合成和真实全双工对话上的实验表明，该框架能够提供鲁棒的行为检测，产生可解释的推理链，并为全双工语音对话系统中的对话推理基准测试奠定基础。

Conclusion: 该研究提出了一个基于图思维的框架，成功建模了对话中的感知路径，实现了对言语行为的预测和可解释推理，为全双工交互系统的发展提供了重要基础。
...
TL;DR: Hibiki-Zero是一种无需词级对齐的实时语音翻译方法，通过强化学习优化延迟，在多语言任务中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 传统实时语音翻译方法依赖词级对齐数据，但这类数据难以大规模收集，且需要语言特定的启发式对齐方法，这限制了多语言扩展和性能优化

Method: 1) 首先在句子级对齐数据上训练高延迟语音翻译模型；2) 使用GRPO强化学习策略优化延迟同时保持翻译质量；3) 完全消除词级对齐需求，简化训练流程

Result: 在5个X到英语任务中达到SOTA性能（翻译准确性、延迟、语音传递和自然度）；新语言适配仅需不到1000小时语音数据；发布了包含45小时多语言数据的基准测试

Conclusion: Hibiki-Zero通过消除词级对齐需求，简化了训练流程并实现了多语言扩展，为实时语音翻译提供了高效可扩展的解决方案
...
TL;DR: SteuerEx是首个基于真实德国大学税法考试的开放基准测试，包含115个专家验证的问题，采用部分评分框架。SteuerLLM是基于该领域数据训练的28B参数模型，在税法推理任务上优于同规模通用模型，证明领域特定数据和架构适配比参数规模更重要。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用推理和语言理解方面表现出色，但在需要严格形式规则、精确术语和法律约束结构的领域（如税法）中性能下降。税法要求准确的法规引用、结构化法律论证和数值精度，现有模型难以满足这些要求。

Method: 1. 算法生成SteuerEx基准：从真实德国大学税法考试中提取115个专家验证的问题，涵盖六个核心税法领域和多个学术级别，采用语句级部分评分框架。
2. 开发SteuerLLM：使用受控检索增强管道从真实考试材料生成大规模合成数据集，训练领域适应的28B参数LLM。

Result: SteuerLLM（28B参数）在SteuerEx基准上持续优于同规模通用指令调优模型，在某些情况下甚至优于规模更大的系统。这表明对于现实法律推理任务，领域特定数据和架构适配比参数规模更具决定性。

Conclusion: 领域特定数据和架构适配对于法律AI系统在严格形式规则领域的表现至关重要。SteuerEx基准、训练数据、模型权重和评估代码已公开发布，支持可复现的领域特定法律AI研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准如SAFETUNEBED、VALUEBENCH和WORLDVIEW-BENCH主要孤立评估安全、价值和文化维度，无法揭示这些维度之间的交互和权衡关系。虽然MIB和INTERPRETABILITY BENCHMARK提供了模型失效的机制解释视角，但仍不足以系统表征跨维度权衡。

Method: 1. 构建MISALIGNTRADE数据集：包含112个规范性领域（14个安全、56个价值、42个文化），每个提示分为对象、属性或关系三种语义类型错位。使用Gemma-2-9B-it分类，Qwen3-30B-A3B-Instruct-2507扩展，SimHash指纹去重。2. 通过两阶段拒绝采样为每个提示配对错位和对齐响应。3. 在MISALIGNTRADE上评估通用、微调和开源权重LLMs。

Result: 基准测试显示，不同LLMs在跨维度上存在12%-34%的错位权衡。这表明模型在同时满足安全、价值和文化维度时存在显著困难。

Conclusion: MisAlign-Profile基准能够系统测量LLMs在安全、价值和文化维度间的错位权衡，揭示了现有模型在真实世界多维度约束下的局限性，为未来对齐研究提供了重要工具。
...
TL;DR: SafeThink是一种轻量级推理时防御方法，通过监控推理过程并在安全阈值被违反时注入优化的短前缀来恢复多模态大模型的安全性，同时保持推理性能。


<details>
  <summary>Details</summary>
Motivation: 基于强化学习的后训练虽然能提升多模态大模型的推理能力，但会同时降低安全对齐性并增加越狱成功率，需要在保持推理性能的同时恢复安全性。

Method: 提出SafeThink方法，将安全恢复视为满足性约束而非最大化目标。通过安全奖励模型监控推理轨迹，当安全阈值被违反时，条件性地注入优化的短纠正前缀（如"Wait, think safely"）。

Result: 在六个开源MLRMs和四个越狱基准测试中，SafeThink将攻击成功率降低30-60%（例如LlamaV-o1从63.33%降至5.74%），同时保持推理性能（MathVista准确率从65.20%降至65.00%）。关键发现是安全恢复通常只需1-3个推理步骤的干预。

Conclusion: SafeThink提供了一种有效的推理时防御方法，能够在保持模型推理能力的同时显著提升安全性，且干预成本低，只需在少数推理步骤中进行条件性干预。
...
TL;DR: 提出TEG方法，通过图结构整合外部知识来提升虚假信息检测性能，并进一步开发TEGRA框架集成领域特定知识
...
Method: 提出TEG方法，从文档中提取结构化信息形成图，同时对文本和图进行编码用于分类；进一步开发TEGRA框架集成领域特定知识

Result: 实验表明这种混合表示相比单独使用语言模型能提升虚假信息检测性能；TEGRA在大多数情况下能进一步提高分类准确率

Conclusion: 通过图结构整合外部知识的方法能有效提升虚假信息检测性能，领域特定知识的集成能带来额外改进
...
TL;DR: 研究发现，在固定训练步数下，对小规模思维链数据进行多轮重复训练，比单轮训练大规模数据效果更好，且完全记忆化时泛化性能达到最佳。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习直觉认为更多独特训练样本能带来更好泛化，但本文发现思维链监督微调中存在反直觉现象：重复训练小数据集反而优于单次训练大数据集。

Method: 在固定更新预算下，比较不同训练策略：多轮训练小数据集 vs 单轮训练大数据集。使用Olmo3-7B模型，在AIME'24/25和GPQA基准上测试，以token准确率作为训练饱和信号。

Result: 128轮训练400个样本比单轮训练51200个样本在基准测试上提升12-26个百分点，且没有额外灾难性遗忘。完全记忆化时泛化性能达到平台期。

Conclusion: 提出用token准确率作为停止准则，通过扩展训练轮次替代昂贵的数据扩展，为推理SFT提供实用方法。将"重复优势"现象作为理解LLM训练动态的新开放问题。
...
### [45] [Large Language Models Predict Functional Outcomes after Acute Ischemic Stroke](https://arxiv.org/abs/2602.10119)
...
Method: 研究评估了编码器模型（BERT、NYUTron）和生成模型（Llama-3.1-8B、MedGemma-4B），在冻结和微调两种设置下，使用大型真实世界卒中登记数据预测出院和90天mRS。数据集包括9,485份入院记录用于出院结局预测，1,898份记录用于90天结局预测，采用时间分割，最近12个月数据用于测试。

Result: 微调后的Llama模型表现最佳：90天mRS预测准确率为33.9%（95% CI 27.9-39.9%），二元分类准确率为76.3%（95% CI 70.7-81.9%）；出院预测准确率为42.0%（95% CI 39.0-45.0%），二元分类准确率为75.0%（95% CI 72.4-77.6%）。在90天预测上，Llama与基于结构化变量的基线模型表现相当。
...
### [46] [Towards Autonomous Mathematics Research](https://arxiv.org/abs/2602.10177)
*Tony Feng,Trieu H. Trinh,Garrett Bingham,Dawsen Hwang,Yuri Chervonyi,Junehyuk Jung,Joonkyung Lee,Carlo Pagano,Sang-hyun Kim,Federico Pasqualotto,Sergei Gukov,Jonathan N. Lee,Junsu Kim,Kaiying Hou,Golnaz Ghiasi,Yi Tay,YaGuang Li,Chenkai Kuang,Yuan Liu,Hanzhao,Lin,Evan Zheran Liu,Nigamaa Nayakanti,Xiaomeng Yang,Heng-tze Cheng,Demis Hassabis,Koray Kavukcuoglu,Quoc V. Le,Thang Luong*

Main category: cs.LG

TL;DR: Aletheia是一个数学研究智能体，能够从奥数竞赛题到博士级问题，再到完全自主生成数学研究论文，展示了AI在数学研究中的突破性进展。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型已在国际数学奥林匹克竞赛中达到金牌水平，但从竞赛级问题解决过渡到专业数学研究需要处理大量文献和构建长程证明。本研究旨在开发能够自主进行数学研究的AI系统。

Method: Aletheia是一个端到端的数学研究智能体，采用迭代生成、验证和修订解决方案的方法。它结合了Gemini Deep Think的增强版本、新颖的推理时间缩放定律（超越奥数问题级别）以及密集的工具使用来应对数学研究的复杂性。

Result: Aletheia展示了从奥数问题到博士级练习的能力，并实现了三个重要里程碑：1)完全由AI生成的关于算术几何中特征权结构常数的研究论文；2)人机协作证明交互粒子系统边界的研究论文；3)对700个开放问题的半自主评估，包括自主解决了四个开放问题。

Conclusion: 研究建议建立量化AI辅助结果自主性和新颖性的标准等级，以帮助公众理解AI与数学的发展，并反思人机协作在数学研究中的前景。
...
### [47] [Signature-Kernel Based Evaluation Metrics for Robust Probabilistic and Tail-Event Forecasting](https://arxiv.org/abs/2602.10182)
*Benjamin R. Redhead,Thomas L. Lee,Peng Gu,Víctor Elvira,Amos Storkey*

Main category: cs.LG

TL;DR: 提出两种基于核的度量方法（Sig-MMD和CSig-MMD）来解决概率预测评估中存在的问题，特别是对尾部事件敏感度不足和依赖关系假设不当的问题。
...
Method: 提出两种基于核的度量方法：签名最大均值差异（Sig-MMD）和新的截尾Sig-MMD（CSig-MMD）。利用签名核捕捉复杂的变量间和时间依赖关系，对缺失数据保持鲁棒性。CSig-MMD引入截尾方案，优先考虑预测尾部事件的能力，同时严格保持适当性。

Result: 这些度量方法能够更可靠地评估直接多步预测，促进更鲁棒的概率算法开发。

Conclusion: 提出的Sig-MMD和CSig-MMD度量解决了当前概率预测评估框架的关键缺陷，特别是在捕捉复杂依赖关系和提高对尾部事件敏感性方面，为概率预测提供了更可靠的评估工具。
...
### [48] [Adaptive Optimization via Momentum on Variance-Normalized Gradients](https://arxiv.org/abs/2602.10204)
...
Method: MVN-Grad结合了两个互补思想：1）基于方差的归一化，通过梯度不确定性的指数移动平均值缩放每个坐标；2）在归一化后的梯度上应用动量。这种方法将动量应用在归一化之后，消除了标准Adam中动量与归一化器之间的耦合。

Result: 理论证明：1）在标准噪声假设下，MVN-Grad的一步条件更新方差严格小于先动量后归一化的方法；2）对异常值具有鲁棒性，对单个梯度尖峰具有均匀有界响应；3）在低方差情况下避免符号型崩溃并可能加速收敛。实验表明在CIFAR-100图像分类和GPT风格语言建模基准测试中，MVN-Grad匹配或优于Adam、AdaBelief和LaProp，提供更平滑的训练和更好的泛化能力，且无额外开销。

Conclusion: MVN-Grad通过将动量应用在方差归一化之后，提供了一种更稳定、对异常值更鲁棒且性能优越的优化器，在多个深度学习任务中表现出色，无需额外计算成本。
...
### [49] [How Much Reasoning Do Retrieval-Augmented Models Add beyond LLMs? A Benchmarking Framework for Multi-Hop Inference over Hybrid Knowledge](https://arxiv.org/abs/2602.10210)
*Junhong Lin,Bing Zhang,Song Wang,Ziyan Liu,Dan Gutfreund,Julian Shun,Yada Zhu*

Main category: cs.LG

TL;DR: HybridRAG-Bench是一个评估混合知识增强LLM检索和推理能力的基准框架，通过结合arXiv最新科学文献的非结构化文本和结构化知识图谱，生成需要多跳推理的知识密集型问答对，避免预训练数据污染问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试与LLM预训练数据重叠严重，难以区分真正的检索推理能力和参数记忆，需要构建能够评估混合知识（非结构化文本+结构化知识图谱）增强LLM在知识密集型多跳推理任务上真实能力的评估框架。

Method: 从arXiv最新科学文献自动提取非结构化文本和结构化知识图谱表示，生成基于显式推理路径的知识密集型问答对，支持灵活选择领域和时间范围，实现污染感知和可定制的评估。

Result: 在人工智能、治理与政策、生物信息学三个领域的实验表明，HybridRAG-Bench能够有效评估真正的检索和推理能力而非参数记忆，为混合知识增强推理系统提供了原则性的测试平台。

Conclusion: HybridRAG-Bench提供了一个可扩展的基准框架，能够评估LLM在混合知识增强下的真实检索和推理能力，解决了现有基准受预训练数据污染的问题，支持随着模型和知识演进的持续评估。
...
### [50] [Rank-Accuracy Trade-off for LoRA: A Gradient-Flow Analysis](https://arxiv.org/abs/2602.10212)
*Michael Rushka,Diego Klabjan*

Main category: cs.LG

TL;DR: 论文从动力系统角度分析LoRA更新秩与精度的关系，推导出闭式表达式


<details>
  <summary>Details</summary>
Motivation: 虽然实证研究表明LoRA在低秩更新时能达到与全参数方法相当的精度，但其精度与更新秩的理论关系尚未充分探索

Method: 从动力系统角度比较秩-r LoRA更新与全参数更新，在满秩和低秩两种机制下进行梯度流分析，推导LoRA梯度流方程并证明同时和顺序参数更新的等价性

Result: 获得了LoRA秩与精度之间的闭式关系，适用于迹平方和Frobenius范数低秩近似损失函数

Conclusion: 通过严格的动力系统分析，建立了LoRA更新秩与模型精度的理论关系，为LoRA方法的理论理解提供了基础
...
### [51] [ELROND: Exploring and decomposing intrinsic capabilities of diffusion models](https://arxiv.org/abs/2602.10216)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型对同一文本提示会产生多种随机视觉输出，用户无法直接控制具体语义变化。现有无监督方法仅分析输出特征而忽略生成过程，需要更直接的控制机制

Method: 收集固定提示下不同随机实现之间的梯度差异，通过反向传播获得梯度集合，使用主成分分析或稀疏自编码器分解为有意义的控制方向

Result: 方法能够：(1)分离可解释、可控制的方向实现细粒度概念控制；(2)有效缓解蒸馏模型中的模式崩溃问题，恢复丢失的多样性；(3)建立基于发现子空间维度的概念复杂度估计器

Conclusion: 提出了一种在扩散模型输入嵌入空间直接解耦语义方向的框架，实现了对生成过程更精细的控制，并为概念复杂度提供了新的量化方法
...
### [52] [Internalizing Meta-Experience into Memory for Guided Reinforcement Learning in Large Language Models](https://arxiv.org/abs/2602.10224)
*Shiting Huang,Zecheng Li,Yu Zeng,Qingnan Ren,Zhen Fang,Qisheng Su,Kou Shi,Lin Chen,Zehui Chen,Feng Zhao*

Main category: cs.LG

TL;DR: MEL框架通过将自我蒸馏的元经验融入LLM参数记忆，解决了RLVR中缺乏错误归因和经验内化的问题，在基准测试中实现了3.92%-4.73%的Pass@1提升。


<details>
  <summary>Details</summary>
Motivation: 尽管RLVR能有效提升LLM的推理能力，但它面临元学习瓶颈：缺乏人类学习周期中固有的错误归因和经验内化机制，限制了细粒度信用分配和可重用知识形成。

Method: 提出Meta-Experience Learning框架，在标准RLVR基础上增加设计：利用LLM的自我验证能力对正确和错误轨迹进行对比分析，识别推理错误的具体分叉点，将其总结为可泛化的元经验，并通过最小化负对数似然将元经验内化到LLM参数记忆中。

Result: 实验结果显示MEL在基准测试中实现了3.92%-4.73%的Pass@1提升，在不同模型规模下都取得了稳定改进。

Conclusion: MEL框架通过引入元经验学习和内化机制，有效解决了RLVR的元学习瓶颈，显著提升了LLM的推理性能，证明了元经验内化对知识重用和推理改进的重要性。
...
### [53] [Frame-Level Internal Tool Use for Temporal Grounding in Audio LMs](https://arxiv.org/abs/2602.10230)
*Joesph An,Phillip Keung,Jiaqi Wang,Orevaoghene Ahia,Noah A. Smith*

Main category: cs.LG

TL;DR: 提出帧级内部工具使用框架，让音频语言模型利用自身内部音频表示直接进行时间定位，相比基于文本token的时间戳生成方法，实现了50倍推理加速和更好的长度泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型在处理需要精确时间定位的任务（如词对齐、说话人分离）时存在困难。标准的基于文本token生成时间戳的方法计算成本高、容易产生幻觉，特别是在处理超出训练分布长度的音频时表现不佳。

Method: 提出帧级内部工具使用框架，训练音频语言模型利用自身内部音频表示直接进行时间定位。采用轻量级预测机制，通过两个目标进行训练：二元帧分类器和新颖的非齐次泊松过程损失，后者用于建模时间事件强度。

Result: 在词定位、说话人分离和事件定位任务上，该方法优于基于token的基线方法。最显著的是实现了超过50倍的推理加速，并展现出强大的长度泛化能力，在超出分布范围的音频时长上仍保持高精度，而标准的基于token的模型则完全失效。

Conclusion: 帧级内部工具使用框架为音频语言模型的时间定位任务提供了一种高效、鲁棒的解决方案，显著提升了推理速度和泛化能力，解决了传统方法在处理长音频时的局限性。
...
### [54] [Blockwise Advantage Estimation for Multi-Objective RL with Verifiable Rewards](https://arxiv.org/abs/2602.10231)
*Kirill Pavlenko,Alexander Golubev,Simon Karasik,Boris Yangel*

Main category: cs.LG

TL;DR: 提出Blockwise Advantage Estimation方法，解决GRPO在结构化生成中奖励信号耦合问题，通过为每个目标分配独立优势值并仅应用于对应文本块，减少对人工设计标量奖励的依赖


<details>
  <summary>Details</summary>
Motivation: GRPO方法为整个完成序列分配单一标量优势值，在具有明确分段和目标的结构化生成中，这会导致不同段落的无关奖励信号相互耦合，造成目标干扰和信用分配错误

Method: 提出Blockwise Advantage Estimation方法族，为每个目标分配独立的优势值并仅应用于对应文本块；引入Outcome-Conditioned Baseline，通过基于前缀派生的中间结果对样本进行分层，仅使用组内统计来近似中间状态值，避免昂贵的嵌套rollout

Result: 在数学任务的不确定性估计中，该方法缓解了奖励干扰问题，与最先进的奖励设计方法具有竞争力，并保持了置信度加权集成在测试时的增益

Conclusion: 该方法为优化结构化生成中的顺序目标提供了模块化方案，无需额外rollout，能够自然地扩展到更多目标，减少对人工设计标量奖励的依赖
...
### [55] [Risk-Equalized Differentially Private Synthetic Data: Protecting Outliers by Controlling Record-Level Influence](https://arxiv.org/abs/2602.10232)
*Amir Asiaee,Chao Yan,Zachary B. Abrams,Bradley A. Malin*

Main category: cs.LG

TL;DR: 论文提出风险均衡差分隐私合成框架，通过降低高风险记录对生成器的影响来优先保护这些记录，从而减少针对异常记录的成员推理攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 当发布合成数据时，某些个体比其他个体更难保护。具有罕见疾病组合的患者或具有异常特征的交易在人群中显得突出。差分隐私提供最坏情况保证，但经验攻击（特别是成员推理攻击）对这类异常值的成功率要高得多，尤其是在中等隐私预算和辅助信息的情况下。

Method: 风险均衡DP合成框架分两个阶段：首先，使用少量隐私预算估计每个记录的"异常度"；其次，DP学习过程根据风险评分对每个记录进行加权，风险越高权重越低。在高斯机制下，记录的隐私损失与其对输出的影响成正比，因此故意减少异常值的贡献可以为最需要保护的记录提供更严格的每实例隐私界限。

Result: 在具有受控异常注入的模拟数据实验中，风险加权显著降低了针对高异常度记录的成员推理攻击成功率；消融实验证实是针对性加权而非随机降权驱动了改进。在真实世界基准数据集（乳腺癌、成人、德国信贷）上，改进效果依赖于数据集，突出了评分器质量与合成流程之间的相互作用。

Conclusion: 风险均衡DP合成框架通过优先保护高风险记录，在保持整体隐私保证的同时，为最脆弱的个体提供更强的保护，特别是在中等隐私预算下对抗成员推理攻击方面表现出优势。
...
### [56] [Modeling Programming Skills with Source Code Embeddings for Context-aware Exercise Recommendation](https://arxiv.org/abs/2602.10249)
*Carlos Eduardo P. Silva,João Pedro M. Sena,Julio C. S. Reis,André G. Santos,Lucas N. Ferreira*

Main category: cs.LG

TL;DR: 提出基于源代码嵌入的学生编程技能建模方法，通过余弦相似度匹配学生技能与作业要求，实现个性化推荐


<details>
  <summary>Details</summary>
Motivation: 传统基于正确率或解题时间的推荐方法不能准确反映学生的编程技能水平，需要更精细的技能建模方法来提供个性化练习推荐

Method: 使用源代码嵌入技术建模学生编程技能，将学生提交的代码转换为向量表示，预测学生在多个编程主题上的技能水平，通过余弦相似度匹配学生技能向量与作业技能要求向量

Result: Jina嵌入在技能预测上优于TF-IDF、CodeBERT-cpp和GraphCodeBERT；基于技能预测的推荐系统比基于正确率或解题时间的基线方法产生更合适的练习推荐

Conclusion: 预测的编程技能为练习推荐提供了更强的信号，基于源代码嵌入的技能建模方法能有效支持个性化学习路径设计
...
### [57] [Kernel-Based Learning of Chest X-ray Images for Predicting ICU Escalation among COVID-19 Patients](https://arxiv.org/abs/2602.10261)
...
<details>
  <summary>Details</summary>
Motivation: 传统单核方法在处理复杂非线性数据时存在局限性，而现有的多核学习方法主要针对连续型结果变量，无法处理更广泛的指数族分布数据类型。

Method: 提出GLIMARK方法，将多核学习扩展到广义线性模型框架，支持指数族分布，通过组合多个简单核函数来整合异质数据源的信息。

Result: GLIMARK能够有效恢复或近似真实数据生成机制，在COVID-19胸部X光数据集上成功预测ICU升级的二元结果，并提取了具有临床意义的特征。

Conclusion: GLIMARK扩展了多核学习的应用范围，使其能够处理更广泛的数据类型，在实际医疗场景中展示了实用价值。
...
### [58] [Linear-LLM-SCM: Benchmarking LLMs for Coefficient Elicitation in Linear-Gaussian Causal Models](https://arxiv.org/abs/2602.10282)
...
TL;DR: Linear-LLM-SCM：一个用于评估大语言模型在线性高斯结构因果模型参数化能力的基准框架，当DAG已知时


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在识别定性因果关系方面表现出潜力，但它们在连续领域进行定量因果推理（估计效应大小参数化函数关系）的能力尚未得到充分探索

Method: 提出Linear-LLM-SCM框架，将DAG分解为局部父子集，提示LLM为每个节点生成回归式结构方程，然后聚合并与真实参数进行比较

Result: 实验显示基准测试中的几个挑战：某些模型结果具有强随机性，在连续领域容易受到虚假边导致的DAG错误设定的影响。不同模型间系数估计存在显著变异性，对结构和语义扰动敏感

Conclusion: LLM作为定量因果参数化工具目前存在局限性，作者开源了基准框架供研究者使用自己的DAG和现成LLM进行轻松评估
...
### [59] [Configuration-to-Performance Scaling Law with Neural Ansatz](https://arxiv.org/abs/2602.10300)
*Huaqing Zhang,Kaiyue Wen,Tengyu Ma*

Main category: cs.LG

TL;DR: 提出神经配置-性能缩放定律（NCPL），使用LLM建模完整训练配置到性能的映射，相比Chinchilla定律预测误差降低20-40%，能泛化到训练集10倍计算量的运行


<details>
  <summary>Details</summary>
Motivation: 现有缩放定律假设超参数已最优选择，这需要大量调优工作且受硬件限制。需要更全面的配置-性能映射来提高可预测性和简化大规模调优

Method: 提出配置-性能缩放定律（CPL）概念，使用大语言模型（LLM）参数化完整训练配置到训练性能的映射，基于多来源开源预训练日志进行拟合

Result: NCPL能准确预测训练配置对最终预训练损失的影响，预测误差比配置无关的Chinchilla定律低20-40%，能泛化到训练集10倍计算量的运行，支持多超参数联合调优

Conclusion: NCPL为训练配置到性能提供了更准确、更通用的预测框架，能有效扩展到更丰富的预测目标如损失曲线预测，简化大规模模型训练的调优过程
...
### [60] [Confounding Robust Continuous Control via Automatic Reward Shaping](https://arxiv.org/abs/2602.10305)
*Mateo Juliani,Mingxuan Li,Elias Bareinboim*

Main category: cs.LG

TL;DR: 本文提出一种从离线数据集中自动学习连续控制问题奖励塑造函数的方法，该方法基于因果贝尔曼方程学习最优状态值的紧上界，并将其用作基于势能的奖励塑造框架中的势函数。


<details>
  <summary>Details</summary>
Motivation: 奖励塑造被广泛应用于加速强化学习智能体的训练，但目前缺乏设计有效奖励塑造函数的原则性方法，特别是在复杂的连续控制问题中。此外，离线数据集可能受到未观测混杂变量的污染，这给奖励设计带来了额外挑战。

Method: 基于最近提出的因果贝尔曼方程，学习最优状态值的紧上界，然后将该上界用作基于势能的奖励塑造框架中的势函数。该方法能够处理未观测混杂变量，并在Soft-Actor-Critic算法上进行测试。

Result: 在多个常用连续控制基准测试中，提出的奖励塑造算法表现出强大的性能保证，特别是在存在未观测混杂变量的情况下。该方法标志着从因果视角实现混杂鲁棒连续控制的坚实第一步。

Conclusion: 本文提出了一种从因果角度自动学习奖励塑造函数的方法，能够有效处理未观测混杂变量，为连续控制问题提供了新的奖励设计思路，并在多个基准测试中验证了其有效性。
...
### [61] [R2RAG-Flood: A reasoning-reinforced training-free retrieval augmentation generation framework for flood damage nowcasting](https://arxiv.org/abs/2602.10312)
*Lipai Huang,Kai Yin,Chia-Fu Liu,Ali Mostafavi*

Main category: cs.LG

TL;DR: R2RAG-Flood是一个无需训练、基于推理增强的检索增强生成框架，用于风暴后财产损害临近预报。它利用现有监督表格预测器构建知识库，通过检索相关推理轨迹来指导大语言模型进行预测，无需特定任务训练。


<details>
  <summary>Details</summary>
Motivation: 现有监督表格预测器虽然有效但缺乏可解释性，且需要特定任务训练。需要开发一个无需训练、具有可解释性且能接近监督模型性能的财产损害预测框架。

Method: 基于现有监督表格预测器构建推理中心知识库，包含结构化预测变量、文本摘要和模型生成的推理轨迹。推理时检索地理邻近和典型类别原型的相关推理轨迹，通过上下文增强提示指导大语言模型模仿先前推理。采用两阶段预测：先确定损害发生，然后在三级损害程度分类中细化严重程度，包含条件降级步骤纠正过度预测。

Result: 在德克萨斯州哈里斯县的案例研究中，监督表格基线总体准确率为0.714，中高损害类别准确率为0.859。R2RAG-Flood在七个大语言模型上获得0.613-0.668总体准确率和0.757-0.896损害类别准确率，接近监督基线同时提供结构化推理。轻量级变体在严重程度-成本效率指标上显著优于监督基线和大型语言模型。

Conclusion: R2RAG-Flood无需特定任务训练即可接近监督模型性能，同时提供可解释的推理轨迹，在效率上优于传统方法，为灾害损害预测提供了有前景的免训练替代方案。
...
### [62] [Stop Training for the Worst: Progressive Unmasking Accelerates Masked Diffusion Training](https://arxiv.org/abs/2602.10314)
*Jaeyeon Kim,Jonathan Geuter,David Alvarez-Melis,Sham Kakade,Sitan Chen*

Main category: cs.LG

TL;DR: 提出PUMA方法改进掩码扩散模型，通过渐进式解掩码对齐训练和推理时的掩码模式，加速训练并提升性能


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在离散空间生成建模中表现优异，支持任意顺序生成和并行解码，但存在训练复杂度问题：需要训练指数级数量的掩码模式，计算成本高，且训练时的随机掩码与推理时的结构化掩码存在不匹配

Method: 提出渐进式解掩码方法，修改前向掩码过程，使训练时的掩码模式与推理时对齐，优化集中在推理对齐的掩码上，从而加速训练

Result: 在125M规模上，PUMA将预训练速度提升约2.5倍，并在自回归初始化等常见方法基础上提供互补优势

Conclusion: PUMA通过对齐训练和推理的掩码模式，有效解决了掩码扩散模型的训练复杂度和不匹配问题，显著加速训练过程
...
### [63] [Theoretical Analysis of Contrastive Learning under Imbalanced Data: From Training Dynamics to a Pruning Solution](https://arxiv.org/abs/2602.10357)
*Haixu Liao,Yating Zhou,Songyang Zhang,Meng Wang,Shuai Zhang*

Main category: cs.LG

TL;DR: 论文分析了Transformer编码器在数据不平衡情况下的对比学习训练动态，揭示了神经元权重演变的三个阶段，并发现不平衡会降低表示能力、增加架构复杂度需求、阻碍特征分离。通过剪枝可以恢复性能并增强特征分离。


<details>
  <summary>Details</summary>
Motivation: 对比学习虽然能学习到泛化性强的表示，但其理论理解仍有限，特别是在现实应用中普遍存在的数据不平衡情况下。这种不平衡会降低表示质量并导致模型行为偏差，但缺乏对这些影响的严格理论分析。

Method: 开发了一个理论框架来分析基于Transformer编码器的对比学习在数据不平衡情况下的训练动态。通过理论分析揭示了神经元权重演变的三个阶段，并研究了多数特征、少数特征和噪声的不同动态。

Result: 研究发现：1）神经元权重演变经历三个不同阶段；2）少数特征会降低表示能力；3）需要更复杂的架构来处理不平衡；4）不平衡会阻碍真实特征与噪声的分离；5）剪枝可以恢复因不平衡而降低的性能并增强特征分离。

Conclusion: 该研究为不平衡数据下的对比学习提供了理论框架，揭示了神经元层面的训练动态，并提出了剪枝作为恢复性能和增强特征分离的实用方法，为理解不平衡对比学习提供了概念性见解和实践指导。
...
### [64] [Simple LLM Baselines are Competitive for Model Diffing](https://arxiv.org/abs/2602.10371)
*Elias Kempf,Simon Schrodi,Bartosz Cywiński,Thomas Brox,Neel Nanda,Arthur Conmy*

Main category: cs.LG

TL;DR: 论文提出了评估模型差异检测方法的新指标，比较了基于LLM和稀疏自编码器的方法，发现改进的LLM基线在保持性能的同时能发现更抽象的行为差异


<details>
  <summary>Details</summary>
Motivation: 标准LLM评估只能测试设计好的能力，无法发现模型修订间的意外行为差异或新出现的未对齐倾向。现有的模型差异检测方法缺乏系统比较和评估标准

Method: 提出了评估模型差异检测方法的三个关键指标：泛化性、有趣性和抽象程度，并用这些指标比较基于LLM的方法和基于稀疏自编码器的方法

Result: 改进的LLM基线方法与SAE方法表现相当，但通常能发现更抽象的行为差异

Conclusion: 需要建立模型差异检测的评估标准，改进的LLM方法在发现抽象行为差异方面具有优势，为系统比较不同方法提供了框架
...
### [65] [Time-to-Event Transformer to Capture Timing Attention of Events in EHR Time Series](https://arxiv.org/abs/2602.10385)
...
<details>
  <summary>Details</summary>
Motivation: 从大规模时间序列数据中自动发现个性化的序列事件对于精准医疗至关重要，但现有AI模型（如Transformer）大多忽略事件时序和顺序，无法进行因果推理。需要一种能够评估患者特定轨迹"对齐程度"并识别共享模式的方法。

Method: 提出LITT（Timing-Transformer）架构，将时间作为可计算维度，允许模型为候选事件分配"相对时间戳"，在虚拟"相对时间线"上临时对齐序列事件，实现事件时序聚焦的注意力机制。

Result: 在3,276名乳腺癌患者的真实世界纵向电子健康记录数据上验证了可解释性和有效性，成功预测心脏毒性引发的心脏病发病时间。在公共数据集上优于基准和最先进的生存分析方法。

Conclusion: LITT代表了临床AI在精准医疗方面的重要进展，通过关注事件时序和顺序，为个性化临床轨迹解释提供了有效工具。
...
### [66] [Deep learning outperforms traditional machine learning methods in predicting childhood malnutrition: evidence from survey data](https://arxiv.org/abs/2602.10381)
*Deepak Bastola,Yang Li*

Main category: cs.LG

TL;DR: 该研究首次在尼泊尔系统评估了16种机器学习和深度学习算法用于识别5岁以下儿童营养不良，发现TabNet表现最佳，并确定了母亲教育、家庭财富指数和儿童年龄为主要预测因素。
...
Method: 使用尼泊尔2019年多指标类集调查数据，构建综合营养不良指标（结合发育迟缓、消瘦和体重不足），系统比较16种算法（深度学习、梯度提升和传统机器学习），使用10个评估指标，重点关注F1分数和召回率以应对类别不平衡。

Result: TabNet在所有模型中表现最佳，优于支持向量机和AdaBoost分类器。特征重要性分析显示母亲教育、家庭财富指数和儿童年龄是主要预测因素，其次是地理特征、疫苗接种状况和进餐频率。

Conclusion: 研究展示了一个可扩展的基于调查的筛查框架，可用于识别营养不良高风险儿童并指导针对性营养干预，支持可持续发展目标，并为全球类似资源匮乏环境提供可转移的方法模板。
...
### [67] [Affordances Enable Partial World Modeling with LLMs](https://arxiv.org/abs/2602.10390)
*Khimya Khetarpal,Gheorghe Comanici,Jonathan Richens,Jeremy Shar,Fei Xia,Laurent Orseau,Aleksandra Faust,Doina Precup*

Main category: cs.LG

TL;DR: 该研究证明大型语言模型可以作为部分世界模型，通过引入分布鲁棒的可用性概念，从多任务智能体中提取部分模型来显著提高搜索效率。


<details>
  <summary>Details</summary>
Motivation: 虽然预训练大模型被认为包含丰富的世界知识，但直接用于搜索过程效率低且不准确。部分世界模型专注于对与用户意图相关的状态和动作子集进行高质量预测。研究旨在探索大模型是否能作为部分世界模型，并证明智能体实现任务无关、语言条件意图时必然拥有基于可用性的预测性部分世界模型。

Method: 1. 形式化证明智能体实现任务无关、语言条件意图时必然拥有基于可用性的预测性部分世界模型；2. 在多任务设置中引入分布鲁棒的可用性概念；3. 从智能体中提取部分模型以提高搜索效率；4. 在桌面机器人任务中进行实证评估，比较部分世界模型与完整世界模型的性能。

Result: 实证评估显示，基于可用性的部分世界模型显著减少了搜索分支因子，相比完整世界模型获得了更高的奖励。这验证了部分模型在提高搜索效率方面的有效性。

Conclusion: 大型语言模型确实可以作为部分世界模型，通过引入分布鲁棒的可用性概念，从多任务智能体中提取的部分模型能够显著提高搜索效率，在机器人任务中表现出优于完整世界模型的性能。
...
### [68] [Modular Multi-Task Learning for Chemical Reaction Prediction](https://arxiv.org/abs/2602.10404)
*Jiayun Pang,Ahmed M. Zaitoun,Xacobe Couso Cambeiro,Ivan Vulić*

Main category: cs.LG

TL;DR: LoRA微调在有机化学反应预测中表现与全参数微调相当，能有效缓解灾难性遗忘，更好地保持多任务性能，并编码了不同的反应性模式。


<details>
  <summary>Details</summary>
Motivation: 将在大规模有机化学数据上训练的LLMs适应到较小的领域特定反应数据集是化学和药物研发的关键挑战。需要学习新的反应知识同时保持跨相关任务的一般化学理解。

Method: 使用低秩适应（LoRA）作为全参数微调的高效参数替代方案，在USPTO反应类别和具有挑战性的C-H官能化反应上进行评估，基准测试包括前向反应预测、逆合成和试剂预测。

Result: LoRA达到与全参数微调相当的准确性，同时有效缓解灾难性遗忘，更好地保持多任务性能。两种微调方法都能泛化到训练分布之外，产生合理的替代溶剂预测。C-H官能化微调显示LoRA和全参数微调编码了微妙不同的反应性模式。

Conclusion: 随着LLMs规模不断扩大，研究结果突显了模块化、参数高效微调策略在化学应用中灵活部署的实用性，LoRA能实现更有效的反应特定适应。
...
### [69] [Colorful Talks with Graphs: Human-Interpretable Graph Encodings for Large Language Models](https://arxiv.org/abs/2602.10386)
*Angelo Zangari,Peyman Baghershahi,Sourav Medya*

Main category: cs.LG

TL;DR: 该研究提出了一种将图结构编码为人类可解释的颜色标记的方法，以解决大语言模型在图问题上的挑战，通过Weisfeiler-Lehman相似度类映射到颜色标记，显著提升了LLM在图任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理图问题时面临根本性挑战，因为图任务需要处理显式结构、置换不变性和计算复杂的图关系，这与基于文本的模型表示不匹配。需要找到有效的方法让LLM能够处理图问题。

Method: 提出了一种人类可解释的结构编码策略，用于图到文本的转换。该方法计算Weisfeiler-Lehman相似度类的变体，并将其映射到类似人类的颜色标记而非数字标签。关键见解是：语义上有意义且人类可解释的提示可能比不透明的符号编码更有效地被LLM处理。

Result: 在多个算法和预测性图任务上的实验结果表明，该方法在合成和真实世界数据集上都带来了显著改进。通过捕捉局部和全局范围的依赖关系，该方法特别增强了LLM在需要全局图结构推理的任务上的性能。

Conclusion: 通过将图结构编码为人类可解释的颜色标记，可以有效克服LLM处理图问题的障碍，特别是在需要全局图结构推理的任务上表现优异，为LLM在图领域的应用提供了新思路。
...
### [70] [AI-rithmetic](https://arxiv.org/abs/2602.10416)
*Alex Bie,Travis Dick,Alex Kulesza,Prabhakar Raghavan,Vinod Raman,Sergei Vassilvitskii*

Main category: cs.LG

TL;DR: 研究发现前沿AI模型在基础整数加法运算中存在系统性问题，随着数字位数增加准确率显著下降，主要错误可归因于操作数对齐失败和进位错误两类。


<details>
  <summary>Details</summary>
Motivation: 尽管现代AI系统在高级数学任务上表现出色，但在基础算术运算（如两个数字相加）上却表现不佳，这种矛盾现象值得系统研究。

Method: 通过实证研究，分析前沿模型在整数加法任务上的表现，特别关注随着数字位数增加时的准确率变化，并对错误类型进行系统分类和归因分析。

Result: 所有前沿模型在整数加法任务中随着位数增加准确率显著下降；大多数错误可归为操作数对齐错误和进位错误两类，分别解释了Claude Opus 4.1的87.9%、GPT-5的62.9%和Gemini 2.5 Pro的92.4%的错误；对齐错误常与分词相关，进位错误则表现为独立的随机失败。

Conclusion: AI模型在基础算术运算中存在系统性缺陷，主要错误模式具有高度可解释性，这为改进模型的基础数学能力提供了明确方向。
...
### [71] [Breaking the Curse of Repulsion: Optimistic Distributionally Robust Policy Optimization for Off-Policy Generative Recommendation](https://arxiv.org/abs/2602.10430)
*Jie Jiang,Yusen Huo,Xiangxin Zhan,Changping Wang,Jun Zhang*

Main category: cs.LG

TL;DR: 论文提出DRPO方法解决离线强化学习中低质量数据导致模型崩溃的问题，通过乐观分布鲁棒优化和硬过滤机制，在混合质量推荐基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 基于策略的强化学习在生成式推荐中占主导地位，但在离线历史日志中应用时，低质量数据的主导会导致严重的模型崩溃。现有方法无法协调方差减少和噪声模仿的困境。

Method: 提出分布鲁棒策略优化（DRPO），将目标重新表述为乐观分布鲁棒优化问题，通过硬过滤机制识别并分离噪声行为策略中的潜在高质量分布。

Result: DRPO在混合质量推荐基准上实现了最先进的性能，能够最优地恢复高质量行为，同时严格丢弃导致发散的低质量噪声。

Conclusion: 通过建立排斥优化发散理论，揭示了离线训练中负梯度更新的本质问题，并提出DRPO作为解决方案，成功解决了低质量数据导致的模型崩溃问题。
...
### [72] [Gated Removal of Normalization in Transformers Enables Stable Training and Efficient Inference](https://arxiv.org/abs/2602.10408)
*Andrei Kanavalau,Carmen Amo Alonso,Sanjay Lall*

Main category: cs.LG

TL;DR: TaperNorm是一种用于Transformer的归一化方法，它在训练初期表现如标准归一化，随后逐渐过渡到学习到的样本无关线性映射，最终可以折叠到相邻线性投影中，提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 重新审视Transformer中样本依赖归一化的必要性，探索是否可以在保持训练稳定性的同时消除每令牌统计量，实现归一化层的折叠以提高推理效率。

Method: 提出TaperNorm作为RMSNorm/LayerNorm的替代方案，使用全局门控机制：训练初期门控为1（标准归一化），通过EMA校准缩放分支，然后余弦衰减到0，此时每令牌统计消失，固定缩放可折叠到相邻线性投影中。

Result: TaperNorm在相同设置下匹配归一化基线的性能，同时消除每令牌统计量，使这些层可以在推理时折叠到相邻线性投影中。在效率微基准测试中，折叠内部缩放使最后令牌对数模式吞吐量提高1.22倍。

Conclusion: 研究揭示了输出归一化的关键作用是尺度锚定，防止对数追逐问题。TaperNorm为实现无归一化Transformer迈出一步，同时识别了输出归一化的特殊作用。
...
### [73] [LUCID: Attention with Preconditioned Representations](https://arxiv.org/abs/2602.10410)
*Sai Surya Duvvuri,Nirmal Patel,Nilesh Gupta,Inderjit S. Dhillon*

Main category: cs.LG

TL;DR: LUCID Attention通过引入基于键-键相似度的预处理器来改进标准注意力机制，解决长序列中softmax注意力概率质量分散的问题，在保持相同计算复杂度的同时提升长上下文检索性能。


<details>
  <summary>Details</summary>
Motivation: 标准softmax注意力在长序列场景中存在根本性限制：随着上下文长度增加，softmax函数倾向于将概率质量分散到不相关的token上，导致性能下降。同时，通过降低softmax温度来增强聚焦的方法会因梯度消失而阻碍可学习性。

Method: 提出LUCID Attention架构修改，对注意力概率应用预处理器。该预处理器基于指数化的键-键相似度推导而来，在再生核希尔伯特空间中最小化键之间的重叠，使查询能够在大量键中准确聚焦于重要键，同时保持与标准注意力相同的计算复杂度。
...
### [74] [A Unified Theory of Random Projection for Influence Functions](https://arxiv.org/abs/2602.10449)
*Pingbang Hu,Yuzheng Hu,Jiaqi W. Ma,Han Zhao*

Main category: cs.LG

TL;DR: 该论文研究了通过随机投影（sketching）计算影响函数（influence functions）的理论保证，分析了投影在无正则化、正则化和结构化近似等不同设置下如何保持影响函数的准确性。


<details>
  <summary>Details</summary>
Motivation: 在现代过参数化模型中，计算影响函数需要处理高维曲率矩阵F，其形成和求逆计算代价高昂。虽然随机投影技术被广泛使用，但缺乏理论保证来解释投影在矩阵求逆、正则化和结构化近似等复杂操作下的行为。

Method: 开发统一理论分析投影何时能保持影响函数：1）无正则化投影：分析投影在F值域上的单射性条件；2）正则化投影：分析岭正则化如何改变投影障碍，近似保证由F在正则化尺度下的有效维度决定；3）因子化影响：分析Kronecker分解曲率下的解耦投影；4）超出值域情况：分析测试梯度在F核空间分量引起的泄漏项。

Result: 建立了投影保持影响函数的理论条件：1）无正则化时，需要投影在F值域上单射，即m≥rank(F)；2）正则化时，近似保证由有效维度决定；3）Kronecker分解曲率下，解耦投影即使违反i.i.d.假设仍保持保证；4）量化了测试梯度在F核空间分量引起的泄漏项。

Conclusion: 该工作建立了投影保持影响函数的统一理论框架，为实践中选择投影大小提供了原则性指导，解决了现有JL引理无法覆盖的矩阵求逆和正则化等复杂操作的理论空白。
...
### [75] [Constructing Industrial-Scale Optimization Modeling Benchmark](https://arxiv.org/abs/2602.10450)
*Zhong Li,Hongliang Lu,Tao Wei,Wenyu Liu,Yuxuan Chen,Yuan Lan,Fan Zhang,Zaiwen Wen*

Main category: cs.LG

TL;DR: MIPLIB-NL是一个新的基准测试集，用于评估LLM将自然语言需求转换为优化模型的能力，基于真实的工业级混合整数线性规划问题构建，揭示了现有基准测试无法发现的性能问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在将自然语言需求转换为优化模型方面的评估主要基于玩具规模或合成基准，无法反映工业实际问题（具有10^3-10^6个变量和约束）的真实难度，缺乏与真实优化模型对齐的自然语言规范基准。

Method: 通过结构感知的逆向构建方法，从MIPLIB 2017中的真实混合整数线性规划问题出发：(1)从平面求解器公式中恢复紧凑、可重用的模型结构；(2)在统一的模型-数据分离格式下，基于恢复的结构逆向生成自然语言规范；(3)通过专家评审和人-LLM交互进行迭代语义验证和独立重建检查。

Result: 构建了223个一对一重建实例，保留了原始实例的数学内容，同时支持真实的自然语言到优化模型的评估。实验显示，在现有基准上表现良好的系统在MIPLIB-NL上性能显著下降，暴露了在玩具规模下不可见的失败模式。

Conclusion: MIPLIB-NL填补了自然语言优化建模评估的空白，提供了基于真实工业问题的基准测试集，揭示了当前LLM系统在处理复杂优化问题时的局限性，为未来研究提供了更真实的评估平台。
...
TL;DR: ADAlign是一个用于图域适应的自适应分布对齐框架，无需手动指定对齐标准，能自动识别并联合对齐每个迁移任务中最相关的分布差异，在10个数据集和16个迁移任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的图域适应方法通常需要手动选择图元素（如节点属性或结构统计）进行对齐，并依赖手动设计的图滤波器提取相关特征。这种方法不够灵活，依赖于场景特定的启发式规则，且在不同迁移场景中主导差异变化时效果不佳。

Method: 提出了ADAlign自适应分布对齐框架，引入神经谱差异（NSD）作为理论上有原则的参数化距离。NSD在谱域中利用神经特征函数编码所有阶的特征-结构依赖关系，同时通过可学习的频率采样器以极小极大范式自适应地强调每个任务中最具信息量的谱成分。

Result: 在10个数据集和16个迁移任务上的广泛实验表明，ADAlign不仅优于最先进的基线方法，而且实现了更高的效率，具有更低的内存使用和更快的训练速度。

Conclusion: ADAlign通过自适应识别和联合对齐图域适应中最相关的分布差异，提供了一种灵活、场景感知且对多样化和动态演化偏移具有鲁棒性的解决方案，无需手动指定对齐标准。
...
### [77] [Binary Flow Matching: Prediction-Loss Space Alignment for Robust Learning](https://arxiv.org/abs/2602.10420)
*Jiadong Hong,Lei Liu,Xinyu Bian,Wenjie Wang,Zhaoyang Zhang*

Main category: cs.LG

TL;DR: 该论文研究流匹配在二进制流形上的应用，发现信号空间预测（x-预测）与速度目标（v-损失）结合时存在结构不匹配问题，提出预测-损失对齐作为流匹配训练的必要条件，为二进制域上的稳健扩散学习提供理论基础和实践指南。


<details>
  <summary>Details</summary>
Motivation: 流匹配在生成建模中表现出强大能力，但现有研究主要关注连续数据。本文旨在将这一范式转移到二进制流形这一离散数据生成建模的基本设置，探索在二进制域上稳健的流匹配方法。

Method: 1. 识别x-预测与v-损失结合时的潜在结构不匹配问题，发现这会引入时间相关的奇异权重，放大梯度对近似误差的敏感性；2. 形式化预测-损失对齐作为流匹配训练的必要条件；3. 证明将目标重新对齐到信号空间（x-损失）可以消除奇异权重，产生均匀有界梯度；4. 在确保对齐的基础上，研究二进制数据特有的设计选择，比较概率目标（如交叉熵）和几何损失（如均方误差）的拓扑依赖差异。

Result: 1. 理论证明预测-损失对齐可以消除时间相关的奇异权重，实现均匀有界的梯度；2. 使训练能够在均匀时间步采样下稳健进行，无需依赖启发式调度；3. 揭示了二进制数据中概率目标与几何损失之间的拓扑依赖区别；4. 为二进制及相关离散域上的稳健流匹配提供了理论基础和实践指南。

Conclusion: 信号空间对齐是二进制及类似离散域上稳健扩散学习的关键原则。通过确保预测与损失对齐，可以消除训练中的结构不匹配问题，实现稳健的训练过程。该研究为离散数据生成建模中的流匹配应用提供了重要的理论洞见和实用指导。
...
### [78] [Low-Dimensional Execution Manifolds in Transformer Learning Dynamics: Evidence from Modular Arithmetic Tasks](https://arxiv.org/abs/2602.10496)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: 研究发现过参数化Transformer模型在训练过程中会快速坍缩到3-4维的低维执行流形上，揭示了Transformer学习的统一几何框架


<details>
  <summary>Details</summary>
Motivation: 研究过参数化Transformer模型学习动态的几何结构，特别是在模块化算术任务中，探索高维参数空间中的训练轨迹特征

Method: 通过精心设计的模块化算术任务，分析Transformer训练轨迹在高维参数空间(d=128)中的几何结构，研究维数坍缩现象

Result: 训练轨迹快速坍缩到3-4维低维执行流形；注意力集中表现为流形内路由坐标饱和；SGD在投影到执行子空间时呈现近似可积动态；稀疏自编码器能捕获辅助路由结构但无法隔离执行本身

Conclusion: Transformer学习的统一几何框架：大部分参数用于吸收优化干扰，核心计算发生在显著缩减的子空间中，这对可解释性、训练课程设计和理解过参数化作用有重要意义
...
### [79] [QTALE: Quantization-Robust Token-Adaptive Layer Execution for LLMs](https://arxiv.org/abs/2602.10431)
*Kanghyun Noh,Jinheon Choi,Yulwha Kim*

Main category: cs.LG

TL;DR: QTALE是一个新颖的框架，通过引入训练策略和后训练机制，实现了token自适应层执行与量化的无缝集成，在保持精度的同时减少计算量和内存占用。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要大量计算和内存资源，现有token自适应层执行和量化技术单独使用有效，但简单结合会导致精度下降，因为token自适应模型减少了冗余性。

Method: QTALE包含两个关键组件：1）训练策略确保在微调期间探索多样化的执行路径；2）后训练机制允许在推理时灵活调整执行比例以重新引入冗余。

Result: 实验结果显示QTALE能够无缝集成token自适应层执行与量化，与仅量化模型相比精度差距保持在0.5%以下（在CommonsenseQA基准测试中），没有明显的精度差异。

Conclusion: QTALE通过结合token自适应执行减少计算量和量化减少内存占用，为高效部署LLM提供了有效的解决方案。
...
### [80] [A Swap-Adversarial Framework for Improving Domain Generalization in Electroencephalography-Based Parkinson's Disease Prediction](https://arxiv.org/abs/2602.10528)
*Seongwon Jin,Hanseul Choi,Sunggu Yang,Sungho Park,Jibum Kim*

Main category: cs.LG

TL;DR: 该研究提出了首个帕金森病预测的可重复基准数据集，并开发了Swap-Adversarial Framework (SAF)来应对ECoG数据中的高主体间变异性和HDLSS问题，实现了跨主体、跨会话和跨数据集的稳健性能。


<details>
  <summary>Details</summary>
Motivation: ECoG在帕金森病早期预测方面比传统EEG更有优势，但缺乏公开基准数据集和伦理限制阻碍了可重复比较。研究旨在填补这一空白，提供可重复基准并解决ECoG数据中的主体间变异性和高维低样本问题。

Method: 提出了Swap-Adversarial Framework (SAF)，包含三个核心组件：1) 稳健预处理；2) 主体间平衡通道交换(ISBCS)进行跨主体数据增强；3) 领域对抗训练抑制主体特异性偏差。ISBCS通过随机交换不同主体间的通道来减少主体间变异性。

Result: 在跨主体、跨会话和跨数据集设置下，该方法在所有基准测试中均表现最优，在高度变异环境中改进最显著。在公开EEG基准上的跨数据集性能也表现优异，展示了从ECoG到EEG数据的强泛化能力。

Conclusion: 该研究提供了首个帕金森病预测的ECoG基准数据集，提出的SAF框架有效解决了ECoG数据中的主体间变异性和HDLSS问题，实现了稳健的跨领域泛化，为帕金森病早期诊断提供了新工具。
...
### [81] [Chamfer-Linkage for Hierarchical Agglomerative Clustering](https://arxiv.org/abs/2602.10444)
*Kishen N Gowda,Willem Fletcher,MohammadHossein Bateni,Laxman Dhulipala,D Ellis Hershkowitz,Rajesh Jayaram,Jakub Łącki*

Main category: cs.LG

TL;DR: 本文提出了一种基于Chamfer距离的新型层次凝聚聚类链接函数Chamfer-linkage，相比传统链接函数在多个数据集上表现更优且保持O(n²)时间复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统层次凝聚聚类（HAC）中常用的链接函数（如单链接、平均链接、Ward方法）在实际应用中表现不稳定，质量参差不齐，缺乏一致的高质量聚类结果。HAC本身不优化明确的全局目标函数，因此链接函数的选择对聚类质量至关重要。

Method: 提出Chamfer-linkage链接函数，使用机器学习中广泛应用的Chamfer距离来衡量簇间距离。该方法满足理想的概念表示特性，且理论分析表明其时间复杂度为O(n²)，与传统链接函数相同。

Result: 实验结果表明，Chamfer-linkage在多个多样化数据集上相比平均链接和Ward方法等传统链接函数，能够产生更高质量的聚类结果。该方法可作为传统链接函数的实用替代方案。

Conclusion: Chamfer-linkage是一种有效的层次凝聚聚类链接函数，扩展了层次聚类的理论工具集，并在实践中表现出优于传统方法的性能。
...
### [82] [LLM-Based Scientific Equation Discovery via Physics-Informed Token-Regularized Policy Optimization](https://arxiv.org/abs/2602.10576)
*Boxiao Wang,Kai Li,Tianyi Liu,Chen Li,Junzhe Wang,Yifan Zhang,Jian Cheng*

Main category: cs.LG

TL;DR: 提出PiT-PO框架，通过强化学习将LLM转变为自适应符号回归生成器，结合物理约束和token级正则化，在标准基准和流体动力学湍流建模中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有符号回归方法主要将LLM作为静态生成器，仅通过提示引导探索，无法基于搜索反馈更新模型内部表示，导致生成物理不一致或数学冗余的表达式。

Method: 提出PiT-PO框架，通过强化学习将LLM演化为自适应生成器。核心是双重约束机制：严格强制执行分层物理有效性，同时应用细粒度的token级惩罚来抑制冗余结构。

Result: 在标准基准测试中达到最先进性能，成功为具有挑战性的流体动力学问题发现新的湍流模型。小规模模型能够超越闭源大型模型，实现高性能科学发现的民主化。

Conclusion: PiT-PO框架通过将LLM与强化学习结合，实现了物理一致且结构简洁的方程发现，为科学发现提供了高效且可访问的新方法。
...
### [83] [Driving Reaction Trajectories via Latent Flow Matching](https://arxiv.org/abs/2602.10476)
*Yili Shen,Xiangliang Zhang*

Main category: cs.LG

TL;DR: LatentRxnFlow提出了一种新的反应预测范式，将反应建模为以热力学产物状态为锚点的连续潜在轨迹，结合了强预测准确性和改进的可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前反应预测模型虽然准确率高，但大多将任务简化为从反应物到产物的一步映射，无法揭示反应过程。而逐步生成方法又需要机制特定的监督、离散符号编辑和计算昂贵的推理。

Method: 基于条件流匹配构建LatentRxnFlow，直接从标准反应物-产物对学习时间依赖的潜在动力学，无需机制注释或中间标签。将反应建模为连续潜在轨迹。

Result: 在USPTO基准测试中达到最先进性能，更重要的是连续公式化暴露了完整的生成轨迹，实现了轨迹级诊断、失败模式定位、门控推理纠错，以及基于轨迹几何特性的认知不确定性量化。
...
### [84] [VESPO: Variational Sequence-Level Soft Policy Optimization for Stable Off-Policy LLM Training](https://arxiv.org/abs/2602.10693)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型强化学习中，策略陈旧、异步训练以及训练与推理引擎不匹配会导致行为策略与当前策略偏离，引发训练崩溃风险。重要性采样虽然提供了理论上的修正，但存在高方差问题，现有方法缺乏统一的理论基础。

Method: 提出VESPO方法，将方差减少纳入提案分布的变分公式中，推导出直接在序列级重要性权重上操作的闭式重塑核，无需长度归一化。

Result: 在数学推理基准测试中，VESPO在陈旧比高达64倍和完全异步执行下仍能保持训练稳定，在密集模型和混合专家模型上均能带来一致的性能提升。

Conclusion: VESPO为大语言模型强化学习提供了一种理论上有基础、实践上稳定的训练方法，有效解决了策略陈旧和异步训练带来的挑战。
...
### [85] [Enhancing Ride-Hailing Forecasting at DiDi with Multi-View Geospatial Representation Learning from the Web](https://arxiv.org/abs/2602.10502)
*Xixuan Hao,Guicheng Li,Daiqiang Wu,Xusen Guo,Yumeng Zhu,Zhichao Zou,Peng Zhen,Yao Yao,Yuxuan Liang*

Main category: cs.LG

TL;DR: MVGR-Net：一种用于网约车预测的两阶段多视图地理空间表示学习框架，通过整合POI和时空移动模式学习地理空间表示，并利用提示微调大语言模型结合外部事件进行预测


<details>
  <summary>Details</summary>
Motivation: 网约车服务改变了城市出行模式，准确预测对优化乘客体验和交通效率至关重要。但现有方法面临地理空间异质性和对外部事件高度敏感的挑战

Method: 提出MVGR-Net两阶段框架：1）预训练阶段整合兴趣点和时空移动模式，从语义属性和时空移动模式两个视图学习地理空间表示；2）预测阶段通过提示微调大语言模型，并融入外部事件信息

Result: 在滴滴真实数据集上的大量实验表明，该方法达到了最先进的性能

Conclusion: MVGR-Net通过多视图地理空间表示学习和提示微调大语言模型，有效解决了网约车预测中的地理空间异质性和外部事件敏感性问题
...
### [86] [Interpretable Graph-Level Anomaly Detection via Contrast with Normal Prototypes](https://arxiv.org/abs/2602.10708)
*Qiuran Zhao,Kai Ming Ting,Xinpeng Li*

Main category: cs.LG

TL;DR: ProtoGLAD是一个基于原型的图级异常检测框架，通过对比异常图与最近正常原型图提供可解释的异常检测结果。


<details>
  <summary>Details</summary>
Motivation: 现有深度图级异常检测方法存在黑盒问题，缺乏可解释性。虽然有些方法尝试提供解释，但要么没有参考正常图，要么依赖抽象的潜在向量而非具体的数据集图。

Method: 提出ProtoGLAD框架：1) 使用点集核迭代发现多个正常原型图及其关联簇；2) 将与所有正常簇距离较远的图识别为异常；3) 通过对比异常图与其最近正常原型图提供解释。

Result: 在多个真实数据集上的实验表明，ProtoGLAD在异常检测性能上与最先进的GLAD方法相当，同时提供更好的人类可理解的原型解释。

Conclusion: ProtoGLAD是一个可解释的无监督框架，能够提供基于原型的异常解释，提高了图级异常检测的可信度和实际应用价值。
...
### [87] [Don't Eliminate Cut: Exponential Separations in LLM-Based Theorem Proving](https://arxiv.org/abs/2602.10512)
*Sho Sonoda,Shunta Akiyama,Yuya Uezato*

Main category: cs.LG

TL;DR: 该论文为LLM引导的形式定理证明建立了理论框架，通过建模为有限时域确定性MDP中的随机策略，分析了分层学习相对于平面学习的指数级优势。


<details>
  <summary>Details</summary>
Motivation: 解释LLM引导的形式定理证明中，最坏情况的理论困难与实际经验成功之间的差距，为智能体定理证明器中的子目标分解提供理论依据。

Method: 将策略建模为紧致度量空间上的Lipschitz策略，引入由参考策略生成的问题分布，包括具有可重用引理/草图结构的潜在变量模型，在top-k搜索协议和Tsybakov型边界条件下进行分析。

Result: 推导出有限时域成功概率的下界，可分解为搜索和学习项；主要分离结果表明：当消割将深度D的DAG扩展为规模Ω(Λ^D)的无割树，而割感知分层过程规模为O(λ^D)且λ≪Λ时，平面学习器需要指数级更多数据。

Conclusion: 该理论分析为最近智能体定理证明器中的子目标分解提供了原则性证明，解释了分层学习在形式定理证明中的指数优势。
...
### [88] [Prioritize the Process, Not Just the Outcome: Rewarding Latent Thought Trajectories Improves Reasoning in Looped Language Models](https://arxiv.org/abs/2602.10520)
*Williams Jonathan,Tureci Esin*

Main category: cs.LG

TL;DR: RLTT（奖励潜在思维轨迹）是一种强化学习框架，通过在整个潜在推理轨迹上分配奖励来改进LoopLM的推理能力，相比只奖励最终状态的GRPO方法有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习方法如GRPO只对最终潜在状态分配奖励，这与LoopLM的内部多步推理计算存在根本性不匹配，导致强化学习在改进LoopLM推理方面失败。

Method: 提出RLTT框架，将奖励分布到完整的潜在推理轨迹上，提供密集的轨迹级信用分配，无需依赖外部验证器，可直接替换GRPO且开销极小。

Result: 在相同训练和推理条件下，RLTT相比GRPO在数学推理基准上取得显著改进：MATH-500准确率提升14.4%，AIME24提升16.6%，BeyondAIME提升10.0%。尽管仅在数学上训练，RLTT也能有效迁移到非数学推理任务。

Conclusion: 轨迹级信用分配对于LoopLM中的强化学习是有效的，RLTT框架解决了传统方法只奖励最终状态的不匹配问题，显著提升了推理性能并具有良好的迁移能力。
...
TL;DR: SnapMLA是一个针对DeepSeek MLA架构解码阶段的FP8优化框架，通过硬件感知的算法-内核协同优化技术，在长上下文任务中实现高达1.91倍的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: FP8注意力机制在FlashAttention-3等创新中展现出潜力，但将其集成到DeepSeek MLA架构的解码阶段面临挑战：位置嵌入解耦导致的数值异质性、FP8 PV GEMM中量化尺度错位，以及需要优化的系统级支持。

Method: 采用三种硬件感知的算法-内核协同优化技术：1) RoPE感知的每令牌KV量化，保持RoPE部分高精度；2) 量化PV计算管道重构，解决MLA KV缓存共享结构导致的量化尺度错位；3) 端到端数据流优化，使用专用内核建立高效数据读写工作流。

Result: 在先进的MLA LLMs上进行广泛实验，SnapMLA在具有挑战性的长上下文任务（包括数学推理和代码生成基准测试）中实现高达1.91倍的吞吐量提升，性能退化风险可忽略不计。

Conclusion: SnapMLA成功解决了FP8在MLA解码阶段的集成挑战，通过创新的量化策略和系统优化，显著提升了长上下文处理的效率，为高效LLM推理提供了实用解决方案。
...
### [90] [Rotary Positional Embeddings as Phase Modulation: Theoretical Bounds on the RoPE Base for Long-Context Transformers](https://arxiv.org/abs/2602.10959)
*Feilong Liu*

Main category: cs.LG

TL;DR: 该研究重新解释RoPE为相位调制，推导出保持长上下文位置连贯性的下界（防混叠和稳定性）和上界（浮点精度限制），定义了长上下文Transformer的"金发姑娘区域"。


<details>
  <summary>Details</summary>
Motivation: 虽然RoPE在大语言模型中广泛用于编码token位置，但其在长上下文长度下的行为特征尚不清楚。研究旨在通过信号处理理论分析RoPE在长上下文中的表现，为模型设计提供理论指导。

Method: 将RoPE重新解释为应用于复数振荡器组的相位调制，运用经典信号处理理论进行分析。推导出保持位置连贯性的下界（包括类似奈奎斯特极限的混叠边界和低频位置模式的相位漂移稳定性边界），并扩展到深层Transformer。同时推导出由有限浮点分辨率导致的精度相关上界。

Result: 建立了精度和深度相关的可行性区域（金发姑娘区域）。通过对LLaMA、Mistral、DeepSeek等最先进模型的案例研究验证了框架，显示观察到的成功、失败和社区改造与预测边界高度一致。违反稳定性边界的模型表现出注意力崩溃和长距离退化，而尝试扩展到百万token以上会遇到与架构或训练无关的硬精度墙。

Conclusion: 该研究为长上下文Transformer中的RoPE参数选择提供了理论框架，定义了确保位置连贯性的下界和避免数值精度问题的上界，为模型设计和扩展提供了重要指导。
...
### [91] [What Makes Value Learning Efficient in Residual Reinforcement Learning?](https://arxiv.org/abs/2602.10539)
*Guozheng Ma,Lu Li,Haoyu Wang,Zixuan Liu,Pierre-Luc Bacon,Dacheng Tao*

Main category: cs.LG

TL;DR: 本文提出DAWN方法解决残差强化学习中价值学习的两个关键瓶颈：冷启动问题和结构尺度不匹配问题，通过基策略转换作为价值锚点和评论家归一化来提升学习效率。


<details>
  <summary>Details</summary>
Motivation: 残差强化学习能够通过冻结基策略并学习有界修正来稳定在线精炼预训练策略，但其价值学习存在独特挑战，特别是冷启动病理（评论家缺乏基策略周围价值景观知识）和结构尺度不匹配（残差贡献被基动作淹没）这两个关键瓶颈。

Method: 提出DAWN方法，包含两个核心组件：1）使用基策略转换作为隐式预热的价值锚点，解决冷启动问题；2）采用评论家归一化有效恢复表示敏感性，使系统能够辨别价值差异，解决结构尺度不匹配问题。

Result: DAWN方法在多样化基准测试、策略架构和观察模态中展示了显著的效率提升，通过解决这两个瓶颈实现了高效的价值学习。

Conclusion: 残差强化学习中的价值学习瓶颈可以通过简单而原则性的解决方案有效解决，基策略转换作为价值锚点和评论家归一化是提升残差RL效率的关键机制。
...
### [92] [ROCKET: Rapid Optimization via Calibration-guided Knapsack Enhanced Truncation for Efficient Model Compression](https://arxiv.org/abs/2602.11008)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型压缩方法（如因子分解、结构化稀疏化和动态压缩）在性能和效率方面存在局限，需要一种无需训练、能全局优化压缩分配且保持高性能的压缩方法。
...
Conclusion: ROCKET是一种高效的无训练模型压缩方法，通过全局优化压缩分配和创新的单步稀疏分解技术，在保持高性能的同时显著减少模型参数，为大规模模型部署提供了实用解决方案。
...
### [93] [Transport, Don't Generate: Deterministic Geometric Flows for Combinatorial Optimization](https://arxiv.org/abs/2602.10794)
*Benjy Friedmann,Nadav Dym*

Main category: cs.LG

TL;DR: CycFlow是一种用于神经组合优化的新框架，用确定性点传输替代迭代边去噪，通过将2D坐标连续传输到规范圆形排列来求解旅行商问题，速度比现有扩散模型快3个数量级。


<details>
  <summary>Details</summary>
Motivation: 当前神经组合优化领域主要被扩散模型主导，这些模型将欧几里得旅行商问题视为随机热图生成任务，存在计算效率低的问题。需要一种更高效的求解方法。

Method: CycFlow学习一个实例条件化的向量场，将输入的2D坐标连续传输到规范的圆形排列，然后通过角度排序从2N维表示中恢复最优路径。采用数据依赖的流匹配方法，避免了边评分的二次复杂度。

Result: 相比最先进的扩散基线，CycFlow将求解速度提升了3个数量级，同时保持了有竞争力的最优性差距。

Conclusion: 通过用确定性点传输替代迭代边去噪，CycFlow实现了从二次复杂度到线性复杂度的范式转变，为神经组合优化提供了更高效的求解框架。
...
### [94] [Bridging the Compression-Precision Paradox: A Hybrid Architecture for Clinical EEG Report Generation with Guaranteed Measurement Accuracy](https://arxiv.org/abs/2602.10544)
*Wuyang Zhang,Zhen Luo,Chuqiao Gu,Jianming Ma,Yebo Cao,Wangming Yuan,Yinzhi Jin*

Main category: cs.LG

TL;DR: 提出混合架构解决EEG自动监测中LLM因极端压缩导致时间精度丢失和测量值幻觉的问题，通过信号处理提取精确临床值，实现首个保证临床测量精度的自动EEG报告系统


<details>
  <summary>Details</summary>
Motivation: 临床EEG监测需要医生级别的精度，但现有LLM方法面临两个核心问题：1) EEG记录超出LLM上下文窗口，需要400:1以上的极端压缩，破坏细粒度时间精度；2) LLM缺乏时间序列理解能力，依赖压缩表示的统计关联，导致产生临床错误的测量值幻觉

Method: 提出混合架构：1) 信号处理提取精确临床值（压缩前）；2) 跨模态桥接实现EEG到语言的翻译；3) 参数高效微调配合约束解码；4) 多速率采样保持长范围上下文同时保留事件级精度

Result: 在TUH和CHB-MIT数据集上评估：误报减少60%，检测速度提升50%，达到亚临床测量精度，是首个保证临床测量准确性的自动EEG报告系统

Conclusion: 通过分离测量提取和文本生成，解决了LLM在EEG分析中的根本限制，实现了临床可用的自动EEG监测系统，为医疗时间序列分析提供了新范式
...
### [95] [Enhancing Multivariate Time Series Forecasting with Global Temporal Retrieval](https://arxiv.org/abs/2602.10847)
*Fanpu Cao,Lu Dai,Jindong Han,Hui Xiong*

Main category: cs.LG

TL;DR: GTR是一个轻量级即插即用模块，通过维护全局时间嵌入和动态检索对齐相关全局片段，有效扩展时间序列预测模型的全局周期性建模能力，无需改变原模型架构。
...
Method: GTR维护整个周期的自适应全局时间嵌入，动态检索并输入序列对齐相关全局片段，通过2D卷积和残差融合联合建模局部和全局依赖关系。

Result: 在六个真实世界数据集上的实验表明，GTR在短期和长期预测场景中均能持续提供最先进的性能，同时引入最小的参数和计算开销。

Conclusion: GTR是一种高效通用的解决方案，能够增强多元时间序列预测任务中的全局周期性建模能力，且无需改变宿主模型架构。
...
TL;DR: 提出一种无需训练、基于token级别的早期停止方法，通过轻量级信号动态判断每个token何时收敛，从而减少扩散语言模型的生成步骤，提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过迭代细化生成文本，但许多token在最终去噪步骤之前很早就达到稳定状态，导致计算效率低下。

Method: 提出训练免费的token级早期停止方法，利用模型预测和局部上下文的轻量级信号，独立识别每个位置的收敛情况，动态确定何时可以最终确定单个token。

Result: 在数学推理、通用问答和科学理解等多样化基准测试中，该方法实现了最先进的效率提升，同时保持了生成质量。

Conclusion: 该方法通过自适应token冻结显著减少了所需的总扩散步骤数，无需任务特定的微调，为扩散语言模型提供了高效的生成方案。
...
### [97] [Time Series Foundation Models for Energy Load Forecasting on Consumer Hardware: A Multi-Dimensional Zero-Shot Benchmark](https://arxiv.org/abs/2602.10848)
*Luigi Simeone*

Main category: cs.LG

TL;DR: 该研究评估了四种时间序列基础模型在电力需求预测中的零样本能力，发现它们在长上下文长度下表现优异（MASE约0.31），比季节性朴素基准提升47%，且在小样本情况下比传统方法更稳定。


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型虽然具备零样本预测能力，但其在电力需求预测等关键任务中的实际表现（包括准确性、校准性和鲁棒性）尚未得到充分验证，这直接关系到电网运营的可靠性。

Method: 使用ERCOT 2020-2024年每小时负荷数据，在消费级硬件上评估四种TSFM模型（Chronos-Bolt、Chronos-2、Moirai-2、TinyTimeMixer），以Prophet作为行业基准，SARIMA和季节性朴素作为统计参考。评估包括四个维度：上下文长度敏感性（24-2048小时）、概率预测校准、分布偏移鲁棒性（COVID-19封锁和冬季风暴Uri）以及操作决策支持。

Result: 最佳基础模型在长上下文长度（C=2048h，日前预测）下达到MASE约0.31，比季节性朴素基准提升47%。Prophet在拟合窗口短于季节性周期时表现很差（MASE>74），而TSFM即使在最小上下文下也能保持稳定，因为它们利用了预训练学到的模式。校准性差异显著：Chronos-2校准良好（95%经验覆盖率对应90%名义水平），而Moirai-2和Prophet则过于自信（约70%覆盖率）。

Conclusion: 时间序列基础模型在电力需求预测中展现出实际应用潜力，特别是在小样本和长上下文场景下优于传统方法。研究提供了实用的模型选择指南并发布了完整的基准框架，为关键任务应用中的模型部署提供了重要参考。
...
### [98] [Online Min-Max Optimization: From Individual Regrets to Cumulative Saddle Points](https://arxiv.org/abs/2602.10565)
*Abhijeet Vyas,Brian Bullins*

Main category: cs.LG

TL;DR: 该论文研究了在线min-max优化的新性能度量，提出了静态对偶间隙和动态鞍点遗憾的概念，并在强凸-强凹、min-max指数凹性和PL条件下给出了算法性能保证。


<details>
  <summary>Details</summary>
Motivation: 传统静态纳什均衡在在线min-max优化中与个体遗憾不兼容，需要开发新的性能度量来评估在线min-max算法在非凸凹设置下的表现。

Method: 提出静态对偶间隙(SDual-Gap_T)和动态鞍点遗憾(DSP-Reg_T)作为新的性能度量，通过将问题约简到经典在线凸优化问题来设计算法，并在强凸-强凹、min-max指数凹性和PL条件下分析性能。

Result: 证明了在强凸-强凹和min-max指数凹性条件下，算法能够获得SDual-Gap_T和DSP-Reg_T的界；建立了满足min-max指数凹性的函数类，包含双玩家投资组合选择问题；在PL条件下获得了与个体遗憾兼容的动态遗憾界。

Conclusion: 论文为在线min-max优化提出了新的性能度量框架，在不同函数类下提供了算法性能保证，扩展了传统凸凹设置下的分析，为更广泛的在线优化问题提供了理论基础。
...
TL;DR: 研究发现预训练中较大的权重衰减值能增强模型的可塑性，使模型在下游任务微调时获得更大性能提升，揭示了超参数优化应考虑下游适应性而不仅仅是验证损失。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型开发主要关注预训练基础模型的验证损失，而忽略了模型在下游任务中的适应能力。研究者希望从模型可塑性角度研究预训练，即基础模型通过微调适应下游任务的能力。

Method: 通过系统实验研究权重衰减这一关键正则化参数在预训练中的作用，分析不同权重衰减值对模型可塑性的影响，并深入探究权重衰减对模型行为的机制性影响。

Result: 使用较大权重衰减值训练的模型具有更高的可塑性，在下游任务微调时表现出更大的性能提升。这可能导致反直觉的权衡：预训练后表现较差的基础模型在微调后可能表现更好。权重衰减促进线性可分表示、正则化注意力矩阵并减少训练数据过拟合。

Conclusion: 超参数优化应考虑超越交叉熵损失的评估指标，单个优化超参数在塑造模型行为中扮演多方面的角色。权重衰减不仅影响预训练性能，更重要的是影响模型在下游任务中的适应能力。
...
### [100] [Gauss-Newton Unlearning for the LLM Era](https://arxiv.org/abs/2602.10568)
*Lev McKinney,Anvith Thudi,Juhan Bae,Tara Rezaei,Nicolas Papernot,Sheila A. McIlraith,Roger Grosse*

Main category: cs.LG

TL;DR: 提出K-FADE方法，使用K-FAC近似Hessian矩阵，通过少量上坡高斯-牛顿步骤实现LLM遗忘，在遗忘目标数据集的同时最小化对保留数据集性能的影响


<details>
  <summary>Details</summary>
Motivation: 标准大语言模型训练可能产生部署中不可接受的输出，现有遗忘方法在遗忘特定数据集时会降低模型在其他分布上的性能，需要改善这种权衡关系

Method: 提出K-FADE方法，使用Kronecker-Factored Approximate Curvature（K-FAC）近似Hessian矩阵，通过少量上坡高斯-牛顿步骤实现遗忘，将整个保留集的输出约束转化为权重约束，最小化对保留集行为的影响

Result: 在WMDP和ToFU基准测试中，K-FADE有效抑制遗忘集的输出，在输出空间上近似于无遗忘集重新训练的结果，同时对保留集输出的改变小于先前方法，且遗忘更新可在后续训练中重复应用

Conclusion: K-FADE提供了一种概念简单、性能先进的LLM遗忘方法，通过参数化Hessian近似和上坡高斯-牛顿步骤，在有效遗忘的同时最小化对保留性能的影响，且支持低成本维护遗忘效果
...
### [101] [FedPS: Federated data Preprocessing via aggregated Statistics](https://arxiv.org/abs/2602.10870)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中数据预处理对模型性能至关重要，但现有研究大多忽视此环节。实际联邦系统中，隐私约束禁止集中原始数据，通信效率也给分布式预处理带来挑战。

Method: 基于聚合统计的统一框架，利用数据草图技术高效汇总本地数据集统计信息，设计联邦算法处理特征缩放、编码、离散化、缺失值填补，并扩展k-Means、k-NN、贝叶斯线性回归等预处理相关模型到水平和垂直联邦学习场景。

Result: FedPS为实际联邦学习部署提供了灵活、通信高效且一致的预处理管道，解决了隐私约束下的分布式数据预处理挑战。

Conclusion: FedPS框架填补了联邦学习中数据预处理的研究空白，通过聚合统计方法在保护隐私的同时实现了高效的分布式预处理，为实际联邦学习系统提供了完整的预处理解决方案。
...
### [102] [When Gradient Clipping Becomes a Control Mechanism for Differential Privacy in Deep Learning](https://arxiv.org/abs/2602.10584)
*Mohammad Partohaghighi,Roummel Marcia,Bruce J. West,YangQuan Chen*

Main category: cs.LG

TL;DR: 提出了一种基于控制理论的差分隐私训练剪裁阈值自适应方法，通过权重矩阵谱分析来调整剪裁阈值，避免传统方法依赖梯度统计带来的计算开销和敏感性


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私训练中的梯度剪裁阈值设置存在两难：设置过小会导致优化偏差，设置过大会使噪声主导更新而降低精度。现有自适应剪裁方法依赖每个样本的梯度范数统计，计算开销大且对数据集和架构敏感。

Method: 提出控制驱动的剪裁策略，使用轻量级的仅权重谱诊断来自适应调整阈值。在周期性探测步骤中，通过谱分解分析指定权重矩阵，估计与训练稳定性相关的重尾谱指标。该指标经过时间平滑后输入有界反馈控制器，在log域中乘法更新剪裁阈值。

Result: 该方法仅使用隐私保护训练期间产生的参数，因此阈值更新属于后处理，不会在标准组合计算下增加底层DP优化器的隐私损失。

Conclusion: 提出的控制驱动剪裁策略提供了一种高效、隐私安全的自适应剪裁方法，避免了传统方法对梯度统计的依赖，同时保持了隐私保护训练的有效性。
...
### [103] [Resource-Efficient Model-Free Reinforcement Learning for Board Games](https://arxiv.org/abs/2602.10894)
*Kazuki Ota,Takayuki Osa,Motoki Omura,Tatsuya Harada*

Main category: cs.LG

TL;DR: 提出一种用于棋盘游戏的模型无关强化学习算法，相比AlphaZero等搜索方法更高效，在五种棋盘游戏中验证了学习效率优势


<details>
  <summary>Details</summary>
Motivation: AlphaZero等搜索型强化学习方法在棋盘游戏中取得了显著成功，但计算需求大，可复现性差，需要更高效的替代方案

Method: 提出一种模型无关的强化学习算法，专门为棋盘游戏设计，通过消融研究验证了核心技术的有效性

Result: 在Animal Shogi、Gardner Chess、Go、Hex和Othello五种棋盘游戏中，相比现有方法实现了更高效的学习
...
### [104] [TRACE: Theoretical Risk Attribution under Covariate-shift Effects](https://arxiv.org/abs/2602.10588)
...
<details>
  <summary>Details</summary>
Motivation: 当源域训练的模型Q被在偏移数据上训练的模型~Q替换时，其在源域上的性能可能发生不可预测的变化。需要理解这种风险变化的原因，以便安全地进行模型替换。

Method: 提出TRACE框架，将风险变化|ΔR|分解为四个可解释因素：两个泛化差距、模型变化惩罚和协变量偏移惩罚。通过模型敏感度因子（来自高分位数输入梯度）和数据偏移度量（默认使用特征空间最优传输OT，或最大均值差异MMD）估计协变量偏移惩罚；模型变化惩罚通过目标样本上两个模型的平均输出距离控制；泛化差距在保留数据上估计。

Result: 在线性回归设置中验证TRACE边界正确捕捉了真实风险差异随偏移大小的缩放关系。在合成和视觉基准测试中，TRACE诊断有效且与真实性能退化保持强单调关系。部署门控分数与|ΔR|强相关，在门控决策中实现高AUROC/AUPRC。

Conclusion: TRACE框架为理解模型替换后的风险变化提供了可解释的诊断工具，通过可计算的分解帮助识别性能变化的原因，并支持安全、标签高效的模型替换决策。
...
### [105] [Roughness-Informed Federated Learning](https://arxiv.org/abs/2602.10595)
*Mohammad Partohaghighi,Roummel Marcia,Bruce J. West,YangQuan Chen*

Main category: cs.LG

TL;DR: RI-FedAvg是一种新的联邦学习算法，通过引入粗糙度指数(RI)正则化项来缓解非IID数据下的客户端漂移问题，提高收敛速度和精度。
...
Method: 提出RI-FedAvg算法，在本地目标函数中引入基于粗糙度指数(RI)的正则化项，该指数量化高维损失函数的粗糙度，自适应地惩罚基于损失函数波动性的本地更新。

Result: 在MNIST、CIFAR-10和CIFAR-100数据集上的实验表明，RI-FedAvg优于FedAvg、FedProx、FedDyn、SCAFFOLD和DP-FedAvg等先进基线方法，在非IID场景下实现了更高的准确率和更快的收敛速度。

Conclusion: RI-FedAvg通过粗糙度指数正则化有效缓解了客户端漂移问题，增强了联邦学习在异构环境下的鲁棒性和效率，具有实际应用潜力。
...
### [106] [dnaHNet: A Scalable and Hierarchical Foundation Model for Genomic Sequence Learning](https://arxiv.org/abs/2602.10603)
...
<details>
  <summary>Details</summary>
Motivation: 基因组基础模型面临输入表示的权衡：固定词汇分词器会破坏生物基序（如密码子和调控元件），而核苷酸级模型虽保持生物学连贯性但计算成本过高。需要一种既能保持生物学意义又能高效处理长序列的方法。

Method: dnaHNet采用无需分词器的自回归模型，通过可微分动态分块机制将原始核苷酸自适应压缩为潜在标记，平衡压缩率和预测准确性。使用递归分块实现二次FLOP减少。

Result: 在古细菌基因组上预训练后，dnaHNet在扩展性和效率上超越StripedHyena2等领先架构，推理速度比Transformer快3倍以上。在零样本任务中，在预测蛋白质变异适应性和基因必需性方面表现优异，并能无监督地自动发现层次化生物结构。

Conclusion: dnaHNet为下一代基因组建模提供了一个可扩展、可解释的框架，解决了基因组表示中的基本权衡问题，为解码DNA语法提供了新途径。
...
### [107] [Pupillometry and Brain Dynamics for Cognitive Load in Working Memory](https://arxiv.org/abs/2602.10614)
...
<details>
  <summary>Details</summary>
Motivation: 认知负荷评估在神经科学、心理学和人机交互中至关重要，但现有方法如EEG存在可穿戴性差、成本高等问题，而瞳孔测量虽便携但比较研究不足。现有深度学习模型解释性差且计算成本高，需要更实用的解决方案。

Method: 使用OpenNeuro的'数字广度任务'数据集，结合特征驱动和模型驱动方法进行时间序列分析。采用Catch-22特征提取和经典机器学习模型，与深度学习方法比较，对EEG和瞳孔测量数据进行认知负荷分类（二分类和多分类）。

Result: 基于Catch-22特征的传统机器学习方法在二分类和多分类任务中均优于深度学习。瞳孔测量单独使用即可与EEG的认知负荷分类性能竞争，为便携式监测提供了可行方案。SHAP特征分析提供了生理学上有意义的解释。

Conclusion: 瞳孔测量可作为EEG的有效替代方案，结合可解释模型和特征分析，能够开发出可穿戴、经济实惠的认知监测系统，应用于神经精神病学、教育和医疗保健领域。
...
### [108] [OSIL: Learning Offline Safe Imitation Policies with Safety Inferred from Non-preferred Trajectories](https://arxiv.org/abs/2602.11018)
*Returaj Burnwal,Nirav Pravinbhai Bhatt,Balaraman Ravindran*

Main category: cs.LG

TL;DR: OSIL是一种离线安全模仿学习算法，通过从非偏好轨迹中推断安全约束，无需显式安全成本和奖励标注，学习安全且奖励最大化的策略。


<details>
  <summary>Details</summary>
Motivation: 现实世界中在线学习存在风险，且准确指定安全成本困难，但收集反映不安全行为的轨迹相对容易。这些非偏好轨迹隐含传达了智能体应避免的行为。

Method: 将安全策略学习建模为约束马尔可夫决策过程（CMDP），通过推导奖励最大化目标的下界，并学习估计非偏好行为可能性的成本模型，从离线演示中学习安全策略。

Result: 实验证明OSIL能够学习满足成本约束的安全策略，且不降低奖励性能，优于多个基线方法。

Conclusion: OSIL算法能够仅从离线演示中学习安全且奖励最大化的行为，为现实世界安全关键应用提供了一种有效的离线安全模仿学习方法。
...
### [109] [Generative clinical time series models trained on moderate amounts of patient data are privacy preserving](https://arxiv.org/abs/2602.10631)
*Rustam Zhumagambetov,Niklas Giesa,Sebastian D. Boie,Stefan Haufe*

Main category: cs.LG

TL;DR: 该研究对医疗时间序列生成模型的隐私保护进行审计，发现当训练数据集足够大时，现有隐私攻击对生成的多元临床时间序列无效，且差分隐私机制不会带来隐私改进，只会降低数据效用。


<details>
  <summary>Details</summary>
Motivation: 医疗数据共享面临隐私泄露风险，生成式AI模型虽能生成合成数据，但现有隐私保护机制（如k-匿名化、差分隐私）在时间序列数据上存在局限，需要系统评估生成模型的隐私保护能力。

Method: 使用一系列已建立的隐私攻击方法，对基于MIMIC-IV数据集训练的最先进医院时间序列生成模型进行隐私审计，并利用eICU数据集对MIMIC-IV训练的合成数据生成器发起隐私攻击。

Result: 当合成数据生成器在足够大的训练数据集上训练时，现有隐私攻击对生成的多元临床时间序列无效；现有差分隐私机制不会带来期望的隐私改进，只会降低机器学习预测任务的效用。

Conclusion: 对于大型训练数据集训练的医疗时间序列生成模型，现有隐私攻击效果有限，而差分隐私机制在隐私保护方面无实质性改进，反而会损害数据效用，需要更有效的隐私保护方法。
...
### [110] [Evaluation metrics for temporal preservation in synthetic longitudinal patient data](https://arxiv.org/abs/2602.10643)
...
<details>
  <summary>Details</summary>
Motivation: 当前合成纵向患者数据的评估缺乏专门针对时间保持性的系统指标。现有评估方法可能过于关注边际相似性，而忽略了协方差结构和个体轨迹等重要时间特征，导致对合成数据质量的评估不全面。

Method: 提出了一套多维度的评估指标，将时间特征分为四类：边际结构、协方差结构、个体水平结构和测量结构。通过分析这些不同层次的时间特征，评估合成数据如何保持原始数据的时间依赖性。

Result: 研究发现：1）强边际相似性可能掩盖协方差扭曲和个体轨迹中断；2）时间保持性受原始数据质量、测量频率和预处理策略（如分箱、变量编码和精度）影响；3）稀疏或不规则测量时间的变量提供有限的时间依赖信息；4）单一指标不足以全面评估时间保持性，需要多维评估。

Conclusion: 提出的多维评估指标能够更全面地评估合成纵向患者数据的时间保持性，揭示时间结构如何被保持或退化，有助于改进生成模型并创建更真实的合成纵向患者数据。
...
### [111] [In-the-Wild Model Organisms: Mitigating Undesirable Emergent Behaviors in Production LLM Post-Training via Data Attribution](https://arxiv.org/abs/2602.11079)
...
<details>
  <summary>Details</summary>
Motivation: 需要追踪后训练语言模型行为变化到具体训练数据点，特别是在现实场景中（如OLMo 2的DPO训练）发现有害行为时，能够识别责任数据并进行干预。

Method: 计算测试提示和偏好对的激活差异向量，通过余弦相似度排序识别相关训练数据点；通过聚类行为-数据点相似度矩阵实现无监督行为发现；通过修改数据重新训练进行因果验证。

Result: 在OLMo 2的DPO训练中发现了"分心触发合规"有害行为：模型在附加良性格式化指令时会遵守危险请求；过滤排名靠前的数据点可将该行为减少63%，切换标签可减少78%；方法优于基于梯度的归因和LLM-judge基线，成本降低10倍以上。

Conclusion: 基于激活的数据归因方法能有效识别导致特定行为的训练数据点，为安全技术提供了现实基准；该方法成本低、效果好，能发现从受污染偏好数据中自然产生的有害行为，而非人为注入。
...
### [112] [Domain Knowledge Guided Bayesian Optimization For Autonomous Alignment Of Complex Scientific Instruments](https://arxiv.org/abs/2602.10670)
*Aashwin Mishra,Matt Seaberg,Ryan Roussel,Daniel Ratner,Apurva Mehta*

Main category: cs.LG

TL;DR: 该论文提出了一种基于领域知识引导的贝叶斯优化方法，通过坐标变换解耦高维参数并对齐活跃子空间，解决了传统贝叶斯优化在复杂高维问题中性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯优化（包括TurBO等先进方法）在高维、参数紧密耦合、目标函数高度不对称且奖励稀疏的"大海捞针"场景中性能显著下降，无法满足复杂科学仪器（如X射线自由电子激光器）的自动化调谐需求。

Method: 提出领域知识引导的贝叶斯优化方法：1）利用物理洞察进行坐标变换，解耦输入特征；2）将活跃子空间与主要搜索轴对齐；3）结合反向退火探索策略；4）在12维6晶体分束延迟光学系统上验证。

Result: 该方法在传统方法（标准BO、TurBO、多目标BO）均失败的挑战性问题上，能够可靠收敛到全局最优解。坐标变换是成功的关键，通过对齐活跃子空间显著加速搜索过程。

Conclusion: 通过物理洞察将高维耦合优化问题转化为简单表示，为复杂科学仪器的快速鲁棒自动化调谐提供了通用范式，同时保留了现有优化算法的优势。
...
### [113] [GRASP: group-Shapley feature selection for patients](https://arxiv.org/abs/2602.11084)
...
<details>
  <summary>Details</summary>
Motivation: 医学预测中的特征选择面临挑战，现有方法如LASSO缺乏鲁棒性和可解释性
...
Conclusion: GRASP提供了一个稳定且可解释的特征选择框架，特别适用于医学预测任务
...
### [114] [General Flexible $f$-divergence for Challenging Offline RL Datasets with Low Stochasticity and Diverse Behavior Policies](https://arxiv.org/abs/2602.11087)
*Jianxun Wang,Grant C. Forbes,Leonardo Villalobos-Arias,David L. Roberts*

Main category: cs.LG

TL;DR: 本文提出了一种基于f-散度的离线强化学习框架，通过线性规划形式和凸共轭理论连接f-散度与贝尔曼残差约束，引入灵活函数公式来根据数据集特性自适应调整约束强度。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习面临数据集多样性不足、探索有限以及行为策略质量参差不齐的挑战。有限探索会影响Q/V值估计，而对多样化行为策略的约束可能过于保守，需要在RL目标和行为策略约束之间找到平衡。

Method: 首先通过更一般的RL线性规划形式和凸共轭理论建立f-散度与贝尔曼残差优化约束之间的联系。然后引入f-散度的一般灵活函数公式，基于离线训练数据集对算法学习目标进行自适应约束。

Result: 在MuJoCo、Fetch和AdroitHand环境中的实验结果表明，所提出的线性规划形式的正确性，以及灵活f-散度在应用于兼容的约束优化算法时，能够有效提升从挑战性数据集学习的性能。

Conclusion: 通过连接f-散度与贝尔曼残差约束，并引入自适应约束机制，该方法能够在保持算法安全性的同时，更好地利用离线数据集中的信息，平衡RL目标与行为策略约束。
...
### [115] [Rising Multi-Armed Bandits with Known Horizons](https://arxiv.org/abs/2602.10727)
*Seockbean Song,Chenyu Gan,Youngsik Yoon,Siwei Wang,Wei Chen,Jungseul Ok*

Main category: cs.LG

TL;DR: 本文提出了一种针对上升多臂老虎机（RMAB）问题的CURE-UCB算法，该算法显式地整合了时间范围信息，在奖励随使用次数增加的场景中显著优于不考虑时间范围的策略。


<details>
  <summary>Details</summary>
Motivation: RMAB框架模拟了奖励随使用次数增加的环境（如机器人控制和超参数调优），但现有研究对时间范围感知的设置探索不足。由于RMAB具有时间范围依赖的最优性特点，了解时间范围T能带来显著优势，但这一方向尚未得到充分研究。

Method: 提出了CUmulative Reward Estimation UCB (CURE-UCB)算法，该算法显式地整合了时间范围信息，专门针对RMAB问题设计，能够根据可用预算T调整决策策略。

Result: 理论分析建立了新的遗憾上界，并证明在"线性-平坦"等结构化环境中，CURE-UCB严格优于不考虑时间范围的策略。大量实验验证了该方法相对于基线的显著优越性。

Conclusion: CURE-UCB算法成功解决了RMAB中时间范围感知的挑战，通过显式整合时间范围信息，在奖励随使用次数增加的场景中实现了更好的性能，为这类实际问题提供了有效的解决方案。
...
### [116] [GENIUS: Generative Fluid Intelligence Evaluation Suite](https://arxiv.org/abs/2602.11144)
*Ruichuan An,Sihan Yang,Ziyu Guo,Wei Dai,Zijun Shen,Haodong Li,Renrui Zhang,Xinyu Wei,Guopeng Li,Wenshan Wu,Wentao Zhang*

Main category: cs.LG

TL;DR: 论文提出了GENIUS评估套件，用于评估统一多模态模型的生成性流体智能（GFI），即模型在即时情境中归纳模式、执行约束和适应新场景的能力，而非依赖已有知识的结晶智能。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估依赖累积知识和学习模式的结晶智能，忽视了生成性流体智能——即模型在即时情境中归纳模式、通过约束推理和适应新场景的能力。需要建立专门评估GFI的基准来推动模型向动态、通用推理方向发展。

Method: 提出GENIUS评估套件，将GFI形式化为三个基本能力：归纳隐含模式（如推断个性化视觉偏好）、执行临时约束（如可视化抽象隐喻）、适应情境知识（如模拟反直觉物理）。对12个代表性模型进行系统评估，并通过诊断分析分离失败模式。

Result: 评估显示现有模型在GFI任务上存在显著性能缺陷。诊断分析表明这些缺陷源于有限的情境理解能力，而非内在生成能力不足。为此提出了无需训练的注意力干预策略来弥补这一差距。

Conclusion: GENIUS为GFI建立了严格评估标准，引导领域超越知识利用，向动态、通用推理方向发展。该基准将推动统一多模态模型在即时情境理解和适应性推理方面的进步。
...
### [117] [Collaborative Threshold Watermarking](https://arxiv.org/abs/2602.10765)
*Tameem Bakr,Anish Ambreth,Nils Lukas*

Main category: cs.LG

TL;DR: 提出(t,K)-阈值水印方案，解决联邦学习中多客户端水印稀释和单客户端可验证/移除水印的问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习中多个客户端联合训练模型时，需要证明模型来源但传统水印方法存在两个问题：1) 随着客户端数量增加，每个客户端的水印会被稀释；2) 单个客户端能够验证并可能移除水印

Method: 引入(t,K)-阈值水印：客户端在训练期间协作嵌入共享水印，只有至少t个客户端组成的联盟才能重构水印密钥并验证可疑模型。通过秘密共享水印密钥τ，确保少于t个客户端的联盟无法重构密钥，且验证过程无需明文暴露τ

Result: 在图像分类任务上评估，水印在大规模场景(K=128)下仍可检测，精度损失最小，在自适应微调攻击(使用高达20%训练数据)下检测阈值保持(z≥4)

Conclusion: 提出的(t,K)-阈值水印方案解决了联邦学习中多客户端水印的可扩展性和安全性问题，能够在保持模型性能的同时提供可靠的来源证明
...
### [118] [RePO: Bridging On-Policy Learning and Off-Policy Knowledge through Rephrasing Policy Optimization](https://arxiv.org/abs/2602.10819)
*Linxuan Xia,Xiaolong Yang,Yongyuan Chen,Enyue Zhao,Deng Cai,Yasheng Wang,Boxi Wu*

Main category: cs.LG

TL;DR: RePO是一种新的强化学习方法，通过让模型重新表述离线知识来平衡离线知识吸收与在线训练稳定性，在保持模型泛化能力的同时有效利用困难样本。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在领域特定数据对齐方面存在挑战：监督微调会损害模型泛化能力，在线强化学习无法有效吸收超出模型当前推理水平的困难样本，而离线强化学习虽然能利用困难样本但存在训练不稳定问题。

Method: 提出Rephrasing Policy Optimization (RePO)方法：让策略模型首先理解离线知识，然后将其重新表述为符合自身风格和参数分布的轨迹，动态替换低奖励的rollouts，在保持在线训练动态的同时引导模型走向正确推理路径。

Result: 在多个基准测试中，RePO改善了困难样本的利用率，超越了现有基线方法，实现了最先进的性能表现。

Conclusion: RePO成功调和了有效吸收离线知识与保持在线强化学习稳定性之间的矛盾，为大语言模型的领域对齐提供了一种新的有效方法。
...
### [119] [Adaptive Sampling for Private Worst-Case Group Optimization](https://arxiv.org/abs/2602.10820)
...
<details>
  <summary>Details</summary>
Motivation: 传统的最差组优化方法在差分隐私环境下存在问题，因为不等的数据加权会导致不同组获得不均匀的隐私保护，特别是少数群体获得较弱的隐私保障。

Method: 提出ASC算法，自适应控制每个组的采样率和裁剪阈值，允许难学习的组被更频繁采样，同时确保所有组获得一致的隐私保证。

Result: ASC相比先前工作，实现了更低方差的梯度、更严格的隐私保证，以及显著更高的最差组准确率，同时不牺牲整体平均准确率。

Conclusion: ASC算法有效解决了差分隐私下最差组优化的隐私不均问题，在保证隐私一致性的同时提升了模型对少数群体的性能。
...
### [120] [SimuScene: Training and Benchmarking Code Generation to Simulate Physical Scenarios](https://arxiv.org/abs/2602.10840)
*Yanan Wang,Renxi Wang,Yongxin Wang,Xuezhi Liang,Fajri Koto,Timothy Baldwin,Xiaodan Liang,Haonan Li*

Main category: cs.LG

TL;DR: SimuScene是首个系统研究LLM通过代码模拟物理场景能力的工作，构建了包含7,659个物理场景的数据集，发现现有最强模型仅21.5%通过率，并提出了基于视觉奖励的强化学习训练方法。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在数学竞赛、复杂编程和科学推理等任务上已有广泛研究，但其通过代码准确表示和模拟物理场景的能力尚未得到充分探索。现有研究缺乏对LLM物理模拟能力的系统性评估。

Method: 1) 构建SimuScene数据集：通过自动化流程收集数据，人工验证确保质量，涵盖5个物理领域和52个物理概念，包含7,659个物理场景，其中334个作为测试集；2) 评估10个当代LLM；3) 提出基于视觉奖励的强化学习管道，使用视觉语言模型作为评判来训练文本模型。

Result: 1) 现有最强LLM在物理场景模拟任务上仅达到21.5%的通过率，表明任务难度很高；2) 使用SimuScene数据进行训练能显著提升模型通过代码进行物理模拟的能力；3) 训练还大幅提升了模型的一般代码生成性能。

Conclusion: SimuScene填补了LLM物理模拟能力评估的空白，展示了现有模型的局限性，提出的强化学习训练方法有效提升了模型的物理模拟和代码生成能力，为LLM在物理场景理解和模拟方面的发展提供了重要基础。
...
TL;DR: 本文研究了在最小化单比特反馈下学习n维累积分布函数均匀近似的样本复杂度，发现样本复杂度具有近维度不变性，维度n仅影响对数项。
...
Result: 获得了样本复杂度的近维度不变性：在任意精细网格上获得均匀ε-近似所需的样本复杂度为1/ε³ × log(1/ε)^O(n)，其中维度n仅影响对数项。

Conclusion: 该结果为小市场（如双边交易场景）中学习固定价格机制提供了紧密的样本复杂度界限和新的遗憾保证，扩展了多元DKW不等式到老虎机反馈设置。
...
### [122] [Natural Hypergradient Descent: Algorithm Design, Convergence Analysis, and Parallel Implementation](https://arxiv.org/abs/2602.10905)
...
Method: 利用内层优化问题的统计结构，使用经验Fisher信息矩阵作为Hessian的渐近一致替代，实现并行优化-近似框架，同步更新Hessian逆近似与随机内层优化。

Result: 理论分析建立了高概率误差界和样本复杂度保证，与最先进的优化-后近似方法相匹配，同时显著减少计算时间开销。实证评估在代表性双层学习任务中展示了NHGD的可扩展性和有效性。

Conclusion: NHGD通过创新的Hessian逆近似方法，在保持理论保证的同时显著降低了计算成本，为大规模机器学习中的双层优化问题提供了高效解决方案。
...
### [123] [Tuning the burn-in phase in training recurrent neural networks improves their performance](https://arxiv.org/abs/2602.10911)
*Julian D. Schiller,Malte Heinrich,Victor G. Lopez,Matthias A. Müller*

Main category: cs.LG

TL;DR: 该论文研究了在时间序列任务中使用截断BPTT训练RNN时，burn-in阶段对训练效果的重要影响，建立了理论性能界限并通过实验验证。


<details>
  <summary>Details</summary>
Motivation: 标准BPTT训练RNN在处理长输入序列时存在计算和内存开销大的问题，截断BPTT是实用的替代方案，但需要理论分析其性能损失和优化策略。

Method: 建立了截断BPTT训练RNN的理论性能界限，特别关注burn-in阶段的影响，并在系统辨识和时间序列预测的标准基准上进行了实验验证。

Result: 理论分析表明burn-in阶段是重要的调优参数，实验验证了burn-in阶段对训练过程的显著影响，适当调优可使训练和测试数据的预测误差降低超过60%。

Conclusion: 截断BPTT训练RNN时，burn-in阶段是关键的调优参数，合理设置可以显著提升性能，为实际应用提供了理论指导和实用建议。
...
### [124] [Near-Constant Strong Violation and Last-Iterate Convergence for Online CMDPs via Decaying Safety Margins](https://arxiv.org/abs/2602.10917)
*Qian Zuo,Zhiyong Wang,Fengxiang He*

Main category: cs.LG

TL;DR: FlexDOME算法首次在约束MDP中实现近常数强约束违反与次线性强遗憾，解决了现有方法约束违反增长或仅平均收敛的问题。


<details>
  <summary>Details</summary>
Motivation: 现有原始对偶方法在实现次线性强奖励遗憾时，不可避免地会导致强约束违反增长或仅限于平均迭代收敛，这是由于固有的振荡问题。需要一种能同时实现近常数约束违反、次线性遗憾和非渐近最后迭代收敛的方法。

Method: 提出FlexDOME算法，在原始对偶框架中引入时变安全裕度和正则化项。采用新颖的逐项渐近主导策略，通过严格调度安全裕度来渐近主导优化和统计误差的函数衰减率，从而将累积违反限制在近常数水平。

Result: FlexDOME算法可证明地实现近常数Õ(1)强约束违反，同时保持次线性强遗憾和非渐近最后迭代收敛。实验验证了理论发现。

Conclusion: FlexDOME算法首次解决了约束MDP中安全在线强化学习的关键挑战，实现了近常数约束违反与次线性遗憾的平衡，并通过策略-对偶Lyapunov论证建立了非渐近最后迭代收敛保证。
...
### [125] [CMAD: Cooperative Multi-Agent Diffusion via Stochastic Optimal Control](https://arxiv.org/abs/2602.10933)
...
<details>
  <summary>Details</summary>
Motivation: 现有连续时间生成模型在图像恢复和合成方面取得显著成功，但控制多个预训练模型的组合仍是一个开放挑战。当前方法主要将组合视为概率密度的代数组合（如乘积或专家混合），这需要显式知道目标分布，而实际情况几乎不可能。

Method: 提出新范式：将组合生成建模为协作随机最优控制问题。不组合概率密度，而是将预训练扩散模型视为交互智能体，通过最优控制联合引导其扩散轨迹，朝向基于聚合输出定义的共享目标。

Result: 在条件MNIST生成任务上验证了该框架，并与使用每步梯度指导的朴素推理时DPS风格基线进行比较。

Conclusion: 提出了一种基于协作随机最优控制的组合生成新方法，避免了传统方法需要显式目标分布的局限性，为多模型协同控制提供了新思路。
...
### [126] [Stochastic Parroting in Temporal Attention -- Regulating the Diagonal Sink](https://arxiv.org/abs/2602.10956)
*Victoria Hankemeier,Malte Hankemeier*

Main category: cs.LG

TL;DR: 该论文分析了时空模型中时间注意力机制存在的对角线注意力沉没问题，提出了正则化方法来解决这一偏差。
...
Method: 推导了时间注意力层雅可比矩阵期望值的敏感度边界，从理论上分析了非对角线注意力分数如何依赖于序列长度，证明时间注意力矩阵存在对角线注意力沉没问题，并提出了正则化方法。

Result: 理论分析表明时间注意力机制确实存在对角线注意力沉没问题，实验验证了提出的正则化方法的有效性。
...
### [127] [MoEEdit: Efficient and Routing-Stable Knowledge Editing for Mixture-of-Experts LLMs](https://arxiv.org/abs/2602.10965)
*Yupu Gu,Rongzhe Wei,Andy Zhu,Pan Li*

Main category: cs.LG

TL;DR: MoEEdit：首个针对稀疏MoE架构大语言模型的知识编辑框架，通过专家空问投影保持路由稳定性，实现高效、精确的知识修改。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法主要针对密集架构设计，不适用于日益流行的稀疏MoE模型。直接适配密集模型编辑器会导致计算成本高、路由分布偏移，影响稳定性和一致性。

Method: 提出MoEEdit框架，通过专家级空问投影重新参数化专家更新，保持路由器输入不变以抑制路由偏移，采用块坐标下降法进行高效优化。

Result: MoEEdit在效果和泛化能力上达到最先进水平，同时保持高特异性和路由稳定性，在计算和内存效率方面表现优异。
...
### [128] [A Jointly Efficient and Optimal Algorithm for Heteroskedastic Generalized Linear Bandits with Adversarial Corruptions](https://arxiv.org/abs/2602.10971)
*Sanghwa Kim,Junghyun Lee,Se-Young Yun*

Main category: cs.LG

TL;DR: 该论文提出了HCW-GLB-OMD算法，用于解决具有对抗性扰动的异方差广义线性赌博机问题，实现了计算高效且接近最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 研究具有对抗性扰动的异方差广义线性赌博机问题，该问题包含了多种随机上下文赌博机设置，如异方差线性赌博机、逻辑回归/泊松赌博机等。需要设计一个既能处理异方差性又能抵抗对抗性扰动的算法。

Method: 提出HCW-GLB-OMD算法，包含两个核心组件：基于在线镜像下降的估计器和基于Hessian的置信权重。该算法计算高效，每次迭代仅需O(1)的空间和时间复杂度。

Result: 在链接函数满足自洽性假设下，获得了遗憾界为$\tilde{O}\left( d \sqrt{\sum_t g(τ_t) \dotμ_{t,\star}} + d^2 g_{\max} κ+ d κC \right)$，并给出了匹配的下界$\tildeΩ(d \sqrt{\sum_t g(τ_t) \dotμ_{t,\star}} + d C)$，表明算法在对抗性扰动项上达到κ因子内的实例级极小极大最优性。

Conclusion: HCW-GLB-OMD算法在各种具有对抗性扰动的异方差广义线性赌博机实例中，实现了计算高效且接近最优的性能，统一了先前特定问题的下界结果。
...
TL;DR: TVCACHE：一种用于LLM智能体后训练的状态感知工具值缓存系统，通过最长前缀匹配确保缓存一致性，显著减少工具调用时间


<details>
  <summary>Details</summary>
Motivation: LLM智能体后训练中，外部工具调用耗时数秒甚至数分钟，导致GPU空闲、增加训练时间和成本。虽然许多工具调用在并行rollout中重复出现，但简单缓存输出是不正确的，因为工具输出依赖于先前智能体交互诱导的环境状态。

Method: TVCACHE维护观察到的工具调用序列树，执行最长前缀匹配进行缓存查找：只有当智能体的完整工具历史与先前执行的序列完全匹配时才发生缓存命中，从而保证环境状态一致。

Result: 在三个多样化工作负载上（终端任务、SQL生成和视频理解），TVCACHE实现了高达70%的缓存命中率，将工具调用执行时间中位数减少高达6.9倍，且后训练奖励积累没有下降。
...
### [130] [Motion Capture is Not the Target Domain: Scaling Synthetic Data for Learning Motion Representations](https://arxiv.org/abs/2602.11064)
*Firas Darwish,George Nicholson,Aiden Doherty,Hang Yuan*

Main category: cs.LG

TL;DR: 合成数据在真实数据稀缺时可用于大规模预训练，但在人体运动识别中，仅用合成数据预训练的模型难以可靠迁移到实际部署场景，需要与真实数据混合或足够规模才能改善泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究在人体运动识别（HAR）中，当大规模真实数据收集不可行时，使用合成数据进行预训练的可行性。穿戴式设备的人体活动识别需要大量数据，而合成运动数据可以从运动捕捉表示生成，但模型在合成数据上预训练后能否有效迁移到实际部署环境存在挑战。

Method: 使用从运动捕捉表示生成的合成运动数据预训练运动时间序列模型，然后在多个下游HAR任务上评估模型的迁移能力。研究合成数据与真实数据混合的效果，以及大规模运动捕捉预训练的局限性。

Result: 合成数据预训练在以下情况下能改善泛化：1）与真实数据混合使用；2）达到足够规模。大规模运动捕捉预训练仅带来边际收益，主要原因是穿戴式信号与运动捕捉数据之间存在领域不匹配问题。

Conclusion: 阐明了合成到真实迁移的关键挑战，以及合成运动数据在可迁移HAR表示中的局限性和机会。合成数据预训练需要谨慎处理领域不匹配问题，混合真实数据或扩大规模是改善迁移效果的有效策略。
...
### [131] [Token-Efficient Change Detection in LLM APIs](https://arxiv.org/abs/2602.11083)
*Timothée Chauvin,Clément Lalanne,Erwan Le Merrer,Jean-Michel Loubes,François Taïani,Gilles Tredan*

Main category: cs.LG

TL;DR: 论文提出B3IT方法，通过边界输入实现低成本、严格黑盒的LLM远程变化检测，相比现有方法成本降低30倍，性能与最佳灰盒方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有LLM远程变化检测方法要么成本过高难以大规模部署，要么需要白盒（模型权重）或灰盒（对数概率）访问权限。需要实现低成本且严格黑盒（仅观察输出token）的解决方案。

Method: 提出黑盒边界输入追踪（B3IT）方案，利用边界输入（存在多个输出top token的特定输入）。从统计角度看，最优变化检测依赖于模型的Jacobian和输出分布的Fisher信息，在低温机制下分析这些量表明边界输入能实现强大的变化检测测试。

Result: 实验显示边界输入在非推理测试端点中容易找到，B3IT性能与最佳可用灰盒方法相当，同时成本降低30倍，并在严格黑盒设置下运行。

Conclusion: B3IT方法成功实现了低成本、严格黑盒的LLM远程变化检测，解决了现有方法的部署限制，为大规模监控LLM变化提供了实用解决方案。
...
### [132] [MerLin: A Discovery Engine for Photonic and Hybrid Quantum Machine Learning](https://arxiv.org/abs/2602.11092)
...
TL;DR: MerLin是一个开源框架，用于系统探索光子和混合量子机器学习模型，通过集成线性光学电路的优化强模拟到PyTorch和scikit-learn工作流，支持端到端可微分训练。
...
Method: 开发MerLin框架，将线性光学电路的优化强模拟集成到标准PyTorch和scikit-learn工作流中，实现量子层的端到端可微分训练，并围绕系统化基准测试和可重复性设计。

Result: 成功复现了18个最先进的光子和混合QML工作，涵盖核方法、储备池计算、卷积和循环架构、生成模型及现代训练范式，并发布为可重用、模块化的实验。

Conclusion: MerLin通过将光子量子模型嵌入已建立的机器学习生态系统，使从业者能够利用现有工具进行消融研究、跨模态比较和混合经典量子工作流，并作为连接算法、基准测试和硬件的未来验证协同设计工具。
...
### [133] [The Offline-Frontier Shift: Diagnosing Distributional Limits in Generative Multi-Objective Optimization](https://arxiv.org/abs/2602.11126)
*Stephanie Holly,Alexandru-Ciprian Zăvoianu,Siegfried Silber,Sepp Hochreiter,Werner Zellinger*

Main category: cs.LG

TL;DR: 生成方法在离线多目标优化中虽然在高维体积指标上表现良好，但在其他指标（如世代距离）上系统性地逊于进化方法，这源于离线前沿偏移问题。


<details>
  <summary>Details</summary>
Motivation: 研究生成方法（如扩散模型）在离线多目标优化中的局限性，特别是在不同评估指标下的表现差异，揭示其根本原因。

Method: 分析生成方法与进化方法在离线多目标优化中的表现对比，识别离线前沿偏移问题，提出通过目标空间中的分布外采样来克服这一限制。

Result: 生成方法在世代距离等指标上系统性地逊于进化方法，这源于它们保守地停留在离线目标分布附近，无法有效应对离线前沿偏移。

Conclusion: 离线多目标优化是一个受分布偏移限制的问题，生成方法的失败源于无法有效进行分布外采样，这为理解生成优化方法的局限性提供了诊断视角。
...
### [134] [Asymmetric Prompt Weighting for Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2602.11128)
*Reinhard Heckel,Mahdi Soltanolkotabi,Christos Thramboulidis*

Main category: cs.LG

TL;DR: 该论文研究了强化学习中非对称提示加权策略，发现在低成功率阶段，对低成功概率提示赋予更高权重可以加速模型收敛。


<details>
  <summary>Details</summary>
Motivation: 当前流行的强化学习算法（如GRPO、DAPO、RLOO）主要关注中等成功概率的模糊提示，而对非常容易或非常困难的提示降权。然而在低成功率阶段，信息性响应稀少且响应成本占主导，需要更有效的加权策略来加速收敛。

Method: 提出非对称提示加权方法，为低成功概率甚至零成功概率的提示分配更高权重。通过理论分析确定了在固定更新预算下，将成功概率从初始水平提升到目标准确率所需时间最小化的最优权重。

Result: 非对称加权特别有利于从零开始的强化学习（如R1-Zero），因为训练过程覆盖了广泛的准确率范围；对于后SFT的强化学习，由于模型已具备较高准确率，效果相对较小。在低成功率阶段，最优权重变得非对称，通过上加权低成功概率来加速有效时间收敛。

Conclusion: 在强化学习训练中，特别是在低成功率阶段，采用非对称提示加权策略可以显著加速收敛，这对于从零开始的强化学习训练尤为重要，为LLM后训练提供了更有效的优化方法。
...
### [135] [TabICLv2: A better, faster, scalable, and open tabular foundation model](https://arxiv.org/abs/2602.11139)
*Jingang Qu,David Holzmüller,Gaël Varoquaux,Marine Le Morvan*

Main category: cs.LG

TL;DR: TabICLv2是一种新的表格基础模型，通过创新的合成数据生成、架构改进和优化训练协议，在回归和分类任务上超越了当前最先进的方法，无需调参即可在多个基准测试中取得更好性能。
...
Method: TabICLv2基于三个支柱构建：(1) 新颖的合成数据生成引擎，旨在实现高预训练多样性；(2) 多种架构创新，包括新的可扩展softmax注意力机制，无需昂贵的长序列预训练即可泛化到更大数据集；(3) 优化的预训练协议，特别是用Muon优化器替代AdamW。

Result: 在TabArena和TALENT基准测试中，未经任何调优的TabICLv2超越了当前最先进的RealTabPFN-2.5（经过超参数调优、集成和真实数据微调）。模型在50GB GPU内存下能有效泛化到百万规模数据集，且速度明显快于RealTabPFN-2.5。

Conclusion: TabICLv2在表格预测任务上建立了新的最先进性能，通过创新的合成数据生成、架构改进和优化训练协议实现了这一突破。作者承诺开放研究，已发布推理代码和模型权重。
...
### [136] [LiveMedBench: A Contamination-Free Medical Benchmark for LLMs with Automated Rubric Evaluation](https://arxiv.org/abs/2602.10367)
*Zhiling Yan,Dingjie Song,Zhe Fang,Yisheng Ji,Xiang Li,Quanzheng Li,Lichao Sun*

Main category: cs.AI

TL;DR: LiveMedBench是一个持续更新的、无数据污染的、基于评分标准的医疗基准测试，通过每周从在线医疗社区收集真实临床案例，解决了现有医疗基准测试的数据污染和时间错位问题。


<details>
  <summary>Details</summary>
Motivation: 现有医疗基准测试存在两个关键限制：1) 数据污染问题，测试集无意中泄露到训练语料中，导致性能估计虚高；2) 时间错位问题，无法捕捉医学知识的快速演变。此外，当前开放式临床推理的评估指标要么依赖浅层词汇重叠，要么依赖主观的LLM作为评判者，都不足以验证临床正确性。

Method: 1) 构建LiveMedBench基准：每周从在线医疗社区收集真实临床案例，确保与模型训练数据的严格时间分离；2) 多智能体临床筛选框架：过滤原始数据噪声，并根据循证医学原则验证临床完整性；3) 自动化基于评分标准的评估框架：将医生回答分解为细粒度的、病例特定的标准，实现比LLM作为评判者更强的专家医生一致性。

Result: LiveMedBench包含2,756个真实世界病例，涵盖38个医学专业和多种语言，配有16,702个独特的评估标准。对38个LLM的广泛评估显示，即使表现最好的模型也只达到39.2%的正确率，84%的模型在截止日期后的病例上表现出性能下降，证实了普遍存在的数据污染风险。错误分析进一步发现上下文应用（而非事实知识）是主要瓶颈，35-48%的失败源于无法将医学知识适应于患者特定约束。

Conclusion: LiveMedBench为临床LLM评估提供了一个持续更新、无数据污染、基于评分标准的基准测试，揭示了当前LLM在临床推理中的严重局限性，特别是上下文应用能力不足和数据污染导致的性能虚高问题。
...
### [137] [Found-RL: foundation model-enhanced reinforcement learning for autonomous driving](https://arxiv.org/abs/2602.10458)
*Yansong Qu,Zihao Sheng,Zilin Huang,Jiancong Chen,Yuhao Luo,Tianyi Wang,Yiheng Feng,Samuel Labi,Sikai Chen*

Main category: cs.AI

TL;DR: Found-RL平台通过异步批量推理框架解决VLM在自动驾驶RL训练中的延迟问题，使用VMR和AWAG机制将VLM知识蒸馏到RL策略中，并采用改进的CLIP进行奖励塑造，实现轻量级RL模型达到接近VLM的性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习在自动驾驶中存在样本效率低和语义解释性不足的问题，而基础模型（特别是视觉语言模型）能提供丰富的上下文感知知识，但其高推理延迟阻碍了在高频RL训练循环中的部署。

Method: 1. 异步批量推理框架：将繁重的VLM推理与仿真循环解耦，解决延迟瓶颈；2. 监督机制：值边界正则化(VMR)和优势加权动作指导(AWAG)将VLM专家动作建议蒸馏到RL策略；3. 奖励塑造：采用高吞吐量CLIP，通过条件对比动作对齐解决CLIP的动态盲区问题，基于速度/命令离散化的提示生成归一化边界奖励。

Result: 轻量级RL模型能够达到接近十亿参数VLM的性能，同时保持实时推理能力（约500 FPS），提供了端到端的微调VLM集成管道。

Conclusion: Found-RL平台有效解决了VLM在自动驾驶RL训练中的延迟问题，通过创新的异步推理和监督机制，使轻量级RL模型能够高效利用VLM的丰富知识，实现高性能实时自动驾驶。
...
### [138] [MERIT Feedback Elicits Better Bargaining in LLM Negotiators](https://arxiv.org/abs/2602.10467)
*Jihwan Oh,Murad Aghazada,Yooju Shin,Se-Young Yun,Taehyeon Kim*

Main category: cs.AI

TL;DR: 论文提出了一个基于效用反馈的框架来提升LLM在讨价还价任务中的表现，包括新基准AgoraBench、经济学指标和人类偏好数据集。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在讨价还价任务中表现不佳，现有基准无法充分捕捉其战略深度不足和适应复杂人类因素困难的问题。

Method: 提出了一个效用反馈中心框架，包括：1) AgoraBench基准，涵盖9个挑战性场景；2) 基于效用理论的人类对齐经济学指标；3) 人类偏好数据集和学习管道，通过提示和微调增强LLM讨价还价能力。

Result: 基线LLM策略常偏离人类偏好，而提出的机制显著改善了谈判性能，产生了更深层次的战略行为和更强的对手意识。

Conclusion: 该框架有效提升了LLM在复杂讨价还价场景中的表现，使其更符合人类偏好和经济理性。
...
### [139] [Abstraction Generation for Generalized Planning with Pretrained Large Language Models](https://arxiv.org/abs/2602.10485)
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能作为QNP抽象生成器解决广义规划问题，以及如何通过自动调试修复抽象错误
...
Conclusion: LLMs可以作为QNP抽象生成器，自动调试是确保生成有用抽象的关键
...
### [140] [Flow of Spans: Generalizing Language Models to Dynamic Span-Vocabulary via GFlowNets](https://arxiv.org/abs/2602.10583)
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归语言模型使用固定词汇表，形成树状状态空间，限制了灵活性和表达能力。现有动态词汇方法虽然采样检索文本span，但忽略了同一句子可由不同长度span组成，缺乏对DAG状态空间的显式建模，导致组合路径探索受限和路径选择偏差。

Method: 提出Flow of SpanS (FOSS)框架：1) 通过灵活分割检索文本构建动态span词汇表；2) 确保DAG结构状态空间；3) 利用GFlowNets探索多样组合路径；4) 结合专用奖励模型生成高质量文本。

Result: 实验表明：1) 在文本生成任务上，MAUVE分数比Transformer提升高达12.5%；2) 在知识密集型任务上获得3.5%的提升；3) 持续优于最先进方法；4) 扩展实验显示FoSS受益于更大模型、更多数据和更丰富的检索语料库。

Conclusion: FoSS通过GFlowNets框架实现span级生成，构建DAG状态空间，有效探索多样组合路径，显著提升文本生成的多样性和质量，在多个任务上优于现有方法。
...
### [141] [Neuro-symbolic Action Masking for Deep Reinforcement Learning](https://arxiv.org/abs/2602.10598)
*Shuai Han,Mehdi Dastani,Shihan Wang*

Main category: cs.AI

TL;DR: NSAM：一种自动学习符号模型并生成动作掩码的神经符号框架，用于在深度强化学习中约束不可行动作，提高样本效率并减少约束违反
...
Method: 提出神经符号动作掩码（NSAM）框架，在最小监督下自动学习与高维状态域约束一致的符号模型。基于学习的符号状态模型，NSAM学习动作掩码来排除不可行动作，实现符号推理和深度策略优化的端到端集成。

Result: 在多个具有约束的领域进行评估，实验结果表明NSAM显著提高了DRL智能体的样本效率，同时大幅减少了约束违反。

Conclusion: NSAM通过自动学习符号模型和动作掩码，实现了符号推理与深度策略优化的协同增强，为解决DRL中的约束动作问题提供了一种有效的端到端解决方案。
...
### [142] [To Think or Not To Think, That is The Question for Large Reasoning Models in Theory of Mind Tasks](https://arxiv.org/abs/2602.10625)
*Nanxu Gong,Haotian Li,Sixun Dong,Jianxun Lian,Yanjie Fu,Xing Xie*

Main category: cs.AI

TL;DR: 研究发现，大型推理模型在心理理论任务上并不比非推理模型表现更好，有时甚至更差，揭示了推理模型在社交推理任务上的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型在数学和编程等正式推理任务上取得了显著进展，但尚不清楚这种优势是否能转移到心理理论等社会认知技能上。本研究旨在系统评估推理模型在心理理论任务上的表现。

Method: 对9个先进的大型语言模型进行系统研究，比较推理模型与非推理模型在三个代表性心理理论基准测试上的表现。通过细粒度分析揭示性能模式，并设计了两种干预方法：慢到快自适应推理和思考到匹配捷径预防。

Result: 推理模型并不一致优于非推理模型，有时表现更差。分析发现：1）慢思考崩溃现象：回答越长准确率显著下降，更大的推理预算反而损害性能；2）适度自适应推理有益：限制推理长度可缓解失败，不同成功模式显示动态适应的必要性；3）选项匹配捷径：移除选择题选项后，推理模型表现显著改善，表明其依赖选项匹配而非真正推理。
...
### [143] [OmniSapiens: A Foundation Model for Social Behavior Processing via Heterogeneity-Aware Relative Policy Optimization](https://arxiv.org/abs/2602.10635)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常孤立地建模人类行为维度（情感、认知或社会属性），任务特定建模增加了训练成本并限制了跨行为设置的泛化能力。最近的推理RL方法虽然能在多个行为任务上训练统一模型，但未明确解决跨异构行为数据的学习问题。

Method: 提出了异构感知相对策略优化（HARPO），这是一种RL方法，通过调节优势函数来确保在策略优化过程中没有任何单个任务或样本产生不成比例的影响，从而平衡跨异构任务和样本的学习。

Result: 使用HARPO开发了Omnisapiens-7B 2.0社交行为基础模型，相对于现有行为基础模型，在多任务和保留设置上分别实现了高达+16.85%和+9.37%的性能提升，同时产生更明确和鲁棒的推理轨迹。HARPO在行为任务上也比最近的RL方法表现更一致。

Conclusion: HARPO方法有效解决了跨异构行为数据学习的挑战，开发的Omnisapiens-7B 2.0模型在社交行为处理任务上表现出色，为开发社交智能AI提供了有效工具。
...
### [144] [Spend Search Where It Pays: Value-Guided Structured Sampling and Optimization for Generative Recommendation](https://arxiv.org/abs/2602.10699)
...
<details>
  <summary>Details</summary>
Motivation: 生成式推荐中的强化学习存在概率-奖励不匹配问题：传统基于似然的解码（如束搜索）对局部高概率前缀产生短视偏见，导致探索不足（高奖励但低概率分支被过早剪枝）和优势压缩（共享高概率前缀的轨迹奖励高度相关，比较信号弱）

Method: 提出V-STAR框架，包含两个协同组件：1）价值引导高效解码（VED），识别决定性节点并选择性深化高潜力前缀，提升探索效率；2）Sibling-GRPO，利用诱导的树拓扑计算兄弟相对优势，将学习信号集中在决定性分支决策上
...
### [145] [Integrating Generative AI-enhanced Cognitive Systems in Higher Education: From Stakeholder Perceptions to a Conceptual Framework considering the EU AI Act](https://arxiv.org/abs/2602.10802)
*Da-Lun Chen,Prasasthy Balasubramanian,Lauri Lovén,Susanna Pirttikangas,Jaakko Sauvola,Panagiotis Kostakos*

Main category: cs.AI

TL;DR: 研究调查了高等教育中生成式AI的感知情况，重点关注ITEE领域，通过混合方法分析师生看法，提出负责任AI集成的框架和要求。


<details>
  <summary>Details</summary>
Motivation: 高等教育中生成式AI工具已被广泛采用，但利益相关者的看法存在分歧，且欧盟AI法案要求大学确保认知系统的监管合规性，需要了解学科特定需求并制定负责任集成策略。

Method: 采用混合方法，调查了奥卢大学ITEE学院的61名教职员工和37名学生，收集他们对生成式AI的看法和体验。

Result: 研究发现共享和学科特定的主题：对编程支持的强烈兴趣，以及对响应质量、隐私和学术诚信的担忧。识别了高层级需求并提出了负责任AI集成的概念框架。

Conclusion: 学科特定需求强调了利益相关者参与的重要性，提出的高层级需求和框架为大学利用生成式AI同时解决利益相关者关切和确保监管合规提供了实践指导。
...
### [146] [SynergyKGC: Reconciling Topological Heterogeneity in Knowledge Graph Completion via Topology-Aware Synergy](https://arxiv.org/abs/2602.10845)
*Xuecheng Zou,Yu Tang,Bingbing Wang*

Main category: cs.AI

TL;DR: SynergyKGC通过跨模态协同专家和密度依赖的身份锚定策略，解决知识图谱补全中结构分辨率不匹配问题，显著提升命中率


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱补全方法面临"结构分辨率不匹配"问题，无法在不同图密度下协调表示需求，导致密集簇中的结构噪声干扰和稀疏区域的灾难性表示崩溃

Method: 提出SynergyKGC自适应框架：1）将传统邻居聚合提升为主动跨模态协同专家，通过关系感知交叉注意力和语义意图驱动门控；2）结合密度依赖的身份锚定策略和双塔一致性架构，协调拓扑异构性并确保表示稳定性

Result: 在两个公共基准测试上的系统评估验证了方法的优越性，显著提升了KGC命中率，为非均匀结构化数据中的弹性信息集成提供了经验证据

Conclusion: SynergyKGC通过自适应融合预训练实体语义与异构拓扑结构，有效解决了结构分辨率不匹配问题，为知识图谱补全提供了更稳健的关系推理框架
...
### [147] [Reinforcing Chain-of-Thought Reasoning with Self-Evolving Rubrics](https://arxiv.org/abs/2602.10885)
*Leheng Sheng,Wenchang Ma,Ruixin Hong,Xiang Wang,An Zhang,Tat-Seng Chua*

Main category: cs.AI

TL;DR: RLCER：一种无需人工标注、通过自我提出和演化的评分标准来奖励思维链的强化学习方法，优于结果中心的RLVR方法


<details>
  <summary>Details</summary>
Motivation: 思维链在LLM推理中至关重要，但直接奖励思维链很困难：训练奖励模型需要大量人工标注，静态奖励模型难以适应思维链分布的演变且容易受到奖励攻击。这些挑战促使研究者寻求无需人工标注、能够逐渐演化的自主思维链奖励方法。

Method: 提出RLCER（强化学习与思维链监督通过自我演化评分标准），该方法通过自我提出和演化的评分标准来奖励思维链，增强了结果中心的RLVR方法。自我提出和演化的评分标准即使没有结果奖励也能提供可靠的思维链监督信号。

Result: RLCER在性能上优于结果中心的RLVR方法。此外，当将这些自我提出的评分标准用作提示中的提示时，还能进一步提高推理时的性能。

Conclusion: 自我提出和演化的评分标准为思维链提供了有效的监督信号，使得RLCER方法能够在无需人工标注的情况下实现优于传统结果中心方法的性能，同时还能通过提示增强推理效果。
...
### [148] [Can LLMs Cook Jamaican Couscous? A Study of Cultural Novelty in Recipe Generation](https://arxiv.org/abs/2602.10964)
*F. Carichon,R. Rampa,G. Farnadi*

Main category: cs.AI

TL;DR: LLMs在文化内容生成中存在系统性文化偏见，无法根据文化距离进行有意义的跨文化适应，特别是在烹饪食谱领域。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地用于生成和塑造文化内容，但研究表明它们存在系统性文化偏见，可能导致刻板印象、同质化和文化特定表达形式的消失。了解LLMs是否能超越主导文化而有意义地适应多元文化是一个关键挑战。
...
Result: LLMs无法产生具有文化代表性的适应。与人类不同，它们生成的食谱差异与文化距离不相关。文化信息在模型内部表示中保存较弱，模型通过误解创造力和传统等概念来夸大新颖性，并且无法识别适应相关的国家及其文化显著元素（如食材）。
...
### [149] [CLI-Gym: Scalable CLI Task Generation via Agentic Environment Inversion](https://arxiv.org/abs/2602.10999)
*Yusong Lin,Haiyang Wang,Shuzhe Wu,Lue Fan,Feiyang Pan,Sanyuan Zhao,Dandan Tu*

Main category: cs.AI

TL;DR: CLI-Gym：首个可扩展的环境密集型任务生成管道，通过Dockerfile历史回溯模拟环境故障，生成1655个任务，并训练出在Terminal-Bench上达到46.1%性能的LiberCoder模型


<details>
  <summary>Details</summary>
Motivation: 解决智能体编码中环境密集型任务（如CLI交互、依赖解决、系统修复）数据稀缺问题，现有方法难以大规模获取此类任务来增强智能体能力

Method: 基于Dockerfile与智能体任务的类比，提出CLI-Gym方法：让智能体模拟探索环境历史，通过执行反馈指导，从健康环境状态回溯到早期故障状态，将错误状态和对应错误信息打包生成任务

Result: 生成了1655个环境密集型任务，是目前同类任务的最大集合；基于精选的成功轨迹微调的LiberCoder模型在Terminal-Bench上取得+21.1%的绝对提升，达到46.1%的性能，超越多个强基线

Conclusion: 提出了首个公开的可扩展环境密集型任务生成管道CLI-Gym，有效解决了此类任务数据稀缺问题，显著提升了智能体在环境交互任务上的能力
